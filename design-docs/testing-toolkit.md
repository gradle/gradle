...
* The plugin automatically creates a new task for generating the manifest file named `createClasspathManifest`.
    * Create implementation as custom task.
    * The classpath generated by the task is stored in a plain text file. Each classpath entry is written line-by-line.
    * The task defines inputs and outputs for the task for supporting incremental build functionality. Make input/output fields final and do not expose setter methods
    to prevent users from reconfiguring assigned default values.
    * The task uses the value of `sourceSets.main.runtimeClasspath` to derive the classpath.
    * The output file for the generated classpath is `$buildDir/testkit/test/plugin-classpath.txt`. The task automatically creates the output directory if it doesn't exist yet.
    * A task dependency is established from the `test` task to the `createClasspathManifest` task.
...
* A user can manually execute the task for generating classpath by executing the `createClasspathManifest` task.
* The `createClasspathManifest` task is executed before the `test` task is executed.
* Default values for inputs/outputs of the task `createClasspathManifest` are used.
* The generated classpath file contains the expected entries.
* Inputs and outputs of the `createClasspathManifest` task cannot be reconfigured.
...
* Where should the bulk of documentation live - in the TestKit guide or the one for the plugin-dev-plugin?
