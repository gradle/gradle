...
The extension is defined with the following properties:
    ...
    public class JavaGradlePluginExtension {
        public void functionalTestClasspath(Closure config) {
            // delegate configuration to FunctionalTestClasspath.class
        }
    }
    ...
    project.getExtensions().create("javaGradlePlugin", JavaGradlePluginExtension.class);
...
* A user can only configure the input of the `generatePluginClasspathManifest` task but not its output.
