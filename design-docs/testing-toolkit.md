...
* The plugin automatically creates a new task for generating the manifest file named `generatePluginClasspathManifest`.
    ...
    * The classpath generated by the task is stored in a plain text file. Each classpath entry is written line-by-line. The file separator used for the entries is "/" for all
    operating systems.
    * The task defines inputs and outputs for the task for supporting incremental build functionality. For now do not let the end user re-configure them.
    ...
    * It's the responsibility of the end user to declare the dependency on `gradleTestKit()` for a compile configuration.
    * At runtime check if any of the source sets declares the dependency on `gradleTestKit()`. If it is found, automatically assign the task `generatePluginClasspathManifest`
    to the source set's runtime configuration via `dependencies.<runtime-configuration> files(generatePluginClasspathManifest)`.
* A new method `GradleRunner.withCodeUnderTestOnClasspath()` will be introduced. When invoked, the method will read the `plugin-classpath.txt` file, construct the classpath and
use it for test execution.
...
Enhance the the existing API of `GradleRunner`:

    package org.gradle.testkit.runner;

    public abstract class GradleRunner {
        public abstract GradleRunner withCodeUnderTestOnClasspath();
    }

### Test Coverage

* By applying this plugin sensible default values are set for plugin classpath generation.
* Classpath injection is only set up if at least one of the source sets applies the TestKit dependency.
* A user can manually execute the task for generating classpath by executing the `generatePluginClasspathManifest` task.
* The `generatePluginClasspathManifest` task is executed before the `Test` task is executed that corresponds to the source set declaring the dependency on the TestKit API.
...
* A user cannot reconfigure inputs and outputs of the `generatePluginClasspathManifest` task.
* The generated classpath file is read when invoking `GradleRunner.withCodeUnderTestOnClasspath()`.
    * An exception is thrown, if the file does not exist.
    * An exception is thrown, if the file cannot be parsed or the classpath cannot be constructed from its contents.
* If the user calls the method `GradleRunner.withPluginClasspath(Iterable<? extends File> classpath)` and `GradleRunner.withCodeUnderTestOnClasspath()` for the same `GradleRunner`
instance, the classpath set by the last method invocation wins.
