...
This story separates the file store and metadata cache versions so that they can evolve separately.
...
* `$gradleHome/caches/modules-${l}/metadata-${l}.${m}` - this is the metadata directory, includes the artifact, dependency and module metadata caches.
  When the metadata format changes, the version `${m}` is changed.

Initial values: `l` = 1, `f` = 1, `m` = 27.

1. Change `CacheLockingManager.createCache()` to accept a relative path name instead of a `File`. This path should be resolved relative to the metadata directory.
2. Add methods to `CacheLockingManager` to create the file store and module metadata `PathKeyFileStore` implementations.
