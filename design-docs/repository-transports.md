...
To use SFTP, SSH or any other protocol, users currently need to revert to using a (deprecated) custom Ivy resolver.
...
* Publish to and resolve from a repository via ssh/scp, supplying user/password credentials
* Publish to and resolve from a sftp/ssh repository, using public key authentication
...
## User/password credentials can be supplied via the userinfo portion of the URL

## Use public key authentication when accessing sftp repository

## Support 'ssh' scheme for ivy and maven repository URL
