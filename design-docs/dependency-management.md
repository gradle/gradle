...
## Story: Restructure Gradle cache layout so that artifacts are not copied on meta-data schema change
...
* `$gradleHome/caches/modules-1/` - this is the cache base directory, which includes the lock. When the locking protocol changes, this directory name is changed.
* `$gradleHome/caches/modules-1/files-1` - this is the file store directory. When the file store layout changes, this directory name is changed.
* `$gradleHome/caches/modules-1/metadata-27` - this is the meta-data directory, includes the artifact, dependency and module meta-data caches. When the meta-data format
  changes, this directory name is changed.
