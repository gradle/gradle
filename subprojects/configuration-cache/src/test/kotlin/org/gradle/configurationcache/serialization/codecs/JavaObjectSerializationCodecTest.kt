...
class JavaObjectSerializationCodecTest : AbstractUserTypeCodecTest() {
    ...
    fun `can handle writeReplace readResolve`() {
        verifyRoundtripOf(JavaObjectSerializationCodecTest::SerializableWriteReplaceBean) {
            assertThat(
                it.value,
                equalTo<Any>("42")
            )
    ...
    fun `can handle Externalizable beans`() {
        verifyRoundtripOf(JavaObjectSerializationCodecTest::ExternalizableBean) {
            assertThat(
                it.value,
                equalTo<Any>(42)
            )
    ...
    fun `can handle multiple writeObject implementations in the hierarchy`() {
        verifyRoundtripOf(JavaObjectSerializationCodecTest::MultiWriteObjectBean) { bean ->
            assertThat(
                bean.stringValue,
                equalTo("42")
            )
    ...
    fun `can handle writeObject without readObject`() {
        verifyRoundtripOf(JavaObjectSerializationCodecTest::SerializableWriteObjectOnlyBean) {
            assertThat(
                it.value,
                equalTo<Any>("42")
            )
