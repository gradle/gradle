...
import org.gradle.api.internal.provider.ValueSupplier.ExecutionTimeValueWithSideEffect
    ...
    suspend fun WriteContext.encodeValue(value: ValueSupplier.ExecutionTimeValue<*>) {
        when {
            ...
            value.isFixedValue -> {
                // Can serialize a fixed value and discard the provider
                // TODO - should preserve information about the source, for diagnostics at execution time
                if (value is ExecutionTimeValueWithSideEffect<*>) {
                    writeByte(2)
                    write(value.fixedValue)
                    write(value.sideEffect)
                } else {
                    writeByte(3)
                    write(value.fixedValue)
                }
    ...
    suspend fun ReadContext.decodeValue(): ValueSupplier.ExecutionTimeValue<*> =
        when (readByte()) {
            ...
            2.toByte() -> {
                ...
                ValueSupplier.ExecutionTimeValue.ofNullable(value).withSideEffect(read()!!.uncheckedCast())
            }
            3.toByte() -> ValueSupplier.ExecutionTimeValue.ofNullable(read()) // nullable because serialization may replace value with null, eg when using provider of Task
