...
class CppExtensionSmokeSpec extends CppProjectSpec {
    ...
    def "extensions are available"() {
        ...
        executables instanceof NamedDomainObjectContainer
        libraries instanceof NamedDomainObjectContainer
    }
    ...
    def "can create some cpp source sets"() {
        ...
        then:
        cpp.sourceSets.size() == 2
        cpp.sourceSets*.name == ["s1", "s2"]
    ...
    def "can create some binaries"() {
        ...
        then:
        executables.size() == 2
        executables*.name == ["e1", "e2"]
        libraries.size() == 2
        libraries*.name == ["l1", "l2"]
    }
