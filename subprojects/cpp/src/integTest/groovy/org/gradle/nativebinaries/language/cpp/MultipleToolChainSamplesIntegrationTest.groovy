...
class MultipleToolChainSamplesIntegrationTest extends AbstractIntegrationSpec {
    ...
    def toolChains() {
        def gcc = AvailableToolChains.getToolChain('gcc 4')
        def clang = AvailableToolChains.getToolChain('clang')
        def visualCpp = AvailableToolChains.getToolChain('visual c++')

        given:
        sample toolChains

        when:
        run "buildAllExecutables"

        then:
        checkToolChainBuild(gcc, 'gcc', 'GCC')
        checkToolChainBuild(clang, 'clang', 'Clang')
        checkToolChainBuild(visualCpp, 'visualCpp', 'Visual C++')
    }
