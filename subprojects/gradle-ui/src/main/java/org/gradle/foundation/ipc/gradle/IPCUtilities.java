...
/**
 Just some convenience functions to startup a GradleClient. See GradleClient
...
public class IPCUtilities {
    ...
    /**
       ...
       expects the port number to set as a system property. Note: this is using
       gradle to find the port. See getPort().
    ...
    */
    ...
    /**
       This gets the port out of the start parameters. Why? Because this is
       meant to be run from the init script and the system properties haven't
       been set yet. That is due to how gradle is run from the bat file/shell
       script. It has to manually set the java system properties (-D). I don't
    ...
    */
    ...
    /**
     * This starts a gradle client that sends a task list back to the server. It
     * expects the port number to set as a system property. You probably should
     * be executing the "-t" command. Note: this is using gradle to find the
     * port. See getPort().
     *
