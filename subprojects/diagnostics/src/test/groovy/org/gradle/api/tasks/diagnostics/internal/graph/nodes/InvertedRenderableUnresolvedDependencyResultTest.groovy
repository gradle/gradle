 ...
 */
class InvertedRenderableUnresolvedDependencyResultTest extends Specification {
    def "renders name"() {
        given:
    ...
    private InvertedRenderableUnresolvedDependencyResult dep(ModuleVersionSelector requested, ModuleVersionSelector selected) {
        UnresolvedDependencyResult dependencyResult = Stub() {
            getRequested() >> requested
            getAttempted() >> selected
        }
        return new InvertedRenderableUnresolvedDependencyResult(dependencyResult, null)
    }
}
