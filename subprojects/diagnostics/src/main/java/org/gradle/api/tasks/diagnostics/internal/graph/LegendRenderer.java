...
public class LegendRenderer {
    ...
    public void printLegend() {
        if (hasConstraints) {
            output.println();
            output.withStyle(Info).text("(c) - This element is a dependency constraint, not a dependency. Look for the matching dependency elsewhere in the tree.");
        }
        if (hasCyclicDependencies) {
            output.println();
            output.withStyle(Info).println("(*) - Repeat occurrence of a transitive dependency subtree. Gradle expands transitive dependency subtrees only once per project; repeat occurrences only display the root of the subtree, followed by this annotation.");
        }
