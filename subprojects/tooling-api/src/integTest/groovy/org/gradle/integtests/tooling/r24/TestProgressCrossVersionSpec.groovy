...
class TestProgressCrossVersionSpec extends ToolingApiSpecification {
    ...
    @ToolingApiVersion(">=2.4")
    @TargetGradleVersion(">=2.4")
    def "receive test success events in real time"() {
        given:
        buildFile << """
            apply plugin: 'java'
            repositories { mavenCentral() }
        ...
        file("src/test/java/example/MyTest.java") << """
            ...
            public class MyTest {
                @org.junit.Test public void foo() throws Exception {
                     Thread.sleep(200); // give some time for the test to execute
                     org.junit.Assert.assertEquals(1, 1);
                }
        ...
        when:
        def eventTimes = []
        def eventCountAfterAsyncRun = Integer.MAX_VALUE
        def eventCountAfterAsyncRunCompleted = Integer.MAX_VALUE
        def lastEventTime = System.currentTimeMillis()

        withConnection {
            ProjectConnection connection ->
                connection.newBuild().forTasks('test').addTestProgressListener { ProgressEvent event ->
                    if (event instanceof SuccessEvent
                            && event.descriptor instanceof JvmTestOperationDescriptor
                            && event.descriptor.methodName in ['foo','bar']
                    ) {
                    ...
                    void onComplete(Void result) {
                        eventCountAfterAsyncRunCompleted = eventTimes.size()
                    }
                ...
                eventCountAfterAsyncRun = eventTimes.size()
        }
        then:
        eventCountAfterAsyncRun == 0
        eventCountAfterAsyncRunCompleted == 2
        eventTimes.each {
            assert it-lastEventTime>=100 // we can't use 200 because of the granularity of System.currentTimeMillis
            lastEventTime = it
        }
    }
    ...
    @ToolingApiVersion(">=2.4")
    @TargetGradleVersion(">=2.4")
    def "receive test failure events in real time"() {
        given:
        buildFile << """
            apply plugin: 'java'
            repositories { mavenCentral() }
