...
class BuildProgressCrossVersionSpec extends ToolingApiSpecification {
    ...
    def "receive current build progress event even if one of multiple build listeners throws an exception"() {
        ...
        withConnection {
            ProjectConnection connection ->
                ...
                }, EnumSet.of(ProgressEventType.GENERIC)).addProgressListener(new ProgressListener() {
                    @Override
                    void statusChanged(ProgressEvent event) {
                        throw new IllegalStateException("Throwing an exception on purpose")
                    }
    ...
    def "receive all possible build progress events types for successful run"() {
        ...
        OperationDescriptor buildDescriptor
        OperationDescriptor configDescriptor

        when:
        withConnection {
            ProjectConnection connection ->
                connection.newBuild().forTasks('test').addProgressListener(listener, EnumSet.of(ProgressEventType.GENERIC)).run()
        }
    ...
    def "should receive build events from GradleBuild"() {
        ...
        withConnection {
            ProjectConnection connection ->
                connection.newBuild().forTasks('innerBuild').addProgressListener(listener, EnumSet.of(ProgressEventType.GENERIC)).run()
        }

        then:
        2 * listener.statusChanged(_ as StartEvent)
        6 * listener.statusChanged(_ as StartEvent) // settings loaded, projects loaded, projects evaluated
        2 * listener.statusChanged(_ as FinishEvent) >> { DefaultFinishEvent event ->
            def result = event.result
            assert result instanceof SuccessResult
        }
    }
