...
public class PhasedActionAwareConsumerConnection extends ParameterAcceptingConsumerConnection {
    ...
    public void run(PhasedBuildAction phasedBuildAction, ConsumerOperationParameters operationParameters) {
        ...
        PhasedActionResultListener listener = new DefaultPhasedActionResultListener(getHandler(phasedBuildAction.getAfterLoadingAction()),
            getHandler(phasedBuildAction.getAfterConfigurationAction()), getHandler(phasedBuildAction.getAfterBuildAction()));
    ...
    @Nullable
    private static <T> ResultHandler<? super T> getHandler(@Nullable PhasedBuildAction.BuildActionWrapper<T> wrapper) {
    ...
    private static InternalPhasedAction getPhasedAction(PhasedBuildAction phasedBuildAction, File rootDir, VersionDetails versionDetails) {
        return new InternalPhasedActionAdapter(getAction(phasedBuildAction.getAfterLoadingAction(), rootDir, versionDetails),
            getAction(phasedBuildAction.getAfterConfigurationAction(), rootDir, versionDetails), getAction(phasedBuildAction.getAfterBuildAction(), rootDir, versionDetails));
    }
