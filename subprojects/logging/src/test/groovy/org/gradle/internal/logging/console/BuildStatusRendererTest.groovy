...
class BuildStatusRendererTest extends OutputSpecification {
    ...
    def startRootBuildOperation(Long id) {
        new ProgressStartEvent(new OperationIdentifier(id), null, currentTimeMs, "category", "description", null, null, null, 0, true, new OperationIdentifier(id), null, BuildOperationCategory.UNCATEGORIZED)
    }
    ...
    def startExecuteRootBuild(Long id, Long parentId, int totalProgress) {
        return start(id, parentId, BuildOperationCategory.RUN_TASKS_ROOT_BUILD, totalProgress)
    }

    def startExecuteBuild(Long id, Long parentId, int totalProgress) {
        return start(id, parentId, BuildOperationCategory.RUN_TASKS, totalProgress)
    }
    ...
    def start(Long id, Long parentId, BuildOperationCategory category, int totalProgress = 0) {
        new ProgressStartEvent(new OperationIdentifier(id), new OperationIdentifier(parentId), currentTimeMs, "category", "description", null, null, null, totalProgress, true, new OperationIdentifier(id), new OperationIdentifier(parentId), category)
    }
