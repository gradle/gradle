...
class GradleVsMavenBuildResultsStoreTest extends ResultSpecification {
    @Rule
    ...
    def "persists results"() {
        given:
        def results1 = gradleVsMavenBuildResults(testId: "test1", testGroup: "group1")
        ...
        and:
        def results2 = gradleVsMavenBuildResults(testId: "test2", testGroup: "group2")
        results2.buildResult(new BuildDisplayInfo("simple", "simple display", ["build"], ["-i"], [], true))

        when:
        def writeStore = new GradleVsMavenBuildResultsStore(dbFile)
        ...
        when:
        def readStore = new GradleVsMavenBuildResultsStore(dbFile)
    ...
    def "scenario settings can change over time"() {
        given:
        def results1 = gradleVsMavenBuildResults(testId: "test1", testGroup: "group1", testTime: 100)
        def buildResults1 = results1.buildResult(
                new BuildDisplayInfo(
                        "project-1",
                        "scenario 1",
        ...
        def results2 = gradleVsMavenBuildResults(testId: "test1", testGroup: "group1", testTime: 200)
        ...
        def results3 = gradleVsMavenBuildResults(testId: "test1", testGroup: "group1", testTime: 300)
    ...
    def "returns top n results in descending date order"() {
        given:
        def results1 = gradleVsMavenBuildResults(testId: "test1", testTime: 1000)
        results1.buildResult(new BuildDisplayInfo("simple1", "simple 1", ["build"], ["-i"], [], true))

        and:
        def results2 = gradleVsMavenBuildResults(testId: "test1", testTime: 2000)
        results2.buildResult(new BuildDisplayInfo("simple2", "simple 2", ["build"], ["-i"], [], true))

        and:
        def results3 = gradleVsMavenBuildResults(testId: "test1", testTime: 3000)
