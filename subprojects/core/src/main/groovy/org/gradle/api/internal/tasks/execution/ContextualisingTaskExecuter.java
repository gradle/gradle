...
/**
 * Adapts a TaskExecuter to a ContextualTaskExecuter.
 */
public class ContextualisingTaskExecuter implements TaskExecuter {
    private final ContextualTaskExecuter delegate;

    public ContextualisingTaskExecuter(ContextualTaskExecuter delegate) {
        this.delegate = delegate;
    }

    public void execute(TaskInternal task, TaskStateInternal state) {
        delegate.execute(task, state, new DefaultTaskExecutionContext());
    }
