...
class ModuleNameDeduper {
    ...
    void dedupe(Collection<Deduplicable> deduplicables) {
        def allNames = []
        deduplicables.each { deduplicable ->
            def name = deduplicable.candidateNames.find { !allNames.contains(it) }
            if (name) {
                allNames << name
                deduplicable.moduleNameSetter.call(name)
            }
        }
    }
}