...
public class DefaultWorkValidationWarningRecorder
        implements ValidateStep.ValidationWarningRecorder, WorkValidationWarningReporter {
    private static final Logger LOGGER = LoggerFactory.getLogger(DefaultWorkValidationWarningRecorder.class);

    // TODO If we can ensure that the recorder is only called once per work execution, then we can demote this to a
    // simple counter
    private final Set<Identity> workWithWarnings = ConcurrentHashMap.newKeySet();

    @Override
    public void recordValidationWarnings(
        ...
        String uniqueWarnings = warnings.stream()
                ...
                .collect(joining());
        LOGGER.warn(
                "Execution optimizations have been disabled for {} to ensure correctness due to the following reasons:{}",
                work.getDisplayName(),
                uniqueWarnings);

        // We are logging all the warnings that we encountered during validation here
        warnings.forEach(warning -> withDocumentation(
                        warning,
                        deprecateBehaviour(convertToSingleLine(renderMinimalInformationAbout(warning, false, false)))
