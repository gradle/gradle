...
public class LocalTaskNode extends TaskNode {
    ...
    private Set<Provider<? extends BuildService<?>>> adHocServicesConsumed;
    ...
    private void addServiceReferences(Collection<Object> servicesConsumed) {
        adHocServicesConsumed = servicesConsumed.stream().map(it -> (DefaultProperty<BuildService<?>>) ((PropertyValue) it).call()).map(it -> it.getProvider()).map(it -> (Provider<? extends BuildService<?>>) it).collect(Collectors.toSet());
    }
    ...
    private List<ResourceLock> findSharedResourcesUsed() {
        ...
        return buildServiceRegistry.getSharedResources(adHocServicesConsumed);
    }
