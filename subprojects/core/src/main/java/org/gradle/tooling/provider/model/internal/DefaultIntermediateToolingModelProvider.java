...
public class DefaultIntermediateToolingModelProvider implements IntermediateToolingModelProvider {
    ...
    public <T> List<T> getModels(
            ProjectState requester,
            List<ProjectState> targets,
            String modelName,
            Class<T> modelType,
    ...
    @Override
    public <P extends Plugin<Project>> void applyPlugin(
            ProjectState requester, List<ProjectState> targets, Class<P> pluginClass) {
        List<Object> rawModels = fetchModels(
                requester, targets, PluginApplyingBuilder.MODEL_NAME, createPluginApplyingParameter(pluginClass));
        ensureModelTypes(Boolean.class, rawModels);
    }

    private static <P extends Plugin<Project>> PluginApplyingParameter createPluginApplyingParameter(
            Class<P> pluginClass) {
        return () -> pluginClass;
    }

    private List<Object> fetchModels(
            ProjectState requester, List<ProjectState> targets, String modelName, @Nullable Object parameter) {
        reportToolingModelDependencies(requester, targets);
        BuildState buildState = extractSingleBuildState(targets);
        ToolingModelParameterCarrier carrier =
                parameter == null ? null : parameterCarrierFactory.createCarrier(parameter);
    ...
    private List<Object> getModels(
            ...
            List<ProjectState> targets,
            String modelName,
            @Nullable ToolingModelParameterCarrier parameter) {
        List<Supplier<Object>> fetchActions = targets.stream()
                .map(targetProject ->
                        (Supplier<Object>) () -> fetchModel(modelName, controller, targetProject, parameter))
                .collect(toList());
    ...
    @Nullable
    private static Object fetchModel(
            String modelName,
            BuildToolingModelController controller,
            ProjectState builderTarget,
            @Nullable ToolingModelParameterCarrier parameter) {
        ...
        for (ProjectState target : targets) {
            if (result != target.getOwner()) {
                throw new IllegalArgumentException(String.format(
                        "Expected target projects to share the same build state. Found at least two: '%s' and '%s'",
        ...
        for (Object rawModel : rawModels) {
            if (rawModel == null) {
                throw new IllegalStateException(
                        String.format("Expected model of type %s but found null", implementationType.getName()));
            }
            if (!implementationType.isInstance(rawModel)) {
                throw new IllegalStateException(String.format(
                        "Expected model of type %s but found %s",
                        implementationType.getName(), rawModel.getClass().getName()));
            }
        }
