...
public class DefaultClasspathHasher implements ClasspathHasher {
    ...
    public HashCode hash(ClassPath classpath) {
        ...
        TaskCacheKeyBuilder keyBuilder = new DefaultTaskCacheKeyBuilder();
        snapshot.appendToCacheKey(keyBuilder);
        return HashCode.fromString(keyBuilder.build().getHashCode());
    }
}
