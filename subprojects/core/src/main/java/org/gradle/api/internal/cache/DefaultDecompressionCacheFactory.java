...
/**
 * Implements a factory to create a {@link DecompressionCache} using a given {@link CacheBuilderFactory}.
 *
...
public class DefaultDecompressionCacheFactory implements DecompressionCacheFactory, Closeable {
    ...
    private final Supplier<? extends ScopedCacheBuilderFactory> cacheBuilderFactorSupplier;
    private volatile DecompressionCache cache;

    public DefaultDecompressionCacheFactory(Supplier<? extends ScopedCacheBuilderFactory> cacheBuilderFactorSupplier) {
        this.cacheBuilderFactorSupplier = cacheBuilderFactorSupplier;
    }
    ...
    public DecompressionCache create() {
        if (cache == null) {
            synchronized (this) {
                if (cache == null) {
                    cache = new DefaultDecompressionCache(cacheBuilderFactorSupplier.get());
                }
            }
        }
        return cache;
