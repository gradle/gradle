...
/**
 * Validates and performs lookups of the services that Gradle exposes to build, settings and init
 * scripts via {@code Script.service(Class)} and the {@code service()} members of the Kotlin DSL
 * script templates, and to tasks via {@code Task.service(Class)}.
 *
...
public final class PublicServiceLookups {
    ...
    /**
     * The kind of script a lookup was made from, or a task, determining which services are available
     ...
     * <ul>
     * <li>It must be safe to use with Isolated Projects from every scope it is listed for, including the
     * instance registered in that scope's registry (see the audit in https://github.com/gradle/gradle/issues/38028).
     * Nothing listed here should ever need to be restricted again for Isolated Projects.</li>
     * <li>Whatever is available to init scripts must also be available to project build scripts and settings scripts:
     * a script applied with {@code apply from: ..., to: gradle} targets the {@code Gradle} instance and must not
     * see more than the script that applied it.</li>
     * </ul>
     */
     ...
     *
     * <p>The marker interfaces ({@code ProjectService}, {@code TaskService}, {@code SettingsService}, {@code GradleService}) are only a compile-time
     ...
     * @param entryPoint the kind of script the lookup was made from; supplied by Gradle, never {@code null}
    ...
    private static String wrongScopeMessage(Class<?> serviceType, EntryPoint entryPoint, Set<EntryPoint> availableIn) {
        return String.format(
            ...
            availableIn.stream().map(EntryPoint::getDisplayName).collect(RenderingUtils.oxfordJoin("and")));
    }
