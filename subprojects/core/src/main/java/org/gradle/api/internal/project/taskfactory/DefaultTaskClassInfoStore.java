...
public class DefaultTaskClassInfoStore implements TaskClassInfoStore {
    ...
    private TaskClassInfo createTaskClassInfo(Class<? extends Task> type) {
        ...
        Optional<String> reasonNotToTrackState = typeMetadata
                .getTypeAnnotationMetadata()
                .getAnnotation(UntrackedTask.class)
        ...
        typeMetadata.getFunctionMetadata().stream()
                .filter(functionMetadata ->
                        functionMetadata.getAnnotation(TaskAction.class).isPresent())
                .forEach(functionMetadata -> {
                    ...
                    if (alreadySeen != null) {
                        throw new GradleException(String.format(
                                ...
                                typeMetadata.getType().getSimpleName(),
                                functionMetadata.getMethod().getName()));
                    }

                    // TODO These validations should be done as validation in TaskActionAnnotationHandler and surfaced
                    // as problems
                    Class<?> declaringClass = functionMetadata.getMethod().getDeclaringClass();
                    final Class<?>[] parameterTypes =
                            functionMetadata.getMethod().getParameterTypes();
        ...
        if (functions.values().stream()
                        .filter(functionMetadata -> functionMetadata.getMethod().getParameterTypes().length > 0)
                        .count()
                > 1) {
            throw new GradleException(String.format(
                    "Cannot have multiple @TaskAction methods accepting an %s parameter.",
