...
public abstract class AbstractExecTask<T extends AbstractExecTask> extends ConventionTask implements ExecSpec {
    ...
    @Input
    @Override
    @ToBeReplacedByLazyProperty(skipForReport = true, comment = "Skipped for reporting since setter is using generics")
    ...
    @Internal
    @Override
    @ToBeReplacedByLazyProperty(skipForReport = true)
    ...
    @Internal
    @Override
    @ToBeReplacedByLazyProperty(comment = "Should this be lazy? Probably not because it's a stream", skipForReport = true)
