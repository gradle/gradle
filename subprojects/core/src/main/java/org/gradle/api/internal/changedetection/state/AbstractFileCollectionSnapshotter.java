...
public abstract class AbstractFileCollectionSnapshotter implements FileCollectionSnapshotter {
    ...
    public AbstractFileCollectionSnapshotter(StringInterner stringInterner, FileSystem fileSystem, DirectoryFileTreeFactory directoryFileTreeFactory, FileSystemMirror fileSystemMirror) {
    ...
    @Override
    public FileCollectionSnapshot snapshot(FileCollection input, TaskFilePropertyCompareStrategy compareStrategy, final SnapshotNormalizationStrategy snapshotNormalizationStrategy, final TaskExecution current) {
        final List<FileDetails> fileTreeElements = Lists.newLinkedList();
        FileCollectionInternal fileCollection = (FileCollectionInternal) input;
        FileCollectionVisitorImpl visitor = new FileCollectionVisitorImpl(fileTreeElements, current);
    ...
    private FileHashSnapshot fileSnapshot(FileVisitDetails fileDetails, TaskExecution current) {
        return new FileHashSnapshot(doHash(fileDetails, current), fileDetails.getLastModified());
    }
    ...
    protected abstract HashCode doHash(FileVisitDetails fileDetails, TaskExecution current);
    ...
    private class FileCollectionVisitorImpl implements FileCollectionVisitor {
        ...
        FileCollectionVisitorImpl(List<FileDetails> fileTreeElements, TaskExecution current) {
        ...
        private DefaultFileDetails calculateDetails(File file) {
            ...
            switch (stat.getType()) {
                ...
                case RegularFile:
                    DefaultFileVisitDetails fileDetails = new DefaultFileVisitDetails(file, new RelativePath(true, file.getName()), new AtomicBoolean(), fileSystem, fileSystem, false, stat.getLastModified(), stat.getLength());
                    return new DefaultFileDetails(path, new RelativePath(true, file.getName()), RegularFile, true, fileSnapshot(fileDetails, current));
                default:
        ...
        public void visitTree(FileTreeInternal fileTree) {
            ...
            fileTree.visitTreeOrBackingFile(new FileVisitorImpl(elements, current));
        ...
        public void visitDirectoryTree(DirectoryFileTree directoryTree) {
            ...
            if (!directoryTree.getPatterns().isEmpty()) {
                ...
                directoryTree.visit(new FileVisitorImpl(elements, current));
            } else {
    ...
    private class FileVisitorImpl implements FileVisitor {
        ...
        FileVisitorImpl(List<FileDetails> fileTreeElements, TaskExecution current) {
            this.fileTreeElements = fileTreeElements;
            this.current = current;
        }
        ...
        public void visitFile(FileVisitDetails fileDetails) {
            fileTreeElements.add(new DefaultFileDetails(getPath(fileDetails.getFile()), fileDetails.getRelativePath(), RegularFile, false, fileSnapshot(fileDetails, current)));
        }
    }
}
