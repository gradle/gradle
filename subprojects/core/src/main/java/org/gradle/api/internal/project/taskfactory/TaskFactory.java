...
public class TaskFactory implements ITaskFactory {
    ...
    public <S extends Task> S create(final TaskIdentity<S> identity, @Nullable final Object[] constructorArgs) {
        ...
        } else if (identity.getTaskType() == org.gradle.api.internal.AbstractTask.class
                || identity.getTaskType() == TaskInternal.class) {
            throw new InvalidUserDataException(String.format(
        ...
        Describable displayName =
                Describables.withTypeAndName("task", identity.getBuildTreePath().asString());
            ...
            public S call() {
                try {
                    ...
                    if (constructorArgs != null) {
                        instance = instantiationScheme
                                .instantiator()
                                .newInstanceWithDisplayName(implType, displayName, constructorArgs);
                    } else {
                        instance = instantiationScheme
                                .deserializationInstantiator()
                                .newInstance(implType, org.gradle.api.internal.AbstractTask.class);
                    }
                    return identity.getTaskType().cast(instance);
                } catch (ObjectInstantiationException e) {
                    throw new TaskInstantiationException(
                            String.format(
                                    "Could not create task of type '%s'.",
                                    identity.getTaskType().getSimpleName()),
                            e.getCause());
                }
            }
