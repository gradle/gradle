...
public class InputFilesTaskStateChanges extends AbstractNamedFileSnapshotTaskStateChanges {
    public InputFilesTaskStateChanges(@Nullable TaskExecution previous, TaskExecution current, TaskInternal task, FileSystemSnapshotter fileSystemSnapshotter) {
        super(task.getName(), previous, current, "Input", task.getInputs().getFileProperties(), (SnapshottingConfigurationInternal) task.getProject().getSnapshotting(), fileSystemSnapshotter);
