...
public class CopySpecActionImpl implements Action<CopySpecResolver> {
    ...
    public void execute(final CopySpecResolver specResolver) {
        ...
        Property<LinksStrategy> linksStrategyProperty = specResolver.getLinksStrategy();
        FileVisitor visitor = new CopyFileVisitorImpl(specResolver, action, instantiator, objectFactory, fileSystem, reproducibleFileOrder);
        if (linksStrategyProperty.isPresent()) {
            source.visit(visitor, linksStrategyProperty.get());
        } else {
