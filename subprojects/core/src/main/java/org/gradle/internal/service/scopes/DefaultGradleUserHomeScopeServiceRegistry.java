...
public class DefaultGradleUserHomeScopeServiceRegistry implements GradleUserHomeScopeServiceRegistry, Closeable {
    ...
    public void release(ServiceRegistry registry) {
        lock.lock();
        try {
            release(servicesForHomeDir.entrySet().stream().filter(entry -> isReleaseCandidate(registry, entry.getValue())).findFirst());
        } finally {
            lock.unlock();
        }
    }

    private boolean isReleaseCandidate(ServiceRegistry registry, Services services) {
        return services.registry == registry
            && --services.count == 0
            && (servicesForHomeDir.size() > 1 || getProperty(REUSE_USER_HOME_SERVICES, "true").equals("false"));
    }

    private void release(Optional<Map.Entry<File, Services>> release) {
        release.ifPresent(entry -> {
                stoppable(entry.getValue().registry).stop(); // Other home dir in use, close these. Otherwise, keep the services for next time.
