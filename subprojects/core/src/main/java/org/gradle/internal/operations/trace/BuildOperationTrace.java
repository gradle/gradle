 ...
 * Can be enabled for any build with {@code -Dorg.gradle.operations.trace=«path»}.
 ...
 *
 * Later, this will change to emit a JSON chrome-trace file that can be visualised with Google Chrome.
 *
 * @since 4.0
 */
public class BuildOperationTrace implements Stoppable {
    ...
    public static Map<Object, BuildOperationRecord> read(InputStream input) {
        try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(input, "UTF-8"));

            JsonSlurper slurper = new JsonSlurper();
            Map<Object, SerializedOperationStart> pending = new HashMap<Object, SerializedOperationStart>();
            Map<Object, BuildOperationRecord> operations = new HashMap<Object, BuildOperationRecord>();
            ...
            while (line != null) {
                ...
                if (map.containsKey("startTime")) {
                    ...
                    BuildOperationRecord previous = operations.put(start.id, new BuildOperationRecord(
                        start.id,
                        start.parentId,
                        start.displayName,
                        start.startTime,
