    ...
    public void add(final T operation) {
        ...
        try {
            ...
            if (workerCount == 0 || workerCount < workerLeases.getMaxWorkerCount() - 1) {
                // `getMaxWorkerCount() - 1` because main thread executes work as well. See
                // https://github.com/gradle/gradle/issues/3273
                // TODO This could be more efficient, so that we only start a worker when there are none idle _and_
                // there is a worker lease available
    ...
    private class WorkerRunnable implements Runnable {
            ...
            completeOperations(
                    ...
                    workerLeases.runAsWorkerThread(() -> {
                        if (allowAccessToProjectState) {
                            ...
                            // Disallow this thread from making any changes to the project locks while it is running the
                            // work. This implies that this thread will not
                            // block waiting for access to some other project, which means it can proceed even if some
                            // other thread is waiting for a project lock it
                            // holds without causing a deadlock. This in turn implies that this thread does not need to
                            // release the project locks it holds while
                            // blocking waiting for an operation to complete and does not need to deal with another
                            // thread stealing its project lock(s) while blocking.
                            //
                            // Eventually, this should become the default and only behaviour for all worker threads and
                            // changes to locks made only when starting or
                            // finishing an execution node. Adding this constraint here means that we can make all build
                            // operation queue workers compliant with this
                            // constraint and then gradually roll this out to other worker threads, such as task action
                            // workers.
                            //
        ...
        private int doRunBatch(T firstOperation) {
            ...
            while (operation != null) {
                if (queueState == QueueState.Cancelled) {
                    // If an operation was pulled from the queue, but the queue was cancelled before this operation
                    // could start
