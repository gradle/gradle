...
public class DefaultBuildTreeModelCreator implements BuildTreeModelCreator {
    ...
    private class DefaultBuildTreeModelController implements BuildTreeModelController {
        ...
        public Object getModel(BuildTreeModelTarget target, String modelName, @Nullable Object parameter, boolean isFetch) throws UnknownModelException {
            ...
            return buildOperationRunner.call(new CallableBuildOperation<Object>() {
                ...
                public Object call(BuildOperationContext context) {
                    ToolingModelScope scope = locateBuilderForTarget(target, new ToolingModelContext(modelName, parameter != null, isFetch));
        ...
        private ToolingModelScope locateBuilderForTarget(BuildTreeModelTarget target, ToolingModelContext toolingModelContext) {
        ...
        private ToolingModelScope locateBuilderForDefaultTarget(ToolingModelContext toolingModelContext) {
            return locateBuilderForBuildTarget(defaultTarget, toolingModelContext);
        }

        private ToolingModelScope locateBuilderForProjectTarget(BuildTreeModelTarget.Project projectTarget, ToolingModelContext toolingModelContext) {
        ...
        private ToolingModelScope locateBuilderForBuildTarget(BuildTreeModelTarget.Build buildTarget, ToolingModelContext toolingModelContext) {
            BuildState build = findBuild(buildTarget.getBuildRootDir());
            return locateBuilderForBuildTarget(build, toolingModelContext);
        }

        private ToolingModelScope locateBuilderForBuildTarget(BuildState target, ToolingModelContext toolingModelContext) {
            return target.withToolingModels(toolingModelContext.isFetch(), controller -> controller.locateBuilderForTarget(toolingModelContext));
        }

        private ToolingModelScope locateBuilderForProjectTarget(ProjectState target, ToolingModelContext toolingModelContext) {
            return target.getOwner().withToolingModels(toolingModelContext.isFetch(), controller -> controller.locateBuilderForTarget(target, toolingModelContext));
        }
