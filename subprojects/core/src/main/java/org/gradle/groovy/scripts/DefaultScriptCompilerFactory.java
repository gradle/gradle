...
public class DefaultScriptCompilerFactory implements ScriptCompilerFactory {
    ...
    private class ScriptCompilerImpl implements ScriptCompiler {
        ...
        @Override
        public <T extends Script, M> ScriptRunner<T, M> compile(Object target, Class<T> scriptType, CompileOperation<M> extractingTransformer, ClassLoaderScope targetScope, Action<? super ClassNode> verifier) {
            CompiledScript<T, M> compiledScript = scriptClassCompiler.compile(target, source, targetScope, extractingTransformer, scriptType, verifier);
