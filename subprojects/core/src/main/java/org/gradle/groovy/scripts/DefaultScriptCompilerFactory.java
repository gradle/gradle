...
public class DefaultScriptCompilerFactory implements ScriptCompilerFactory {
    ...
    private class ScriptCompilerImpl implements ScriptCompiler {
        ...
        @Override
        public <T extends Script, M> ScriptRunner<T, M> compile(Class<T> scriptType, CompileOperation<M> extractingTransformer, ClassLoaderScope targetScope, Action<? super ClassNode> verifier, ScriptHandler scriptHandler) {
            CompiledScript<T, M> compiledScript = scriptClassCompiler.compile(source, targetScope, extractingTransformer, scriptType, verifier, scriptHandler);
