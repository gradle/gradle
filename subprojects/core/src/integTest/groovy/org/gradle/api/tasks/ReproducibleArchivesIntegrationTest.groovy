...
class ReproducibleArchivesIntegrationTest extends AbstractIntegrationSpec {
    ...
    @Unroll
    "#taskName preserves order of child specs"() {
        given:
        createTestFiles()
        buildFile << """
            task ${taskName}(type: ${taskType}) {
        ...
        then:
        getFilesInArchive(file("build/test.${fileExtension}")) == [
            'file11.txt',
            'file13.txt',
            'dir2/',
            'dir2/file21.txt',
    ...
    "#taskName can use zipTree and tarTree"() {
        ...
        then:
        // TODO wolfs: It would be nice for this to be sorted - it doesn't break reproducibility but still is unexpected
        getFilesInArchive(file("build/combined.${fileExtension}")) == [
            'file21.txt',
            'file22.txt',
            'file23.txt',
            'file24.txt',
    ...
    "#taskName sorts by target file name"() {
        ...
        then:
        def archiveFile = file("build/test.${fileExtension}")
        archive(archiveFile).content('test3.txt') == 'test2'
        archive(archiveFile).content('test4.txt') == 'test1'
        getFilesInArchive(archiveFile) == [
                'test3.txt',
                'test4.txt'
        ]
    ...
    List<String> getFilesInArchive(File archiveFile) {
        String type = FilenameUtils.getExtension(archiveFile.name)
        if (type == 'zip') {
            def file = new ZipFile(archiveFile)
            return file.withCloseable {
                file.entries()*.name
            }
        } else {
            return archiveFile.withInputStream {
                def tarInputStream = new TarInputStream(it)
                def entries = []
                for (TarEntry entry = tarInputStream.nextEntry; entry != null; entry = tarInputStream.nextEntry) {
                    entries += entry.name
                }
