...
class BuildCacheConfigurationIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "system properties still have an effect on pushing and pulling"() {
        ...
        succeeds("tasks")
        then:
        result.assertOutputContains "Using a local build cache (${file("local-cache")}) as local cache, push is enabled."
    ...
    def "does not populate the build cache when we cannot push to it"() {
        ...
        succeeds("customTask")
        then:
        result.assertOutputContains("Using a local build cache (${file("local-cache")}) as local cache, push is disabled")
        and:
