...
public class TarGzipPacker implements Packer {
    @Override
    public void pack(List<DataSource> inputs, DataTarget output) throws IOException {
        TarOutputStream tarOutput = new TarOutputStream(new GZIPOutputStream(output.openOutput()));
    ...
    public void unpack(DataSource input, DataTargetFactory targetFactory) throws IOException {
        TarInputStream tarInput = new TarInputStream(new GZIPInputStream(input.openInput()));
        while (true) {
