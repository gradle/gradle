...
class DefaultProjectDescriptorTest extends Specification {
    ...
    def "build file name is resolved to existing #buildFilename with script languages for #extensions"() {
        given:
        def descriptor = projectDescriptor(scriptLanguages(extensions))
    ...
    private ProjectDescriptor projectDescriptor(Iterable<ScriptingLanguage> scriptLanguages = []) {
        def parentDescriptor = new DefaultProjectDescriptor(null, "somename", new File("somefile"), descriptorRegistry, fileResolver, scriptLanguages)
        def descriptor = new DefaultProjectDescriptor(parentDescriptor, testName.methodName, tmpDir.testDirectory, descriptorRegistry, fileResolver, scriptLanguages)
    ...
    private Iterable<ScriptingLanguage> scriptLanguages(List<String> extensions) {
        extensions.collect { extension ->
            Mock(ScriptingLanguage) {
