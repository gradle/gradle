...
class DefaultDependencyHandlerTest extends Specification {
    ...
    void "creates test-kit dependency"() {
        Dependency dependency = Mock()

        when:
        def result = dependencyHandler.testKit()

        then:
        result == dependency

        and:
        1 * dependencyFactory.createDependency(DependencyFactory.ClassPathNotation.TEST_KIT) >> dependency
    }
