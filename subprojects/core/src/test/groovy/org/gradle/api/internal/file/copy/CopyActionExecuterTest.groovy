...
class CopyActionExecuterTest extends WorkspaceTest {
    ...
    def "correctly executes copy actions, normalising and handling excludes"() {
        ...
        def copySpec = new DestinationRootCopySpec(resolver, TestFiles.filePropertyFactory(testDirectory), new DefaultCopySpec(fileCollectionFactory, TestUtil.propertyFactory(), TestUtil.instantiatorFactory().decorateLenient(), TestFiles.patternSetFactory))
        copySpec.with {
            into "out"
            from "a", {
                from "b/b1", {
