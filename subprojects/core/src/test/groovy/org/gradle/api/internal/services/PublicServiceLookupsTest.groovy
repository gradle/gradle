...
class PublicServiceLookupsTest extends Specification {
    ...
    def "resolves #serviceType.simpleName from #entryPoint"() {
        ...
        [serviceType, entryPoint] << [
            ...
            [[ProjectLayout, PROJECT], [ProjectLayout, TASK], [ExecOperations, TASK], [BuildLayout, SETTINGS]]
        ].collectMany { it }
    }
    ...
    def "rejects #serviceType.simpleName from #entryPoint with pointer to available scopes"() {
        ...
        ProjectLayout  | SETTINGS   | "org.gradle.api.file.ProjectLayout is not available in settings scripts.\nIt is available in project build scripts and tasks."
        ProjectLayout  | GRADLE     | "org.gradle.api.file.ProjectLayout is not available in init scripts.\nIt is available in project build scripts and tasks."
        BuildLayout    | PROJECT    | "org.gradle.api.file.BuildLayout is not available in project build scripts.\nIt is available in settings scripts."
        BuildLayout    | GRADLE     | "org.gradle.api.file.BuildLayout is not available in init scripts.\nIt is available in settings scripts."
        BuildLayout    | TASK       | "org.gradle.api.file.BuildLayout is not available in tasks.\nIt is available in settings scripts."
        ExecOperations | PROJECT    | "org.gradle.process.ExecOperations is not available in project build scripts.\nIt is available in tasks."
    }
    ...
    def "rejects build service type with pointer to shared build services"() {
        when:
        PublicServiceLookups.lookup(SomeBuildService, SETTINGS, registry)
