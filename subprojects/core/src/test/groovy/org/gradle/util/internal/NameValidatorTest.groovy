...
class NameValidatorTest extends Specification {
    ...
    def "tasks are not allowed to be named '#name'"() {
        ...
        new TaskInstantiator(new TaskIdentityFactory(new InConfigurationIdFactory()), new TaskFactory(project, Mock(InstantiationScheme)), project).create(name, DefaultTask)
