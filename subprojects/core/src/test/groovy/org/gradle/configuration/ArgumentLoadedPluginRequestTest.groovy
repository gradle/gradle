...
class ArgumentLoadedPluginRequestTest extends Specification {
    def "can parse plugin id"() {
        when:
        ArgumentLoadedPluginRequest request = ArgumentLoadedPluginRequest.parsePluginRequest("org.barfuin.gradle.taskinfo:2.2.0")
    ...
    def "reports error if invalid arg provided: #coords"() {
        when:
        ArgumentLoadedPluginRequest.parsePluginRequest(coords)
