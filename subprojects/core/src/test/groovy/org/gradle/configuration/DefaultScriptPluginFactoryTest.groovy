...
class DefaultScriptPluginFactoryTest extends Specification {
    ...
    void "configures a target object using script"() {
        ...
        1 * scriptCompiler.compile(DefaultScript, target, baseScope, _ as NoDataCompileOperation) >> classPathScriptRunner
        1 * classPathScriptRunner.run(target, _ as ServiceRegistry)
        1 * scriptCompiler.compile(DefaultScript, target, targetScope, { it.transformer != null }) >> scriptRunner
    ...
    void "configures a project object using script with imperative and inheritable code"() {
        ...
        1 * loggingManagerFactory.createLoggingManager() >> loggingManager
        1 * scriptCompilerFactory.createCompiler(scriptSource) >> scriptCompiler
        1 * scriptCompiler.compile(ProjectScript, target, baseScope, _ as NoDataCompileOperation) >> classPathScriptRunner
        1 * classPathScriptRunner.run(target, _ as ServiceRegistry)
        1 * scriptCompiler.compile(ProjectScript, target, targetScope, { it.transformer != null }) >> scriptRunner
        _ * scriptRunner.data >> new BuildScriptData(true)
        _ * scriptRunner.runDoesSomething >> true
