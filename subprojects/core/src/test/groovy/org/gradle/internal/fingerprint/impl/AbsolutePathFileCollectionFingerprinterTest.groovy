...
class AbsolutePathFileCollectionFingerprinterTest extends Specification {
    ...
    def "retains order of files in the snapshot"() {
        ...
        when:
        def fingerprint = takeFingerprint(files(file, file2, file3))
    ...
    def "fingerprint includes existing root elements"() {
        ...
        when:
        def fingerprint = takeFingerprint(files(file, dir, noExist))
    ...
    def "retains order of elements in the snapshot"() {
        ...
        when:
        def fingerprint = takeFingerprint(files(file, file2, file3, file4))
    ...
    def generatesEventWhenFileAdded() {
        ...
        when:
        def fingerprint = takeFingerprint(files(file1))
        virtualFileSystem.invalidateAll()
        changes(fingerprint, takeFingerprint(files(file1, file2)), listener)
    ...
    def generatesEventWhenFileRemoved() {
        ...
        when:
        FileCollectionFingerprint fingerprint = takeFingerprint(files(file1, file2))
        virtualFileSystem.invalidateAll()
        changes(fingerprint, takeFingerprint(files(file1)), listener)
    ...
    def doesNotGenerateEventForFileWhoseTypeAndMetaDataAndContentHaveNotChanged() {
        ...
        when:
        FileCollectionFingerprint fingerprint = takeFingerprint(files(file))
        virtualFileSystem.invalidateAll()
        changes(fingerprint, takeFingerprint(files(file)), listener)
    ...
    def generatesEventWhenFileBecomesADirectory() {
        ...
        when:
        FileCollectionFingerprint fingerprint = takeFingerprint(fileCollection)
        file.delete()
        file.createDir()
        virtualFileSystem.invalidateAll()
        changes(fingerprint, takeFingerprint(fileCollection), listener)
    ...
    def doesNotGenerateEventForDirectoryThatHasNotChanged() {
        ...
        when:
        FileCollectionFingerprint fingerprint = takeFingerprint(files(dir))
        virtualFileSystem.invalidateAll()
        changes(fingerprint, takeFingerprint(files(dir)), listener)
    ...
    def canCreateEmptySnapshot() {
        ...
        FileCollectionFingerprint newFingerprint = takeFingerprint(files(file))
        virtualFileSystem.invalidateAll()
