...
public class DefaultIncrementalCompilerBuilder implements IncrementalCompilerBuilder {
    ...
    public DefaultIncrementalCompilerBuilder(FileHasher hasher, CompilationStateCacheFactory compilationStateCacheFactory, CSourceParser sourceParser) {
        this.hasher = hasher;
    ...
    public <T extends NativeCompileSpec> Compiler<T> createIncrementalCompiler(TaskInternal task, Compiler<T> compiler, NativeToolChain toolchain, HeaderDependenciesCollector headerDependenciesCollector) {
        return new IncrementalNativeCompiler<T>(task, hasher, compilationStateCacheFactory, compiler, toolchain, headerDependenciesCollector, sourceParser);
    }
}
