...
class ShallowDirectorySnapshotTest extends Specification {
    def "can obtain metadata from Directory"() {
        def metadataSnapshot = new ShallowDirectorySnapshot("some/prefix", ImmutableList.of())
        expect:
        metadataSnapshot.getSnapshot("/absolute/path", "/absolute/path".length() + 1).get() == metadataSnapshot
        metadataSnapshot.getSnapshot("another/path", 0).get().type == FileType.Missing
    }
    ...
    def "invalidating something in a directory retains the directory information"() {
        ...
        !invalidated.getSnapshot("another/path", 0).present
    }
