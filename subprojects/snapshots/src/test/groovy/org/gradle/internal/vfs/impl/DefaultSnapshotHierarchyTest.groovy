...
class DefaultSnapshotHierarchyTest extends Specification {
    ...
    def "can add dir to empty set"() {
        ...
        expect:
        def s1 = empty.store(dir1.absolutePath, dir1Snapshot, changeListener)
        snapshotPresent(s1, dir1)
        !snapshotPresent(s1, dir2)

        def s2 = empty.store(dir2.absolutePath, dir2Snapshot, changeListener)
    ...
    def "root is handled correctly"() {
        ...
        when:
        def set = EMPTY.store("/", new CompleteDirectorySnapshot("/", "", [new RegularFileSnapshot("/root.txt", "root.txt", HashCode.fromInt(1234), new FileMetadata(1, 1))], HashCode.fromInt(1111)), changeListener)
        then:
        set.getMetadata("/root.txt").get().type == FileType.RegularFile

        when:
        set = set.invalidate("/root.txt", changeListener).store("/", new CompleteDirectorySnapshot("/", "", [new RegularFileSnapshot("/base.txt", "base.txt", HashCode.fromInt(1234), new FileMetadata(1, 1))], HashCode.fromInt(2222)), changeListener)
        then:
    ...
    def "can add to completely different paths with Unix paths"() {
        ...
        def set = EMPTY
            .store(firstPath, directorySnapshotForPath(firstPath), changeListener)
            .store(secondPath, directorySnapshotForPath(secondPath), changeListener)
            .store("/other/just-checking", directorySnapshotForPath("/other/just-checking"), changeListener)
        ...
        when:
        def invalidated = set.invalidate(firstPath, changeListener)
        then:
    ...
    def "can update the root path"() {
        when:
        def set = EMPTY
            .store("/", rootDirectorySnapshot(), changeListener)
        then:
        ...
        when:
        set = EMPTY
            .store("/some/path", directorySnapshotForPath("/some/path"), changeListener)
            .store("/", rootDirectorySnapshot(), changeListener)
        then:
        set.getMetadata("/").present
        set.getMetadata("/root.txt").get().type == FileType.RegularFile
        ...
        set = EMPTY
            ...
            .store("/secondPath", directorySnapshotForPath("/secondPath"), changeListener)
        then:
        set.invalidate("/", changeListener) == EMPTY

        when:
        set = EMPTY
            .store("/", rootDirectorySnapshot(), changeListener)
            .invalidate("/root.txt", changeListener)
        then:
        set.getMetadata("/").get().type == FileType.Directory
        !set.getSnapshot("/").present
    ...
    def "can add to completely different paths with Windows paths"() {
        ...
        def set = EMPTY
            .store(firstPath, directorySnapshotForPath(firstPath), changeListener)
            .store(secondPath, directorySnapshotForPath(secondPath), changeListener)
            .store(thirdPath, directorySnapshotForPath(thirdPath), changeListener)
        ...
        when:
        invalidated = set.invalidate("C:\\", changeListener)
        then:
        !invalidated.getMetadata(firstPath).present
        invalidated.getMetadata(secondPath).present

        when:
        invalidated = set.invalidate("D:\\", changeListener)
        then:
        invalidated.getMetadata(firstPath).present
        !invalidated.getMetadata(secondPath).present
    }
    ...
    def "can handle UNC paths"() {
        ...
        def set = EMPTY
            ...
            .store(secondPath, directorySnapshotForPath(secondPath), changeListener)
            .store(thirdPath, directorySnapshotForPath(thirdPath), changeListener)
            .store("C:\\Some\\Location", directorySnapshotForPath("C:\\Some\\Location"), changeListener)
        ...
        when:
        invalidated = set.invalidate("\\\\server", changeListener)
        then:
        !invalidated.getMetadata(firstPath).present
        !invalidated.getMetadata(secondPath).present
        ...
        when:
        invalidated = set.invalidate("\\\\otherServer", changeListener)
        then:
        invalidated.getMetadata(firstPath).present
        invalidated.getMetadata(secondPath).present
        !invalidated.getMetadata(thirdPath).present
    ...
    private SnapshotHierarchy invalidate(SnapshotHierarchy set, File location) {
        set.invalidate(location.absolutePath, changeListener)
    }

    private SnapshotHierarchy fileHierarchySet(File location) {
        EMPTY.store(location.absolutePath, location.directory ? snapshotDir(location) : snapshotFile(location), changeListener)
    }

    private SnapshotHierarchy fileHierarchySet(Iterable<? extends File> locations) {
        SnapshotHierarchy set = EMPTY
        for (File location : locations) {
            set = set.store(location.absolutePath, location.directory ? snapshotDir(location) : snapshotFile(location), changeListener)
        }
        return set
    }

    private SnapshotHierarchy updateDir(SnapshotHierarchy set, File dir) {
        set.store(dir.absolutePath, snapshotDir(dir), changeListener)
    }
