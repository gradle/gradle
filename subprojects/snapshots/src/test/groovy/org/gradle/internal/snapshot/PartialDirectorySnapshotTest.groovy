...
class PartialDirectorySnapshotTest extends Specification {
    def "can obtain metadata from Directory"() {
        ...
        expect:
        metadataSnapshot.getSnapshot("/absolute/path", "/absolute/path".length() + 1, true).get() == metadataSnapshot
        !metadataSnapshot.getSnapshot("another/path", 0, true).present
    }
    ...
    def "invalidating something in a directory retains the directory information"() {
        ...
        expect:
        metadataSnapshot.invalidate("whatever", 0).get().getSnapshot("/absolute/some/prefix", "/absolute/some/prefix".length() + 1, true).get().type == FileType.Directory
    }
}
