...
class JavaToolchainQueryServiceTest extends Specification {
    ...
    private static final File CURRENT_JVM_HOME = new File("/path/6.0")
    ...
    def "can query for matching toolchain using version #versionToFind"() {
        ...
        def queryService = new JavaToolchainQueryService(registry, toolchainFactory, Mock(JavaToolchainProvisioningService), createProviderFactory(), CURRENT_JVM_HOME)
    ...
    def "returns failing provider if no toolchain matches"() {
        ...
        def queryService = new JavaToolchainQueryService(registry, toolchainFactory, provisioningService, createProviderFactory(), CURRENT_JVM_HOME)
    ...
    def "install toolchain if no matching toolchain found"() {
        ...
        def queryService = new JavaToolchainQueryService(registry, toolchainFactory, provisionService, createProviderFactory(), CURRENT_JVM_HOME)
    ...
    def "prefer version Gradle is running on as long as it is a match"() {
        ...
        def toolchainFactory = newToolchainFactory()
        def queryService = new JavaToolchainQueryService(registry, toolchainFactory, Mock(JavaToolchainProvisioningService), createProviderFactory(), new File("/path/1.8.2"))
        def versionToFind = JavaLanguageVersion.of(8)
