...
class DefaultSuiteCompileDepIntegrationTest extends AbstractCompileDepTestSuitesTransparencyIntegrationTest {
    def "default suite lacks collections dep and fails"() {
        expect:
        failsAtRuntimeDueToMissingCollectionsDependencyNoClassDefFound("test")
    }
    ...
    def "default suite with runtimeOnly collections dep succeed"() {
        given: "the dependency added to the test suite's as runtimeOnly"
        buildFile << """
            testing {
                suites {
                    test {
