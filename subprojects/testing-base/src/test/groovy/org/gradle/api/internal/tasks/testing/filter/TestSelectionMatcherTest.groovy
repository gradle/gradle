...
class TestSelectionMatcherTest extends Specification {
    ...
    def "knows if test matches class"() {
        ...
        ["com*FooTest.aaa"]   | "com.foo.FooTest"     | "aaa"      || true
        ["com*FooTest.aaa"]   | "com.foo.FooTest"     | "bbb"      || false
    ...
    def "script includes and command line includes both have to match"() {
        expect:
        new TestSelectionMatcher(input, [], inputCommandLine).matchesTest(className, methodName) == match
    ...
    def 'can exclude as many classes as possible'() {
        expect:
        new TestSelectionMatcher(input, [], []).mayIncludeClass(fullyQualifiedClassName) == maybeMatch
        new TestSelectionMatcher([], [], input).mayIncludeClass(fullyQualifiedClassName) == maybeMatch

        where:
        input                             | fullyQualifiedClassName || maybeMatch
        ['.']                             | 'FooTest'               || false
    ...
    def 'can use multiple patterns'() {
        expect:
        new TestSelectionMatcher(pattern1, [], pattern2).mayIncludeClass(fullyQualifiedClassName) == maybeMatch

        where:
        pattern1                | pattern2                 | fullyQualifiedClassName || maybeMatch
    ...
    def 'can collect selected methods'() {
        expect:
        new TestSelectionMatcher(includedTests, excludedTests, includedTestsCommandLine)
            .determineSelectedMethods(fullyQualifiedClassName) == selectedMethods as Set

        where:
        includedTests       | includedTestsCommandLine | excludedTests | fullyQualifiedClassName || selectedMethods
        ['FooTest.a']       | []                       | []            | 'FooTest'               || ['a']
        ['FooTest.a']       | ['BarTest']              | []            | 'FooTest'               || []
        ['FooTest.a']       | []                       | ['*']         | 'FooTest'               || []
        ['com.*.FooTest.a'] | []                       | []            | 'com.acme.FooTest'      || ['a']
        ['*.a', '*.b']      | []                       | []            | 'FooTest'               || ['a', 'b']
        []                  | ['*.a', '*.b']           | []            | 'FooTest'               || ['a', 'b']
        ['*.a', '*.b']      | ['*.a']                  | []            | 'FooTest'               || ['a']
        ['*.b']             | ['*.a', '*.b']           | []            | 'FooTest'               || ['b']
