...
public class FailsafePhasedActionResultListener implements PhasedActionResultListener {
    ...
    public void rethrowErrors() {
        if (listenerFailure != null) {
            throw new ListenerNotificationException(null, "One or more build phasedAction listeners failed with an exception.", Collections.singletonList(listenerFailure));
        }
    }
}
