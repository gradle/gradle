...
class SupportedBuildJvmIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "fails when build is configured to use Java #jdk.javaVersion"() {
        ...
        expect:
        fails("help")
        failure.assertHasDescription("Gradle ${GradleVersion.current().version} requires Java 1.8 or later to run. Your build is currently configured to use Java ${jdk.javaVersion.majorVersion}.")
