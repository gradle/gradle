This guide demonstrates how to create a C++ application with Gradle using `gradle init`.
...
When prompted, select the `2: application` project type and `1: C++` as implementation language.
...
Our build contains one subproject called `app` that represents the C++ application we are building.
...
include::{samplesPath}/generated/cpp-application/tests/build.out[]
----
...
The `build` task compiles the C++ sources, links the object files, and runs the tests.
...
That's it! You've now successfully configured and built a C++ application project with Gradle.
