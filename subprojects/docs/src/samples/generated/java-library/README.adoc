...
> Task :init
https://docs.gradle.org/6.7-20200825220000+0000/userguide/java_library_plugin.html
...
<3> This dependency is exported to consumers, that is to say found on their compile classpath.
<4> This dependency is used internally, and not exposed to consumers on their own compile classpath.
...
include::{samplesPath}/generated/java-library/tests/build.out[]
----
