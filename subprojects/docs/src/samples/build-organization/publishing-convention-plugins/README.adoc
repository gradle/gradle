...
Here, we have configured the project to publish the plugins using link:{userManualPath}/publishing_maven.html[maven-publish plugin]
to a filesystem directory for demonstration purposes.
...
The `com.myorg.java-conventions` plugin uses SpotBugs plugin to perform static code analysis.
