...
This sample shows how create a plugin with a task that accepts arguments.
The project itself has no sources.
...
[.sample-command]
----
$ ./gradlew --include-build project-info-plugin projectInfo --format json
{
