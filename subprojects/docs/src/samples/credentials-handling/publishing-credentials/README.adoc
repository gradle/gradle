...
The code in `maven-repository-stub` directory builds a plugin used to stub the Maven repository in order to demonstrate the authentication flow.
To demonstrate the concept, we will be using the following hardcoded credentials on the server stub:
====
