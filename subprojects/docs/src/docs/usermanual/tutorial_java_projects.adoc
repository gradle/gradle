...
----
> gradle build
include::{samples-path}/java/quickstart/javaQuickstart.out[]
----
...
.build.gradle
----
include::{samples-path}/java/quickstart/build.gradle[tag=repo]
----
...
.build.gradle
----
include::{samples-path}/java/quickstart/build.gradle[tag=dependencies]
----

You can find out more in <<dependency_management_for_java_projects#,Dependency Management for Java Projects>>.
...
.build.gradle
----
include::{samples-path}/java/quickstart/build.gradle[tag=customization]
----
...
The tasks which the Java plugin adds are regular tasks, exactly the same as if they were declared in the build file. This means you can use any of the mechanisms shown in earlier chapters to customize these tasks. For example, you can set the properties of a task, add behaviour to a task, change the dependencies of a task, or replace a task entirely. In our sample, we will configure the `test` task, which is of type link:{groovy-dsl-path}/org.gradle.api.tasks.testing.Test.html[Test], to add a system property when the tests are executed:
...
.build.gradle
----
include::{samples-path}/java/quickstart/build.gradle[tag=task-customization]
----
...
.build.gradle
----
include::{samples-path}/java/quickstart/build.gradle[tag=upload]
----
...
.build.gradle
----
include::{samples-path}/java/quickstart/build.gradle[tag=use-eclipse-plugin]
----

Now execute `gradle eclipse` command to generate Eclipse project files. More information about the `eclipse` task can be found in the <<eclipse_plugin#,Eclipse Plugin>> chapter.
...
.build.gradle
----
include::{samples-path}/java/quickstart/build.gradle[tags=*]
----
...
.settings.gradle
----
include::{samples-path}/java/multiproject/settings.gradle[tag=include-projects]
----

You can find out more about the settings file in <<multi_project_builds#,Authoring Multi-Project Builds>>.
...
.build.gradle
----
include::{samples-path}/java/multiproject/build.gradle[tag=configuration-injection]
----
...
.api/build.gradle
----
include::{samples-path}/java/multiproject/api/build.gradle[tag=dependencies]
----
...
.api/build.gradle
----
include::{samples-path}/java/multiproject/api/build.gradle[tag=dists]
----
