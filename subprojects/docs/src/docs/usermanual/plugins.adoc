...
* Including the plugin from the plugin portal or a <<#sec:custom_plugin_repositories,custom repository>> using the plugins DSL (see <<plugins#sec:plugins_block,Applying plugins using the plugins DSL>>).
* Including the plugin from an external jar defined as a buildscript dependency (see see <<plugins#sec:applying_plugins_buildscript,Applying plugins using the buildscript block>>).
* Defining the plugin as a source file under the buildSrc directory in the project (see <<organizing_gradle_projects#sec:build_sources,Using buildSrc to extract functional logic>>).
* Defining the plugin as an inline class declaration inside a build script.

For more on defining your own plugins, see <<custom_plugins#,"Custom Plugins">>.
...
Custom Maven and Ivy plugin repositories must contain <<#sec:plugin_markers,plugin marker artifacts>> in addition to the artifacts which actually implement the plugin. For more information on publishing plugins to custom repositories read <<java_gradle_plugin#,"Gradle Plugin Development Plugin">>.
...
This chapter aims to serve as an introduction to plugins and Gradle and the role they play. For more information on the inner workings of plugins, see <<custom_plugins#,"Custom Plugins">>.
