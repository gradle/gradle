...
If you want to skip the tests when running a build, you have a few options. You can either do it via <<command_line_interface#sec:excluding_tasks_from_the_command_line,command line arguments>> or <<sec:skipping_tasks,in the build script>>. To do it on the command line, you can use the `-x` or `--exclude-task` option like so:
...
`cleanTest` is based on a _<<more_about_tasks#sec:task_rules,task rule>>_ provided by the <<base_plugin#sec:base_tasks,Base Plugin>>. You can use it for _any_ task.
