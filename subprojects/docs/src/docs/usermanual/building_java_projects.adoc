...
You can easily create Java properties files via the `link:{groovy-dsl-path}/org.gradle.api.tasks.WriteProperties.html[WriteProperties]` task, which fixes a well-known problem with `Properties.store()` that can reduce the usefulness of <<sec:up_to_date_checks,incremental builds>>.
