 ...
 * _<<java_testing#sec:configuring_java_integration_tests,Configuring Java integration tests>>_
 ...
 * Declaring dependencies with _<<declaring_dependencies#sub:declaring_dependency_with_changing_version,changing>>_ (e.g. SNAPSHOT) and _<<declaring_dependencies#sub:declaring_dependency_with_dynamic_version,dynamic>>_ (range) versions
...
Since `processResources` is an instance of the `Copy` task, you can perform any of the processing described in the _<<working_with_files#sec:copying_files,Working With Files>>_ chapter.
