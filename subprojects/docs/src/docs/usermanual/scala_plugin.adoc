...
You can diagnose problems with the version of the Zinc compiler selected by running <<inspecting_dependencies#,dependencyInsight>> for the `zinc` configuration.
...
|  `scala`
|  `link:{groovy-dsl-path}/org.gradle.api.file.SourceDirectorySet.html[SourceDirectorySet]` (read-only)
| Not null
...
|  `scala.srcDirs`
|  `Set&lt;File&gt;`. Can set using anything described in <<working_with_files#sec:specifying_multiple_files,Understanding implicit conversion to file collections>>.
...
|  `allScala`
|  `link:{groovy-dsl-path}/org.gradle.api.file.FileTree.html[FileTree]` (read-only)
| Not null
...
The Scala compiler ignores Gradle's `targetCompatibility` and `sourceCompatibility` settings. In Scala 2.11, the Scala compiler always compiles to Java 6 compatible bytecode. In Scala 2.12, the Scala compiler always compiles to Java 8 compatible bytecode. If you also have Java source, you can follow the same steps as for the <<building_java_projects#sec:java_cross_compilation,Java plugin>> to ensure the correct Java compiler is used.
