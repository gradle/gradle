...
* Configuring any dependency (transitive or not) as _forced_. This approach is useful if the dependency in conflict is a transitive dependency. See <<managing_dependency_versions#sec:enforcing_dependency_version,Enforcing a particular dependency version>> for examples.
