...
Gradle models source code directories with the help of the <<building_java_projects#sec:java_source_sets,source set concept>>.
...
----
include::{samples-path}/userguide/organizingGradleProjects/separatedTestTypes/gradle/integration-test.gradle[tag=custom-source-set]
----

Source sets are only responsible for compiling source code, but do not deal with executing the byte code.
For the purpose of test execution, a corresponding task of type `link:{groovy-dsl-path}/org.gradle.api.tasks.testing.Test.html[Test]` needs to be established.
...
.gradle/integration-test.gradle
----
include::{samples-path}/userguide/organizingGradleProjects/separatedTestTypes/gradle/integration-test.gradle[tag=test-task]
----
...
The directory `buildSrc` is treated as an <<composite_builds#composite_build_intro,included build>>. Upon discovery of the directory, Gradle automatically compiles and tests this code and puts it in the classpath of your build script.
For multi-project builds there can be only one `buildSrc` directory, which has to sit in the root project directory.
`buildSrc` should be preferred over <<plugins#sec:script_plugins,script plugins>> as it is easier to maintain, refactor and test the code.

`buildSrc` uses the same <<java_plugin#javalayout,source code conventions>> applicable to Java and Groovy projects.
...
----
include::{samples-path}/java/multiproject/buildSrc/build.gradle[]
----
...
It's a good location for placing <<build_environment#sec:gradle_configuration_properties,properties that control the build environment>>.
...
Tasks should define inputs and outputs to get the performance benefits of <<more_about_tasks#sec:up_to_date_checks,incremental build functionality>>.
