...
.build.gradle
----
include::{samples-path}/toolingApi/runBuild/build.gradle[tag=use-tooling-api]
----

The main entry point to the Tooling API is the `link:{javadoc-path}/org/gradle/tooling/GradleConnector.html[GradleConnector]`. You can navigate from there to find code samples and explore the available Tooling API models. You can use api:org.gradle.tooling.GradleConnector#connect()[] to create a `link:{javadoc-path}/org/gradle/tooling/ProjectConnection.html[ProjectConnection]`. A `ProjectConnection` connects to a single Gradle project. Using the connection you can execute tasks, tests and retrieve models relative to this project.
