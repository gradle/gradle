...
[NOTE]
====
Dependency locking makes sense only with <<declaring_dependencies#sub:declaring_dependency_with_dynamic_version,dynamic versions>>.
It will have no impact on <<declaring_dependencies#sub:declaring_dependency_with_changing_version,changing versions>> (like `-SNAPSHOT`) whose coordinates remain the same, though the content may change.
...
Locking of a configuration happens through the `link:{groovy-dsl-path}/org.gradle.api.artifacts.ResolutionStrategy.html[ResolutionStrategy]`:
...
.build.gradle
----
include::{samples-path}/userguide/dependencyManagement/dependencyLocking/lockingSingleConfiguration/build.gradle[tag=locking-one]
----
...
.build.gradle
----
include::{samples-path}/userguide/dependencyManagement/dependencyLocking/lockingAllConfigurations/build.gradle[tag=locking-all]
----
...
.build.gradle
----
include::{samples-path}/userguide/dependencyManagement/dependencyLocking/lockingAllConfigurations/build.gradle[tag=resolve-all]
----
...
----
include::{samples-path}/userguide/dependencyManagement/dependencyLocking/lockingSingleConfiguration/gradle/dependency-locks/compileClasspath.lockfile[]
----
...
.build.gradle
----
include::{samples-path}/userguide/dependencyManagement/dependencyLocking/lockingSingleConfiguration/build.gradle[tag=locking-deps]
----
...
* It is currently not possible to lock the <<plugins#sec:applying_plugins_buildscript,`classpath` configuration>> used for script plugins.
