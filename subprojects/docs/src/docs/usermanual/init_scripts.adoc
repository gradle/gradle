...
The closure passed to the `initscript()` method configures a `link:{groovy-dsl-path}/org.gradle.api.initialization.dsl.ScriptHandler.html[ScriptHandler]` instance. You declare the init script classpath by adding dependencies to the `classpath` configuration. This is the same way you declare, for example, the Java compilation classpath. You can use any of the dependency types described in <<declaring_dependencies#,"Declaring Dependencies">>, except project dependencies.
...
.init.gradle
----
include::{samples-path}/userguide/initScripts/externalDependency/init.gradle[]
----
