...
* <<build_environment.adoc#sec:gradle_system_properties, System properties>> such as `systemProp.http.proxyHost=somehost.org` stored in a `gradle.properties` file.
* <<build_environment.adoc#sec:gradle_configuration_properties, Gradle properties>> such as `org.gradle.caching=true` that are typically stored in a `gradle.properties` file in a project root directory or `GRADLE_USER_HOME` environment variable.
* <<build_environment.adoc#sec:gradle_environment_variables, Environment variables>> such as `GRADLE_OPTS` sourced by the environment that executes Gradle.

Aside from configuring the build environment, you can configure a given project build using <<build_environment.adoc#sec:project_properties, Project properties>> such as `-PreleaseType=final`.
...
Specifies the JVM arguments used for the Gradle Daemon. The setting is particularly useful for <<build_environment.adoc#sec:configuring_jvm_memory,configuring JVM memory settings>> for build performance.
`org.gradle.logging.level=(quiet,warn,lifecycle,info,debug)`::
When set to quiet, warn, lifecycle, info, or debug, Gradle will use this log level. The values are not case sensitive. The `lifecycle` level is the default. See <<logging.adoc#sec:choosing_a_log_level>>.
...
When set to `all`, `summary` or `none`, Gradle will use different warning type display. See <<command_line_interface.adoc#sec:command_line_logging>> for details.
...
.gradle.properties
----
include::{samples-path}/userguide/tutorial/properties/gradle.properties[]
----

[source.multi-language-sample,groovy]
.build.gradle
----
include::{samples-path}/userguide/tutorial/properties/build.gradle[]
----
...
include::{samples-path}/userguide/tutorial/properties/properties.out[]
----
...
Specify user name to download Gradle distributions from servers using HTTP Basic Authentication. Learn more in <<gradle_wrapper.adoc#sec:authenticated_download,the section on "authenticated wrapper downloads">>.
...
You can add properties directly to your link:{groovy-dsl-path}/org.gradle.api.Project.html[Project] object via the `-P` command line option.
...
You should check for existence of optional project properties before you access them using the link:{groovy-dsl-path}/org.gradle.api.Project.html#org.gradle.api.Project:hasProperty(java.lang.String)[Project.hasProperty(java.lang.String)] method.
====
...
Certain tasks in Gradle also fork additional JVM processes, like the `test` task when using link:{javadoc-path}/org/gradle/api/tasks/testing/Test.html#setMaxParallelForks-int-[Test.setMaxParallelForks(int)] for JUnit or TestNG tests. You must configure these through the tasks themselves.

=== Example: Set Java compile options for link:{groovy-dsl-path}/org.gradle.api.tasks.compile.JavaCompile.html[JavaCompile] tasks
...
See other examples in the link:{groovy-dsl-path}/org.gradle.api.tasks.testing.Test.html[Test] API documentation and <<java_testing.adoc#sec:test_execution, test execution in the Java plugin reference>>.
...
.build.gradle
----
include::{samples-path}/userguide/tutorial/configureTaskUsingProjectProperty/build.gradle[]
----

.Output of **`gradle performRelease -PisCI=true --quiet`**
----
> gradle performRelease -PisCI=true --quiet
include::{samples-path}/userguide/tutorial/configureTaskUsingProjectProperty/configureTaskUsingProjectProperty.out[]
----
