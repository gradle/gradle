...
You can add properties directly to your `link:{groovy-dsl-path}/org.gradle.api.Project.html#org.gradle.api.Project.html[Project]` object via the `-P` command line option.
...
.Set Java compile options for `link:{groovy-dsl-path}/org.gradle.api.tasks.compile.JavaCompile.html[JavaCompile]` tasks
====
[source,groovy]
----
apply plugin: "java"
...
See other examples in the `link:{groovy-dsl-path}/org.gradle.api.tasks.testing.Test.html[Test]` API documentation and <<sec:test_execution, test execution in the Java plugin reference>>.
