...
The Play plugin uses a _software model_ to describe a Play application and how to build it. The Play software model extends the base Gradle <<software_model_concepts#,software model>> to add support for building Play applications. A Play application is represented by a `link:{javadoc-path}/org/gradle/play/PlayApplicationSpec.html[PlayApplicationSpec]` component type. The plugin automatically creates a single `link:{javadoc-path}/org/gradle/play/PlayApplicationBinarySpec.html[PlayApplicationBinarySpec]` instance when it is applied. Additional Play components cannot be added to a project.
...
The `runPlayBinary` task starts the Play application under development. During development it is beneficial to execute this task as a <<command_line_interface#sec:continuous_build,continuous build>>. Continuous build is a generic feature that supports automatically re-running a build when inputs change. The `runPlayBinary` task is “continuous build aware” in that it behaves differently when run as part of a continuous build.
...
| -
| `link:{groovy-dsl-path}/org.gradle.api.tasks.application.CreateStartScripts.html[CreateStartScripts]`
| Generates scripts to run the Play application distribution.

| `stagePlayBinaryDist`
| `playBinary`, `createPlayBinaryStartScripts`
| `link:{groovy-dsl-path}/org.gradle.api.tasks.Copy.html[Copy]`
...
|
| `link:{groovy-dsl-path}/org.gradle.api.tasks.bundling.Tar.html[Tar]`
