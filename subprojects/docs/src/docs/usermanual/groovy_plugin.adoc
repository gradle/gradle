...
The Groovy plugin adds a `link:{groovy-dsl-path}/org.gradle.api.tasks.compile.GroovyCompile.html[GroovyCompile]` task for each source set in the project. The task type extends the `JavaCompile` task (see <<sec:compile>>). The `GroovyCompile` task supports most configuration options of the official Groovy compiler.
...
|  `source`
| `link:{javadoc-path}/org/gradle/api/file/FileTree.html[FileTree]`. Can set using anything described in <<sec:specifying_multiple_files>>.
