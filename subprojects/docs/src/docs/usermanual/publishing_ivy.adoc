 ...
 * <<sec:publishing_custom_artifacts_to_maven,Custom artifacts>> — via the api:org.gradle.api.publish.ivy.IvyPublication#artifact(java.lang.Object)[] method. See `link:{groovy-dsl-path}/org.gradle.api.publish.ivy.IvyArtifact.html[IvyArtifact]` for the available configuration options for custom Ivy artifacts.
