...
The dependency of `taskX` to `taskY` is declared before `taskY` is defined. This is very important for multi-project builds. Task dependencies are discussed in more detail in <<more_about_tasks#sec:adding_dependencies_to_tasks,Adding dependencies to a task>>.
...
Extra properties aren't limited to tasks. You can read more about them in <<writing_build_scripts#sec:extra_properties,Extra properties>>.
...
There is lots more you can do with Ant in your build scripts. You can find out more in <<ant#,"Ant">>.
...
Later you will see that such methods can be shared among subprojects in multi-project builds. If your build logic becomes more complex, Gradle offers you other very convenient ways to organize it. We have devoted a whole chapter to this. See <<organizing_gradle_projects#,"Organizing Gradle Projects">>.
...
As we later describe in full detail (see <<build_lifecycle#,"Build Lifecycle">>), Gradle has a configuration phase and an execution phase. After the configuration phase, Gradle knows all tasks that should be executed. Gradle offers you a hook to make use of this information. A use-case for this would be to check if the release task is among the tasks to be executed. Depending on this, you can assign different values to some variables.
...
The closure passed to the `buildscript()` method configures a link:{groovy-dsl-path}/org.gradle.api.initialization.dsl.ScriptHandler.html[ScriptHandler] instance. You declare the build script classpath by adding dependencies to the `classpath` configuration. This is the same way you declare, for example, the Java compilation classpath. You can use any of the <<dependency_types#,dependency types>> except project dependencies.
...
.build.gradle
----
include::{samples-path}/userguide/tutorial/externalDependency/build.gradle[]
----
...
Build script dependencies may be Gradle plugins. Please consult <<plugins#,"Plugins">> for more information on Gradle plugins.
...
In this chapter, we have had a first look at tasks. But this is not the end of the story for tasks. If you want to jump into more of the details, have a look at <<more_about_tasks#,"More About Tasks">>.

Otherwise, continue on to <<tutorial_java_projects#,the tutorials>> and <<dependency_management_for_java_projects#,"Dependency Management for Java Projects">>.
