...
.build.gradle
----
include::{samples-path}/signing/maven/build.gradle[tag=use-plugin]
----
...
NOTE: Given the personal and private nature of these values, a good practice is to store them in the `gradle.properties` file in the user's Gradle home directory (described in <<build_environment#sec:gradle_system_properties,System properties>>) instead of in the project directory itself.
...
.build.gradle
----
include::{samples-path}/signing/gnupg-signatory/build.gradle[tag=configure-signatory]
----

This tells the Signing Plugin to use the `GnupgSignatory` instead of the default link:{javadoc-path}/org/gradle/plugins/signing/signatory/pgp/PgpSignatory.html[PgpSignatory]. The `GnupgSignatory` relies on the gpg2 program to sign the artifacts. Of course, this requires that GnuPG is installed.
...
.gradle.properties
----
include::{samples-path}/signing/gnupg-signatory/gradle.properties[tag=user-properties]
----
...
include::{samples-path}/signing/maven-publish/signingPluginSignPublication.out[]
----
...
.build.gradle
----
include::{samples-path}/signing/maven/build.gradle[tag=sign-archives]
----

This will create a task (of type link:{groovy-dsl-path}/org.gradle.plugins.signing.Sign.html[Sign]) in your project named `signArchives`, that will build any `archives` artifacts (if needed) and then generate signatures for them. The signature files will be placed alongside the artifacts being signed.
...
----
> gradle signArchives
include::{samples-path}/signing/maven/signingArchivesOutput.out[]
----
...
.build.gradle
----
include::{samples-path}/signing/tasks/build.gradle[tag=sign-task]
----

This will create a task (of type link:{groovy-dsl-path}/org.gradle.plugins.signing.Sign.html[Sign]) in your project named `signStuffZip`, that will build the input task's archive (if needed) and then sign it. The signature file will be placed alongside the artifact being signed.
...
----
> gradle signStuffZip
include::{samples-path}/signing/tasks/signingTaskOutput.out[]
----

For a task to be _signable_, it must produce an archive of some type, i.e. it must extend link:{groovy-dsl-path}/org.gradle.api.tasks.bundling.AbstractArchiveTask.html[AbstractArchiveTask]. Tasks that do this are the link:{groovy-dsl-path}/org.gradle.api.tasks.bundling.Tar.html[Tar], link:{groovy-dsl-path}/org.gradle.api.tasks.bundling.Zip.html[Zip], link:{groovy-dsl-path}/org.gradle.api.tasks.bundling.Jar.html[Jar], link:{groovy-dsl-path}/org.gradle.api.tasks.bundling.War.html[War] and link:{groovy-dsl-path}/org.gradle.plugins.ear.Ear.html[Ear] tasks.
...
.build.gradle
----
include::{samples-path}/signing/conditional/build.gradle[tag=only-if]
----
...
.build.gradle
----
include::{samples-path}/signing/maven/build.gradle[tag=sign-pom]
----
