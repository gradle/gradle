...
`__sourceSet__Classes` — type: `link:{groovy-dsl-path}/org.gradle.api.Task.html#org.gradle.api.Task.html[Task]`::
...
To find information on the `api` configuration, please consult the <<sec:java_library_separation, Java Library Plugin>> reference documentation and the <<dependency_management_for_java_projects#, Dependency Management Tutorial>>.
...
The Java plugin uses the `link:{groovy-dsl-path}/org.gradle.api.tasks.Copy.html[Copy]` task for resource handling. It adds an instance for each source set in the project. You can find out more about the copy task in <<sec:copying_files>>.
...
Please first have a look at <<sec:incremental_compile>>, as incremental annotation processing builds on top of it.
