...
`output.classesDirs` — (read-only) link:{javadoc-path}/org/gradle/api/file/FileCollection.html[FileCollection]::
...
`compileClasspath` — link:{javadoc-path}/org/gradle/api/file/FileCollection.html[FileCollection]::
...
`annotationProcessorPath` — link:{javadoc-path}/org/gradle/api/file/FileCollection.html[FileCollection]::
...
`runtimeClasspath` — link:{javadoc-path}/org/gradle/api/file/FileCollection.html[FileCollection]::
...
See the <<java_testing#sec:configuring_java_integration_tests,integration test example>> in the _Testing in Java & JVM projects_ chapter.
...
`components.java`::
A link:{javadoc-path}/org/gradle/api/component/SoftwareComponent.html[SoftwareComponent] for <<publishing_overview#,publishing>> the production JAR created by the `jar` task. This component includes the runtime dependency information for the JAR.
...
`(read-only) link:{javadoc-path}/org/gradle/api/tasks/SourceSetContainer.html[SourceSetContainer] sourceSets`::
Contains the project's source sets. Default value: Not null link:{javadoc-path}/org/gradle/api/tasks/SourceSetContainer.html[SourceSetContainer]
...
`File destinationDir`::
Default value: `__sourceSet__.output.resourcesDir`. Can set using anything described in <<working_with_files#sec:locating_files,file paths in depth>>.
...
[source.multi-language-sample,java,indent=0]
