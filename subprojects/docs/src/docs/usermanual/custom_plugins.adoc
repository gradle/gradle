...
One thing to note is that a new instance of a plugin is created for each project it is applied to. Also note that the `link:{groovy-dsl-path}/org.gradle.api.Plugin.html[Plugin]` class is a generic type. This example has it receiving the `link:{groovy-dsl-path}/org.gradle.api.Project.html[Project]` type as a type parameter. A plugin can instead receive a parameter of type `link:{groovy-dsl-path}/org.gradle.api.initialization.Settings.html[Settings]`, in which case the plugin can be applied in a settings script, or a parameter of type `link:{groovy-dsl-path}/org.gradle.api.invocation.Gradle.html[Gradle]`, in which case the plugin can be applied in an initialization script.
