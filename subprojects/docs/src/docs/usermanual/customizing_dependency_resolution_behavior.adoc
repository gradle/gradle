...
Values from the Ivy descriptor are made available via the `link:{groovy-dsl-path}/org.gradle.api.artifacts.ivy.IvyModuleDescriptor.html[IvyModuleDescriptor]` interface.
...
Note that while any rule can request the `link:{groovy-dsl-path}/org.gradle.api.artifacts.ivy.IvyModuleDescriptor.html[IvyModuleDescriptor]`, only components sourced from an Ivy repository will have a non-null value for it.
...
* may have additional parameters of type `link:{javadoc-path}/org/gradle/api/artifacts/ComponentMetadata.html[ComponentMetadata]` and/or `link:{groovy-dsl-path}/org.gradle.api.artifacts.ivy.IvyModuleDescriptor.html[IvyModuleDescriptor]`.
