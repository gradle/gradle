...
Rule source plugins can be packaged and distributed in the same manner as other types of plugins (see <<custom_plugins#,"Custom Plugins">>). They also may be applied in the same manner (to project objects) as `link:{groovy-dsl-path}/org.gradle.api.Plugin.html[Plugin]` implementations (i.e. via `link:{groovy-dsl-path}/org.gradle.api.Project.html#org.gradle.api.Project:apply(java.util.Map)[Project.apply(java.util.Map)]`).
