...
NOTE: Looking for information on upload tasks and the `archives` configuration? See the <<artifact_management#,Artifact Management>> chapter.
 ...
 * Publishing plugins support different artifact configuration properties, so always check the plugin documentation for more details. The `classifier` and `extension` properties are supported by both the <<publishing_maven#,Maven Publish Plugin>> and the <<publishing_ivy#,Ivy Publish Plugin>>.
...
The <<signing_plugin#, Signing Plugin>> can be used to sign all artifacts and metadata files that make up a publication, including Maven POM files and Ivy module desciptors. In order to use it:
 ...
 2. Configure the <<signing_plugin#sec:signatory_credentials,signatory credentials>> — follow the link to see how
...
[[glossary:component]]
Component::
Any single version of a <<dependency_management_terminology#sub:terminology_module,module>>.
+
...
Configuration::
A named collection of <<dependency_management_terminology#sub:terminology_configuration,dependencies>> or <<#glossary:artifact,artifacts>>.
+
