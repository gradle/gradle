...
### A number of types were converted from Groovy to Java

And as a result might cause different behavior when used as base types.

Gradle core is now implemented in pure Java.
The following plugins were fully converted to Java: `jacoco`, `scala`, `osgi`, `javascript`, `distribution` and `announce`.

Some plugins were partially converted to Java, keeping some types in Groovy:

- `init`: the `InitBuild` task and `Maven2Gradle` converter;
- `checkstyle`, `codenarc`, `findbugs`, `pmd` and `jdepend`: all task types ;
- `java`: the `Jar` task ;
- `war`: the `War` task ;
- `ear`: the `Ear` task ;
- `application`: the `CreateStartScripts` task ;
- `signing`: the `Sign` task ;
- `comparison`: all HTML renderers ;
- `idea` and `eclipse`: all plugin and task types.
