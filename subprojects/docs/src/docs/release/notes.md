...
## Step-by-step samples with Gradle team's recommendations

If you are new to Gradle, you might wonder how to get started with a new project and where to put what.
While this information has been around in the Gradle user manual and guides in previous releases, it was scattered and not always up to date with what we recommend for users starting using Gradle today.
Hence, we consolidated this information into multiple samples, that you can download or setup yourself using `gradle init`. 
The corresponding samples page walks you through the process of creating the projects, explains all the files and their content and how to actually build the project with Gradle.

There are versions of the samples for different types of projects and programming languages:
...
To use it, create an empty folder and call `gradle init` in that folder.
Gradle then gives you a number of interactive choices and creates an initial project structure for you.

Due to the flexibility of Gradle, you can customize everything in your project structure if you desire.
However, in most cases you are happy to follow good recommendations to end up with a maintainable software project and build. 

To cater to this need, `gradle init` has been updated to follow the recommendations of the Gradle team.
Everything `gradle init` produces has been carefully reconsidered and new options were added.
In particular, you can now directly create a project with multiple subprojects that share build logic through _convention plugins_ in `buildSrc`.

To explore it, use `gradle init` to create a new project and choose _application_ with _application and library projects_:
...
Afterwards, choose the JVM language and build script DSL of you preference.

The update also includes improvements to the [Maven-to-Gradle conversion](userguide/migrating_from_maven).
This can trigger it by using `gradle init` in the root of a Maven project.
For a Maven multi-module project, the conversion will now also place shared build logic in `buildSrc`.
Furthermore, the conversion can now produce Kotlin DSL scripts if you choose so.
