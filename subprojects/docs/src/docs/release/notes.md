...
When using annotation processors, generated Java source files can be generated into a directory. When the directory is specified via the new `compileJava.options.annotationProcessorGeneratedSourcesDirectory` property, the generated sources are also stored and loaded from the build cache.
