...
### Group opposite boolean task options together

The console output of the `help` task renders options in alphabetical order.

For better readability, this has been changed for opposite boolean task options.

Disable options are now sorted after their opposite option, for example `--foo` and `--no-foo` are grouped together and are rendered after `--bar` and `--no-bar`.
