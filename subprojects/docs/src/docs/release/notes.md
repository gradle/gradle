...
Gradle's [Java plugin](userguide/java_plugin.html) provides a lot of options to configure test execution.
This version of Gradle adds support for TestNG preserveOrder and groupByInstances options to control test execution order. More information about these features can be found in the [userguide](userguide/java_plugin.html#test_execution_order).
...
Gradle's [Build Init plugin](userguide/build_init_plugin.html) can be used to bootstrap the process of creating a new Gradle project.
It now supports [Spock framework](https://code.google.com/p/spock/) and [TestNG](http://testng.org/doc/index.html) for Java projects in addition to JUnit that was already supported.
You can specify a different test framework by using the following command:
...
You can use to Gradle's [Ivy Publishing plugin](userguide/publishing_ivy.html) to publish build artifacts in the [Apache Ivy](http://ant.apache.org/ivy/) format.
The Ivy descriptor file generated by the plugin now includes dependency exclude information. Exclusions configured in your Gradle build script on project or external module dependencies will be included in the published _ivy.xml_ file.
...
Gradle's [Play plugin](userguide/play_plugin.html) provides support for configuring compilation of [Twirl](https://github.com/playframework/twirl) templates.
