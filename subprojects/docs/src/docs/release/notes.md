...
The HTML test report for TestNG Test are now generated by default. You can switch off the report generation by setting the `testReport` to false:
...
We deprecated the `useDefaultListeners` property in org.gradle.api.tasks.testing.testng.TestNGOptions as Gradle now always generates test results in XML format for TestNG based Tests and enabled the HTML report generation for TestNG by default.
