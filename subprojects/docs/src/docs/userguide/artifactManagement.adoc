...
This chapter describes the _original_ publishing mechanism available in Gradle 1.0, which has since been superseded by <<publishing_overview,an alternative model>>. The approach detailed in this chapter — based on api:org.gradle.api.tasks.Upload[] tasks — should not be used in new builds. We cover it in order to help users work with and update existing builds that use it.
