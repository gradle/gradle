...
====
include::sample[dir="groovy/incrementalCompilation/groovy",files="build.gradle[tags=enable-groovy-incremental]"]
include::sample[dir="groovy/incrementalCompilation/kotlin",files="build.gradle.kts[tags=enable-groovy-incremental]"]
====

This gives you the following benefits:

* Incremental builds are much faster.
* If only a small sets of Groovy source files are changed, only the affected source files will be recompiled.
...
* Groovy compiler doesn't keep `@Retention` in generated annotation class bytecode(https://issues.apache.org/jira/browse/GROOVY-9185[GROOVY-9185]), thus all annotations are `RUNTIME`.
