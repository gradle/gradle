...
_Caveat: the Groovy compilation avoidance is an incubating feature since Gradle 5.6. There're known inaccuracies so please enable it at your own risk._
...
then Groovy compilation tasks are supposed to be up-to-date. This means that if project `A` depends on project `B` and a class in `B` is changed in an ABI-compatible way (typically, changing only the body of a method), then Gradle won't recompile `A`.
...
====
include::sample[dir="java/compilationAvoidance/groovy", files="build.gradle[tags=groovy-compilation-avoidance]"]
include::sample[dir="java/compilationAvoidance/kotlin", files="build.gradle.kts[tags=groovy-compilation-avoidance]"]
====
