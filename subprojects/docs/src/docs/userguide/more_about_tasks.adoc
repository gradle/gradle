...
And finally, there is the language specific syntax for Kotlin DSL:
...
.Configuring a task using the link:{javadocPath}/org/gradle/api/tasks/TaskProvider.html[TaskProvider] API
====
...
There is another way of configuring a task.

.Configuring a task using a the <<task_configuration_avoidance.adoc,task configuration avoidance>> APIs
====
...
This works for _any_ task. Task access in Kotlin is just a shortcut for the `tasks.named()` method. It is important to note that if you pass a block to the `named()` method, this block is applied only <<task_configuration_avoidance.adoc#how_does_it_work, when the task is required>>.
...
We also recommend using the <<task_configuration_avoidance.adoc#old_vs_new_configuration_api_overview, task configuration avoidance>> APIs alternative.
...
====
It's recommended to use the <<task_configuration_avoidance.adoc,Task Configuration Avoidance>> APIs instead to improve configuration time.
====
