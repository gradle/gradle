...
.build.gradle
----
include::{samples-path}/play/basic/build.gradle[tag=use-plugin]
----
...
The Play plugin uses a _software model_ to describe a Play application and how to build it. The Play software model extends the base Gradle <<software_model_concepts.adoc#software_model_concepts,software model>> to add support for building Play applications. A Play application is represented by a link:{javadoc-path}/org/gradle/play/PlayApplicationSpec.html[PlayApplicationSpec] component type. The plugin automatically creates a single link:{javadoc-path}/org/gradle/play/PlayApplicationBinarySpec.html[PlayApplicationBinarySpec] instance when it is applied. Additional Play components cannot be added to a project.
...
| java
| link:{javadoc-path}/org/gradle/language/java/JavaSourceSet.html[JavaSourceSet]
| app
| \**/*.java

| scala
| link:{javadoc-path}/org/gradle/language/scala/ScalaLanguageSourceSet.html[ScalaLanguageSourceSet]
| app
| \**/*.scala

| routes
| link:{javadoc-path}/org/gradle/language/routes/RoutesSourceSet.html[RoutesSourceSet]
| conf
| routes, *.routes

| twirlTemplates
| link:{javadoc-path}/org/gradle/language/twirl/TwirlSourceSet.html[TwirlSourceSet]
| app
| \**/*.scala.*

| javaScript
| link:{javadoc-path}/org/gradle/language/javascript/JavaScriptSourceSet.html[JavaScriptSourceSet]
| app/assets
| \**/*.js
|===

These source sets can be configured or additional source sets can be added to the Play component. See <<play_plugin.adoc#configuring_play,Configuring Play>> for further information.

Another element of configuring a Play application is the _platform_. To build a Play application, Gradle needs to understand which versions of Play, Scala and Java to use. The Play component specifies this requirement as a link:{javadoc-path}/org/gradle/play/platform/PlayPlatform.html[PlayPlatform]. If these values are not configured, a default version of Play, Scala and Java will be used. See <<play_plugin.adoc#configuring_play_platform,Targeting a certain version of Play>> for information on configuring the Play platform.
...
A Play application component is compiled and packaged to produce a set of outputs which are represented by a link:{javadoc-path}/org/gradle/play/PlayApplicationBinarySpec.html[PlayApplicationBinarySpec]. The Play binary specifies the jar files produced by building the component as well as providing elements by which additional content can be added to those jar files. It also exposes the tasks involved in building the component and creating the binary.

See <<play_plugin.adoc#configuring_play,Configuring Play>> for examples of configuring the Play binary.

[[sec:play_project_layout]]
== Project Layout

The Play plugin follows the typical Play application layout. You can <<play_plugin.adoc#configuring_play_sourcesets,configure source sets>> to include additional directories or change the defaults.
...
`playBinary` — type: link:{groovy-dsl-path}/org.gradle.api.Task.html[Task]::
...
`dist` — type: link:{groovy-dsl-path}/org.gradle.api.Task.html[Task]::
_Depends on_: `createPlayBinaryZipDist`, `createPlayBinaryTarDist`
+
Assembles the Play distribution.

`stage` — type: link:{groovy-dsl-path}/org.gradle.api.Task.html[Task]::
...
`runPlayBinary` — type: link:{groovy-dsl-path}/org.gradle.play.tasks.PlayRun.html[PlayRun]::
_Depends on_: `playBinary` to build Play application.
+
Runs the Play application for local development. See <<play_plugin.adoc#play_continuous_build,how this works with continuous build.>>

`testPlayBinary` — type: link:{groovy-dsl-path}/org.gradle.api.tasks.testing.Test.html[Test]::
...
`compilePlayBinaryScala` — type: link:{javadoc-path}/org/gradle/language/scala/tasks/PlatformScalaCompile.html[PlatformScalaCompile]::
_Depends on_: Scala and Java
+
Compiles all Scala and Java sources defined by the Play application.

`compilePlayBinaryPlayTwirlTemplates` — type: link:{groovy-dsl-path}/org.gradle.play.tasks.TwirlCompile.html[TwirlCompile]::
...
`compilePlayBinaryPlayRoutes` — type: link:{groovy-dsl-path}/org.gradle.play.tasks.RoutesCompile.html[RoutesCompile]::
_Depends on_: Play Route files
+
Compiles routes files into Scala sources.

`minifyPlayBinaryJavaScript` — type: link:{groovy-dsl-path}/org.gradle.play.tasks.JavaScriptMinify.html[JavaScriptMinify]::
...
----
> gradle components
include::{samples-path}/play/basic/playComponentReport.out[]
----
...
By default, Gradle uses Play 2.6.15, Scala 2.12 and the version of Java used to start the build. A Play application can select a different version by specifying a target link:{groovy-dsl-path}/org.gradle.play.PlayApplicationSpec.html#org.gradle.play.PlayApplicationSpec:platform(java.lang.Object)[PlayApplicationSpec.platform(java.lang.Object)] on the Play application component.
...
.build.gradle
----
include::{samples-path}/play/advanced/build.gradle[tag=play-platform]
----
...
You can add compile, test and runtime dependencies to a Play application through link:{groovy-dsl-path}/org.gradle.api.artifacts.Configuration.html[Configuration] created by the Play plugin.
...
.build.gradle
----
include::{samples-path}/play/advanced/build.gradle[tag=play-dependencies]
----
...
.build.gradle
----
include::{samples-path}/play/play-2.6/build.gradle[tag=play-platform-2.6]
----
...
.build.gradle
----
include::{samples-path}/play/play-2.6/build.gradle[tag=play-dependencies-2.6]
----
...
.build.gradle
----
include::{samples-path}/play/sourcesets/build.gradle[tag=default-sourcesets]
----
...
.build.gradle
----
include::{samples-path}/play/sourcesets/build.gradle[tag=play-extra-sourcesets]
----
...
.build.gradle
----
include::{samples-path}/play/configure-compiler/build.gradle[tag=play-configure-compiler]
----
...
.build.gradle
----
include::{samples-path}/play/play-2.4/build.gradle[tag=play-injected-routes-compiler]
----
...
A custom Twirl template format can be configured independently for each Twirl source set. See the link:{javadoc-path}/org/gradle/language/twirl/TwirlSourceSet.html[TwirlSourceSet] for an example.

[[sec:injecting_a_custom_asset_pipeline]]
=== Injecting a custom asset pipeline

Gradle Play support comes with a simplistic asset processing pipeline that minifies JavaScript assets. However, many organizations have their own custom pipeline for processing assets. You can easily hook the results of your pipeline into the Play binary by utilizing the link:{javadoc-path}/org/gradle/play/PublicAssets.html[PublicAssets] property on the binary.
...
.build.gradle
----
include::{samples-path}/play/custom-assets/build.gradle[tag=custom-assets]
----
...
.build.gradle
----
include::{samples-path}/play/multiproject/build.gradle[tag=play-multiproject-dependencies]
----
...
`createPlayBinaryStartScripts` — type: link:{javadoc-path}/org/gradle/api/tasks/application/CreateStartScripts.html[CreateStartScripts]::
+
Generates scripts to run the Play application distribution.

`stagePlayBinaryDist` — type: link:{groovy-dsl-path}/org.gradle.api.tasks.Copy.html[Copy]::
...
`createPlayBinaryZipDist` — type: link:{groovy-dsl-path}/org.gradle.api.tasks.bundling.Zip.html[Zip]::
+
Bundles the Play application as a standalone distribution packaged as a zip.

`createPlayBinaryTarDist` — type: link:{groovy-dsl-path}/org.gradle.api.tasks.bundling.Tar.html[Tar]::
+
...
You can add additional files to the distribution package using the link:{javadoc-path}/org/gradle/api/distribution/Distribution.html[Distribution] API.
...
.build.gradle
----
include::{samples-path}/play/custom-distribution/build.gradle[tag=play-custom-distribution]
----
...
.build.gradle
----
include::{samples-path}/play/configure-compiler/build.gradle[tag=play-idea]
----
...
** link:{javadoc-path}/org/gradle/play/PlayApplicationBinarySpec.html[PlayApplicationBinarySpec]
** link:{javadoc-path}/org/gradle/play/PlayApplicationSpec.html[PlayApplicationSpec]
** link:{javadoc-path}/org/gradle/play/platform/PlayPlatform.html[PlayPlatform]
** link:{javadoc-path}/org/gradle/play/JvmClasses.html[JvmClasses]
** link:{javadoc-path}/org/gradle/play/PublicAssets.html[PublicAssets]
** link:{javadoc-path}/org/gradle/play/distribution/PlayDistributionContainer.html[PlayDistributionContainer]
** link:{groovy-dsl-path}/org.gradle.play.tasks.JavaScriptMinify.html[JavaScriptMinify]
** link:{groovy-dsl-path}/org.gradle.play.tasks.PlayRun.html[PlayRun]
** link:{groovy-dsl-path}/org.gradle.play.tasks.RoutesCompile.html[RoutesCompile]
** link:{groovy-dsl-path}/org.gradle.play.tasks.TwirlCompile.html[TwirlCompile]
