...
tasks.register("aVerificationTask") {
    check.dependsOn it
    // ...
}
----
Executing the `check` task won't follow the dependency to `aVerificationTask`.
Nothing causes the task to realized.
The dependency between `aVerificationTask` and `check` task won't register.
To work around this issue, you need to modify the other tasks outside of configuration action.
