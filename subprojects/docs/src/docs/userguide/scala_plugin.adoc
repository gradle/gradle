...
.build.gradle
----
include::{samples-path}/scala/quickstart/build.gradle[tag=use-plugin]
----
...
`compileScala` — type: link:{groovy-dsl-path}/org.gradle.api.tasks.scala.ScalaCompile.html[ScalaCompile]::
_Depends on_: `compileJava`
+
Compiles production Scala source files.

`compileTestScala` — type: link:{groovy-dsl-path}/org.gradle.api.tasks.scala.ScalaCompile.html[ScalaCompile]::
_Depends on_: `compileTestJava`
+
Compiles test Scala source files.

`compile__SourceSet__Scala` — type: link:{groovy-dsl-path}/org.gradle.api.tasks.scala.ScalaCompile.html[ScalaCompile]::
_Depends on_: `compile__SourceSet__Java`
+
Compiles the given source set's Scala source files.

`scaladoc` — type: link:{groovy-dsl-path}/org.gradle.api.tasks.scala.ScalaDoc.html[ScalaDoc]::
+
...
.build.gradle
----
include::{samples-path}/scala/customizedLayout/build.gradle[tag=custom-source-locations]
----
...
Scala projects need to declare a `scala-library` dependency. This dependency will then be used on compile and runtime class paths. It will also be used to get hold of the Scala compiler and Scaladoc tool, respectively.footnote:[See <<scala_plugin.adoc#sec:configure_scala_classpath,Automatic configuration of Scala classpath>>.]
...
.build.gradle
----
include::{samples-path}/scala/quickstart/build.gradle[tag=scala-dependency]
----
...
.build.gradle
----
include::{samples-path}/userguide/scala/scalaDependency/build.gradle[tag=scala-test-dependency]
----
...
.build.gradle
----
include::{samples-path}/userguide/scala/zincDependency/build.gradle[tag=zinc-dependency]
----
...
.build.gradle
----
include::{samples-path}/userguide/scala/zincOverride/build.gradle[tag=all-dependency]
----
...
.build.gradle
----
include::{samples-path}/userguide/scala/zincOverride/build.gradle[tag=zinc-dependency]
----
...
`scala` — type: link:{groovy-dsl-path}/org.gradle.api.file.SourceDirectorySet.html[SourceDirectorySet] (read-only)::
...
`allScala` — type: link:{javadoc-path}/org/gradle/api/file/FileTree.html[FileTree] (read-only)::
...
These convention properties are provided by a convention object of type link:{groovy-dsl-path}/org.gradle.api.tasks.ScalaSourceSet.html[ScalaSourceSet].
...
.build.gradle
----
include::{samples-path}/scala/zinc/build.gradle[tag=adjust-memory]
----
...
.build.gradle
----
include::{samples-path}/scala/force/build.gradle[tag=force-compilation]
----
...
.gradle.properties
----
include::{samples-path}/scala/crossCompilation/gradle.properties[]
----

[source.multi-language-sample,groovy]
.build.gradle
----
include::{samples-path}/scala/crossCompilation/build.gradle[tag=scala-cross-compilation]
----
...
When the IDEA plugin encounters a Scala project, it adds additional configuration to make the project work with IDEA out of the box. Specifically, the plugin adds a Scala SDK (IntelliJ IDEA 14+) and a Scala compiler library that matches the Scala version on the project's class path. The Scala plugin is backwards compatible with earlier versions of IntelliJ IDEA and it is possible to add a Scala facet instead of the default Scala SDK by configuring `targetVersion` on link:{groovy-dsl-path}/org.gradle.plugins.ide.idea.model.IdeaModel.html[IdeaModel].
...
.build.gradle
----
include::{samples-path}/userguide/scala/ideaTargetVersion/build.gradle[tag=scala-idea-target-version]
----
