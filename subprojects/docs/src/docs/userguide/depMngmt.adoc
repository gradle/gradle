...
When using a GCP GCS backed repository default application credentials will be used with no further configuration required:

++++
<sample id="mavenIvyGCSRepositoriesAuthDefault" dir="userguide/artifacts/defineRepository" title="Declaring a GCS backed Maven and Ivy repository using default application credentials">
...
The following system properties can be used to configure the interactions with gcs repositories:
...
| Used to override the GCP GCS endpoint when using a non GCP, GCS API compatible, storage service.

| org.gradle.gcs.servicePath
| Used to override the GCP GCS root service path which the GCS client builds requests from, defaults to `/`.
...
GCS URL's are 'virtual-hosted-style' and must be in the following format `gcs://&lt;bucketName&gt;/&lt;objectKey&gt;`

e.g. `gcs://myBucket/maven/release`

* `myBucket` is the GCP GCS bucket name.
* `/maven/release` is the GCP GCS key (unique identifier for an object within a bucket)
