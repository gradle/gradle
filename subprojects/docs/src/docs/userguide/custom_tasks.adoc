...
There are 2 ways that a task can receive the services that it needs. One option is to inject the services as parameters of the task class constructor. The constructor must be annotated with the `javax.inject.Inject` annotation.

Alternatively, a service can be injected as a property of the task using a getter method annotated with the `javax.inject.Inject` annotation. This can be useful, for example, when you cannot change the constructor of the task class due to backwards compatibility constraints. This pattern also allows Gradle to defer creation of the service until it is required, rather than when the task instance is created, which helps with performance.
