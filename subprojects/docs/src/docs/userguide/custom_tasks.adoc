...
The usage of this API is deprecated, since it does not allow querying individual input properties for changes.
The `IncrementalTaskInputs` API will be eventually removed.
...
The parameters for the unit of work are defined as an interface that extends link:{javadocPath}/org/gradle/workers/WorkParameters.html[WorkParameters].  The implementation is a class that extends link:{javadocPath}/org/gradle/workers/WorkAction.html[WorkAction]. This class should be abstract and should not implement the `getParameters()` method (Gradle will inject this method at runtime with the parameters object for each unit of work).
