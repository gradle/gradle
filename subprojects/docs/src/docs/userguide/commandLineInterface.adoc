...
Options that accept values can be specified with or without `=` between the option and argument. The following are equivalent, but the `=` variant makes it clear that `plain` is an argument to `--console` and not a task name.
----
--console=plain
--console plain
----
...
It is common for tests to be run using the `test` task, which assembles the project and executes tests.

----
❯ gradle test
----

If you'd like to skip running tests, when running another task, you can use the `--exclude-task` option.
