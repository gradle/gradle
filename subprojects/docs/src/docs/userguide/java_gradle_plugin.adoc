...
* Configure the <<publishing_maven,Maven>> or <<publishing_ivy,Ivy Publish Plugins>> to publish a <<sec:plugin_markers,Plugin Marker Artifact>> for each plugin.
* Moreover, if the https://plugins.gradle.org/docs/publish-plugin[Plugin Publishing Plugin] is applied, it will publish each plugin using the same name, plugin id, display name, and description to the Gradle Plugin Portal (see https://guides.gradle.org/publishing-plugins-to-gradle-plugin-portal/[Publishing Plugins to Gradle Plugin Portal] for details).
