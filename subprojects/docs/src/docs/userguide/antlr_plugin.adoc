...
.build.gradle
----
include::{samples-path}/antlr/build.gradle[tag=use-plugin]
----
...
`generateGrammarSource` — type: link:{groovy-dsl-path}/org.gradle.api.plugins.antlr.AntlrTask.html[AntlrTask]::
Generates the source files for all production ANTLR grammars.

`generateTestGrammarSource` — type: link:{groovy-dsl-path}/org.gradle.api.plugins.antlr.AntlrTask.html[AntlrTask]::
Generates the source files for all test ANTLR grammars.

`generate__SourceSet__GrammarSource` — type: link:{groovy-dsl-path}/org.gradle.api.plugins.antlr.AntlrTask.html[AntlrTask]::
...
.build.gradle
----
include::{samples-path}/antlr/build.gradle[tag=declare-dependency]
----
...
`antlr` — type: link:{groovy-dsl-path}/org.gradle.api.file.SourceDirectorySet.html[SourceDirectorySet]::
...
The ANTLR tool is executed in a forked process. This allows fine grained control over memory settings for the ANTLR process. To set the heap size of an ANTLR process, the `maxHeapSize` property of link:{groovy-dsl-path}/org.gradle.api.plugins.antlr.AntlrTask.html[AntlrTask] can be used. To pass additional command-line arguments, append to the `arguments` property of link:{groovy-dsl-path}/org.gradle.api.plugins.antlr.AntlrTask.html[AntlrTask].
...
.build.gradle
----
include::{samples-path}/antlr/build.gradle[tag=generate-grammar-settings]
----
