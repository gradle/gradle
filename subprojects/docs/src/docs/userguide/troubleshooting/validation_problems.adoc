...
From Java code, it is quite easy to accidentally use a lambda to implement a task action.
...
Gradle isn't able to track the implementation of this task action.
What you can do instead is to use an anonymous inner class.
...
Starting with Gradle 7.5, `.gradle.kts` scripts and plugins using the `kotlin-dsl` plugin are compiled with `-Xsam-conversion=class`.
This instructs the Kotlin compiler to produce classes instead of lambdas for SAM wrappers.
It is not needed to stick with Kotlin 1.4 api/language anymore.
