...
`java.net.URL`:: This type cannot be annotated with `@Input` because Java Serialization can lead to the same object of this type being detected as different by Gradle. This is a known issue in Java, see the https://bugs.openjdk.org/browse/JDK-8075619[OpenJDK bug report] for more details.
