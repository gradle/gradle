...
[[cannot_write_output]]
== Cannot write to an output directory

This error indicates that an output directory cannot be written because the directory property which has been configured actually refers to a regular file (or something else than an actual directory).
...
To fix this problem, make sure that the configured output is actually a directory.
