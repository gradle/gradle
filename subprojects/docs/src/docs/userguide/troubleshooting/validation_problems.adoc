...
Nested types must declare annotated properties, which themselves are checked for annotations.
Types of the Java SE API, types of the Kotlin stdlib, and Groovy's GString are not supported, because they cannot have any annotated properties.
This includes primitive wrapper types such as `Integer` as well as types like `String` or `File`.

To fix this problem, declare annotated properties on the nested type, e.g. `Provider<T>`, `Iterable<T>`, or `<MapProperty<K, V>>`, where `T` and `V` must have one or more annotated properties.
