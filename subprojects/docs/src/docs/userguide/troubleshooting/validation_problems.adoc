...
or rely on Gradle's built-in capacity to inject final properties:
...
For example, a property cannot be annotated both with `@InputFile` and `@InputArtifact` at the same time.

To fix this problem, you need to verify what are the semantics of the annotations and select one.
