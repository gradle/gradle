...
Gradle currently isn't able to track the implementation of this task action.
...
For the second case, instead of loading your class by a classloader unknown to Gradle, you can use Gradle's built-in ways to load the class.
For example, the <<worker_api.adoc#,worker API>> allows using a different classloader.
