...
Because a Java Platform is a special kind of component, a dependency on a Java platform has to be declared using the `platform` or `enforcedPlatform` keyword, as explained in the <<controlling_transitive_dependencies.adoc#sub:bom_import,managing transitive dependencies>> section.
