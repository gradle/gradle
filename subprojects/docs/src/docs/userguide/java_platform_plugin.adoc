...
For this purpose, the plugin exposes two <<declaring_dependencies.adoc#what-are-dependency-configurations,configurations>> that can be used to declare dependencies: `api` and `runtime`.
...
Because a Java Platform is a special kind of component, a dependency on a Java platform has to be declared using the `platform` or `enforcedPlatform` keyword, as explained in the <<declaring_dependencies.adoc#sec:bom_import,managing transitive dependencies>> section.
