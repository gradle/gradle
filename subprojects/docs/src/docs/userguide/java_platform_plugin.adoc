...
- a description of modules which are published together and share the same version (and for example, share the same version)
...
While many users will only care about _compile time_ dependencies, it allows declaring dependencies or <<dependency_management_terminology.adoc#sub:terminology_dependency_constraint,constraints>> which only apply to runtime.
...
Because a Java Platform is a special kind of component, a dependency on a Java platform has to be declared using the `platform` or `enforcedPlatform` keyword, as explained in the <<managing_dependency_configurations.adoc#sec:bom_import,managing transitive dependencies>> section.
