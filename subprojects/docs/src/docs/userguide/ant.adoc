...
In your build script, a property called `ant` is provided by Gradle. This is a reference to an link:{groovyDslPath}/org.gradle.api.AntBuilder.html[AntBuilder] instance. This `AntBuilder` is used to access Ant tasks, types and properties from your build script. There is a very simple mapping from Ant's `build.xml` format to Groovy, which is explained below.
...
Sometimes it may be necessary to “rename” the task generated for an Ant target to avoid a naming collision with existing Gradle tasks. To do this, use the link:{javadocPath}/org/gradle/api/AntBuilder.html#importBuild-java.lang.Object,%20org.gradle.api.Transformer-[AntBuilder.importBuild(java.lang.Object, org.gradle.api.Transformer)] method.
...
Note that while the second argument to this method should be a link:{groovyDslPath}/org.gradle.api.Transformer.html[Transformer], when programming in Groovy we can simply use a closure instead of an anonymous inner class (or similar) due to http://mrhaki.blogspot.ie/2013/11/groovy-goodness-implicit-closure.html[Groovy's support for automatically coercing closures to single-abstract-method types].
...
The Ant integration is provided by link:{groovyDslPath}/org.gradle.api.AntBuilder.html[AntBuilder].
