...
Instead, it is recommended to replace the Gradle `force` with a <<rich_versions.adoc#sec:strict-version,`strictly` version>>, which provides <<dependency_constraints.adoc#sec:enforcing_dependency_version,better semantics>> and is supported by the Gradle Module Metadata format.
