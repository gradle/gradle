...
Gradle is a https://en.wikipedia.org/wiki/Build_automation[build automation] tool. It is available as an open-source Build Tool and an Enterprise tool.
...
https://gradle.com/[Gradle Enterprise] is a powerful tool for managing and monitoring complex software builds. It provides teams with centralized visibility into their build process to quickly identify and address issues as they arise.
...
Gradle is based on the following fundamentals:

.Gradle fundamentals
image::what-is-gradle.png[]

Users provide instructions to Gradle using either Groovy or Kotlin in a script to build a Project (your code). The build script contains default Tasks and optional custom Tasks that can further define build steps. If your Project uses Plugins and other Dependencies, they can be managed by Gradle's Dependency Manager.
