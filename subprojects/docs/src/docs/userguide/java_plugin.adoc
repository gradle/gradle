...
The Java plugin adds a number of <<declaring_dependencies.adoc#what-are-dependency-configurations,dependency configurations>> to your project, as shown below. Tasks such as `compileJava` and `test` then use one or more of those configurations to get the corresponding files and use them, for example by placing them on a compilation or runtime classpath.
