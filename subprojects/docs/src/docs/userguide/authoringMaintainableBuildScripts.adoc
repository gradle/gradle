...
Use of Gradle internal APIs in plugins and build scripts causes builds to break when either Gradle or plugins change.
...
Instead of `org.gradle.internal.os.OperatingSystem`, use another method to detect operating system, such as `System.getProperty("os.name")`.
...
[[sec:avoiding_execution_actions_during_configuration]]
=== Avoiding execution actions during Gradle configuration
