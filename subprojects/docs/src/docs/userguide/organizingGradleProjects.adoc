...
=== Standardizing builds with a custom distribution

<<init_scripts,Initialization scripts>> make it extremely easy to apply logic across all projects on a machine. A typical use case for setting up an initialization script is specify an in-house binary repository including its credentials. Often times enterprises would like to standardize the build platform for all projects in the organization. You achieve that with initialization script though you'd have to communicate the setup process to all developers in the company. Another challenge is updating the initialization script logic or enforcing the use it.

Custom Gradle distributions are a practical solution to this very problem. A custom Gradle distribution is the standard Gradle distribution plus additional initialization scripts. The initialization scripts come bundled with the distribution and are applied every time the build is run. Developers would point to the custom distribution as part in their checked-in <<gradle_wrapper,Wrapper>> files.

Let's talk about the process for creating a custom Gradle distribution.
...
4. Upload the the Gradle distribution archive to a HTTP server.
