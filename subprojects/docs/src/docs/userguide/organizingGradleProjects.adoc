...
For example you may want pass in a specific property value to control runtime behavior just for this one invocation of the build.
...
Intermingling or overwriting output files produced by different tasks compromises up to date checking leading to longer build times.
In turn, Gradle's <<build_cache,build cache>> won't work properly either as incremental task are a foundational mechanism to making a task cacheable.
...
The drawback is that you will have to communicate the setup process across all developers in the company.
Another challenge is updating the initialization script logic uniformly or enforcing the use it in all developer and Continuous Integration environments.
