...
[[ant]]
= Build Ant Projects with Gradle
...
In your build script, a property called `ant` is provided by Gradle. This is a reference to an link:{javadocPath}/org/gradle/api/AntBuilder.html[AntBuilder]
instance. This `AntBuilder` is used to access Ant tasks, types and properties from your build script. There is a very simple mapping from Ant's `build.xml` format to Groovy, which is explained below.
