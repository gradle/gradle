...
- <<build_lifecycle.adoc#sec:settings_file,Settings scripts>>
...
Almost all top-level properties and blocks in a build script are part of the `Project` API.
...
of link:https://gradle.github.io/kotlin-dsl-docs/api/org.gradle.kotlin.dsl/-kotlin-build-script/index.html[KotlinBuildScript].
As a result, builds scripts have access to all visible properties and functions declared by the `KotlinBuildScript` type.
...
Groovy provides many features to help create DSLs. These features can help you write better build scripts, plugins, and tasks.
...
The following list shows the imports Gradle adds to each script:
