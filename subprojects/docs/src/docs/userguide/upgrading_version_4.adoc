 ...
 * The `gradlePluginPortal()` repository no more infers artifact metadata from artifact files and now <<#rel5.0:gradle_plugin_portal_metadata,only uses Maven POMs>>.
...
=== [5.0] `gradlePluginPortal()` repository now fetches artifact metadata from Maven POMs only

The `gradlePluginPortal()` repository is now stricter when resolving artifact metadata, it doesn't infer the metadata from the sole presence of artifact files such as JARs. Only Maven POMs are now considered.

This repository is used by default when resolving plugins from the `plugins {}` block.
