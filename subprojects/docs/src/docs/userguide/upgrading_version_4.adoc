 ...
 * The `forkEvery` property of the `Test` task type now doesn't accept being set to `null`, use `0` to specify no maximum instead.
 * The `gradleUserHomeDir`, `currentDir` and `taskNames` properties of `StartParameter` no longer accept being set to `null`, use the corresponding `resetGradleUserHomeDir()`, `resetCurrentDir()` and `resetTaskNames()` methods to reset to defaults instead.
 * The `tasks` property of `GradleBuild` no longer accept being set to `null`, use an empty list instead.
