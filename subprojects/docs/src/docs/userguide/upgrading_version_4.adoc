...
To use the new approach, simply replace any `upload<Conf>` configuration with a `publishing {}` block. See the <<publishing.adoc#publishing_overview,publishing overview chapter>> for more information.
