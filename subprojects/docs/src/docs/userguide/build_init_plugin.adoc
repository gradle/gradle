...
The Gradle Build Init plugin can be used to create a new Gradle build. It supports creating brand new Gradle builds of different types as well as converting existing Apache Maven builds to Gradle.
...
The Build Init plugin supports generating various different _types_ of builds. These are listed below and more detail is available in the following <<sec:build_init_types,section>>.
...
|<<sec:kotlin_application,kotlin-application>>|A command-line application implemented in Kotlin
|<<sec:kotlin_gradle_plugin,kotlin-gradle-plugin>>|A Gradle plugin implemented in Kotlin
...
* `gradle init --type java-application --test-framework junit-jupiter`: Uses http://juni.org[Junit 5] for testing instead of JUnit 4
...
=== `java-gradle-plugin` build init type

The “`java-gradle-plugin`” build init type is not inferable. It must be explicitly specified.
...
=== `kotlin-gradle-plugin` build init type

The “`kotlin-gradle-plugin`” build init type is not inferable. It must be explicitly specified.
...
=== `groovy-gradle-plugin` build init type

The “`groovy-gradle-plugin`” build init type is not inferable. It must be explicitly specified.
...
The “`basic`” build init type is useful for creating a fresh new Gradle build. It creates sample settings and build files, with comments and links to help get started.
