...
A custom task implementation can expose properties to make the runtime behavior configurable. Sometimes a user wants to declare the value of an exposed task property on the command line instead of the build script. By annotating a setter method of a property with api:org.gradle.api.tasks.options.Option[], Gradle automatically exposes a new command line option with a specific name.

[[sec:declaring_task_option]]
==== Declaring an option

Let's have a look at an example to illustrate the functionality. The custom task `UrlVerify` verifies whether a given URL can be resolved by making a HTTP call and checking the response code. The URL to be verified is configurable through the property `url`. The setter method for the property is annotated with api:org.gradle.api.tasks.options.Option[]. An option requires a mandatory identifier. Additionally, you can provide an optional description.
...
All options declared for a task can be <sec:listing_task_options,rendered as console output>> by running the `help` task.

[[sec:using_task_option_command_line]]
==== Using an option from the command line

Using an option on the command line has to adhere to the following rules:

- The option uses a double-dash as prefix e.g. `--url`. A single dash is not valid notation.
- The option parameter follows directly after the task declaration e.g. `verifyUrl --url=http://www.google.com/`.
- Multiple options of a task can be declared in any order on the command line after the task name.
...
Gradle limits the set of data types that can be used for declaring command line options. Be aware that their use on the command line differs.

`boolean`, `Boolean`::
Describes an option with the value `true` or `false`. Passing the option on the command line does not require assigning a value. For example `--enabled` equates to `true`, not the option evaluates to `false`.
...
`Enum`::
Describes an option as enum. The enum has to be passed on the command line as key-value pair similar to the String type e.g. `--log-level=DEBUG`.

`List<String>`, `List<Enum>`::
Describes an option that can takes multiple values of a given type. The values for the option have to be provided as distinct declarations e.g. `--imageId=123 --imageId=456`. Other notations like comma-separated list or multiple values separated by a space character are currently not supported.

[[sec:documenting_available_task_option_values]]
==== Documenting available values for an option

In theory, an option for a property of type `String` or `List<String>` can accept any arbitrary value. Expected values for such an option can be documented programmatically with the help of the annotation api:org.gradle.api.tasks.options.OptionValues[]. This annotation can be assigned to any method that returns a `List` of one of the supported data types. In addition, you have to provide the option identifier to indicated the relationship between option and available values.

[NOTE]
====
Passing in a value on the command line that is not supported by the option does not fail the build or throw an exception. If you'd like to see such runtime behavior then you'll have to implement it in the task action.
====
...
++++
<sample id="helpTaskOptions" dir="userguide/tasks/commandLineOption/optionValues/buildSrc/src/main/java" title="Listing available values for option">
...
- Command line options can only be declared for custom tasks via annotation. There's no programmatic equivalent to define options.
- Options cannot be declared globally e.g. on a project-level or as part of a plugin.
- When assigning an option on the command line then the task exposing the option needs to be spelled out e.g. `gradle check --tests abc` does not work even though the `check` task depends on the `test` task.
