...
To create a Gradle plugin, you need to write a class that implements the link:{javadoc-path}/org/gradle/api/Plugin.html[Plugin] interface. When the plugin is applied to a project, Gradle creates an instance of the plugin class and calls the instance's link:{javadoc-path}/org/gradle/api/Plugin.html#apply-T-[Plugin.apply()] method. The project object is passed as a parameter, which the plugin can use to configure the project however it needs to. The following sample contains a greeting plugin, which adds a `hello` task to the project.
...
.build.gradle
----
include::{samples-path}/userguide/customPlugins/customPlugin/build.gradle[]
----

.Output of **`gradle -q hello`**
----
> gradle -q hello
include::{samples-path}/userguide/customPlugins/customPlugin/customPlugin.out[]
----

One thing to note is that a new instance of a plugin is created for each project it is applied to. Also note that the link:{javadoc-path}/org/gradle/api/Plugin.html[Plugin] class is a generic type. This example has it receiving the link:{groovy-dsl-path}/org.gradle.api.Project.html[Project] type as a type parameter. A plugin can instead receive a parameter of type link:{groovy-dsl-path}/org.gradle.api.initialization.Settings.html[Settings], in which case the plugin can be applied in a settings script, or a parameter of type link:{groovy-dsl-path}/org.gradle.api.invocation.Gradle.html[Gradle], in which case the plugin can be applied in an initialization script.

[[sec:getting_input_from_the_build]]
== Making the plugin configurable

Most plugins need to obtain some configuration from the build script. One method for doing this is to use _extension objects_. The Gradle link:{groovy-dsl-path}/org.gradle.api.Project.html[Project] has an associated link:{javadoc-path}/org/gradle/api/plugins/ExtensionContainer.html[ExtensionContainer] object that contains all the settings and properties for the plugins that have been applied to the project. You can provide configuration for your plugin by adding an extension object to this container. An extension object is simply a Java Bean compliant class. Groovy is a good language choice to implement an extension object because plain old Groovy objects contain all the getter and setter methods that a Java Bean requires. Java and Kotlin are other good choices.
...
.build.gradle
----
include::{samples-path}/userguide/customPlugins/customPluginWithConvention/build.gradle[]
----

.Output of **`gradle -q hello`**
----
> gradle -q hello
include::{samples-path}/userguide/customPlugins/customPluginWithConvention/customPluginWithConvention.out[]
----
...
.build.gradle
----
include::{samples-path}/userguide/customPlugins/customPluginWithAdvancedConvention/build.gradle[]
----

.Output of **`gradle -q hello`**
----
> gradle -q hello
include::{samples-path}/userguide/customPlugins/customPluginWithAdvancedConvention/customPluginWithAdvancedConvention.out[]
----
...
When developing custom tasks and plugins, it's a good idea to be very flexible when accepting input configuration for file locations. To do this, you can leverage the link:{groovy-dsl-path}/org.gradle.api.Project.html#org.gradle.api.Project:file(java.lang.Object)[Project.file(java.lang.Object)] method to resolve values to files as late as possible.
...
.build.gradle
----
include::{samples-path}/userguide/tasks/customTaskWithFileProperty/build.gradle[tags="task,config"]
----

.Output of **`gradle -q sayGreeting`**
----
> gradle -q sayGreeting
include::{samples-path}/userguide/tasks/customTaskWithFileProperty/lazyFileProperties.out[]
----

In this example, we configure the `greet` task `destination` property as a closure, which is evaluated with the link:{groovy-dsl-path}/org.gradle.api.Project.html#org.gradle.api.Project:file(java.lang.Object)[Project.file(java.lang.Object)] method to turn the return value of the closure into a `File` object at the last minute. You will notice that in the example above we specify the `greetingFile` property value after we have configured to use it for the task. This kind of lazy evaluation is a key benefit of accepting any value when setting a file property, then resolving that value when reading the property.
...
.build.gradle
----
include::{samples-path}/userguide/tasks/mapExtensionPropertiesToTaskProperties/build.gradle[]
----
...
----
> gradle -q hello
include::{samples-path}/userguide/tasks/mapExtensionPropertiesToTaskProperties/mapExtensionPropertiesToTaskProperties.out[]
----
...
So how does Gradle find the link:{javadoc-path}/org/gradle/api/Plugin.html[Plugin] implementation? The answer is you need to provide a properties file in the jar's `META-INF/gradle-plugins` directory that matches the id of your plugin.
...
----
include::{samples-path}/customPlugin/plugin/src/main/resources/META-INF/gradle-plugins/org.samples.greeting.properties[]
----

Notice that the properties filename matches the plugin id and is placed in the resources folder, and that the `implementation-class` property identifies the link:{javadoc-path}/org/gradle/api/Plugin.html[Plugin] implementation class.
...
.build.gradle
----
include::{samples-path}/customPlugin/consumer/build.gradle[tag=use-plugin]
----
...
You can use the link:{javadoc-path}/org/gradle/testfixtures/ProjectBuilder.html[ProjectBuilder] class to create link:{groovy-dsl-path}/org.gradle.api.Project.html[Project] instances to use when you test your plugin implementation.
...
----
include::{samples-path}/customPlugin/plugin/src/test/groovy/org/gradle/GreetingPluginTest.groovy[tag=test-plugin]
----
...
When Gradle creates a task or extension object, Gradle _decorates_ the implementation class to mix in DSL support. To create a nested DSL element you can use the link:{javadoc-path}/org/gradle/api/model/ObjectFactory.html[ObjectFactory] type to create objects that are similarly decorated. These decorated objects can then be made visible to the DSL through properties and methods of the plugin's extension:
...
.build.gradle
----
include::{samples-path}/userguide/customPlugins/customPluginWithNestedDsl/build.gradle[]
----

.Output of **`gradle -q hello`**
----
> gradle -q hello
include::{samples-path}/userguide/customPlugins/customPluginWithNestedDsl/nestedDslElement.out[]
----
