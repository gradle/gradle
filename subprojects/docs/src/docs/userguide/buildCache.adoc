...
Missing task inputs can cause incorrect cache hits, where different results are treated as identical because the same cache key is used by both executions. Missing task outputs can cause build failures if Gradle does not completely capture all outputs for a given task.

A good way to decide if something should be considered an input or not is to check if it has a _direct_ influence on the files the task produces. When there's a direct impact (consider the `encoding` property for a task producing text files), then it should be declared as an input. If there's no impact at all on the files (think of a `verbose` option only affecting the console output), then it's not an input.

It is also possible that multiple properties are combined into a value that has an effect on the output, but not the individual components. For example, a task might have an input property specifying a source directory, which could be combined with another property declaring a filter. Declaring the combined value as input instead of the individual properties is better, as it allows Gradle to ignore changes in the individual property values. In the previous example, we could track the files that matched the filter. This way irrelevant changes to the filter could be safely ignored by Gradle, resulting in fewer unnecessary cache misses.
...
[[sec:build_cache_http_backend]]
=== How to set up an HTTP build cache backend

If you are using Gradle Enterprise, you should use the included https://docs.gradle.com/enterprise/build-cache/[HTTP build cache backend].

One way to set up an HTTP build cache backend is to configure https://www.nginx.com/[nginx] accordingly.
