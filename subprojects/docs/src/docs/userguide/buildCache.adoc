...
For your build to play well with task output caching it is required to work well with the <<sec:up_to_date_checks,incremental build>> feature.
...
Note that `@api:org.gradle.api.tasks.CacheableTask[]` is, like `@api:org.gradle.api.tasks.ParallelizableTask[]`, not inherited to subclasses.
...
NOTE: It's very important that a cacheable task has a complete picture of its inputs so that the results from one build can be safely re-used somewhere else.
...
The task output caching feature has known issues that may impact the correctness, usability and performance of your build when using the build cache.
...
Problems which affect incremental build can also affect task output caching even if the affected tasks are not cacheable.
Most issues can only cause problems if your build cache is populated by non-clean builds or caching has been enabled for unsupported tasks.
...
More precisely, Gradle is not able to determine which outputs belong to which task and will possible store the combined output of both tasks in the build cache.
...
These issues may affect the number of cache hits you may get between machines.
...
For an example on what this could look like see the https://github.com/gradle/gradle-hazelcast-plugin[Gradle Hazelcast plugin].
