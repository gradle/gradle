...
The following table gives a couple use cases and how to combine the different terms for a rich version declaration:

.Rich version use cases
[%header%autowidth,compact]
|===
| Use case | `strictly` | `require` | `prefer` | `rejects` | Locking recommended? | Potential result

| No strong opinion, works with 1.5
|
|
| 1.5
|
...
| Should not be downgraded below 1.5, but can be upgraded
|
| 1.5
|
|
|
| any starting from 1.5, equivalent of `org:foo:1.5`

| Any version between 1.0 and below 2.0, tested with 1.5
| [1.0, 2.0[
|
| 1.5
|
| yes
| any between 1.0 and 2.0 excluded, 1.5 if nobody else cares

| Any version between 1.0 and below 2.0, tested with 1.5, 1.4 known broken
| [1.0, 2.0[
|
| 1.5
| 1.4
| yes
| any between 1.0 and 2.0 excluded, 1.4 excluded, 1.5 if nobody else cares

| Give me whatever the latest release version is, cannot downgrade
|
| `latest.release`
|
|
...
| Give me whatever the latest release version is, previous versions work
|
|
| `latest.release`
|
...
|
|
| 1.5, or failure if another `strict` or `require` constraint disagrees

| No other version than 1.5 or a patch version
| [1.5,1.6[
|
|
|
| yes
| latest release between 1.5 and 1.6 excluded, or failure if another `strict` or `require` constraint disagrees
|===
