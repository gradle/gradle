...
But when your project isn't building as fast as you'd like, it's time to optimize.
...
The same is true for Continuous Integration (CI) builds: the faster they are, the faster you can react to new issues
...
TIP: Staying up-to-date also makes transitioning to the next major version easier, since you'll get early deprecation warnings.
...
Yet by default, Gradle only runs one task at a time.
...
To execute project tasks in parallel by default, add the following setting to the _gradle.properties_ file in the project root:
...
NOTE: Parallel builds require independent subprojects; tasks in different subprojects must not share state.
...
To enable the configuration cache by default, add the following setting to the _gradle.properties_ file in the project root:
...
To make a custom task compatible with incremental build, mark the inputs that impact task output.

For more information about incremental builds, check out the
<<more_about_tasks.adoc#sec:up_to_date_checks,incremental builds documentation>>.
...
To enable the build cache by default, add the following setting to the _gradle.properties_ file in the project root:
...
- If multiple subprojects, but not all, use the script, you could refactor the script and
  all surrounding logic into a custom plugin located in <<organizing_gradle_projects#sec:build_sources,_buildSrc_>>.
...
But Groovy syntax comes with a cost: Groovy is dynamically interpreted. As a result, method calls in Groovy take more time and use
...
==== Evaluation triggers dependency resolution
...
The time spent resolving dependencies during _"project configuration"_ should be 0 seconds.
...
This section discusses task avoidance in your Gradle build.
...
By default, Gradle reserves 1GB of heap space for your build. This is plenty for most projects.
...
- continuous runtime JVM optimization thanks to HotSpot in OpenJDK
- watching the file system to calculate exactly what needs to be rebuilt before you run a build

=== Enable the daemon on older Gradle versions

Gradle 3.0 and above enable the daemon by default. If you are using an older version,
you should <<parallel_execution,update to the latest version of Gradle>>.
If you cannot update your Gradle version, you can enable the daemon at build time with the `daemon` flag:
...
_gradle.properties_ file in the project root:
...
On CI machines, long-lived agents can benefit from the daemon. But short-lived machines won't.
Daemons automatically shut down on memory pressure in Gradle 3.0 and above, so it's safe to leave daemons on.
...
To enable this feature, override the value of `maxParallelForks` on the relevant `Test` task(s).
...
WARNING: Forking a VM is an expensive operation. Setting too small a value here will slow down your tests.
...
- you use build scans, since they provide more information than a local report
...
Or configure the property in the `gradle.properties` file in the root of your project:
...
In Gradle 3.4 and above, ABI-compatible changes no longer trigger recompilation of downstream projects.
This especially improves build times in large multi-project builds with deep dependency chains.

You must upgrade to a Gradle version above 3.4 to benefit from compile avoidance.

Note: If you use annotation processors, you need to explicitly declare them in order for compilation avoidance to work.
To learn more, read the <<java_plugin.adoc#sec:java_compile_avoidance,compile avoidance documentation>>.
...
Implementation dependencies are not leaked into the compile classpath of downstream projects.
As a result, Gradle no longer recompiles them when implementation details change.
...
large multi-project builds. Only the `implementation` configuration is available in the `java` plugin.
Only libraries can define `api` dependencies. Use the
<<java_library_plugin.adoc#java_library_plugin,`java-library`>> plugin to define API dependencies in your libraries.

=== Incremental compilation

Gradle can analyze dependencies down to the individual class level
to recompile only the classes affected by a change.
In Gradle 4.10 and above, incremental compilation is the default.
...
Yet Android also introduces unique opportunities for optimization.
