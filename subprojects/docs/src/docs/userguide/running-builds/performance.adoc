...
To execute project tasks in parallel by default, add the following setting to the `gradle.properties` file in the project root:
...
To enable the configuration cache by default, add the following setting to the `gradle.properties` file in the project root:
...
To enable the build cache by default, add the following setting to the `gradle.properties` file in the project root:
...
If this is the case for you, you can check in this larger memory requirement in your `gradle.properties` file:
...
== Improving the performance of older Gradle releases

Some Gradle projects cannot easily upgrade to a current Gradle version. While you should
