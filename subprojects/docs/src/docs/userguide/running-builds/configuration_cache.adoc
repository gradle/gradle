...
As such, if your build configuration includes any sort of sensitive information, you should ensure
...
via the `GRADLE_ENCRYPTION_KEY` environment variable (see <<#sec:gradle_environment_variables, see Environment variables>>).
...
One way is by using a command like this, which should work on Linux and MacOSx, or even on Windows, if using a tool like Cygwin:
