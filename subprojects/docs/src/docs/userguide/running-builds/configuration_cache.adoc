...
This can happen if you <<configuration_cache#config_cache:usage:ignore_problems,ignored problems>>, or when your build uses <<dynamic_versions#sec:dynamic_versions_and_changing_modules,changing dependency versions>> and remote dependencies have changed for example.
...
[[config_cache:not_yet_implemented:changing_dependencies]]
=== Changing dependencies

<<dynamic_versions#sec:dynamic_versions_and_changing_modules,Dependencies with files that change over time>> aren't supported yet.
With the configuration cache enabled, no problem will be reported and changes to these dependencies won't be detected.

You can force refresh changing dependencies using `--refresh-dependencies`.
