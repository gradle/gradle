...
The first time you run this task, it succeeds. Re-running the task with the configuration cache enabled produces the following error:
----
include::{snippetsPath}/configurationCache/topLevel/tests-groovy/notSupported.out[]
----

To prevent the task from failing when loaded from the configuration cache, convert the referenced top-level method to a static method within a class:
====
