...
Prefix-based filtering is a common scenario, so the support for that <<config_cache:not_yet_implemented:reading_subset_of_system_properties_and_environment_variables, is being added in the future
versions of Gradle>>.
...
[[config_cache:not_yet_implemented:reading_subset_of_system_properties_and_environment_variables]]
=== Reading a subset of system properties or environment variables at configuration time

Currently, it isn't possible to read a subset of system properties or environment variables at configuration time in a way that doesn't make all properties or variables build configuration inputs.

See link:{gradle-issues}13819[gradle/gradle#13819].
