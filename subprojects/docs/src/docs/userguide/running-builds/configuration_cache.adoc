...
See the <<configuration_cache#constraints>> and <<configuration_cache#not_yet_implemented>> sections below for more information.
...
to fix: use <<custom_gradle_types#service_injection,injected services>> instead of the methods of `Project`.

* `project.file()` -> `ProjectLayout`
* `project.files()` / `project.fileTree()` -> `ObjectFactory`
* `project.exec {}` / `project.javaexec {}` -> `ExecOperations`
* `project.copy {}` / `project.sync {}` / `project.delete {}` -> `FileSystemOperations`
...
.Read system properties directly using the Java API
====
...
.Read system properties using the value supplier API
====
[.multi-language-sample]
=====
.build.gradle
...
.Read environment variables directly using the Java API
====
[.multi-language-sample]
=====
.build.gradle
...
.Read environment variables using the value supplier API
====
[.multi-language-sample]
=====
.build.gradle
...
.Read files directly using the language API
====
[.multi-language-sample]
=====
.build.gradle
...
.Read files using the value supplier API
====
[.multi-language-sample]
=====
.build.gradle
...
Problems displayed in the report have links to the corresponding <<configuration_cache#constraints,constraint>> where you can find guidance on how to fix the problem or to the corresponding <<configuration_cache#not_yet_implemented,not yet implemented>> feature.
