...
// TODO java serialization support
...
Please refer to issue link:{gradle-issues}13490[gradle/gradle#13490] to learn about the status of third party plugins.
...
Live JVM state types (e.g. `ClassLoader`, `Thread`, `OutputStream`, `Socket` etc...) are simply forbidden.
...
Referencing heavyweight model objects such as `Configuration`, `Dependency`, `Publication` or `ArtifactResolutionQuery` is forbidden, more precise types must be used instead.
...
Referencing dependency resolution results is also forbidden (e.g. `ArtifactResolutionQuery`, `ResolvedArtifact`, `ArtifactResult` etc...).
...
The rule of thumb here is that tasks must not reference _resolved_ results, but lazy specifications instead, in order to do the dependency resolution at execution time.
...
Note that sometime the disallowed type is indirectly referenced.
...
====
<1> no more problem reported, the injected `ExecOperations` service is supported as a replacement for `project.exec {}`
...
<3> no more problem reported, the task action references `injected` that provides the `ExecOperations` service, supported as a replacement for `project.exec {}`
...
| `project.rootDir`
| A task property or script variable to capture the root directory.

| `project.projectDir`
| link:{javadocPath}/org/gradle/api/file/ProjectLayout.html#getProjectDirectory--[ProjectLayout.projectDirectory]

| `project.buildDir`
| link:{javadocPath}/org/gradle/api/file/ProjectLayout.html#getBuildDirectory--[ProjectLayout.buildDirectory]

| `project.name`
| A task property or script variable to capture the project name.

| `project.description`
| A task property or script variable to capture the project description.

| `project.group`
| A task property or script variable to capture the project group.

| `project.version`
| A task property or script variable to capture the project version.
...
| `project.file(path)`
| link:{javadocPath}/org/gradle/api/file/ProjectLayout.html#getProjectDirectory--[ProjectLayout.projectDirectory].link:{javadocPath}/org/gradle/api/file/Directory.html#file-java.lang.String-[file(path)] or link:{javadocPath}/org/gradle/api/file/ProjectLayout.html#getBuildDirectory--[ProjectLayout.buildDirectory].link:{javadocPath}/org/gradle/api/file/DirectoryProperty.html#file-java.lang.String-[file(path)]
...
See link:{gradle-issues}13508[gradle/gradle#13508].
