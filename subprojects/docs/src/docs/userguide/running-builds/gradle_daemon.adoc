...
When <<sec:daemon_watch_fs,watching the file system>> is enabled, the Daemon can re-use information about the file system from the last build.
For incremental builds, there are only a few changes between builds.
Therefore, incremental builds can re-use most of the information about the file system from the last build and benefit the most from watching the file system.
...
Gradle caches information about the file system in-memory (aka _Virtual File System_, or VFS).
