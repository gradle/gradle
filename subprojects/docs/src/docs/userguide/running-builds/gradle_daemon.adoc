...
If you are using an IDE, then you probably already did increase the limits.
...
----
+
To increase the limit to e.g. 512K watches:

1. Add the following line to either `/etc/sysctl.conf` or a new `*.conf` file under `/etc/sysctl.d/` directory:
+
[source,properties]
----
...
File system watching initializes an Inotify instance per daemon.
While the default per-user instant limit is usually high enough, you may hit that limit if you have many programs running which also use Inotify.
...
----
You can temporarily increase the limit to e.g. 256 instances by running:
+
[source]
----
