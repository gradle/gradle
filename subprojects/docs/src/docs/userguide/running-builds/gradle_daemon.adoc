...
In order to honour the required JVM options for your build, Gradle will normally spawn a separate process for build invocation, even when the Daemon is disabled.
See <<build_environment.adoc#sec:configuring_jvm_memory,Configuring JVM Memory>> for more details.
