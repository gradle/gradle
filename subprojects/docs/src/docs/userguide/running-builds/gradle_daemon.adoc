...
Here are some operating system specific notes.

===  Linux

On Linux, file system watching uses inotify.
Depending on the size of your build, it may be necessary to increase inotify limits.

[[sec:inotify_watches_limit]]
Inotify watches limit::
To raise the inotify watches limit see https://confluence.jetbrains.com/display/IDEADEV/Inotify+Watches+Limit[here].

[[sec:inotify_instances_limit]]
Inotify instances limit::
To raise the inotify instances limit see https://confluence.jetbrains.com/display/IDEADEV/Inotify+Instances+Limit[here].
