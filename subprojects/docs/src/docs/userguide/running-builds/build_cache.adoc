...
The <<organizing_gradle_projects.adoc#sec:build_sources,`buildSrc` directory>> is treated as an <<composite_builds.adoc#composite_build_intro,included build>> as well.
Therefore, it inherits the build cache configuration from the top-level build as well.
