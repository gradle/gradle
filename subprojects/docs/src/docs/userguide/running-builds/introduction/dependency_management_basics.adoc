...
Dependencies in Gradle are grouped into sets called *configurations*.

TIP: A *configuration* is simply a named set of dependencies.

When you’re building and running a Java project there are two classpaths involved:

- *compile classpath* - Dependencies required for the JDK to be able to compile Java code
- *runtime classpath* - Dependencies required to run the compiled Java code

Given the two classpaths, there must be three options to declare our dependencies:
...
The dependencies in this example use the `implementation` dependency configuration as they are needed on both the compile and runtime classpaths.
...
TIP: Local dependencies are not recommended.
