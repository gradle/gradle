...
For example, in order for Gradle to execute the `build` task, the Java code must first be compiled.
...
Task execution order is automatically determined by Gradle.
Build scripts can optionally add task dependency logic.
