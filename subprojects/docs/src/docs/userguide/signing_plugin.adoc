...
Using the following setup, you can pass the key identifier, secret keyring (in ascii-armored format) and the password using the `ORG_GRADLE_PROJECT_signingKeyId`, `ORG_GRADLE_PROJECT_signingKeyring` and `ORG_GRADLE_PROJECT_signingPassword`
