...
[NOTE]
====
If you're using <<toolchains.adoc#toolchains,Java Toolchains>>, the Java major version, the vendor (if specified) and implementation (if specified) will be tracked automatically as an input for compilation
and test execution.
====
...
This can be achieved by using the <<more_about_tasks.adoc#sec:task_input_output_runtime_api,runtime API>> as shown in the following snippet.
