...
The HTTP build cache client allows opt-in use of https://www.w3.org/Protocols/rfc2616/rfc2616-sec8.html#sec8.2.3[HTTP Expect-Continue].
...
This is particularly suitable for cache servers that routinely redirect or reject PUT requests,
as it avoids transmitting the cache entry just to have it rejected (e.g. the cache entry is larger than the cache will allow).
This additional check incurs extra latency when the server accepts the request, but reduces latency when the request is rejected.
...
To opt-in, set link:{groovyDslPath}/org.gradle.caching.http.HttpBuildCache.html#org.gradle.caching.http.HttpBuildCache:allowUntrustedServer[HttpBuildCache.isUseExpectContinue()] to `true`.
