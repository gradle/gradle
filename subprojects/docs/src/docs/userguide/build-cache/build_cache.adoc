...
Servers must take care when redirecting `PUT` requests, to achieve the desired result as only `307` and `308` redirect responses will be followed with a `PUT` request.
...
2. Change the <<build_environment,build environment>> to use an alternative trust store for the build runtime
...
This is particularly suitable for cache servers that routinely redirect or reject upload requests,
as it avoids transmitting the cache entry just to have it rejected (e.g. the cache entry is larger than the cache will allow) or redirected.
