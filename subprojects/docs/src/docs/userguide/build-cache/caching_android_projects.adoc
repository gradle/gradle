...
This defeats <<more_about_tasks.adoc#sec:up_to_date_checks,incremental build>> (i.e., `UP-TO-DATE` checks), but this issue is mitigated via use of the local build cache.
...
Like unit tests in a pure Java project, the equivalent test task in an Android project (`AndroidUnitTest`) is also cacheable thanks to work by the https://issuetracker.google.com/issues/115873047[Google Team].
...
This defeats <<more_about_tasks.adoc#sec:up_to_date_checks,incremental build>>, because each build will be unique.
