...
This defeats <<incremental_builds.adoc#incremental_builds,incremental build>> (i.e., `UP-TO-DATE` checks), but this issue is mitigated via use of the local build cache.
...
This defeats <<incremental_builds.adoc#incremental_builds,incremental build>>, because each build will be unique.
