...
Sometimes you may want to depend on an output produced by another task and in turn make sure that the task is executed beforehand to produce that very output. Declaring a task dependency is a poor way to model this kind of relationship and introduces unnecessary coupling. The recommended way to model such a dependency is to produce the output, mark it as "outgoing" artifact or add it to the output of the `main` source set which you can depend on in the consuming project.

Let's say you are working in a multi-project build with the two subprojects `producer` and `consumer`. The subproject `producer` defines a task that generates a properties file containing build information e.g. the project version. The build also adds the output file of the task `buildInfo`. The attribute `builtBy` takes care of establishing an inferred task dependency. For more information on `builtBy`, see api:org.gradle.api.tasks.SourceSetOutput[].
...
The consuming project is supposed to be able to read the properties file at runtime. To do so the properties file needs to be available in the runtime classpath of the project.
