...
Gradle supports the concept of _feature variants_:
When building a library, it's often the case that some features are only available when some dependencies are present, or when special artifacts are used.

Feature variants will let consumers choose what _features_ they need, and the dependency management engine is going to select the right artifacts and dependencies based on those requirements.

This allows a number of different scenarios (list is non exhaustive):

- a (better) substitute to {maven-optional-deps}
- a _main_ library is built with support for different runtime features, and the user has to choose between one of them
...
All variants of a component provide a number of _capabilities_, which are denoted similarly using GAV coordinates.
...
A typical component will *only* provide variants with the default capability. A Java library, for example, exposes two variants (API and runtime) which provide the _same capability_.
...
This will automatically setup a number of things for you, in a very similar way to what the <<java_library_plugin.adoc,_Java Library Plugin>> defines configurations:
...
For example, here the group is `org.gradle.demo`, the name of the main component is `provider`, version is `1.0` and the feature is named `mongodbSupport`, so the feature variant will be `org.gradle.demo:provider-mongodb-support:1.0`.
...
- using {metadata-file-spec}[Gradle metadata], everything is published and consumers will get the full benefit of feature variants
...
The Java Plugin will take care of registering the additional variants for you, so there's no additional configuration required, only the regular publications:
...
The rule is simple: it's not allowed to have two variants of components that provide the same capability in a dependency graph.
...
trying to resolve would lead to an error:
...
Because both variants provide the same capability.