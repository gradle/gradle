...
| `Iterable&lt;File&gt;`*
| An iterable of input files and directories that represent a Java compile classpath. This allows the task to ignore irrelevant changes that do not affect the API of the classes in classpath. See also <<sec:task_input_differences_input_files_classpath>>.
...
[[sec:task_input_differences_input_files_classpath]]
====== Differences between `@InputFiles`, `@Classpath` and `@CompileClasspath`

For a `File` input, there are basically three different ways how Gradle detects changes to the inputs.
Additionally to looking at the plain file contents and file names, Gradle can also interpret the files as a runtime or compile classpath and only detect changes relevant to these use cases.

Let us look at different kinds of changes to a set of input files and how it would affect a task being up to date with regards to the different input types.
Let us assume that we have an input property that contains the following files:

 * a directory `classes` containing
 ** a class `Foo.class` compiled from `public class Foo {}`
 ** a resource `test.properties` with contents `foo=bar`
 * a jar file `other-project.jar` containing some classes and resources

The following table shows what kind of change to the files in this file collection will make the task out of date, depending on the input property annotation.

.Effects of changes on input types
...
| Foo.class compiled from `public class Foo { private someMethod() {} }`
| change detected
| change detected
| no change

| Foo.class compiled from `public class Foo { public someMethod() {} }`
| change detected
| change detected
| change detected

| `other-project.jar` recreated with same contents
| change detected
| no change
| no change

| resource `test.properties` contents changed to `foo=baz`
| change detected
| change detected
| no change

| `classes` directory renamed to `my-classes`
| change detected
| no change
| no change

| order of `classes` and `other-project.jar` changes within the input property
| no change
| change detected
| change detected
|===

We see that `@CompileClasspath` and `@Classpath` are treating a `FileCollection` as a classpath.
This means that the order of the entries is important while the file name of the entries is not.
Moreover, both ignore timestamps and order of the zip entries in jar files.
`PathSensitivity` is automatically done in the right way, i.e. the names of classpath entries like `classes` or `other-project.jar` are ignored, while the relative path to the parent entry is taken into account for resources on the classpath.

`@CompileClasspath` instructs Gradle to even do a deeper analysis of the classpath and extracts the ABI as described in <<sec:java_compile_avoidance>>.
