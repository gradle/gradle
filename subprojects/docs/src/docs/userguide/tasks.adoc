...
It is also possible to customize Gradle's built in normalization strategies and provide the information to Gradle what is and what is not considered a difference in inputs.

Currently, customization is only available for runtime classpath inputs. These are all inputs annotated with `@api:org.gradle.api.tasks.Classpath[]`.

Let's say you want to add a file `build-info.properties` to all your produced jar files which contains information about the build, e.g. the timestamp when the build started or the `BUILD_ID` when running on CI.
This file is only for auditing purposes but has no effect on the outcome of running tests.
Still, when running the test task the file would be on the classpath and would cause the `test` task to be executed since Gradle detects the change of the timestamp in `build-info.properties` every time the build is run.
In order to benefit from incremental builds again, you are able tell Gradle to ignore this file on the classpath at the project level by using api:org.gradle.api.Project#normalization(org.gradle.api.Action)[]:
