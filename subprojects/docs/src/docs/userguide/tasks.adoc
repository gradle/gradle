...
For JVM-related tasks there are two additional ways for Gradle to detect changes to file inputs:
Gradle can interpret the files as a runtime or compile classpath.

If an input property is annotated with `@api:org.gradle.api.tasks.Classpath[]` it is considered as a runtime classpath.
This means that names of classpath entries, e.g. names of directories and jar files, are ignored.
Files within classpath entries like class files and resources are treated with <<inputs_path_sensitivity,PathSensitive.RELATIVE>>.
Moreover, if a jar file is encountered as a classpath entry then the actual files within the jar file are examined as opposed to the jar file itself.
This prevents changes in timestamps and order of the zip entries within a jar file to make the task out of date.
As opposed to input properties annotated with `@api:org.gradle.api.tasks.InputFiles[]`, the order of the elements in the file collection matter.

Input properties annotated with `@api:org.gradle.api.tasks.CompileClasspath[]` are considered compile classpaths and expose the same behavior as runtime classpaths.
Additionally, Gradle ignores changes to everything but class files and does an ABI analysis of classes as described in <<sec:java_compile_avoidance>>.
