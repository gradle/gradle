...
If we want to support tasks as arguments as well and treat their outputs as the inputs, we can use the `project.layout.filesFor()` method like so:
...
As an added benefit, our use of `getProject().getLayout().filesFor()` means that our custom method can set up an inferred task dependency.
...
Alternatively, you can get Gradle to access the appropriate property itself by using one of `project.files()`, `project.layout.filesFor()` or `project.layout.mutableFilesFor()` in place of `project.fileTree()`:
...
Remember that `files()`, `layout.filesFor()` and `layout.mutableFilesFor()` can take tasks as arguments, whereas `fileTree()` cannot.
