...
Using nested inputs allows richer modeling and extensibility for tasks, as e.g. shown by `api:org.gradle.api.tasks.compile.CompileOptions#getCompilerArgumentProviders[]`.

For example, to declare annotation processor arguments, it is possible to do the following:

[source,groovy]
----
class MyAnnotationProcessor implements CommandLineArgumentProvider {
    @InputFile
