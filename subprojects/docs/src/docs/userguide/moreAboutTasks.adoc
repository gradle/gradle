...
The implementation of nested inputs is tracked, too.
This ensures that changes to the implementation cause the build to be out of date.
As a nice side effect, this allows tracking behavior, e.g. by annotating an `@api:org.gradle.api.Action[]` property with `@api:org.gradle.api.tasks.Nested[]`.
