...
Gradle supports incremental compilation for two common categories of annotation processors: "Isolated" and "Aggregating".
...
++++
<sample xmlns:xi="http://www.w3.org/2001/XInclude" id="isolatedAnnotationProcessor" dir="java/incrementalAnnotationProcessing" title="An isolated annotation processor">
     <sourcefile file="processor/src/main/java/EntityProcessor.java" snippet="isolated-annotation-processor"/>
</sample>
++++
...
Gradle will always recompile all files that contain annotations which the aggregating processor registered itself for, as well as all files generated by it.
...
* Simple class name or simple method name is supported if the pattern starts with an upper-case letter, e.g. `SomeTest`, `SomeTest.someMethod` (Since 4.7).
...
Generally speaking, there're two kinds of pattern:

[[simple_name_pattern]]
===== Simple name pattern

Since 4.7, a pattern starting with an upper-case letter matches simple class name:

```
# can match 'SomeTest' and corresponding method at any package
...
Otherwise, the pattern matches full qualified class name.
...
++++
<sample xmlns:xi="http://www.w3.org/2001/XInclude" id="jupiterexample" dir="testing/junitplatform/mix" title="Jupiter test example">
...
To control specific test engine implementations explicitly, you can do as follows:
