...
_Compile classpath, used when compiling source._ Used by `compile__SourceSet__Java`.
...
This is something you can combine with <<sec:continuous_build, continuous build>> using `--continuous` (or `-t`, for short) to re-execute a subset of tests immediately after every change.
