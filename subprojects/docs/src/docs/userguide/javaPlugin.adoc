...
| ``sourceSet`Classes`
| The `compile`SourceSet`Java` task and the `process`SourceSet`Resources` task. Some plugins add additional compilation tasks for the source set.
| api:org.gradle.api.Task[]
...
| ``sourceSet`Compile`, ``sourceSet`CompileOnly`
| `compile`SourceSet`Java`
| Compile classpath, used when compiling source
