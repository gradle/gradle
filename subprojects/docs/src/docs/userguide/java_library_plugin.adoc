...
At the moment, the Java Library plugin is wired to behave correctly with the `java`, `groovy` and `kotlin` plugins. In Gradle 5.1.1 or early, some plugins, such as the Groovy plugin, may not behave correctly. In particular, if the Groovy plugin is used in addition to the `java-library` plugin, then consumers may not get the Groovy classes when they consume the library. To workaround this, you need to explicitly wire the Groovy compile dependency, like this:

.Configuring the Groovy plugin to work with Java Library in Gradle 5.1.1 or earlier
====
...
In more recent versions of Gradle, you should add java-library to your groovy projects to benefit from the additional work avoidance gained from depending on class directories instead of jar files.
