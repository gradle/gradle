...
At the moment, the Java Library plugin is wired to behave correctly with the `java`, `groovy`, `scala` and `kotlin` plugins. In Gradle 5.3 or earlier, some plugins, such as the Groovy plugin, may not behave correctly.
In particular, if the Groovy or Scala plugin is used in addition to the `java-library` plugin, then consumers may not get the Groovy/Scala classes when they consume the library.

To workaround this, you need to explicitly wire the Groovy/Scala compile dependency, like this:

.Configuring the Groovy plugin to work with Java Library in Gradle 5.3 or earlier
====
