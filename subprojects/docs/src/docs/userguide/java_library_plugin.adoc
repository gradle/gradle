...
A feature enabled by the `java-library` plugin is the ability for projects consuming the library to only require the classes output instead of the full JAR to be built.
This enables a lighter inter-project dependency as resources processing and archive construction is not strictly needed.

[NOTE]
====
The usage or not of the classes output instead of the JAR is a _consumer_ decision.
For example, Groovy consumers will request classes _and_ processed resources as these may be needed for compilation.
====
...
This may lead to increased memory consumption for large projects.
