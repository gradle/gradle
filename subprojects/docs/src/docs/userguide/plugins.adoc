...
The new plugins DSL provides a succinct and convenient way to declare plugin dependencies. It works with the http://plugins.gradle.org[Gradle plugin portal] to provide easy access to both core and community plugins. The plugins DSL block configures an instance of api:org.gradle.plugin.dsl.PluginDependenciesSpec[].
...
See api:org.gradle.plugin.dsl.PluginDependenciesSpec[] for more information on using the Plugin DSL.
