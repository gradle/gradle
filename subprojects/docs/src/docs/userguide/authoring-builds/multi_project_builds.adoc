...
[[sec:sharing_configuration_between_subprojects]]
== Sharing configuration between subprojects
...
At the same time the subprojects that share one common trait may serve different purposes - they may produce different _artifact types_, for example:
...
In fact, Gradle core plugins are modeled in the same way - for example `java` plugin configures a generic `java` project,
while `java-library` plugin internally applies the `java` plugin and configures aspects specific to a generic Java library.

Similarly, we can compose custom build logic by applying and configuring both core and external plugins and create custom plugins
that define new project _types_.
For each of the example traits from the beginning of this section, we can write a plugin that encapsulates
the logic common to the project of a given _type_.

Have a look at the link:{samplesPath}/sample_convention_plugins.html[sample that demonstrates a multi-project build that models the build logic using convention plugins].
...
An alternative to <<#sec:sharing_configuration_between_subprojects,sharing build logic between subprojects via convention plugins>> is cross project configuration.
