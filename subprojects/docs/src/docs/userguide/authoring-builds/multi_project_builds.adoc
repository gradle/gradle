...
Next, we will look at multi-project builds that contain more than one subproject,
how build logic can be shared across subprojects and how subprojects can depend on one another.
...
For example, a project might consist of several subprojects containing _code_ written in a particular programming language as well as a subproject
dedicated for _documentation_.
There may be further traits that the _code_ subprojects share, such as certain code quality rules that we want to apply to all of them.
...
Gradle's recommended way of organizing build logic is its plugin system.
...
doing so will make it harder to understand the logic of a particular subproject in the long term.
With cross configuration, build logic gets injected into a subproject and this is not evident from looking at the subproject's
build script.
In the long run cross configuration usually grows complex with more and more conditional logic being added which leads to a maintenance burden.
...
- Extracting information from subprojects of certain type.
...
Before we dive into the details, do check if your <<sec:convention_plugins_vs_cross_configuration,use case can be solved using convention plugin instead>>.
...
We have an interesting set of dependencies. Obviously the `date` and `hello` projects have a _configuration_ dependency on `web-dist`, as all the build logic for the webapp projects is injected by `web-dist`. The _execution_ dependency is in the other direction, as `web-dist` depends on the build artifacts of `date` and `hello`. There is even a third dependency. `web-dist` has a _configuration_ dependency on `date` and `hello` because it needs to know the `archivePath`. But it asks for this information at _execution time_. Therefore we have no circular dependency.
...
Configuration here means executing the `build.gradle` (or `build.gradle.kts`) file of a project, which implies e.g. downloading all plugins that were declared using '```apply plugin```' or a `plugins` block.
By default, the configuration of all projects happens before any task is executed.
This means that when a single task, from a single project is requested, _all_ projects of multi-project build are configured first.
...
Configuration on demand mode attempts to configure only projects that are relevant for requested tasks, i.e. it only executes the `build.gradle[.kts]` file of projects that are participating in the build.
...
This way the typical common configuration is supported (allprojects or subprojects script blocks).
...
Eager to try out this new feature? To configure on demand with every build run see <<build_environment.adoc#sec:gradle_configuration_properties,Gradle properties>>.
