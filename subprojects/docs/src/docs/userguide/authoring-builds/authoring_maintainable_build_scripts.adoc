...
* Model the build as <<creating_multi_project_builds.adoc#multi_project_builds,multi-project build>> if the intention is to execute tasks from different modules as unified build.
...
Gradle does not restrict build script authors from reaching into the domain model from one project into another one in a <<creating_multi_project_builds.adoc#multi_project_builds,multi-project build>>.
