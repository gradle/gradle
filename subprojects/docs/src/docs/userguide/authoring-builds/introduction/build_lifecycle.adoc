...
Gradle guarantees that these tasks are executed in the order of their dependencies.
Your build scripts and plugins configure this dependency graph.
This page discusses the phases of Gradle's lifecycle as it interprets those scripts.
...
With configuration avoidance, Gradle skips configuration for tasks not part of the current build.

Within each project, tasks form a http://en.wikipedia.org/wiki/Directed_acyclic_graph[Directed Acyclic Graph] (DAG).

This diagram shows two example task graphs: abstract and concrete.
...
Gradle runs these phases in order: initialization, configuration, and execution.
...
Execution::
- Schedules and executes each selected task in the order of their dependencies.
...
In the initialization phase, Gradle detects the set of projects and includes builds participating in the build.
...
You can receive a notification immediately before and after evaluating a project.
...
You can receive a notification immediately after Gradle populates a project's task execution graph.
...
For example, you could use these notifications to set default values or methods.
...
To receive these events via a listener instead of a closure, add a link:{javadocPath}/org/gradle/api/Action.html[Action] to a link:{javadocPath}/org/gradle/api/tasks/TaskContainer.html[TaskContainer].
