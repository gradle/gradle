...
This is because the `application` plugin contains additional tasks now available in your project.

Many of these tasks, such as `assemble`, `build`, and `clean`, should be familiar to you.
...
As we can see we have created a `hello` task in the `custom` group and added a simple description.
...
We can update the `hello` task from the previous example to include a dependency:
