...
Even if your build script is empty, Gradle provides a number of default tasks for a project which are listed by running `./gradlew tasks`:
...
This is because the `application` plugin code contains additional tasks which are now available in your project.
...
Groovy or Kotlin can be used for more than defining what a task does.

As an example, you can use it to register multiple tasks of the same type in a loop:
...
Gradle scales in how you can organize your build logic.

The first level of organizing your build logic for the example above, is extracting a method:
