...
The Gradle User Home (`<home directory of the current user>/.gradle` by default) is used to store global configuration properties and initialization scripts, caches, and log files.
...
NOTE: The version-compliant script should reside somewhere other than the `init.d` directory (such as a sub-directory) so that it is not automatically applied.
...
It also contains files and directories that are generated by Gradle such as `.gradle` and `build`.
