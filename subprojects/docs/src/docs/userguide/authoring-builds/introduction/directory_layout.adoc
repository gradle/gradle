...
The Gradle User Home (`~/.gradle` or `C:\Users\username\.gradle` by default) stores global configuration properties, initialization scripts, caches, and log files.

It can be set with the environment variable `GRADLE_USER_HOME`.

TIP: Not to be confused with the `GRADLE_HOME` which is the installation directory for Gradle.
...
However, older versions have retention periods that are fixed (7 or 30 days, depending on the cache).
These shared caches could be accessed by versions of Gradle with different settings for the retention of cache artifacts.
