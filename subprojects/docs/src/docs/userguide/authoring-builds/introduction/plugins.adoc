...
The example below is the most straightforward plugin, a "hellow world" *script plugin*:
...
[[sec:plugins_block]]
== 1. Applying plugins using the `plugins{}` block from the plugin DSL
...
== 2. Applying plugins using the `buildscript{}` block
...
== 3. Applying plugins from the `buildSrc` directory
