...
You add tasks to a project using the `TaskContainer.register(java.lang.String)` method.
...
You can locate existing tasks using the `TaskCollection.name(java.lang.String)` method:
...
While best practice is to define tasks via plugins, tasks can be added to build scripts directly.
