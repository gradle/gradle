...
Plugins can be applied using the `PluginAware.apply(java.util.Map)` method or the `PluginDependenciesSpec` plugins script block.
...
A plugin can add properties and methods to a project through the project's `Convention` object.
