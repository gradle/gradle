...
You add OR DEFININING? tasks to a project using the `TaskContainer.register(java.lang.String)` method:
...
You can locate existing tasks (to update the configuration, or modfiy...) using the `TaskCollection.named(java.lang.String)` method:
...
While tasks can be added in build scripts, best practice is to define tasks via plugins, tasks can be added to build scripts directly.
