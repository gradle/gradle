...
image::author-gradle-4.png[Build,500,63,align="center"]
...
Every Gradle build is made up of one or more *projects*; a _root_ project and _subprojects_.
...
When Gradle executes a Groovy build script (`build.gradle`), it compiles the script into a class that
...
When Gradle executes a Kotlin build script (`build.gradle.kts`), it compiles the script into a subclass of link:{kotlinDslPath}/gradle/org.gradle.kotlin.dsl/-kotlin-project-script-template/index.html[KotlinProjectScriptTemplate].
...
The Script API is particularly useful for working with files and paths; it provides useful methods such as `copy()`, `delete()`, `exec()` and `uri()` that are not available via the `Project` object.
...
----
<1> Add Gradle extensions via plugins.
<2> Define dependency location.
<3> Add dependencies.
...
=== Add Gradle extensions via plugins
...
=== Define dependency location

A project generally has a number of dependencies it needs in order to do its work.
Dependencies include plugins, libraries or components which Gradle must download for the build to succeed.
The locations of the binaries for the dependencies must be described.
...
A project generally has a number of dependencies it needs in order to do its work.
These dependencies are typically imports in the source code.
...
In the example, the application code uses the `guava` libraries from Google.
...
The capabilities of the Groovy or Kotlin language can be used in the build script.

As an example, you can register multiple tasks in a loop:
...
[.text-right]
**Next Step:** <<using_tasks.adoc#tutorial_using_tasks,Learn how to use Tasks>> >>
