...
|See <<#sec:plugins_block,Applying plugins using the plugins DSL>>

|2
|Including the plugin from an external jar defined as a `buildscript` dependency
|`buildscript { dependencies { classpath 'com.gradle:gradle-plugin:2.3.2' } }`
|See <<#sec:applying_plugins_buildscript,Applying plugins using the `buildscript` block>>

|3
|Defining the plugin as a source file under the `buildSrc` directory in the project
|`dependencies { testImplementation 'junit:junit:4.13' }`
|See <<organizing_gradle_projects.adoc#sec:build_sources,Using `buildSrc` to extract functional logic>>

|4
|Defining the plugin as an inline class declaration inside a build script
|`apply plugin: PrintClass`
|See <<sec:script_plugins, Script plugins>>
|===
...
This requires specifying plugins, so Gradle can easily and quickly extract before executing the rest of the build script.
...
There are some constraints, some temporary limitations, while the mechanism is under development.
Some are inherent to the new approach.
...
The `plugins{}` block can only be used in a project's build script and the `settings.gradle(.kts)` file.
...
The following example shows how to tie a plugin implementation class — `my.MyPlugin` — defined in `buildSrc` to the ID "my-plugin":
