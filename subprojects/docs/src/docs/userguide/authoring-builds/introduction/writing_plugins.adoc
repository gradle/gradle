...
Gradle at its core is simple.
The majority of features, like the ability to compile Java code, are added by _plugins_.
...
_Resolving_ a plugin means finding the correct version of the jar which contains a given plugin and adding it to the script classpath.
...
_Applying_ a plugin means actually executing the plugin's link:{javadocPath}/org/gradle/api/Plugin.html#apply-T-[Plugin.apply(T)] on the Project you want to enhance with the plugin.
Applying plugins is _idempotent_.
That is, you can safely apply any plugin multiple times without side effects.

The <<#sec:plugins_block,plugins DSL>> is recommended to both resolve and apply plugins in one step.
...
A plugin is simply any class that implements the link:{javadocPath}/org/gradle/api/Plugin.html[Plugin] interface.

Gradle provides the core plugins (e.g. `JavaPlugin`) as part of its distribution which means they are automatically resolved.

However, non-core binary plugins need to be resolved before they can be applied.
...
It works with the http://plugins.gradle.org[Gradle plugin portal] to provide easy access to both core and community plugins.
...
The plugins DSL is processed in a way which allows Gradle to determine the plugins in use very early and very quickly.
...
Many build authors would also like to resolve plugins from private Maven or Ivy repositories because the plugins contain proprietary implementation details, or just to have more control over what plugins are available to their builds.
...
Custom Maven and Ivy plugin repositories must contain <<#sec:plugin_markers,plugin marker artifacts>> in addition to the artifacts which actually implement the plugin.
