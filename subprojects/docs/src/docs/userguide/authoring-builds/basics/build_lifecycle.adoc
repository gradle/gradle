...
For example, if your project tasks include `assemble`, `build`, `createDocs`, your build script(s) can ensure that they are executed in the order `assemble` -> `build` -> `createDoc`.
...
The automatic search for a settings file only works for multi-project builds with a <<intro_multi_project_builds.adoc#intro_multi_project_builds,standard project layout>>.
...
=== B. Task Execution Graph Assembly

During project evaluation, Gradle assembles the *task execution graph*: a http://en.wikipedia.org/wiki/Directed_acyclic_graph[DAG] representing the dependency relationships between tasks in all projects.
There can be relationships between tasks in different subprojects which will be reflected in the DAG.
