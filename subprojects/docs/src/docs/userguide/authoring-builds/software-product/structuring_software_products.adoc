...
In Gradle, you assign a type to a project by <<writing_plugins.adoc#plugins,applying a plugin>>.
...
Or, if the included component provides a plugin, you <<writing_plugins.adoc#sec:plugins_block,apply the plugin by ID>> similar to how you would apply a plugin from the plugin portal:
