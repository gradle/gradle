...
We see that the `settings.gradle(.kts)` file only defines location for build logic components, other production code components and the inner structue of the component.
