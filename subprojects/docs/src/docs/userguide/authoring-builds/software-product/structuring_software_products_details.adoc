...
Note that when publishing build logic components, the `maven-publish` will also publish so called <<writing_plugins.adoc#sec:plugin_markers,plugin markers>> that allow Gradle to find plugins by ID – even if they are located in a repository.
