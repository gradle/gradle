...
You may also use the normal `includeBuild` mechanism to include plugin builds.
However, this does not support all use cases and will be deprecated once the new mechanism is stable.
...
Subproject and task names can be <<command_line_interface.adoc#sec:name_abbreviation,abbreviated>>.
