...
NOTE: You may also use the `includeBuild` mechanism outside `pluginManagement` to include plugin builds. +
...
NOTE: When a composite build is included into another composite build, then both included builds would have the same parent. +
