...
Gradle supports this behavior out of the box through a feature called *incremental builds*.
You have almost certainly already seen it in action.
When you run a task and the task is marked with `UP-TO-DATE` in the console output, this means incremental builds are at work.
...
=== Managing inputs and outputs of ad-hoc tasks
...
==== Registering inputs and outputs for custom task types
