...
A plugin specified by an id and (optionally) a version can also be added to the build script classpath, similarly a plugin that is declared in a version catalog.
