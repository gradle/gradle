...
====
<1> Adding a plugin and applying it.
<2> Adding a plugin using its id and version.
<3> Adding the `jmh` plugin as declared in the `libs` version catalog, see the section on <<platforms.adoc#sec:plugins, plugins>>.
