...
A plugin specified by a plugin identifier and (optionally) a version can be added to the build script classpath as shown below, similarly a plugin that is declared in a version catalog can be added.
...
====
<1> Adding a plugin using its id and version.
<2> Adding the `jmh` plugin as declared in the `libs` version catalog, see the section on <<platforms.adoc#sec:plugins, plugins>>.
<3> Adding a plugin using its Maven coordinates and applying it.
