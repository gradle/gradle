...
Similarly, plugins declared in a version catalog can be added to the build script classpath.
...
<2> Adding the plugin `jmh` as declared in the `libs` version catalog, see the section on link:{javadocPath}/org/gradle/dep-man/03-controlling-transitive-dependencies/platforms.html#sec:plugins[plugins].
====
