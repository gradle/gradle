...
Many tasks need to create directories to store the files they generate, which is why Gradle automatically manages this aspect of tasks when they explicitly define file and directory outputs. You can learn about this feature in the <<incremental_builds.adoc#incremental_builds,incremental build>> section of the user manual. All core Gradle tasks ensure that any output directories they need are created if necessary using this mechanism.
...
Using a task directly as an argument like this relies on it having <<incremental_builds.adoc#sec:task_inputs_outputs,defined outputs>>, so it won't always be possible.
...
You can even pass in tasks as arguments if they have <<incremental_builds.adoc#sec:task_inputs_outputs,defined outputs>>.
 ...
 * A task — the files or directories that form a task's <<incremental_builds.adoc#sec:task_inputs_outputs,defined outputs>> are included
...
The above example demonstrates the basic syntax and also highlights two major limitations of using the `copy()` method:
 ...
 1. The `copy()` method is not <<incremental_builds.adoc#incremental_builds,incremental>>. The example's `copyMethod` task will _always_ execute because it has no information about what files make up the task's inputs. You have to manually define the task inputs and outputs.
...
The following example shows you how to workaround these limitations by using the <<incremental_builds.adoc#sec:task_input_output_runtime_api,dynamic API for task inputs and outputs>>:
