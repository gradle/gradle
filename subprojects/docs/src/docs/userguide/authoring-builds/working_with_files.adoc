...
NOTE: There are two common issues people come across when using regular expressions in this context: +
...
NOTE: The configuration defined by `webAssetsSpec` will _not_ apply to the app classes included by the `distApp` task. That's because `from appClasses` is its own child specification independent of `with webAssetsSpec`. +
