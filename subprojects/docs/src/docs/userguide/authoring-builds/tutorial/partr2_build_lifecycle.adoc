...
Phase 2 - Configuration  :: During the configuration phase, the `Project` objects are configured using the build scripts of all projects in the build and Gradle determines the set of tasks to be executed.
...
<3> *Execution*: Gradle executes the tasks passed on the command line as well as any prerequisite tasks.
