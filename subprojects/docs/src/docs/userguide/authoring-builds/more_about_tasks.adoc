...
In the <<writing_build_scripts#tutorial_using_tasks,introductory tutorial>> you learned how to create simple tasks. You also learned how to add additional behavior to these tasks later on, and you learned how to create dependencies between tasks. This was all about simple tasks, but Gradle takes the concept of tasks further. Gradle supports tasks that have their own properties and methods. Such tasks are either provided by you or built into Gradle.
...
We have already seen how to define tasks using strings for task names in <<writing_build_scripts#tutorial_using_tasks,this chapter>>. There are a few variations on this style, which you may need to use in certain situations.
