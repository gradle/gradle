...
In the event that a file matches a rule, but cannot be loaded as a properties file (e.g. because it is not formatted properly or uses a non-standard encoding), it will be ignored by the rule and will be incorporated into the up-to-date or build cache key calculation as a normal file.
