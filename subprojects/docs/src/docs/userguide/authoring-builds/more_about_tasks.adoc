...
.Multi-project Java application with dependencies
====
include::sample[dir="samples/writing-tasks/tasks-with-dependency-resolution-result-inputs/groovy",files="settings.gradle[tag=includes];list/build.gradle[];utilities/build.gradle[];build.gradle[]"]
include::sample[dir="samples/writing-tasks/tasks-with-dependency-resolution-result-inputs/kotlin",files="settings.gradle.kts[tag=includes];list/build.gradle.kts[];utilities/build.gradle.kts[];build.gradle.kts[]"]
====

.Resolved artifacts metadata
====
.Task input declaration in `ListResolvedArtifactIdentifiers.java`
[source,java]
----
include::{samplesPath}/writing-tasks/tasks-with-dependency-resolution-result-inputs/common/dependency-reports/src/main/java/com/example/ListResolvedArtifactIdentifiers.java[tag=inputs]
----

.Task registration and configuration in `DependencyReportsPlugin.java`
[source,java]
----
include::{samplesPath}/writing-tasks/tasks-with-dependency-resolution-result-inputs/common/dependency-reports/src/main/java/com/example/DependencyReportsPlugin.java[tag=listResolvedArtifactIdentifiers]
----

.Output of **`gradle listResolvedArtifactIdentifiers`**
----
> gradle listResolvedArtifactIdentifiers
...
The above use cases consuming dependency resolution results are demonstrated in the link:../samples/sample_tasks_with_dependency_resolution_result_inputs.html[Tasks with dependency resolution result inputs] sample.
