...
| `@link:{javadocPath}/org/gradle/api/tasks/Untracked.html[Untracked]`
| `File` or `Iterable&lt;File&gt;`+++*+++
| Used on an input or output file property to declare that the changes for the property should not be tracked by Gradle.
...
You can disable Gradle's up-to-date checks by using the `@link:{javadocPath}/org/gradle/api/tasks/Untracked.html[Untracked]` annotation on the output directory of the task wrapping the tool.
...
====
- (1) Declare the output of the task as untracked.
