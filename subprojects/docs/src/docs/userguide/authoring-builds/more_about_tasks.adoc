...
[NOTE]
====
Tasks that fail validation are executed without any optimizations.
...
If one task produces an output location and another task consumes that location by declaring it as an input, then Gradle validates that the consumer task depends on the consumer task.
When the producer and the consumer task are executing at the same time, the build fails to avoid capturing an incorrect state.
