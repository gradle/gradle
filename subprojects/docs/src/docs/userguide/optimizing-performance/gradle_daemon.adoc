...
Single-use Deamon :: If the JVM args of the client process don't match what the build requires (org.gradle.jvmargs), the long-lived Daemon becomes a single-used Daemon.
This means the Daemon will be used for the build, but it will be stopped at the end of the build.

No Daemon :: If the `JAVA_OPTS` / `GRADLE_OPTS` match `org.gradle.jvmargs`, the Daemon will not be used at all.
