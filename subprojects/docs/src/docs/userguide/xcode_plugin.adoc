...
The Xcode Plugin generate files that are used by the https://developer.apple.com/xcode/[Xcode IDE], thus making it possible to open the workspace into Xcode (`File` - `Open...`).
...
====
include::sample[dir="userguide/xcode/groovy",files="build.gradle[tags=apply-plugin]"]
include::sample[dir="userguide/xcode/kotlin",files="build.gradle.kts[tags=apply-plugin]"]
====
...
Both the <<cpp_application_plugin.adoc#,{cpp} Application Plugin> and <<cpp_library_plugin.adoc#{cpp} Library Plugin>> introduce the same tasks for generating Xcode projects to be included in Xcode workspace.
...
The Xcode Plugin allows for some customization of the generated files.
The following sections are shows the customization.

=== Change workspace generated file location

The location of the generated workspace files can be configured on the root project using the `xcode` script bloc as follow:

.Change the generated workspace files location
====
include::sample[dir="userguide/xcode/groovy",files="build.gradle[tags=configure-workspace-location]"]
include::sample[dir="userguide/xcode/kotlin",files="build.gradle.kts[tags=configure-workspace-location]"]
====