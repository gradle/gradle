...
.build.gradle
----
include::{samplesPath}/webApplication/quickstart/build.gradle[tag=use-war-plugin]
----
...
`war` — type: link:{groovyDslPath}/org.gradle.api.tasks.bundling.War.html[War]::
...
`components.web`::
A link:{javadocPath}/org/gradle/api/component/SoftwareComponent.html[SoftwareComponent] for <<publishing_overview.adoc#publishing_overview,publishing>> the production WAR created by the `war` task.
...
.build.gradle
----
include::{samplesPath}/webApplication/customized/build.gradle[tag=customization]
----
