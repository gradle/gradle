...
This is the best place where to find how to do this and what with each DSL ; and it covers all Gradle features from <<writing_plugins.adoc#plugins,using plugins>> to <<dependency_constraints.adoc#dependency-constraints,customizing the dependency resolution behavior>>.
--
...
- <<writing_plugins.adoc#sec:plugins_block,declaratively, using the `plugins {}` block>>,
- <<writing_plugins.adoc#sec:old_plugin_application,imperatively, using the legacy `apply(..)` functions>>.
...
See the <<writing_plugins.adoc#sec:plugins_block,`plugins {}` block documentation>> in the Gradle user manual for more information.
--
...
The <<writing_plugins.adoc#sec:plugins_block,`plugins {}` block>> is about keeping your build scripts declarative in order to get the best out of the Kotlin DSL.
