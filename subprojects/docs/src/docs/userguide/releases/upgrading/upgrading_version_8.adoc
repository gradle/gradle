...
The `groovy-base` plugin is now responsible for configuring source and target compatibility version conventions on any `GroovyCompile` tasks.
...
We recommend applying the `groovy-base` plugin whenever working with Groovy language tasks.
