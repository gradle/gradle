...
* Use of the <<writing_plugins#sec:plugins_block,plugins {}>> block to declare Gradle plugins significantly improves the editing experience and is highly recommended.
...
Plugins fetched from a source other than the link:https://plugins.gradle.org/[Gradle Plugin Portal] can only be declared via the `plugins {}` block if they are published with their <<writing_plugins#sec:plugin_markers,plugin marker artifacts>>.
...
It depends on how they have been published and, specifically, whether they have been published with the necessary <<writing_plugins#sec:plugin_markers,plugin marker artifacts>>.
...
See the <<writing_plugins#sec:plugin_management,Plugin Management>> section of the Gradle user manual for more information on the `pluginManagement {}` block and what it can be used for.
