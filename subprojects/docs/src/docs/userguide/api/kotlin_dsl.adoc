...
Property assignment works for properties that are `final` and have type `Property` or `ConfigurableFileCollection`.
Once the feature is released out of experimental status, using the `=` operator will be the preferred way over calling `set()` in Kotlin DSL.
