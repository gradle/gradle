...
CAUTION: Use of internal Kotlin DSL APIs in plugins and build scripts has the potential to break builds when either Gradle or plugins change.
...
TIP: Your IDE knows about the type-safe accessors, so it will include them in its suggestions. +
...
TIP: When publishing plugins, please use Gradle's built-in <<java_gradle_plugin#java_gradle_plugin,Gradle Plugin Development Plugin>>. +
