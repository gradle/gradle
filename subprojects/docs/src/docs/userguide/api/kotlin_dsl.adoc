...
Gradle's Kotlin DSL supports Property assignment as an experimental feature that can be enabled by adding `systemProp.org.gradle.unsafe.kotlin.assignment=true` to the `gradle.properties` file.
Property assignment reduces the verbosity for Kotlin DSL when <<lazy_configuration#lazy_properties,lazy properties>> are used.
Property assignment works for properties that are _final_ and have type _Property_ or _ConfigurableFileCollection_.
Property assignment should be preferred over custom property setters since it makes Kotlin DSL code style consistent.

.Kotlin Property assignment
====
include::sample[dir="snippets/kotlinDsl/assignment/kotlin",files="build.gradle.kts[tags=assignment]"]
====
<1> Set value old way, with `.set()` method (still supported)
<2> Set value with Property assignment with `=` operator
<3> Property assignment also supports lazy values

[CAUTION]
====
Enabling Kotlin Property assignment will fail your build if `allWarningsAsErrors` feature is enabled for the Kotlin compiler for build logic, since the Kotlin compiler plugin is still experimental.
====

=== How Property assignment works?

Property assignment is implemented via a Kotlin compiler plugin, and it works similarly to operator overloads that are natively supported by the Kotlin language, e.g. `+=`.
The main difference from overloading other Kotlin operators is that it works just for final properties.

The assignment overload is declared as an extension method, e.g.:

.KotlinAssignmentExtensions.kt
[source,kotlin]
```
fun <T> Property<T>.assign(value: T?) = this.set(value)
```

Because the Property assignment feature is implemented as a compiler plugin, you can easily inspect the declaration of the assignment overload in the IDE, by highlighting the `=` and inspecting the declaration, e.g. in the IntelliJ you can use _Go To Declaration_.

[NOTE]
====
While it's possible to implement custom `=` overloads, it's not recommended in public plugins, due to possible conflicts and Kotlin DSL code style consistency in general.
====

=== IDE support

Property assignment is supported from IntelliJ 2022.3 and from Android Studio Giraffe.
For IDE to correctly detect the Kotlin assignment `systemProp.org.gradle.unsafe.kotlin.assignment=true` has to be added to the root project `gradle.properties` file (also for all included builds).
Additionally, after the property value is changed, the project has to be reopened.
In future releases, we will work on improving the IDE experience and remove these requirements.
