...
Gradle's Kotlin DSL supports lazy property assignment using the `=` operator as an experimental feature that can be enabled by adding `systemProp.org.gradle.unsafe.kotlin.assignment=true` to the `gradle.properties` file.
...
Since properties have to be `final`, our general recommendation is to not implement custom setters for properties with lazy types and if possible implement such properties via an abstract getter.

Once the feature is released out of experimental status, using the `=` operator will be the preferred way to calling `set()` in the Kotlin DSL.
...
[CAUTION]
====
Enabling lazy property assignment will fail your build if link:https://kotlinlang.org/docs/gradle-compiler-options.html#attributes-common-to-jvm-js-and-js-dce[allWarningsAsErrors] option is enabled for the Kotlin compiler for build logic, since the Kotlin compiler plugin is still experimental.
====

=== IDE support

Lazy property assignment is supported from IntelliJ 2022.3 and from Android Studio Giraffe.
The IDE integration is not stable yet.
For IDE to detect the Kotlin assignment, `systemProp.org.gradle.unsafe.kotlin.assignment=true` has to be added to the root project `gradle.properties` file (also for all included builds).
