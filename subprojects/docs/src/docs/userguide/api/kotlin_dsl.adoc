...
[CAUTION]
====
Enabling lazy property assignment will fail your build if the Kotlin compiler `allWarningsAsErrors` option is enabled for the build logic, since the Kotlin compiler plugin is still experimental and emits a warning.
====
...
== Writing plugins in Kotlin

See <<../developing-plugins/kotlin_gradle_plugins.adoc, the dedicated page>> for more details about how to write your plugins in Kotlin
