...
The {cpp} Library Plugin provides the tasks, conventions and conventions for building {cpp} libraries.
...
====
include::sample[dir="userguide/cpp/cppLibrary/groovy",files="build.gradle[tags=apply-plugin]"]
include::sample[dir="userguide/cpp/cppLibrary/kotlin",files="build.gradle.kts[tags=apply-plugin]"]
====

== Build variants

The {cpp} Library Plugin understands the following dimensions which participate in the variant configuration.
...
The build type control the debuggability as well as the optimization of the generated binaries.
- `debug` - generate debug symbols and isn’t optimized
- `release` - generate debug symbols extracted from the binary and is optimized
...
====
include::sample[dir="userguide/cpp/cppLibrary/groovy",files="build.gradle[tags=configure-linkages]"]
include::sample[dir="userguide/cpp/cppLibrary/kotlin",files="build.gradle.kts[tags=configure-linkages]"]
====

Target machines - default to the build host::
The target machine expresses what machines the library expects to run.
...
====
include::sample[dir="userguide/cpp/cppLibrary/groovy",files="build.gradle[tags=configure-target-machines]"]
include::sample[dir="userguide/cpp/cppLibrary/kotlin",files="build.gradle.kts[tags=configure-target-machines]"]
====
...
This task is added by the <<base_plugin.adoc,Base Plugin>>.
// TODO: Link to the C++ Unit Test Plugin reference chapter
...
`api`::
Used for declaring API dependencies (see <<sec:api_vs_implementation,API vs implementation section>>).
...
Used for declaring implementation dependencies for all variants of the main component (see <<sec:api_vs_implementation,API vs implementation section>>).
...
Used for declaring implementation dependencies for a specific variant of the main component (see <<sec:api_vs_implementation,API vs implementation section>>).
...
.Adding dependencies
====
include::sample[dir="userguide/cpp/cppLibrary/groovy",files="build.gradle[tags=dependency-management]"]
include::sample[dir="userguide/cpp/cppLibrary/kotlin",files="build.gradle.kts[tags=dependency-management]"]
====
...
link:{groovyDslPath/org.gradle.language.cpp.tasks.CppCompile.html#org.gradle.language.cpp.tasks.CppCompile:objectFileDir[objectFileDir]:: `$buildDir/obj/main/__variant__`
...
link:{groovyDslPath}/org.gradle.nativeplatform.tasks.LinkSharedLibrary.html#org.gradle.nativeplatform.tasks.LinkSharedLibrary:linkedFile[linkedFile]:: `$buildDir/lib/main/__variant__/libBaseName[.so|dylib]` (*nix) or `$buildDir\lib\main{backslash}__variant__\baseName.dll` (Windows)
...
[horizontal]
link:{groovyDslPath}/org.gradle.nativeplatform.tasks.CreateStaticLibrary.html#org.gradle.nativeplatform.tasks.CreateStaticLibrary:outputFile[outputFile]:: `$buildDir/lib/main/__variant__/libBaseName.a` (*nix) or `$buildDir\lib\main{backslash}__variant__\baseName.lib` (Windows)
