...
The C\++ Library Plugin provides the tasks and conventions for building C++ libraries.
In particular, a C\++ library exposes an API to consumers (i.e., other projects using C++ Library or C++ Application Plugin).
// TODO: Link to C++ application plugin chapter

== Usage

.Applying the C++ Library Plugin
====
...
The C++ Library Plugin manages the following dimensions, in order, which participate in the variant configuration.
...
Note the default linkage of a C++ library is shared linkage and is shown on the diagram.

.C++ Library Plugin default task graph
image::cpp-shared-library-task-graph.png[]

Changing the linkage to static linkage, the diagram looks like the following:

.C++ Library Plugin static library only task graph
...
.C++ Library Plugin variant-dependent task graph
...
For example, the C++ Unit Test Plugin attach his test task to this lifecycle task.
This task is added by the Lifecycle Base Plugin.
...
Just like the tasks created by the C\++ Library Plugin, the configurations are created based on the variant of the library component.
...
.C++ Library Plugin configurations
...
`cppCompileVariant` (e.g. `cppCompileDebug` and `cppCompileRelease`)::
Used for compiling this library.
This configuration contains the compile include roots of this library and is therefore used when invoking the C++ compiler to compile it.

`nativeLink__Variant__` (e.g. `nativeLinkDebug` and `nativeLinkRelease`)::
Used for linking this library the shared library only.
This configuration contains the libaries of this library and is therefore used when invoking the C++ linker to link it.
...
The C++ Library Plugin will compile whatever it finds, and handles anything which is missing.
...
The C++ Library Plugin adds a link:{groovyDslPath}/org.gradle.language.cpp.tasks.CppCompile.html[CppCompile] instance for each variant of the library component to build (e.g. `compileDebugCpp` and `compileReleaseCpp`).
...
ConfigurableFileCollection includes:: `configurations.cppCompileVariant` + `library.publicHeaders` + `library.privateHeaders`

Map<String, String> macros:: [:]

DirectoryProperty objectFileDir:: `$buildDir/obj/main/variant`

boolean optimized:: `false` for debug build type or `true` otherwise

boolean positionIndependentCode:: `true` for shared linkage or `false` otherwise

ConfigurableFileCollection source:: `library.cppSource`

ConfigurableFileCollection systemIncludes:: []

Property<NativePlatform> targetPlatform:: derived from the `TargetMachine` of the binary

Property<NativeToolChain> toolChain:: <<building_cpp_projects.adoc#cpp_supported_tool_chain,automatically selected based on target machine>>

=== `link__Variant__` Task

The C++ Library Plugin adds a link:{groovyDslPath}/org.gradle.language.cpp.tasks.LinkSharedLibrary.html[LinkSharedLibrary] instance for each variant of the library containing shared linkage as a dimension - e.g. `linkDebug` and `linkRelease`.
Some of the most common configuration options are shown below.

[horizontal]
Property<Boolean> debuggable:: `true`

ConfigurableFileCollection libs:: `configurations.nativeLinkVariant`

RegularFileProperty linkedFile:: `$buildDir/lib/main/variant/libBaseName[.so|dylib]` (*nix) or `$buildDir\lib\main\variant\baseName.dll` (Windows)

ListProperty<String> linkerArgs:: []

ConfigurableFileCollection source:: `compileVariantCpp.objects`
...
The C++ Library Plugin adds a link:{groovyDslPath}/org.gradle.language.cpp.tasks.CreateStaticLibrary.html[CreateStaticLibrary] instance for each variant of the library containing static linkage as a dimension - e.g. `createDebug` and `createRelease`.
Some of the most common configuration options are shown below.

[horizontal]
RegularFileProperty outputFile:: `$buildDir/lib/main/variant/libBaseName.a` (*nix) or `$buildDir\lib\main\variant\baseName.lib` (Windows)

FileCollection source:: `compileVariantCpp.objects`
