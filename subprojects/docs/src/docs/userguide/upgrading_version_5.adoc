...
Gradle 5.6 no longer supplies custom classpath attributes in the Eclipse model. Instead, it provides the attributes for [Eclipse test sources](https://www.eclipse.org/eclipse/news/4.8/jdt.php#jdt-test-sources). This change requires Buildship version 3.1.1 or later.
...
==== Classpath for worker processes
