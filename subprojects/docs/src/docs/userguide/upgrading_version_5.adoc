...
==== Copy or archive duplicate paths without explicitly specifying a duplicates strategy has been deprecated

Copy and archive tasks default to the `INCLUDE` duplicates strategy, allowing the same path to be copied multiple times to the destination or to exist multiple times in an archive.

Trying to copy or archive duplicate paths without explicitly specifying a duplicates strategy is now deprecated.
This is to prevent user from accidentally creating https://github.com/gradle/gradle/issues/9990["grossly invalid zip files" that trigger zip bomb detection].

If you still want to allow them, you can specify that explicitly:
