...
==== Java plugins register javadocJar and sourcesJar tasks

To provide better support for publishing of Javadoc and sources, the Java plugins (Java and Java Library) now register and configure the `javadocJar` and `sourcesJar` tasks.
If you defined these tasks yourself, following the documentation and examples on the topic, you can remove your task definitions as the new tasks follow the configuration that has been recommended and documented before.
If you have custom configurations of these tasks, you can change your task creation into a configuration of the existing tasks.
If you maintain code that creates these tasks and is supposed to work with multiple Gradle versions, you can conditionally create the tasks (e.g. by checking `tasks.names.contains("javadocJar")`)

Similarly, you can remove the custom artifact declarations `artifact javadocJar` and `artifact sourcesJar` from your publication and instead activate the built-in support for Javadoc and sources publishing: `java { publishJavadocAndSources() }`.
