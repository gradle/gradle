...
- The `validateTaskProperties` task is now deprecated, use `validatePlugin` instead.
...
- The `ValidateTaskProperties` type is replaced by `ValidatePlugin`.
...
- The link:{javadocPath}/org/gradle/plugin/devel/tasks/ValidatePlugin.html#getFailOnWarning--[failOnWarning] option is now enabled by default.
- The following task validation errors now fail the build at runtime and are promoted to errors for link:{javadocPath}/org/gradle/plugin/devel/tasks/ValidatePlugin.html[ValidatePlugin]:
