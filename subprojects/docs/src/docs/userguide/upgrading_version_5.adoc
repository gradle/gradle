...
- Replacing a registered (unrealized) task with a task that has an incompatible type (i.e. a type that is not the same type or a sub-type of the previously registered type)
- Attempting to replace a task that has not been previously been registered.
