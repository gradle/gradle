...
The `WorkerExecutor.submit()` method is now deprecated.  The new `noIsolation()`, `classLoaderIsolation()` and `processIsolation()` methods should now be used to submit work.  See <<custom_tasks.html#using-the-worker-api, the userguide>> for more information on using these methods.
