...
When configuring the local build cache the use of `BuildCacheConfiguration.local(Class)` and `local(Class, Action)` have now been deprecated; use `getLocal()` or `local(Action)` instead.
...
==== Archives fail on duplicates

Until now archives defaulted to the `INHERIT` duplicates strategy, allowing the same path to exist multiple times in an archive.
...
*Note* that `Copy` and `Sync` tasks are unaffected: they still use the `INHERIT` duplicates strategy as default.
