...
==== pluginManagement {} in settings scripts is now isolated
...
This also means that code inside such a block cannot reference anything declared elsewhere in the script.

This change has been made so that the `pluginManagement {}` configuration can also be applied when resolving plugins for the settings script itself.
