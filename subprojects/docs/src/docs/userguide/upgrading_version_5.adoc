...
==== Play framework support moved to external plugin

The Play Framework support bundled with earlier Gradle versions was removed.
Please use the new link:https://gradle.github.io/playframework[Gradle Play Plugin] instead which offers similar functionality.
...
The built-in Play plugin has been deprecated and will be replaced by a new link:https://gradle.github.io/playframework[Play Framework plugin] available from the plugin portal.
