...
First, we use the <<more_about_tasks.adoc#sec:task_input_output_runtime_api,runtime API>> to declare the inputs and outputs.

.A task that delegates to NPM to bundle the JavaScript files found in a directory
====
...
To achieve that, we need to tell Gradle when it's allowed to cache the output of that task, using the link:{javadocPath}/org/gradle/api/tasks/TaskOutputs.html#cacheIf-org.gradle.api.specs.Spec-[TaskOutputs.cacheIf()] method.
In this case, it's always allowed.
We also need to assign a property name to each input and output, used by the cache to identify them.
...
Instead of using the runtime API, it is actually better to create a custom task class which is cacheable using <<more_about_tasks.adoc#sec:task_input_output_annotations,annotations to declare the inputs and outputs>>.

We continue the example from above, trying to get a cacheable bundle task.
Given that we already have the `NpmTask`, we now create a subclass declaring the inputs and outputs.

When possible, it is better to use delegation instead of creating a subclass.
For example, instead of subclassing the `JavaExec` or `Exec` task, create a new task with proper input properties and call `project.javaexec()` or `project.exec()` in its task action.
...
The annotations have the same effect as in the runtime API example.
