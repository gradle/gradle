...
As we have seen, built-in tasks, or tasks provided by plugins, are cacheable if their class is annotated with the `Cacheable` annotation. But what if you want to make cacheable a task whose class is not cacheable? Let's take a concrete example: your build script uses an Exec task to create a JavaScript bundle by delegating to webpack. This process is similar to a complex compilation task, but obviously, Exec is too generic to be cacheable by default. We will simulate this situation by using a `JavaExec` task which concatenates the files of a source directory.
...
.A fake bundle task that concatenates JavaScript files found in a directory
====
include::sample[dir="buildCache/non-cacheable-bundle/groovy",files="build.gradle[tags=bundle-task]"]
include::sample[dir="buildCache/non-cacheable-bundle/kotlin",files="build.gradle.kts[tags=bundle-task]"]
====

This fake task isn't worth caching: simply concatenating files is very fast: faster than getting the cached result from the cache. But a real JavaScript bundling task can be quite long, and is worth caching. To achieve that, we need to tell Gradle when it's allowed to cache the output of that task, using the link:{javadocPath}/org/gradle/api/tasks/TaskOutputs.html#cacheIf-org.gradle.api.specs.Spec-[TaskOutputs.cacheIf()] method. In this case, it's always allowed. We also need to assign a property name to each input and output, used by the cache to identify them.
...
Note that, since the arguments of the `JavaExec` task are declared as inputs by the `JavaExec` task itself, we also need to use relative paths as arguments.
