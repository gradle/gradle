...
== Enable caching of ad-hoc tasks

As we have seen, built-in tasks, or tasks provided by plugins, are cacheable if their class is annotated with the `Cacheable` annotation. But what if you want to make an ad-hoc task cacheable? Or if you want to make cacheable a specific instance of a task whose class is not cacheable? Let's take a concrete example: your build script uses an Exec task to create a JavaScript bundle by delegating to webpack. This process is similar to a complex compilation task, but obviously, Exec is too generic to be cacheable by default. We will simulate this situation by simply writing an ad-hoc task that concatenates the files of a source directory.
