...
If you are coming from another native build system, those concepts may be overwhelming at first but serve the purpose to simplify the whole build script authoring.
It borrows several conventions from Apache Maven.
In particular, it uses the same default directory pattern for source files and headers.

We will look at C\++ projects in detail in this chapter, but most of the topics apply to other supported native languages as well.
If you don’t have much experience with building native projects with Gradle, take a look at the C++ tutorials for step-by-step instructions on how to build various types of basic C++ projects as well as some common use case.

The C++ plugins covered in this chapter were https://blog.gradle.org/introducing-the-new-cpp-plugins[introduced in 2018] and we recommend users to use those plugins over <<native_software.adoc#native_binaries,the older Native plugins>>.
...
By applying any of C++ plugin, you get a whole host of features:

 * A `compileDebugCpp` and `compileReleaseCpp` tasks that compiles the C++ source files under _src/main/cpp_ for the well-known debug and release build type respectively.
 * A `linkDebug` and `linkRelease` tasks that links the compiled C++ object files into an executable for applications or shared library for libraries with shared linkage for the well-known debug and release build type respectively.
 * A `createDebug` and `createRelease` tasks that assemble the compiled C++ object files into a static library for libraries with shared linkage for the well-known debug and release build type respectively

This isn’t sufficient to build any non-trivial C++ project - at the very least, you’ll probably have some file dependencies.
But it means that your build script only needs the information that is specific to _your_ project.

// TODO: Add base plugin link
The C++ plugins also integrate the above tasks into the standard Lifecycle Base Plugin lifecycle tasks:

 * `linkDebug` is attached to `assemble`

The rest of the chapter explains the different avenues for customizing the build to your requirements. You will also see later how to adjust the build specifically for libraries and applications.

== Build variants
Gradle is able to build a number of different native binary variants.
They can be differentiated on build type footnote:[For example, debug or release], platform footnote:[For example, Windows x86 or macOS x86-64] and linkage footnote:[For example, shared or static linkage] (library-only).
For each of these 'variant dimensions', tasks, binaries and configuration are created.

[[sec:cpp_source_sets]]
== Declaring your source files

// TODO: Add DSL link to application and library script block
Gradle’s C++ support use `FileCollection` configurable directly from the `application` or `library` script block.
You can configure the compilation unit as well as the include roots to use for resolving headers.
...
// TODO: Add link to Testing C++ projects
...
The vast majority of C++ projects rely on libraries, so managing a project’s dependencies is an important part of building a C++ project.
Dependency management is a big topic, so we will focus on the basics for C++ projects here.
...
Unlike the Java world, C++ project often built from source.
However, Gradle provide support for consuming pre-built binaries from Maven repositories footnote:[Unfortunately, Conan and Nuget repositories aren’t yet support as core features] published by Gradle.
We will cover how to add dependencies on projects within a multi-build project.

Specifying the dependencies for your C++ project requires just two pieces of information:

 * Which dependency you need as the project path
 * What it’s needed for, e.g. compilation, linking, or both

The information are specified in a `dependencies {}` block of the C++ `application` or `library` script block.
For example, to tell Gradle that your project requires library `Foo` to compile and link your production code, you can use the following fragment:

.Declaring dependencies
====
include::sample[dir="userguide/cpp/basic/groovy",files="build.gradle[tags=cpp-dependency-mgmt]"]
include::sample[dir="userguide/cpp/basic/kotlin",files="build.gradle.kts[tags=cpp-dependency-mgmt]"]
====
...
// TODO: Link to C++ Library Plugin reference chapter
 ...
 * Defining a custom <<repository_types.adoc#maven_repo,Maven-compatible>> repository
 * Declaring dependencies with <<declaring_dependencies.adoc#declaring_dependency_with_changing_version,changing>> (e.g. SNAPSHOT) and <<declaring_dependencies.adoc#declaring_dependency_with_dynamic_version,dynamic>> (range) versions
 * Declaring a sibling <<declaring_dependencies.adoc#declaring_project_dependency,project as a dependency>>
 * <<managing_transitive_dependencies.adoc#managing_transitive_dependencies,Controlling transitive dependencies and their versions>>
 * Testing your fixes to 3rd-party dependency via <<composite_builds.adoc#composite_builds,composite builds>> (a better alternative to publishing to and consuming from <<repository_types.adoc#maven_local,Maven Local>>)
...
3. Run the `linkDebug` or `linkRelease` task for executables and shared libraries or the `createDebug` or `createRelease` task for static libraries

Other C++ language plugins follow the same pattern of conventions.
...
GCC and Clang installed using Macports and Homebrew have been reported to work fine but isn’t tested continuously.

// TODO: Check to see if we use any headers
==== Windows
...
You do that via the `library` script block.
...
====
include::sample[dir="userguide/cpp/basic/groovy",files="build.gradle[tags=cpp-select-target-machines]"]
include::sample[dir="userguide/cpp/basic/kotlin",files="build.gradle.kts[tags=cpp-select-target-machines]"]
====
 ...
 * For application, Gradle support installing all dependencies (shared library) and the executable with run script that configure everything properly.
 * Executable can also be published inside maven repositories
 * Shared and static libraries are published inside maven repositories together with a zip of their headers

[[sec:cleaning_cpp_build]]
== Cleaning the build

The C++ Application and Library Plugin adds a `clean` task to you project by virtue of applying the Lifecycle Base Plugin.
...
// TODO: Link to C++ library plugin reference chapter
Gradle manages this distinction via the C++ Library Plugin, which introduces an _api_ configuration in addition to the _implementation_ once covered in this chapter.
...
// TODO: link to C++ library plugin reference chapter
// FIXME: Ensure the difference between API and implementation is explained
...
// TODO: link to C++ application plugin reference chapter
