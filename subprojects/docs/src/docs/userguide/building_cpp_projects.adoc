...
A dimension is simply a category, where each dimension is orthogonal to the rest.
For example, the "build type" dimension is a category that includes debug and release builds.
The "architecture" dimension covers processor architectures like x86_64 and PowerPC.

A variant is a combination of values for these dimensions, consisting of exactly one value for each dimension.
So based on the dimensions just mentioned, you might have a "debug x86_64" or a "release PowerPC" variant.
