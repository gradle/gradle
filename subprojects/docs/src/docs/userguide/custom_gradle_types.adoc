...
The custom Gradle types that you implement often holds some configuration that you want to make available to build scripts and other plugins.
This configuration is represented as Java bean properties. For example, a download task may have properties that specify the URL to download
from, and the location to write the result to.
...
Kotlin and Groovy both provide conveniences for implementing Java bean properties. These are demonstrated in the samples below.

=== Managed mutable properties

Gradle can optionally provide an implementation of a property declared on a type. This is called a _managed property_, as Gradle takes care of managing the state of the property.
To declare a managed mutable property, add an abstract getter method and an abstract setter method for the property.
...
Note that for a property to be considered a managed mutable property, _all_ of the property's getter methods and setter methods must be `public` or `private` and `abstract`.

=== Managed read-only properties

Gradle can also provide an implementation of a managed read only property declared on a type. To declare a managed read-only property, add an abstract getter method for the property.
The property should not have any setter methods. This is a useful pattern to use when the property uses one of Gradle's configurable lazy property types.
...
Note that for a property to be considered a managed read only property, _all_ of the property's getter methods must be `public` or `private` and `abstract` and the property must not have any setter methods.
