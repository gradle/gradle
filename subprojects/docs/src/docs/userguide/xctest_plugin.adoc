...
You <<building_swift_projects.adoc#sec:custom_cpp_source_set_paths,configure the project layout>> by configuring the `source` on the `xctest` script block.
...
link:{groovyDslPath}/org.gradle.nativeplatform.tasks.LinkMachOBundle.html#org.gradle.nativeplatform.tasks.LinkMachOBundle:toolChain[toolChain]:: automatically selected based on target machine
