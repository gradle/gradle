...
When this property has no value, which is the default, Gradle does not constrain access to the service. When this property has a value > 0, Gradle will allow no more than the specified number of concurrent usages.
By default, each task consumes 1 parallel usage, but this can be adjusted by using link:{javadocPath}/org/gradle/api/services/BuildServiceProvider.html#withParallelUsages--[BuildServiceProvider.withParallelUsages(int)].
