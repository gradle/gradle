...
- If you specify a task option name that conflicts with the name of a <<command_line_interface.adoc#sec:command_line_debugging, built-in Gradle option>>, use the `--` delimiter before calling your task to reference that option. For more information, see <<command_line_interface.adoc#disambiguate_task_options_from_built_in_options, Disambiguate Task Options from Built-in Options>>.
