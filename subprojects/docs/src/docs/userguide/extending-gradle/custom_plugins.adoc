...
Now we will move our plugin to a standalone project so that we can publish it and share it with others. This project is simply a Groovy project that produces a JAR containing the plugin classes.
...
// TODO - see if the snippets can be cleaned up/removed before deleting this
//.A build for a custom plugin
//====
