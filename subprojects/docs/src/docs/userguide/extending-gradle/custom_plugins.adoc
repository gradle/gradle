...
In addition to plugins written as regular projects, Gradle allows you to provide the plugin logic
in either Groovy or Kotlin DSL as precompiled script plugins. You write these as `\*.gradle` files in `src/main/groovy`
directory or `*.gradle.kts` files in `src/main/kotlin` directory.

Precompiled script plugins are compiled as part of a regular source set and placed on the build classpath or packaged in
a binary plugin, depending on what type of project they are in. For all intents and purposes, they are binary plugins,
particularly as they can be applied by plugin ID, just like a regular binary plugin. In fact, the plugin metadata for
them is generated via implicitly applied <<java_gradle_plugin#java_gradle_plugin,Gradle Plugin Development Plugin>>.
...
Groovy DSL precompiled script plugins are available starting with Gradle 6.4 and they can not be used with earlier versions.
====
...
[.multi-language-text.lang-groovy]
First, you need a `buildSrc/build.gradle` file that applies the `precompiled-groovy-plugin` plugin:
