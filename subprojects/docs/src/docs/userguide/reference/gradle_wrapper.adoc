...
To specify default HTTP Basic Authentication credentials, add the following lines to the system properties file:
...
Note! These credentials will be sent to any distribution server not explicitly specified. It is recommended to not use these properties!

To ensure that HTTP Basic Authentication credentials are sent only to a specific distribution server (preferred, from 8.5):

[source,properties]
----
systemProp.gradle.wrapperUser.somehost.domain.name=username-only-sent-to-somehost.domain.name
systemProp.gradle.wrapperPassword.somehost.domain.name=password-only-sent-to-somehost.domain.name
----
