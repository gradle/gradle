...
[NOTE]
====
The `Wrapper` task fails if `gradle-wrapper.properties` contains
`distributionSha256Sum`, but the task configuration does not define a sum.
Executing the `Wrapper` task preserves the `distributionSha256Sum`
configuration when the Gradle version does not change.
====
