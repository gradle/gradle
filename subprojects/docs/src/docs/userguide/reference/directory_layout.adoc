...
- Version-specific caches in `caches/{gradleVersion}/` (or `caches/<gradle-version>`)) are checked for whether they are still in use.  If not, directories for release versions are deleted after 30 days of inactivity, snapshot versions after 7 days of inactivity.
...
After building the project, version-specific cache directories in `.gradle/{gradleVersion}/` (or `./gradle/<gradle-version>`) are checked periodically (at most every 24 hours) for whether they are still in use.
