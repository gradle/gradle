...
You can also run a task for _all_ subprojects by again using a task _selector_ that consists of the task name only.
...
You also get a project report within Build Scan™. Learn more about https://scans.gradle.com/[creating Build Scan™].
...
A Build Scan™ gives a full, visual report of what dependencies exist on which configurations, transitive dependencies, and dependency version selection.
