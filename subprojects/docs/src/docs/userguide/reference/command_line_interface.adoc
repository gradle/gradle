...
Then set to `true` and debugging enabled, Gradle will run the build with socked-attach mode of debugger. Otherwise socket-listen is used.

`-Dorg.gradle.debug.suspend=true`::
When set to `true` and debugging enabled, Gradle will suspend the virtual machine until a debugger is attached.
