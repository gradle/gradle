...
Especially, continuous build does not work when using `--no-daemon`.
...
Under some circumstances continuous build will not detect changes to inputs.

[[sec:continuous_build_missing_files]]
==== Adding new root input directories

Sometimes, adding root input directories does not trigger a build, due to the way file system watching works.
For example, when adding the `src/main/java` directory, Gradle may not trigger a build.
Similarly, if the input is a <<working_with_files.adoc#filtering_files,filtered file tree>>, Gradle may not detect the addition of the first file matching the filter.

[[sec:continuous_build_untracked]]
==== Inputs of untracked tasks

Gradle will only watch for changes of inputs of tasks if it captures their state.
That means changes to the inputs of <<more_about_tasks.adoc#sec:disable-state-tracking,untracked tasks>> or tasks that have no outputs will not trigger a build.
...
In general, Gradle will not detect changes to symbolic links or to files in the target of symbolic links.
