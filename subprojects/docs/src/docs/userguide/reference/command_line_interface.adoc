...
Toggles the <<configuration_cache.adoc#sec:config_cache,Configuration Cache>>. Gradle will try to reuse the build configuration from previous builds. This is an incubating and highly experimental feature, it is disabled by default.

`--configuration-cache-problems=(fail,warn)`::
Configures how the configuration cache handles problems. Defaults to `fail`.
...
`-t`, `--continuous`::
Enables continuous build. Gradle does not exit and will re-execute tasks when task file inputs change. See <<#sec:continuous_build, Continuous Build section>> for more details.
...
Learn more about this in <<dependency_verification.adoc#sec:verifying-dependencies,dependency verification>>.
...
Continuous Build allows you to automatically re-execute the requested tasks when task inputs change. You can execute the build in this mode the `-t` or `--continuous` command-line option.
