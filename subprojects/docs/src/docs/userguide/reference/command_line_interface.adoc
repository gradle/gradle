...
Note that command line order safety relies on tasks properly declaring what they create, consume or remove.  See <<incremental_builds.adoc#incremental_builds,up-to-date checks>> for further information.
...
You can force Gradle to execute all tasks ignoring <<incremental_builds.adoc#incremental_builds,up-to-date checks>> using the `--rerun-tasks` option:
...
Changes to the inputs of <<incremental_builds.adoc#sec:disable-state-tracking,untracked tasks>> or tasks that have no outputs may not trigger a build.
