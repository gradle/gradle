...
Gradle assumes to find metadata when resolving dependencies. Projects may want to use Gradle's dependency resolution engine to download artifacts from repositories that do not provide metadata e.g. just a plain ZIP file or if the project only needs to process a particular artifact without
