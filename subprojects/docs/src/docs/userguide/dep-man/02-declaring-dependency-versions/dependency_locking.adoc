...
In order to lock the <<writing_plugins.adoc#sec:applying_plugins_buildscript,`classpath` configuration>> used for script plugins, do the following:
...
The one exception to this rule is for the lock file for the <<writing_plugins.adoc#sec:applying_plugins_buildscript,buildscript itself>>.
