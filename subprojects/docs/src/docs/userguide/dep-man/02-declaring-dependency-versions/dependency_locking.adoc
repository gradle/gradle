...
In some scenarios, projects use dependency locking to prevent unwanted dependency updates more than for the reproducibility aspect.
In that context, it is convenient to be able to ignore some dependencies for the lock state.
An example is company internal dependencies that should always use the most recent version.
