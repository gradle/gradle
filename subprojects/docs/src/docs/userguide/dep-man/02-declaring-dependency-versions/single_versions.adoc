...
[NOTE]
====
Gradle 6.5 introduces a number of changes around version sorting, by special casing more suffixes:

* The string `SNAPSHOT` will be sorted higher than `rc`: `1.0-rc` < `1.0-SNAPSHOT` < `1.0`
* The string `ga` will be sorted next to `final` and `release`, in alphabetical order: `1.0-rc` < `1.0-final` < `1.0-ga` < `1.0-release` < `1.0`
* The string `sp` will be sorted higher than `release`, it remains however lower than an unqualified version, limiting its use to versioning schemes using either `final`, `ga` or `release`: `1.0-release` < `1.0-sp1` < `1.0`

This set of changes is enabled through the feature preview `DEPENDENCY_VERSION_SORTING` in `settings.gradle(.kts)`:
```
