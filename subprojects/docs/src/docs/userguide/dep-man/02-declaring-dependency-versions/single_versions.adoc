...
[NOTE]
====
Gradle 6.5 supports an alternate, opt-in, version sorting scheme which special cases more suffixes:
...
Activating the feature preview `VERSION_SORTING_V2` in `settings.gradle(.kts)` enables this set of changes:
```
