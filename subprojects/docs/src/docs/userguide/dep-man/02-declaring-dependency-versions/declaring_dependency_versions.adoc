...
====
include::sample[dir="userguide/dependencyManagement/declaringDependencies/strictForSubgraph/groovy",files="build.gradle[tags=dependencies-for-subgraph]"]
include::sample[dir="userguide/dependencyManagement/declaringDependencies/strictForSubgraph/kotlin",files="build.gradle.kts[tags=dependencies-for-subgraph]"]
====
...
You can also _inherit_ the strict version constraints defined by another module so that they are applied to all your dependencies as well.
...
You can however activate it for arbitrary dependencies by setting `inheritStrictConstraints()` on the dependency.
