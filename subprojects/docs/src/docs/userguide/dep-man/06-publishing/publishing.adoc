...
To some extent, you can see those extra artifacts as being published "out of context".
The only way to address them as a consumer is to either to download them directly or use a _classifier_ notation.
...
Please refer to the <<modeling_features.adoc#sec:variant-aware-sharing, variant-aware sharing>> section of the documentation to see how to declare such variants and <<#publishing-custom-components, check out how to publish custom components>>.
====
...
- <<modeling_features.adoc#understanding-variant-selection,variant-aware resolution>>
...
|<<#publishing-custom-components,Custom component types>>
...
Because Gradle Module Metadata is not widely spread and that it aims at <<#mapping-with-other-formats,maximizing compatibility with other tools>>, Gradle does a couple of things:
...
It doesn't mean that consumption from Maven or Ivy would be broken either, only that it works in <<#mapping-with-other-formats,degraded mode>>.

[NOTE]
====
This must be seen as a _performance optimization_: instead of having to do 2 network requests, one to get Gradle Metadata, then one to get the POM/Ivy file in case of a miss, Gradle will first look at the file which is most likely to be present, then only perform a 2d request if the module was actually published with Gradle Module Metadata.
====
...
[[sec:software_component_factory]]
...
The `AdhocComponentWithVariants` component declares a method called `addVariantsFromConfiguration` which accepts two parameters:

- the <<declaring_dependencies.adoc#sub:resolvable-consumable-configs,outgoing configuration>> that is going to be used as a variant source
...
In the <<sec:software_component_factory, previous example>>, we have demonstrated how to create an adhoc component.
...
We still recommend to use <<#sec:software_component_factory,adhoc component publication>> which is safer and future-proof, but here's how you can do it.
...
If this is what you need, then you should most likely define an <<#sec:software_component_factory,adhoc component>> instead.
====
