...
This plugin provides <<dependency_management_terminology.adoc#sub:terminology_publication,publications>> of type link:{groovyDslPath}/org.gradle.api.publish.ivy.IvyPublication.html[IvyPublication]. To learn how to define and use publications, see the section on <<publishing_setup.adoc#sec:basic_publishing,basic publishing>>.
...
====
include::sample[dir="snippets/ivy-publish/java-multi-project/groovy",files="build.gradle[tags=all]"]
include::sample[dir="snippets/ivy-publish/java-multi-project/kotlin",files="build.gradle.kts[tags=all]"]
====
...
Instead, it is recommended to replace the Gradle `force` with a <<rich_versions.adoc#sec:strict-version,`strictly` version>>, which provides <<dependency_downgrade_and_exclude.adoc#sec:enforcing_dependency_version,better semantics>> and is supported by the Gradle Module Metadata format.
