...
for example `foo-bar` as an alias is converted to `foo.bar` automatically. Normalized aliases must be also used when programmatically accessing catalog with VersionCatalogExtension API.
====
