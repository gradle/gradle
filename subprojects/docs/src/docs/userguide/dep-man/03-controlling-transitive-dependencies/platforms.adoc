...
A _version catalog_ is a list of dependencies, represented as dependency coordinates only, that a user can pick from when declaring dependencies in a build script.
...
- catalogs are visible to all projects of a build: in a single place, you can declare the version of a dependency and make sure that if you change this version, it's done in every subproject
- catalogs can declare <<platforms.adoc#sec:dependency-bundles, dependency bundles>>, which are "groups of dependencies" which are commonly used together
- catalogs can separate the group and name of a dependency from its actual version and use <<platforms.adoc#sec:common-version-numbers, version references>> instead, making it possible to share a version declaration for different dependencies.
...
WARNING: A dependency catalog doesn't enforce the version of a dependency: like a regular dependency notation, it declares the requested version and can consist of <<rich_versions.adoc#rich-version-constraints,rich versions>>. It's not necessarily what is going to be resolved in a dependency graph: the usual conflict resolution still happens.
...
Declaring a dependency using the following notation:
...
The semantics are again strictly equivalent: adding a single bundle is equivalent to adding individually the dependencies which are part of the bundle.
...
But be aware that it's likely that some plugins add dependencies transparently, therefore dependencies defined outside of this file.

Just like `src/main/java` is a convention to find the Java sources which doesn't prevent additional source directories to be declared (either in a build script or a plugin), the presence of the `dependencies.toml` file doesn't mean that it's impossible to declare dependencies elsewhere.
...
.The dependencies.toml file
====
include::sample[dir="snippets/dependencyManagement/catalogs-toml/groovy/gradle",files="dependencies.toml"]
include::sample[dir="snippets/dependencyManagement/catalogs-toml/kotlin/gradle",files="dependencies.toml"]
====
...
====
include::sample[dir="snippets/dependencyManagement/catalogs-toml/groovy/gradle",files="test-dependencies.toml"]
include::sample[dir="snippets/dependencyManagement/catalogs-toml/kotlin/gradle",files="test-dependencies.toml"]
====
...
====
include::sample[dir="snippets/dependencyManagement/catalogs-toml/groovy/buildSrc",files="settings.gradle[tags=consume_catalog]"]
include::sample[dir="snippets/dependencyManagement/catalogs-toml/kotlin/buildSrc",files="settings.gradle.kts[tags=consume_catalog]"]
====
...
One option to share a catalog is to write a settings plugin, publish it on the Gradle plugin portal, and let the consumers apply the plugin on their settings file.
...
Because platforms and catalogs both talk about dependency versions, can both be used to share dependency versions in a project, there might be a confusion regarding what to use and if one is preferable to the other.

Ignoring the incubating state of version catalogs, the answer is that you should always use catalogs and _maybe_ use platforms in addition.
...
It will be used by Gradle to generate type-safe accessors, present short-hand notations for external dependencies and it allows sharing easily those coordinates between different projects.
...
dependencies {
    constraints {
        api(libs.mylib) // a platform can reference a dependency declared in a catalog
    }
}
```
