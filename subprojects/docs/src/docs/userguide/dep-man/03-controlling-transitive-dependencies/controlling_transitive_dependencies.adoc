...
In most cases it's exclusively composed of link:#sec:adding-constraints-transitive-deps[dependency constraints] which will either _suggest_ dependency versions or _enforce_ some versions.
...
It's also common to find platforms published as Maven BOMs which link:#sec:sub:bom_import[Gradle supports natively].
