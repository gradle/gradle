...
In all situations, this is better expressed by saying that your code _strictly depends on_ a version of a transitive.
...
A dependency version can be enforced by declaring as strict it in the build script:
...
Using a strict version must be done with care, in particular for library authors.
