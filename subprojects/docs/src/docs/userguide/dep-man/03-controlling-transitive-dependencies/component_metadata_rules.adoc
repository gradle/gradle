...
- The <<modeling_features.adoc#declaring-component-capabilities,capabilities>> the variant provides - `withCapabilities { }` block
...
- The <<controlling_transitive_dependencies.adoc#sec:adding-constraints-transitive-deps,dependency constraints>> of the variant, including <<rich_versions.adoc#,rich versions>> - `withDependencyConstraints {}` block
...
- The _belongsTo_ property for <<modeling_features.adoc#sec:declaring-capabilities-external-modules,version alignment through virtual platforms>>
...
Such information cannot be represented in pom files, but in Gradle Module Metadata through variants and <<modeling_features.adoc#declaring-component-capabilities,capabilities>>.
...
This example, and how it can be tackled with a component metadata rule, is described in detail in the  <<modeling_features.adoc#sec:declaring-capabilities-external-modules,feature modelling>> section.
...
It is explained in detail in the section on <<controlling_transitive_dependencies.adoc#sec:align-versions-virtual,aligning versions of modules not published with Gradle>>.
...
While variant selection can handle <<variant_aware_plugins.adoc#sec:abm_declaring_attributes,any custom attribute>>, version selection only considers attributes for which specific semantics are implemented.
...
A module's status is taken into consideration when a <<declaring_dependency_versions.adoc#sec:dynamic_versions_and_changing_modules,_latest_ version selector>> is resolved. Specifically, `latest.someStatus` will resolve to the highest module version that has status `someStatus` or a more mature status.
