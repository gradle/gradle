...
Instead of declaring rules on a project's component metadata handler, it is possible to declare rules in the `settings.gradle(.kts)` file.
In this case, rules declared in settings are the _conventional_ rules applied to a project: if the project doesn't declare any rules, the rules from the settings script will be used.
