...
[NOTE]
====
When a region-specific endpoint is not specified for buckets requiring V4 Signatures, Gradle uses the default AWS region (us-east-1) and the
...
that allows you to declare only the type of required credentials. Gradle looks up credentials from your <<build_environment.adoc#sec:gradle_configuration_properties,Gradle Properties>> when a build requires them.
...
You can provide credentials to Gradple properties in the following ways:
...
However, sometimes you must use a local Maven repository:
...
If you do use a local Maven repository combine it with <<#sec:repository-content-filtering,a repository filter>> to ensure it only provides specific dependencies.
