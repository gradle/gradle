...
When using an AWS S3-backed repository, you need to authenticate using link:{groovyDslPath}/org.gradle.api.credentials.AwsCredentials.html[AwsCredentials], via an access-key and a private-key.
The following example shows how to declare a S3 backed repository and provide AWS credentials:
...
For details about authenticating with AWS S3, see <<#sec:s3-repositories,Declare an AWS S3 Repository>>.
...
For details about authenticating with GCS, see <<#sec:gcs-repositories,Declare a GCS Repository>>.
...
When you define a custom repository, you should configure this behavior.
...
When parsing a metadata file, Gradle looks for a marker indicating that a matching Gradle Module Metadata file exists.
...
Used to override the AWS S3 endpoint when using an S3-compatible storage service that is _not_ AWS.
...
For more information, see the link:https://docs.aws.amazon.com/AmazonS3/latest/dev/s3-access-control.html[AWS S3 Access Permissions].
...
Used to override the GCS endpoint when using a GCS-compatible storage service that is _not_ GCP.

`org.gradle.gcs.servicePath`::
Used to override the GCS root service path used to build Google Cloud Storage client requests. Defaults to `/`.
...
If you _do_ use a local Maven repository, combine it with <<declaring_repositories.adoc#sec:repository-content-filtering,a repository filter>> to ensure it only provides specific dependencies.
