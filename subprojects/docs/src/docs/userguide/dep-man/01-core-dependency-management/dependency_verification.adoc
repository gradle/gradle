...
[NOTE]
====
By default, this command will generate *both* the binary version and the ASCII armored file.
...
You can also configure which file type would be generated when keys are exported by Gradle by adding the `keyring-format` setting.
...
Available options for keyring format are `text` and `gpg`.
Without this setting, Gradle will generate both formats.
