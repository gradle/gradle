...
You can visualize dependencies with the built-in Gradle CLI `dependencies` task or with link:https://scans.gradle.com/[build scans].
...
By default, the dependency tree renders dependencies for all <<declaring_dependencies.adoc#sec:what-are-dependency-configurations,configurations>>, but only a <<command_line_interface#executing_tasks_in_multi_project_builds,single project>>.
...
- `(c)`: Dependencies whose version is limited by dependency constraints.
- `(n)`: A dependency or dependency configuration that cannot be resolved. Sometimes called a producer.

For more information about unresolvable dependency configurations, see <<declaring_dependencies.adoc#sec:resolvable-consumable-configs>>.
...
To see a list of all the pre-defined configurations added by the `java` plugin, see <<java_plugin.adoc#sec:java_plugin_and_dependency_management,the documentation for the Java Plugin>>.
...
Dependency insights only provide information about a single project at a time.
...
The dependency to generate an insight report for.
You can supply a complete `group:name`, or part of it.
If multiple dependencies match, Gradle generates a report for each matching dependency.
`--configuration <name>` (mandatory)::
The dependency configuration from which to supply the dependency information.
...
| Was requested : <text>
| The dependency appears and the inclusion came with a <<declaring_dependencies#sec:documenting-dependencies, `because` text>>.
