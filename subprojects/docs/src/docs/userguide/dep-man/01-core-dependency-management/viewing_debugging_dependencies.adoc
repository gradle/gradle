...
Dependency insights provide information about a single <<declaring_dependencies.adoc#sec:what-are-dependency-configurations,configuration>> within a single <<command_line_interface#executing_tasks_in_multi_project_builds,project>>.
...
The following code snippet demonstrates how to run a dependency insight report for a all paths to a dependency named "commons-codec" within the "scm" configuration:
