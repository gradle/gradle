...
There are a number of ways a configuration might be resolved unsafely and Gradle will fail with an exception for each unsafe access.
...
If you find a use case that cannot be resolved using these techniques, please let us know by filing a https://github.com/gradle/gradle/issues[GitHub Issue] adhering to our issue guidelines.
