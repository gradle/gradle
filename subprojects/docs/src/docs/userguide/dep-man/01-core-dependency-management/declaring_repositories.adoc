...
Instead of declaring repositories in every subproject of your build or via an `allprojects` block, Gradle offers a way to declare them once for every project.
...
This means that repositories declared in settings are the _conventional_ repositories used by a project, but they can be overridden by project.
