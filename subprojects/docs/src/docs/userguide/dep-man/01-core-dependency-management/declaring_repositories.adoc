...
You can specify repositories as locations in a file system, or as URLs.
...
To declare a repository that doesn't have a built-in shorthand notation, <<#sec:declaring_custom_repository,declare a custom repository>>.
...
To add an Ivy repository with a named layout to your build, add the following to your build script:
...
To use Maven style organization layouts, assign `m2compatible` a value of "true" in your pattern layout configuration.
