...
====
include::sample[dir="snippets/artifacts/defineRepository/groovy",files="build.gradle[tags=maven-jcenter]"]
include::sample[dir="snippets/artifacts/defineRepository/kotlin",files="build.gradle.kts[tags=maven-jcenter]"]
====
...
In the real world scenario you should not specify repository credentials in your build script but rather keep them externalized.
...
if it knows that the build will fail down the line because of missing credentials.
...
Same rules apply for supplyng link:{javadocPath}/org/gradle/api/credentials/AwsCredentials.html[AwsCredentials].
