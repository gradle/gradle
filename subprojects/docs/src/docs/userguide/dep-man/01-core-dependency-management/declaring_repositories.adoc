...
You can declare more than one repository within a single build.
...
You can filter the following properties with strict string matching or regular expressions:
...
Version filtering also supports version ranges using <<single_versions.adoc#single-version-declarations,the Gradle version format>>.
