...
[[sec:gradle-plugin-dependencies]]
==== Gradle plugin dependencies
<<plugins.adoc#plugins, Gradle plugins>> are used to add new tasks, domain objects and conventions to a project as well as to extend core objects and objects from other plugins.
A dependency on a plugin can be declared using the id and (optionally) the version of the plugin.

====
include::sample[dir="snippets/plugins/declaringDependencies/kotlin", files="build.gradle.kts[tags=declaring_dependencies_block]"]
include::sample[dir="snippets/plugins/declaringDependencies/groovy", files="build.gradle[tags=declaring_dependencies_block]"]
====
<1> Declaring plugin dependencies in the main dependency block.
<2> Declaring plugin dependencies in the test-suites block.

For more information see the API documentation for <<plugins.adoc#plugins, plugins>>.
