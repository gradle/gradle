...
If no such module metadata file exists, as of Gradle 6.0, you need to configure <<repositories_cheat_sheet.adoc#sec:supported_metadata_sources,metadata sources definitions>> to look for an artifact file called `hibernate-3.0.5.jar` directly.
====
...
You can also tweak which format will be looked up in the <<repositories_cheat_sheet.adoc#sec:supported_metadata_sources,repository definition>>.
