...
In the past, Configurations did not differentiate between roles.
Some configurations which previously had extra roles enabled may mark one or more roles as _deprecated_ before fully disabling them.
When a feature associated with a deprecated role is utilized, a deprecation warning is emitted.
Configurations with deprecated roles will stop supporting the marked role in future versions.
