...
Type-safe project accessors also accept optional parameter for dependency configuration. For example, `project(":some:path", "distribution")` can be rewritten as follows:

.Declaring project dependencies with custom configurations using the type-safe API
====
include::sample[dir="snippets/dependencyManagement/declaringDependencies-typeSafeProjectDependencies/groovy",files="web-service-custom-configurations/build.gradle[tags=project-dependencies]"]
include::sample[dir="snippets/dependencyManagement/declaringDependencies-typeSafeProjectDependencies/kotlin",files="web-service-custom-configurations/build.gradle.kts[tags=project-dependencies]"]
====
