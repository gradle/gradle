...
Before looking at dependency declarations themselves, the concept of _dependency scope configuration_ needs to be defined.

[[sec:what-are-dependency-configurations]]
== What are dependency scope configurations

Every dependency declared within a Gradle project applies to a specific scope.
...
Gradle represents the scope of a dependency with the help of a specialized link:{groovyDslPath}/org.gradle.api.artifacts.Configuration.html[Configuration], called a _Dependency Scope Configuration_.
The two other types of configurations, <<#sec:resolvable-consumable-configs,_Resolvable_ and _Consumable_>>, will be described later.
Within a single project, every configuration is uniquely identified by its name.

Many Gradle plugins add pre-defined configurations to your project.
The Java plugin, for example, adds configurations to represent the various classpaths it needs for compiling source code, executing tests, etc.
See <<java_plugin.adoc#sec:java_plugin_and_dependency_management,the Java plugin chapter>> for more details.
...
For example, the `testImplementation` configuration extends the `implementation` configuration.
 The configuration hierarchy has a practical purpose: compiling tests requires the dependencies of the source code under test in addition to the dependencies needed to write the test class.
 A Java project that requires Guava for its production code will also require it during testing, along with a JUnit dependency.

.Configuration inheritance provided by the Java plugin
image::dependency-management-configuration-inheritance.png[]

Under the covers, the `testImplementation` and `implementation` configurations form an inheritance hierarchy by calling the method link:{groovyDslPath}/org.gradle.api.artifacts.Configuration.html#org.gradle.api.artifacts.Configuration:extendsFrom(org.gradle.api.artifacts.Configuration++[]++)[Configuration.extendsFrom(Configuration++[]++)].
A configuration can extend any other configuration irrespective of its definition in the build script or a plugin.

[NOTE]
====
It is best practice to only extend from a _Dependency Scope_ configuration.
Extending from a _Resolvable_ or _Consumable_ configuration is not recommended and will become an error in future Gradle versions.
====

Let's say you wanted to write a suite of smoke tests.
Each smoke test makes an HTTP call to verify a web service endpoint.
Since the underlying test framework for the project already uses JUnit, you can define a new configuration named `smokeTest` that extends from the `testImplementation`.
All dependencies declared on the `testImplementation` configuration will be inherited by the `smokeTest` configurations, allowing it to reuse the existing test framework dependency.
...
While a _Dependency Scope_ configuration is used to declare dependencies, _Resolvable_ and _Consumable_ configurations leverage those declared dependencies.
These three configuration types are formally defined in the <<dependency_management_terminology.adoc#sub:terminology_configuration, terminology section>>.

A _Consumable_ configuration is a _producer_ of artifacts and represents a <<variant_model.adoc#understanding-variant-selection,variant>>.
It exposes a project's artifacts and their dependencies, to be _consumed_ by other projects.

A _Resolvable_ configuration acts as a _consumer_.
It performs _resolution_, which may then _consume_ the artifacts from external dependencies or from the Consumable configurations of other projects.

All Configurations in a given project are created and owned by its  link:{groovyDslPath}/org.gradle.api.artifacts.ConfigurationContainer.html[ConfigurationContainer].

Consider the following example, which uses Dependency Scope Configurations to declare the that current project `app` depends on other Gradle projects in the build.
We declare that our implementation _depends on_ the library produced by the `lib` project.
We also declare that our runtime _depends on_ the library produced by the `anotherLib` project.

.Dependency Scope Configurations are used to declare dependencies
====
...
1. **Defining Dependency Scope configurations**.
This defines a logical group of dependencies which are used for a specific purpose.
Writing `isCanBeResolved = false` and `isCanBeConsumed = false` defines the configuration as a Dependency Scope configuration.
Resolvable and Consumable configurations may extend the configuration to gain access to its declared dependencies.
2. **Declaring a dependency on the configuration**.
These declarations express that the current project depends on the libraries produced by the `lib` and `anotherLib` projects.
Consumable configurations that extend these configurations will advertise these dependencies to consumers.
Resolvable configurations that extend these configurations will resolve these dependencies when building their graphs.

Notice that the code above does not describe how the dependencies declared on `implementation` or `runtimeOnly` are actually used.
Imagine that `lib` is a Java library and therefore might expose different variants, such as its API, runtime, or test fixtures.

Depending on the task being performed in `app`, we may want the dependency on `lib` to resolve differently.
For example, if we are compiling `app`, we only want the dependency to resolve to its API.
Similarly, if we are executing `app`, we want the dependency to resolve to its runtime.

A _Resolvable Configuration_ controls how a given set of dependencies is resolved.
Different resolvable configurations may resolve the same set of dependencies differently.

.Resolvable Configurations representing concrete dependency graphs
====
include::sample[dir="snippets/dependencyManagement/attributeMatching/kotlin",files="build.gradle.kts[tags=resolvable-configurations]"]
include::sample[dir="snippets/dependencyManagement/attributeMatching/groovy",files="build.gradle[tags=resolvable-configurations]"]
====

Above, we declare two resolvable configurations: `compileClasspath` and `runtimeClasspath`.
`implementation` and `runtimeOnly` are solely used to declare the dependencies of the application.
By extending `implementation`, both resolvable configurations will resolve the dependencies declared there.
Only `runtimeClasspath` will resolve the dependencies declared on `runtimeOnly`.
Notice that we set `isCanBeConsumed = false` and `isCanBeDeclared = false` in order to denote that `compileClasspath` and `runtimeClasspath` are Resolvable Configurations.

In order to influence the manner in which the resolvable configurations resolve their dependencies, we can declare _attributes_ on the configurations.
Attributes declared on a resolvable configuration define _how_ its dependencies should be resolved.
For example `compileClasspath` and `runtimeClasspath` will have different attributes to ensure that the `lib` dependency resolves to the API and runtime of the target project depending on which resolvable configuration is resolved.

See <<variant_attributes.adoc#sec:declaring_attributes,declaring attributes>> for more information on how configure a resolvable configuration.
To use resolvable configurations to navigate, inspect and post-process metadata and artifacts of assigned dependencies, use the <<dependency_resolution.adoc#sec:programmatic_api,resolution result APIs>>.

.Consumable Configurations represent outgoing artifacts and their dependencies
====
include::sample[dir="snippets/dependencyManagement/attributeMatching/kotlin",files="build.gradle.kts[tags=consumable-configurations]"]
include::sample[dir="snippets/dependencyManagement/attributeMatching/groovy",files="build.gradle[tags=consumable-configurations]"]
====

Above, we declare two consumable configurations: `apiElements` and `runtimeElements`.
These configurations also extend `implementation` and `runtimeOnly` respectively as a source of dependencies.
We also declare `isCanBeResolved = false` and `isCanBeDeclared = false` to mark these as Consumable Configurations.
`apiElements` represents the API of our application, and `runtimeElements` represents the runtime of our application.
Consumers who wish to compile against our project will select `apiElements` and therefore do not need our `runtimeOnly` dependencies.

Consumable configurations are in a way the counterpart to a Resolvable configuration.
Each consumable configuration advertises artifacts and their dependencies, to be consumed by other projects.
When a resolvable configuration wants to resolve a project dependency, it chooses between the consumable configurations of the target project.
The actual method of choosing the proper target consumable configuration is defined by attribute matching.

.Configuration roles
|===
|Configuration role|canBeConsumed|canBeResolved|canBeDeclared
|Consumable|true|false|false
|Resolvable|false|true|false
|Dependency Scope|false|false|true
|===

All three role flags, `canBeConsumed`, `canBeResolved`, and `canBeDeclared`, are enabled by default.
However, they should always be set properly to reflect the intended role of the configuration.

[NOTE]
====
While a Configuration _can_ function with multiple roles enabled, this is behavior discouraged.
In future versions of Gradle, using configurations that have multiple roles enabled will become an error.
====

Since Gradle 8.4, Configurations can be declared with their intended role _upon creation_.
In addition to reducing boilerplate, this API creates _role-locked_ configurations, meaning the role flags cannot be changed after creation.

[NOTE]
====
This is an incubating API and may change in future Gradle releases.
====

.Declaring configurations with factory methods
====
include::sample[dir="snippets/dependencyManagement/definingUsingConfigurations-factory-methods/kotlin",files="build.gradle.kts[tags=declare-configurations]"]
include::sample[dir="snippets/dependencyManagement/definingUsingConfigurations-factory-methods/groovy",files="build.gradle[tags=declare-configurations]"]
====

[[sec:choosing-dependency-scope]]
== Choosing the right dependency scope for dependencies

The choice of the dependency scope where you declare a dependency is important.
The proper dependency scope for a given dependency is largely ecosystem-dependent.
...
In addition to using the configurations provided by plugins, you can define configurations yourself to resolve dependencies which should remain separate from those managed by the plugin.
...
File dependencies allow you to directly add a set of files to a dependency scope configuration, without first adding them to a repository. This can be useful if you cannot, or do not want to, place certain files in a repository. Or if you do not want to use any repositories at all for storing your dependencies.

To add some files as a dependency for a dependency scope configuration, you simply pass a <<working_with_files.adoc#sec:file_collections,file collection>> as a dependency:
