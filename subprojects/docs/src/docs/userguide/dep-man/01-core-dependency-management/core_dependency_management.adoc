...
In Gradle, the <<declaring_dependencies.adoc#what-are-dependency-configurations,scope of a dependency>> is called a _configuration_.
For a full overview, see the reference material on <<declaring_dependencies.adoc#dependency_types,dependency types>>.
...
The reference on <<declaring_repositories.adoc#repository_types,repository types>> provides a broad coverage on this topic.
...
Once resolved, the resolution mechanism <<declaring_repositories.adoc#dependency_cache,stores the underlying files of a dependency in a local cache>>, also referred to as the _dependency cache_.
...
If needed, you can <<controlling_transitive_dependencies.adoc#managing_transitive_dependencies,customize the behavior the handling of transitive dependencies>> to your project's requirements.
...
In dependency management, a lot of the decisions we make depend on the type of project we are building, that is to say, <<being-respectful-consumers,what kind of _consumer_ we are>>.
...
In Gradle, _producer variants_ are exposed via <<declaring_dependencies.adoc,consumable configurations>>.
