...
A container is typically spawned to execute a single build, then is destroyed.
This can become a practical problem when a build depends on a lot of dependencies and that each container has to re-download all dependencies.
...
The dependency cache, both the file and metadata parts, are fully encoded using relative path.
...
When Gradle will look for dependencies (artifacts or metadata), it will look into the classic writable cache, but will also search into the read-only cache.
It means that if an artifact is present in the read-only cache, it will not be downloaded, but if it's missing from the read-only dependency cache, it will be added to the _local_, writable cache of the Gradle instance.
In practice it means that the writable cache will contain the _diff_ between what the build requires and what is available in the read-only cache.

NOTE: The shared read-only dependency cache is an incubating feature.

The first step in using the shared dependency cache is actually to create a copy of an existing cache.
...
You shouldn't use an existing Gradle installation cache as the read-only cache, because this directory may contain locks and be accessed by the seeding build.
