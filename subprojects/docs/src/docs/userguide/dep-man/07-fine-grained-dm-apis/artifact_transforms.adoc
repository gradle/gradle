...
As described in <<declaring_dependencies.adoc#sub:resolvable-consumable-configs,different kinds of configurations>>, there may be different variants for the same dependency.
...
Gradle identifies a variant of a dependency uniquely by its set of <<modeling_features.adoc#sec:abm_configuration_attributes,attributes>>.
The `java-api` variant of a dependency is the variant identified by the `org.gradle.usage` attribute with value `java-api`.

When Gradle resolves a configuration, the <<modeling_features.adoc#sec:abm_configuration_attributes,attributes>> on the resolved configuration determine the _requested attributes_.
