...
| `TestSuiteType` values built from constants defined in link:{javadocPath}/org/gradle/api/attributes/java/TestSuiteType.html[TestSuiteType] or other custom values for user-defined test suite types.
| No default, no compatibility
|===
