...
Gradle supports the concept of _feature variants_: when running a library, it's often the case that some features should only be available when some dependencies are present, or when special artifacts are used.
