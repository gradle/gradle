...
NOTE: By convention, Gradle maps the feature name to a capability whose group and version are the same as the group and version of the main component, respectively, but whose name is the main component name followed by a `-` followed by the kebab-cased feature name. +
...
WARNING: Depending on the metadata file format, publishing feature variants may be lossy: +
