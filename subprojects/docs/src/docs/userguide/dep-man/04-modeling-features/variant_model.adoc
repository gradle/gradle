...
NOTE: Variants and configurations are sometimes used interchangeably in the documentation, DSL or API for historical reasons. +
...
NOTE: The variant _name_ is mostly for debugging purposes and error messages.
...
NOTE: There are two exceptions to this rule that bypass variant aware resolution: +
...
NOTE: Gradle Module Metadata is a metadata format for modules published on Maven, Ivy and other kinds of repositories.
