...
In practice, variants are selected thanks to the use of <<variant_attributes.adoc#abm_declaring_attributes,attributes>>, which provide semantics to the variants and help the engine in achieving a _consistent resolution result_.
...
So there is not notion of compile/runtime scope or compile/runtime variants in Ivy in general.
...
====
include::sample[dir="userguide/dependencyManagement/customizingResolution/ivyMetadataRule/groovy",files="build.gradle[tags=ivy-component-metadata-rule]"]
include::sample[dir="userguide/dependencyManagement/customizingResolution/ivyMetadataRule/kotlin",files="build.gradle.kts[tags=ivy-component-metadata-rule]"]
====
