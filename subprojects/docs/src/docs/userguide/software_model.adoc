...
A counterpart to the model space is the “project space”, which should be familiar to readers. The “project space” is a graph of objects (e.g `project.repositories`, `project.tasks` etc.) having a link:{groovy-dsl-path}/org.gradle.api.Project.html[Project] as its root. A build script is effectively adding and configuring objects of this graph. For the most part, the “project space” is opaque to Gradle. It is an arbitrary graph of objects that Gradle only partially understands.
...
One way to define rules is via a link:{javadoc-path}/org/gradle/model/RuleSource.html[RuleSource] subclass. If an object extends RuleSource and contains any methods annotated by '@Mutate', then each such method defines a rule. For each such method, the first argument is the subject, and zero or more subsequent arguments may follow and are inputs of the rule.
...
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=managed-type-and-plugin]
----

.Output of **`gradle hello`**
----
> gradle hello
include::{samples-path}/modelRules/basicRuleSourcePlugin/basicRuleSourcePlugin-all.out[]
----
...
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=create-rule]
----

This rule declares that there is a model element at path `"person"` (defined by the method name), of type `Person`. This is the form of the link:{javadoc-path}/org/gradle/model/Model.html[Model] type rule for link:{javadoc-path}/org/gradle/model/Managed.html[Managed] types. Here, the person object is the rule subject. The method could potentially have a body, that mutated the person instance. It could also potentially have more parameters, which would be the rule inputs.
...
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=plugin-mutate-rule]
----

This link:{javadoc-path}/org/gradle/model/Mutate.html[Mutate] rule mutates the person object. The first parameter to the method is the subject. Here, a by-type reference is used as no link:{javadoc-path}/org/gradle/model/Path.html[Path] annotation is present on the parameter. It could also potentially have more parameters, that would be the rule inputs.
...
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=task-create-rule]
----

This link:{javadoc-path}/org/gradle/model/Mutate.html[Mutate] rule effectively adds a task, by mutating the tasks collection. The subject here is the `"tasks"` node, which is available as a link:{javadoc-path}/org/gradle/model/ModelMap.html[ModelMap] of link:{groovy-dsl-path}/org.gradle.api.Task.html[Task]. The only input is our person element. As the person is being used as an input here, it will have been realised before executing this rule. That is, the task container effectively _depends on_ the person element. If there are other configuration rules for the person element, potentially specified in a build script or other plugin, they will also be guaranteed to have been executed.

As `Person` is a link:{javadoc-path}/org/gradle/model/Managed.html[Managed] type in this example, any attempt to modify the person parameter in this method would result in an exception being thrown. Managed objects enforce immutability at the appropriate point in their lifecycle.

Rule source plugins can be packaged and distributed in the same manner as other types of plugins (see <<custom_plugins.adoc#custom_plugins,Custom Plugins>>). They also may be applied in the same manner (to project objects) as link:{javadoc-path}/org/gradle/api/Plugin.html[Plugin] implementations (i.e. via link:{groovy-dsl-path}/org.gradle.api.Project.html#org.gradle.api.Project:apply(java.util.Map)[Project.apply(java.util.Map)]).

Please see the documentation for link:{javadoc-path}/org/gradle/model/RuleSource.html[RuleSource] for more information on constraints on how rule sources must be implemented and for more types of rules.
...
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=managed-type]
----
...
| Yes
|
.Example: a String property
[source.multi-language-sample,groovy,indent=0]
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=property-type-string]
----

| `File`
| Yes
|
.Example: a File property
[source.multi-language-sample,groovy,indent=0]
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=property-type-file]
----

| `Integer`, `Boolean`, `Byte`, `Short`, `Float`, `Long`, `Double`
| Yes
|
.Example: a Long property
[source.multi-language-sample,groovy,indent=0]
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=property-type-long]
----
...
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=property-type-boolean]
----

.Example: an int property
[source.multi-language-sample,groovy,indent=0]
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=property-type-int]
----

| Another _managed_ type.
| Only if read/write
|
.Example: a managed property
[source.multi-language-sample,groovy,indent=0]
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=property-type-managed]
----

| An _enumeration_ type.
| Yes
|
.Example: an enumeration type property
[source.multi-language-sample,groovy,indent=0]
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=property-type-enum]
----

| A `ManagedSet`. A managed set supports the creation of new named model elements, but not their removal.
| Only if read/write
|
.Example: a managed set
[source.multi-language-sample,groovy,indent=0]
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=property-type-managedset]
----

| A `Set` or `List` of scalar types. All classic operations on collections are supported: add, remove, clear...
| Only if read/write
|
.Example: a scalar collection
[source.multi-language-sample,groovy,indent=0]
.build.gradle
----
include::{samples-path}/modelRules/basicRuleSourcePlugin/build.gradle[tag=property-type-collection-scalar]
----
...
Managed types can be defined out of interfaces or abstract classes and are usually defined in plugins, which are written either in Java or Groovy. Please see the link:{javadoc-path}/org/gradle/model/Managed.html[Managed] annotation for more information on creating managed model objects.
...
| Managed type
| Any class which is a valid managed model (i.e.annotated with @link:{javadoc-path}/org/gradle/model/Managed.html[Managed])

| Managed collection
| A link:{javadoc-path}/org/gradle/model/ModelMap.html[ModelMap] or link:{javadoc-path}/org/gradle/model/ModelSet.html[ModelSet]
...
* Any managed type
* link:{javadoc-path}/org/gradle/language/base/FunctionalSourceSet.html[FunctionalSourceSet] (when the link:{javadoc-path}/org/gradle/language/base/plugins/LanguageBasePlugin.html[LanguageBasePlugin] plugin has been applied)
* Subtypes of link:{javadoc-path}/org/gradle/language/base/LanguageSourceSet.html[LanguageSourceSet] which have been registered via link:{javadoc-path}/org/gradle/platform/base/ComponentType.html[ComponentType]
...
* A managed type
* A type which is annotated with @link:{javadoc-path}/org/gradle/model/Unmanaged.html[Unmanaged]
* A Scalar Collection
* A Managed collection containing managed types
* A Managed collection containing link:{javadoc-path}/org/gradle/language/base/FunctionalSourceSet.html[FunctionalSourceSet]'s (when the link:{javadoc-path}/org/gradle/language/base/plugins/LanguageBasePlugin.html[LanguageBasePlugin] plugin has been applied)
* Subtypes of link:{javadoc-path}/org/gradle/language/base/LanguageSourceSet.html[LanguageSourceSet] which have been registered via link:{javadoc-path}/org/gradle/platform/base/ComponentType.html[ComponentType]
...
link:{javadoc-path}/org/gradle/language/base/FunctionalSourceSet.html[FunctionalSourceSet]s and subtypes of link:{javadoc-path}/org/gradle/language/base/LanguageSourceSet.html[LanguageSourceSet] (which have been registered via link:{javadoc-path}/org/gradle/platform/base/ComponentType.html[ComponentType]) can be added to the model space via rules or via the model DSL.
...
.build.gradle
----
include::{samples-path}/modelRules/language-support/build.gradle[tag=model-language-support]
----

.Output of`gradle help`
----
> gradle help
include::{samples-path}/modelRules/language-support/model-language-support-all.out[]
----
...
As previously mentioned, a rule has a subject and zero or more inputs. The rule's subject and inputs are declared as “references” and are “bound” to model elements before execution by Gradle. Each rule must effectively forward declare the subject and inputs as references. Precisely how this is done depends on the form of the rule. For example, the rules provided by a link:{javadoc-path}/org/gradle/model/RuleSource.html[RuleSource] declare references as method parameters.

A reference is either “by-path” or “by-type”.

A “by-type” reference identifies a particular model element by its type. For example, a reference to the link:{javadoc-path}/org/gradle/api/tasks/TaskContainer.html[TaskContainer] effectively identifies the `"tasks"` element in the project model space. The model space is not exhaustively searched for candidates for by-type binding; rather, a rule is given a scope (discussed later) that determines the search space for a by-type binding.
...
Rules are bound within a “scope”, which determines how references bind. Most rules are bound at the project scope (i.e. the root of the model graph for the project). However, rules can be scoped to a node within the graph. The link:{javadoc-path}/org/gradle/model/ModelMap.html#named-java.lang.String,%20java.lang.Class-[ModelMap.named(java.lang.String, java.lang.Class)] method is an example of a mechanism for applying scoped rules. Rules declared in the build script using the `model {}` block, or via a `RuleSource` applied as a plugin use the root of the model space as the scope. This can be considered the default scope.
...
In the example below, a `@Defaults` rule is applied to each `FileItem` in the model setting a default file size of "1024". Another rule applies a link:{javadoc-path}/org/gradle/model/RuleSource.html[RuleSource] to every `DirectoryItem` that makes sure all file sizes are positive and divisible by "16".
...
.build.gradle
----
include::{samples-path}/modelRules/ruleSourcePluginEach/build.gradle[]
----
...
.build.gradle
----
include::{samples-path}/modelRules/modelDsl/build.gradle[tag=configure-rule]
----
...
.build.gradle
----
include::{samples-path}/userguide/modelRules/configureAsRequired/build.gradle[tag=configure-rule]
----

.Output of **`gradle showPerson`**
----
> gradle showPerson
include::{samples-path}/userguide/modelRules/configureAsRequired/modelDslConfigureRuleRunWhenRequired.out[]
----
...
include::{samples-path}/userguide/modelRules/configureAsRequired/modelDslConfigureRuleNotRunWhenNotRequired.out[]
----
...
.build.gradle
----
include::{samples-path}/modelRules/modelDsl/build.gradle[tag=create-rule]
----
...
.build.gradle
----
include::{samples-path}/modelRules/modelDsl/build.gradle[tag=create-rule-no-config]
----
...
.build.gradle
----
include::{samples-path}/userguide/modelRules/initializationRuleRunsBeforeConfigurationRules/build.gradle[tag=configure-and-create-rules]
----

.Output of **`gradle showPerson`**
----
> gradle showPerson
include::{samples-path}/userguide/modelRules/initializationRuleRunsBeforeConfigurationRules/modelDslInitializationRuleRunsBeforeConfigurationRule.out[]
----
...
[TIP]
====
You can use the <<software_model.adoc#model-report,model report>> to determine the type of a particular model element.
====
...
A link:{javadoc-path}/org/gradle/model/ModelMap.html[ModelMap] is basically a map of model elements, indexed by some name. When a `ModelMap` is used as the subject of a DSL rule, the rule closure can use any of the methods defined on the link:{javadoc-path}/org/gradle/model/ModelMap.html[ModelMap] interface.
...
.build.gradle
----
include::{samples-path}/modelRules/modelDsl/build.gradle[tag=model-map-nested-create-rule]
----
...
.build.gradle
----
include::{samples-path}/modelRules/modelDsl/build.gradle[tag=model-map-nested-configure-rule]
----
...
.build.gradle
----
include::{samples-path}/userguide/modelRules/configureElementsOfMap/build.gradle[tag=create-and-configure]
----

.Output of **`gradle listPeople`**
----
> gradle listPeople
include::{samples-path}/userguide/modelRules/configureElementsOfMap/modelDslModelMapNestedAll.out[]
----

Any method on link:{javadoc-path}/org/gradle/model/ModelMap.html[ModelMap] that accepts an link:{javadoc-path}/org/gradle/api/Action.html[Action] as its last parameter can also be used to define a nested rule.
...
.build.gradle
----
include::{samples-path}/modelRules/modelDsl/build.gradle[tag=managed-type-nested-config-rule]
----
...
.build.gradle
----
include::{samples-path}/modelRules/modelDslCoercion/build.gradle[]
----
...
.build.gradle
----
include::{samples-path}/modelRules/modelDsl/build.gradle[tag=rule-inputs]
----
NOTE: The code for this example can be found at **`samples/modelRules/modelDsl`** in the ‘-all’ distribution of Gradle.

In the above snippet, the `$.person` construct is an input reference. The construct returns the value of the model element at the specified path, as its default type (i.e. the type advertised by the <<software_model.adoc#model-report,Model Report>>). It may appear anywhere in the rule that an expression may normally appear. It is not limited to the right hand side of variable assignments.
...
----
> gradle model
include::{samples-path}/modelRules/basicRuleSourcePlugin/basicRuleSourcePlugin-model-task.out[]
----
