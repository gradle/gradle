...
== 4. Remote Caching
The remote cache is a shared build cache.
Reading from and writing to the remote cache is done via HTTP.

One of the most common use cases of remote cache is continuous integration builds. On every clean build, the CI server populates the remote cache. So, the components that are changed will not be rebuilt, thereby speeding up the CI builds. Moreover, task outputs can also be shared among CI agents. The remote cache is not enabled by default.
