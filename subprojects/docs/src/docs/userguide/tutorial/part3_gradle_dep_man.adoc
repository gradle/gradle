...
The output clearly depicts that `com.google.guava:guava:31.1-jre` has a dependency on `com.google.guava:failureaccess:1.0.1`.

== Step 3. Viewing Dependencies in a Build Scan®
...
== Step 4. Adding a Version Catalog
Declare all direct dependencies used in central location
Just declare modules, not dependency configurations
Reference catalog in subproject build files
Can also specify plugin versions

Declare all direct dependencies used in central location
gradle/libs.versions.toml
Only declare modules, not dependency configurations
Reference catalog in subproject build files
