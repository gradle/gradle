...
= Part 6: Enabling the Gradle Cache
...
Gradle is efficient, especially with the local cache turned on.
...
In addition to the local cache present on a developer's machine, Gradle can make use of remote caches which can be used by multiple developers.
