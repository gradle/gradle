...
Add `org.gradle.console=verbose` so the contents of the file look like:
[source]
----
...
Gradle lets us know that inputs and outputs have not changed using the _outcome label_ `UP-TO-DATE`.
...
|NO-SOURCE
|Task is not executed because the input data required for its execution was not found
|===
...
[.text-right]
**Next Step:** <<part6_gradle_caching#part6_begin,Enabling the Gradle Cache>> >>
