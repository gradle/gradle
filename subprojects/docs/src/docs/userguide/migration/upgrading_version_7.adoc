...
within named container configure methods that throw `MissingMethodException`.
...
==== Modifying GMM after a publication has been populated

Altering the link:publishing_gradle_module_metadata.html[Gradle Module Metadata] (e.g. changing a component configuration variants) after a Maven or Ivy publication have been populated from their components, is now deprecated and scheduled to be disallowed in Gradle 9.0.

Eager "realization" of the publication can happen if the following methods are called:
...
Before, it was possible to have this code block without any warnings, but it created inconsistencies between the published artifacts:
...
(components["java"] as DefaultAdhocSoftwareComponent).apply {
...
In this example, the Maven and Ivy publications will contain the main JAR artifacts for the project, whereas the GMM link:https://github.com/gradle/gradle/blob/master/subprojects/docs/src/docs/design/gradle-module-metadata-latest-specification.md[module file] will have them skipped.
