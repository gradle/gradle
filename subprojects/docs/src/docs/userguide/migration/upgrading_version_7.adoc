...
`ConfigureUtil` is used in plugins for applying configuration defined in a Groovy closure to DSL objects. This implementation should be avoided it will be bound to the Groovy. Instead, the object
configuration should be  modeled with link:{javadocPath}/org/gradle/api/Action.html[Actions], which is language-agnostic.
...
If the DSL object is created via link:{javadocPath}/org/gradle/api/model/ObjectFactory.html[ObjectFactory] then Gradle will generate a closure-taking method at runtime for each method with an `Action`
