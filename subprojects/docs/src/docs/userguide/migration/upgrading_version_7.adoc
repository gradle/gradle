...
==== Changes to configurations created by the War Plugin

The link:war_plugin.html[War Plugin] creates configurations named `providedCompile` and `providedRuntime`.  These were previously used for both declaring dependencies and resolving them into classpaths.

These configurations are no longer resolvable.  They are extended by the new `providedCompileClasspath` and `providedRuntimeClasspath` configurations, ,which are resolvable.  The plugin will now use these new configurations internally to resolve dependencies.
