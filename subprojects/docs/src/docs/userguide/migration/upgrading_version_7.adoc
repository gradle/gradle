...
The `BasePlugin` convention defines `distsDirName` and `libsDirName` that are not present on `BasePluginExtension` as they are redundant and can be set on the corresponding `distsDirectory` and `libsDirectory` properties:
```
base {
    archivesBaseName = 'customBase'
