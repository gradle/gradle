...
Instead, an Eclipse Temurin or IBM build is returned.
...
If an Eclipse Temurin or IBM build works for you, specify `JvmVendorSpec.ADOPTIUM` or `JvmVendorSpec.IBM` as the vendor or leave the vendor unspecified.
