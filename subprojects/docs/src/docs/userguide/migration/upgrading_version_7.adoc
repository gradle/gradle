...
Instead, obtain a `WorkQueue` via the `noIsolation()`. `classLoaderIsolation()`, `processIsolation()`, methods and use the `submit(Class, Action)` method on the `WorkQueue` instead.
...
You can extend configurations using the `extendsFrom(Configuration)` method on `Configuration` instead to replicate many of the results of this behavior.

==== AntlrPlugin now creates a non-consumable configuration

The `antlr` configuration created by the `AntlrPlugin` was never meant to be consumed.  It was deprecated for consumption and is now no longer consumable.
Attempting to consume it will now result in an error.
