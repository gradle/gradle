...
The convention properties contributed by the `ear` plugin have been deprecated and scheduled for removal in Gradle 8.0.
They are replaced by the the properties of link:{groovyDslPath}/org.gradle.plugins.ear.EarPluginExtension.html[EarPluginExtension] which can we configured in the `earPlugin {}` block.
