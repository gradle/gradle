...
==== Jar, War, and Ear destination directory defaults are set by JavaBasePlugin

Previously, the `base` plugin would configure the
...
In Gradle 8.0, this behavior now requires the `java-base` plugin to be applied. For projects which already apply the
