...
=== Ambiguous method call in Kotlin DSL when adding dependency management attributes

Gradle 7.4 introduces an overloaded version of the link:{javadocPath}/org/gradle/api/attributes/AttributeContainer.html#attribute-org.gradle.api.attributes.Attribute-org.gradle.api.provider.Provider-[`attribute(Attribute<T>, Provider<? extends T>)`] method to allow users to add an attribute with a value based on a link:{javadocPath}/org/gradle/api/provider/Provider.html[`Provider`].

This new method may break builds that use Kotlin DSL. Kotlin is unable to select between the two overload methods in some cases and produces an error like this:
```
Overload resolution ambiguity:
  public abstract fun <T : Any!> attribute(p0: Attribute<TypeVariable(T)!>, p1: TypeVariable(T)): AttributeContainer defined in org.gradle.api.attributes.AttributeContainer
  public abstract fun <T : Any!> attribute(p0: Attribute<TypeVariable(T)!>, p1: Provider<out TypeVariable(T)!>): AttributeContainer defined in org.gradle.api.attributes.AttributeContainer
```

This only occurs in Kotlin DSL due to an extension function. The function `ObjectFactory.named(name: String)` acts as a shortcut for defining attribute values. Build scripts or plugins may have used it like this:
```
attributes {
    attribute(Usage.USAGE_ATTRIBUTE, objects.named(Usage.JAVA_RUNTIME))
}
```
This adds an attribute of type `Usage` with the named value `Usage.JAVA_RUNTIME`. The type of the attribute is not specified.

In Gradle 7.4, the type of the attribute needs to be explicitly provided:
```
attributes {
...
Code samples throughout the Gradle documentation have been updated to reflect this change.
