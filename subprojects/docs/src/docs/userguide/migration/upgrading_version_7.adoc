...
==== Removed support for TAR trees from resources without backing files

Creating a TAR tree from a resource with no backing file is not supported anymore.
Instead, convert the resource to a file and use `project.tarTree()` on the file.
See <<tar_tree_no_backing_file>> for more information.
