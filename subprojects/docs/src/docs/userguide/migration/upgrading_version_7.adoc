...
With link:toolchains.html[Java toolchains] feature users are able to select specific toolchains per project or per task.
Along with the Java language version, the toolchain DSL allows configuring other criteria such as specific vendors or VM implementations.
...
In prior versions of Gradle, if the language version was not set then Gradle would ignore values provided for other properties of the specification.
...
In case you encounter these problems, make sure to link:toolchains.html#sec:consuming[specify the language version] as described in the guide.
