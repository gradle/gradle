...
In order to make the ignore directories behavior consistent, Gradle will now ignore directories for both the `@SkipWhenEmpty` check and for determining changes only when the `@IgnoreEmptyDirectories` annotation is present.
...
Moreover, Gradle will issue a deprecation warning to advice the user that the behavior will change in Gradle 8.0 and that the input property should be annotated with `@IgnoreEmptyDirectories`.

Finally, `@InputDirectory` implies `@IgnoreEmptyDirectories`, so no changes are necessary when using this annotation.
