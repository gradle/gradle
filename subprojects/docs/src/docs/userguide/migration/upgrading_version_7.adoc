...
When a task input is annotated with `@Input` and is not a type Gradle understand directly (like `String`), then Gradle compares uses the serialized form of the input for up-to-date checks and the build cache key.
