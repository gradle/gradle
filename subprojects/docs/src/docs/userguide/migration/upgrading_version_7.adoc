...
Prior to Gradle 8.0, JDK modules `java.base/java.util` and `java.base/java.lang` were automatically opened in Gradle workers on JDK9+ by passing `--add-opens` CLI arguments. This meant any code executed in a Gradle worker was able to perform deep reflection on JDK internals without warning or failing. These implicit arguments have been removed from all workers and are no longer added by default.

Gradle uses workers internally for several tasks, including the code-quality plugins (Checkstyle, CodeNarc, Pmd), ScalaDoc, AntlrTask, and the JVM compiler daemons. Further, tasks executed using process isolation via the link:worker_api.html[Worker API] are also affected. If the tool in question, any custom extensions to these tools, or plugins which utilize the worker API directly attempt to perform deep reflection into JDK internals during task execution, new warning or errors may appear.

In most cases, these errors can be resolved by updating the code or dependency performing the illegal access. This may include updating code-quality tool versions, updating annotation processor versions, or updating any Gradle plugins which use the worker API. For some examples on possible error or warning outputs which may arise due to this change, see <<remove_test_add_opens>>.
...
[[exported_key_deduplication]]
=== Exported keys are now deduplicated

link:dependency_verification.html#sec:local-keyring[Dependency verification's] way of key serialization was changed in 7.6: keys now are sorted and deduplicated.
Forward compatibility was retained, but the following exports might cause a large VCS diff between the old, and the new keyfile.
