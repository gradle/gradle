...
When configuring the built-in test task for Java/Groovy/Scala projects, Gradle no longer allows you to
change the test framework used by the `Test` task after configuring options. This was deprecated and silently
...
```
You should either set the test framework before configuring options or look into migrating to the <<jvm_test_suite_plugin#jvm_test_suite_plugin,JVM Test Suite Plugin>>.
```
test {
