...
==== Legacy convention configuration for `war` API

The deprecated `war` convention (of type `WarPluginConvention`) defined by the `war` plugin has been removed.
In Gradle 8.0 and above you need to configure the `war` task directly.
...
Groovy has been updated to https://groovy-lang.org/changelogs/changelog-3.0.12.html[Groovy 3.0.12].
