...
=== Overloaded Attribute methods to support Providers as values

There is now an overloaded version of the `link:{javadocPath}/org/gradle/api/attributes/AttributeContainer.html#attribute-org.gradle.api.attributes.Attribute-org.gradle.api.provider.Provider-[attribute(Attribute<T>, Provider<? extends T>)]` method available that allows you to specify a `link:{javadocPath}/org/gradle/api/provider/Provider.html[Provider]` which will supply the value of an attribute.

This existence of this method will break builds that use the kotlin extension function `inline fun <reified T : Named> ObjectFactory.named(name: String): T =
named(T::class.java, name)` as a shortcut when defining attributes.  Code in .kts files that defined attributes like this:
...
will now have to provide an explicit type argument so that the compiler can disambiguate the method call, like this:
