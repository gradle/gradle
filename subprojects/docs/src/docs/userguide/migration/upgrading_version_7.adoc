...
[[project_reporting_convention_deprecation]]
=== Deprecated `project-report` plugin conventions

The convention properties contributed by the `project-report` plugin have been deprecated and scheduled for removal in Gradle 8.0.
They are replaced by the the properties of link:{groovyDslPath}/org.gradle.api.plugins.ProjectReportsPluginExtension.html[ProjectReportsPluginExtension] which can we configured in the `projectReports {}` block.
