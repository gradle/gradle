...
==== Setting custom build layout via command line
...
==== Setting custom build file in GradleBuild task
...
==== Setting custom build layout via StartParameter API
...
Consider structuring the build so that it can be executed without specifying custom build or settings files.
