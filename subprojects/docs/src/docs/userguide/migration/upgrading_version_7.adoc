...
The `IncrementalTaskInputs` type was used to implement _incremental tasks_, that is to say tasks which can be optimized to run on a subset of changed inputs instead of the whole input.
This type had a number of drawbacks, in particular the fact of not being able to determine to what input a change was associated with.
