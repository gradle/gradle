...
When executed with `--warning-mode all` the individual warnings will be shown like this:
...
If you own the code of the tasks in question, you can fix them by following the instructions.
...
See <<more_about_tasks.adoc#sec:task_input_validation, the userguide>> for more information on addressing these warnings.
...
https://blog.gradle.org/gradle-metadata-1.0[Gradle Module Metadata], officially introduced in Gradle 5.3, was created to solve many of the problems that have plagued dependency management for years, in particular, but not exclusively, in the Java ecosystem.
...
==== Plugins and classes loaded in settings scripts are visible to project scripts and build src
...
They are also visible to the buildSrc build's script, including its settings script.

This change has been made so that settings plugins can contribute logic to the entire build.
...
This behavior led to broken Gradle Module Metadata and was difficult to understand due to different handling of different packaging types.
...
==== Removed Deprecated Behaviors and APIs
