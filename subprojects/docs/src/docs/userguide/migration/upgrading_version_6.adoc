...
==== Removal of legacy Worker API

The portions of the Worker API, introduced in 3.5, that were deprecated and replaced by the new _work queue_ based APIs in 5.6, have now been removed.
If you are using the Worker API and have not yet migrated: You now have to use the `noIsolation()`, `classLoaderIsolation()` and `processIsolation()` methods to submit work.
