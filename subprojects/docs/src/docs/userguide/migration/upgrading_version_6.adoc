...
Gradle 6.3 uses `native-platform` 0.22, which drops support for 32-bit Unix systems and for old FreeBSD versions (older than FreeBSD 10).
While Gradle will still continue building your projects on that systems, you may have a worse user experience especially when it comes to console output.
