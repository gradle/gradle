...
==== Updates to Groovy and Groovy DSL

Due to the nature of the changes in the new Groovy version (new parser), you may experience minor issues when upgrading to Gradle 7.0.
The new Groovy parser is stricter in some cases than older Groovy versions. In case you see any syntax errors, ensure to check with the https://groovy.apache.org/#reporting-issues[Groovy issue tracker].
Depending on the size if your project and the number of Groovy build scripts you have, you might see slight performance regressions for cold daemons or full rebuilds.
The Groovy team is already working on some improvements to the parser to mitigate these.

===== Groovy 3 modularization

Groovy used to be packaged as a `groovy-all` artifact including all groovy modules. With the upgrade to Groovy 3, Gradle has changed to only pull in the neccessary modules.
While `gradleApi` and `localGroovy` include a set of most commonly used Groovy modules as listed below
...
The following Groovy modules are *not* included in the Gradle distribution and may need to be added manually to the buildscript classpath if neccessary.
...
With the upgrade to Groovy 3, plugin build for Gradle using `gradleApi` or `localGroovy` as dependency will now have Groovy 3 on their classpath. This mostly affects tests written in Spock as Spock 1.x is not compatible with Groovy 3 anymore. Instead, Spock tests need to be upgraded to use Spock 2.x.

Ensure you're using the Groovy 3 variant of Spock 2. Note that Spock 2 is based on JUnit Platform which needs to be enabled as well. At the time of writing, Spock 2.0 Milestone 4 was the most recent version.
...
While generally, Groovy 3 is on par with Groovy 2, certain usecases (especially cold daemons) may see slight performance regressions. Some specific regressions are already fixed in an upcoming Groovy version which Gradle will upgrade to once released.
...
.Common configuration upgradxes
|===
