...
Alternatives were introduced in Gradle 3.4:

- the `implementation` configuration should be used to declare dependencies which are _implementation details_ of a library: they will not leak to consumers when themselves are built
- the `api` configuration, available only if you apply the `java-library` plugin, should be used to declare dependencies which are part of the API of a component, that is to say exposed to consumers

In Gradle 7, both the `compile` and `runtime` configurations were removed.
