...
==== Using files as inputs of artifact transforms

It's no longer valid to use a `File` as an input of an artifact transform.
Instead, you should use a `Provider<FileSystemLocation>`.
Look at the <<artifact_transforms.adoc#sec:implementing-artifact-transforms,artifact transforms>> documentation for more information.
