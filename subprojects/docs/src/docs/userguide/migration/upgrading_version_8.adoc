...
==== `link:{javadocPath}/org/gradle/api/artifacts/component/BuildIdentifier.html[BuildIdentifier]` method deprecations
...
These methods allowed to externally recreate identifiers for builds.
However, for certain composite build setups, they did not provide enough information to guarantee uniqueness.

Current usages of these methods should migrate to `link:{javadocPath}/org/gradle/api/artifacts/component/BuildIdentifier.html#getBuildPath--[String getBuildPath()]`.
