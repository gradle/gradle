...
This `ArtifactView` selects a variant without the benefit of the `org.gradle.libraryeleemnt=classes` attribute, which was added the `compileClasspath` configuration lazily, after the `ArtifactView` was created.
