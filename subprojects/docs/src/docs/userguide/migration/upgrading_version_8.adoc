...
When performing dependency resolution, Gradle creates an internal representation of the available link:{javadocPath}/org/gradle/api/artifacts/Configuration.html[Configuration]s. This required inspecting all configurations and all artifacts. Processing artifacts created by tasks would cause those tasks to be realized and configured.

This internal representation is now created more lazily, which can cause changes to the order that tasks are configured. Some tasks may never be configured. 
