...
In order to prepare to for this change in behavior, either declare the required dependencies explicitly,
or migrate to link:jvm_test_suite_plugin.html[Test Suites], where these dependencies are managed automatically.

===== Test Suites

If you are using test suites, this change will not affect you. The test framework dependencies are managed
by the suite itself by automatically declaring the proper dependencies on the test suite's configurations.
In order to minimally migrate to test suites, use the following code:
...
This change in behavior can also be mitigated without migrating to test suites. In this case, the proper
dependencies must be manually declared on the test runtime classpath:
