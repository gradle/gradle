...
Gradle now emits a `CACHEDIR.TAG` file in some global cache directories (see the release notes for details).

This may cause these directories to no longer be searched or backed up by some tools. In order to disable
this behavior, configure the link:{groovyDslPath}/org.gradle.api.initialization.Settings.html[settings] object
in an init script in the Gradle user home:
