...
See the release notes for link:https://github.com/JetBrains/kotlin/releases/tag/v1.8.21[Kotlin 1.8.21].
...
functionality has since been replaced by <<component_metadata_rules.adoc#sec:component_metadata_rules,Component Metadata Rules>>
...
While the updated solution may be more verbose, we decided that
reducing the number of ways to achieve the same result is worth
the trade-off. In any case, component metadata rules like this
should live within build-logic and thus should not clutter up
the buildscript.
