...
* The Eclipse Plugin default output folder is now `bin` instead of `bin/default`.
Previously the default output folder didn't configure the output folder for the different source sets, now it does.
