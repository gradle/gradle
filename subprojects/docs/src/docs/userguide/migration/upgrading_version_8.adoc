...
the default behavior is to use the tests from the default `test` link:jvm_test_suite_plugin.html[`TestSuite`].
...
test suite under a different environment, it is not fit for the use case of executing
a separate set of tests entirely.

If the current default behavior is desired, use the following code to avoid the deprecation warning in 8.1
