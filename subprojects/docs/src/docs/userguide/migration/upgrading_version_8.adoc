...
This meant the attributes used for resolution by those types could unexpectedly differ when the attributes on the `Configuration` changed after an `ArtifactView` or `ArtifactCollection` was created.
Some default attributes are lazily and automatically added to `Configuration` instances by core Java plugins, so could lead to unexpected behavior.
...
That attribute allows for a performance optimization where the secondary `classes` variant is selected instead of the `producer` project's primary `jar` variant.
As it never added to the `ArtifactView`'s attributes, the `ArtifactView` selects the primary `jar` variant instead.
