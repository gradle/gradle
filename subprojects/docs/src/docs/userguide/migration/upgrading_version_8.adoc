...
==== Lazier construction of `RootComponentMetadata`

During resolution, when Gradle constructs root `ComponentMetadata` objects it now does so more lazily, without causing tasks that create artifacts to be realized.
This can change the timing of when certain attributes are added to a `Configuration`, such as the `LibraryElements` attribute added by internal methods of the `java` plugin.

If your build relies on modifying this or other attributes added by a Gradle core plugin, you should double-check that your modifications are still applied at the right time for your use cases.

We recommend not modifying attributes on Gradle-created configurations to avoid being surprised by implementation changes in internal behavior.
