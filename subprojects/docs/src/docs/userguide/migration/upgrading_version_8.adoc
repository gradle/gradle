...
Prior to Gradle 8.3, `Configuration.getIncoming().artifactView()` and `Configuration.getIncoming().getArtifacts()` captured immutable copies of the `Configuration`'s attributes upon invocation. Consequently, if the `Configuration`'s attributes were changed after these methods were called, the attributes used for resolution by these methods could unexpectedly differ from the final `Configuration` attributes.  

Given that some attributes are lazily added to `Configuration`s during resolution, the behavior of these methods could have differed based on the order that these methods were called. 

Consider the following scenario where `producer` is a sibling `java-library` project to the build below:
...
```groovy
    ...
    logger.warn 'Incoming Files:'
    incomingFiles.each { logger.warn 'Name: ' + it.name }

    logger.warn 'Artifact View Files:'
    artifactViewFiles.each { logger.warn 'Name: ' + it.name }

    assert incomingFiles*.name == artifactViewFiles*.name // FAILS!
```

Running `gradle help` shows the following logger output that demonstrates the problem:
...
This `ArtifactView` selects a variant without the benefit of the `org.gradle.libraryelements=classes` attribute, which was added the `compileClasspath` configuration lazily, after the `ArtifactView` was created.
Since the attribute is never added to the `ArtifactView`'s attributes, the `ArtifactView` selects the primary `jar` variant instead of the `classes` variant.

The above scenario demonstrates the previous attribute snapshotting behavior, where the assertion would fail.
With the new behavior, the attribute collection used by `ArtifactView` is "live" and sees the modifications to the configuration's attributes; because of this the assertion now passes.
...
A system setting `link:{javadocPath}/org/gradle/api/internal/artifacts/configuration/DefaultConfiguration.html#USE_LEGACY_ATTRIBUTE_SNAPSHOT_BEHAVIOR--[org.gradle.configuration.use-legacy-attribute-snapshot-behavior]` has been provided to temporarily restore the legacy behavior for those who need time to migrate.
This setting should be considered immediately deprecated and will be removed in Gradle 9.0.
