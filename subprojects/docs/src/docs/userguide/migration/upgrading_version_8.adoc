...
==== `link:{javadocPath}/org/gradle/api/artifacts/ResolvedConfiguration.html[ResolvedConfiguration]`
The `ResolvedConfiguration` API is deprecated.
This type was primarily used internally to represent the result of resolving a configuration.
Its functionality can be replaced by various methods available on the result of calling `getIncoming()`.
