...
If the `Configuration`'s attributes were changed after these methods were called, the artifacts selected by these methods could be unexpected or fail to be resolved.

Consider the following scenario where the root project depends on a nested `producer` project:
...
tasks.register("verifyFiles") {
    ...
    doLast {
        ...
        // val artifactViewFiles = consumerConf.artifactView { withVariantReselection() }.files
        ...
        incomingFiles.forEach { println("Name: " + it.name) }

        println("Artifact View Files:")
        artifactViewFiles.forEach { println("Name: " + it.name) }

        check(incomingFiles.map { it.name } == artifactViewFiles.map { it.name }) // FAILS!
    }
}
...
Running `gradle verifyFiles` shows a build failure message that demonstrates the problem:

```
> Could not resolve all files for configuration ':consumerConf'.
   > No variants of project :producer match the consumer attributes:
       - Configuration ':producer:producerConfVanilla':
           - Incompatible because this component declares attribute 'flavor' with value 'vanilla' and the consumer needed attribute 'flavor' with value 'chocolate'

```

This `ArtifactView` tries to use the original `flavor=chocolate` value set for the attribute, which was later changed to the value `vanilla`, after the `ArtifactView` was created.
Since the attribute was never updated in the `ArtifactView`'s schema, the `ArtifactView` fails to reselect a matching variant from the candidates selected by the configuration (which used `flavor=vanilla`).

If you replace the `artifactViewFiles` declaration line with the commented-out line below, reselection allows for a variant to be found - but it is the **wrong** variant.
Since the attributes used for selection are different from the configuration's attributes at selection time, the check now fails.

The above scenario demonstrates the previous attribute snapshotting behavior.
With the new behavior, the attribute collection used by `ArtifactView` is "live" and sees the modifications to the configuration's attributes; because of this the build succeeds and the check passes.
...
Obtaining artifacts instead of files from an `ArtifactView` had a similar problem, which is also now fixed.

This behavior is relevant to `project()` dependencies when using the `java-library`.
That plugin automatically adds certain attributes to configurations late in the build, which resulted in dependant projects selecting different (but compatible) secondary variants when using `ArtifactView`.
