...
NOTE:  Support for building Play applications is currently <<feature_lifecycle,incubating>>. Please be aware that the DSL, APIs and other configuration may change in later Gradle versions. 
...
To enable continuous build, run Gradle with [userinput]#-t runPlayBinary# or [userinput]#--continuous runPlayBinary#.
