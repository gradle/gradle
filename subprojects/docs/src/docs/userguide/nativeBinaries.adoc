...
If a default tool chain is not defined or configured in the model, a default tool chain of that type is created with reference to the table above. When this default tool chain is created, or if a tool chain sharing the same default name is configured in the model, the default platforms are added to this toolchain. For tool chains that do not use the default name, it will not respond to any platforms by default.
...
Cross-compiling is possible with the api:org.gradle.nativeplatform.toolchain.Gcc[] and api:org.gradle.nativeplatform.toolchain.Clang[] tool chains, by adding support for target platforms. This is done by specifying a target platform for a toolchain. For each target platform a custom configuration can be specified.
...
Note that, since the toolchain in the above example is named the default `gcc`, it will also respond to `x86` and `x86_64` platforms.
