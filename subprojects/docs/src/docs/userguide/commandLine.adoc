...
`--configure-on-demand (deprecated)`::
Only relevant projects are configured in this build run. This option has been deprecated and is scheduled to be removed Gradle 5.0. See <<sec:configuration_on_demand>>.
`--no-configure-on-demand (deprecated)`::
