...
You can learn about what projects and tasks are available in the <<command_line_interface.adoc#sec:command_line_project_reporting, project reporting section>>.
...
`-S`, `--full-stacktrace`::
Print out the full (very verbose) stacktrace for any exceptions. See also <<command_line_interface.adoc#sec:command_line_logging, logging options>>.

`-s`, `--stacktrace`::
Print out the stacktrace also for user exceptions (e.g. compile error).  See also <<command_line_interface.adoc#sec:command_line_logging, logging options>>.
