...
If that file doesn’t exist, just create it using a text editor. You can find details of other ways to disable (and enable) the Daemon in <<gradle_daemon.adoc#daemon_faq,Daemon FAQ>> further down. That section also contains more detailed information on how the Daemon works.
