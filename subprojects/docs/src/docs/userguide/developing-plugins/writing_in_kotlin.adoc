...
`gradleApi()` is the primary way of interacting with the https://docs.gradle.org/current/javadoc/index.html[Gradle core].

It is provided by the version of Gradle you're using to build your plugin. It is built the first time you run a build and contains all the symbols in your current version of Gradle. You will usually find it under `~/.gradle/caches/$version/generated-gradle-jars/gradle-api-$version.jar`.

Because `gradleApi()` uses the same version as the one you're currently running, it's easy to use symbols that are not present in previous versions of Gradle. If your plugin is run by an older version that is missing such a symbol, your plugin will throw a `NoSuchMethodError` at runtime.

Note: Because it is provided by Gradle at runtime, `gradleApi()` should be a `compileOnly` dependency.
