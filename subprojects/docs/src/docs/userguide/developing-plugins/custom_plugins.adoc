...
[WARNING]
====
There are two important limitations for declaring precompiled script plugins: they cannot be declared in the `org.gradle` package, and they cannot have a name that is equal to a built-in plugin's ID. This ensures that the precompiled script plugins won't be silently ignored.
====
