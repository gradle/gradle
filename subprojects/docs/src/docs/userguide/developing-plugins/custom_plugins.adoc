...
[WARNING]
====
There are two important limitations for declaring precompiled script plugins: they cannot be declared in the `org.gradle` package, and they cannot have the name that is equal to a built-in plugin's ID. This ensures that the script plugin applications won't be silently ignored.
====
...
----
include::{snippetsPath}/plugins/customPlugin/groovy/plugin/src/test/java/org/example/GreetingPluginTest.java[tag=test-plugin]
----
