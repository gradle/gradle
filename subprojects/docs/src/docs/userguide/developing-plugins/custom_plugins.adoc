...
generate the required plugin descriptors in the resulting JAR file and configure the <<writing_plugins.adoc#sec:plugin_markers,Plugin Marker Artifact>> to be used when publishing.
...
If your plugin was published without using the <<java_gradle_plugin.adoc#java_gradle_plugin,Java Gradle Plugin Development Plugin>>, the publication will be lacking <<writing_plugins.adoc#sec:plugin_markers,Plugin Marker Artifact>>, which is needed for <<writing_plugins.adoc#sec:plugins_block,plugins DSL>> to locate the plugin.
