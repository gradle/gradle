...
[WARNING]
====
Precompiled script plugin names have two important limitations: they cannot start with `org.gradle` and they cannot be the same as a built-in plugin ID. This ensures that the precompiled script plugins won't be silently ignored.
====
