...
Gradle supports writing plugins in Groovy, Java, and Kotlin. 
Java is the most universal language because it doesn't come with an additional runtime. 
For this reason, this page focuses on Java. 
If you're writing your plugins in Kotlin, check out the dedicated <<kotlin_gradle_plugins.adoc, dedicated Kotlin Gradle plugin page>>.
