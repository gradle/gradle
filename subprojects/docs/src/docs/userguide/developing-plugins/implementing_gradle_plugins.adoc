...
Gradle supports writing plugins in Groovy, Java and Kotlin. Java is the most universal language because it doesn't come with an additional runtime. Because of this reason, this page focuses on Java. If you're writing your plugins in Kotlin make sure to check the <<kotlin_gradle_plugins.adoc, dedicated>> page for Kotlin-specific considerations.
