...
To avoid evaluation order problems you should use link:{user-manual}lazy_configuration.html#lazy_properties[the public API `Property`] which was introduced in Gradle 4.0.
...
NOTE: The support for multi-variant plugins currently requires you to use the raw <<variant_model.adoc#,variant aware dependency management>> APIs of Gradle.
More conveniences around this may be provided in the future.

Currently, the most convenient way to configure additional plugin variants is to use <<feature_variants.adoc#,feature variants>>, a concept available in all Gradle projects that apply one of the Java plugins.
As described in the <<feature_variants.adoc#,documentation>>, there are several options to design feature variants.
...
<1> Assign the <<component_capabilities.adoc#declaring_additional_capabilities_for_a_local_component,implicit capability that corresponds to the components GAV>> to the variant.
