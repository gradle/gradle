...
<3> Set the human-readable description for all plugins unless overridden in the `gradlePlugins` block.
<4> Set the tags to be used for all plugins unless overridden in the `gradlePlugins` block.
...
<7> Set a description that will be displayed on the portal. It provides useful information to people who might want to use your plugin.
    Overrides any description provided for all plugins at the plugin bundle level.
<8> Specifies the categories your plugin covers. Makes the plugin more likely to be discovered by people needing its functionality.
    Overrides any tags provided for all plugins at the plugin bundle level.
