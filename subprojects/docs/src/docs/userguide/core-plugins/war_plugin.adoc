...
The War Plugin adds the link:{groovyDslPath}/org.gradle.api.plugins.WarPluginExtension.html[`war` extension] to the project.
This allows to configure the following properties inside a dedicated DSL block.

`webAppDir` — `DirectoryProperty`::
_Default value_: `$projectDir/src/main/webapp`

.Setting web app directory in `warPlugin` extension
====
include::sample[dir="snippets/webApplication/extension/groovy",files="build.gradle[tags=war-extension]"]
include::sample[dir="snippets/webApplication/extension/kotlin",files="build.gradle.kts[tags=war-extension]"]
====
...
The conventions contributed by the War plugin have been deprecated and superseded by the extension described above.
See the link:{groovyDslPath}/org.gradle.api.plugins.WarPluginConvention.html[WarPluginConvention] DSL documentation for information on them.
