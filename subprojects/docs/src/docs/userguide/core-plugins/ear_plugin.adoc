...
The Ear plugin adds the link:{groovyDslPath}/org.gradle.plugins.ear.EarPluginExtension.html[`earPlugin` extension] to the project.
This allows to configure a number of properties inside a dedicated `earPlugin{ }` DSL block.
...
The Ear plugin adds a number of convention properties to the project. You can use these properties in your build script as though they were properties of the project object.
These are *deprecated* and superseded by the extension described above.
See the link:{groovyDslPath}/org.gradle.plugins.ear.EarPluginConvention.html[EarPluginConvention] DSL documentation for information on them.
