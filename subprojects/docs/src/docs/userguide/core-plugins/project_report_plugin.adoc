...
These convention properties are provided by a convention object of type link:{groovyDslPath}/org.gradle.api.plugins.ProjectReportsPluginConvention.html[ProjectReportsPluginConvention].
 ...
 Configuring the project report tasks via convention properties is **deprecated**. If you need to set default values the `tasks then configure them directly. If you want to configure all report tasks of the same type in the project then use link:{javadocPath}/org/gradle/api/DomainObjectCollection.html#withType-java.lang.Class-[tasks.withType(War.class).configureEach(...)].
