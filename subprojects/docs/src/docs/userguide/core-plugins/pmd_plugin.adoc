...
It means that you have to pay attention to the value entered here and make sure it still makes sense in a multi project.
Because parallel Gradle execution could result in different PMD tasks from different project running in parallel, having a multiplier effect on the configured value.
====
