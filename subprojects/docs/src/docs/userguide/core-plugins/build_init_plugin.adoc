...
Gradle enters interactive mode and prompts the user when any parameter is missing.
...
You can see available options from the task help:
...
All build types also setup the link:{groovyDslPath}/org.gradle.api.tasks.wrapper.Wrapper.html[Gradle wrapper].
...
When creating a java project you must set the java version. You can do that by issuing *the major version* of java you have available to use:
