...
[WARNING]
====
It should be noted that adding possible values to existing attributes may break disambiguation rules.
It's especially true for the `org.gradle.usage` attribute, which is defined by Gradle and assumes a known set of possible values.
If you add a possible value, you may have to reimplement the disambiguation logic in your own rule.
====
