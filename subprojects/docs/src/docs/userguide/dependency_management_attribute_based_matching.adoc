...
Another example is that you want minify JARs by only including some `class` files from them.
You need to provide the classes to retain in the minified JARs per JAR which should be minified.
Note the use of the `parameters {}` block to provide the classes to retain in the minified JARs to the `Minify` transform.
...
For example, if the configuration consists of the artifacts `lib1.jar`, `lib2.jar`, `lib3.jar`, and the transform action registers a minified output artifact `<artifact-name>-min.jar` for each input artifact, then the transformed configuration consists of the artifacts `lib1-min.jar`, `lib2-min.jar` adn `lib3-min.jar`.
