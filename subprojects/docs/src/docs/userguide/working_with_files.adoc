...
As with a normal copy, you can control which files are unpacked via filters and even rename files as they are unpacked. Sometimes you'll only need to unpack a subset of the archive. In that case, you can use api:org.gradle.api.tasks.AbstractCopyTask#eachFile(org.gradle.api.Action)[] to apply an action to each file as it is about to be copied into its destination. You can change the relative path of the files to the another root, like so:
