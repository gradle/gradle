...
One notable difference between the `Copy` and `Sync` tasks and the archive tasks is that duplicate files are not allowed in the latter by default.
The duplicates strategy can be changed by setting the link:{groovyDslPath}/org.gradle.api.tasks.bundling.AbstractArchiveTask.html#org.gradle.api.tasks.bundling.AbstractArchiveTask:duplicatesStrategy[duplicatesStrategy] to e.g. link:{javadocPath}/org/gradle/api/file/DuplicatesStrategy.html#INCLUDE[DuplicatesStrategy.INCLUDE].
