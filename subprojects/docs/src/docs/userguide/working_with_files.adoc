...
One notable difference between the `Copy` and `Sync` tasks and the archives is that starting with Gradle 6.0 duplicate files will not be allowed in archives anymore.
