...
In multi-project builds you should centrally declare plugin dependencies in the root project build file, specifying the version, without applying those used in sub-projects.
Then simply apply them by ID in sub projects build scripts.

.Declare plugin dependencies in the root build script
====
...
For the sake of the example, let's look at the same build as above but fully configured from the root build script:
...
====
include::sample[dir="kotlinDsl/androidBuild",files="settings.gradle.kts[tags=android];build.gradle.kts[tags=android];lib/build.gradle.kts[tags=android];app/build.gradle.kts[tags=android]"]
====
