...
====
include::sample[dir="snippets/java-library/module/groovy",files="build.gradle[tags=inferModulePath]"]
include::sample[dir="snippets/java-library/module/kotlin",files="build.gradle.kts[tags=inferModulePath]"]
====
...
----
include::{samplesPath}/java-library/module/groovy/src/main/java/module-info.java[]
----
...
Gradle will automatically put a Jar of your dependencies on the module path, instead of the classpath, if:

* module path inference is turned on via `java. modularity.inferModulePath.set(true)` (Java Module support is currently an incubating feature; in future Gradle versions this will become the default)
* you are building a module: `module-info.java` present or `Automatic-Module-Name` attribute is set in your Jar manifest
* and the Jar itself is a module: `module-info.class` present or `Automatic-Module-Name` attribute is set in the Jar manifest
...
This version can be inspect when a module is running.
...
For example, `com.google.code.gson:gson:2.8.6` that has the module name `com.google.gson`.
...
----
include::{samplesPath}/java-library/module/kotlin/src/main/java/module-info.java[]
----
...
This can be interesting if you want to build a fully <<application_plugin.adoc#sec:application_modular,modular application>> and want the java runtime to treat everything as a real module, e.g. for performance considerations.
...
NOTE: You can define an automatic module as part of a multi-project that otherwise defines real modules (e.g. as an adapter to another library).
