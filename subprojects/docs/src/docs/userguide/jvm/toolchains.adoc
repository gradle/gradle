...
Gradle will select the first installation that matches the toolchain specification of the build, by looking at all known installations
...
[NOTE]
====
There is one important exception to the above-mentioned precedence rules. When <<sec:auto_detection,auto-detection>> is enabled
and the JVM running Gradle matches the toolchain specification, the installation of that JVM will be picked over any other matching
installation.
====
