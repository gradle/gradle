...
[NOTE]
====
None of the toolchain types takes precedence over another. No location is prioritized over another, nor is any of the
package managers. From the set of all detected JRE/JDK installations, one will be picked according to the
<<sec:precedence,Toolchain Precedence Rules>>.
====
...
When there are multiple local JRE/JDK installations matching the toolchain specification of a build, one will be picked according to the
following rules:

. installation currently running Gradle is preferred over any other
. JDK locations are preferred over JRE ones
...
All these rules are applied as multilevel sorting criteria. Let's illustrate with an example. Let's say that the only
criteria we have added to our toolchain specification is that we are interested in HotSpot implementations and that on our local machine
following installations are detected: *Oracle JRE v17.0.1*, *Oracle JDK v17.0.0*, *Microsoft JDK 11.0.13*, *Microsoft JDK 11.0.12.7.1* and
AdoptOpenJDK 16.0.1.j9, which is running Gradle, but which doesn't match the toolchain specification.

The first rule is applied, but since the installation running Gradle doesn't match the toolchain specification (it's not a HotSpot
JVM), this rule doesn't yield anything, so we continue. If this wouldn't be the case, then the installation running Gradle would be picked,
and we would be done.

The second rule is applied, ie. we prefer JDK installations, so we are left with 3 matching ones: *Oracle JDK v17.0.0*, *Microsoft JDK 11.0.13*, *Microsoft JDK 11.0.12.7.1*.

The third rule is applied, ie. we prefer Microsoft as a vendor over Oracle, so we are left with 2 of them: *Microsoft JDK 11.0.13*,
*Microsoft JDK 11.0.12.7.1*.

Then the fourth rule is applied, the highest version number wins. The major version is the same, but the minor versions aren't, so
*Microsoft JDK 11.0.13* wins.

Since we have narrowed down the list to a single candidate, no further rules need to be applied.
