...
[NOTE]
====
None of the toolchain types takes precedence over another. No location is prioritized over another, nor over any of the
...
When there are multiple local JRE/JDK installations matching the toolchain specification of a build, one will be picked according to
following rules:
...
.. SAP
.. everything else
. higher major version takes precedence over lover major version
. higher minor version takes precedence over lover minor version
. installation paths take precedence according to lexicographic ordering (last resort criteria for deterministically deciding
between installations of the same type, from the same vendor and with the same version)

[NOTE]
====
The rules are applied in order, just like sorting over multiple fields. For example if there is one JDK location
detected and two JRE ones, then the JDK location will be picked, regardless of version numbers.
====
