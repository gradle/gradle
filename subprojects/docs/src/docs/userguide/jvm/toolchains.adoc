...
If you are configuring <<sec:custom_loc>>, note that installations that are non-existing or without a
`bin/java` executable will be ignored with a warning, but they won't generate an error.
