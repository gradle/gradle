...
One example of a toolchain resolver plugin is the https://github.com/gradle/disco-toolchains[Disco Toolchains Plugin], based on the https://github.com/foojayio/discoapi[foojay Disco API]. It even has a convention variant, which (if all you need is one repository to download toolchains from) automatically takes care of all the needed configuration (see below), just by being applied:
