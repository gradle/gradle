...
<2> the `javaRepositories` container can be used to define named Java toolchain repository configurations; the configurations are linked to the actual repository implementations via the `implementationClass` property
