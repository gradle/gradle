...
Although the authentication & authorization  mechanisms are shared between the two, toolchain download repositories have nothing else in common with the <<declaring_repositories.adoc,artifact repositories used by Gradle in the context of dependency management>>.
