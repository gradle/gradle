...
Also, you are able to dry run your tests with `--test-dry-run` CLI option. When this option is used, Gradle will simulate the execution of the tests without actually running them. Instead, Gradle will generate a report that lists all the tests that would have been executed based on your filtering configuration. The `--test-dry-run` option can be useful when you want to verify that your test filtering configuration is correct without actually running the tests.
