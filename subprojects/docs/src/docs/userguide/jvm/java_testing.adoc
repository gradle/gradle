...
When `mergeReruns` is disabled (the default), each execution of a test be listed as an separate test case.
...
or any other kind of mechanism the produces a potentially dynamic test name,
care should be take to ensure that the test name is stable and does not unnecessarily change.
