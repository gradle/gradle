...
Here, we have built a convention plugin `myproject.java-conventions` that declares a consumable `binaryTestResultsElements` configuration
that depends on the binary results directory of the `test` task.
This plugin is applied to subprojects from which we want to aggregate the test results.
In the root project's build file, we declare `testReportData` configuration and depend on the projects that we want to aggregate the results from.
Lastly, we add a `testReport` task that sets the `testResultsDirs` property to resolve to `testReportData` configuration.
