...
* They have to read `CLASS` or `RUNTIME` retention annotations only.
    Reading `SOURCE` retention annotations is possible for "isolating" processors on their own. But it will produce incorrect results when a different "aggregating" processor process the same types as the "isolating" processor, so it should not be used.
...
"Aggregating" processors have the following additional limitation:
