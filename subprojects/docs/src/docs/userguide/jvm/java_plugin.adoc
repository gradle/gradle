...
* If a custom `executable` or `javaHome` is configured on the compile task some optimizations will be deactivated, e.g. compile task will not be incremental after a compile error or if a Java constant is changed. It's recommended to use <<toolchains.adoc#toolchains, toolchains>> instead if possible.
