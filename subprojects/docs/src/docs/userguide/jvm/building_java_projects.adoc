...
The simplest build script for a Java project applies the <<java_library_plugin.adoc#,Java Library Plugin>> and optionally sets the project version and select the <<toolchains.adoc#,Java toolchain>> to use:
...
Sometimes however, what you need requires more advanced options.
...
[NOTE]
====
If using Java 9+, leveraging the `release` flag might be an easier solution, see above.
====
...
With the usage of Java toolchains, this is now a trivial thing to do:
...
The only requirement is to have <<toolchains.adoc#sec:auto_detection,Java 7 installed and Gradle aware of it>>.

You can take a look at what you needed to do with previous Gradle versions https://docs.gradle.org/6.6.1/userguide/building_java_projects.html#compiling_and_testing_java_67[here].
...
You can see a basic example of building a Java application in the corresponding link:../samples/building-java-applications.html[sample].
