...
| `codeCoverageReportExecutionData`
...
Note the `codeCoverageReportExecutionData` and `allCodeCoverageReport*Directories` configurations may display resolution errors for external dependencies, as these do not expose code coverage data.  This is expected behavior and does not affect the outcome of the build or code coverage aggregation process.
