...
More information about the variants produced by test execution are available in the <<jvm_test_suite_plugin#_outgoing_variants,Outgoing Variants>> section of the JVM Test Suite Plugin documentation.
...
====
<.> Creates a report named `testAggregateTestReport` of type `AggregateTestReport`.  Sets `TestType` using a constant value from the  link:{javadocPath}/org/gradle/api/attributes/TestSuiteType.html[TestSuiteType] class.
