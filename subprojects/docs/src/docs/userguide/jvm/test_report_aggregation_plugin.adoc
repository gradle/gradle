...
Note the `__testReportName__Results` configuration may display resolution errors for external dependencies, as these do not expose test result data.  This is expected behavior and does not affect the outcome of the build or test report aggregation process.
