...
There are several ways to share configuration between multiple test suites and avoid duplication of dependencies or other elements in a build script.  These methods offer a tradeoff between flexibility and the use of a declarative vs. imperative configuration style.

1. Use the `configureEach` method on the `suites` object to lazily configure every suite
2. Use the `withType`, `matching` and `configureEach` methods and filter by suite name to lazily configure a subset of suites
3. Extract a custom configuration block and apply it only to the desired suites directly

=== Method 1: Use `configureEach`

This is the most straightforward way to share configuration across _every_ test suite.  The configurations is lazily applied to each test suite as it is created.
...
<1> Lazily configure every JVM test suite
...
<4> Add dependencies specific to the `tertiaryTest` test suite

=== Method 2: Use `withType`, `matching` and `configureEach`

This method retains laziness, but adds a filtering commands to only apply the configuration to a subset of the test suites, identified by name.
...
<1> Lazily configure every JVM test suite matching the given criteria
<2> Use a different testing framework for the `tertiaryTest` test suite
<3> Add dependencies specific to the `tertiaryTest` test suite
...
<2> Lazily apply the closure to a test suite, using the default (`test`) test suite
<3> Alternate means of _eagerly_ applying a configuration closure to a test suite outside of its declaration, using the `secondaryTest` test suite
