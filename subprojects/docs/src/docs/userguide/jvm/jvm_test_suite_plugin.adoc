...
The `dependencies` block within a test suite does not (yet) provide access to the full set of dependency configuration methods that are available within a top-level `dependencies` block.
For instance the link:{groovyDslPath}/org.gradle.api.artifacts.dsl.DependencyHandler.html#org.gradle.api.artifacts.dsl.DependencyHandler:platform(java.lang.Object)[platform()] and link:{groovyDslPath}/org.gradle.api.artifacts.dsl.DependencyHandler.html#org.gradle.api.artifacts.dsl.DependencyHandler:platform(java.lang.Object)[enforcedPlatform()] methods described in the section <<java_platform_plugin.html#java_platform_bom_import, Sourcing constraints from another platform>> are not yet available.  You will need to use the equivalent way of configuring dependencies noted above to utilize these methods.

[NOTE]
====
You must access the configurations of a test suite in a top-level `dependencies` block only after creating the test suite in a prior `testing` block.  Adding dependencies is not a lazy operation, and the test suite's configurations must be created prior to adding any dependencies to them using their global names.
====
