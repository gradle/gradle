...
The JVM Test Suite plugin (plugin id: `jvm-test-suite`) allows for modeling multiple groups of automated tests in a Java project.  Tests suites are intended to grouped by similarities between the tests, e.g. fast tests vs. slow tests, or grouped by their common testing framework.

This plugin can be used to define a group of Integration Tests, which might run much longer than unit tests and have different environmental requirements.

[[sec:jvm_test_suite_usage]]
== Usage

This plugin is applied automatically by the `java` plugin, and can be additionally applied explicitly if desired:
...
The plugin cannot be applied standalone as it relies on several conventions of the `java` plugin.
 ...
 * A single `target` backed by a task named `test`.
...
==== Test Suite Target ====
For the initial release of this plugin, each test suite has a single target.  This results in a 1:1:1 relationship between test suite, test suite target and a matching link:{groovyDslPath}/org.gradle.api.tasks.testing.Test.html[Test] task.  The name of the `Test` task is derived from the suite name.  Future iterations of the plugin will allow defining multiple targets based on a `dimension`, such as a particular JDK/JRE against which to execute tests.
...
Note that all tasks synthesized by the registration of the `integrationTest` suite (`compileIntegrationTestJava`, `processIntegrationTestResources`, `integrationTestClasses` and `integrationTest`) do not run until after the `test` suite completes its tasks.
...
== Configuring dependencies of a suite
====
...
<1> Configuration action for the built-in `test` suite.
<2> Add the assertj library to the test's compile and runtime classpaths.  This is equivalent to declaring the library using the `testImplementation` configuration.  Other available configurations in the suite's `dependencies` closure are `compileOnly` and `runtimeOnly`.

== Configuring source directories of a suite
====
...
<4> Reset the `srcDirs` property, replacing the default `src/integrationTest/java` based on the suite name.

== Configuring the `Test` task generated from a suite declaration
====
