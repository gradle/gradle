...
There are several ways to share configuration between multiple test suites and avoid duplication in a build script.  These offer a tradeoff between flexibility and the use of a declarative vs. imperative configuration style.
...
=== Method 1: Use `withType` and `configureEach`
...
<1> Lazily configure every JVM test suite with the same configuration
<2> Configure the dependencies shared by all test suites
<3> Define additional test suites
<4> Configure dependencies specific to the `tertiaryTest` test suite
