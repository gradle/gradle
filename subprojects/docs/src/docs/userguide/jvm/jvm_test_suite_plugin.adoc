...
<4> Add the `projectInternalView()` dependency to the integration test's compile and runtime classpaths. This dependency allows a test suite to access internal project dependencies which are declared on the project's `implementation` and `compileOnly` configurations, in addition to the `api` or `compileOnlyApi` configurations that the `project()` dependency provides.
