...
<4> Configure a new test suite which adds a `projectInternalView()` dependency to the test suite's compile and runtime classpaths. This dependency allows a test suite to access internal project dependencies which are declared on the project's `implementation` and `compileOnly` configurations, in addition to the configurations that the `project()` dependency provides.
