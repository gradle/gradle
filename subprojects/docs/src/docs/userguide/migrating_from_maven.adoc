...
+
Note that the Maven Publish Plugin does not publish *source and Javadoc JARs* by default, but this can easily be configured as explained <<publishing#sec:publishing_custom_artifacts_to_maven,elsewhere in the user manual>>.
...
Don't worry though, you have a lot of control over which versions are selected, as documented in <<controlling_transitive_dependencies#,Managing Transitive Dependencies>>.
...
====
include::sample[dir="userguide/mavenMigration/basic/groovy",files="build.gradle[tags=compile-dependencies]"]
include::sample[dir="userguide/mavenMigration/basic/kotlin",files="build.gradle.kts[tags=compile-dependencies]"]
====
...
====
include::sample[dir="userguide/mavenMigration/basic/groovy",files="build.gradle[tags=repositories]"]
include::sample[dir="userguide/mavenMigration/basic/kotlin",files="build.gradle.kts[tags=repositories]"]
====
...
Lastly, Gradle allows you to resolve dependencies against the <<declaring_repositories.adoc#sec:maven_local,local Maven cache/repository>>.
...
You might also be interested in learning about Gradle's own <<declaring_repositories.adoc#sec:dependency_cache,dependency cache>>, which behaves more reliably than Maven's and can be used safely by multiple concurrent Gradle processes.
 ...
 * <<controlling_transitive_dependencies#sec:adding-constraints-transitive-deps,Dependency constraints>>
 * <<migmvn:using_boms,Bills of materials>> (Maven BOMs)
 * <<controlling_transitive_dependencies#sec:enforcing_dependency_version,Version forcing>>
...
If you want to exclude a dependency for reasons unrelated to versions, then check out the section on <<controlling_transitive_dependencies#excluding-transitive-deps>>.
...
include::sample[dir="userguide/mavenMigration/importBom/kotlin",files="build.gradle.kts[tags=bom]"]
====
<1> Applies the Spring Boot Dependencies BOM
<2> Adds a dependency whose version is defined by that BOM

You can learn more about this feature and the difference between `platform()` and `enforcedPlatform()` in the section on <<controlling_transitive_dependencies#sec:bom_import,importing version recommendations from a Maven BOM>>.
...
====
include::sample[dir="userguide/mavenMigration/multiModule/groovy",files="settings.gradle"]
include::sample[dir="userguide/mavenMigration/multiModule/kotlin",files="settings.gradle.kts"]
====
...
----
> gradle projects
include::{samplesPath}/userguide/mavenMigration/multiModule/projects.out[]
----
...
====
include::sample[dir="userguide/mavenMigration/profiles/groovy",files="build.gradle;profile-default.gradle;profile-test.gradle;profile-prod.gradle"]
include::sample[dir="userguide/mavenMigration/profiles/kotlin",files="build.gradle.kts;profile-default.gradle.kts;profile-test.gradle.kts;profile-prod.gradle.kts"]
====
...
----
> gradle greeting
include::{samplesPath}/userguide/mavenMigration/profiles/greeting-default.out[]
----
.Output of **`gradle -PbuildProfile=test greeting`**
----
> gradle -PbuildProfile=test greeting
include::{samplesPath}/userguide/mavenMigration/profiles/greeting-test.out[]
----
...
====
include::sample[dir="userguide/mavenMigration/basic/groovy",files="build.gradle[tags=process-resources]"]
include::sample[dir="userguide/mavenMigration/basic/kotlin",files="build.gradle.kts[tags=process-resources]"]
====
...
====
include::sample[dir="userguide/mavenMigration/basic/groovy",files="build.gradle[tags=checkstyle]"]
include::sample[dir="userguide/mavenMigration/basic/kotlin",files="build.gradle.kts[tags=checkstyle]"]
====
...
====
include::sample[dir="userguide/mavenMigration/basic/groovy",files="build.gradle[tags=depends-on]"]
include::sample[dir="userguide/mavenMigration/basic/kotlin",files="build.gradle.kts[tags=depends-on]"]
====
...
include::sample[dir="userguide/mavenMigration/ant/kotlin",files="build.gradle.kts"]
====
...
As a final note, this guide has only touched on a few of Gradle's features and we encourage you to learn about the rest from the other chapters of the user manual and from our tutorial-style link:{guidesUrl}/[Gradle Guides].
