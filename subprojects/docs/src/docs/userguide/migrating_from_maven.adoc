...
`compile`::
Use the `classes` task provided by the <<java_plugin.html#sec:java_tasks,Java Plugin>> and other JVM language plugins.
...
It runs just the unit tests, or more specifically, the tests that make up the <<java_plugin.html#source_sets,`test` source set>>.
