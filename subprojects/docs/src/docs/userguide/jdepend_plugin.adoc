...
WARNING: Since JDepend is unmaintained and does not support bytecode compiled for Java 8 and above, the JDepend plugin has been deprecated and is scheduled to be removed in Gradle 6.0.
...
====
include::sample[dir="codeQuality/groovy",files="build.gradle[tags=use-jdepend-plugin]"]
include::sample[dir="codeQuality/kotlin",files="build.gradle.kts[tags=use-jdepend-plugin]"]
====
