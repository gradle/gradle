 ...
 *
 * <p>The filtered collections returned by the filtering methods, such as {@link #matching(Spec)}, return collections that are <em>live</em>. That is, they reflect
...
public interface DomainObjectCollection<T> extends Collection<T> {
    ...
    // Do not delete!
    // This is here to override the default Groovy Collection.findAll(Closure) method.
    /**
     * Returns a collection which contains the objects in this collection which meet the given specification.
     * <p>
     * NOTE: This eagerly iterates over the collection. Elements added after this method is called will not be present in the returned Collection.
     * You should use {@link #matching(Spec)} for a lazy equivalent.
     * </p>
     *
     * @param spec The specification to use. The specification gets a collection element as an argument.
     * @return The collection of matching objects. Returns an empty collection if there are no such objects in this collection.
     * @see #matching(Spec)
     */
    Collection<T> findAll(Spec<? super T> spec);
}
