...
/**
 * This type only exists to make older plugins work after makinng the return type of {@link BuildServiceRegistry#registerIfAbsent(String, Class, Action)} more concrete.
 *
 * @since 7.2
 */
@Incubating
public interface BuildServiceRegistryLegacySupport {

    /**
     * Registers a service, if a service with the given name is not already registered. The service is not created until required, when the returned {@link BuildServiceProvider} is queried.
     *
     ...
     * @return A {@link BuildServiceProvider} that will create the service instance when queried.
     */
