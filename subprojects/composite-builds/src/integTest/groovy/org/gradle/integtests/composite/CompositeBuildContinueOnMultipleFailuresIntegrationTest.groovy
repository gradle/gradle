...
class CompositeBuildContinueOnMultipleFailuresIntegrationTest extends AbstractCompositeBuildIntegrationTest {
    ...
    def "can collect build failures from multiple included builds"() {
        ...
        failure.assertHasDescription("Execution failed for task ':buildB:test' (registered by plugin 'org.gradle.jvm-test-suite').")
        failure.assertHasDescription("Execution failed for task ':buildC:sub1:test' (registered by plugin 'org.gradle.jvm-test-suite').")
        failure.assertHasDescription("Execution failed for task ':buildC:sub2:test' (registered by plugin 'org.gradle.jvm-test-suite').")
        failure.assertHasDescription("Execution failed for task ':buildC:sub3:test' (registered by plugin 'org.gradle.jvm-test-suite').")
        failure.assertHasDescription("Execution failed for task ':buildD:test' (registered by plugin 'org.gradle.jvm-test-suite').")
    }
