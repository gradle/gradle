...
"""
        ...
        */
        ...
        buildFile << """
            ...
            task checkDeps << {
                ...
                def a = result.allComponents.find { it.id.name == 'a' }
                assert a.dependencies*.toString() == ['org:b:2.0']
                def b = result.allComponents.find { it.id.name == 'b' }
    ...
    void "can deal with transitive with parent in conflict"() {
        ...
        task checkDeps << {
            ...
            def bChild = result.allComponents.find { it.id.name == 'b-child' }
            assert bChild.dependencies*.toString() == ['org:in-conflict:2.0']
            def target = result.allComponents.find { it.id.name == 'target' }
...
task checkDeps(dependsOn: configurations.compile) << {
    ...
    assert configurations.compile.incoming.resolutionResult.allComponents.find { it.id.name == 'b' }.dependencies.size() == 1
}
"""
