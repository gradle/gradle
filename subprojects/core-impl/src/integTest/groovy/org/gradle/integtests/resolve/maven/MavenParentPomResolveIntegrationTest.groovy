...
""")
        ...
        def dep = mavenHttpRepo.module('group-two', 'artifact-two', '1.1').publish()
        def module = mavenHttpRepo.module('group', 'artifact', '1.0').parent('group', 'parent', '1.0').publish()
        module.pomFile.text = module.pomFile.text.replace("</project>", """
    <dependencies>
        <dependency>
            <groupId>group-two</groupId>
            <artifactId>artifact-two</artifactId>
...
""")
        ...
        buildFile << """
            ...
            task libs << { assert configurations.compile.files.collect {it.name} == ['artifact-1.0.jar', 'artifact-two-1.1.jar'] }
        """
