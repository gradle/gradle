...
val wrapperJar = tasks.register<Copy>("wrapperJar") {
    from(project(":wrapper").tasks.getByName("executableJar"))
    into(wrapperJarDir)
}

java.sourceSets {
    "main" {
        // TODO Replace with better way of modelling buildBy with Kotlin DSL once available
        output.dir(mapOf("buildBy" to wrapperJar), wrapperJarDir)
    }
}
