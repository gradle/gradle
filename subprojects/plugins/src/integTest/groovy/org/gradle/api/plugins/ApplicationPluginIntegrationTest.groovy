...
class ApplicationPluginIntegrationTest extends WellBehavedPluginTest {
    ...
    def "can generate start scripts with minimal user configuration"() {
        ...
        unixStartScriptContent.contains('#   sample start up script for POSIX generated by Gradle.')
        unixStartScriptContent.contains('DEFAULT_JVM_OPTS=\$( cat ) <<EndOfGradleSubstitution\n\nEndOfGradleSubstitution')
        unixStartScriptContent.contains('APP_NAME=\$( cat ) <<EndOfGradleSubstitution\nsample\nEndOfGradleSubstitution')
        unixStartScriptContent.contains('CLASSPATH=\$( cat ) <<EndOfGradleSubstitution\n\$APP_HOME/lib/sample.jar\nEndOfGradleSubstitution')
...
"""
        ...
        String unixStartScriptContent = unixStartScript.text
        unixStartScriptContent.contains('#   myApp start up script for POSIX generated by Gradle.')
        unixStartScriptContent.contains('APP_NAME=\$( cat ) <<EndOfGradleSubstitution\nmyApp\nEndOfGradleSubstitution')
        unixStartScriptContent.contains('DEFAULT_JVM_OPTS=\$( cat ) <<EndOfGradleSubstitution\n"-Dgreeting.language=en" "-DappId=sample"\nEndOfGradleSubstitution')
        unixStartScriptContent.contains('CLASSPATH=\$( cat ) <<EndOfGradleSubstitution\n\$APP_HOME/lib/sample.jar\nEndOfGradleSubstitution')
        unixStartScriptContent.contains('exec "\$JAVACMD" "\$@"')
    ...
    def "can change template file for default start script generators"() {
        given:
        file('customUnixStartScript.txt') << '${applicationName} start up script for POSIX generated by Gradle.'
...
"""
        ...
        then:
        File unixStartScript = assertGeneratedUnixStartScript('myApp')
        unixStartScript.text == 'myApp start up script for POSIX generated by Gradle.'
        File windowsStartScript = assertGeneratedWindowsStartScript('myApp.bat')
...
class CustomUnixStartScriptGenerator implements ScriptGenerator {
    void generateScript(JavaAppStartScriptGenerationDetails details, Writer destination) {
        destination << "\${details.applicationName} start up script for POSIX generated by Gradle."
    }
}
