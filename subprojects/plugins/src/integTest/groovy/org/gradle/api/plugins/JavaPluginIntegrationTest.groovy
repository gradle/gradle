...
class JavaPluginIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "settings classesDir restores old behavior"() {
        ...
        result.deprecationReport.contains("Gradle now uses separate output directories for each JVM language, but this build assumes a single directory for all classes from a source set.")
    }
