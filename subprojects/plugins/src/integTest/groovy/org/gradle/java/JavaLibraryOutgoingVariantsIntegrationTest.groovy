...
"""
    ...
    def "provides runtime JAR as default variant"() {
        ...
        result.assertOutputContains("files: [file-dep.jar, java.jar, api-1.0.jar, compile-1.0.jar, other-java.jar, implementation-1.0.jar, runtime-1.0.jar, runtime-only-1.0.jar]")
...
"""
        ...
        result.assertOutputContains("files: [file-dep.jar, main, api-1.0.jar, compile-1.0.jar, main, runtime-1.0.jar]")
        result.assertOutputContains("file-dep.jar {artifactType=jar}")
        result.assertOutputContains("compile.jar (test:compile:1.0) {artifactType=jar}")
...
"""
        ...
        then:
        result.assertTasksExecuted(":other-java:compileJava", ":java:compileJava", ":consumer:resolve")
        result.assertOutputContains("files: [file-dep.jar, main, api-1.0.jar, compile-1.0.jar, main, implementation-1.0.jar, runtime-1.0.jar, runtime-only-1.0.jar]")
        result.assertOutputContains("file-dep.jar {artifactType=jar}")
        result.assertOutputContains("compile.jar (test:compile:1.0) {artifactType=jar}")
