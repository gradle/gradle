...
public abstract class VersionCatalogPlugin implements Plugin<Project> {
    ...
    private void createPublication(ProjectInternal project, TaskProvider<TomlFileGenerator> generator) {
        Configuration exported = project.getConfigurations().migratingUnlocked(VERSION_CATALOG_ELEMENTS, ConfigurationRolesForMigration.CONSUMABLE_BUCKET_TO_CONSUMABLE);
        exported.setDescription("Artifacts for the version catalog");
        exported.getOutgoing().artifact(generator);
        exported.attributes(attrs -> {
    ...
    private Configuration createDependenciesConfiguration(ProjectInternal project) {
        Configuration conf = project.getConfigurations().dependenciesUnlocked(GRADLE_PLATFORM_DEPENDENCIES);
