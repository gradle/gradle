...
public class DefaultJvmFeature implements JvmFeatureInternal {
    ...
    private static Configuration maybeCreateElementsConfiguration(
        ...
        if (useMigrationRoleForElementsConfigurations) {
            return configurations.maybeCreateMigratingUnlocked(name, ConfigurationRolesForMigration.CONSUMABLE_BUCKET_TO_CONSUMABLE);
        } else {
    ...
    private Configuration bucket(String kind, String suffix, boolean create, boolean warnOnDuplicate) {
        String configName = getConfigurationName(suffix);
        Configuration configuration = create
            ? project.getConfigurations().maybeCreateDependenciesUnlocked(configName, warnOnDuplicate)
