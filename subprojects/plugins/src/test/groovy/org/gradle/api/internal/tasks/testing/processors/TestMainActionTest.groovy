...
class TestMainActionTest {
    ...
    private final TestMainAction action = new TestMainAction(detector, processor, resultProcessor, timeProvider, "123", "Test Run", "Action Display")
    ...
    public void firesStartAndEndEventsAroundDetectorExecution() {
        context.checking {
            ...
            will { TestDescriptorInternal suite, TestStartEvent event ->
                assertThat(suite.id as String, equalTo("Test Run"))
            ...
            one(resultProcessor).completed(withParam(equalTo("Test Run")), withParam(notNullValue()))
            will { id, TestCompleteEvent event ->
                assertThat(id as String, equalTo("Test Run"))
