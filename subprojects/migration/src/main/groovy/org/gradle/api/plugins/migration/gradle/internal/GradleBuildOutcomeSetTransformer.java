...
public class GradleBuildOutcomeSetTransformer implements Transformer<Set<BuildOutcome>, ProjectOutput> {
    ...
    private void addArchivesByTaskPath(ProjectOutput output, Set<BuildOutcome> outcomes) {
        for (Archive archive : output.getArchives()) {
            File storedFile = fileStore.move(String.format("%s/%s", archive.getTaskPath(), archive.getFile().getName()), archive.getFile()).getFile();
            BuildOutcome buildOutcome = new GeneratedArchiveBuildOutcome(
                    archive.getTaskPath(), "Archive generated by Gradle task", storedFile
            );
