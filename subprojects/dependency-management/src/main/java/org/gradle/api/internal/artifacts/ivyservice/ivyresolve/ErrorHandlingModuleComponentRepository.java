...
public class ErrorHandlingModuleComponentRepository implements ModuleComponentRepository {
    ...
    private static final class ErrorHandlingModuleComponentRepositoryAccess implements ModuleComponentRepositoryAccess {
        ...
        public ErrorHandlingModuleComponentRepositoryAccess(ModuleComponentRepositoryAccess delegate, String repositoryId, RepositoryBlacklister repositoryBlacklister) {
            this.delegate = delegate;
        ...
        public void listModuleVersions(DependencyMetadata dependency, BuildableModuleVersionListingResolveResult result) {
            if (repositoryBlacklister.isBlacklisted(repositoryId)) {
                result.unresolved(new ModuleVersionResolveException(dependency.getSelector(), BLACKLISTED_REPOSITORY_ERROR_MESSAGE));
                return;
            }
            ...
            try {
                ...
                boolean blacklisted = repositoryBlacklister.blacklistRepository(repositoryId, throwable);

                if (blacklisted) {
                    result.unresolved(new ModuleVersionResolveException(dependency.getSelector(), throwable));
                } else {
        ...
        public void resolveComponentMetaData(ModuleComponentIdentifier moduleComponentIdentifier, ComponentOverrideMetadata requestMetaData, BuildableModuleComponentMetaDataResolveResult result) {
            if (repositoryBlacklister.isBlacklisted(repositoryId)) {
                result.unresolved(new ModuleVersionResolveException(moduleComponentIdentifier, BLACKLISTED_REPOSITORY_ERROR_MESSAGE));
                return;
            }
            ...
            try {
                ...
                ModuleVersionResolveException resolveException = new ModuleVersionResolveException(moduleComponentIdentifier, throwable);
                boolean blacklisted = repositoryBlacklister.blacklistRepository(repositoryId, throwable);

                if (blacklisted) {
                    result.unresolved(resolveException);
                } else {
                    result.failed(resolveException);
                }
            }
        }
        ...
        public void resolveArtifactsWithType(ComponentResolveMetadata component, ArtifactType artifactType, BuildableArtifactSetResolveResult result) {
            if (repositoryBlacklister.isBlacklisted(repositoryId)) {
                result.unresolved(new ArtifactResolveException(component.getComponentId(), BLACKLISTED_REPOSITORY_ERROR_MESSAGE));
                return;
            }
            ...
            try {
                ...
                ArtifactResolveException resolveException = new ArtifactResolveException(component.getComponentId(), throwable);
                boolean blacklisted = repositoryBlacklister.blacklistRepository(repositoryId, throwable);
        ...
        public void resolveArtifact(ComponentArtifactMetadata artifact, ModuleSource moduleSource, BuildableArtifactResolveResult result) {
            if (repositoryBlacklister.isBlacklisted(repositoryId)) {
                result.unresolved(new ArtifactResolveException(artifact.getId(), BLACKLISTED_REPOSITORY_ERROR_MESSAGE));
                return;
            }
            ...
            try {
                ...
                ArtifactResolveException resolveException = new ArtifactResolveException(artifact.getId(), throwable);
                boolean blacklisted = repositoryBlacklister.blacklistRepository(repositoryId, throwable);
