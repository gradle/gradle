...
/**
 * TODO: This class sources Gradle metadata files, but there's no such thing as a MutableGradleModuleComponentResolveMetadata yet: dependending on the repository type, we will generate either Ivy or
 * Maven module metadata. Ideally, this shouldn't be the case, we should have a Gradle specific metadata format, and we shouldn't bound the component resolve metadata format to the repository type.
 * But this is for later. As a result, we need to pass in a factory that will create an "empty shell" for resolved metadata that the Gradle module metadata parser will feed.
 */
public class DefaultGradleModuleMetadataSource extends AbstractMetadataSource<MutableModuleComponentResolveMetadata> implements GradleModuleMetadataSource {
    ...
    @Inject
    public DefaultGradleModuleMetadataSource(ModuleMetadataParser metadataParser, MutableModuleMetadataFactory<MutableModuleComponentResolveMetadata> mutableModuleMetadataFactory) {
