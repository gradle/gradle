...
public class DefaultIvyModulePublishMetaData implements BuildableIvyModulePublishMetaData, BuildableLocalComponentMetaData {
    ...
    private final Map<IvyArtifactName, File> artifactFiles = Maps.newHashMap();
    ...
    public void addArtifacts(String configuration, Iterable<? extends PublishArtifact> artifacts) {
        for (PublishArtifact artifact : artifacts) {
            ...
            descriptor.addArtifact(ivyName, Collections.singleton(configuration));
            artifactFiles.put(ivyName, artifact.getFile());
        }
    }

    public void addPublishedArtifact(IvyArtifactName artifact, File file) {
        artifactFiles.put(artifact, file);
    }

    public Collection<IvyModuleArtifactPublishMetaData> getArtifacts() {
        List<IvyModuleArtifactPublishMetaData> artifacts = Lists.newArrayList();
        for (Map.Entry<IvyArtifactName, File> entry : artifactFiles.entrySet()) {
            IvyArtifactName artifactName = entry.getKey();
            File artifactFile = entry.getValue();
            artifacts.add(new DefaultIvyModuleArtifactPublishMetaData(id, artifactName, artifactFile));
        }
        return artifacts;
    }
