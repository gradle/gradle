...
class JavaEcosystemAttributeMatcherTest extends Specification {
    ...
    def "resolve compileClasspath with java plugin"() {
        ...
        def compileElements = [attributes(Usage.JAVA_API, LibraryElements.JAR, 8, CompileView.JAVA_COMPLETE)]
    ...
    def "resolve compileClasspath for complete compile view with java plugin"() {
        def requested = attributes(Usage.JAVA_API, LibraryElements.CLASSES, 8, CompileView.JAVA_COMPLETE)
    ...
    def "resolve compileClasspath for complete compile view with java-library plugin"() {
        def requested = attributes(Usage.JAVA_API, LibraryElements.CLASSES, 8, CompileView.JAVA_COMPLETE)
    ...
    def "resolve runtimeClasspath for complete compile view with java plugin"() {
        // Even if we request the complete compile view during compile-time, we still want runtimeElements during runtime
    ...
    def "resolve runtimeClasspath for complete compile view with java-library plugin"() {
        // Even if we request the complete compile view during compile-time, we still want runtimeElements during runtime
        def requested = attributes(Usage.JAVA_RUNTIME, LibraryElements.JAR, 8, CompileView.JAVA_API)
    ...
    def "resolve compileClasspath with java plugin targetJvm={8,11} requesting 9"() {
        ...
        def compileElements11 = [attributes(Usage.JAVA_API, LibraryElements.JAR, 11, CompileView.JAVA_COMPLETE)]
    ...
    def "resolve compileClasspath with java plugin targetJvm={8,9,11} requesting 9"() {
        ...
        def compileElements9 = [attributes(Usage.JAVA_API, LibraryElements.JAR, 9, CompileView.JAVA_COMPLETE)]
     ...
     * First, attributec matching is performed against the implicit variants of each consumable configuration.
     * Then, assuming attribute matching returns a single matched configuration, Gradle will then perform
     * attribute matching against each variant (including the implicit) of the matched configuration.
     * <p>
     ...
     * variants. The outer array is the collection of all consumable configurations being tested.
     *
