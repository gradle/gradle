...
class CachedCodePathComponentMetadataProcessorTest extends Specification {
    ...
    private DefaultComponentMetadataProcessor processorWithCacheThatNeverHits() {
        ...
        cacheBuilder = Mock(CacheBuilder) {
            withLockOptions(_ as LockOptions) >> { cacheBuilder }
            open() >> {
                Mock(PersistentCache) {
                    createCache(_ as PersistentIndexedCacheParameters) >> {
                        Mock(PersistentIndexedCache) {
                            getIfPresent(_) >> null
                        }
                    }
                }
        ...
        GlobalScopedCache cacheRepository = Mock(GlobalScopedCache) {
            cache(_ as String) >> cacheBuilder
        }
