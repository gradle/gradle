...
class DefaultArtifactTransformsTest extends Specification {
    ...
    def "selects producer variant with requested attributes"() {
        ...
        expect:
        def result = transforms.variantSelector(typeAttributes("classes"), true, configuration).select(set)
  ...
  - <variant2>: Required artifactType 'classes' and found incompatible value 'jar'.""")
    ...
    def "selects variant with attributes that can be transformed to requested format"() {
        ...
        def result = transforms.variantSelector(targetAttributes, true, configuration).select(set)
        ...
        1 * transformation.transform({ it.files == [sourceArtifactFile]}) >> TransformationSubject.initial(sourceArtifactId, sourceArtifactFile, configuration).transformationSuccessful(ImmutableList.of(outFile1, outFile2))

        1 * transformation.transform({ it.files == [sourceFile] }) >> TransformationSubject.initial(sourceFile, configuration).transformationSuccessful(ImmutableList.of(outFile3, outFile4))
    ...
    def "fails when multiple transforms match"() {
        ...
        def selector = transforms.variantSelector(typeAttributes("dll"), true, configuration)
...
Found the following transforms:
  ...
  - Transform from <variant2>: artifactType 'classes'""")
    ...
    def "returns empty variant when no variants match and ignore no matching enabled"() {
        ...
        expect:
        def result = transforms.variantSelector(typeAttributes("dll"), true, configuration).select(set)
    ...
    def "fails when no variants match and ignore no matching disabled"() {
        ...
        when:
        def result = transforms.variantSelector(typeAttributes("dll"), false, configuration).select(set)
        visit(result)
