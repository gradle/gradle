...
import org.gradle.language.gosu.fixtures.BadGosuClass
import org.gradle.language.gosu.fixtures.TestGosuComponent

class GosuLanguageIntegrationTest extends AbstractJvmLanguageIntegrationTest {
    TestJvmComponent app = new TestGosuComponent()

    def 'reports failure to compile bad gosu sources'() {
        when:
        def badApp = new BadGosuClass()
        badApp.sources*.writeToDir(file('src/myLib/gosu'))
