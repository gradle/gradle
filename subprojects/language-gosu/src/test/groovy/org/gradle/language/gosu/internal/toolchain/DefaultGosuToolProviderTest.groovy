...
class DefaultGosuToolProviderTest extends Specification {
    FileResolver fileResolver = Mock()
    CompilerDaemonManager compilerDaemonManager = Mock()
    Set<File> gosuClasspath = Mock()
    File gradleUserHome = Mock()
    File rootProjectDir = Mock()

    def "newCompiler provides decent error for unsupported CompileSpec"() {
        setup:
        DefaultGosuToolProvider gosuToolProvider = new DefaultGosuToolProvider(gradleUserHome, rootProjectDir, compilerDaemonManager, gosuClasspath, gosuClasspath)

        when:
        gosuToolProvider.newCompiler(UnknownCompileSpec.class)
