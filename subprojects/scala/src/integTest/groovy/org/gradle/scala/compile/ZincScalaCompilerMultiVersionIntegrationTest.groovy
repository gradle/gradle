    ...
    def setup() {
        buildFile << """
            ...
            task assertZincVersion {
                ...
                doLast {
                    def resolvedDependencies = configurations.zinc.incoming.dependencies
                    assert resolvedDependencies.find { it.group == "org.scala-sbt" && it.name == "zinc_2.13" && it.version == "${version}" }
                }
            }
        """
        args("--info")
