...
class ExperimentalIncrementalArtifactTransformationsRunner extends BehindFlagFeatureRunner {
    ...
    static boolean isIncrementalArtifactTransformations() {
        Boolean.getBoolean(INCREMENTAL_ARTIFACT_TRANSFORMATIONS)
    }

    static void configureIncrementalArtifactTransformations(TestFile settingsFile) {
        if (incrementalArtifactTransformations) {
            FeaturePreviewsFixture.enableIncrementalArtifactTransformations(settingsFile)
        }
    }
}
