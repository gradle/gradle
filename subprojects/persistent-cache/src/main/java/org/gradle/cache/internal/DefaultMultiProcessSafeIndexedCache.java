...
public class DefaultMultiProcessSafeIndexedCache<K, V> implements MultiProcessSafeIndexedCache<K, V> {
    ...
    private final Factory<PersistentMap<K,V>> factory;
    private PersistentMap<K, V> cache;

    public DefaultMultiProcessSafeIndexedCache(Factory<PersistentMap<K,V>> factory, FileAccess fileAccess) {
    ...
    public V getIfPresent(final K key) {
        final PersistentMap<K, V> cache = getCache();
        try {
    ...
    private PersistentMap<K, V> getCache() {
        if (cache == null) {
            // Use writeFile because the cache can internally recover from datafile
            // corruption, so we don't care at this level if it's corrupt
