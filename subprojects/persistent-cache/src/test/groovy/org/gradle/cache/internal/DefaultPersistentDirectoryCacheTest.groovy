...
class DefaultPersistentDirectoryCacheTest extends AbstractProjectBuilderSpec {
    ...
    def initialisesCacheWhenCacheDirDoesNotExist() {
        ...
        when:
        def cache = new DefaultPersistentDirectoryExclusiveCache(emptyDir, "<display-name>", properties, CacheBuilder.LockTarget.DefaultTarget, mode(FileLockManager.LockMode.Shared), initializationAction, cacheCleanup, lockManager, Mock(ExecutorFactory), progressLoggerFactory)
        try {
            cache.open()
        } finally {
            cache.close()
    ...
    def initializesCacheWhenPropertiesFileDoesNotExist() {
        ...
        def cache = new DefaultPersistentDirectoryExclusiveCache(dir, "<display-name>", properties, CacheBuilder.LockTarget.DefaultTarget, mode(FileLockManager.LockMode.Shared), initializationAction, cacheCleanup, lockManager, Mock(ExecutorFactory), progressLoggerFactory)
    ...
    def rebuildsCacheWhenInitializerFailedOnPreviousOpen() {
        ...
        when:
        cache = new DefaultPersistentDirectoryExclusiveCache(dir, "<display-name>", properties, CacheBuilder.LockTarget.DefaultTarget, mode(FileLockManager.LockMode.Shared), initializationAction, cacheCleanup, lockManager, Mock(ExecutorFactory), progressLoggerFactory)
        try {
            cache.open()
        } finally {
            cache.close()
    ...
    def "will rebuild cache if not unlocked cleanly"() {
        ...
        def cache = new DefaultPersistentDirectoryExclusiveCache(dir, "test", [:], CacheBuilder.LockTarget.DefaultTarget,
    ...
    def "will rebuild cache if cache.properties is missing and properties are not empty"() {
        ...
        def cache = new DefaultPersistentDirectoryExclusiveCache(dir, "test", properties, CacheBuilder.LockTarget.DefaultTarget,
            mode(FileLockManager.LockMode.Exclusive), init, cacheCleanup, createDefaultFileLockManager(), Mock(ExecutorFactory), progressLoggerFactory)
    ...
    private TestFile createCacheDir(Map<String, ?> extraProps = [:]) {
        ...
        DefaultPersistentDirectoryExclusiveCache cache = new DefaultPersistentDirectoryExclusiveCache(dir, "<display-name>", properties, CacheBuilder.LockTarget.DefaultTarget, mode(FileLockManager.LockMode.Shared), null, null, lockManager, Mock(ExecutorFactory), progressLoggerFactory)
