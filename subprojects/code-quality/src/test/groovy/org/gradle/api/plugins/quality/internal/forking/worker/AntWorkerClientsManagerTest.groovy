...
class AntWorkerClientsManagerTest extends Specification {

    def workingDir = new File("some-dir")

    def options = Stub(DaemonForkOptions)
    def starter = Stub(AntWorkerDaemonStarter)

    @Subject manager = new AntWorkerClientsManager(starter)
    ...
    def "does not reserve idle client when no matching client found"() {
        def noMatch = Stub(AntWorkerDaemonClient) {
    ...
    def "reserves idle client when match found"() {
        def noMatch = Stub(AntWorkerDaemonClient) { isCompatibleWith(_) >> false }
        def match = Stub(AntWorkerDaemonClient) { isCompatibleWith(_) >> true }
    ...
    def "reserves new client"() {
        def newClient = Stub(AntWorkerDaemonClient)
    ...
    def "can stop all created clients"() {
        def client1 = Mock(AntWorkerDaemonClient)
        def client2 = Mock(AntWorkerDaemonClient)
    ...
    def "clients can be released for further use"() {
        def client = Mock(AntWorkerDaemonClient) { isCompatibleWith(_) >> true }
