...
class GosuCompileOptionsTest {

    private GosuCompileOptions compileOptions

    @Before
    void setUp() {
        compileOptions = new GosuCompileOptions()
    }

    @Test
    void testOptionMapContainsFailOnError() {
        assertBooleanValue('failOnError', 'failOnError', true)
    }

    @Test
    void testOptionMapDoesNotContainTargetCompatibility() {
        assert !compileOptions.optionMap().containsKey('target')
    }

    @Test
    @Ignore
    void testOptionMapContainsListFiles() {
        assertBooleanValue('listFiles', 'listFiles', false)
    }

    @Test
    @Ignore
    void testOptionMapContainsLoggingLevelIfSpecified() {
        assertSimpleStringValue('loggingLevel', 'logging', null, 'verbose')
    }

    @Test
    void disablingUseAntEnablesFork() {
