...
abstract class AbstractHttpBuildCacheServiceTest extends Specification {
    ...
    final key = new DefaultBuildCacheKey(HashCode.fromString("01234567abcdef"))
    final config = TestUtil.newInstance(HttpBuildCache.class)
    final buildCacheDescriber = new NoopBuildCacheDescriber()
    final httpClientHelperFactory = HttpClientHelper.Factory.createFactory(new DocumentationRegistry())
    final bufferProvider = Stub(BufferProvider) {
        getBuffer() >> new byte[1024]
    }
    final buildCacheServiceFactory = new DefaultHttpBuildCacheServiceFactory(
    ...
    protected abstract void store(BuildCacheKey key, Writer writer)

    protected abstract boolean load(BuildCacheKey key, Reader reader)
    ...
    static class NoopBuildCacheDescriber implements BuildCacheServiceFactory.Describer {
    ...
    static class Writer implements BuildCacheEntryWriter, NextGenBuildCacheService.EntryWriter {
    ...
    static abstract class Reader implements BuildCacheEntryReader, NextGenBuildCacheService.EntryReader {
        @Override
        final void readFrom(InputStream input) throws IOException {
            doRead(input)
        }

        abstract protected void doRead(InputStream input) throws IOException
    }
