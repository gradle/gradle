...
public abstract class AbstractFingerprintCompareStrategy extends AbstractCompareStrategy<FileCollectionFingerprint, FileSystemLocationFingerprint> implements FingerprintCompareStrategy {
    ...
    public AbstractFingerprintCompareStrategy() {
        super(FileCollectionFingerprint::getFingerprints);
    }
    ...
    @Override
    protected boolean hasSamePath(FileSystemLocationFingerprint previous, FileSystemLocationFingerprint current) {
        return previous.getNormalizedPath().equals(current.getNormalizedPath());
    }

    @Override
    protected boolean hasSameContent(FileSystemLocationFingerprint previous, FileSystemLocationFingerprint current) {
        return previous.getNormalizedContentHash().equals(current.getNormalizedContentHash());
    }

    @Override
    protected Change added(String path, String propertyTitle, FileSystemLocationFingerprint current) {
        return DefaultFileChange.added(path, propertyTitle, current.getType(), current.getNormalizedPath());
    }

    @Override
    protected Change removed(String path, String propertyTitle, FileSystemLocationFingerprint previous) {
        return DefaultFileChange.removed(path, propertyTitle, previous.getType(), previous.getNormalizedPath());
    }

    @Override
    protected Change modified(String path, String propertyTitle, FileSystemLocationFingerprint previous, FileSystemLocationFingerprint current) {
