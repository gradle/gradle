...
class ImplementationStateChangesTest extends Specification {
    ...
    HistoricalTaskExecution previous(ImplementationSnapshot impl, ImplementationSnapshot... actions) {
        return Mock(HistoricalTaskExecution) {
            getImplementation() >> impl
            getAdditionalImplementations() >> ImmutableList.copyOf(actions)
        }
    }

    CurrentTaskExecution current(ImplementationSnapshot impl, ImplementationSnapshot... actions) {
        return Mock(CurrentTaskExecution) {
            getImplementation() >> impl
            getAdditionalImplementations() >> ImmutableList.copyOf(actions)
        }
    }
    ...
    def "up-to-date when task is the same"() {
        expect:
        changesBetween(
            previous(impl(SimpleTask), impl(TestAction)),
            current(impl(SimpleTask), impl(TestAction))
        ).empty
    }
    ...
    def "not up-to-date when task name changed"() {
        expect:
        changesBetween(
            previous(impl(PreviousTask), impl(TestAction)),
            current(impl(SimpleTask), impl(TestAction))
    ...
    def "not up-to-date when class-loader has changed"() {
        ...
        expect:
        changesBetween(
            previous(impl(SimpleTask, previousHash), impl(TestAction)),
            current(impl(SimpleTask), impl(TestAction))
    ...
    def "not up-to-date when action class-loader has changed"() {
        ...
        expect:
        changesBetween(
            previous(impl(SimpleTask), impl(TestAction, previousHash)),
            current(impl(SimpleTask), impl(TestAction))
    ...
    def "not up-to-date when action with same class-loader is added"() {
        ...
        changesBetween(
            previous(impl(SimpleTask), impl(TestAction)),
            current(impl(SimpleTask), impl(TestAction), impl(TestAction))
        ) == ["One or more additional actions for task ':test' have changed."]
    }
    ...
    def "not up-to-date when task is loaded with an unknown classloader"() {
        ...
        expect:
        changesBetween(
            previous(impl(simpleTaskClass, null), impl(TestAction)),
            current(impl(simpleTaskClass, null), impl(TestAction))
    ...
    def "not up-to-date when task action is loaded with an unknown classloader"() {
        ...
        changesBetween(
            previous(impl(SimpleTask), impl(TestAction)),
            current(impl(SimpleTask), impl(TestAction, null))
    ...
    def "not up-to-date when task was previously loaded with an unknown classloader"() {
        expect:
        changesBetween(
            previous(impl(SimpleTask, null), impl(TestAction)),
            current(impl(SimpleTask), impl(TestAction))
    ...
    def "not up-to-date when task action was previously loaded with an unknown classloader"() {
        expect:
        changesBetween(
            previous(impl(SimpleTask), impl(TestAction, null)),
            current(impl(SimpleTask), impl(TestAction))
    ...
    List<String> changesBetween(HistoricalTaskExecution previous, CurrentTaskExecution current) {
        def visitor = new CollectingChangeVisitor()
        new ImplementationStateChanges(previous, current, executable).accept(visitor)
