...
class CleanupStaleOutputsStepTest extends StepSpec<WorkspaceContext> {
    ...
    @Shared
    def existing = file("existing.txt").createFile()
    @Shared
    def missing = file("missing.txt")
    ...
    def "#description is cleaned up: #cleandUp"() {
        ...
        _ * work.visitOutputs(_) { UnitOfWork.OutputVisitor visitor ->
            visitor.visitOutputProperty("output", TreeType.FILE, target, TestFiles.fixed(target))
        }
        ...
        where:
        description                                          | cleandUp | ownedByBuild | generatedByGradle | creator
        ...
        "symlink owned by but not generated by build"        | true     | true         | false             | { it.createLink(existing) }
        "broken symlink owned by but not generated by build" | true     | true         | false             | { it.createLink(missing) }
