...
class CacheStepTest extends StepSpec implements FingerprinterFixture {
    ...
    def "executes work #description non-incrementally and stores after unpack failure"() {
        ...
        1 * work.visitOutputProperties(_) >> { UnitOfWork.OutputPropertyVisitor visitor ->
            visitor.visitOutputProperty(outputFileProperty("outputFile", TreeType.FILE, ImmutableList.of(loadedOutputFile)))
            visitor.visitOutputProperty(outputFileProperty("outputDir", TreeType.DIRECTORY, ImmutableList.of(loadedOutputDir)))
            visitor.visitOutputProperty(outputFileProperty("missingOutputFile", TreeType.FILE, ImmutableList.of(file("missing.txt"))))
            visitor.visitOutputProperty(outputFileProperty("missingOutputDir", TreeType.DIRECTORY, ImmutableList.of(file("missing"))))
        }
