...
class RemovePreviousOutputsStepTest extends StepSpec<ChangingOutputsContext> implements SnapshotterFixture {
    ...
    void cleanupOverlappingOutputs(WorkOutputs outputs) {
        ...
        _ * work.visitOutputs(_, _) >> { File workspace, OutputVisitor visitor ->
            visitor.visitOutputProperty("dir", TreeType.DIRECTORY, new UnitOfWork.FinalizedOutputFileValueSupplier(outputs.dir, TestFiles.fixed(outputs.dir)))
            visitor.visitOutputProperty("file", TreeType.FILE, new UnitOfWork.FinalizedOutputFileValueSupplier(outputs.file, TestFiles.fixed(outputs.file)))
        }
