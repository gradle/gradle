...
public class DefaultJavaCompilerFactory implements JavaCompilerFactory {
    ...
    private <T extends CompileSpec> Compiler<T> createTargetCompiler(Class<T> type) {
        ...
        if (ForkingJavaCompileSpec.class.isAssignableFrom(type)) {
            return (Compiler<T>) new DaemonJavaCompiler(workingDirProvider.getWorkingDirectory(), JdkJavaCompiler.class, new Object[]{getJavaHomeBasedJavaCompilerFactory()}, new ProcessIsolatedDaemonCompilerWorkerFactory(workerDaemonFactory), forkOptionsFactory, classPathRegistry, actionExecutionSpecFactory);
        } else {
