...
public class AnnotationProcessorPathFactory {
    ...
    /**
     * Calculates the annotation processor path to use given the supplied compile options.
     *
     * @return An empty collection when annotation processing should not be performed, non-empty when it should.
     */
    public FileCollection getEffectiveAnnotationProcessorClasspath(CompileOptions compileOptions) {
        FileCollection annotationProcessorPath = compileOptions.getAnnotationProcessorPath();
        if (annotationProcessorPath == null || compileOptions.getAllCompilerArgs().contains("-proc:none")) {
