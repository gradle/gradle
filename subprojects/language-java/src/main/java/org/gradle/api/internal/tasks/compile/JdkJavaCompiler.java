...
import org.gradle.api.internal.tasks.compile.JavaCompileSpec.JavaClassCompileOrder;
...
public class JdkJavaCompiler implements Compiler<JavaCompileSpec>, Serializable {
    ...
    private JavaCompiler.CompilationTask createCompileTask(JavaCompileSpec spec, ApiCompilerResult result) {
        ...
        Iterable<File> sourceFiles = spec.getJavaClassCompileOrder() == JavaClassCompileOrder.STABLE
            ? StreamSupport.stream(spec.getSourceFiles().spliterator(), false).sorted().collect(Collectors.toList())
            : spec.getSourceFiles();
