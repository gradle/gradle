...
class CommandLineJavaCompilerArgumentsGeneratorTest extends Specification {
    ...
    def "inlines arguments if they are short enough"() {
        ...
        then:
        Lists.newArrayList(args) == ["-J-Xmx256m", "-g", "-sourcepath", "", "-processorpath", "", "-proc:none", USE_UNSHARED_COMPILER_TABLE_OPTION, "-classpath", spec.classpath.join(File.pathSeparator), *spec.source*.path]
    }
    ...
    def "creates arguments file if arguments get too long"() {
        ...
        argsFile.readLines() == ["-g", "-sourcepath", quote(""), "-processorpath", quote(""), "-proc:none", USE_UNSHARED_COMPILER_TABLE_OPTION, "-classpath", quote("${spec.classpath.join(File.pathSeparator)}"), *(spec.source*.path.collect { quote(it) })]
    }
