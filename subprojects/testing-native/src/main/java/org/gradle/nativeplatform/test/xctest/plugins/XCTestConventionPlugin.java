...
public class XCTestConventionPlugin implements Plugin<ProjectInternal> {
            ...
            public void execute(final Project project) {
                ...
                testComponent.getBinaries().whenElementKnown(DefaultSwiftXCTestBinary.class, new Action<DefaultSwiftXCTestBinary>() {
                    ...
                    public void execute(DefaultSwiftXCTestBinary binary) {
                        ...
                        createTestingTask(project.getTasks(), binary);
    ...
    private void configureTestSuiteBuildingTasks(final Project project, final DefaultSwiftXCTestBinary binary) {
        if (binary instanceof SwiftXCTestBundle) {
            ...
            // TODO: Need binary.getCompileTask().configure {}
            ...
            compile.getCompilerArgs().addAll(project.provider(new Callable<List<String>>() {
                ...
                public List<String> call() {
                    // TODO: This should be a dependency, not directly added like this.
            ...
            final Provider<RegularFile> linkedFile = link.map(new Transformer<RegularFile, LinkMachOBundle>() {
                @Override
                public RegularFile transform(LinkMachOBundle link) {
                    return link.getLinkedFile().get();
                }
            });

            TaskProvider<InstallXCTestBundle> install = tasks.createLater(names.getTaskName("install"), InstallXCTestBundle.class, new Action<InstallXCTestBundle>() {
                @Override
                public void execute(InstallXCTestBundle install) {
                    install.getBundleBinaryFile().set(linkedFile);
                    install.getInstallDirectory().set(project.getLayout().getBuildDirectory().dir("install/" + names.getDirName()));
                }
            });
            binary.getInstallDirectory().set(install.map(new Transformer<Directory, InstallXCTestBundle>() {
                @Override
                public Directory transform(InstallXCTestBundle install) {
                    return install.getInstallDirectory().get();
                }
            }));

            binary.getExecutableFile().set(linkedFile);
            binary.getRunScriptFile().set(install.map(new Transformer<RegularFile, InstallXCTestBundle>() {
                @Override
                public RegularFile transform(InstallXCTestBundle install) {
                    return install.getRunScriptFile().get();
                }
            }));
            ...
            executable.getRunScriptFile().set(executable.getInstallTask().map(new Transformer<RegularFile, InstallExecutable>() {
                @Override
                public RegularFile transform(InstallExecutable installExecutable) {
                    return installExecutable.getRunScriptFile().get();
                }
            }));
        }
    }

    private void createTestingTask(TaskContainer tasks, final DefaultSwiftXCTestBinary binary) {
        TaskProvider<XCTest> xcTest = tasks.createLater("xcTest", XCTest.class, new Action<XCTest>() {
            @Override
