...
class LanguageLibraryProjectInitDescriptorTest extends Specification {
    ...
    def "points to default settings file template"(){
        when:
        def descriptor1 = new LanguageLibraryProjectInitDescriptor("someId", Mock(FileResolver), Mock(DocumentationRegistry))
        def descriptor2 = new LanguageLibraryProjectInitDescriptor("someOtherId", Mock(FileResolver), Mock(DocumentationRegistry))
        then:
