...
public class TaskConfiguration {
    ...
    public static void configureInit(final Project project, InitBuild init) {
        init.setGroup(GROUP);
        init.setDescription("Initializes a new Gradle build. [incubating]");
        final Factory<String> setupCanBeSkipped = new Factory<String>() {
            @Override
        ...
        init.onlyIf(new Spec<Task>() {
            ...
            public boolean isSatisfiedBy(Task element) {
                Object skippedMsg = setupCanBeSkipped.create();
                if (skippedMsg != null) {
        ...
        init.dependsOn(new Callable<String>() {
            ...
            public String call() throws Exception {
                if (setupCanBeSkipped.create() == null) {
                    return "wrapper";
                } else {
                    return null;
                }
    ...
    public static void createInitTask(Project project) {
        configureInit(project, project.getTasks().create(INIT_BUILD_TASK_NAME, InitBuild.class));
    }
    ...
    public static void addInitPlaceholder(final ProjectInternal projectInternal) {
        if (projectInternal.getParent() == null) {
            projectInternal.getTasks().addPlaceholderAction("init", InitBuild.class, new Action<InitBuild>() {
                @Override
                public void execute(InitBuild initBuild) {
                    configureInit(projectInternal, initBuild);
                }
            });
        }
    }
    ...
    public static void addWrapperPlaceholder(ProjectInternal projectInternal) {
        if (projectInternal.getParent() == null) {
            projectInternal.getTasks().addPlaceholderAction("wrapper", Wrapper.class, new Action<Wrapper>() {
                @Override
