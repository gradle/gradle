...
public interface WorkerLeaseRegistry {
    /**
     * Ensures that there is a started worker lease associated with the current thread, starting one if necessary.
     * If a lease needs to be started, then the returned {@link WorkerLeaseCompletion} will complete the lease.
     * If there is already a lease for the current thread, the returned {@link WorkerLeaseCompletion} is a no-op.
     */
    WorkerLeaseCompletion maybeStartWorkerLease();
