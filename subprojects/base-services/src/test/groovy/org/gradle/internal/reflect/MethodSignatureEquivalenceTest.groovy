...
class MethodSignatureEquivalenceTest extends Specification  {
    ...
    def "distinguish overloaded overriden methods"() {
        ...
        eq.equivalent(takeObjReturnStr, takeIntReturnInt) == false
        eq.hash(takeObjReturnStr) != eq.hash(takeIntReturnInt)

        eq.equivalent(takeObjReturnObj, takeObjReturnStr) == true
        eq.hash(takeObjReturnObj) == eq.hash(takeObjReturnStr)
    }
