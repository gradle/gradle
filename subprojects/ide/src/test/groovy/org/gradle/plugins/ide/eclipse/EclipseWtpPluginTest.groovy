...
class EclipseWtpPluginTest extends AbstractProjectBuilderSpec {
    ...
    def "wb resource honors web app dir even if configured after plugin appliance"() {
        ...
        project.apply(plugin: 'war')
        project.apply(plugin: 'eclipse-wtp')
        project.tasks.withType(War) { webAppDirectory = project.layout.projectDirectory.dir('foo') }
    ...
    private void checkEclipseWtpComponentForWar() {
        ...
        assert wtp.resources == [new WbResource('/', project.layout.projectDirectory.asFile.relativePath(project.tasks['war'].webAppDirectory.get().asFile))]
