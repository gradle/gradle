            ...
            contextualize(_) >> { MethodRuleAction action ->
                Stub(ModelAction) {
                    ...
                    execute(_, _) >> { MutableModelNode node, List<ModelView<?>> inputs ->
                        action.execute(new DefaultModelRuleInvoker(rule.ruleDefinition.method, { ClassUtils.newInstance(rule.ruleDefinition.method.method.declaringClass) } as Factory), node, inputs) }
                }
            }
