...
class DefaultWorkerExecutorTest extends Specification {
    ...
    def "can add to classpath on executor"() {
        ...
        then:
        1 * classLoaderStructureProvider.getDefaultClassLoaderStructure(_) >> { args -> new ClassLoaderStructure(new VisitableURLClassLoader.Spec("test", args[0].collect { it.toURI().toURL() }))}
