...
public abstract class AbstractModuleDependency extends AbstractDependency implements ModuleDependency {
    ...
    protected boolean isKeyEquals(ModuleDependency dependencyRhs) {
        if (getGroup() != null ? !getGroup().equals(dependencyRhs.getGroup()) : dependencyRhs.getGroup() != null) return false;
        if (!getName().equals(dependencyRhs.getName())) return false;
        if (!getConfiguration().equals(dependencyRhs.getConfiguration())) return false;
        if (getVersion() != null ? !getVersion().equals(dependencyRhs.getVersion()) : dependencyRhs.getVersion() != null) return false;
        return true;
    }
    ...
    protected boolean isCommonContentEquals(ModuleDependency dependencyRhs) {
        ...
        if (isTransitive() != dependencyRhs.isTransitive()) return false;
        if (getArtifacts() != null ?
                !getArtifacts().equals(dependencyRhs.getArtifacts()) : dependencyRhs.getArtifacts() != null) return false;
        if (getExcludeRules() != null ?
                !getExcludeRules().equals(dependencyRhs.getExcludeRules()) : dependencyRhs.getExcludeRules() != null) return false;
        return true;
    }
