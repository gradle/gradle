...
Module dependencies are the most common dependencies. They correspond to a dependency in a repository you have declared. Gradle provides different notations for module dependencies. The most common notation for such a dependency follows the  pattern: \texttt{[group]:[artifact]:[version]}. Here is an example for a module dependency:
\begin{Verbatim}
dependencies {
...
A module dependency has an API to allow for further configuration. For example if the module descriptor (e.g. pom.xml) declares dependencies you don't want you have, you can exclude them: \begin{Verbatim}
dependencies {
...
\end{Verbatim}
Have a look at \href{\API dependencies/ModuleDependency.html}{org.gradle.api.dependencies.ModuleDependency} to learn more about the API. 

\subsubsection{Classifiers} % (fold)
\label{ssub:classifiers}
The Maven dependency management has the notion of classifiers. Gradle supports this. To use Classifiers in Maven you can type: \texttt{"org.testng:testng:5.8:jdk15"}.
% subsubsection classifiers (end)
\subsubsection{Artifact only notation} % (fold)
\label{ssub:artifact_dependencies}
An artifact only notation creates a module dependency which only downloads one artifact file. The notation for such a dependency follows the  pattern: \texttt{[group]:[artifact]:[version]@[extension]} or \texttt{[group]:[artifact]:[version]:[classifier]@[extension]}. For example:
\begin{Verbatim}
dependencies {
	compile "org.apache.ant:ant-junit:1.7.0@jar"
}
\end{Verbatim}
Gradle needs a repository somewhere where this artifact can be found, but there is \texttt{no} xml descriptor required in the repository. If there is one, it is ignored. In the simplest case such a repository is a folder on your local machine. 
...
\end{Verbatim}
This declares a dependency of your project on Groovy. Groovy itself has dependencies. But Gradle does not look for an xml descriptor to figure them out but gets the information from the build file. The dependencies of a client module can be normal module dependencies or artifact dependencies or another client module. Have also a look at \href{\API dependencies/ClientModule.html}{org.gradle.api.dependencies.ClientModule}.

In the current release \emph{Client Modules} have one limitation. For example if your project is a library and you want this library to be uploaded to your company's Maven or Ivy repository. Gradle uploads the jars of your project to the company repository together with the XML descriptor file of the dependencies. If you use \texttt{Client Modules} the dependency declaration in the XML descriptor file is not correct. We try to improve this in a future release of Gradle.
...
\end{Verbatim}
Have also a look at \href{\API dependencies/ProjectDependency.html}{org.gradle.api.dependencies.ProjectDependency}.

Multi-project builds are discussed in chapter \ref{cha:multi_project_builds}.
% subsection project_dependencies (end)

\subsection{Global Excludes} % (fold)
...
\begin{Verbatim}
dependencies {
    excludeRules.getNativeRules().add(new DefaultExcludeRule(new ArtifactId(
...
\end{Verbatim}
Have also a look at \href{\API dependencies/ExcludeRuleContainer.html}{org.gradle.api.dependencies.ExcludeRuleContainer}.

% subsection global_excludes (end)
...
You can use our ivy transforms to modify the Ivy objects created by Gradle. Look at the Javadoc \href{\API dependencies/Dependency.html}{org.gradle.api.dependencies.Dependency} to learn more.
