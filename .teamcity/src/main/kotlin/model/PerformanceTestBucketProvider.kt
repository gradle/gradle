...
private fun splitBucketsByScenarios(
    scenarios: List<PerformanceScenario>,
    testProjectToScenarioDurations: Map<String, List<PerformanceTestDuration>>,
    testProjectScenarioDurationsFallback: Map<String, List<PerformanceTestDuration>>,
    numberOfBuckets: Int,
): List<PerformanceTestBucket> {
    val testProjectDurations =
        scenarios
...
fun determineScenarioTestDurations(
    os: Os,
    performanceTestDurations: OperatingSystemToTestProjectPerformanceTestDurations,
): Map<String, List<PerformanceTestDuration>> = performanceTestDurations.getOrDefault(os, emptyMap())

fun determineScenariosFor(
...
data class PerformanceTestDuration(
...
data class TestProjectDuration(
    val testProject: String,
    val scenarioDurations: List<PerformanceTestDuration>,
) {
    ...
    fun split(expectedBucketNumber: Int): List<PerformanceTestBucket> =
        if (expectedBucketNumber == 1 || scenarioDurations.size == 1) {
            ...
            val buckets: List<List<PerformanceTestDuration>> =
                splitIntoBuckets(
                    list,
                    toIntFunction,
...
data class Scenario(
    ...
    override fun toString(): String = "$className.$scenario"
}

class SingleTestProjectBucket(
    val testProject: String,
    val scenarios: List<Scenario>,
) : PerformanceTestBucket {
    override fun createPerformanceTestsFor(
        model: CIBuildModel,
        stage: Stage,
        performanceTestCoverage: PerformanceTestCoverage,
...
class MultipleTestProjectBucket(
    private val projectDurations: List<TestProjectDuration>,
) : PerformanceTestBucket {
    override fun createPerformanceTestsFor(
        model: CIBuildModel,
        stage: Stage,
...
private fun projectDurationsToScenariosPerTestProject(projectDurations: List<TestProjectDuration>): Map<String, List<Scenario>> {
...
private fun createPerformanceTest(
    model: CIBuildModel,
    performanceTestCoverage: PerformanceTestCoverage,
    stage: Stage,
    bucketIndex: Int,
...
private fun prepareScenariosStep(
    testProject: String,
...
data class PerformanceTestGroup(
    val testProject: String,
    val performanceTestTypes: Map<PerformanceTestType, Set<Os>>,
)

data class PerformanceTestConfiguration(
    val testId: String,
    val groups: List<PerformanceTestGroup>,
)
