...
class PerformanceTest(model: CIBuildModel, type: PerformanceTestType) : BuildType({
    ...
    steps {
        ...
        gradle {
            name = "GRADLE_RUNNER"
            tasks = ""
            gradleParams = "cleanDistributed${type.taskId} distributed${type.taskId}s -x prepareSamples %performance.baselines% ${type.extraParameters} -PtimestampedVersion -Porg.gradle.performance.branchName=%teamcity.build.branch% -Porg.gradle.performance.db.url=%performance.db.url% -Porg.gradle.performance.db.username=%performance.db.username% -PteamCityUsername=%TC_USERNAME% -PteamCityPassword=%teamcity.password.restbot% -Porg.gradle.performance.buildTypeId=${PerformanceSupportProject(model).extId} -Porg.gradle.performance.workerTestTaskName=fullPerformanceTest -Porg.gradle.performance.coordinatorBuildId=%teamcity.build.id% -Porg.gradle.performance.db.password=%performance.db.password.tcagent% " + gradleParameters.joinToString(separator = " ")
