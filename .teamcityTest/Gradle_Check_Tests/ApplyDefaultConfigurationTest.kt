...
class ApplyDefaultConfigurationTest {
    ...
    fun expectedRunnerParam(daemon: String = "--daemon", extraParameters: String = "") =
        "-Dorg.gradle.workers.max=%maxParallelForks% -PmaxParallelForks=%maxParallelForks% -s --no-daemon --continue $extraParameters -PteamCityBuildId=%teamcity.build.id% \"-Dscan.tag.Check\" \"-Dscan.tag.\""
}
