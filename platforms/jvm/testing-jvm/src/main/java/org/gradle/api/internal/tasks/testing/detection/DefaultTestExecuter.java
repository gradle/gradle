...
public class DefaultTestExecuter implements TestExecuter<JvmTestExecutionSpec> {
    ...
    public void execute(final JvmTestExecutionSpec testExecutionSpec, TestResultProcessor testResultProcessor) {
        ...
        // This is safe because the frameworks that don't accept all TestDefinitions will have the dir selection
        // filtered out earlier
        // If a TestFramework begins to reject ClassTestDefinitions, this needs rethinking.
        final WorkerTestDefinitionProcessorFactory<TestDefinition> testInstanceFactory =
                Cast.uncheckedNonnullCast(testFramework.getProcessorFactory());

        ForkedTestClasspath classpath =
                testClasspathFactory.create(testExecutionSpec.getClasspath(), testExecutionSpec.getModulePath());

        final Factory<TestDefinitionProcessor<TestDefinition>> forkingProcessorFactory =
                () -> new ForkingTestDefinitionProcessor<>(
                        workerLeaseService,
                        workerFactory,
                        testInstanceFactory,
        ...
        final Factory<TestDefinitionProcessor<TestDefinition>> reforkingProcessorFactory = () ->
                new RestartEveryNTestDefinitionProcessor<>(forkingProcessorFactory, testExecutionSpec.getForkEvery());
        processor = new PatternMatchTestDefinitionProcessor<>(
                testFilter,
                new RunPreviousFailedFirstTestDefinitionProcessor<>(
                        testExecutionSpec.getPreviousFailedTestClasses(),
                        Collections.emptySet(),
                        new MaxNParallelTestDefinitionProcessor<>(
                                getMaxParallelForks(testExecutionSpec), reforkingProcessorFactory, actorFactory)));

        final FileTree testClassFiles = testExecutionSpec.isScanForTestClasses()
                ? testExecutionSpec.getCandidateClassFiles()
                : FileCollectionFactory.emptyTree();
        ...
        if (testFramework.getDetector() != null) {
            ...
            testFrameworkDetector.setTestClasses(
                    new ArrayList<>(testExecutionSpec.getTestClassesDirs().getFiles()));
            testFrameworkDetector.setTestClasspath(classpath.getApplicationClasspath());
        }

        TestDetector detector =
                new DefaultTestScanner(testClassFiles, testDefinitionDirs, testFramework.getDetector(), processor);

        // What is this?
        // In some versions of the Gradle retry plugin, it would retry any test that had any kind of failure associated
        // with it.
        // We attempt to capture assumption violations as failures for skipped tests.
        //
        // This would cause any test that had been skipped to be executed multiple times. This could sometimes cause
        // real failures.
        ...
        if (testResultProcessor != null) {
            ...
            if (canonicalName != null
                    && canonicalName.endsWith("org.gradle.testretry.internal.executer.RetryTestResultProcessor")) {
