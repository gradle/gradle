...
public abstract class Test extends AbstractTestTask implements JavaForkOptions, PatternFilterable {
    ...
    protected JvmTestExecutionSpec createTestExecutionSpec() {
        ...
        return new JvmTestExecutionSpec(getTestFramework(), classpath, modulePath,
            getCandidateClassFiles(), isScanForTestClasses(), candidateResourceFiles().getAsFileTree(), getScanForTestResources().get(),
     ...
     */
    @Incubating
    public ConfigurableFileCollection candidateResourceFiles() { // TODO: also adjust naming to get (avoided for now to avoid property must be abstract check)
