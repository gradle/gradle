...
trait TestFrameworkStartupTestFixture {
    void assertTestWorkerFailedToStart(String taskName = ":test") {
        failure.assertHasDescription("Execution failed for task '$taskName' (created by plugin 'org.gradle.jvm-test-suite').")
    ...
    void assertTestWorkerStartedAndTestFrameworkFailedToStart(String taskName = ":test", int expectedWorkerFailures = 1) {
        failure.assertHasFailure("Execution failed for task '$taskName' (created by plugin 'org.gradle.jvm-test-suite').") {
