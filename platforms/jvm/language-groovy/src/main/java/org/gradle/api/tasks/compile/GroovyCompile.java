...
public abstract class GroovyCompile extends AbstractCompile implements HasCompileOptions {
    ...
    private GroovyJavaJointCompileSpec createSpec() {
        ...
        getOptions().getForkOptions().setExecutable(executable);
        ...
        if (getGroovyOptions().getStubDir() == null) {
            File dir = new File(getTemporaryDir(), "groovy-java-stubs");
            GFileUtils.mkdirs(dir);
            getGroovyOptions().setStubDir(dir);
        }
        spec.setGroovyCompileOptions(MinimalGroovyCompileOptionsConverter.toMinimalGroovyCompileOptions(getGroovyOptions()));
