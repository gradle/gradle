...
class DefaultJavaInstallationRegistryTest extends Specification {
    ...
    def "normalize installations to account for macOS folder layout"() {
        ...
        binDir.createFile(OperatingSystemFileResolver.of(OperatingSystem.MAC_OS).getExecutableName("java"))
    ...
    def "skip path normalization on non-osx systems"() {
        ...
        binDir.createFile(OperatingSystemFileResolver.of(OperatingSystem.LINUX).getExecutableName("java"))

        // Make it look like a macOS installation
        def macOsJdkHomeBinDir = jdkHome.createDir("Contents/Home/bin")
        macOsJdkHomeBinDir.createFile(OperatingSystemFileResolver.of(OperatingSystem.LINUX).getExecutableName("java"))
    ...
    private TestFile createJdkInstallation(String version) {
        ...
        def binDir = jdkHome.createDir("bin")
        binDir.createFile(OperatingSystemFileResolver.current().getExecutableName("java"))
        return jdkHome
    }
    ...
    private TestFile createJreInstallation(String version) {
        ...
        jdkHomeBinDir.createFile(OperatingSystemFileResolver.current().getExecutableName("java"))
        return jdkHome
    }
