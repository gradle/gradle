...
public abstract class ScalaBasePlugin implements Plugin<Project> {
    ...
    private void configureConfigurations(final ProjectInternal project, Category incrementalAnalysisCategory, final Usage incrementalAnalysisUsage, ScalaPluginExtension scalaPluginExtension) {
        ...
        project.getConfigurations().resolvableDependencyScope(SCALA_COMPILER_PLUGINS_CONFIGURATION_NAME, plugins -> {
            plugins.setTransitive(false);
            jvmPluginServices.configureAsRuntimeClasspath(plugins);
        });

        project.getConfigurations().resolvableDependencyScope(ZINC_CONFIGURATION_NAME, zinc -> {
    ...
    private void configureSourceSetDefaults(final ProjectInternal project, Category incrementalAnalysisCategory, final Usage incrementalAnalysisUsage, ScalaPluginExtension scalaPluginExtension) {
        javaPluginExtension(project).getSourceSets().all(sourceSet -> {
            ...
            createScalaCompileTask(project, sourceSet, scalaSource, incrementalAnalysisCategory, incrementalAnalysisUsage);
        });
    }
    ...
    private static NamedDomainObjectProvider<ResolvableConfiguration> createIncrementalAnalysisConfigurationFor(ConfigurationContainer configurations, Category incrementalAnalysisCategory, Usage incrementalAnalysisUsage, SourceSet sourceSet) {
        return configurations.resolvable("incrementalScalaAnalysisFor" + sourceSet.getName(), incrementalAnalysis -> {
            incrementalAnalysis.setDescription("Incremental compilation analysis files for " + ((DefaultSourceSet) sourceSet).getDisplayName());
            incrementalAnalysis.extendsFrom(configurations.getByName(sourceSet.getImplementationConfigurationName()));
    ...
    private void createScalaCompileTask(Project project, SourceSet sourceSet, ScalaSourceDirectorySet scalaSource, Category incrementalAnalysisCategory, Usage incrementalAnalysisUsage) {
        NamedDomainObjectProvider<ResolvableConfiguration> incrementalAnalysis = createIncrementalAnalysisConfigurationFor(project.getConfigurations(), incrementalAnalysisCategory, incrementalAnalysisUsage, sourceSet);
    ...
    private void configureIncrementalAnalysis(Project project, SourceSet sourceSet, NamedDomainObjectProvider<ResolvableConfiguration> incrementalAnalysis, ScalaCompile scalaCompile) {
        ...
        scalaCompile.getAnalysisFiles().from(
            incrementalAnalysis.map(conf ->
                conf.getIncoming().artifactView(viewConfiguration -> {
