...
public abstract class ScalaBasePlugin implements Plugin<Project> {
    ...
    public ScalaBasePlugin(
            ObjectFactory objectFactory, JvmPluginServices jvmPluginServices, DependencyFactory dependencyFactory) {
    ...
    public void apply(final Project project) {
        ...
        Provider<ResolvableConfiguration> toolchainClasspath =
                createToolchainRuntimeClasspath(project, scalaPluginExtension);
        ...
        configureConfigurations(
                (ProjectInternal) project, incrementalAnalysisCategory, incrementalAnalysisUsage, scalaPluginExtension);

        configureCompileDefaults(
                project,
                scalaRuntime,
                (DefaultJavaPluginExtension) javaPluginExtension(project),
    ...
    private void configureConfigurations(
            ...
            Category incrementalAnalysisCategory,
            final Usage incrementalAnalysisUsage,
            ScalaPluginExtension scalaPluginExtension) {
        DependencyHandler dependencyHandler = project.getDependencies();

        project.getConfigurations()
                .resolvableDependencyScopeLocked(SCALA_COMPILER_PLUGINS_CONFIGURATION_NAME, plugins -> {
        ...
        project.getConfigurations().resolvableDependencyScopeLocked(ZINC_CONFIGURATION_NAME, zinc -> {
            ...
            zinc.getResolutionStrategy().eachDependency(rule -> {
                if (rule.getRequested().getGroup().equals("com.typesafe.zinc")
                        && rule.getRequested().getName().equals("zinc")) {
            ...
            zinc.defaultDependencies(dependencies -> {
                dependencies.add(dependencyHandler.create("org.scala-sbt:zinc_" + DEFAULT_SCALA_ZINC_VERSION + ":"
                        + scalaPluginExtension.getZincVersion().get()));
                ...
                zinc.getIncoming().afterResolve(resolvableDependencies -> {
                    resolvableDependencies.getResolutionResult().allComponents(component -> {
                        if (component.getModuleVersion() != null
                                && component.getModuleVersion().getName().equals("scala-library")) {
                            if (!component.getModuleVersion().getVersion().startsWith(DEFAULT_SCALA_ZINC_VERSION)) {
                                throw new InvalidUserCodeException(
                                        "The version of 'scala-library' was changed while using the default Zinc version. "
                                                ...
                                                + " is not compatible with org.scala-sbt:zinc_"
                                                + DEFAULT_SCALA_ZINC_VERSION + ":" + DEFAULT_ZINC_VERSION);
                            }
                        }
                    });
                });
        ...
        AttributeMatchingStrategy<Usage> matchingStrategy =
                dependencyHandler.getAttributesSchema().attribute(USAGE_ATTRIBUTE);
    ...
    private Provider<ResolvableConfiguration> createToolchainRuntimeClasspath(
            Project project, ScalaPluginExtension scalaPluginExtension) {
        Provider<DependencyScopeConfiguration> scalaToolchain = project.getConfigurations()
                .dependencyScope("scalaToolchain", conf -> {
    ...
    private void configureSourceSetDefaults(
        ...
        javaPluginExtension(project).getSourceSets().all(sourceSet -> {
            ...
            sourceSet.getResources().getFilter().exclude(spec(element -> scalaSourceFiles.contains(element.getFile())));
            ...
            FileCollection incrementalAnalysis = createIncrementalAnalysisConfigurationFor(
                    project.getConfigurations(), incrementalAnalysisCategory, incrementalAnalysisUsage, sourceSet);
    ...
    private Provider<Dependency> createScalaBridgeDependency(ScalaPluginExtension scalaPluginExtension) {
        return scalaPluginExtension
                .getScalaVersion()
                .zip(scalaPluginExtension.getZincVersion(), (scalaVersion, zincVersion) -> {
                    if (ScalaRuntimeHelper.isScala3(scalaVersion)) {
    ...
    private ScalaSourceDirectorySet createScalaSourceDirectorySet(SourceSet sourceSet) {
        ...
        ScalaSourceDirectorySet scalaSourceDirectorySet = objectFactory.newInstance(
                DefaultScalaSourceDirectorySet.class, objectFactory.sourceDirectorySet("scala", displayName));
        scalaSourceDirectorySet.getFilter().include("**/*.java", "**/*.scala");
        return scalaSourceDirectorySet;
    }

    private static FileCollection createIncrementalAnalysisConfigurationFor(
            RoleBasedConfigurationContainerInternal configurations,
            Category incrementalAnalysisCategory,
            Usage incrementalAnalysisUsage,
            SourceSet sourceSet) {
    ...
    private void createScalaCompileTask(
            final Project project,
            final SourceSet sourceSet,
            ScalaSourceDirectorySet scalaSource,
            final FileCollection incrementalAnalysis) {
        final TaskProvider<ScalaCompile> compileTask = project.getTasks()
                .register(sourceSet.getCompileTaskName("scala"), ScalaCompile.class, scalaCompile -> {
                    ...
                    JvmPluginsHelper.configureAnnotationProcessorPath(
                            sourceSet, scalaSource, scalaCompile.getOptions(), project);
        ...
        JvmPluginsHelper.configureOutputDirectoryForSourceSet(
                sourceSet, scalaSource, project, compileTask, compileTask.map(AbstractScalaCompile::getOptions));

        project.getTasks().named(sourceSet.getClassesTaskName(), task -> task.dependsOn(compileTask));
    }

    private void configureIncrementalAnalysis(
            Project project, SourceSet sourceSet, FileCollection incrementalAnalysis, ScalaCompile scalaCompile) {
        scalaCompile
                .getAnalysisMappingFile()
                .set(project.getLayout()
                        .getBuildDirectory()
                        .file("tmp/scala/compilerAnalysis/" + scalaCompile.getName() + ".mapping"));

        // cannot compute at task execution time because we need association with source set
        IncrementalCompileOptions incrementalOptions =
                scalaCompile.getScalaCompileOptions().getIncrementalOptions();
        incrementalOptions
                .getAnalysisFile()
                .set(project.getLayout()
                        .getBuildDirectory()
                        .file("tmp/scala/compilerAnalysis/" + scalaCompile.getName() + ".analysis"));

        incrementalOptions
                .getClassfileBackupDir()
                .set(project.getLayout()
                        .getBuildDirectory()
                        .file("tmp/scala/classfileBackup/" + scalaCompile.getName() + ".bak"));
    ...
    private static void configureCompileDefaults(
        ...
        project.getTasks().withType(ScalaCompile.class).configureEach(compile -> {
            ...
            conventionMapping.map("scalaClasspath", (Callable<FileCollection>) () -> getScalaToolchainClasspath(
                    scalaPluginExtension, scalaToolchainRuntimeClasspath, scalaRuntime, compile.getClasspath()));
            conventionMapping.map("zincClasspath", (Callable<Configuration>)
                    () -> project.getConfigurations().getAt(ZINC_CONFIGURATION_NAME));
            conventionMapping.map("scalaCompilerPlugins", (Callable<FileCollection>)
                    () -> project.getConfigurations().getAt(SCALA_COMPILER_PLUGINS_CONFIGURATION_NAME));
            conventionMapping.map(
                    "sourceCompatibility", () -> computeJavaSourceCompatibilityConvention(javaExtension, compile)
                            .toString());
            conventionMapping.map(
                    "targetCompatibility", () -> computeJavaTargetCompatibilityConvention(javaExtension, compile)
                            .toString());
            compile.getScalaCompileOptions().getKeepAliveMode().convention(KeepAliveMode.SESSION);
        });
    }

    private static JavaVersion computeJavaSourceCompatibilityConvention(
            DefaultJavaPluginExtension javaExtension, ScalaCompile compileTask) {
        ...
        return JavaVersion.toVersion(compileTask
                .getJavaLauncher()
                .get()
                .getMetadata()
                .getLanguageVersion()
    ...
    private static JavaVersion computeJavaTargetCompatibilityConvention(
            DefaultJavaPluginExtension javaExtension, ScalaCompile compileTask) {
    ...
    private static void configureScaladoc(
        ...
        project.getTasks().withType(ScalaDoc.class).configureEach(scalaDoc -> {
            scalaDoc.getConventionMapping()
                    .map("scalaClasspath", (Callable<FileCollection>) () -> getScalaToolchainClasspath(
                            scalaPluginExtension,
                            scalaToolchainRuntimeClasspath,
                            scalaRuntime,
                            scalaDoc.getClasspath()));
            scalaDoc.getConventionMapping().map("destinationDir", (Callable<File>) () -> javaPluginExtension(project)
                    .getDocsDir()
                    .dir("scaladoc")
                    .get()
                    .getAsFile());
            scalaDoc.getConventionMapping()
                    .map("title", (Callable<String>) () -> ReportUtilities.getApiDocTitleFor(project));
