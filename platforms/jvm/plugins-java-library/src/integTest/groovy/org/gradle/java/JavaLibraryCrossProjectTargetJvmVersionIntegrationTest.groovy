...
class JavaLibraryCrossProjectTargetJvmVersionIntegrationTest extends AbstractIntegrationSpec {
  ...
  - Variant 'apiElements' declares the API compile view of a library for use during compile-time, packaged as a jar, and its dependencies declared externally:
  ...
  - Variant 'implementationCompileElements' declares the implementation compile view of a library for use during compile-time, packaged as a jar, and its dependencies declared externally:
      - Incompatible because this component declares a component, compatible with Java 7 and the consumer needed a component, compatible with Java 6
      - Other compatible attribute:
          - Doesn't say anything about its target Java environment (preferred optimized for standard JVMs)
  ...
  - Variant 'testResultsElementsForTest':
    ...
    def "can select the most appropriate producer variant (#expected) based on target compatibility (#requested)"() {
        file('producer/build.gradle') << """
            ...
            configurations.implementationCompileElements.attributes.attribute(TargetJvmVersion.TARGET_JVM_VERSION_ATTRIBUTE, 1000)
