...
class JavaLibraryPublishedTargetJvmVersionIntegrationTest extends AbstractHttpDependencyResolutionTest {
    ...
    def "can fail resolution if producer doesn't have appropriate target version"() {
        ...
        then:
        failure.assertHasCause('''No matching variant of org:producer:1.0 was found. The consumer was configured to find the API compile view of a library for use during compile-time, compatible with Java 5, preferably in the form of class files, preferably optimized for standard JVMs, and its dependencies declared externally but:
  - Variant 'apiElementsJdk6' declares a library for use during compile-time, packaged as a jar, and its dependencies declared externally:
      ...
      - Other compatible attributes:
          - Doesn't say anything about its compile view (required the API compile view)
