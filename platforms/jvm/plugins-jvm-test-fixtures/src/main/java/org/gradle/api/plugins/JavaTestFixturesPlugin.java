...
public abstract class JavaTestFixturesPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        JvmFeatureInternal feature = new DefaultJvmFeature(
                ...
                Collections.singleton(
                        new ProjectDerivedCapability((ProjectInternal) project, TEST_FIXTURES_FEATURE_NAME)),
        ...
        project.getPluginManager().withPlugin("java", plugin -> {
            DefaultJvmSoftwareComponent component =
                    (DefaultJvmSoftwareComponent) JavaPluginHelper.getJavaComponent(project);
            component.getFeatures().add(feature);

            component.addVariantsFromConfiguration(
                    feature.getApiElementsConfiguration(),
                    new JavaConfigurationVariantMapping("compile", true, feature.getCompileClasspathConfiguration()));
            component.addVariantsFromConfiguration(
                    feature.getRuntimeElementsConfiguration(),
                    new JavaConfigurationVariantMapping("runtime", true, feature.getRuntimeClasspathConfiguration()));
    ...
    private void createImplicitTestFixturesDependencies(JvmFeatureInternal feature, Project project) {
        ...
        testSourceSet.setCompileClasspath(project.getObjects()
                .fileCollection()
                .from(configurations.getByName(testSourceSet.getCompileClasspathConfigurationName())));
        testSourceSet.setRuntimeClasspath(project.getObjects()
                .fileCollection()
                .from(
