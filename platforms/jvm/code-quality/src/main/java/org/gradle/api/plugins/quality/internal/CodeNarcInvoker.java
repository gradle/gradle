...
class CodeNarcInvoker implements Action<AntBuilderDelegate> {
        ...
        try {
                    ...
                    () -> {
                        reports.forEach(r -> {
                            ...
                            if (r.getName().get().equals("console")) {
                                ...
                                // Prefer to use the IDE based formatter because this produces a useful/clickable link
                                // to the violation on the console
                                ant.invokeMethod(
                                        "report",
                                        ImmutableMap.of("type", "ide"),
                                        () -> ant.invokeMethod(
                                                "option",
                                                ImmutableMap.of("name", "writeToStandardOut", "value", true)));
                            } else if (r.getName().get().equals("html")) {
                                ant.invokeMethod(
                                        "report",
                                        ImmutableMap.of("type", "sortable"),
                                        () -> ant.invokeMethod(
                                                "option",
                                                ImmutableMap.of(
                                                        "name",
                                ...
                                ant.invokeMethod(
                                        "report",
                                        ImmutableMap.of("type", r.getName().get()),
                                        () -> ant.invokeMethod(
                                                "option",
                                                ImmutableMap.of(
                                                        "name",
            ...
            if (e.getMessage().matches("Exceeded maximum number of priority \\d* violations.*")) {
                ...
                if (!reportsWithFiles.isEmpty()) {
                    CodeNarcActionParameters.EnabledReport humanReadableReport = reportsWithFiles.stream()
                            .filter(it -> it.getName().get().equals("html"))
                            .findFirst()
                            .orElse(null);
                    if (humanReadableReport == null) {
                        humanReadableReport = reportsWithFiles.stream()
                                .filter(it -> it.getName().get().equals("text"))
                                .findFirst()
                                .orElse(null);
                    }
                    if (humanReadableReport == null) {
                        humanReadableReport = reportsWithFiles.stream()
                                .filter(it -> it.getName().get().equals("xml"))
                                .findFirst()
                                .orElse(null);
                    }
                    ...
                    if (humanReadableReport != null) {
                        String reportUrl = new ConsoleRenderer()
                                .asClickableFileUrl(humanReadableReport
                                        .getOutputLocation()
                                        .getAsFile()
                                        .get());
            ...
            if (e.getMessage().contains("codenarc doesn't support the nested \"classpath\" element.")) {
                String message =
                        "The compilationClasspath property of CodeNarc task can only be non-empty when using CodeNarc 0.27.0 or newer.";
    ...
    static void setLifecycleLogLevel(AntBuilderDelegate ant, @Nullable String lifecycleLogLevel) {
        try {
            ...
            List<Object> buildListeners = (List<Object>)
                    project.getClass().getMethod("getBuildListeners").invoke(project);
            for (Object it : buildListeners) {
                // We cannot use instanceof or getClass().equals(AntLoggingAdapter.class) since they're in different
                // class loaders
                if (it.getClass().getName().equals(AntLoggingAdapter.class.getName())) {
                    it.getClass()
                            .getMethod("setLifecycleLogLevel", String.class)
                            .invoke(it, lifecycleLogLevel);
                }
            }
