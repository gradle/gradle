...
public class ProcessIsolatedCompilerWorkerExecutor extends AbstractIsolatedCompilerWorkerExecutor {
    public ProcessIsolatedCompilerWorkerExecutor(WorkerDaemonFactory delegate, ActionExecutionSpecFactory actionExecutionSpecFactory, BuildLayout buildLayout, ProjectCacheDir projectCacheDir) {
        super(delegate, actionExecutionSpecFactory, buildLayout, projectCacheDir);
    }

    @Override
    public IsolatedClassLoaderWorkerRequirement getIsolatedWorkerRequirement(DaemonForkOptions daemonForkOptions) {
        return new ForkedWorkerRequirement(getBuildLayout().getRootDirectory(), daemonForkOptions.getJavaForkOptions().getWorkingDir(), getProjectCacheDir().getDir(), daemonForkOptions);
    }
}
