...
class JavaCompilerArgumentsBuilderTest extends Specification {
    ...
    def setup() {
        ...
        def options = new MinimalJavaCompileOptions()
        options.debug = true
        options.warnings = true
        options.compilerArgs = []
        options.debugOptions = new MinimalCompilerDaemonDebugOptions(null)
    ...
    def "does not include processor options when source compatibility is Java 5 or lower"() {
        ...
        expect:
        builder.build() == ["-source", "1.5"] + (defaultOptions.findAll { it != "-proc:none" })
    }
    ...
    def "generates -g option"() {
        ...
        when:
        spec.compileOptions.debugOptions = new MinimalCompilerDaemonDebugOptions("source,vars")
    ...
    def "generates -bootclasspath option"() {
        spec.compileOptions.bootClasspath = TestFiles.fixed(new File("lib1.jar"), new File("lib2.jar")).getAsPath()
    ...
    def "can include/exclude launcher options"() {
        spec.compileOptions.forkOptions = new MinimalJavaCompilerDaemonForkOptions("64m", "1g", [], null, null, null)
