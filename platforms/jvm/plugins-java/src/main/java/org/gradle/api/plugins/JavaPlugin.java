...
public abstract class JavaPlugin implements Plugin<Project> {
    ...
    public void apply(final Project project) {
        ...
        DeprecationLogger.whileDisabled(() -> {
            project.getConfigurations().getByName(Dependency.ARCHIVES_CONFIGURATION).getArtifacts().addLater(project.provider(() ->
                javaComponent.getMainFeature().getRuntimeElementsConfiguration().get().getArtifacts().iterator().next())
            );
        });
