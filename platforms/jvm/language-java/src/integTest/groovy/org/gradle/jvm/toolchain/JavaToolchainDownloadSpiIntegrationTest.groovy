    ...
    def "downloaded JDK is checked against the spec"() {
        ...
        failure.assertHasDescription("Could not determine the dependencies of task ':compileJava'.")
               ...
               .assertHasCause("No matching toolchain could be found in the locally installed toolchains or the configured toolchain download repositories. " +
                   ...
                   "from '$uri', due to: Toolchain provisioned from '$uri' doesn't satisfy the specification: {languageVersion=11, vendor=any, implementation=vendor-specific} and must have the executable 'javac' and the executable 'javadoc'.).")
    }
