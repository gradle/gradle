...
class ExecIntegrationTest extends AbstractIntegrationSpec {
    ...
    @Issue("https://github.com/gradle/gradle/issues/30790")
    def "#task task can replace all inherited environment variables"() {
        given:
        def shellScript = ShellScript.builder()
            .printEnvironmentVariableIfPresent("TEST_VAR0")
            .printEnvironmentVariableIfPresent("TEST_VAR1")
    ...
    def "#task task can remove inherited environment variables"() {
        ...
        when:
        executer.withEnvironmentVars(TEST_VAR0: "FOO", TEST_VAR1: "BAR")
        succeeds("run$task")

        then:
        outputContains("TEST_VAR0=FOO")
        outputDoesNotContain("TEST_VAR1=")
        outputDoesNotContain("TEST_VAR2=")

        when:
        executer.withEnvironmentVars(TEST_VAR0: "BAZ", TEST_VAR2: "BAR")
        succeeds("run$task")
