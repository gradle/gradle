...
public abstract class ValidatePlugins extends DefaultTask {
    ...
    public void validateTaskClasses() throws IOException {
        ...
        List<? extends ProblemsInternal> problems = ValidationProblemSerialization.parseMessageList(new String(readAllBytes(getOutputFile().get().getAsFile().toPath()), UTF_8));
    ...
    private void reportProblems(List<? extends Problem> problems) {
        ProblemReporterInternal reporter = getServices().get(ProblemsInternal.class).getInternalReporter();
