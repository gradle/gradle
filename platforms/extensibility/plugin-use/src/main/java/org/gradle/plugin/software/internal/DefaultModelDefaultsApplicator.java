...
public class DefaultModelDefaultsApplicator implements ModelDefaultsApplicator {
    ...
    @Override
    public <T> void applyDefaultsTo(T target, ClassLoaderScope classLoaderScope, Plugin<?> plugin, SoftwareFeatureImplementation<?> softwareFeatureImplementation) {
        defaultsHandlers.forEach(handler -> handler.apply(target, classLoaderScope, softwareFeatureImplementation.getFeatureName(), plugin));
    }
}
