...
public class DefaultPluginHandler implements PluginHandler {
    ...
    public PluginRequests getAutoAppliedPlugins(PluginRequests initialRequests, Object pluginTarget) {
        if (pluginTarget instanceof Project) {
            ...
            return filterAlreadyAppliedOrRequested(
                    autoAppliedPlugins, initialRequests, project.getPlugins(), project.getBuildscript());
            ...
            return filterAlreadyAppliedOrRequested(
                    autoAppliedPlugins, initialRequests, settings.getPlugins(), settings.getBuildscript());
        } else {
    ...
    private static PluginRequests filterAlreadyAppliedOrRequested(
            PluginRequests autoAppliedPlugins,
            final PluginRequests initialRequests,
        ...
        return PluginRequests.of(ImmutableList.copyOf(StreamSupport.stream(autoAppliedPlugins.spliterator(), false)
                .filter(autoAppliedPlugin -> !isAlreadyAppliedOrRequested(
                        PluginCoordinates.from(autoAppliedPlugin), initialRequests, pluginContainer, scriptHandler))
                .filter(autoAppliedPlugin -> autoAppliedPlugin
    ...
    private static boolean isAlreadyAppliedOrRequested(
            PluginCoordinates autoAppliedPlugin,
            PluginRequests requests,
            PluginContainer pluginContainer,
            ScriptHandler scriptHandler) {
        return isAlreadyApplied(autoAppliedPlugin, pluginContainer)
                || isAlreadyRequestedInPluginsBlock(autoAppliedPlugin, requests)
                || isAlreadyRequestedInBuildScriptBlock(autoAppliedPlugin, scriptHandler);
    }
    ...
    private static boolean isAlreadyRequestedInPluginsBlock(
            PluginCoordinates autoAppliedPlugin, PluginRequests requests) {
    ...
    private static boolean isAlreadyRequestedInBuildScriptBlock(
            PluginCoordinates autoAppliedPlugin, ScriptHandler scriptHandler) {
        ...
        if (selector instanceof ModuleComponentSelector) {
            ...
            predicate = predicate.or(
                    dependency -> hasMatchingCoordinates(dependency, moduleSelector.getModuleIdentifier()));
        }
    ...
    private static boolean hasMatchingCoordinates(Dependency dependency, ModuleIdentifier module) {
        return module.getGroup().equals(dependency.getGroup())
                && module.getName().equals(dependency.getName());
    }
}
