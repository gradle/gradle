...
class GradleBuildAction implements BuildAction<GradleBuildModel>, Serializable {
    ...
    @Override
    public GradleBuildModel execute(BuildController controller) {
        if (resilient)  {
            FetchModelResult<GradleBuild> result = controller.fetch(GradleBuild.class);
            return new GradleBuildModel(result.getModel(), result.getFailures());
        } else {
            GradleBuild model = controller.getModel(GradleBuild.class);
