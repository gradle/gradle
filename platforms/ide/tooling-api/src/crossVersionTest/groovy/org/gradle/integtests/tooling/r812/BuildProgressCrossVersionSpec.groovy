...
class BuildProgressCrossVersionSpec extends ToolingApiSpecification implements WithOldConfigurationsSupport {
    ...
    def "receive build progress events when requesting a model"() {
        ...
        withConnection {
            ProjectConnection connection ->
                connection.model(BuildInvocations).forTasks('assemble').addProgressListener(events, EnumSet.of(OperationType.GENERIC, OperationType.ROOT)).get()
        }
    ...
    def "receive build progress events when launching a build"() {
        ...
        withConnection {
            ProjectConnection connection ->
                connection.newBuild().forTasks('assemble').addProgressListener(events, EnumSet.of(OperationType.GENERIC, OperationType.ROOT)).run()
        }
    ...
    def "receive build progress events for successful operations"() {
        ...
        withConnection {
            ProjectConnection connection ->
                connection.newBuild().forTasks('classes').addProgressListener(events, EnumSet.of(OperationType.GENERIC, OperationType.ROOT)).run()
        }
    ...
    def "receive build progress events for failed operations"() {
        ...
        withConnection {
            ProjectConnection connection ->
                connection.newBuild().forTasks('test').addProgressListener(events, EnumSet.of(OperationType.GENERIC, OperationType.ROOT)).run()
        }
