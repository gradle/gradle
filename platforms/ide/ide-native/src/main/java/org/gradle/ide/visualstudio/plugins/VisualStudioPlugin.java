...
public abstract class VisualStudioPlugin extends IdePlugin {
    ...
    protected void onApply(final Project target) {
        ...
        if (isRoot()) {
            extension = (VisualStudioExtensionInternal) project.getExtensions()
                    .create(
                            VisualStudioRootExtension.class,
                            "visualStudio",
                            DefaultVisualStudioRootExtension.class,
                            project.getName(),
                            instantiator,
                            target.getObjects(),
                            fileResolver,
            ...
            extension = (VisualStudioExtensionInternal) project.getExtensions()
                    .create(
                            VisualStudioExtension.class,
                            "visualStudio",
                            DefaultVisualStudioExtension.class,
                            instantiator,
                            fileResolver,
                            artifactRegistry,
                            collectionCallbackActionDecorator,
    ...
    private void applyVisualStudioCurrentModelRules(final VisualStudioExtensionInternal extension) {
        project.getComponents().withType(CppApplication.class).all(cppApplication -> {
            DefaultVisualStudioProject vsProject =
                    extension.getProjectRegistry().createProject(project.getName(), cppApplication.getName());
            ...
            cppApplication.getBinaries().whenElementFinalized(CppExecutable.class, executable -> {
                extension
                        .getProjectRegistry()
                        .addProjectConfiguration(new CppApplicationVisualStudioTargetBinary(
                                project.getName(), project.getPath(), cppApplication, executable, project.getLayout()));
            });
        });
        project.afterEvaluate(proj -> {
            project.getComponents().withType(CppLibrary.class).all(cppLibrary -> {
                for (Linkage linkage : cppLibrary.getLinkage().get()) {
                    ...
                    DefaultVisualStudioProject vsProject = extension
                            .getProjectRegistry()
                            .createProject(project.getName() + projectType.getSuffix(), cppLibrary.getName());
                ...
                cppLibrary.getBinaries().whenElementFinalized(CppSharedLibrary.class, library -> {
                    extension
                            .getProjectRegistry()
                            .addProjectConfiguration(new CppSharedLibraryVisualStudioTargetBinary(
                                    project.getName(), project.getPath(), cppLibrary, library, project.getLayout()));
                });
                cppLibrary.getBinaries().whenElementFinalized(CppStaticLibrary.class, library -> {
                    extension
                            .getProjectRegistry()
                            .addProjectConfiguration(new CppStaticLibraryVisualStudioTargetBinary(
                                    project.getName(), project.getPath(), cppLibrary, library, project.getLayout()));
                });
            });
        });
    ...
    private void applyVisualStudioSoftwareModelRules() {
        ...
        project.getPlugins().withType(ComponentModelBasePlugin.class).all(it -> project.getPluginManager()
                .apply(VisualStudioPluginProjectRules.class));
    }
    ...
    private Task createSolutionTask(VisualStudioSolution solution) {
        return project.getTasks()
                .create(solution.getName() + "VisualStudioSolution", GenerateSolutionFileTask.class, solution);
    }

    @SuppressWarnings("deprecation")
    private Task createProjectsFileTask(VisualStudioProject vsProject) {
        GenerateProjectFileTask task = project.getTasks()
                .create(vsProject.getName() + "VisualStudioProject", GenerateProjectFileTask.class, vsProject);
    ...
    private Task createFiltersFileTask(VisualStudioProject vsProject) {
        return project.getTasks()
                .create(vsProject.getName() + "VisualStudioFilters", GenerateFiltersFileTask.class, vsProject);
    }
}
