...
class PredefinedProblemGroupsIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "can report a problem into a sub-group of the closed Gradle root group"() {
        given:
        withReportProblemTask """
            problems.reporter.report(problems.groups.gradle.deprecation.problemId("Deprecated feature used")) {}
        """
    ...
    def "can report a problem into user-defined sub-groups"() {
        ...
        withReportProblemTask """
            ...
            problems.reporter.report(kmp.group("JavaScript").problemId("Bundle failed")) {}
            problems.reporter.report(kmp.problemId("Compilation failed")) {}
        """
    ...
    def "can report a problem into Undefined groups"() {
        given:
        withReportProblemTask """
            problems.reporter.report(problems.groups.others.undefined.problemId("Something odd")) {}
            problems.reporter.report(problems.groups.compilation.undefined.problemId("Unknown compiler")) {}
            problems.reporter.report(problems.groups.compilation.java.undefined.problemId("Unknown compiler")) {}
        """
    ...
    def "looking up a predefined sub-group by name returns the predefined group"() {
        ...
        withReportProblemTask """
            ...
            problems.reporter.report(java.problemId("Unused import")) {}
        """
    ...
    def "creating a group whose name differs only by case from a predefined group creates a distinct user group"() {
        given:
        withReportProblemTask """
            problems.reporter.report(problems.groups.compilation.group("java").problemId("Unused import")) {}
        """
        ...
        buildKotlinFile """
            ...
            abstract class ProblemReportingTask @Inject constructor(private val problems: Problems) : DefaultTask() {
                ...
                fun run() {
                    ...
                    problems.reporter.report(problems.groups.transformation.group("KMP").problemId("Compilation failed")) {}
                }
            }
