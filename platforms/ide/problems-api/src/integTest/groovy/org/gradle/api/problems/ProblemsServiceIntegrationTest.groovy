...
class ProblemsServiceIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "can emit a problem with additional data"() {
        ...
        withReportProblemTask """
            ${problemIdScript()}
            problems.getReporter().report(problemId) {
                it.additionalData(org.gradle.api.problems.GeneralDataSpec) {
                    it.put('key','value')
                }
            }
        """
    ...
    def "can rethrow a caught exception"() {
        ...
        withReportProblemTask """
            ${problemIdScript()}
            try {
                problems.getReporter().throwing(new RuntimeException("test"), ${IdFactory.name}.instance().createProblemId("type11", "inner", problemGroup)) { }
            } catch (RuntimeException ex) {
                problems.getReporter().throwing(ex, ${IdFactory.name}.instance().createProblemId("type12", "outer", problemGroup)) { }
            }
        """
    ...
    def "problem progress events in report"() {
        ...
        withReportProblemTask """
            ${problemIdScript()}
            for (int i = 0; i < 10; i++) {
                problems.getReporter().report(${IdFactory.name}.instance().createProblemId("type\$i", "This is the heading problem text\$i", problemGroup)) {
                        it.severity(Severity.WARNING)
    ...
    static String problemIdScript() {
        """${ProblemGroup.name} problemGroup = ${IdFactory.name}.instance().createRootProblemGroup("generic", "group label");
           ${ProblemId.name} problemId = ${IdFactory.name}.instance().createProblemId("type", "label", problemGroup)"""
    }
}
