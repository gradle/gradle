...
public class DefaultProblemDiagnosticsFactory implements ProblemDiagnosticsFactory {
    ...
    private static int getMaxStackTraces(BuildModelParameters buildModelParameters) {
        return buildModelParameters.isIsolatedProjects()
                ? ISOLATED_PROJECTS_MAX_STACKTRACE_COUNT
                : MAX_STACKTRACE_COUNT;
    }
    ...
    private ProblemDiagnostics locationFromStackTrace(
        ...
        return new DefaultProblemDiagnostics(
                stackTracingFailure, keepException ? throwable : null, stackTrace, location, source);
    }
