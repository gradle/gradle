...
class TaskOriginTracker implements BuildOperationTracker {
    ...
    public void started(BuildOperationDescriptor buildOperation, OperationStartEvent startEvent) {
        if (buildOperation.getDetails() instanceof RealizeTaskBuildOperationType.Details) {
            RealizeTaskBuildOperationType.Details details =
                    (RealizeTaskBuildOperationType.Details) buildOperation.getDetails();
            ...
            RegisterTaskBuildOperationType.Details details =
                    (RegisterTaskBuildOperationType.Details) buildOperation.getDetails();
            storeOrigin(buildOperation, details.getTaskId());
        }
    }
    ...
    private void storeOrigin(BuildOperationDescriptor buildOperation, long taskId) {
        origins.computeIfAbsent(taskId, key -> {
            PluginApplication pluginApplication =
                    pluginApplicationTracker.findRunningPluginApplication(buildOperation.getParentId());
