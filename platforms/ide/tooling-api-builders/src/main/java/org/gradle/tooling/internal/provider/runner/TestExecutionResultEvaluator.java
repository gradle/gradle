...
class TestExecutionResultEvaluator implements BuildOperationListener {
    ...
    public TestExecutionResultEvaluator(
            BuildOperationAncestryTracker ancestryTracker, TestExecutionRequestAction internalTestExecutionRequest) {
    ...
    private boolean noTestsSelectedInTaskSpecs() {
        return internalTestExecutionRequest.getTaskSpecs().stream()
    ...
    public void evaluate() {
        ...
        if (hasFailedTests()) {
            StringBuilder failedTestsMessage =
                    new StringBuilder("Test failed.\n").append(INDENT).append("Failed tests:");
    ...
    private String formatInternalTestExecutionRequest() {
        ...
        for (InternalTestDescriptor internalTestDescriptor :
                internalTestExecutionRequest.getTestExecutionDescriptors()) {
            requestDetails.append("\n").append(twoIndent()).append(internalTestDescriptor.getDisplayName());
            requestDetails
                    .append(" (Task: '")
                    .append(((DefaultTestDescriptor) internalTestDescriptor).getTaskPath())
                    .append("')");
        }
        final Collection<InternalJvmTestRequest> internalJvmTestRequests =
                internalTestExecutionRequest.getInternalJvmTestRequests();
            ...
            if (taskSpec instanceof InternalTestSpec) {
                ...
                for (String cls : testSpec.getClasses()) {
                    requestDetails
                            ...
                            .append("Test class: ")
                            .append(cls)
                            .append(" in task " + taskSpec.getTaskPath());
                }
                for (Map.Entry<String, List<String>> methods :
    ...
    public void finished(BuildOperationDescriptor buildOperation, OperationFinishEvent finishEvent) {
        if (buildOperation.getDetails() instanceof ExecuteTaskBuildOperationDetails) {
            ...
            TestDescriptorInternal testDescriptor = (TestDescriptorInternal)
                    ((ExecuteTestBuildOperationType.Details) buildOperation.getDetails()).getTestDescriptor();
            ...
            if (!testDescriptor.isComposite() && testResult.getFailedTestCount() != 0) {
                failedTests.add(new FailedTest(
                        ...
                        getTaskPath(buildOperation.getId(), testDescriptor)));
            }
        }
    }
