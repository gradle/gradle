...
class TestExecutionResultEvaluator implements BuildOperationListener {
    ...
    public void evaluate() {
        ...
        if (hasFailedTests()) {
            ...
            for (FailedTest failedTest : failedTests) {
                failedTestsMessage.append("\n").append(twoIntent()).append(failedTest.getDescription());
            }
    ...
    private String formatInternalTestExecutionRequest() {
        ...
        for (InternalTestDescriptor internalTestDescriptor : internalTestExecutionRequest.getTestExecutionDescriptors()) {
            requestDetails.append("\n").append(twoIntent()).append(internalTestDescriptor.getDisplayName());
        ...
        for (InternalJvmTestRequest internalJvmTestRequest : internalJvmTestRequests) {
            ...
            if (methodName == null) {
                requestDetails.append("\n").append(twoIntent()).append("Test class ").append(className);
            } else {
                requestDetails.append("\n").append(twoIntent()).append("Test method ").append(className).append(".").append(methodName).append("()");
            }
        }
        ...
        for (InternalTaskSpec taskSpec : internalTestExecutionRequest.getTaskSpecs()) {
            if (taskSpec instanceof InternalTestSpec) {
                ...
                for (String cls : testSpec.getClasses()) {
                    requestDetails.append("\n").append(twoIntent()).append("Test class: ").append(cls).append(" in task " + taskSpec.getTaskPath());
                }
                for (Map.Entry<String, List<String>> methods : testSpec.getMethods().entrySet()) {
                    for (String method : methods.getValue()) {
                        requestDetails.append("\n").append(twoIntent()).append("Test method ").append(methods.getKey()).append(".").append(method).append("()").append(" in task " + taskSpec.getTaskPath());
                    }
                }
                for (String pkg : testSpec.getPackages()) {
                    requestDetails.append("\n").append(twoIntent()).append("Test package ").append(pkg).append(" in task " + taskSpec.getTaskPath());
                }
                for (String pattern : testSpec.getPatterns()) {
                    requestDetails.append("\n").append(twoIntent()).append("Test pattern ").append(pattern).append(" in task " + taskSpec.getTaskPath());
                }
            }
        }
    ...
    private static String twoIntent() {
