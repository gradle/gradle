...
class ConsoleProblemEmitterTest extends Specification {
    ...
    def "renders problems from other groups"() {
        expect:
        ConsoleProblemEmitter.shouldRender(problem(problems.groups.compilation.kotlin.problem("Unused import")))
    }
    ...
    def "does not render Java compilation problems, which the compiler output already shows"() {
        ...
        id << [
            ...
            problems.groups.compilation.java.problem("Compiler initialization failed"),
            problems.groups.compilation.java.group("Lint").problem("Unused import"),
        ]
    }
