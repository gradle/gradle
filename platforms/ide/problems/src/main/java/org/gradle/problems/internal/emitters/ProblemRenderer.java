...
//                output.printf(sep);
    ...
    private static void renderProblem(PrintWriter output, InternalProblem problem) {
        formatMultiline(output, problemHeaderMessage(problem), 0);
        output.printf("%n");
        ...
        if (!problem.getSolutions().isEmpty()) {
            for (String solution : problem.getSolutions()) {
                output.printf("%n");
                formatMultiline(output, "Solution: " + solution, 2);
            }
        }
        ...
        for (FileLocation location : fileLocations) {
            output.printf("%n");
            formatMultiline(output, "Location: " + location.getPath(), 2);
        }
    }
