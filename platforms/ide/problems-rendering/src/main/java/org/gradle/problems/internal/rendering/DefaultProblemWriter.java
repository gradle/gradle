...
class DefaultProblemWriter implements SelectiveProblemWriter {
    ...
    private static PartialProblemWriter header = new HeaderWriter();
    private static PartialProblemWriter body = new BodyWriter();

    @Override
    public void write(InternalProblem problem, RenderOptions options, PrintWriter output) {
        header.write(problem, options, output);
        body.write(problem, options, output);
    }
