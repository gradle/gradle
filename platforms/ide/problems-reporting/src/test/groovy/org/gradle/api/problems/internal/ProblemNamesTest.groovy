...
class ProblemNamesTest extends Specification {
    ...
    def "rejects invalid group name '#name'"() {
        ...
        "Java > Kotlin" | "must not contain '>' or '\"'"   // the separator of the rendered group chain
        'Say "hi"'      | "must not contain '>' or '\"'"   // would need quoting in the rendered chain
    }
    ...
    def "accepts valid problem name '#name'"() {
        ...
        name << ["Unused import", "Class 'Foo' is not serializable", 'Class "Foo" is not serializable', "a > b", "Compilation:Java failed", "a" * 2000, "Émoji 🚀", "\uD83D\uDE80" * 2000, "می\u200Cخواهم", "👨\u200D👩\u200D👧"]
    }
