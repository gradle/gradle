...
import static org.gradle.api.problems.internal.PredefinedRoots.allPredefinedChildrenOf
...
class PredefinedProblemGroupsTest extends Specification {
    ...
    def "root groups match the spec order and are found by exact name"() {
        expect:
    ...
    def "predefined sub-groups match the spec names and carry a description and their parent"() {
        expect:
        for (root in DefaultProblemGroups.INSTANCE.roots) {
            for (child in allPredefinedChildrenOf(root)) {
                assert child.parent.is(root)
                assert child.description
                assert child.displayName == child.name
                assert child instanceof ProblemGroupInternal
                assert child.name == ProblemNames.validateGroupName(child.name)
            }
        }

        and:
        allPredefinedChildrenOf(groups.gradle)*.name == ["Build Cache", "Build Definition", "Build Logic", "Configuration Cache", "Deprecation", "DSL Evaluation", "Invocation", "Isolated Projects", "Plugin Validation", "Undefined"]
        allPredefinedChildrenOf(groups.dependencies)*.name == ["Declaration", "Locking", "Graph Resolution", "Artifact Resolution", "Verification", "Undefined"]
        allPredefinedChildrenOf(groups.transformation)*.name == ["Binary Transformation", "Code Transformation", "Resource Transformation", "Undefined"]
        allPredefinedChildrenOf(groups.compilation)*.name == ["C++", "Groovy", "Java", "Kotlin", "Scala", "Swift", "Undefined"]
        allPredefinedChildrenOf(groups.externalProcesses)*.name == ["Application", "Service", "Tool", "Undefined"]
        allPredefinedChildrenOf(groups.packaging)*.name == ["Distributions", "JAR", "Native Linking", "Signing", "Undefined"]
        allPredefinedChildrenOf(groups.verification)*.name == ["Code Coverage", "Code Quality", "Security", "Testing", "Undefined"]
        allPredefinedChildrenOf(groups.documentation)*.name == ["Javadoc", "Groovydoc", "Scaladoc", "Undefined"]
        allPredefinedChildrenOf(groups.provisioning)*.name == ["Infrastructure", "Tools and Toolchains", "Undefined"]
        allPredefinedChildrenOf(groups.delivery)*.name == ["Publishing", "Deployment", "Undefined"]
        allPredefinedChildrenOf(groups.others)*.name == ["Undefined"]
    }

    def "the Gradle root cannot create sub-groups"() {
        expect:
    ...
    def "every root and every open sub-group has an Undefined group with a templated description"() {
        ...
        for (root in DefaultProblemGroups.INSTANCE.roots) {
            ...
            assert undefined instanceof UndefinedProblemGroup
        }
    ...
    def "group(name) creates user groups without description, not memoized but equal"() {
        ...
        then:
        kmp instanceof SecondLevelProblemGroup
        kmp.name == "KMP"
        kmp.displayName == "KMP"
        ...
        !kmp.is(kmpAgain)
        js instanceof ThirdLevelProblemGroup
        js.parent == kmp
        js.description == null
    ...
    def "group(name) on a predefined sub-group creates a terminal user group below it"() {
        ...
        def id = lombok.problemId("Annotation processing failed")

        then:
        lombok instanceof ThirdLevelProblemGroup
        lombok.name == "Lombok"
    ...
    def "each kind of closed group has its own type, so a level can be opened later without opening the others"() {
        def gradleSub = groups.gradle.buildCache
        def thirdLevel = groups.transformation.group("KMP").group("JavaScript")
        def undefineds = [groups.compilation.undefined, groups.gradle.undefined, groups.compilation.java.undefined, groups.transformation.group("KMP").undefined]

        expect:
        gradleSub instanceof GradleSecondLevelProblemGroup
        !(gradleSub instanceof SecondLevelProblemGroup)
        !(gradleSub instanceof ThirdLevelProblemGroup)
        !(gradleSub instanceof UndefinedProblemGroup)
        gradleSub.parent.is(groups.gradle)

        thirdLevel instanceof ThirdLevelProblemGroup
        !(thirdLevel instanceof GradleSecondLevelProblemGroup)
        !(thirdLevel instanceof UndefinedProblemGroup)

        undefineds.every { it instanceof UndefinedProblemGroup }
        undefineds.every { !(it instanceof GradleSecondLevelProblemGroup) && !(it instanceof ThirdLevelProblemGroup) }

        // the guard: adding group(String) to one of these types must be a deliberate change to that type alone
        [GradleSecondLevelProblemGroup, ThirdLevelProblemGroup, UndefinedProblemGroup].every { type ->
            type.declaredMethods.every { it.name != "group" }
        }

        // the types survive serialization
        roundTrip(gradleSub) instanceof GradleSecondLevelProblemGroup
        roundTrip(thirdLevel) instanceof ThirdLevelProblemGroup
        undefineds.every { roundTrip(it) instanceof UndefinedProblemGroup }
    }

    def "group(name) validates names"() {
        // the rules themselves are covered by ProblemNamesTest; this only checks that both entry points validate
        when:
    ...
    def "problemId(name) creates an id in the group with displayName equal to name"() {
        when:
        def id = groups.compilation.java.problemId("Unused import")
        def leafId = groups.transformation.group("KMP").group("JavaScript").problemId("Something failed")
        def undefinedId = groups.others.undefined.problemId("Whatever")
        def gradleId = groups.gradle.buildLogic.problemId("Configuration used after resolution")
        ...
        gradleId.group.is(groups.gradle.buildLogic)
        leafId.group == groups.transformation.group("KMP").group("JavaScript")
        undefinedId.group.is(groups.others.undefined)
    }

    def "the Deprecation sub-group of the Gradle root is reserved for Gradle and hidden from the public type"() {
        def deprecation = ((GradleProblemGroupInternal) groups.gradle).deprecation

        expect: "reachable internally, with its description, below the Gradle root"
        deprecation.name == "Deprecation"
        deprecation.parent.is(groups.gradle)
        deprecation.description == PredefinedProblemGroupDescriptions.GRADLE_DEPRECATION
        deprecation instanceof GradleSecondLevelProblemGroup

        and: "not reachable through the public type"
        !GradleProblemGroup.methods*.name.contains("getDeprecation")

        and: "still resolved to the canonical instance when read back, so Gradle's own problems keep their group"
        roundTrip(deprecation).is(deprecation)
    }

    def "problemId(name) validates names"() {
        // the rules themselves are covered by ProblemNamesTest; this only checks that both entry points validate
        when:
        groups.compilation.java.problemId("a\tb")

        then:
        thrown(IllegalArgumentException)

        when:
        groups.compilation.java.undefined.problemId("a\tb")
    ...
    def "serialization round trip re-creates user groups below the canonical predefined parent"() {
        ...
        jsCopy == js
        jsCopy.parent == kmp
        jsCopy instanceof ThirdLevelProblemGroup
