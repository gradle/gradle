...
class PredefinedProblemGroupsTest extends Specification {
    ...
    def "gradle sub-groups are terminal, open root sub-groups are extensible"() {
        expect:
        predefinedChildren(groups.gradle).every { it instanceof GradleSecondLevelProblemGroup || it instanceof UndefinedProblemGroup }
        predefinedChildren(groups.compilation).findAll { it.name != "Undefined" }.every { it instanceof SecondLevelProblemGroup }
    ...
    def "problemId(name) creates an id in the group with displayName equal to name"() {
        ...
        def gradleId = groups.gradle.deprecation.problemId("Deprecated feature used")
    ...
    def "problem builder keeps predefined groups, so descriptions reach the reported problem"() {
        ...
        def fromId = new DefaultProblemBuilder(infrastructure).id(groups.compilation.java.problemId("Unused import")).build()
