 ...
 *     of the constant; a sentence may follow it, as in the Gradle row.</li>
...
class PredefinedProblemGroupsDocumentationTest extends Specification {
    ...
    def "the hierarchy table in the user guide matches the predefined groups"() {
        ...
        and: "each row lists the root's predefined sub-groups and quotes its description"
        groups.roots.every { root ->
            def row = table.find { it.root == root.name }
            def expectedSubgroups = predefinedChildren(root).findAll { it.name != ProblemGroupSupport.UNDEFINED_NAME }*.name
    ...
    def "the Javadoc of the public group types quotes the descriptions of the predefined groups"() {
        ...
        groups.roots.every { root ->
            ...
            assertDocumented(publicType, "sub-group", predefinedChildren(root).findAll { it.name != ProblemGroupSupport.UNDEFINED_NAME })
            true
        }
    }
