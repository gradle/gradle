...
import static org.gradle.api.problems.internal.PredefinedRoots.publicPredefinedChildrenOf
 ...
 *     sub-groups in the order of the root's {@code PredefinedChildren}, and the description cell must start with the exact text
...
class PredefinedProblemGroupsDocumentationTest extends Specification {
    ...
    def "the hierarchy table in the user guide matches the predefined groups"() {
        ...
        groups.roots.every { root ->
            ...
            def publicNames = publicPredefinedChildrenOf(root)*.name
            def expectedSubgroups = allPredefinedChildrenOf(root).findAll { it.name != ProblemNames.UNDEFINED_NAME }.collect { publicNames.contains(it.name) ? it.name : it.name + RESERVED_MARKER }
    ...
    def "the Javadoc of the public group types quotes the descriptions of the predefined groups"() {
        ...
        groups.roots.every { root ->
            ...
            assertDocumented(publicType, "sub-group", publicPredefinedChildrenOf(root).findAll { it.name != ProblemNames.UNDEFINED_NAME })
            true
        }
    }
