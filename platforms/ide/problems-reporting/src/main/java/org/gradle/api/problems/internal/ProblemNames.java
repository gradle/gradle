...
final class ProblemNames {
    ...
    /**
     * Validates a group name created through the predefined-group API. Besides the rules shared with problem names, a group
     * name must not contain {@code >} or {@code "}: group names are rendered as a chain separated by {@code >}, and neither
     * character has a place in a category name.
     *
    ...
    static String validateGroupName(@Nullable String name) {
        ...
        checkArgument(validated.indexOf('>') < 0 && validated.indexOf('"') < 0, "Problem group name must not contain '>' or '\"': '%s'", validated);
        return validated;
    }
