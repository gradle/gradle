...
class DefaultConfigurableFileCollectionSpec extends FileCollectionSpec {
    ...
    def "can incrementally set paths as convention to the collection"() {
        ...
        collection.unset()
        collection.convention("src1", "src2")
        collection.actualValue.from("src3")
        then:
        collection.from as List == ["src1", "src2", "src3"]
    }

    def "can incrementally set paths as convention to the collection using a configurer"() {
        when:
        collection.configureConvention {
            it.from("src1", "src2")
            it.from("src3")
        }
        collection.unset()
    ...
    def "can set explicit value using an explicit configurer2"() {
        when:
        collection.setFrom()
        ...
        then:
        collection.from as List == ["src3", "src4"]
    }

    def "can set explicit value using an explicit configurer"() {
        when:
        collection.configureConvention {
            it.from("src1", "src2")
        }
    ...
    def "can set convention using the actual value configurer"() {
        when:
        collection.configureConvention {
            it.from("src1", "src2")
        }
    ...
    def "can incrementally set paths as convention to the collection using a configurer 3"() {
        when:
        collection.configure {
            it.from("src0")
        }
    ...
    def "conventions are ignored if a value is already explicitly set"() {
        when:
