...
public class DefaultDeclaredProjectFeatureBindingBuilder<T extends Definition<V>, V extends BuildModel>
    ...
    @Nullable
    private Class<?> dslImplementationType;

    @Nullable
    private Class<?> buildModelImplementationType;
    ...
    @Override
    public DeclaredProjectFeatureBindingBuilder<T, V> withDefinitionImplementationType(
    ...
    @Override
    public DeclaredProjectFeatureBindingBuilder<T, V> withBuildModelImplementationType(
            Class<? extends V> implementationType) {
    ...
    public ProjectFeatureBindingDeclaration<T, V> build() {
        if (dslImplementationType != null && !dslType.isAssignableFrom(dslImplementationType)) {
            throw new IllegalArgumentException(
                    "Implementation type " + dslImplementationType + " is not a subtype of dsl type " + dslType);
        }

        if (buildModelImplementationType != null && !buildModelType.isAssignableFrom(buildModelImplementationType)) {
            throw new IllegalArgumentException("Implementation type " + buildModelImplementationType
                    + " is not a subtype of build model type " + buildModelType);
        }
