...
public class ModelTypeInitializationException extends GradleException {
    ...
    private static final String UNMANAGED_PROPERTY_DESCRIPTION =
    ...
    private static <T> String toMessage(
        ...
        Optional<? extends NodeInitializerContext.PropertyContext> propertyContextOptional =
                context.getPropertyContextOptional();
        ...
        if (propertyContextOptional.isPresent()) {
            ...
            s.append(String.format(
                    ...
                    propertyContext.getDeclaringType().getName()));
            ...
            if (isManagedCollection(propertyType)) {
                ...
                s.append(String.format(
                        "Its property '%s %s' can not be constructed%n", propertyType.getName(), propertyName));
    ...
    private static String explainScalarCollections(Iterable<? extends ModelType<?>> scalarTypes) {
        return String.format(
                "A valid scalar collection takes the form of List<T> or Set<T> where 'T' is one of (%s)",
                describe(scalarTypes));
    }

    private static String appendManagedCollections(
            StringBuilder s, int pad, Iterable<? extends ModelType<?>> constructibleTypes) {
        s.append(String.format(
    ...
    private static void maybeAppendConstructibles(
            StringBuilder s, Iterable<? extends ModelType<?>> constructibleTypes, int pad) {
    ...
    private static String describe(Iterable<? extends ModelType<?>> types) {
        return Joiner.on(", ")
                .join(ImmutableSet.copyOf(Iterables.transform(types, new Function<ModelType<?>, String>() {
                    @Override
