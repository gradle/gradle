...
import org.gradle.internal.declarativedsl.dom.resolution.DocumentResolution.ElementResolution
import org.gradle.internal.declarativedsl.dom.resolution.DocumentResolution.PropertyResolution
import org.gradle.internal.declarativedsl.dom.resolution.DocumentResolution.ValueResolution.LiteralValueResolved
import org.gradle.internal.declarativedsl.dom.resolution.DocumentResolution.ValueResolution.ValueFactoryResolution
...
fun resolutionContainer(
    ...
    schema,
    tracingCodeResolver(analysisStatementFilter).also { it.resolve(schema, languageTreeResult.imports, languageTreeResult.topLevelBlock) }.trace,
...
class DefaultDocumentResolutionContainer(
    private val elementResolution: Map<DeclarativeDocument.DocumentNode.ElementNode, ElementResolution>,
    private val propertyResolution: Map<DeclarativeDocument.DocumentNode.PropertyNode, PropertyResolution>,
    private val valueFactoryResolution: Map<DeclarativeDocument.ValueNode.ValueFactoryNode, ValueFactoryResolution>
) : DocumentResolutionContainer {
    override fun data(node: DeclarativeDocument.DocumentNode.ElementNode): ElementResolution = elementResolution.getValue(node)
    override fun data(node: DeclarativeDocument.DocumentNode.PropertyNode): PropertyResolution = propertyResolution.getValue(node)
    override fun data(node: DeclarativeDocument.DocumentNode.ErrorNode): DocumentResolution.ErrorResolution = DocumentResolution.ErrorResolution
    override fun data(value: DeclarativeDocument.ValueNode.ValueFactoryNode): ValueFactoryResolution = valueFactoryResolution.getValue(value)
...
class DocumentResolver(
    ...
    fun resolutionContainer(document: DeclarativeDocument): DefaultDocumentResolutionContainer {
        val elementResolution = mutableMapOf<DeclarativeDocument.DocumentNode.ElementNode, ElementResolution>()
        val propertyResolution = mutableMapOf<DeclarativeDocument.DocumentNode.PropertyNode, PropertyResolution>()
        val valueFactoryResolution = mutableMapOf<DeclarativeDocument.ValueNode.ValueFactoryNode, ValueFactoryResolution>()
