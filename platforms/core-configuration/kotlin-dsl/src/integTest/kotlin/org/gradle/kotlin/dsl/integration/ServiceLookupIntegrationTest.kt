...
class ServiceLookupIntegrationTest : AbstractKotlinIntegrationTest() {
    ...
    @Test
    @Requires(TestExecutionPreconditions.NotIsolatedProjects::class) // configuring another project is reported by Isolated Projects
    fun `inside a subprojects block the lookup resolves the services of the subproject`() {
        ...
        assertThat(build("help").output, containsString("layout for :a: a"))
    }

    @Test
    fun `service lookup is not available on the Gradle object`() {
        withSettings("""
    ...
    fun `looking up a settings-only service from a task does not compile`() {
        ...
        buildAndFail("useLayout").apply {
            assertHasErrorOutput("Script compilation error")
            assertHasErrorOutput("TaskService")
        }
    }
}
