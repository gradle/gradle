...
class SourceDistributionResolver(private val project: Project) : SourceDistributionProvider {
    ...
    fun createSourceRepository() = ivy {
        ...
        setUrl(gradleDistRepository.repoBaseUrl)
        metadataSources {
            artifact()
        }
        patternLayout {
        ...
        gradleDistRepository.credentials?.let { distRepoCredentials ->
            credentials {
                username = distRepoCredentials.username
                password = distRepoCredentials.password
            }
        }
    }
