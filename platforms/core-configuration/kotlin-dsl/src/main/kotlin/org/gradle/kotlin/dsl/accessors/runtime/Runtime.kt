...
fun applySoftwareFeature(
    ...
    val context = when (target) {
        is ProjectInternal ->
            target.objects.newInstance<SoftwareFeatureSupportInternal.DefaultProjectFeatureDefinitionContext.Factory>()
                .create(target)

        is HasBuildModel<*> -> SoftwareFeatureSupportInternal.getContext(target)
        else -> throw IllegalArgumentException("Unexpected definition object $target, cannot get context to apply the feature")
    }
    val softwareFeature = context.softwareFeatureRegistry.softwareFeatureImplementations.getValue(name)
    context.softwareFeatureApplicator.applyFeatureTo(target, softwareFeature)
    configure.invoke(getSoftwareFeatureDefinitionInstance(softwareFeature, target))
}

@Suppress("UNCHECKED_CAST")
private fun <T: Any> getSoftwareFeatureDefinitionInstance(softwareType: SoftwareFeatureImplementation<T, *>, receiverObject: DynamicObjectAware): T =
    (SoftwareFeatureSupportInternal.getContext(receiverObject).childrenDefinitions()[softwareType]
        ?: error("No definition found for software feature ${softwareType.featureName} in ${receiverObject::class.qualifiedName}")) as T
