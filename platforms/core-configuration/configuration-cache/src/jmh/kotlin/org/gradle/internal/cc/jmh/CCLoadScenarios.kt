...
object CCLoadScenarios {
    ...
    // Zstd with configurable level
    fun writeWithZstdLevel(graph: Any, level: Int) =
        writeToByteArrayWith(graph) {
            KryoBackedEncoder(
                ZstdOutputStream(encrypted(it), level)
            )
        }

    fun readWithZstdLevel(bytes: ByteArray) =
        readFrom(ZstdInputStream(decrypted(bytes)))

    // LZ4 fast compression
    fun writeWithLz4Fast(graph: Any) =
        writeToByteArrayWith(graph) {
            KryoBackedEncoder(
                LZ4FrameOutputStream(encrypted(it))
            )
        }
