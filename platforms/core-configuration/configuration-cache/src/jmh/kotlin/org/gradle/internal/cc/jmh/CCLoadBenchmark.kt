...
open class CCLoadBenchmark {

    private
    lateinit var state: CCLoadScenarios.State

    @Setup
    fun setup() {
        state = CCLoadScenarios.createState(
            (1..1024).map { Peano.fromInt(1024) }
        )
    }

    @Benchmark
    fun withoutCompression(bh: Blackhole) {
        bh.consume(readUncompressed(state.uncompressed))
    }

    @Benchmark
    fun withSnappyCompression(bh: Blackhole) {
        bh.consume(readWithSnappy(state.snappy))
    }
