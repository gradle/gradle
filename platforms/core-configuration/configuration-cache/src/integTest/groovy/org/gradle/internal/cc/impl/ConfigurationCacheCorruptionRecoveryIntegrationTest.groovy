...
class ConfigurationCacheCorruptionRecoveryIntegrationTest extends AbstractConfigurationCacheIntegrationTest {
    ...
    def "recovers from a corrupted work graph (IP: #ipEnabled)"() {
        given:
    ...
    def "recovers from #corruptState (IP: #ipEnabled)"() {
        ...
        then:
        outputContains(CORRUPT_ON_CHECK)
        outputContains(FAILURE_STACK_TRACE)
        outputContains("Hello")
        assertNoCorruptedState()
        ...
        where:
        corruptState << ["corruptMetadata", "corruptFingerprint"]

        combined:
        ipEnabled << [false, true]
    }

    // No IP variant: Isolated Projects does not fingerprint classloader scopes, so it stores no such file.
    def "recovers from corrupted classloader scopes"() {
        given:
        buildFile """
            tasks.register("hello") {
                doLast { println "Hello" }
    ...
    def "fails the build on #corruptState when recovery is disabled (IP: #ipEnabled)"() {
        ...
        where:
        // Isolated Projects stores no classloader scopes yet, so it's excluded
        ipEnabled | corruptState
        false     | "corruptWorkState"
        true      | "corruptWorkState"
        false     | "corruptMetadata"
        true      | "corruptMetadata"
        false     | "corruptFingerprint"
        true      | "corruptFingerprint"
        false     | "corruptClassLoaderScopes"
    }
    ...
    def "fails the build on #corruptState when integrity check is enabled"() {
        given:
        buildFile """
            tasks.register("hello") {
                doLast { println "Hello" }
