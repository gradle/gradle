...
class IsolatedProjectsServiceLookupIntegrationTest extends AbstractIsolatedProjectsIntegrationTest {
    ...
    def "reports a problem when a build script looks up a service inside a subprojects block"() {
        createDirs("a")
        settingsFile """
            include("a")
        """
        ...
        fixture.assertIsolatedProjectsProblems(mode) {
            projectsConfigured(":", ":a")
            problem("Build file 'build.gradle': line 3: Project ':' cannot access 'Project.service' functionality on subprojects")
        }
