 ...
 * (such as a value source throwing or an integrity check detecting a problem).
 */
internal class ConfigurationCacheEntryReadException(cause: Throwable) : RuntimeException(cause)


/**
 * Reads a part of a stored cache entry. An unclassified failure becomes a [ConfigurationCacheEntryReadException] unless
 * integrity checking is enabled.
 */
internal inline fun <T> readStoredState(isIntegrityCheckEnabled: Boolean, read: () -> T): T =
    try {
        read()
    } catch (e: Exception) {
