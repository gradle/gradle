...
class KotlinDslPluginCrossVersionSmokeTest : AbstractKotlinIntegrationTest() {
    ...
    val oldestSupportedKotlinDslPluginVersion = "4.1.1"

    @Test
    @Requires(IntegTestPreconditions.NotEmbeddedExecutor::class)
    fun `can run with first version of kotlin-dsl plugin supporting Gradle 9_0`() {
        ...
        maybeSetTargetJvmVersion(buildScript)
    ...
    @Test
    @Requires(IntegTestPreconditions.NotEmbeddedExecutor::class)
    fun `can run kotlin-dsl plugin with deprecation warning`() {
        withDefaultSettingsIn("buildSrc")
        val buildScript = withBuildScriptIn("buildSrc", scriptWithKotlinDslPlugin(oldestSupportedKotlinDslPluginVersion))
        maybeSetTargetJvmVersion(buildScript)
        withFile("buildSrc/src/main/kotlin/some.gradle.kts", """println("some!")""")
    ...
    private fun maybeSetTargetJvmVersion(buildScript: File) {
        if (JavaVersion.current().isCompatibleWith(JavaVersion.VERSION_22)) {
            // The Kotlin version that is a dependency of kotlin-dsl version `oldestSupportedKotlinDslPluginVersion`
            // doesn't work with Java22+ without setting jvmTarget that is lower than JvmTarget.JVM_22
            buildScript.appendText("""
