...
class OptimisticBuildCacheStepTest extends StepSpec<TestCachingContext> implements SnapshotterFixture {
    ...
    def step = new OptimisticBuildCacheStep<TestCachingContext>(buildCacheController, deleter, fileSystemAccess, outputChangeListener, delegate)
    def delegateResult = Mock(AfterExecutionResult)

    def "loads from local cache"() {
        ...
        1 * buildCacheController.loadLocally(cacheKey, _) >> Optional.of(loadMetadata)
    ...
    def "executes optimistically on local cache miss and stores result"() {
        given:
    ...
    def "fails when cache backend throws exception while storing"() {
        given:
        def execution = Mock(Execution)
