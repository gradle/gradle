...
public class OutputFileChanges implements ChangeContainer {
    ...
    public OutputFileChanges(
            SortedMap<String, FileSystemSnapshot> previous, SortedMap<String, FileSystemSnapshot> current) {
    ...
    public boolean accept(ChangeVisitor visitor) {
        return SortedMapDiffUtil.diff(
                previous, current, new PropertyDiffListener<String, FileSystemSnapshot, FileSystemSnapshot>() {
                    @Override
    ...
    public boolean visitChangesSince(
            FileSystemSnapshot previous, FileSystemSnapshot current, String propertyTitle, ChangeVisitor visitor) {
        if (previous == current) {
            return true;
        }
        if (previous == FileSystemSnapshot.EMPTY) {
            return visitAllChanges(
                    current,
                    visitor,
                    (relativePath, snapshot) -> FINGERPRINT_CHANGE_FACTORY.added(
                            snapshot.getAbsolutePath(), propertyTitle, createFingerprint(relativePath, snapshot)));
        } else if (current == FileSystemSnapshot.EMPTY) {
            return visitAllChanges(
                    previous,
                    visitor,
                    (relativePath, snapshot) -> FINGERPRINT_CHANGE_FACTORY.removed(
                            snapshot.getAbsolutePath(), propertyTitle, createFingerprint(relativePath, snapshot)));
        } else {
            ...
            if (previousSnapshot.getHash().equals(currentSnapshot.getHash())) {
                return true;
            } else if (currentSnapshot.getType() != FileType.Directory
                    || previousSnapshot.getType() != FileType.Directory) {
                ...
                if (visitor.visitChange(FINGERPRINT_CHANGE_FACTORY.modified(
                    ...
                    // Only one of the types will be a directory, so we now need to report all children of the other
                    // one.
                    if (visitAllChildChanges(
                            previousSnapshot,
                            visitor,
                            (relativePath, snapshot) -> FINGERPRINT_CHANGE_FACTORY.removed(
        ...
        return NormalizedPathChangeDetector.INSTANCE.visitChangesSince(
                previousFingerprint, currentFingerprint, propertyTitle, visitor);
    }

    private static boolean visitAllChanges(
            FileSystemSnapshot root,
            ChangeVisitor visitor,
            BiFunction<RelativePathSupplier, FileSystemLocationSnapshot, Change> changeFactory) {
        return root.accept(new RelativePathTracker(), (snapshot, relativePath) -> {
                    boolean shouldContinue = visitor.visitChange(changeFactory.apply(relativePath, snapshot));
                    return shouldContinue ? SnapshotVisitResult.CONTINUE : SnapshotVisitResult.TERMINATE;
                })
                == SnapshotVisitResult.CONTINUE;
    }

    private static boolean visitAllChildChanges(
            FileSystemSnapshot root,
            ChangeVisitor visitor,
            BiFunction<RelativePathSupplier, FileSystemLocationSnapshot, Change> changeFactory) {
        return root.accept(new RelativePathTracker(), (snapshot, relativePath) -> {
    ...
    private static DefaultFileSystemLocationFingerprint createFingerprint(
            RelativePathSupplier relativePath, FileSystemLocationSnapshot snapshot) {
    ...
    private static DefaultFileSystemLocationFingerprint createFingerprint(
            String relativePath, FileSystemLocationSnapshot snapshot) {
