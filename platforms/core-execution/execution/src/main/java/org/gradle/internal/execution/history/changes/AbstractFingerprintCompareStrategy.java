...
public abstract class AbstractFingerprintCompareStrategy
    ...
    public static final ChangeFactory<FileSystemLocationFingerprint> FINGERPRINT_CHANGE_FACTORY =
            new ChangeFactory<FileSystemLocationFingerprint>() {
                @Override
                ...
                public Change removed(String path, String propertyTitle, FileSystemLocationFingerprint previous) {
                    return DefaultFileChange.removed(
                            path, propertyTitle, previous.getType(), previous.getNormalizedPath());
                }

                @Override
                public Change modified(
                        String path,
                        String propertyTitle,
                        FileSystemLocationFingerprint previous,
                        FileSystemLocationFingerprint current) {
                    return DefaultFileChange.modified(
                            path, propertyTitle, previous.getType(), current.getType(), current.getNormalizedPath());
                }
            };

    private static final TrivialChangeDetector.ItemComparator<FileSystemLocationFingerprint> ITEM_COMPARATOR =
            new TrivialChangeDetector.ItemComparator<FileSystemLocationFingerprint>() {
                @Override
                public boolean hasSamePath(
                        FileSystemLocationFingerprint previous, FileSystemLocationFingerprint current) {
