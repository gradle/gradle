...
public abstract class ValidateStep<C extends BeforeExecutionContext, R extends Result> implements Step<C, R> {
    public static class Immutable<R extends Result> extends ValidateStep<ImmutableBeforeExecutionContext, R> {
        ...
        @Override
        protected R executeDelegate(
                UnitOfWork work, ImmutableBeforeExecutionContext context, List<InternalProblem> problems) {
            return delegate.execute(work, new ImmutableValidationFinishedContext(context, problems));
        }
    }

    public static class Mutable<R extends Result> extends ValidateStep<MutableBeforeExecutionContext, R> {
        ...
        @Override
        protected R executeDelegate(
                UnitOfWork work, MutableBeforeExecutionContext context, List<InternalProblem> problems) {
    ...
    public R execute(UnitOfWork work, C context) {
        ...
        context.getBeforeExecutionState()
                .ifPresent(
                        beforeExecutionState -> validateImplementations(work, beforeExecutionState, validationContext));
    ...
    private static void validateImplementations(
            UnitOfWork work, BeforeExecutionState beforeExecutionState, WorkValidationContext validationContext) {
        ...
        // It doesn't matter whether we use cacheable true or false, since none of the warnings depends on the
        // cacheability of the task.
        ...
        validateImplementation(
                workValidationContext, beforeExecutionState.getImplementation(), "Implementation of ", work);
        beforeExecutionState
                .getAdditionalImplementations()
                .forEach(additionalImplementation -> validateImplementation(
                        workValidationContext, additionalImplementation, "Additional action of ", work));
    ...
    private static void validateNestedInput(
            TypeValidationContext workValidationContext, String propertyName, ImplementationSnapshot implementation) {
        if (implementation instanceof UnknownImplementationSnapshot) {
            ...
            workValidationContext.visitPropertyProblem(problem -> problem.forProperty(propertyName)
                    .id(
                            TextUtil.screamingSnakeToKebabCase(UNKNOWN_IMPLEMENTATION_NESTED),
                            "Unknown property implementation",
    ...
    private static void validateImplementation(
            TypeValidationContext workValidationContext,
            ImplementationSnapshot implementation,
            String descriptionPrefix,
            UnitOfWork work) {
        if (implementation instanceof UnknownImplementationSnapshot) {
            UnknownImplementationSnapshot unknownImplSnapshot = (UnknownImplementationSnapshot) implementation;
            workValidationContext.visitPropertyProblem(problem -> problem.id(
                            TextUtil.screamingSnakeToKebabCase(UNKNOWN_IMPLEMENTATION),
                            "Unknown property implementation",
                            GradleCoreProblemGroup.validation().property())
    ...
    public interface ValidationWarningRecorder {
        void recordValidationWarnings(
                Identity identity, UnitOfWork work, Collection<? extends InternalProblem> warnings);
    }
}
