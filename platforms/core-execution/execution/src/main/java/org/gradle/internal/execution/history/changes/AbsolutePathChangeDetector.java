...
public class AbsolutePathChangeDetector<S> implements ChangeDetector<S> {
    ...
    public AbsolutePathChangeDetector(
            ItemComparator<S> itemComparator, CompareStrategy.ChangeFactory<S> changeFactory) {
    ...
    public boolean visitChangesSince(
        ...
        for (Map.Entry<String, S> currentEntry : current.entrySet()) {
            ...
            if (unaccountedForPreviousItems.remove(currentAbsolutePath)) {
                ...
                if (!itemComparator.hasSameContent(previousItem, currentItem)) {
                    Change modified =
                            changeFactory.modified(currentAbsolutePath, propertyTitle, previousItem, currentItem);
        ...
        for (String previousAbsolutePath : unaccountedForPreviousItems) {
            Change removed =
                    changeFactory.removed(previousAbsolutePath, propertyTitle, previous.get(previousAbsolutePath));
