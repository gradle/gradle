...
public class CaptureMutableStateBeforeExecutionStep<C extends PreviousExecutionContext, R extends CachingResult>
    ...
    private R executeMutable(MutableUnitOfWork work, C context) {
        ...
        if (shouldCaptureBeforeExecutionState) {
            ...
            overlappingOutputs =
                    detectOverlappingOutputs(work, context, beforeExecutionState.getOutputFileLocationSnapshots());
        } else {
            ...
            work.visitMutableInputs(new InputVisitor() {
                ...
                public void visitInputFileProperty(
                    ...
                    ((FileCollectionInternal) value.getFiles())
                            .visitStructure(work.getInputDependencyChecker(context.getValidationContext()));
                }
            });
        }
        return delegate.execute(
                work, new MutableBeforeExecutionContext(context, beforeExecutionState, overlappingOutputs));
    }
    ...
    private BeforeExecutionState captureExecutionState(UnitOfWork work, PreviousExecutionContext context) {
        ...
        return operation(
                operationContext -> {
                    ...
                    BeforeExecutionState executionState =
                            captureExecutionStateWithOutputs(work, context, unfilteredOutputSnapshots);
                    operationContext.setResult(Operation.Result.INSTANCE);
                    return executionState;
                },
                BuildOperationDescriptor.displayName(
                                "Snapshot inputs and outputs before executing " + work.getDisplayName())
                        .details(Operation.Details.INSTANCE));
    }

    @Nullable
    private OverlappingOutputs detectOverlappingOutputs(
            MutableUnitOfWork work,
    ...
    private static BeforeExecutionState captureExecutionStateWithOutputs(
            UnitOfWork work,
            PreviousExecutionContext context,
            ImmutableSortedMap<String, FileSystemSnapshot> unfilteredOutputSnapshots) {
        ...
        InputFingerprinter.Result newInputs = work.getInputFingerprinter()
