...
public class DirectorySnapshot extends AbstractFileSystemLocationSnapshot {
    ...
    public DirectorySnapshot(
        ...
        this(
                ...
                accessType,
                contentHash,
                childMapFromSorted(children.stream()
    ...
    public DirectorySnapshot(
            ...
            AccessType accessType,
            HashCode contentHash,
            ChildMap<FileSystemLocationSnapshot> children) {
    ...
    @Override
    protected Optional<DirectorySnapshot> relocateDirectAccess(
            String targetPath, String name, Interner<String> interner) {
        ImmutableList.Builder<ChildMap.Entry<FileSystemLocationSnapshot>> relocatedChildren =
                ImmutableList.builderWithExpectedSize(children.size());
        AtomicBoolean alreadyFoundASymlink = new AtomicBoolean(false);
        children.stream()
                .map(child -> child.getValue()
                        .relocate(targetPath + File.separatorChar + child.getPath(), interner)
                        .map(relocatedSnapshot ->
                                new ChildMap.Entry<FileSystemLocationSnapshot>(child.getPath(), relocatedSnapshot)))
        ...
        return Optional.of(new DirectorySnapshot(
                targetPath, name, getAccessType(), contentHash, childMapFromSorted(relocatedChildren.build())));
    }
    ...
    public SnapshotVisitResult accept(FileSystemSnapshotHierarchyVisitor visitor) {
        ...
        switch (result) {
            case CONTINUE:
                ...
                children.stream().map(ChildMap.Entry::getValue).forEach(child -> child.accept(visitor));
    ...
    public ImmutableList<FileSystemLocationSnapshot> getChildren() {
        return children.stream().map(ChildMap.Entry::getValue).collect(ImmutableList.toImmutableList());
    }

    @Override
    protected Optional<MetadataSnapshot> getChildSnapshot(VfsRelativePath targetPath, CaseSensitivity caseSensitivity) {
        return Optional.of(SnapshotUtil.getMetadataFromChildren(children, targetPath, caseSensitivity, Optional::empty)
    ...
    protected FileSystemNode getChildNode(VfsRelativePath targetPath, CaseSensitivity caseSensitivity) {
        ...
        return childNode.orElseGet(() -> missingSnapshotForAbsolutePath(targetPath.getAbsolutePath()));
    }
    ...
    public Optional<FileSystemNode> invalidate(
        ...
        ChildMap<FileSystemNode> newChildren = children.invalidate(
                targetPath,
                caseSensitivity,
                new ChildMap.InvalidationHandler<FileSystemLocationSnapshot, FileSystemNode>() {
                    @Override
                    public Optional<FileSystemNode> handleAsDescendantOfChild(
                            VfsRelativePath pathInChild, FileSystemLocationSnapshot child) {
                        diffListener.nodeRemoved(DirectorySnapshot.this);
                        Optional<FileSystemNode> invalidated = child.invalidate(
                                pathInChild, caseSensitivity, new SnapshotHierarchy.NodeDiffListener() {
                                    @Override
                    ...
                    public void handleUnrelatedToAnyChild() {
                        diffListener.nodeRemoved(DirectorySnapshot.this);
                        children.stream().map(ChildMap.Entry::getValue).forEach(diffListener::nodeAdded);
                    }
                });
