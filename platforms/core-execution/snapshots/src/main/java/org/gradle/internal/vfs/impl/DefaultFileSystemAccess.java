...
public class DefaultFileSystemAccess implements FileSystemAccess, FileSystemDefaultExcludesListener {
    ...
    public DefaultFileSystemAccess(
        ...
        this.directorySnapshotter =
                new DirectorySnapshotter(hasher, stringInterner, this.defaultExcludes, statisticsCollector);
        this.hasher = hasher;
                ...
                .orElseGet(() -> virtualFileSystem.storeWithAction(location, vfsStorer -> {
                    ...
                    switch (fileMetadata.getType()) {
                        ...
                        case RegularFile:
                            ...
                            // This is only a performance optimization for a common scenario; the VFS handles its own
                            // concurrency
                            return Optional.of(producingSnapshots.guardByKey(location, () -> virtualFileSystem
                                    .findSnapshot(location)
                                    .orElseGet(() -> {
                                        HashCode hashCode = hasher.hash(
                                                file, fileMetadata.getLength(), fileMetadata.getLastModified());
                                        return vfsStorer.store(new RegularFileSnapshot(
                                                location, file.getName(), hashCode, fileMetadata));
                                    })));
                        default:
    ...
    public FileSystemLocationSnapshot read(String location) {
        return readSnapshotFromLocation(location, Optional::of, () -> snapshot(location, SnapshottingFilter.EMPTY))
    ...
    private <T> T readSnapshotFromLocation(
            String location, Function<FileSystemLocationSnapshot, T> snapshotProcessor, Supplier<T> readFromDisk) {
        return virtualFileSystem
                .findSnapshot(location)
                .map(snapshotProcessor)
                // Avoid snapshotting the same location concurrently
                // This is only a performance optimization for a common scenario; the VFS handles its own concurrency
                .orElseGet(() -> producingSnapshots.guardByKey(location, () -> virtualFileSystem
                        .findSnapshot(location)
                        .map(snapshotProcessor)
     ...
     */
    private Optional<FileSystemLocationSnapshot> snapshotAndReuse(
            String location,
            SnapshottingFilter filter,
            ImmutableMap<String, FileSystemLocationSnapshot> previouslyKnownSnapshots) {
        return virtualFileSystem.storeWithAction(location, vfsStorer -> {
            ...
            switch (fileMetadata.getType()) {
                case RegularFile:
                    ...
                    unfilteredSnapshot =
                            vfsStorer.store(new RegularFileSnapshot(location, file.getName(), hash, fileMetadata));
                    break;
                case Missing:
                    unfilteredSnapshot =
                            vfsStorer.store(new MissingFileSnapshot(location, fileMetadata.getAccessType()));
    ...
    public void moveAtomically(String sourceLocation, String targetLocation) throws IOException {
        ...
        write(ImmutableList.of(sourceLocation, targetLocation), () -> {
            ...
            sourceSnapshot.relocate(targetLocation, stringInterner).ifPresent(this::record);
        });
    }
    ...
    public void onDefaultExcludesChanged(List<String> excludes) {
        ...
        if (!defaultExcludes.equals(newDefaultExcludes)) {
            ...
            directorySnapshotter =
                    new DirectorySnapshotter(hasher, stringInterner, newDefaultExcludes, statisticsCollector);
