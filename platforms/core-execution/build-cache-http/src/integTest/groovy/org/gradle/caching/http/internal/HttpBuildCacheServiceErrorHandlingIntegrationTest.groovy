...
class HttpBuildCacheServiceErrorHandlingIntegrationTest extends HttpBuildCacheFixture {
    ...
    def "build does not fail if connection drops during store and server dies"() {
        ...
        String errorPattern = /(Connect to 127\.0\.0\.1:\d+ \[\/127\.0\.0\.1\] failed: Connection refused|127\.0\.0\.1:\d+ failed to respond|Connection reset|Broken pipe|Software caused connection abort: socket write error|An established connection was aborted by the software in your host machine)/
