...
public class WorkerDaemonServer implements RequestHandler<TransportableActionExecutionSpec, DefaultWorkResult> {
    ...
    public DefaultWorkResult run(TransportableActionExecutionSpec spec) {
        try {
            try (WorkerProjectServices internalServices = new WorkerProjectServices(spec.getProjectDir(), spec.getRootDir(), this.internalServices)) {
    ...
    static class WorkerProjectServices extends DefaultServiceRegistry {
        public WorkerProjectServices(File projectDir, File rootDir, ServiceRegistry... parents) {
            super("worker file services for " + projectDir.getAbsolutePath(), parents);
            addProvider(new WorkerSharedProjectScopeServices(projectDir, rootDir));
        }
