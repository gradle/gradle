...
class OnDemandFileAccessTest extends Specification {
    ...
    def "acquires shared lock to read file"() {
        ...
        then:
        !file.exists()
        1 * manager.lock(file, new DefaultLockOptions(LockMode.Shared), "some-lock") >> targetLock
    ...
    def "acquires exclusive lock to update file"() {
        ...
        then:
        !file.exists()
        1 * manager.lock(file, new DefaultLockOptions(LockMode.Exclusive), "some-lock") >> targetLock
