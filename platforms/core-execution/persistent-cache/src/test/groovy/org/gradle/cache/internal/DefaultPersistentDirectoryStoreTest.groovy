...
class DefaultPersistentDirectoryStoreTest extends Specification {
    ...
    @Subject @AutoCleanup
    def store = new DefaultPersistentDirectoryStore(cacheDir, "<display>", new DefaultLockOptions(), cacheCleanup, lockManager, Mock(ExecutorFactory), progressLoggerFactory)
    ...
    def "open does not lock cache directory when None mode requested"() {
        final store = new DefaultPersistentDirectoryStore(cacheDir, "<display>", new DefaultLockOptions(), null, lockManager, Mock(ExecutorFactory), progressLoggerFactory)
    ...
    def "does not use gc.properties when no cleanup action is defined"() {
        given:
        store = new DefaultPersistentDirectoryStore(cacheDir, "<display>", new DefaultLockOptions(), null, lockManager, Mock(ExecutorFactory), progressLoggerFactory)
