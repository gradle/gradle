 ...
 *
 * <p>Keys are hashed to longs (via SipHash-2-4) for fast lookup.
 *
 * <p>{@code put()} and {@code remove()} operations are buffered in an in-memory
 * {@link HashMap} and only flushed to MapDB on {@link #close()}. This eliminates
 * per-operation MapDB overhead from the caller's hot path — the caller pays only
 * serialization + a HashMap insert. {@code get()} checks the buffer first, falling
 * through to MapDB on a miss.
 *
 * <p>Tuned for Gradle's single-threaded access pattern (enforced by
 * {@code DefaultCacheCoordinator.stateLock} above):
 * <ul>
 *   <li>{@code concurrencyDisable()} — no internal locking overhead</li>
 *   <li>{@code fileMmapEnableIfSupported()} — fastest file I/O</li>
...
public class MapDBPersistentIndexedCache<K, V> implements PersistentIndexedCache<K, V> {
    ...
    /** Sentinel value stored in the pending writes map to represent a removed entry. */
