...
A different JVM installation can be specified for the build by using the `org.gradle.java.home` property or programmatically through the Tooling API.

Daemon JVM criteria is a new feature to express the build's JVM requirements.
In a similar way to the `wrapper` task capturing the current Gradle version, the built-in task `updateDaemonJvm` records the JVM requirements for the build based on the current JVM installation.
Gradle currently only considers the major JVM version when checking compatibility.

On subsequent builds, Gradle will use the JVM requirements to locate a compatible JVM installation using the same underlying mechanisms as <<toolchains.adoc, Java Toolchains>>.
