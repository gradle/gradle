...
[[property_accessor_must_not_be_overridden]]
== Property accessor must not be overridden

This error indicates that a getter or a setter for a property annotated with `@ToBeReplacedByLazyProperty` has been overridden by user code.
Gradle is moving away from using JavaBean properties towards rich properties.
Properties marked with the annotation will be replaced by their rich counterparts with a getter returning `Property` or `ConfigurableFileCollection` etc.
When that happens, the overriding method in the user code would stop working.
...
class CustomJavaExec extends JavaExec {
...
To fix this problem, avoid overriding the getter or setter for the property.

[[property_must_not_be_overridden]]
