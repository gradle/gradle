...
The `gradleTestKit()` encompasses the classes of the TestKit, as well as the Gradle Tooling API client. It does not include a version of https://junit.org[JUnit], https://testng.org[TestNG], or any other test execution framework. Such a dependency must be explicitly declared.
...
The supported versions are defined in the <<compatibility.adoc#compatibility,compatibility matrix>>.
