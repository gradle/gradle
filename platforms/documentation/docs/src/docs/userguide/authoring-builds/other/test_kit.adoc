...
It is possible to use the GradleRunner to execute builds with Gradle 4.0 and later. However, some runner features are not supported on earlier versions. In such cases, the runner will throw an exception when attempting to use the feature.
