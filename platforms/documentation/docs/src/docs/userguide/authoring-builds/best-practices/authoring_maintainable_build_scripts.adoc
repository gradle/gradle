...
This particular plugin class lives in the <<multi_project_builds.adoc#sec:multiproject_buildsrc,`buildSrc` directory>> which makes it available to the build script automatically.
...
Strongly-coupled projects hurts <<structuring_software_products_details.adoc#sec:parallel_execution,build execution performance>> as well as readability and maintainability of code.
