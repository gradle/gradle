...
1. link:{javadocPath}/org/gradle/api/DomainObjectSet.html[`DomainObjectSet<T>`]: Represents a set of objects of type T. This set does not allow duplicate elements, and you can add, remove, and query objects in the set.

2. link:{javadocPath}/org/gradle/api/NamedDomainObjectSet.html[`NamedDomainObjectSet<T>`]: A specialization of `DomainObjectSet` where each object has a unique name associated with it. This is often used for collections where each element needs to be uniquely identified by a name.

3. link:{javadocPath}/org/gradle/api/NamedDomainObjectList.html[`NamedDomainObjectList<T>`]: Similar to `NamedDomainObjectSet`, but represents a list of objects where order matters. Each element has a unique name associated with it, and you can access elements by index as well as by name.

4. link:{javadocPath}/org/gradle/api/NamedDomainObjectContainer.html[`NamedDomainObjectContainer<T>`]: A container for managing objects of type T, where each object has a unique name. This container provides methods for adding, removing, and querying objects by name.

5. link:{javadocPath}/org/gradle/api/ExtensiblePolymorphicDomainObjectContainer.html[`ExtensiblePolymorphicDomainObjectContainer<T>`]: An extension of `NamedDomainObjectContainer` that allows you to define instantiation strategies for different types of objects. This is useful when you have a container that can hold multiple types of objects, and you want to control how each type of object is instantiated.
