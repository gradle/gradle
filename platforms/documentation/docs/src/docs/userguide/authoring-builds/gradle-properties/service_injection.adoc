...
1. link:{javadocPath}/org/gradle/api/model/ObjectFactory.html[ObjectFactory] - Allows model objects to be created.
2. link:{groovyDslPath}/org.gradle.api.file.ProjectLayout.html[ProjectLayout] - Provides access to key project locations.
3. link:{groovyDslPath}/org.gradle.api.file.BuildLayout.html[BuildLayout] - Provides access to important locations for a Gradle build.
4. link:{groovyDslPath}/org.gradle.api.provider.ProviderFactory.html[ProviderFactory] - Creates `Provider` instances.
5. link:{javadocPath}/org/gradle/workers/WorkerExecutor.html[WorkerExecutor] - Allows a task to run work in parallel.
6. link:{javadocPath}/org/gradle/api/file/FileSystemOperations.html[FileSystemOperations] - Allows a task to run operations on the filesystem such as deleting files, copying files or syncing directories.
7. link:{javadocPath}/org/gradle/api/file/ArchiveOperations.html[ArchiveOperations] - Allows a task to run operations on archive files such as ZIP or TAR files.
8. link:{javadocPath}/org/gradle/process/ExecOperations.html[ExecOperations] - Allows a task to run external processes with dedicated support for running external `java` programs.
9. link:{javadocPath}/org/gradle/tooling/provider/model/ToolingModelBuilderRegistry.html[ToolingModelBuilderRegistry] - Allows a plugin to registers a Gradle tooling API model.
...
TIP: It is preferable to let Gradle create objects automatically by using <<#managed_properties,managed properties>>.
...
link:{javadocPath}/org/gradle/api/file/FileSystemOperations.html[`FileSystemOperations`] is a service that provides methods for performing file system operations such as copying, deleting, and creating directories.
...
You can obtain an instance of `FileSystemOperations` using the `project.copy` property.
Here's a simple example:

====
include::sample[dir="snippets/providers/services/kotlin",files="build.gradle.kts[tags=file-system]"]
include::sample[dir="snippets/providers/services/groovy",files="build.gradle[tags=file-system]"]
====
...
It is part of the `org.gradle.api.tasks.bundling` package and is typically used in custom tasks or plugins to create archive files.

You can obtain an instance of `ArchiveOperations` using the `project.zipTree` property.
Here's a simple example:

====
include::sample[dir="snippets/providers/services/kotlin",files="build.gradle.kts[tags=archive-op]"]
...
====
include::sample[dir="snippets/providers/services/kotlin",files="build.gradle.kts[tags=exec-op]"]
include::sample[dir="snippets/providers/services/groovy",files="build.gradle[tags=exec-op]"]
====
