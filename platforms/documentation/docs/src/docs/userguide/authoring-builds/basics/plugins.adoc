...
=== 3. Applying plugins from the project root

You can apply plugins from the root project in a multi-project build to configure common logic and behavior across all subprojects. The root project is the project at the top of the directory hierarchy.
...
In the root `build.gradle(.kts)`, plugins are declared but not applied (via `apply false`).
This approach makes the plugins available to be explicitly applied in specific subprojects.
