...
Gradle's functionality primarily comes from plugins, including core plugins distributed with Gradle, third-party plugins, and script plugins in a specific build.
...
When you are working with plugins, you have options.
...
There are three types of plugins based on the context of the application:

1. *Project Plugins*: Implement `Plugin<Project>` and are applied in build scripts.
2. *Settings Plugins*: Implement `Plugin<Settings>` and are applied in settings scripts.
3. *Init Plugins*: Implement `Plugin<Gradle>` and are applied in init scripts.

Plugins can be implemented as *binary plugins* or as *precompiled script plugins*:

*Binary plugins* :: Plugins written either programmatically by implementing the link:{javadocPath}/org/gradle/api/Plugin.html[`Plugin`] interface or through declarations in either Groovy or Kotlin DSL.
+
They can reside within a build script, the project hierarchy, or externally in a plugin jar.

*Script plugins* :: Plugins written as additional build scripts that further configure the build and usually implement a declarative approach to manipulating the build.
+
...
|<<#sec:plugins_block,The `plugins` block>>
a|
[source,kotlin]
----
plugins {
...
|Apply a *core*, *community* or *local* plugin to multiple subprojects.
|<<#sec:buildsrc_plugins_dsl,The `buildSrc` directory>>
a|
[source,kotlin]
----
plugins {
...
|<<#sec:applying_plugins_buildscript,The `buildscript` block>>
a|
[source,kotlin]
----
buildscript {
...
|<<sec:script_plugins,The legacy `apply()` method>>
a|
[source,kotlin]
----
...
The `pluginManagement{}` block may only appear in the `settings.gradle(.kts)` file, where it must be the first block in the file:
