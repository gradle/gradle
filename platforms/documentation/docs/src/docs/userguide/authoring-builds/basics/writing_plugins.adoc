...
====
include::sample[dir="snippets/plugins/simple/kotlin", files="build.gradle.kts[tags=plugin-1]"]
include::sample[dir="snippets/plugins/simple/groovy", files="build.gradle[tags=plugin-1]"]
====
...
====
include::sample[dir="snippets/plugins/simple/kotlin", files="buildSrc/src/main/kotlin/MyConventionPlugin.kt[]"]
====

The plugin can be given an `id` using a `gradlePlugin{}` block so that it can be referenced in the root:

====
include::sample[dir="snippets/plugins/simple/kotlin", files="build.gradle.kts[tags=plugin-3]"]
include::sample[dir="snippets/plugins/simple/groovy", files="build.gradle[tags=plugin-3]"]
====
