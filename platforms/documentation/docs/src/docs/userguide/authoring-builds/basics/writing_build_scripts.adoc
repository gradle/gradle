...
When Gradle evaluates the settings file, it creates a single link:{javadocPath}/org/gradle/api/Settings.html[`Settings`] instance.
...
As a build script executes, it configures either a link:{javadocPath}/org/gradle/api/Settings.html[`Settings`] object or link:{javadocPath}/org/gradle/api/Project.html[`Project`] object and its children.
...
Therefore, each block corresponds to a method on the link:{javadocPath}/org/gradle/api/Project.html[`Project`] API and is evaluated with a *delegate* or *receiver* (more on that below).
