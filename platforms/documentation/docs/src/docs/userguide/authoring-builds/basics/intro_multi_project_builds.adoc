...
1. **<<multi_project_builds.adoc#sec:multibuild_buildSrc,Multi-Project Builds using buildSrc>>** - where `buildSrc` is a subproject-like directory at the Gradle project root containing all the build logic.
2. **<<composite_builds.adoc#composite_builds,Composite Builds>>** - a build that includes other builds where `build-logic` is a build directory at the Gradle project root containing all the build logic.

image::multi-project-standards.png[]

=== 1. Multi-Project Builds using `buildSrc`

Multi-project builds are great for organizing projects with many modules.

For example, a build that has many modules called `mobile-app`, `web-app`, `api`, `lib`, and `documentation` should be structured as follows:
...
The `buildSrc` directory is automatically recognized and compiled by Gradle.
It is a good place to define and maintain shared configuration or imperative build logic, such as custom tasks or plugins.

The compiled code from the `buildSrc` is put in the classpath of the root build script, making it available to any subproject (`web-app`, `mobile-app, `lib`, etc...) in the build.

The `buildSrc` directory is treated as a subproject and follows the same structure including a build script and common logic written in Groovy or Kotlin.

Consult how to declare <<multi_project_builds.adoc#sec:multibuild_buildSrc, dependencies between subprojects>> using `include` to learn more.

=== 2. Composite Builds

Composite Builds, also referred to as _include builds_, are best for sharing logic between projects (_not subprojects_) or isolating access to shared build logic (i.e., convention plugins).

Let's take the previous example.
The logic in `buildSrc` has been turned into a plugin that can be published and worked on independently.

The plugin is now its own build called `build-logic` with its own build script and settings file:
...
The root settings file includes the `build-logic` build:
