...
Multi-project builds are great for organizing projects with many modules such as `mobile-app`, `web-app`, `api`, `lib`, and `documentation`:
...
These modules will have dependencies between them such as `web-app` and `mobile-app` depending on `lib`.
This means that in order for Gradle to build `web-app` or `mobile-app`, it must build `lib` first.
The root settings file will look as follows:
...
The `shared-build-conventions` plugin is now its own project because the owner has decided to publish it to the Gradle Plugin Portal and wants to be able to work on it independently.

The plugin has now become its own project AND it is used as shared build logic:
...
The root settings file now includes the separate build called `build-logic`:
...
See how to declare <<multi_project_builds.adoc#sec:multibuild_buildSrc, dependencies between subprojects>> using `include` and how to <<composite_builds#defining_composite_builds,create composite builds>> with `includeBuild`.
