...
You just registered a task called `hello` and gave it an action using `doLast{}`.
...
Tasks are grouped into types.

You can reuse built-in types like:
...
You can also create your own types by extending `DefaultTask`.
