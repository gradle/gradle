...
By default, a *task* is of *type* `Task` .
...
When you run `./gradlew hello`, it prints your message:
...
When you register a *task* in your build script, you can:

- Use the default *task type* (`Task`) and define the behavior inline.
...
In the example below, the `HelloTask` class, a *custom task type*, is created by extending link:{javadocPath}/org/gradle/api/DefaultTask.html[`DefaultTask`]:
...
Then, a `hello` *task* is registered with the new *type* `HelloTask`.
...
For a *task* to do useful work, it typically needs some *inputs* which it uses to produce *outputs*.
...
*Task actions* are the blocks of code that define what the *task* does when it runs.
...
[.text-right]
**Next Step:** <<plugins.adoc#using_plugins,Learn about Plugins>> >>
