...
As a build grows in complexity, knowing when and where a particular value is configured is difficult to track. Gradle provides several ways to manage this complexity using *lazy configuration*.

[[lazy_properties]]
== Lazy properties

Gradle provides lazy properties, which delay the calculation of a property’s value until it’s actually required.
...
3. Build authors can avoid resource intensive work during the configuration phase, which can have a large impact on build performance. +
For example, when a configuration value comes from parsing a file but is only used when functional tests are run, using a property instance to capture this means that the file is parsed only when the functional tests are run, but not when, for example, `clean` is run.

Gradle represents lazy properties with two interfaces:

1. link:{javadocPath}/org/gradle/api/provider/Provider.html[Provider] represents a value that can only be queried and cannot be changed.
...
2. link:{javadocPath}/org/gradle/api/provider/Property.html[Property] represents a value that can be queried and also changed.
...
This typically happens during the execution phase.
For more information about the Gradle build phases, please see <<build_lifecycle.adoc#sec:build_phases,Build Lifecycle>>.
...
TIP: You should use `map()`, as it has some useful benefits, which we will see later.
...
When writing a plugin or build script with Groovy, you can use the `map(Transformer)` method with a closure and Groovy will take care of converting the closure to a `Transformer`.
...
Here's an example where the property of a task is connected to a property of a project extension:
...
There are more strongly typed models types used to represent elements of the file system:
...
To make this work, we would need to configure each task to know where to look for its inputs and place its outputs, make sure that the producing and consuming tasks are configured with the same location, and attach task dependencies between the tasks.
...
The `Property` API makes this easier by keeping track of not only the value for a property but also the task that produces the value.
...
There are some additional methods to allow entries with lazy values to be added to the map.
...
Often you want to apply some _convention_, or default value, to a property to be used if no value has been configured for the property. You can use the `convention()` method for this.
...
Most properties of a task or project are intended to be configured by plugins or build scripts and then the resulting value used to do something useful.

For example, a property that specifies the output directory for a compilation task may start off with a value specified by a plugin, then a build script might change the value to some custom location, then this value is used by the task when it runs.
However, once the task starts to run, we want to prevent any further change to the property.
...
To see those guidelines in action, have a look at https://github.com/gradle/gradle-site-plugin[gradle-site-plugin], a Gradle plugin demonstrating established techniques and practices for plugin development.
