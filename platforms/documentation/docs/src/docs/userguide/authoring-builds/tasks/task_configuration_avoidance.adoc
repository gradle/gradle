...
| OK to use because it does not require tasks to be created immediately.
...
| `named()` is the closest equivalent, but will fail if the task does not exist. Using `findByName()` will cause tasks registered with the new API to be created/configured.

| link:{javadocPath}/org/gradle/api/tasks/TaskContainer.html#findByPath-java.lang.String-[TaskContainer.findByPath()]
| Do not use.

| link:{javadocPath}/org/org/gradle/api/tasks/TaskCollection.html#getByName-java.lang.String-[TaskCollection.getByName()]
| Do not use.
