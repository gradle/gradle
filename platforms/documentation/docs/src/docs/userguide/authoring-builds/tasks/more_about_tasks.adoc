...
Let's explore this by adding the <<base_plugin.adoc#base_plugin,Gradle core `base` plugin>> to our `app` build script.
...
Actions include compiling code, zipping files, publishing archives, and much more.
...
Actionable tasks are added by plugins like the <<java_library_plugin.adoc#java_library_plugin,`java-library` plugin>>.
...
We see that there are many new tasks available such as `jar` and `testClasses`.
...
The build cache particularly useful if you switch between branches regularly.
...
We then add this dependency in the build script of the `app`:
...
Registration is commonly done with the link:{javadocPath}/org/gradle/api/tasks/TaskContainer.html#register-java.lang.String-java.lang.Class-[`register()`] method and configuring a task is commonly done with the link:{javadocPath}/org/gradle/api/tasks/TaskCollection.html#named-java.lang.String-java.lang.Class-[`named()`] method:
...
<2> Configure the registered `myCopy` task with the inputs and outputs it need according to its link:{javadocPath}/org/gradle/api/tasks/Copy.html[API].
=====
[.multi-language-sample]
=====
[source,groovy]
...
Since the task is of type `Copy`, a Gradle supported task type, it can be configured using its API (see link:{javadocPath}/org/gradle/api/tasks/Copy.html[Copy]).
...
Defining dependencies using task names and the link:{javadocPath}/org/gradle/api/DefaultTask.html#dependsOn-java.lang.Object...-[`dependsOn()`] method is the simplest way.
