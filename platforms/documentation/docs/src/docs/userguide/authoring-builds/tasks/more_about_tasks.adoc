...
Gradle supports tasks.
Such tasks are either provided by you or built into Gradle.
Gradle provides tasks with common functionality like zipping a set of files or copying files from one directory to another.
If users need functionality that is not provided by Gradle tasks, they can create their own custom tasks.
...
`hello` and `copy` are `TaskProvider`s in the example.
...
This avoids duplication of code and minimizes redundancy when configuring multiple tasks:
...
Configure a task immediately upon registering it:
...
This can be done in Groovy only.
It forces the task to be realized:
...
NOTE: This breaks task configuration avoidance and is not recommended!

=== 4. Using a `TaskProvider`:

Great for task configuration avoidance if a task reference is needed in multiple places in the build script:
...
The action is defined in the configuration phase, but executed in the execution phase.
...
So if you only run `taskB` with `mustRunAfter`, you won't cause `taskA` to run.
...
Gradle tasks will only show up when running `tasks` if the `task.group` is set, or no other task depends on it.
Such a task is likely meant to be executed directly by the user.

In contrast, `./gradlew tasks --all` will show all tasks.
