...
In the Kotlin DSL there is a specific link:https://kotlinlang.org/docs/reference/delegated-properties.html[delegated properties] syntax useful if you need the register a task for further reference:
...
You should use the methods that return a _task provider_:

- `register()`
- `named()`

This ensures that you do not break <<task_configuration_avoidance#task_configuration_avoidance,task configuration avoidance>>.

Here is an example accessing a task via the `tasks` collection:
...
This avoids duplication of code and reduces redundancy:
...
WARNING: The following shows how to access a task by path. This is not a recommended practice.
...
--
A task has both configuration and action.

When using `doLast`, you are simply using a shortcut to define an action.
Code defined in the configuration section of your task will get executed during the <<build_lifecycle.adoc#build_lifecycle,configuration phase of the build>> regardless of what task was targeted.
--
...
If the task class has a `@Inject` constructor, you can create a task by passing the constructor arguments at the end of the parameter list:
...
In <<tutorial_using_tasks.adoc#sec:task_dependencies,Task dependencies>> you were introduced to defining dependencies using task names.
...
For more advanced use-cases, you can define a task dependency using a lazy block.
...
In some cases it is useful to control the _order_ in which 2 tasks will execute, without introducing an explicit dependency between those tasks.
...
Secondly when using parallel execution and all dependencies of a task have been satisfied apart from the “should run after” task, then this task will be run regardless of whether its “should run after” dependencies have been run or not.
