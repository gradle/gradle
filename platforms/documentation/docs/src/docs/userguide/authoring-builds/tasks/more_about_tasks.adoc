...
Let's take a very basic Gradle project as an example with the following structure:
...
Currently, the project's build file is empty.
...
Other tasks, such as tasks that build your project or compile your code, are added by plugins.

Let's explore this by adding the `base` plugin to our `app` build script, a Gradle core plugin that adds central lifecycle tasks:
...
Gradle distinguishes between two types of tasks:
...
Actionable tasks are added by plugins like the `java-library` plugin.

Let's update the build script of the previous example so that our `app` subproject is a Java library:
...
Once again we list the available task to see what new tasks are available:
...
There are many new tasks available such as `jar` and `testClasses`.

Additionally, actionable tasks have been wired to lifecycle tasks by the `java-library` plugin.

If we call the `:build` task, we can see that several tasks have been executed including the `:app:compileJava` task:
...
The `build` lifecycle task has been wired to `compileJava`, `testClasses`, etc.
...
Gradle has the capability to reuse results from prior builds.
...
Gradle marks the production code as `UP-TO-DATE`, indicating that it remains unchanged since the last successful build:
...
Gradle can reuse results from past builds.

To enable this feature, activate the Build Cache by using the `--build-cache` command line parameter or by setting `org.gradle.caching=true` in the `gradle.properties` file.

This optimization has the potential to significantly accelerate your builds:
...
2 actionable tasks: 1 executed, 3 from cache
----

When Gradle is able to fetch outputs of a task from the cache, it labels the task with `FROM-CACHE`.

This is particularly useful if you switch between branches regularly.
...
Let's add a dependency to our example `app` subproject and turn into an application by modifying the settings and build files:
...
dependencies {
    implementation(project(":some-logic"))
}
----
=====
[.multi-language-sample]
...
The `app` subproject now depends on another subproject called `some-logic`.

If we now run `:build` again, we see the Java code of `some-logic` is also compiled:
...
When developing Gradle tasks, there are three states to consider:

1. *Registering* a task - using a task class (i.e., *task type*).
2. *Configuring* a task - defining inputs and outputs for a registered task.
3. *Implementing* a task - creating a custom task class (i.e., *custom task type*).
...
Tasks are defined using strings for tasks names:
...
In the example above, the task is added to the `tasks` collection using the `register()` method in link:{javadocPath}/org/gradle/api/tasks/TaskContainer.html[TaskContainer].

[[sec:configuring_tasks]]
== Configuring tasks

Gradle tasks need to be configured in order to successfully complete their action(s).
...
Since the task is of type `Copy`, a Gradle supported task type, it can be configured using its API (see link:{groovyDslPath}/org.gradle.api.tasks.Copy.html[Copy]).
...
If Gradle does not provide a task type that meets your build logic needs, you can implement a custom task type.

To create a custom task class, you extend link:{groovyDslPath}/org.gradle.api.DefaultTask.html[`DefaultTask`] and make the extending class abstract:
...
You can learn more about developing custom task types in <<implementing_tasks.adoc#implementing_tasks,Implementing Tasks>>.

[[sec:configuration_avoidance]]
== Task configuration avoidance

In order to significantly reduce build time, Gradle provides the configuration avoidance API.

The configuration avoidance API avoids configuring tasks if they will not be used for a build.
...
To avoid creating and configuring a task not needed for a build, you should use the `register()` method instead of the `create()` method in link:{javadocPath}/org/gradle/api/tasks/TaskContainer.html[TaskContainer].

When a task is registered using the `register()` method, it is known to the build.
...
Note that `Y.mustRunAfter(X)` or `Y.shouldRunAfter(X)` does not imply any execution dependency between the tasks:

* It is possible to execute tasks `X` and `Y` independently. The ordering rule only has an effect when both tasks are scheduled for execution.
* When run with `--continue`, it is possible for `Y` to execute in the event that `X` fails.
