...
With custom type tasks, there's no need to implement task behavior as done in simple tasks. Instead, you can declare the task and configure it using its properties. Custom type tasks let you reuse a piece of behavior in many different places, possibly across different builds.
