...
The other type of task is the *enhanced task*, where the behavior is built into the task, and the task provides some properties that you can use to configure the behavior.

Most Gradle plugins use enhanced tasks.

With enhanced tasks, you don't need to implement the task behavior as you do with simple tasks.
You declare the task and configure the task using its properties.
In this way, enhanced tasks let you reuse a piece of behavior in many different places, possibly across different builds.

The task's class defines the properties and behavior of an enhanced task.
When you declare an enhanced task, you specify the *type*, or class of the task.

Implementing a custom task class in Gradle is easy.
...
TIP: You don't have to use a method to define the behavior for the task.
...
Now we will move our `greeting` task to a standalone project; so we can publish it and share it with others.
...
When an _output_ file is deleted (or modified), then Gradle cannot determine which input files are out of date.
...
Being able to pass in property values on the command line is particularly helpful if they change more frequently.
...
There are a few rules to use options on the command line:
...
Continuing from the previous example, the build script creates a task instance of type `UrlVerify` and provides a value from the command line through the exposed option:
...
Passing the option on the command line also requires a value that represents a path, e.g., `--output-file=file.txt` or `--output-dir outputDir`. +
Relative paths are resolved relative to the project directory of the project that owns this property instance, see link:{javadocPath}/org/gradle/api/file/FileSystemLocationProperty.html#set-java.io.File[FileSystemLocationProperty.set()].
...
This example below demonstrates the use of multiple options for a single task.
...
In a single-threaded execution, these work units would execute in sequence; however, if we have multiple processors, it would be desirable to perform independent work units concurrently.
...
In order to submit work to the Worker API, two things must be provided:
...
A `WorkAction` implementation can inject services that provide capabilities during work execution, such as the link:{javadocPath}/org/gradle/api/file/FileSystemOperations.html[FileSystemOperations] service in the example above.
...
If any failures occur while executing the asynchronous work, the task will fail and a link:{javadocPath}/org/gradle/workers/WorkerExecutionException.html[WorkerExecutionException] will be thrown detailing the failure for each failed work item.
...
This states that the work should be run in a thread with a minimum of isolation. +
...
This states that the work should be run with a maximum level of isolation by executing the work in a separate process. +
...
.Example of a verification failure allowing a consumer task to run
====
