...
Exceptions thrown during task execution result in task failure which typically immediately terminates a build with a build failure result (except when running with the <<command_line_interface.adoc#sec:continue_build_on_failure, `--continue` flag>>).

Another exception to this rule is when a task throws a link:{javadocPath}/org/gradle/api/tasks/VerificationException.html--[VerificationException].
These exceptions are used to indicate that a task has failed in a controlled manner such that its output is still valid.
Consumer tasks that use this output can still run.

A failed test task, for instance, will cause a failing outcome for the test task.  However, this shouldn't prevent another task from reading and processing the (valid) test results output files it produced.
Verification failures are used in exactly this manner by the <<test_report_aggregation_plugin.adoc, `Test Report Aggregation Plugin`>>.
