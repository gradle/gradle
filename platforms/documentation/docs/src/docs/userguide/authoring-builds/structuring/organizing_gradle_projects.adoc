...
The `gradle.properties` file provides a clean and centralized way to define reusable configuration values.
It’s especially suited for <<build_environment.adoc#sec:gradle_configuration_properties,build environment settings>>, such as:
...
- A `gradle.properties` file in your `<<directory_layout.adoc#dir:gradle_user_home,GRADLE_USER_HOME>>` directory (for user-wide settings)
...
Enterprises often want to standardize the build environment across all projects by enforcing common conventions and rules.
One way to do this is by using <<init_scripts.adoc#init_scripts,Initialization scripts>>, which make it easy to apply shared build logic on a single machine—for example, by configuring an in-house repository and its credentials.

However, relying on local init scripts has drawbacks.
You must communicate the setup process to every developer, and updating the logic consistently across all environments can be difficult.

A custom Gradle distribution offers a more reliable solution.
It consists of the standard Gradle distribution bundled with one or more custom initialization scripts.
These scripts are applied automatically with every build.
Developers only need to configure the checked-in Gradle Wrapper to point to the URL of the custom distribution.

Custom distributions can also include a `gradle.properties` file at the root of the distribution archive to provide an organization-wide set of build environment properties.

To create a custom Gradle distribution:

1. Download and repackage a standard Gradle distribution.
2. Add one or more initialization scripts with the desired logic.
3. Include the scripts in the distribution archive.
4. Upload the archive to an HTTP server.
5. Update each project's Wrapper to use the custom distribution URL.
