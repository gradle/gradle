...
The product built in this sample is an application that displays link:https://gradle.org/releases/[Gradle Build Tool releases].

There are different ways to work with the sample:

1. You may build or import the umbrella build in the root. +
For example, run the Spring Boot web application via `./gradlew :server-application:app:bootRun` or install the Android app using `./gradlew :android-app:app:installDebug`.
2. You may only build or import one of the application builds directly. +
For example, `cd server-application` and run the app using  `../gradlew :app:bootRun`.
3. You may only build or import a selected component (and its dependencies). +
...
As software projects grow, it is common to organize large systems into connected components.
...
You can look at Gradle as a modeling tool for your software:

1. It allows you to describe the architecture of your software in a model written in Gradle's DSLs.
2. The build tool Gradle can then interpret this model to build the software.

To exemplify these features, we explore a sample project with the following architecture:

image::software_architecture.png[]

The structure follows a typical setup that can be found in a similar form in many commonly used software architectures.
...
In the sample, we build a Spring Boot application that supports both features via a web browser.
...
- The _aggregation_ component contains the setup of the delivery pipeline that is required to push the product to production and do automated end-to-end testing as part of that. +
...
Concretely, the application lists Gradle releases with links to release notes (_user_ feature) and offers an administration interface for the range of releases to be listed (_admin_ feature).
====
...
This root folder can be used as the root of a Git repository, for example.

This is only the setup of the sample.
You can freely choose where to locate your components physically.
...
It's up to you to decide what works best for you, the software you build, and the teams working on it.
...
We need to assign a type to the project to inform Gradle of the purpose of such files.
...
For example, if your project should be a "Java Library", it would apply the `java-library` plugin and configure details such as setting the Java version to 11.
You can add details like that directly in `release/build.gradle(.kts)` but then you would have to repeat them in other components containing "Java Library" projects.
...
You can find more information about writing convention plugins in the section on <<sharing_build_logic_between_subprojects.adoc#sharing_build_logic_between_subprojects,sharing build logic>> and the link:../samples/sample_convention_plugins.html[associated sample].
