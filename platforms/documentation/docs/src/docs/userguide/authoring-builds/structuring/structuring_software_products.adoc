...
2. a _state_ component that is responsible for managing a modifiable state of the model during application runtime.
...
Our components rely on external components that are retrieved from binary repositories.
The Spring Boot and Android frameworks.
...
It defines a Java version to use and configurest the test framework. +
...
3. The _aggregation_ component contains the setup of the delivery pipeline that is required to push the product to production and do automated end-to-end testing as part of that. +
...
When we look at the `domain-model` build settings file, we see that `release` is a subproject:
...
Let's zoom into the _domain-model_ component again:
...
The simplest thing you can do is to apply one of Gradle's core plugins such as `java-library`.

You then have additional configuration to do in the context of the product you are building.
For example, if you apply the `java-library` plugin, you must configure details such as setting the Java version to 11.
Configuration is typically done in the build file `domain-model/build.gradle(.kts)`:

The custom type `com.example.kotlin-library` is applied to the `domain-model` component:
====
...
It's important to note that `com.example.kotlin-library` is applied to several other components as well including `state` and `admin-feature`.
...
The `build-logic` component in the sample has several projects that each define a project type through a convention plugin:
...
The convention plugin for Spring applications looks like this:
...
We apply our own `com.example.commons` plugin, which is another convention plugin that, among other things, configures the Java version we are targeting and adds a dependency to a platform (`com.example.platform:product-platform` from our `platforms` component).
And we apply the spring boot plugin.
We also add two dependencies that Spring Boot projects should have.

== Connecting components

The production code components depend on each other.

To make components (builds) know to each other, you use the `includeBuild` statement in the settings file.
This is not adding a dependency between (projects of) components. It just makes the physical location of one component known to another.
...
To declare dependencies between projects of components (subprojects in builds), you use the `dependencies { }` block of a `build.gradle(.kts)` file: `implementation("com.example.platform:product-platform")`.
