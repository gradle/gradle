...
3. There is shared build logic used by `shared`, `api`, and `person-service` in `buildSrc`.
...
Shared build logic is extracted into a _convention plugin_ in `buildSrc` that is applied in the subprojects' build scripts that also define project dependencies:
...
<<#sec:project_jar_dependencies,Project dependencies>> model dependencies between subprojects (i.e., modules).
