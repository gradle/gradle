...
[[tweaking_structure]]
= Multi-project Build Considerations

There are more considerations when structuring and building a software product with Gradle such as umbrella builds, components isolation, multi- and mono-repo setups, and re-using build login in convention plugins.

== Umbrella builds

If all your builds are located in one folder structure, an *umbrella build* in the root folder can include all builds.
...
The <<gradle_wrapper.adoc#sec:adding_wrapper,Gradle wrapper>> should be located in the root as well.

link:../samples/sample_structuring_software_projects.html[The sample] in this section is an umbrella build.
...
In your IDE, you can import the umbrella build and the component builds will be visible in the workspace.
...
Multi-repo development possibly needs some additional guidelines and tooling, so that builds can still find each other.
...
To work with binary versions of certain components instead of the source versions, you can
<<declaring_repositories.adoc#sub:centralized-repository-declaration,add the repository you published>> instead of the corresponding `includeBuild("...")` statements in your `settings.gradle(.kts)` file.
You simply need to define versions for the components, ideally in a <<platforms.adoc#sec:sharing-dep-versions-between-projects,platform project>>.
...
You simply need to <<publishing_gradle_plugins.adoc#custom-plugin-repositories,declare the repositories you want to publish to>> in your build the same way you do it for other components.
