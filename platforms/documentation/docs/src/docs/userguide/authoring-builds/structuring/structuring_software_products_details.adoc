...
There are more considerations when structuring and building a software product with Gradle: umbrella builds, components isolation, multi- and mono-repo setups, and re-using build login in convention plugins.
...
When publishing build logic components, the `maven-publish` will also publish <<plugins.adoc#sec:plugin_markers,plugin markers>> that allow Gradle to find plugins by ID – even if they are located in a repository.
