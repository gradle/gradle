...
Instead of repeating the same included dependencies in each subproject build script, Gradle provides a special directory that serves as a location for storing shared build logic and plugins.

[[sec:using_buildsrc]]
== Share logic in `buildSrc`

`buildSrc` is a Gradle-recognized and protected directory which comes with a number of benefits:
...
This helps in keeping the main build scripts cleaner and more focused on project-specific configurations.
...
apply(from = project(":buildSrc").file("shared.gradle.kts"))
...
// Use the build script defined in buildSrc
apply from: project(':buildSrc').file('shared.gradle')
...
We can write a plugin that encapsulates the build logic common to a number of subprojects in a project.
This kind of plugin is called a *convention plugin*.

While writing plugins is outside the scope of this section, putting common build logic in a convention plugin located in the `buildSrc` is the recommended way to build a Gradle project.
