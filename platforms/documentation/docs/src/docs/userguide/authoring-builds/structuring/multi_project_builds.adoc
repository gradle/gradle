...
NOTE: Setting the link:{groovyDslPath}/org.gradle.api.Project.html#org.gradle.api.Project:projectDir[Project.projectDir] property is important when locating projects in nested subdirectories to <<best_practices_general.adoc#avoid_empty_projects,avoid unintentionally creating empty projects>>.
