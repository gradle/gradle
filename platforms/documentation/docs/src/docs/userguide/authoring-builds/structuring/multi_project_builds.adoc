...
It is important to structure your Gradle project in order to optimize build performance.
...
Multi-project builds contain a single root project in a directory that is considered the top of the path for Gradle (i.e., `.`).

Subprojects are located, physically, under the root path.
...
The root project is called `basic-multiproject`, it is located somewhere on your machine.
...
You can view the structure of a multi-project build by running the `gradle projects` command:
