...
The following examples demonstrate how two Gradle builds, normally developed separately, can be combined into a composite build.
...
For example, the output of `gradle run --include-build ../my-utils`:
...
To execute the `run` task in the `my-app` build, the composite build must define a delegating task:
...
In general, interacting with a composite build is similar to a regular multi-project build.
Tasks can be executed, tests can be run, and builds can be imported into the IDE.

[[composite_build_executing_tasks]]
=== Executing tasks

Tasks from an included build can be executed from the command-line or your IDE in the same way as tasks from a regular multi-project build.
