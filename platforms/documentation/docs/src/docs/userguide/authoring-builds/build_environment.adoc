...
The following table lists these methods in order of highest to lowest precedence (the first one wins) with the added example of setting the `JAVA_HOME`:

[cols="~,~,~,~"]
|===
|Priority |Method |Location |Example
...
----
2. *Command Line:* You can set project properties with specially-named gradle properties using the `-D` command line flag.
+
[source,text]
----
$ ./gradlew build -Dorg.gradle.project.myProperty='Hi, world'
----
3. *Gradle Properties:* You can set project properties with specially-named gradle properties in the `gradle.properties` file.
+
...
----
2. *Gradle Properties:* You can also set system properties in `gradle.properties` files with the prefix `systemProp`.
+
.gradle.properties
[source,text]
----
...
----
2. *Gradle Properties:* Place these settings into a `gradle.properties` file and commit it to your version control system.
+
.gradle.properties
[source,properties]
----
...
== Controlling the JVM used by the build
