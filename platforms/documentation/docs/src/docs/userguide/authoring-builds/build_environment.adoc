...
Project properties can be retrieved:

- Lazily: `providers.gradleProperty()`
- Eagerly: `project.ext()`, `project.findProperty()`, `project.property()`, `project.('<propertyName>')`

If a project property is referenced but does not exist, an exception may be thrown and the build will fail.
You should check for the existence of a project property before directly accessing it with `project.('<propertyName>')` by using the link:{groovyDslPath}/org.gradle.api.Project.html#org.gradle.api.Project:hasProperty(java.lang.String)[`project.hasProperty()`] method.
...
----
2. *As a System Property:* Gradle creates specially-named system properties for project properties which you can set using the `-D` command line flag or `gradle.properties` file. For the project property `myProperty`, the system property created is called `org.gradle.project.__prop__`.
+
[source,text]
----
...
myProperty='Hi, world'
----
4. *Environment Variables:* You can set project properties with environment variables. If the environment variable name looks like `ORG_GRADLE_PROJECT___prop__=somevalue`, then Gradle will set a `prop` property on your project object, with the value of `somevalue`.
+
[source,text]
----
...
System properties can be retrieved:

- Lazily: `providers.systemProperty()`
- Eagerly: `System.getProperty()`
...
=== Gradle properties should not be used in build logic.

[CAUTION]
====

Gradle properties should not be retrieved using the `Project` object or the `ProviderFactory` class due to the following behavior.
If a `build.gradle.kts` file contains the following task:

[source,kotlin]
.build.gradle.kts
----
tasks.register("gradlePropertyTest") {
    val isCI = providers.gradleProperty("org.gradle.caching.debug")
    doLast {
        println(isCI.get())
    }
}
...
And the `gradle.properties` file contains the following line:
...
Then running the `gradlePropertyTest` task yields the following results:

[source,text]
----
$ ./gradlew gradlePropertyTest
...
> Task :model:gradlePropertyTest
verbose
----

The console output is verbose and therefore correct.
Let's see what happens when we run the same task with the added command line option (note that the `gradle.properties` file has not changed):

[source,text]
----
$ ./gradlew :gradlePropertyTest -Dorg.gradle.console=auto

> Task :model:gradlePropertyTest
verbose
----

As expected, the console output is short.
However, the output of `providers.gradleProperty("org.gradle.caching.debug")` is incorrectly `verbose`.
====
...
- Lazily: `providers.environmentVariable()`
- Eagerly: `System.getenv()`
