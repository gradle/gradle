...
All discussions in this section revolve around a sample project called the "URL verifier plugin."
...
It ensures that the software works on a functional and non-functional level before it is released to the end user.
Automated testing also enables the development team to refactor and evolve the code without fearing to introduce regressions in the process.
...
The testing pyramid introduced by Mike Cohen in his book link:Succeeding with Agile: Software Development Using Scrum[Succeeding with Agile: Software Development Using Scrum] describes three types of automated tests:

- *Unit testing* aims to verify the smallest unit of code, usually a method.
Unit tests usually do not interact with other parts of the system (e.g., a database or the file system) and use Stubs or Mocks to cut off interactions.
POJOs and utility classes are good candidates for unit tests as they are self-contained and do not use the Gradle API.
...
- *Functional testing* tests the system from the end user's perspective. End-to-end tests for Gradle plugins stand up a build script, apply the plugin under test, and execute the build with a specific task. The outcome of the build (e.g., standard output/error or generated artifacts) verifies the correctness of the functionality.

=== Tooling support

Implementing manual and automated testing for Gradle plugins is straightforward with the right tooling.
The table below gives a brief overview of how to approach each test type.
You have the free choice of using the test framework you are most familiar with.

For a detailed discussion and code examples, please refer to the dedicated sections further down:
...
Next, you'll learn how to organize and implement automated tests for Gradle plugins.
...
Additionally, if you apply the Groovy plugin, source code under the directory `src/test/groovy` is considered for compilation.
...
In the next section, you will learn how to configure those source directories for compilation and test execution. You can also rely on third-party plugins for convenience, such as the link:https://github.com/nebula-plugins/nebula-project-plugin#nebula-facet-plugin[Nebula Facet plugin]n or the link:https://github.com/unbroken-dome/gradle-testsets-plugin[TestSets plugin].
...
By pointing an instance of a source set to one or many source code directories, Gradle automatically creates a corresponding compilation task.
...
Source sets are responsible for compiling source code but do not deal with executing the bytecode.
...
IMPORTANT: When writing unit tests, it's important to test boundary conditions and various forms of invalid input. Try to extract as much logic as possible from classes that use the Gradle API to make it testable as unit tests. It will result in maintainable code and faster test execution.
