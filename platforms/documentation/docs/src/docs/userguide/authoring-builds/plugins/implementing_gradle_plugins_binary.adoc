...
For example, the Java Base plugin provides un-opinionated functionality like `SourceSets`, while the Java plugin adds tasks and conventions familiar to Java developers like `classes`, `jar` or `javadoc`.
...
Plugins often come with default <<sec:plugin_conventions,conventions>> that make sensible assumptions about the consuming project.
The Java plugin, for example, searches for Java source files in the directory `src/main/java`.

Default conventions help streamline project layouts but fall short when dealing with custom project structures, legacy project requirements, or a different user preference.
...
Let's assume you want to do something with the value of `serverUrl` once captured.
The exposed extension property is often directly mapped to a task property that uses the value when performing work.

Let's look at the internals of the plugin `BinaryRepositoryVersionPlugin` to give you a better idea.
...
Gradle automatically initializes the abstract getters for the properties.
...
You might want to expose a way for users to define multiple, named data objects of the same type.
...
The drawback to this approach is that it automatically forces the project to apply the Java plugin, imposing a strong opinion on it.
