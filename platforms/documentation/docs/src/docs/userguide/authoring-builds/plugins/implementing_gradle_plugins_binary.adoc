...
There are many key aspects of developing Gradle binary plugins, including a proper ID, a helper plugin, and extension objects.

[[plugin-development-plugin]]
== Using the Plugin Development plugin

The <<java_gradle_plugin.adoc#java_gradle_plugin,Gradle Plugin Development plugin>> can be used to assist in the development of Gradle plugins.
...
=== Conventions
Convention properties are properties that are initialized with default values and can be overridden by the user in their build script.
For example:
...
class MyPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        // Define a convention property with a default value
        project.extensions.create("myPluginExtension", MyPluginExtension::class.java)

        // Set default values for the convention property
        project.extensions.getByType(MyPluginExtension::class.java).message = "Hello, World!"

        // Add a task that uses the convention property
        project.task("myTask") {
            doLast {
                println(project.extensions.getByType(MyPluginExtension::class.java).message)
            }
        }
    }
}
...
class MyPlugin implements Plugin<Project> {
    void apply(Project project) {
        // Define a convention property with a default value
        project.extensions.create('myPluginExtension', MyPluginExtension)

        // Set default values for the convention property
        project.myPluginExtension.message = 'Hello, World!'

        // Add a task that uses the convention property
        project.task('myTask') {
            doLast {
                println project.myPluginExtension.message
            }
        }
    }
}
...
In this example, `MyPluginExtension` is a class that represents the convention.
The message property is the convention property with a default value of 'Hello, World!'.
...
The <<java_plugin.adoc#java_plugin,Java plugin>> provides convention properties like `sourceCompatibility` and `targetCompatibility` with default values.
Users can customize these properties in their build scripts to specify the Java version compatibility for their project:
...
The link:https://developer.android.com/build[Android Gradle Plugin] provides convention properties like `compileSdkVersion` and `minSdkVersion` with default values.
Users can customize these properties in their build scripts to specify the Android SDK versions for their app:
...
Plugins often times come with default <<sec:plugin_conventions,conventions>> that make sensible assumptions about the consuming project.
...
It allows developers to simplify code like `obj.prop.set 'foo'` to `obj.prop = 'foo'`.
...
At the moment the only other parameter stored by the data object is a URL.
