...
When reporting problems, Gradle will summarize similar problems before sending them to any progress event listener, including the  link:{javaDocPath}/org/gradle/tooling/events/ProgressListener.html[Tooling API ProgressListener].

- Initially, the _first_ few instances of a problem are reported as a link:{javaDocPath}/org/gradle/tooling/events/problems/Problem.html[Problem], providing all information available for that problem.
- Subsequent occurrences of the same problem are grouped and summarized as a link:{javaDocPath}org/gradle/tooling/events/problems/ProblemSummary.html[ProblemSummary], which is delivered by a link:{javaDocPath}org/gradle/tooling/events/problems/ProblemSummariesEvent.html[ProblemSummariesEvent] at the end of the build with the total count of occurrences.
