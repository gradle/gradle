...
The plugin implementation declares a custom configuration that allows for <<declaring_dependencies.adoc#declaring-basic-dependencies,assigning those external dependencies with dependency coordinates>>:
...
<2> Assign the <<variant_attributes.adoc#sub:gradle_plugins_default_attributes,Gradle API version attribute>> to all <<declaring_dependencies_adv.adoc#sec:resolvable-consumable-configs,consumable configurations>> of our Gradle7 variant. Gradle uses this information to determine which variant to select during plugin resolution.
