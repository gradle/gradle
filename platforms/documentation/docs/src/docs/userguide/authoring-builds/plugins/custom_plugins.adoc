...
2. *Community Plugins* - public plugins that come from the link:https://plugins.gradle.org/[Gradle Plugin Portal].
...
|Standalone project
|A shared plugin (i.e., community plugin)
...
The `business-logic` build file becomes much simpler:
...
Our new convention plugin enables us to easily share common configurations across all our build files.
