...
The `GreetingPlugin` can now be applied in other subprojects' builds by using its `com.example.GreetingPlugin` ID:
...
In our case, we want to have access to community plugins in the convention plugin, so we add the Gradle Plugin Portal as a repository:
...
We create a build file and apply the link:https://plugins.gradle.org/plugin/org.gradle.kotlin.kotlin-dsl[`kotlin-dsl`] plugin, so we can write the convention plugin in Kotlin DSL.
...
We add a Kotlin script called `my-java-library.gradle.kts` inside the `/src/main/kotlin` directory.
