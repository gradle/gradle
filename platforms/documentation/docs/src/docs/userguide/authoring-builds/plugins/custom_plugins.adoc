...
|<<sec:build_script_plugins,Build script and Script plugins>>
|Build script or script
|A local plugin
...
== Build script and Script plugins
...
*Script plugins* are similar to build script plugin, however, the plugin definition is simply done in a separate script which is then applied to the build file using `apply(from = " ")` or `apply from: ''`:
...
These plugins use the Groovy or Kotlin DSL instead of pure Java, Kotlin, or Groovy.
