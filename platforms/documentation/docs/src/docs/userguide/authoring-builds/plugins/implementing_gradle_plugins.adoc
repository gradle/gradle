...
Plugin IDs are fully qualified like Java packages (i.e., a reverse domain name).
This helps avoid collisions and provides a way to group plugins with similar ownership.

Your plugin ID should be a combination of components that reflect the namespace (a reasonable pointer to you or your organization) and the name of the plugin it provides.
For example, if you have a Github account named "foo" and your plugin is named "bar", a suitable plugin ID might be `com.github.foo.bar`.
...
Generally, a namespace that identifies ownership and a name is sufficient for a plugin ID.

[[sec:working_with_files_in_custom_tasks_and_plugins]]
== Working with files in plugins
...
Note that we specify the `greetingFile` property value _after_ we have configured to use it for the task.
