...
Plugins can report problems through Gradle's problem reporting APIs, which allows reporting rich, structured information about problems happening during the build.
This information could be used by different user interfaces like Gradle's console output, build scans, or IDEs to communicate problems to the user in the most appropriate way.

The following brief example shows an example issue reported from a plugin. For a full example, see our link:../samples/render-samples/sample_problems_api_usage.html[end-to-end sample].
...
<2> A problem reporter, is created for the plugin. Meanwhile the namespace is up to the plugin author, it is recommended to use the plugin ID.
<3> A problem is reported. This problem is recoverable, so the build will continue.

=== Problem building

When reporting a problem, a wide variety of information can be provided.
The documentation provided in link:{javaDocPath}/org/gradle/api/problems/ProblemSpec.html[ProblemSpec] describes all the information that can be provided.
...
- link:{javadocPath}/org/gradle/api/problems/ProblemReporter.html#rethrowing-java.lang.RuntimeException-org.gradle.api.Action-[Rethrowing] a problem is used to wrap an already thrown exception. Otherwise, the behavior is the same as for `Throwing`.

See the link:{javaDocPath}/org/gradle/api/problems/ProblemReporter.html[ProblemReporter] documentation for more details.

=== Problem aggregation

When reporting problems, Gradle will aggregate similar problems when it sends it through the Tooling API, based on the problem's category label.

 - When a problem is reported, the _first_ occurrence is going to be reported as link:{javaDocPath}/org/gradle/tooling/events/problems/ProblemDescriptor.html[ProblemDescriptor], containing the full information about the problem.
 - Any subsequent occurrences of the same problem will be reported as link:{javaDocPath}org/gradle/tooling/events/problems/ProblemAggregationDescriptor.html[ProblemAggregationDescriptor]. This descriptor will arrive at the _end_ of the build, and will contain the number of occurrences of the problem.
