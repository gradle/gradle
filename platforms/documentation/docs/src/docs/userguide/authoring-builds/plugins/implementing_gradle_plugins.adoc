...
<3> The plugin creates a handle to the configuration file it needs to read.
<4> As the configuration file is not found, the plugin reports an unrecoverable problem. This will cause the build to fail.
