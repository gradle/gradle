...
This section explains how to properly design Gradle plugins based on established practices and apply them to your own projects.
...
Gradle plugins can provide sensible defaults and standards (i.e., conventions) in a specific context, reducing the number of decisions users need to make.
...
The following example shows such a custom DSL for the example:
...
For example, the link:{javadocPath}/org/gradle/api/plugins/JavaBasePlugin.html[Java Base plugin] provides un-opinionated functionality and general-purpose concepts, while the <<java_plugin.adoc#java_plugin,Java plugin>> adds tasks and conventions familiar to Java developers.
...
Gradle tasks can be defined in different ways, such as ad-hoc tasks, simple task definitions using `DefaultTask` with one or many actions, or as <<custom_tasks.adoc#custom_tasks,enhanced tasks>> that use a custom task type and expose its configurability through properties.
Custom tasks offer reusability, maintainability, configurability, and testability, making them preferable over ad-hoc tasks, especially when providing tasks as part of plugins.

For example, imagine a plugin that resolves the latest version of a dependency in a binary repository using HTTP calls.
...
Gradle uses declared inputs and outputs to determine if a task is up-to-date and needs to perform any work. If none of the inputs or outputs have changed, Gradle skips that task.
...
This ensures that each task has the necessary information for up-to-date checking. R
emember, for `UP_TO-DATE` checking to work properly, a task must define both inputs and outputs.
...
As a plugin author, applying the Plugin Development plugin to your project provides the `validatePlugins` task, which automatically checks for existing input/output annotations for every public property defined in a custom task type implementation.
...
Implementing the backing objects for such an extension is fairly easy. First, introduce a new data object for managing the properties `websiteUrl` and `vcsUrl`:
