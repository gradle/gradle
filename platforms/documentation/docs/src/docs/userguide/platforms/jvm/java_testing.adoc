...
If you are using link:https://scans.gradle.com[Build Scan] or link:https://gradle.com/gradle-enterprise-solution-overview/failure-analytics/[Develocity],
 ...
 * No test sources exist. This is normal behavior.
 * Test sources exist, but they were not compiled. The sources may be in the wrong directory or the source set may not be configured correctly.
 * No test classes are configured on the `Test` task. By default, `Test` tasks do not assume any particular path for test classes. Only the built-in task named `test` is automatically configured to use the `test` source set.
   If you have a custom `Test` task, you need to configure the `testClassesDirs` and `classpath` properties appropriately.

Before Gradle 9.0.0, Gradle <<upgrading_version_8#test_task_default_classpath,implicitly configured all `Test` tasks>> to use the `test` source set.

=== Test process fails to start or stay running

The test process can fail to start if configured incorrectly.
...
For example, tests that rely on changing the link:{javaApi}/java/lang/SecurityManager.html[`SecurityManager`] can interfere with Gradle's internal messaging communication between the build and test process.
...
Before Gradle 9.0.0, Gradle <<upgrading_version_8#test_framework_implementation_dependencies,leaked internal dependencies>> into the test runtime classpath which may have masked missing dependencies.
