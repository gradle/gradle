...
1. *<<#sec:java-toolchains,Java toolchains>>* (Recommended)
2. *<<#sec:source-target-toolchain,Source and Target compatibility>>*
3. *<<#sec:flag-toolchain,The `--release` flag>>*
...
=== 2. Source and Target compatibility
...
Instead of using Gradle toolchains, you can manually specify a JDK globally using the `JAVA_HOME` environment variable:
...
This approach ensures that all Java-based tools (Gradle, Maven, etc.) use the same JDK version.
It enforces a system-wide JDK version and is great for legacy projects that do not support toolchains.

However, it applies the Java version at a global scope, so it is not possible to specify different JDKs per project.

[[sec:ide-settings-toolchain]]
=== 5. IDE settings

Most modern IDEs allow you to specify the Java version for Gradle independently.
This can be useful if you want Gradle to match the JDK used in your development environment.
...
tasks.withType<JavaCompile> {
    options.release.set(11) // Ensures bytecode and API compliance with Java 11
}
----
...
|===
| Method | Ensures Correct JDK? | Auto Downloads JDK? | Prevents Accidental API Use? | Can Be Combined?
...
- *For advanced cases:* Combine toolchains + `--release`.
- *Avoid* `sourceCompatibility` and `targetCompatibility` unless necessary.
- *Use `JAVA_HOME`* only if you need to enforce a system-wide JDK version.
