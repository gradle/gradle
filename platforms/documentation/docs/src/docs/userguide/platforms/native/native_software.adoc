...
The build worker pool is shared across all build tasks. This means that when using <<performance.adoc#sec:parallel_execution,parallel project execution>>, the maximum number of concurrent individual compilation operations does not increase. For example, if the build machine has 4 processing cores and 10 projects are compiling in parallel, Gradle will only use 4 total workers, not 40.
...
.build-resource-only-dll.gradle
----
include::{snippetsPath}/native-binaries/windows-resources-only-dll/groovy/build.gradle[tag=resource-only-library]
----
