...
`apiElements` ::
Represents the artifacts (JARs, WARs, etc. that contain the compiled code, resources, and other necessary files) produced by the project that expose its API to consumers. Used by task `jar`.

`runtimeElements` ::
Represents the artifacts (JARs, WARs, etc. that contain the compiled code, resources, and other necessary files) produced by the project that are intended to be consumed by other projects at runtime. Used by task `assemble`.
 ...
 * purple background — the configuration is for defining artifacts, not for you to declare dependencies.
 * grey background — a task.

.Java plugin - _main_ source set configurations
image::java-all-configurations.png[]
