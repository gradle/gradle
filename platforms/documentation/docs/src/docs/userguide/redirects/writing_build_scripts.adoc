...
A *build script* configures a project which is associated with an object of type link:{javadocPath}/org/gradle/api/Project.html[`Project`].

image::author-gradle-4.png[Build,500,63,align="left"]

As the build script executes, it configures the `Project` object.
