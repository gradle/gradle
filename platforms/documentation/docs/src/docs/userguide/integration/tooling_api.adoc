...
[[sec:tapi_help_and_version]]
=== Getting CLI help and version banner

Starting with 9.3, the Tooling API exposes two lightweight models that return the exact command-line texts without starting a Gradle daemon:

* link:{javadocPath}/org/gradle/tooling/model/build/Help.html[org.gradle.tooling.model.build.Help] — the full output of `gradle --help`.
* link:{javadocPath}/org/gradle/tooling/model/build/VersionBanner.html[org.gradle.tooling.model.build.VersionBanner] — the banner printed by `gradle --version`.

You can fetch them like any other model:
...
try (var connection = GradleConnector.newConnector().forProjectDirectory(projectDir).connect()) {
	...
	System.out.println(help.getHelpOutput());

	var banner = connection.getModel(org.gradle.tooling.model.build.VersionBanner.class);
	System.out.println(banner.getVersionOutput());
}
----

For convenience, the existing link:{javadocPath}/org/gradle/tooling/model/build/GradleEnvironment.html#getVersionOutput--[GradleEnvironment#getVersionOutput()] also returns the same banner text when you already query the BuildEnvironment model.
...
link:{javadocPath}/org/gradle/tooling/UnsupportedVersionException.html[UnsupportedVersionException] for unsupported methods. Feature‑detect and fall back as needed.
