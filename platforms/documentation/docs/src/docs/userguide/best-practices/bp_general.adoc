...
Gradle and many plugins (such as Android Gradle Plugin and Kotlin Gradle Plugin) treat internal APIs, such as those within `internal` packages, as subject to unannounced breaking changes during any new Gradle release, even during minor releases.
There have been numerous cases where even highly experienced plugin developers have been bitten by usage of such APIs leading to breakages for their users.
