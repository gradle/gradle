...
====
include::sample[dir="snippets/bestPractices/useConventionPlugins/kotlin/avoid-this",files="build.gradle.kts[tags=avoid-this]"]
include::sample[dir="snippets/bestPractices/useConventionPlugins/groovy/avoid-this",files="build.gradle[tags=avoid-this]"]
====
...
====
include::sample[dir="snippets/bestPractices/useConventionPlugins/kotlin/buildSrc/src/main/kotlin",files="my.java-common-conventions.gradle.kts[tags=do-this]"]
include::sample[dir="snippets/bestPractices/useConventionPlugins/groovy/buildSrc/src/main/groovy",files="my.java-common-conventions.gradle[tags=do-this]"]
====

And apply the plugin in any build file where you want to use the shared logic:

====
include::sample[dir="snippets/bestPractices/useConventionPlugins/kotlin/do-this",files="build.gradle.kts[tags=do-this]"]
include::sample[dir="snippets/bestPractices/useConventionPlugins/groovy/do-this",files="build.gradle[tags=do-this]"]
====

<1> Apply the convention plugin to inherit all shared build logic
<2> Keeps the build file focused on project-specific configuration
...
You should use convention plugins to encapsulate and reuse common build logic across multiple build files.

=== Explanation

Convention plugins help you avoid duplicating build logic across multiple build files by moving shared configuration into a reusable plugin. This approach:

* Reduces duplication and makes build scripts more maintainable
* Centralizes common configuration, making it easier to update
* Makes build files cleaner and more focused on project-specific configuration
* Enables better IDE support and validation for your build logic

Although the initial setup of a convention plugin requires some effort, it significantly simplifies maintenance and addition of new build logic in the long run.
...
<1> Common compiler settings duplicated across multiple build files
<2> Test configuration that needs to be maintained in multiple places
<3> Common dependencies that could be centralized
...
And apply it in your build files:
...
<1> Apply the convention plugin to get all common configuration
<2> Focus on project-specific configuration only
