...
Do not use APIs that are inside an `internal` Java package or contain `Internal` or `Impl`
...
APIs, such as those within internal packages, as subject to breaking changes, even during minor
releases.

If you require a specific functionality that is missing, it’s best to submit a feature request.
As a temporary workaround consider copying the necessary code into your own codebase.
