...
* There can potentially be fewer task invalidations in a build when files in an included build are modified, leading to faster builds.  A change in `buildSrc` causes the whole project to become out-of-date.
...
Defining these in a `build-logic` project requires it to be included in the `pluginManagement` block of the main build's `settings.gradle` file, in order to make these plugins available to the build early enough to be applied to the `Settings` instance.
This is possible, but imposes a significant performance penalty.
If you need to create and apply a `Settings` plugin, you should create it in `buildSrc` for now.
...
This may have performance implications, as Gradle maintains a separate classloader for each project.
If these subprojects were moved to `buildSrc`, Gradle would only maintain a single classloader for the entire `buildSrc` project - this is more performant but possibly unintiutive, as it would restrict different versions of plugins from being applied in these different subprojects.
