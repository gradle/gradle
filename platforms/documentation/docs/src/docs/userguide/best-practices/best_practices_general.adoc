...
As a result, you should never write build logic or plugins that depend on a specific plugin order.
...
However, in multi-project builds, with multiple `build.gradle(.kts)` files or convention plugin, plugin application becomes non-deterministic.
Gradle executes configuration in parallel when possible, and the exact order of plugin execution may vary between runs:

[source,groovy]
.convention-build.gradle
----
apply plugin: 'pluginC'
apply plugin: 'pluginD'
----
...
- *Brittle plugins* that work locally but fail in CI or in composite builds.
- *Undetectable bugs* when plugins behave inconsistently due to race conditions.

Even techniques like `allprojects {}`, `subprojects {}`, or `afterEvaluate {}` do not reliably control plugin order across all cases.

Always avoid writing build logic that assumes ordering between different projects, included builds, or applied scripts—that order is not guaranteed.

==== Plugin Developers

When developing plugins, there are two things to keep in mind.

If your plugin requires another plugin to function, do not assume it’s already been applied.

Instead, either:

- *Apply it yourself*, or
- *Fail fast with a clear error* or warning if it’s missing.
...
If your plugin can work with or without another plugin, and the behavior changes when the other plugin is present, use `withPlugin()`:

[source,groovy]
----
pluginManager.withPlugin('ivy-publish') {
    // Configure 'ivy-publish' after it's applied
}
----

This is safe and defers configuration until the target plugin is applied, regardless of order.

=== Example

==== Don't Do This

Use afterEvaluate wisely?
Don't apply plugins in your own plugin

[source,kotlin]
----
class MyPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        project.pluginManager.apply("java-library") // Explicit dependency
    }
}
----
...
Use pluginManager.withPlugin() to safely react when another plugin is added

[source,kotlin]
----
// Correct way to react to another plugin
project.pluginManager.withPlugin("java") {
    // Code that configures behavior when the Java plugin is present
}
----
