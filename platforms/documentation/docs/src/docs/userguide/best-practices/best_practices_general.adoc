...
Modularize your builds by creating subprojects.

=== Explanation

Splitting your build's source into multiple modules (Gradle projects) is key to taking advantage of Gradle's automatic work avoidance and parallelization features.
After making a change to a source file, Gradle only needs to recompile affected projects.
If all your source lives in a single project, Gradle can't skip any recompilations and won't be able to run tasks in parallel.
You'll likely find other performance benefits of splitting source between projects due to minimizing the compilation classpath of each subproject to only the required dependencies, only running annotation processors on the source files that need them, and so on.

It makes sense to split your build's source into multiple projects _early_.
...
This helps you plan for future growth as your build matures, and improves comprehensibility as each project can be described independently.

Exactly how to split your source in a build varies.
Common boundaries that make cohesive projects can include horizontal layers (e.g. API vs. implementation, front vs. back end, core business logic vs. UI), vertical slices (e.g. feature modules each containing UI + business logic), inputs to source generation vs. their consumers, or just closely related classes.
In general, it matters less _how_ you split your source than that you _do_ split it according to some reasonable scheme.
