...
Plugins in Gradle can be applied in any order.
If your plugin depends on another plugin, don’t assume it has already been applied, plugin application order is not guaranteed.

=== Explanation

You can specify the order of plugin application by applying them sequentially within your build.gradle file

[source,groovy]
----
// Apply plugin A first
apply plugin: 'pluginA'

// Then apply plugin B, which might depend on configurations from plugin A
apply plugin: 'pluginB'
----

If "My plugin only works if another plugin is present":
Tell users to apply that plugin
...
If "My plugin works differently if another plugin is present":
Use withPlugin() to conditionally configure behavior based on the presence of that plugin.
