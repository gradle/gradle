...
:keywords: properties,structuring-builds
...
You should familiarize yourself with these properties, which can have **enormous** impacts on your build's performance.

You should not rely on supplying these properties on the CLI with every Gradle invocation.
Providing these properties on the command line is meant for temporary testing and debugging, and is easy to forget across different environments.
A permanent, idiomatic location to set and share these properties across different machines and between developers is in the `gradle.properties` file located in the root project directory.

You should be aware of the default values of properties your build uses and avoid explicitly setting properties to their default values.
If Gradle ever changes the default value of a property, this will not be a silent change, but will involve a standard <<feature_lifecycle.adoc#backwards_compatibility,deprecation cycle>>.
...
This build supplies `-Dorg.gradle.caching=true` on the command line on every invocation.
...
====
include::sample[dir="snippets/bestPractices/useTheGradlePropertiesFile-do/kotlin",files="gradle.properties[tags=do-this]"]
include::sample[dir="snippets/bestPractices/useTheGradlePropertiesFile-do/groovy",files="gradle.properties[tags=do-this]"]
====

<1> Set a gradle property in the `gradle.properties` file, so it is explicit and portable across different machines and environments.
