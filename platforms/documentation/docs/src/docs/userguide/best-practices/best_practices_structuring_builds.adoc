...
You can also include projects located in deeper subdirectories by specifying the complete path to the project using the `:` charactor as a path separator.
...
Nesting projects under a sensible hierarchical directory structure is a common practice in larger Gradle builds including more than a dozen or so projects.
Such a layout allows you to organize large builds and improve comprehensibility, versus having all your projects live at the root of the build.

However, without further configuration, Gradle will create empty projects for every element of a hierarchical path.
