...
You can include projects located in nested subdirectories by specifying their full project path by using `:` as a separator between path segments.
...
Nesting projects in a sensible hierarchical directory structure is common practice in larger Gradle builds with many projects. 
This approach helps organize large builds and improves comprehensibility, compared to placing all projects directly under the build root.

However, without further configuration, Gradle will create empty projects for each element in the hierarchical path, even if some of those directories do not contain actual builds.
...
This behavior is usually not intended, as you likely only want to include the `search` project itself.

Unused projects - even if empty - can surprise maintainers, clutter reporting, and make your build harder to understand.
If plugins are applied using `allprojects { ... }` or `subprojects { ... }`, they will apply to each of these empty projects, which can also have performance impacts.
It can also be awkward to invoke tasks on nested projects, as you will have to use the full path to the project, like `gradle :subs:features:search:build`, instead of the shorter `gradle :search:build`.

To avoid these negatives when using a hierarchical project structure, you can provide a non-hierarchical name when including the project and be sure to explicitly set the link:{kotlinDslPath}/gradle/org.gradle.api/-project/get-project-dir.html[Project.projectDir] property of any projects included in nested directories:
```
include(':my-web-module')
project(':my-web-module').projectDir = file("subs/web/my-web-module")
```
This will prevent Gradle from creating empty projects for each element of the path.

NOTE: Always use an identical _logical_ project **name** and _physical_ project **location** to avoid confusion.  Don't include a project named `:search` and locate it at `features/ui/default-search-toolbar`, as this will lead to confusion about the location of the project.  Instead, locate this project at `features/ui/search`.

Avoid unnecessarily deep directory structures - for builds containing only a few projects, it is better to keep the structure flat by locating all projects at the root of the build and avoid the need to explicitly set `projectDir` at all.
Within the context of a particular build, it should always be obvious from the pathless project name where that project is located.
If you find yourself dealing with ambiguity about project locations, consider flattening your directory structure to simplify it and avoid this.
