...
Nesting projects under a sensible hierarchical directory structure is a common practice in Gradle builds.
...
Additional projects - even if empty - will clutter reporting, and make your build harder to understand.
If plugins are applied using `allprojects { ... }` or `subprojects { ... }`, they will apply to each of these empty projects, which can have performance impacts and will further hurt comprehensibility.
...
NOTE: Always keep your _logical_ project paths and _physical_ project locations aligned to avoid confusion.  Don't include a project named `:subs:features:search` and locate it at `features/ui/search`, as this will lead to confusion about the location of the project.  For the same reason, avoid unnecessarily deep directory structures - for builds containing only a few projects, it is better to keep the structure flat by locating all projects at the root of the build and avoid the need to explicitly set `projectDir` at all.
