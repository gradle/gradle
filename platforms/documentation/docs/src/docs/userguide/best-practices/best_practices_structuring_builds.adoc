...
Use <<implementing_gradle_plugins_precompiled.adoc#implemention_precompiled_plugins,convention plugins>> to encapsulate and reuse shared build logic across multiple projects in your build.

=== Explanation

Instead of duplicating configuration across multiple build scripts, you can easily move common logic into a reusable convention plugin.
...
Convention plugins are quicker to create than <<implementing_gradle_plugins_binary.adoc#implemention_binary_plugins,typed binary plugins>> extending the `Plugin` class.
...
Create a convention plugin in `build-logic`:
...
<7> Add additional unique project dependencies to only the projects that use them.

=== References
<<custom_plugins.adoc#custom_plugins,Developing Custom Gradle Plugins>>
<<implementing_gradle_plugins_precompiled.adoc#implemention_precompiled_plugins,Implementing Pre-compiled Script Plugins>>
<<plugins.adoc#sec:types_of_plugins,Types of Plugins>>
