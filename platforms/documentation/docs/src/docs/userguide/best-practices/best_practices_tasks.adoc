...
Calling manual resolution - either directly via the link:{javadocPath}/org/gradle/api/artifacts/Configuration.html#resolve()[`resolve()`] method or indirectly via accessing the set of artifacts provided by a configuration - returns a set of files that does not preserve references to the tasks that produced those files.
