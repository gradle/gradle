...
The same is possible with Kotlin stdlib collection extension methods, or Groovy GDK collection extensions.
Converting a `Configuration` to a simple `Set<File>`s will also cause the loss of any implicit task dependencies it carries.

You should avoid using these methods when configuring your build and stick to the methods defined directly on the Gradle interfaces - this is a necessary _first step_ towards avoiding unwanted eager resolutions.
...
<1> *`isEmpty()` Causes Resolution*: Many seemingly harmless Collections API methods like `isEmpty()` can still cause Gradle to resolve dependencies.
