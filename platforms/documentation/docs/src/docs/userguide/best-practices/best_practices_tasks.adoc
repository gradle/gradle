...
== Do not call `get()`` on a Provider outside of a Task action
:keywords: tasks,inputs-and-outputs

When configuring tasks and extensions do not call link:{javadocPath}/org/gradle/api/provider/Provider.html#get()[`get()`] on a provider, use link:{javadocPath}/org/gradle/api/provider/Provider.html#map(org.gradle.api.Transformer)[`map()`], or link:{javadocPath}/https://docs.gradle.org/nightly/javadoc/org/gradle/api/provider/Provider.html#flatMap(org.gradle.api.Transformer)[`flatMap()`] instead.any

=== Explanation

A provider should be evaluated as late as possible (if evaluated). Calling `get()` leads to evaluating it, which can have multiple side effects on your project:

- The value of the provider becomes an input to configuration, causing potential configuration cache misses.
- It may be too early, so you don’t actually use the final and correct value of the property. This may lead to painful and hard to debug ordering issues
- It breaks build dependencies, making automatic task dependency wiring provided by Gradle impossible.
...
====
include::sample[dir="snippets/bestPractices/avoidProviderGetOutsideTaskAction/kotlin",files="build.gradle.kts[tags=provider-get-task-setup]"]
include::sample[dir="snippets/bestPractices/avoidProviderGetOutsideTaskAction/groovy",files="build.gradle[tags=provider-get-task-setup]"]
====
...
====
include::sample[dir="snippets/bestPractices/avoidProviderGetOutsideTaskAction/kotlin",files="build.gradle.kts[tags=avoid-this]"]
include::sample[dir="snippets/bestPractices/avoidProviderGetOutsideTaskAction/groovy",files="build.gradle[tags=avoid-this]"]
====
...
====
include::sample[dir="snippets/bestPractices/avoidProviderGetOutsideTaskAction/kotlin",files="build.gradle.kts[tags=do-this]"]
include::sample[dir="snippets/bestPractices/avoidProviderGetOutsideTaskAction/groovy",files="build.gradle[tags=do-this]"]
====
