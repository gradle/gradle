...
Making assumptions about task names makes a build fragile to Gradle and plugin updates.

=== Explanation

Task names are consider implementation details of a plugin, so hardcoding task name patterns in your build logic
can result in unexpected behaviors after an upgrade of such plugin as the tasks might be renamed or removed.

Prefer using plugin DSL to achieve your desired behavior. If the DSL is missing, rely on task type to do the configuration.
...
<1> *Using tasks.named to get `JavaCompile`*: This relies on this task name and that's the only `JavaCompile` task that needs configuring.
<2> *Using tasks.named to get `Test`*: This relies on this task name and that's the only `Test` task that needs configuring.

==== Do This Instead

====
include::sample[dir="snippets/bestPractices/avoidHardcodingTaskNames-do/kotlin",files="build.gradle.kts[tags=do-this]"]
include::sample[dir="snippets/bestPractices/avoidHardcodingTaskNames-do/groovy",files="build.gradle[tags=do-this]"]
====
...
<2> *Using `tasks.withType`*: Avoids hardcoding task names and ensures that all `Test` tasks are configured.
