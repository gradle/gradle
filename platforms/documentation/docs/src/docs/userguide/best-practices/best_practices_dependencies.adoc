...
If your build declares more than one repository, you should also declare content filters on these repositories to ensure you search for and obtain dependencies from the correct place.
The repository that contains the remaining artifacts should come at the end, so we only query it if we don’t find the included modules in the other repositories.
Another option is to use the <<filtering_repository_content.adoc#sec:declaring-content-repositories,exclusiveContent>> if you are able to. Here, the order doesn’t matter.

Content filtering has three main benefits:

- **Performance** since you only query one repository for a given artifact
...
- **Reliability**, by avoiding searching repositories that contain invalid or incorrect metadata or artifacts for particular dependencies
...
NOTE: Not using content filtering on the last repository may be a security risk since you might resolve artifacts that you expect to be resolved from other repositories with content filtering from it, so make sure you trust the repository.
