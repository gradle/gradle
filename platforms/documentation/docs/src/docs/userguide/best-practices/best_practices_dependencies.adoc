...
- **Performance**, since you only query repositories for a given artifact that will actually contain it
- **Security**, by avoiding asking potentially every repository for every dependency (even ones they shouldn't contain), and improving resiliency to supply chain attacks by specifying exactly where to obtain each artifact
