...
When declaring dependencies without a <<version_catalogs.adoc#version-catalog,version catalog>>, prefer using the single GAV string form `implementation("org.example:library:1.0")`.
Avoid using the multi-parameter form unless necessary `implementation(group = "org.example", name = "library", version = "1.0")`.

TIP: The multi-parameter format may be deprecated in the future.
