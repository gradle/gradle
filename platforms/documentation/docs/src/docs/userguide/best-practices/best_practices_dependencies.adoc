...
You should use the <<filtering_repository_content.adoc#sec:declaring-content-repositories,exclusiveContent>> feature to restrict dependencies to a particular known repository, regardless of the presence of a "fallback" repository.
...
Use exclusive content filtering to ensure that repositories are used only for the expected artifacts:
...
Alternatively, using non-exclusive content filtering along with a fallback repository is also an option.
In most cases, it is better to use exclusive content filtering, as it ensures that dependencies _can only be found in the expected repository_.
If they are not present there, they will not be found at all.

NOTE: Without exclusive content filtering, if you don’t apply content filtering to the last declared repository, you may inadvertently resolve artifacts from it that you intended to come from other repositories in they were not available in those repositories.
