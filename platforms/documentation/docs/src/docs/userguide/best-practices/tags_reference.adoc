...
[[tag:properties]]
`#properties` :: Items that are related to defining and using <<build_environment.adoc#build_environment,build properties>>, such as those in `gradle.properties`, environment variables, or passed via the command line.
