...
== Do not Run `./gradlew` on External Projects

When working with an external project, you should exercise caution before running `./gradlew`.

=== Explanation

The `gradlew` (Gradle Wrapper) script is an executable file provided by the project's authors.
It automatically downloads and runs a specific version of Gradle and can execute arbitrary code defined in the project's build scripts (`build.gradle`, `settings.gradle`, etc.).
Running it without inspection is similar to running any other untrusted script or executable from the internet.
...
* **Inspect Build Files:** Look at `build.gradle(.kts)`, `settings.gradle(.kts)`, and `gradle.properties` for suspicious dependencies, custom `exec` tasks, or obfuscated code.

===== 2. Verify the Gradle Wrapper Files

* **Audit the Wrapper Scripts:** Check the `gradlew` and `gradlew.bat` files themselves for injected shell commands that shouldn't be there.
...
Instead of running `./gradlew build` (which executes the full lifecycle including tests), start with informational tasks:
