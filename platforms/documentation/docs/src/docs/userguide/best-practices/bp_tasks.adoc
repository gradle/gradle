...
Calling manual resolution via the link:{javadocPath}/org/gradle/api/artifacts/Configuration.html#resolve()[`resolve()`] method returns a set of files that does not preserve references to the tasks that produced those files.
This breaks automatic task dependency wiring through task input properties.

In addition, resolving configurations during the configuration phase can slow down the build, even for tasks (e.g., `help`) that don't require the resolved dependencies.
...
=== Tags
`<<tags_reference.adoc#tag:tasks,#tasks>>`, `<<tags_reference.adoc#tag:inputs-and-outputs,#inputs-and-outputs>>`, `<<tags_reference.adoc#tag:inputs-and-outputs,#configurations>>`

== Avoid using eager APIs on File Collections before Task Execution
...
However, calling eager methods—such as `.size()`, `.isEmpty()`, `.files`, `.asPath`, or `.toList()`—on these types during the configuration phase will trigger their implicit resolution.
