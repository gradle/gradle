...
[NOTE]
====
When there is a significant difference between the Groovy version used for compiling the code and the version that CodeNarc depends on, it's necessary to specify a specific Groovy version.
====
