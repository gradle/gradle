...
Multiple community and vendor plugins provide this capability. Choose one that fits your organization's tolerance for false positives and its policy on database freshness.

*As part of your build platform.* If your organization uses Develocity, dependency scanning is available as a feature of the platform.
Scans run against every build without the individual project having to apply a plugin, results are aggregated centrally, and findings are correlated across every project in the organization.
This is the recommended approach for organizations that already run Develocity: it removes the burden of keeping a plugin and its database current in every build and gives security teams a single vantage point over the whole codebase.
