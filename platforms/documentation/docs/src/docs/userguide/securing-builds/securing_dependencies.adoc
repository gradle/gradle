...
[[securing_dependencies]]
[[verifying-dependencies]]
= Securing Dependencies

Every artifact your build consumes, including transitively pulled-in libraries and plugins, becomes part of the code you ship.
A single compromised dependency deep in the transitive graph can execute during your build, be embedded in your application, or reach your users through a published artifact.
...
This page covers three practices that protect the dependency layer:

1. *Screen for known vulnerabilities:* check the dependencies you already use against public vulnerability databases.
2. *Verify checksums:* confirm that every artifact you download matches an expected fingerprint (integrity).
3. *Verify signatures:* confirm that every artifact you download was actually published by the party you expect (provenance).

image::securing-builds/securing-dependencies-1.png[]

Screening tells you whether the dependencies you have chosen contain publicly reported bugs.
Verification tells you whether the artifacts you actually receive are the ones you meant to receive.
Both are necessary.

NOTE: Dependency verification is meant to protect you from compromised dependencies, not to prevent you from including _vulnerable_ dependencies. Vulnerability screening covers the latter.

[[sec:known_vulnerabilities]]
== Screen for Known Vulnerabilities

Public vulnerability databases catalog security issues found in released library versions.
When one of your dependencies is listed with a known vulnerability, you almost always want to know quickly, so you can decide whether to upgrade, patch, or accept the risk.

Screening for known vulnerabilities is separate from checksum and signature verification.
It answers the question "does this library have a bug I already know about?", not "did I download the file I expected?".
A dependency can pass every checksum and signature check and still contain a publicly disclosed vulnerability, because the vulnerability is in the code the author intentionally published.

[[sec:vulnerability_options]]
=== Two Places to Screen

There are two natural places to run a vulnerability screen against a Gradle build:

*As a build task.* A vulnerability-scanner plugin is applied to your build.
When you run its check task, the plugin resolves the dependency graph, matches every artifact against a vulnerability database, and fails the build (or produces a report) when it finds a match.
Multiple community and vendor plugins provide this capability; the link:https://plugins.gradle.org/search?term=vulnerability[Gradle Plugin Portal] lists current options.
Choose one that fits your organization's tolerance for false positives and its policy on database freshness.

*As part of a centralized platform.* Several options move scanning out of the project's build script and onto infrastructure the organization already runs. Two common ones:

* *GitHub* — if your code is hosted on GitHub, Dependabot scans committed dependency manifests and opens alerts (and optionally pull requests) for known vulnerabilities.
The `dependency-review` GitHub Action extends this to pull requests, failing a PR that introduces a new vulnerable dependency.
For richer coverage of transitive dependencies, the Gradle Dependency Submission action publishes the resolved dependency graph to GitHub so Dependabot sees the full picture, not just what appears in build scripts.
* *Develocity* — dependency scanning is a first-class feature of the platform. Scans run against every build without the individual project having to apply a plugin, results are aggregated centrally, and findings are correlated across every project in the organization.
This is the strongest option for organizations that already run Develocity: it removes the burden of keeping a plugin and its database current in every build and gives security teams a single vantage point over the whole codebase.

Other SCM providers, CI platforms, and security vendors offer comparable capabilities. The pattern to look for is centralized scanning that runs on every build or every change, without requiring per-project plugin upkeep.

[[sec:vulnerability_baseline]]
=== Establish a Baseline

Regardless of where the scan runs, three habits keep vulnerability screening useful:

1. *Run it on every build*, not just on releases.
A vulnerability that appears mid-development is cheapest to fix immediately.
2. *Fail the build on findings above an agreed severity*, rather than treating findings as advisory.
A report no one reads has the same protective value as no report at all.
3. *Keep a record of accepted findings*, with a reason and an expiration date.
Some vulnerabilities have no fix, or the fix is not yet compatible with your code.
An explicit, time-boxed acceptance is far safer than a permanent, silent exclusion.

Screening addresses vulnerabilities in the code an author *intended* to publish.
The rest of this page addresses whether the artifact you receive is the one the author actually published.
...
WARNING: Gradle requires *full 40-character fingerprint IDs* for `pgp` and `trusted-key` elements, not short or long IDs. Short IDs are trivially collidable. For `ignored-key` elements, either fingerprints or long (64-bit) IDs are accepted.
...
It doesn't make sense to require checksums for versions you don't know about yet.
In these cases, you can disable verification for specific configurations using the API.
...
====
include::sample[dir="snippets/securing-builds/dependencyVerification-detachedConfiguration/kotlin",files="build.gradle.kts[tags=detached]"]
include::sample[dir="snippets/securing-builds/dependencyVerification-detachedConfiguration/groovy",files="build.gradle[tags=detached]"]
====
...
A different failure occurs when the signing key cannot be located on any key server at all:
...
The trailing parenthetical reports how many other keys you already trust for the failing artifact, distinguishing keys trusted for the specific `group:module` from keys trusted for the whole `group`.
A non-zero count is a strong signal that the signing key has been rotated, rather than this being a brand-new module or group you are trusting for the first time.
...
When a single key is trusted for several different groups or artifacts, Gradle groups the entries under one `trusted-key` element with nested `trusting` elements.
In that case the key-level `origin` and `reason` act as defaults, and each `trusting` entry may override them with its own values.
This is useful when the same key signs very different artifacts whose provenance you want to document separately:
...
<trusted-keys>
   <trusted-key id="8756c4f765c9ac3cb6b85d62379ce192d401ab61"
      ...
      <trusting group="com.example" reason="Trusted internally after manual review"/>
   </trusted-key>
</trusted-keys>
----

Here the `com.github.javaparser` entry inherits the key-level `origin`/`reason`, while the `com.example` entry overrides the `reason`.
...
NOTE: This is a typical company policy. However, nothing prevents an internal repository from being compromised. Verify internal artifacts as well.
...
<verification-metadata xmlns="https://schema.gradle.org/dependency-verification"
   ...
   <configuration>
      ...
      <verify-signatures>true</verify-signatures>
      <key-servers>
         <key-server uri="hkp://my.server.com"/>
         <key-server uri="https://my.key.server.com"/>
      </key-servers>
   </configuration>
</verification-metadata>
----
...
Without `keyring-format` specified, if `gradle/verification-keyring.gpg` or `gradle/verification-keyring.keys` exists, Gradle will use it.
The binary version (`.gpg`) takes precedence if both exist.
...
However, passing signature verification doesn't automatically mean you can trust the artifact.
You must explicitly trust the keys by adding them to your verification metadata as shown in <<#sec:manual-update-metadata>>.
