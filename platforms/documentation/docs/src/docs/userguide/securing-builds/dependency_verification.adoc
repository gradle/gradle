...
Checksums alone verify integrity but not authenticity. Signatures alone verify authenticity but may use weak hashes.
...
WARNING: Signatures can also be used to assess integrity like checksums, but signatures are signatures of the _hash_ of artifacts, not artifacts themselves.
...
* You verify and add trusted public keys to your verification metadata.
...
By default, Gradle verifies all downloaded artifacts, including javadocs and sources. This can cause issues with IDEs that automatically download them during import—if you haven't set checksums for those, importing would fail.
...
// Really gradle uses gpg to try to recreated the hash and verify that the two hasshes match
