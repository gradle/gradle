...
1. *Manual Update*: Add the new `<component>` entry to the XML file manually with the checksum provided by the library author.
2. *Auto-update*: Run the `--write-verification-metadata` command again to append the new hashes.
...
One option to cleanup the file is to move the existing `verification-metadata.xml` file to a different location and call Gradle with the `--dry-run` mode: while not perfect (it will not notice dependencies only resolved at configuration time), it generates _a new file_ that you can compare with the existing one.
