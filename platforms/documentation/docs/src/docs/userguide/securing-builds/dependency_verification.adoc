...
.Disabling verification for a configuration
[source,kotlin]
----
configurations {
    myConfiguration {
...
.Disabling verification for a detached configuration
[source,kotlin]
----
...
By default, Gradle verifies all artifacts including javadoc and source JARs. For many projects, verifying these artifacts adds significant overhead without much security benefit, since they're not executed during the build.

You can configure Gradle to skip verification of javadoc and source artifacts:
...
<verification-metadata xmlns="https://schema.gradle.org/dependency-verification"
   ...
   <ignored-artifacts>
      <ignore-artifact regex="true" name=".*-javadoc\.jar"/>
      <ignore-artifact regex="true" name=".*-sources\.jar"/>
   </ignored-artifacts>
</verification-metadata>
----

The `<ignored-artifacts>` section uses regular expressions to match artifact filenames. Any artifacts matching these patterns will not be verified.

NOTE: Skipping verification of javadoc and source artifacts reduces the size of your verification metadata file and speeds up bootstrapping, but means these artifacts could be tampered with without detection.

[[sec:verification-modes]]
== Verification Modes
...
When you bootstrap with signature verification enabled using `--write-verification-metadata pgp,sha256`, Gradle takes an _optimistic approach_ and assumes signature verification is sufficient. However, you must still provide a fallback checksum algorithm (like `sha256`) because not all artifacts are signed.
...
Alternatively, ask Gradle to generate the missing information using <<#sec:bootstrapping-verification,bootstrapping>>. Existing information will be preserved—Gradle only adds missing verification metadata.
...
* **Dependency tampered in local cache** - Usually harmless. Delete the file from cache and Gradle will re-download.
