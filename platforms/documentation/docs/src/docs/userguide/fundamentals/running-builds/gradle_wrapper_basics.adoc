...
└── gradlew.bat // <4>
----
<1> `gradle-wrapper.jar`: This is a small JAR file that contains the Gradle Wrapper code. It is responsible for downloading and installing the correct version of Gradle for a project if it's not already installed. It can be omitted from version control: in that case `gradlew`/`gradlew.bat` download it on demand from `wrapperJarUrl` in `gradle-wrapper.properties` (or from a URL derived from the distribution URL if `wrapperJarUrl` is not set).
<2> `gradle-wrapper.properties`: This file contains configuration properties for the Gradle Wrapper, such as the distribution URL (where to download Gradle from), the wrapper JAR URL (where to download a missing wrapper JAR from), and the distribution type (ZIP or TARBALL).
<3> `gradlew`: This is a shell script (Unix-based systems) that acts as a wrapper around `gradle-wrapper.jar`. It downloads the JAR first if it is missing. It is used to execute Gradle tasks on Unix-based systems without needing to manually install Gradle.
