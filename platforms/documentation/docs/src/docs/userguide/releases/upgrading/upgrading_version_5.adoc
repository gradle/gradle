...
You can opt into the old behavior for selected repositories by adding the `artifact()` <<declaring_repositories_adv.adoc#sec:supported_metadata_sources,metadata source>>.
...
* `ComponentSelectionRule` no longer can inject the metadata or ivy descriptor.
  Use the methods on the <<resolution_rules.adoc#sec:component_selection_rules,`ComponentSelection` parameter>> instead.
...
* There are slight changes in the incubating <<component_capabilities.adoc#sub:selecting-between-candidates,capabilities resolution>> API, which has been introduced in 5.6, to also allow variant selection based on variant name
...
See <<component_capabilities.adoc#sub:capabilities, the capabilities section of the documentation>> for more options.
