...
We recommend that you consider <<declaring_repositories.adoc#declaring-basic-repositories,using>> `mavenCentral()`, `google()` or a private `maven` repository instead.
...
Have a look at <<dependency_versions.adoc#single-version-declarations,the documentation>> for all the rules Gradle applies.
