...
In this example, `otherTest` relied on the deprecated convention. This scenario emitted a <<upgrading_version_8#test_task_default_classpath,deprecation warning>> in 8.x.
...
The convention has been removed, but builds will not fail if they were relying on this behavior. Builds that emitted the deprecation warning in 8.x will silently stop executing tests.
