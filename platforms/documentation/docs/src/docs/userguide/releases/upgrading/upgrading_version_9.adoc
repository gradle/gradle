...
The start parameter is provided by Gradle and is not intended to be reconfigured by build logic.

Enable the build cache with the `org.gradle.caching` Gradle property instead.
See <<build_cache.adoc#sec:build_cache_enable,Enable the build cache>> for details about enabling the build cache.
