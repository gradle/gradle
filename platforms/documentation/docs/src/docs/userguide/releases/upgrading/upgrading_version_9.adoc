...
==== Deprecation of querying an undeclared artifact transform output from a task action

Reading the files of a `FileCollection` whose artifacts are produced by an artifact transform of a project artifact, from inside a task action that does not declare those files as a task input, is now deprecated and will become an error in Gradle 10.

Without the Configuration Cache, this pattern silently runs the transform inline on the calling thread.
With the Configuration Cache, the producer project's state is not reachable from a task action that did not declare the transform as an input, and the first build fails with `project <name> not found`.

To fix this, declare the artifact view as a task input.
You can do this by exposing the collection through a typed `@InputFiles` property on a custom task type.
If you must use an ad-hoc task, use  `inputs.files(...)` on the `FileCollection` exposed by `ArtifactView.getFiles()`, or, when consuming an `ArtifactCollection`, on `ArtifactCollection.getArtifactFiles()`.
Both of these methods properly wire the transform steps into the execution plan.
...
// Recommended: declare the FileCollection as a task input.
tasks.register("consume") {
    inputs.files(view).withPropertyName("colored")
    doLast {
        println "result = " + view.files.name
    }
}
