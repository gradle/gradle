...
Changing this to `compileOnly` aligns the behavior to the current dependency resolution behavior and future-proofs the feature for the introduction of different API targets.
In most cases, this is a non-braking change because the Gradle API is expected to be provided at runtime of the plugin anyway. There is an edge case which may break though: if a non-plugin project depends on a plugin project within the same Gradle multi-project build. In this case, the non-plugin project will need `compileOnly(gradleApi())` added to its dependencies if it uses any Gradle APIs.
