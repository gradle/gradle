...
[[deprecated_accessing_parent_project_properties]]
==== Accessing properties and methods from a parent project

In Gradle's Groovy and Kotlin DSLs, when a child project's build script references a property or method that isn't defined locally, the resolution mechanism walks up the project hierarchy looking for a match. This implicit inheritance is deprecated and will be removed in Gradle 10. Both implicit dynamic resolution (e.g. a bare `foo` reference in a Groovy build script, or `val foo by project` in a Kotlin build script) and the explicit API (`project.findProperty("foo")`, `project.property("foo")`, `project.hasProperty("foo")`) emit the deprecation warning when they resolve through the parent.

Implicit parent lookup creates hidden coupling between projects, makes builds harder to reason about (a typo silently resolves to a parent definition instead of failing), and is fundamentally incompatible with link:userguide/isolated_projects.html[Isolated Projects].

Under Isolated Projects, the parent-project lookup is fully disabled — it is not deprecated, it is gone. A reference that would have resolved through the parent now throws the standard `MissingPropertyException` / `MissingMethodException`, just as it will in Gradle 10. There is therefore no Gradle-9 deprecation warning and no separate cross-project access violation for parent-walk references under Isolated Projects: the lookup simply does not happen. The migration paths below apply equally to Vintage builds (to silence the deprecation) and to Isolated Projects builds (to restore the value).

To migrate, choose one of the following depending on what the property represents:
...
- **Plugin defaults / shared logic** — extract into a link:userguide/sharing_build_logic_between_subprojects.html[convention plugin] applied to each subproject. Each subproject gets its own configuration; nothing crosses project boundaries.
- **Explicit reference to the parent's value** — write `rootProject.ext.foo` (or `parent.ext.foo`) in the child build script. This is a transitional fix: it preserves the cross-project coupling and is therefore not Isolated Projects compatible, but it removes the deprecation warning and makes the dependency explicit. A user-defined helper such as

    [source,groovy]
    ----
    def findExtraInHierarchy(Project project, String name) {
        for (def p = project; p != null; p = p.parent) {
            if (p.ext.has(name)) {
                return p.ext.get(name)
            }
        }
        return null
    }
    ...
    can be added to a build script or convention plugin to mimic the original walking semantics where strictly needed.

For the Kotlin DSL specifically, the `val name: Type by project` delegate syntax — already deprecated in its own right (see link:#kotlin_dsl_delegated_properties[Removing Kotlin DSL property delegates]) — is equivalent to the explicit API:

- `val foo: String? by project` corresponds to `project.findProperty("foo") as String?` and now silently returns `null` for parent-only properties when the deprecation is removed. Audit usages with `grep -rn "by project" --include="*.kts"` before relying on the value.
- `val foo: String by project` corresponds to `project.getProperty("foo")` and throws `InvalidUserCodeException` for parent-only properties — loud and easy to locate.
