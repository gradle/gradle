...
Starting in Gradle 10.0, depending on the current project using module coordinates (group, name, version) will no longer resolve to the current project.
Instead, Gradle will attempt to resolve the dependency from a repository.
...
To continue depending on the current project, use a `project` dependency instead:
