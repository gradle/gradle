...
To fix this, declare the artifact view as a task input — either via `inputs.files(...)` on the `FileCollection` exposed by `ArtifactView.getFiles()`, or, when consuming an `ArtifactCollection`, via `inputs.files(...)` on `ArtifactCollection.getArtifactFiles()`. Both wire the transform steps into the execution plan. You can also expose the collection through a typed `@InputFiles` property on a custom task type.
