...
[[shared_transformed_variant_cache]]
==== Transformed external artifact variants are now shared across resolution scopes

Identical transformed external artifact variants are now computed once per build tree and shared between resolution scopes, instead of being rebuilt by every scope that requests them.
Two variants are considered identical when they share the same component, source variant, resolved source artifacts, target attributes, and transform chain.
The same module served by different repositories still yields separate entries, since the resolved artifact instances differ.
This significantly reduces memory usage in builds with many projects applying the same artifact transforms, such as large Android builds.

As part of this change, transform parameters are isolated when the transformed variant is first requested during dependency resolution, rather than on first execution.
Chains whose parameters depend on task outputs are unaffected and continue to isolate lazily.
Other transforms whose parameter values are read lazily (for example, parameters mutated between resolution and execution, or providers not ready to be queried during resolution) may now fail earlier than before.

If your build is affected, you can temporarily restore the previous per-scope behavior in `gradle.properties`:
