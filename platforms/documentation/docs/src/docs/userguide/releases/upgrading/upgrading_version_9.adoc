...
In Gradle's Groovy and Kotlin DSLs, when a child project's build script references a property or method that isn't defined locally, the resolution mechanism walks up the project hierarchy looking for a match:
...
[source,groovy]
----
println(foo) // Resolved from parent — now deprecated
----
=====
[.multi-language-sample]
=====
...
Both implicit dynamic resolution (e.g. a bare `foo` reference in a Groovy build script, or `val foo by project` in a Kotlin build script) and the explicit API (`project.findProperty("foo")`, `project.property("foo")`, `project.hasProperty("foo")`) emit the deprecation warning when they resolve through the parent.

Implicit parent lookup creates hidden coupling between projects, makes builds harder to reason about (a typo silently resolves to a parent definition instead of failing), and is fundamentally incompatible with <<isolated_projects.adoc#isolated_projects, Isolated Projects>>.

Under Isolated Projects, the parent-project lookup is fully disabled.
A reference that would have resolved through the parent instead throws `MissingPropertyException` / `MissingMethodException`; the same behavior Gradle 10 will apply to all builds.
The migration paths below apply equally to standard builds (to silence the deprecation) and Isolated Projects builds (to restore the value).
...
- `val foo: String? by project` corresponds to `project.findProperty("foo") as String?` and will silently return `null` for parent-only properties once the parent lookup is removed in Gradle 10. Audit usages with `grep -rn "by project" --include="*.kts"` before relying on the value.
