...
[[minimum_client_jvm_version]]
==== Running Gradle Clients on older JVMs

Starting in Gradle 10.0.0, Gradle will require JVM 11 or later to run the CLI client and the Tooling API client.

The Gradle Daemon already requires Java 17 to run, so this change will likely be transparent to most users.
