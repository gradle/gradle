...
`Dependencies` is used to configure dependencies in test suites, and `getProject()` was being used by internal Gradle code to retrieve a reference to the current project.
This method was not intended for public use, but it's possible that it could be used unintentionally.

For example, a configuration like the following may accidentally access this method:
...
Instead, you can specify a dependency on the current project using the `project()` method with no arguments:
...
The `project()` method can be used to specify a dependency on the current project in both test suites and in the top-level `dependencies {}` block.
