...
* Use `JavaForkOptions.jvmArgs()`
* Use `JavaForkOptions.setJvmArgs()`
* Provide a `CommandLineArgumentProvider` to add arguments via `JavaForkOptions.getJvmArgumentProviders()`. An example can be found in the
<<incremental_build.adoc#sec:task_input_nested_inputs,Nested inputs>> section of Incremental Build.
