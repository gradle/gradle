...
In previous versions of Gradle, you could configure toolchains using [project properties](userguide/build_environment.html#sec:project_properties) on the command line with the `-P` flag.
For example, to enable toolchain auto-detection, you could use `-Porg.gradle.java.installations.auto-detect=false`.
This behavior is deprecated and will be removed in Gradle 10.0.0.
Instead, you should specify these settings as [Gradle properties](userguide/build_environment.html#sec:gradle_configuration_properties) using the `-D` flag:
