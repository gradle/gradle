...
The `java-gradle-plugin` now attaches the `gradleApi()` dependency to the `compileOnlyApi` scope instead of the `api` scope.

For most plugin projects, this change should be transparent and without any side effects:

- Your plugin still has full access to Gradle types (like `Project`, `Task`, etc.) during build time.
- Gradle will no longer leak its own API as a transitive runtime dependency of your plugin.
