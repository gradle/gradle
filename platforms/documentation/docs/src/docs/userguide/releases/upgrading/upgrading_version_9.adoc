...
[[removed_software_model]]
==== Removal of the software model

The Gradle software model, <<upgrading_version_9.adoc#deprecated_software_model,deprecated earlier in the 9.x line>>, has been removed.

The following are no longer available:

- The `model { }` block in build scripts, and `Project.model(Closure)`.
- The rule-authoring API in `org.gradle.model`: `RuleSource`, `ModelMap`, `ModelSet`, `ModelElement`, and the `@Model`, `@Mutate`, `@Defaults`, `@Finalize`, `@Validate`, `@Each`, `@Path`, `@RuleInput`, `@RuleTarget`, `@Rules`, `@Managed`, and `@Unmanaged` annotations.
  Plugins that implement `RuleSource` can no longer be applied.
...
- The `ruleSource(Class)` methods on component metadata and component selection rules in dependency management.
  Use the `Action`- and `Closure`-accepting overloads instead.
...
- `ComponentMetadataHandler.all(Object)`
- `ComponentMetadataHandler.withModule(Object,Object)`
- `ComponentSelectionRules.all(Object)`
- `ComponentSelectionRules.withModule(Object,Object)`
