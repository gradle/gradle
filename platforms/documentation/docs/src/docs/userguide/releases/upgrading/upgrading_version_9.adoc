...
==== Consumable configurations are now initialized lazily

Consumable configurations created by bundled Gradle plugins are now initialized only when needed.
Configure actions on these configurations will no longer run at configuration time by default.
They will only execute if the configuration is published, consumed as a variant, or otherwise realized by build logic.
...
With this change, the `Configuring apiElements` line is no longer printed during configuration time unless `apiElements` is realized.

To take advantage of this laziness, avoid APIs that eagerly realize configurations:
