...
A _custom_ subtype of one of these types — for example, `class MyList extends ArrayList<String> {}` — cannot be restored as its own type and is instead restored as the nearest standard type, silently losing any state or behavior the custom type adds.

Referencing such a custom subtype from a task field or task action is now deprecated.
It is reported as a configuration cache problem and will become an error in Gradle 10.
