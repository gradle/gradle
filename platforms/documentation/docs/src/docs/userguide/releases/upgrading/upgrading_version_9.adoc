...
The dependency on `gradleApi()` added by the `java-gradle-plugin` will be added to the `compileOnlyApi` scope instead of `api`.

For most plugin projects, this change should be transparent and without any side effects.
...
System properties can be defined for a build at multiple places: on the command line, in the Gradle user home and in the `gradle.properties` file of a project.
In previous Gradle versions, the priority of these options was not following the documented order: command-line options were potentially overridden by properties defined by entries in `gradle.properties` files.
From this version onwards, the priority of system properties follows the documented order:
- Command-line options (highest priority).
- Properties defined in the Gradle user home `gradle.properties` file.
- Properties defined in `gradle.properties` files. (lowest priority).
...
`DomainObjectCollection.findAll(Closure)` has been deprecated. This API relies on a Groovy type and eagerly evaluates the contents of the container.
...
The following APIs have been deprecated and will be removed in Gradle 10.0.0:
