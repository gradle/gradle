...
[[deprecated_project_notation_in_dependency_handler]]
==== Using a Project object as a dependency notation is now deprecated

Passing a `Project` object as a dependency notation to `DependencyHandler` (or `DependencyFactory`) has been deprecated and will become an error in Gradle 10.

In most cases, a deprecation warning will arise when passing the result of calling `Project.project()` with a project path as an argument:

.build.gradle
[source,groovy]
----
val barProject = project(':bar')
dependencies {
    // Deprecated: project.project(':foo') returns a Project object
    implementation(project.project(':foo'))
    // or any variable holding a Project object:
    implementation(barProject)
}
----

Replace this with the new `project(String)` method on `DependencyHandler`:
...
dependencies {
    ...
    // or any variable holding a project path string:
    implementation(project(barProject.path))
}
----
