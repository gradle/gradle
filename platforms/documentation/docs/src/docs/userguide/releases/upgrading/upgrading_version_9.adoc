...
When the target project has no publications, Gradle will silently resolve the dependency to the target project's group, name, and version.

Starting in Gradle 10.0.0, Gradle will no longer silently ignore unpublished projects during publication.
Publishing a project with a dependency on another project that is not also published will be forbidden.
This change will make it more difficult to publish broken metadata with invalid dependency coordinates.

The example project below demonstrates a build that will trigger the deprecated behavior:
...
In order to avoid this deprecation, ensure that all project dependencies of published projects are also published.
In the example above, applying the `maven-publish` plugin and configuring a publication in the `:other` project will resolve the deprecation.
