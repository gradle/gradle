...
This may lead to different output when using TestNG versions before 6.9.13.3, for example, if you had `org.gradle > TestClass > ok` before, it will now emit just `ok`, without any class information.
Additionally, when using TestNG versions starting with 6.9.10 and before 6.9.13.3, the `IClassListener` exists but is broken, which may lead to even worse output, such as an empty name.
It is recommended to update to at least TestNG 6.9.13.3 to get the correct output.
...
Groovy has been updated to https://groovy-lang.org/changelogs/changelog-4.0.29.html[Groovy 4.0.29].
...
==== Deprecated publishing dependency on an unpublished project

When publishing a project, Gradle resolves project dependencies to the coordinates of the target project's publication.
If the target project has no publication, Gradle currently resolves the dependency silently using the target project’s `group`, `name`, and `version`.

Starting with Gradle 10, this behavior is deprecated. Gradle will no longer silently ignore unpublished projects during publication.
Publishing a project that depends on another project without a publication will be forbidden.
This change prevents publishing broken metadata with invalid dependency coordinates.
...
==== Deprecated depending on current project using module coordinates

Starting with Gradle 10, declaring a dependency on the resolving project using module coordinates (`group`, `name`, `version`) will no longer resolve to that project.
...
==== The `java-gradle-plugin` plugin now adds the `gradleApi()` dependency to the `compileOnly` scope

The dependency on `gradleApi()` added by the `java-gradle-plugin` will be added to the `compileOnly` scope instead of `api`.

For most plugin projects, this change should be transparent and without any side effects. 
A potential breaking use-case is a multi-project scenario, where:
 - A plugin project depends on the `gradleApi()`
 - A non-plugin project (a) depends on the plugin project, and (b) uses the Gradle API.

This case will result in a compilation failure, as the plugin project's `gradleApi()` dependency—now being present on the `compileOnly` scope—won't be transitively present on the non-plugin project's compilation classpath. In this case, add the `gradleApi()` dependency to the non-plugin project as well.
