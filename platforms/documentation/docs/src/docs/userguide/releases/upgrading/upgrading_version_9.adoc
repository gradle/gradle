...
To fix this, declare the `FileCollection` as a task input — typically via `inputs.files(...)`, or by exposing it through a typed `@InputFiles` property on a custom task. The transform steps will then be wired into the execution plan.
