...
You can define link:https://docs.gradle.org/8.9/userguide/publishing_customization.html#sec:publishing-custom-components[secondary variants] for a consumable primary variant that has no artifacts.
For more details on how secondary variants participate in selection, see <<variant_aware_resolution.adoc#sec:secondary-variants,Secondary Variants>>.
...
NOTE: In rare cases, when the variant selection request is underspecified, this change may cause Gradle to select a different variant than it did before.
