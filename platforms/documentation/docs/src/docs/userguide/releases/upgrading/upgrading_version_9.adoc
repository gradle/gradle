...
==== Plugins requested by precompiled settings plugins are now resolved in the build that compiles them

When a precompiled settings plugin (`*.settings.gradle.kts`) declares a plugin in a `plugins {}` block, that
plugin request is now resolved against the dependencies of the build that compiles the plugin.

Previously, if a precompiled settings plugin was never applied, it could contain an invalid plugin request
...
----
plugins {
    id("com.example.invalid.plugin") // a plugin that does not exist in the dependencies
}
----

If such a plugin was published, the invalid request would only fail later, when the plugin was used by a
consuming build. Gradle now validates these plugin requests during compilation and fails the build if the
requested plugin cannot be resolved. This behavior now matches that of precompiled script plugins targeting `Project`.

Avoid invalid plugin requests in unused precompiled settings plugins.

==== Calling `.values()` on environment variables or system properties now tracked by Configuration Cache

Gradle <<configuration_cache_requirements.adoc#config_cache:requirements:reading_sys_props_and_env_vars, tracks>> accesses to environment variables and system properties during configuration time to record them as inputs to the Configuration Cache, so the cache entry is invalidated when those variables change.

Starting with Gradle 9.5.0, calling `.values()` on the maps returned from `System.getenv()` or `System.getProperties()` records **all** environment variables or system properties respectively as inputs to the Configuration Cache, similarly to how `.forEach()` calls are tracked in previous versions.
If your build configuration uses `System.getenv().values()` or `System.getProperties().values()`, you may experience more frequent Configuration Cache invalidations.

==== Dependency lockfiles are generated with a platform-independent line ending

Dependency lockfiles now use a platform independent line ending (`\n`) instead of the system default.
Existing lockfiles written with Windows line endings can still be read, but new lockfiles generated will only use platform independent line endings.

During the next lock update using Gradle 9.5.0 or above, if the lockfiles were previously generated with Windows line endings, all line endings will be changed, regardless of whether dependencies have changed.
...
However, this change may have unintended side effects due to certain checks being removed and slight changes in behavior.

The new script features multiple changes:

- No check of the `OS` variable is performed since Gradle does not support versions of Windows that are not NT-based.
- `endlocal` is used before invoking the application so the local environment changes of the script do not affect the invoked process.
Environment variables intended for the invoked process should be set before the `gradlew.bat` invocation.
- `& CALL` is used after invoking the application to remove the "Terminate batch job (Y/N)?" prompt.
- `exit /b` has been changed to `"%COMSPEC%" /c exit`. This allows the use of `&&` and `||` when invoking the script from another batch file.

==== Upgrade to Kotlin 2.3.20-RC2

The embedded Kotlin has been upgraded from 2.3.0 to link:https://github.com/JetBrains/kotlin/releases/tag/v2.3.20-RC2[Kotlin 2.3.20-RC2].

=== Deprecations

[[deprecate_exit_environment_var]]
==== Deprecation of setting an exit environment variable
`CreateStartScripts.getExitEnvironmentVariable()` and `CreateStartScripts.setExitEnvironmentVariable(String)` have been deprecated and will be removed in Gradle 10.0.0.
Due to the changes in the Windows start script template, these functions no longer have any effect with the default templates.

It is recommended that custom start script templates are updated to not rely on this functionality.
