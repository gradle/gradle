...
==== Implicit lookup of properties and methods in the project hierarchy
...
Both an implicit reference (e.g. a bare `foo` in a Groovy build script, or `val foo by project` in a Kotlin build script) and the explicit API (`project.findProperty("foo")`, `project.property("foo")`, `project.hasProperty("foo")`) emit a deprecation warning when they resolve through the hierarchy.
...
Under Isolated Projects, the implicit hierarchy lookup is fully disabled.
A reference that would have resolved through the hierarchy instead throws `MissingPropertyException` / `MissingMethodException`; the same behavior Gradle 10 will apply to all builds.
...
The link:{javadocPath}/org/gradle/process/ExecSpec.html#setCommandLine(java.util.List)[`setCommandLine()` method] on `ExecSpec` and, by inheritance, on `JavaExecSpec` and the `Exec`/`JavaExec` tasks has been deprecated and will be removed in Gradle 10.0.0.
