...
* `ReportingExtension.file(String)` has been removed. Use `ReportingExtension.getBaseDirectory()` with `file(String)` or `dir(String)` instead.
* `ReportingExtension.getApiDocTitle()` has been removed. There is no direct replacement for this method.
