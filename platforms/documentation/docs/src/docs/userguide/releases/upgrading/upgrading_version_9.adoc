...
Furthermore, `Signature` will no longer extend the internal `AbstractPublishArtifact` type or implement `PublishArtifactInternal`.
...
Starting in Gradle 10, the <<signing_plugin.adoc#signing_plugin,Signing Plugin>> will no longer create the `signatures` Configuration or automatically add Signatures to it.
...
Furthermore, attempting to create a configuration named `signatures` prior to applying the signing plugin is deprecated.
...
==== Deprecation of getting Signatures from `SignOperation`
...
Please use `getFilesToSign()` and `getSignatureFiles()` instead.
