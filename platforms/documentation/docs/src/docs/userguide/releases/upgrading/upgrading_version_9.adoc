...
Note that previously, link:{javadocPath}/org/gradle/process/JavaForkOptions.html#setAllJvmArgs(java.util.List)[`setAllJvmArgs()` method] on `JavaForkOptions` cleared any existing JVM argument providers in addition to setting jvmArgs. This should now be done by first clearing the arguments and then setting them to the new desired values.
