...
Identical transformed external artifact variants (same component, source variant, target attributes and transform chain) are now computed once per build tree and shared between resolution scopes, instead of being rebuilt by every scope that requests them.
...
Transform parameter definitions that relied on being read later — for example parameters whose values are mutated between resolution and execution, or providers that are not ready to be queried during resolution — may now fail earlier than before.
If your build is affected, you can temporarily restore the previous per-scope behavior by setting:
