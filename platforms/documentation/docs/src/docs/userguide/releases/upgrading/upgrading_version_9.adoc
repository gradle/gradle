...
==== Required input file collections reject absent providers nested in collections

Required task input file properties now fail validation when a provider nested in an ordinary collection or object array has no value.
This applies to `TaskInputs.files(...)` declarations and to required `@InputFiles` properties returning those values.

Previously, `inputs.files([provider])` silently ignored an absent provider, while `inputs.files(provider)` failed.
Both now fail in the same way.
Provide a value for every required provider, pass an explicitly empty collection when that represents a valid input, or declare the input optional with `optional()` or `@Optional`.

Existing handling of `FileCollection` and `DomainObjectCollection` values is preserved.
