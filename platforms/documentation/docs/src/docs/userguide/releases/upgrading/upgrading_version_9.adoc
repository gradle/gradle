...
Note that previously, link:{javadocPath}/org/gradle/process/JavaForkOptions.html#setAllJvmArgs(java.util.List)[`setAllJvmArgs()` method] on `JavaForkOptions` cleared any existing JVM argument providers in addition to setting jvmArgs. Going forward, this will not happen automatically and will have to be implemented as two separate commands. To clear existing providers, use `JavaForkOptions.getJvmArgumentProviders().clear()`.

Example of replacing `setAllJvmArgs`:

[source,java]
----
import java.util.ArrayList;class Example {
    //Given an existing argumentProvider and some existing jvmArgs
    DefaultJavaForkOptions options; // options contains ['foo', 'bar', 'argFromProvider'];

        void example () {
            options.getJvmArgumentProviders().clear(); // Clear existing providers if needed
            options.setJvmArgs(new ArrayList<>()); // Clear existing jvmArgs
    }
}
----
