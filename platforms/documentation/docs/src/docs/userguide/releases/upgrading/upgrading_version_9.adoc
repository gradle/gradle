...
[[non_dependency_artifact_type_classifier]]
==== Deprecation of non-dependency artifact type or classifier notation

Using the artifact type or classifier notation outside of dependency declarations has been deprecated without replacement and will be removed in Gradle 10.0.0.
This information was never stored, and allowing it to be specified was a source of confusion.

This includes usages in constraints and `ComponentMetadataHandler.withModule(...)`.
Below are examples of the deprecated notation:
...
dependencies {
    constraints {
        implementation("org:foo:1.0:classifier") // deprecated
        implementation("org:foo:1.0@ext") // deprecated
    }
    components {
        withModule("org:foo@ext") {
            // ...
        }
    }
}
