...
[[deprecated-reentrant-build-service-registration]]
==== Deprecation of registering a build service from its own configuration action

Calling link:{javadocPath}/org/gradle/api/services/BuildServiceRegistry.html#registerIfAbsent(java.lang.String,java.lang.Class,org.gradle.api.Action)[`BuildServiceRegistry.registerIfAbsent`] for a service from the configuration action of the registration of the same service (directly, or indirectly through a provider evaluated by the action) is now deprecated and will become an error in Gradle 10.

Previously, such a nested registration silently produced two service providers for the same name: the provider returned by the outer call was not tracked by the registry, so its service instance was never stopped and was invisible to shared-resource accounting.
Now the nested registration takes effect, both calls return the same provider, and the configuration applied by the outer action is discarded.

Restructure the build logic so that the service is registered once, outside of its own configuration action.
...
[[publish_no_sha256_sha512_checksums]]
==== SHA-256 and SHA-512 checksums are no longer published

When publishing to Maven or Ivy repositories, Gradle no longer publishes `.sha256` and `.sha512` checksum files alongside each artifact.
Only universally supported checksums are published: `.sha1` and `.md5` for Maven and `.sha1` for Ivy.

These extra checksums were rarely consumed and frequently rejected by repositories that do not recognize them.
Consuming builds are unaffected as Gradle has never required SHA-256 or SHA-512 checksums to be present in order to resolve or verify artifacts.

The internal, undocumented `org.gradle.internal.publish.checksums.insecure` system property, which previously suppressed these extra checksums, no longer has any effect and can be removed.
