...
This case will result in a compilation failure, as the plugin project's `gradleApi()` dependency—now being present on the `compileOnlyApi` scope—won't be transitively present on the non-plugin project's compilation classpath. In this case, add the `gradleApi()` dependency to the non-plugin project as well.
