...
To preserve the previous outcome of variant selection in this case, Gradle automatically tags such configurations with a new `org.gradle.fallback-variant` attribute: the empty primary receives `org.gradle.fallback-variant=true` and each secondary receives `org.gradle.fallback-variant=false`. A default schema disambiguation rule prefers `false` over `true` when both values are candidates, so a secondary still wins whenever a secondary is compatible with the consumer's request — matching the pre-Gradle 10.0 outcome where the primary was simply absent.

When the consumer explicitly requests a value for `org.gradle.fallback-variant`, the rule honors that request. This gives consumers an escape hatch: a consumer that wants the empty primary can request `org.gradle.fallback-variant=true` on the resolvable configuration to opt into it.
