...
All Kotlin DSL property delegate extensions have been deprecated and will
be removed in Gradle 10. This includes container delegates (`by
registering`, `by creating`, `by existing`, `by getting`), property
delegates (`by project`, `by settings`, `by extra`), and value delegates
(`Property<T>` and `ConfigurableFileCollection` getValue/setValue
operators).

Use the explicit API instead.

The container delegates (`registering`, `creating`, `existing`, `getting`)
are defined on `NamedDomainObjectContainer` and
`NamedDomainObjectCollection`. They apply to any container, not just
`tasks` or `configurations`.

The examples below use a generic
`container` placeholder; replace it with `tasks`, `configurations`,
`sourceSets`, or whichever container you use.
...
Delegated properties are being removed because they create silent
correctness bugs (e.g. no-op delegates, wrong extra property scope),
couple entity names to variable names, diverge between build scripts and
class-based plugins, and duplicate functionality already provided by
explicit APIs or other DSL sugar.

[[deprecated_accessing_parent_project_properties]]
==== Deprecation of accessing properties and methods from parent projects

Gradle has historically allowed child projects to implicitly access properties and methods defined in parent projects.
For example, if a root project defines `ext.foo = 1`, a child project could reference `foo` directly without any explicit declaration.

This implicit inheritance has been deprecated and will become an error in Gradle 10.
It creates hidden coupling between projects, makes build logic harder to understand, and impedes build scalability with features like <<sharing_build_logic_between_subprojects.adoc#sec:convention_plugins_vs_cross_configuration,isolated projects>>.

The following patterns trigger this deprecation:

- Accessing a parent project's extra property (e.g., `ext.foo`) from a child project's build script by name (`foo`), or through API calls like `property("foo")`, `findProperty("foo")`, or `hasProperty("foo")`.
- Invoking a method defined in a parent project's build script from a child project.

To fix this, declare properties explicitly in each project that needs them, or use one of the following alternatives:

[cols="1a,1a", options="header"]
|===
| Before (deprecated) | After (recommended)

| Implicit access to parent property:
[source,groovy]
----
// root build.gradle
ext.myProp = "value"
...
// child build.gradle
println(myProp) // resolves from parent
----
| Use <<build_environment.adoc#sec:project_properties,project properties>> via `gradle.properties`:
[source,groovy]
----
// gradle.properties
myProp=value
...
// child build.gradle
println(providers.gradleProperty("myProp").get())
----

| Shared dependency versions via parent property:
[source,groovy]
----
// root build.gradle
...
----
| Use a <<version_catalogs.adoc#version-catalog,version catalog>>:
[source,groovy]
----
...
| Shared typed values (non-string) via parent property:
[source,groovy]
----
// root build.gradle
ext.supportedLanguages = ["java", "groovy"]

// child build.gradle
if (supportedLanguages.contains("java")) { ... } // reads parent property
----
| Use `gradle.lifecycle.beforeProject` in settings to set properties on each project individually:
[source,groovy]
----
// settings.gradle
...
// child build.gradle
if (supportedLanguages.contains("java")) { ... } // reads own property, not parent's
----
Or use a <<sharing_build_logic_between_subprojects.adoc#sec:sharing_logic_via_convention_plugins,convention plugin>> for more complex cases.

| Shared logic using parent methods:
[source,groovy]
----
// root build.gradle
def sharedMethod() { ... }
...
// child build.gradle
sharedMethod() // resolves from parent
----
| Use a <<sharing_build_logic_between_subprojects.adoc#sec:sharing_logic_via_convention_plugins,convention plugin>>:
[source,groovy]
----
...
// child build.gradle
plugins { id 'my-conventions' }
sharedMethod() // resolves from plugin
----
|===
