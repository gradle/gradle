...
If a precompiled settings plugin (`*.settings.gradle.kts`) requests a plugin in the `plugins { }` block, the
plugin request is now going to be checked against the dependencies in the build that compiles the plugin.

Previously, if a precompiled settings plugin was never applied in the build, it could contain an invalid
plugin request that was not validated:
...
If such a precompiled settings plugin was published, the invalid plugin request would only cause a failure
in the consuming build. Now it will be validated in the build that compiles the plugin, failing if the request
is not satisfied. This behavior is now the same as that of `Project`-targeting precompiled script plugins.
