...
Both an implicit reference (e.g. a bare `foo` in a Groovy build script, or `val foo by project` in a Kotlin build script) and the explicit API (`project.findProperty("foo")`, `project.property("foo")`, `project.hasProperty("foo")`) emit the deprecation warning when they resolve through the hierarchy.
