...
The `JavaForkOptions.setAllJvmArgs()` method has been deprecated and will be removed in Gradle 10.0.0.
Instead, use the `JavaForkOptions.jvmArgs`, `JavaForkOptions.setJvmArgs`, or provide a `CommandLineArgumentProvider` to add arguments via `JavaForkOptions.getJvmArgumentProviders()`.
