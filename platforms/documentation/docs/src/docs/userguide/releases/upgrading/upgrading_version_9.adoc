...
[[nested_output_structure]]
==== Retaining output structure in nested properties

Gradle now infers producer tasks for managed output beans reached through `@Nested Property<T>` declarations.
Once Gradle uses a nested output's producer information or inspects its output declaration, it finalizes the containing property to retain the same output bean.
Replacing that bean afterwards fails with the usual final-property error.
Ordinary bean reads during configuration do not finalize the property, and output locations retain their normal lazy-property behavior.

Configure the output structure before Gradle discovers task dependencies and output declarations.
See <<lazy_configuration.adoc#nested_output_properties,nested output properties>> for examples and limitations.
...
[[nested_output_ownership]]
==== Sharing nested output beans between tasks

Declaring the same managed output bean through resolved `@Nested Property<T>` properties of different tasks is deprecated.
Gradle warns when inspecting these shared output declarations; this will become an error in Gradle 10.
Automatic dependency inference from such an output already fails because it cannot identify a unique producing task.

Create a separate output bean for each producing task.
If a second task consumes the first task's output, declare that file as an input of the second task instead.
Input-only beans remain shareable, and multiple declarations on the same task may refer to the same output bean.
