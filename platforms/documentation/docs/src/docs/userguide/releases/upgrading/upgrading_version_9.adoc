...
Kotlin DSL build scripts (`.gradle.kts`) are now compiled against a pre-built public API ABI JAR from the Gradle distribution instead of the dynamically generated Gradle API JAR.
...
NOTE: This change does not affect plugin projects that depend on `gradleApi()` or `gradleKotlinDsl()`. Those continue to use the full generated Gradle API JAR as before.
