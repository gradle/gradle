...
[[get-path]]
==== Deprecation of `Path.getPath()`

The link:{javadocPath}/org/gradle/util/Path.html#getPath()[`getPath()` method] on `Path` has been deprecated and will be removed in Gradle 10.0.0.

Instead, use link:{javadocPath}/org/gradle/util/Path.html#asString()[`asString()`] on `Path`.
