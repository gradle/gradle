...
==== Concurrency synchronization primitives can no longer be serialized by the Configuration Cache

Configuration cache serialization now emits a proper problem when attempting to serialize most of Java's standard concurrency synchronization primitives.
...
Each task receives its own deserialized instance of the synchronization object (for example, its own lock), so coordination between tasks does not occur.
