...
==== Running Gradle in a directory that is not part of the surrounding build now fails

When Gradle is invoked in a directory that lies inside a build's directory tree but is not a project directory of that build, `gradle help` and `gradle` with no task names previously discarded the surrounding build and reported an undefined build:
...
This is the message `gradle <task>` has always produced in the same directory, so the two no longer disagree.

Two cases are unaffected.

Running Gradle in a directory with no settings or build script anywhere in its parent hierarchy still reports an undefined build and still suggests `gradle init`.

Running `gradle init` in a directory inside an existing build still creates a new build there.
