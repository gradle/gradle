...
==== Unsupported build events listeners are now configuration cache problems
...
Starting with Gradle 9.0, registering such providers becomes a configuration cache problem and fails the build.

If your build was correctly with configuration cache before (for example, the registered listeners weren't crucial for build correctness),
you can temporarily revert to old behavior of ignoring such providers.
To do that, set the Gradle property `org.gradle.configuration-cache.unsafe.ignore.unsupported-build-events-listeners` to `true`.
