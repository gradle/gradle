...
When the value is a negative integer, the space-assignment syntax is interpreted as subtraction instead of a method call.
For example, if `minGranularity` is a `Property<Integer>`, the following does not assign `-1`:
...
It is interpreted as `minGranularity - 1` and fails because `minGranularity` is a `Property`, not an integer value.
Use explicit assignment for negative values:
...
Apache Commons Compress has been updated from 1.21 to https://commons.apache.org/proper/commons-compress/changes.html#a1.25.0[1.25.0].
