...
In order to ensure the accuracy of dependency resolution, Gradle ensures that Configurations are not mutated after they have been used as part of a dependency graph.
...
* Consumable configurations should not have their dependencies, hierarchy, attributes, etc. modified after they have been published or consumed as variant.
...
Gradle already throws an exception for many of these circumstances, but in comes cases it does not.
As of Gradle 9.0, all of these cases will result in an error.
After a configuration of any type has been observed, it should be considered immutable.
The following properties of a configuration are covered by this validation:
...
In many cases, this deprecation is caused by mutating a configuration in a link:{javadocPath}/org/gradle/api/artifacts/ResolvableDependencies.html#beforeResolve-org.gradle.api.Action-[`beforeResolve`] hook.
...
For the following cases, consider the suggested options for replacing mutations within a `beforeResolve` hook:
...
* **Changing dependency versions**: Use dependency constraints. (See <<using_preferred_versions, Declaring preferred version constraints>>)
...
* **Hierarchy**: Configuration hierarchy (`extendsFrom`) should be set upon creation. Mutating the hierarchy prior to resolution is highly discouraged, but is permitted within a `withDependenices` hook.
* **Resolution Strategy**: Mutating a configuration's ResolutionStrategy is still permitted in a `beforeResolve` hook, however this is not a recommended practice.
