...
When this was used to reference the DSL script target object, then use it directly instead: `project`, `settings` and `gradle` respectively.
When this was used to reference a top-level symbol that happens to have the same name of a nested symbol, then use a different name, possibly by adding an intermediary variable.

Continue reading the link:#jspecify[JSpecify section] below to learn about potential breaking changes in Kotlin build logic code related to nullability.
