...
==== Incubating API `EclipseClasspath.baseSourceOutputDir` is now a `DirectoryProperty`

This was erroneously a `Property<File>` and has been corrected to be a `DirectoryProperty` to reflect the expected type of file.
...
=== Deprecated collections in `org.gradle.plugins.ide.idea.model.IdeaModule` now warn
The `testResourcesDirs` and `testSourcesDirs` fields, and their getters and setters were `@Deprecated` in Gradle 7.6.
However, due to existing widespread usage, deprecation warnings were not yet emitted.
