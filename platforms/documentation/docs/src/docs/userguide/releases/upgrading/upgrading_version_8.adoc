...
You can restore the previous behaviour for one or more properties for your task with the next snippet:
...
tasks.withType<AbstractArchiveTask>().configureEach {
    ...
    // File timestamps are read from the file system
    isPreserveFileTimestamps = true
    // Directory permissions are read from the file system
    dirPermissions.unset()
    // File permissions are read from the file system
