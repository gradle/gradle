...
Plugins built and published using the Groovy DSL on Gradle 9.x require Gradle 7.0 or newer to run.
This is because Gradle 7.0 is the first release that embeds Groovy 3.0 or higher, which is required to run Groovy 4.0 compiled code.
However, due to the major version change, some code may still behave unexpectedly.
For full compatibility, it's recommended to run these plugins on Gradle 9.0 or later.
