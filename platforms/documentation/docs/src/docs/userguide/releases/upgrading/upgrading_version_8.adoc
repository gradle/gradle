...
For full details and potential breaking changes, consult the [Kotlin release notes](https://github.com/JetBrains/kotlin/releases/tag/v2.1.20).
...
This is because Gradle 8.11 is the oldest version that embeds Kotlin >=2.0, which is the first Kotlin version capable of reading Kotlin metadata version 2.

If you want your plugin to remain compatible with older Gradle versions, you must explicitly target an earlier Kotlin language version (1.x) when compiling your plugin.

For example, to support Gradle 6.8 or newer, configure your plugin project to target Kotlin 1.7 like this:
...
Refer to Gradle's link:compatibility.html#kotlin[compatibility matrix] to learn which version of Kotlin is embedded in each Gradle release.

Plugins written using the Kotlin DSL and published with Gradle 7.x or 8.x are compatible with Gradle 6.8 and newer.
...
==== Eager artifact configuration accessors have been removed from Kotlin DSL

In Gradle 5.0 the type of artifact configuration accessors changed from `Configuration` to `NamedDomainObjectProvider<Configuration>`.
The Kotlin DSL added specialized extension methods for `NamedDomainObjectProvider<Configuration>` to stay compatible with plugins compiled with the old, non-lazy form.
...
Gradle 8.0 then removed all of these extensions from the API, but the methods were still available for plugins compiled against older versions of Gradle.
Gradle 9.0 finishes up their complete removal.
