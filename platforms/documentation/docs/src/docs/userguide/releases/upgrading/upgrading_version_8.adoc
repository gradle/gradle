...
Starting in Gradle 9.0, the `main` source set will not be treated any differently than any other source set.
With the `java-library` plugin applied, or any other plugin that applies the `java` plugin, this will effectively cause calling `usingSourceSet` with the `main` source set to throw an exception, as the `java` plugin already configures a main feature.
Only if the `java` plugin is not applied will the `main` source set be permitted when calling `usingSourceSet`.
In this case, the `main` source set will be treated the same as any other source set, and no special configurations will be created.
