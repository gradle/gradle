...
In Gradle 9.0, the link:{javadocPath}/org/gradle/caching/local/DirectoryBuildCache.html#setRemoveUnusedEntriesAfterDays-int-[buildCache.local.removeUnusedEntriesAfterDays] property has been removed.

This property was previously used to configure build cache retention period directly on local build cache.

To set local build cache retention period, you should now use <<directory_layout.adoc#dir:gradle_user_home:configure_cache_cleanup,Gradle User Home cache cleanup and retention periods configuration>> instead.
