...
==== Deprecated running the Gradle daemon with JVM versions 16 and earlier

Starting in Gradle 9.0, the Gradle daemon will require JVM 17 or later to run and Gradle API classes will be compiled to target JVM 17 bytecode.

Using Gradle to compile Java code to target JVM version 6 or later is still fully supported.
...
The Gradle Wrapper, Gradle client, Tooling API client, and TestKit client will remain compatible with Java 8 and will be compiled to target JVM 8 bytecode.
These daemon clients can be configured to launch Gradle daemons with a different JVM version than the one used to run the client:
...
Alternatively, the `JAVA_HOME` environment variable can be set to a JVM 17 or newer.

Running Gradle builds with <<gradle_daemon#sec:disabling_the_daemon,--no-daemon>> or using link:{javadocPath}/org/gradle/testfixtures/ProjectBuilder.html[ProjectBuilder] will require a JVM version 17 or later.

We have decided to upgrade the minimum version of Java that Gradle will run on for a number of reasons:

- Gradle's own dependencies are beginning to drop support for older JVM versions. We are concerned about the security risks of not being able to upgrade dependencies due to Java version compatibility constraints.
- Some of the largest ecosystems that leverage Gradle already require JVM 17 or later.
- The language improvements between Java 8 and Java 17 are significant. Upgrading improves development and maintenance efficiency within the Gradle codebase.

In upcoming major Gradle releases, we will continue to evaluate the minimum JVM version required to run Gradle.
