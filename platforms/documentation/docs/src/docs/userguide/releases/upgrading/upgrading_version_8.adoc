...
In previous versions of Gradle, some JVM projects that did not declare any dependencies were erroneously compiled against the Gradle API.
This meant that compilations without any declared dependencies were able to compile against Gradle classes without declaring a `gradleApi` dependency.
Declare a `gradleApi` dependency or use the `java-gradle-plugin` plugin if you wish to compile against the Gradle API.
