...
The Gradle Wrapper JAR (gradle-wrapper.jar) has been converted into an executable JAR.
This means it now includes a `Main-Class` attribute in its manifest, allowing it to be launched using the `-jar` option instead of specifying a classpath and main class manually.
