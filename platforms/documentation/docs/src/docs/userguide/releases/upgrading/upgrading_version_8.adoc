...
[[validate_plugins_without_stricter_validation]]
==== Not enabling `ValidatePlugins.enableStricterValidation` is deprecated

It is now required to enable stricter validation of plugin types.

The `ValidatePlugins` task is registered by the `java-gradle-plugin`, `groovy-gradle-plugin` and `kotlin-dsl` plugins.
Stricter validation needs to be enabled in your local build logic (`buildSrc` or included plugins builds).
This is also required if you are simply building Gradle plugins.
...
Starting with Gradle 9.0, stricter validation of plugin types will be enabled by default.
