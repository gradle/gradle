...
The Gradle Client can still run on Java 8.
...
==== Archive tasks now produce reproducible archives by default

In Gradle 9, the default behavior of archive tasks (such as `Jar`, `Ear`, `War`, `Zip`, and `AbstractArchiveTask`) has changed to produce reproducible archives by default.
That means that:
