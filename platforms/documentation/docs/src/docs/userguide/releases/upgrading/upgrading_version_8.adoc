...
There is no way to use these method without breaking configuration cache requirements so it is advised to migrate to a compatible alternative.
The appropriate replacement for your use case depends on the context the method was called before.
...
The standard Java/Groovy/Kotlin process APIs, like `java.lang.ProcessBuilder` can be used there too.
