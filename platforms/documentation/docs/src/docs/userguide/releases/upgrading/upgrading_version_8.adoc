...
==== Plugins written with the Groovy DSL support only Gradle 7.0 and above

Plugins built and published using the Groovy DSL on Gradle 9.x are only supported on Gradle 7.0 or above.
Gradle 7.0 is the first release that embeds Groovy 3.0 and can run Groovy 4.0 code.

Since Gradle 9.0 uses Groovy 4.0, some code may behave unexpectedly. For full compatibility, it's recommended to run these plugins on Gradle 9.0 or later.
