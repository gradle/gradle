...
We implemented several refactorings around the Problems API.
One notable change is the separation if a problem definition and contextual information.
The complete design spec is available here: https://docs.google.com/document/d/1T_vM-Upa23aA21sanFTTLZa3j9xV6R32djJk6-muWzI/edit#heading=h.610fausqnpu6
Implementing the spec, we introduced the following breaking changes to the `ProblemSpec` interface:
the `label(string)` and `description(String) methods are replaced with the `id(String, String)` method and its overloaded variants.
