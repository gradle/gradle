...
This is a breaking change from previous versions, which supported Java 8 and higher.

Your build can still target lower Java versions using <<toolchains.adoc#toolchains, Toolchains>> for compilation, testing and other workers (Checkstyle, Javadoc, etc).

The <<gradle_wrapper.adoc#gradle_wrapper_reference,Gradle wrapper>> and command-line launcher will run with Java 8 until it needs to start a Gradle daemon.
For more, see the <<#minimum_daemon_jvm_version,Running Gradle on older JVMs>> section below.

<<tooling_api.adoc#embedding,Gradle Tooling API>> remains compatible with Java 8 and higher.
...
For a complete overview of these and more changes from 3.x to 4.x, see the https://groovy-lang.org/releasenotes/groovy-4.0.html[Groovy 4.0 release notes].
