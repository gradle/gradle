...
. Try running `gradle help --scan` and head over to the https://gradle.com/enterprise/releases/2018.4/#identify-usages-of-deprecated-gradle-functionality[deprecations view] of the generated Build Scan.
+
...
. Run your project and debug any errors using the <<troubleshooting.adoc#troubleshooting, Troubleshooting Guide>>.
...
Gradle now uses the `--sdk macosx` argument with `xcrun` to reliably locate the macOS SDK for Swift, ensuring consistent discovery across different environments.
...
For instance, this will *not* be allowed:
...
Currently, in the Groovy DSL, you can set a property using several syntaxes:
...
The last option, _space-assignment_, is specific to Gradle and not part of standard Groovy.

In standard Groovy, `propertyName(value)` is just a method call.
Gradle dynamically generates the `propertyName` method at runtime if it doesn't already exist.
...
These generated methods are now deprecated and will be removed in Gradle 10.0.
Both `propertyName value` and `propertyName(value)` will stop working unless the explicit method `propertyName` is defined.
...
Such a request will now fail due to ambiguity.
To avoid issues, use the exact name instead of a shortened version.
