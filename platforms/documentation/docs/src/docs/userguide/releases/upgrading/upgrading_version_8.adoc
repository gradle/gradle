...
To ensure the accuracy of dependency resolution, Gradle ensures that Configurations are not mutated after they have been used as part of a dependency graph.
...
Gradle already throws an exception for many of these circumstances, but in some cases, it does not.
As of Gradle 9.0, all these cases will result in an error.
