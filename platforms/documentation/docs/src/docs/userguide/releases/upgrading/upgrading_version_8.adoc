...
However, for builds that have configured to fail on warnings - either Kotlin script or plugin code compilation - this may cause the build to fail.

A full Gradle deprecation of these methods will happen in a later version.
