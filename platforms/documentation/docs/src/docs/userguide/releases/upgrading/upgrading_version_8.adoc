...
[[deprecated_string_value_to_enum_rich_property_assignment]]

Assigning a string value to an enum rich property e.g. `Property<T extends Enum>` is deprecated and will become an error with Gradle 10.0.
This behaviour was implemented only as a compatibility feature for Groovy DSL for easier migration of core Gradle APIs to rich properties.
