...
You can define [secondary variants](https://docs.gradle.org/8.9/userguide/publishing_customization.html#sec:publishing-custom-components) for a consumable primary variant that has no artifacts.
...
Starting with Gradle 9.0, such primary variants are now considered during variant selection, even if they have no artifacts.
