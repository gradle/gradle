...
* Enabling the incubating <<structuring_software_products_details.adoc#sec:configuration_on_demand, configuration on demand>> feature is not recommended as it can lead to very hard-to-diagnose problems.
...
Utilizing the <<multi_project_builds.adoc#sec:multiproject_buildsrc,_buildSrc_ project>> is about organizing your build logic into shared local plugins and conventions that are easily testable and provide good IDE support.
