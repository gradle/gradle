 ...
 * Define a <<declaring_repositories_adv.adoc#sub:flat_dir_resolver,flat-directory repository>> and use standard dependency declarations
...
NOTE: If you store your dependencies in the standard Maven repository layout — `<repoDir>/<group>/<module>/<version>` — then you can define a <<declaring_repositories_basics.adoc#sec:declaring_custom_repository,custom Maven repository>> with a `file://` URL.
...
You are able to <<declaring_dependencies_adv.adoc#sec:defining-custom-configurations,define your own dependency configurations>> if needed.
...
See the chapters on <<dependency_configurations.adoc#sec:what-are-dependency-configurations,Managing Dependency Configurations>>, <<declaring_dependencies.adoc#sec:dependency-types,Declaring Dependencies>> and <<declaring_repositories.adoc#declaring-basic-repositories,Declaring Repositories>> for more information.
...
Gradle searches the repositories for those dependencies, with the search order defined by <<declaring_repositories_adv.adoc#sec:declaring_multiple_repositories,the order in which the repositories are declared>>.
+
...
If you decide to use such <<dependency_versions.adoc#sub:declaring_dependency_with_dynamic_version,dynamic>> and <<dependency_versions.adoc#sub:declaring_dependency_with_changing_version,changing>> dependencies, you can configure the caching behavior for them via link:{groovyDslPath}/org.gradle.api.artifacts.ResolutionStrategy.html[resolutionStrategy].
+
