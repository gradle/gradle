...
First you must find the dependency that is causing the unwanted transitive.
You can use the `dependencies` task for this.
In this example, we want to remove the `commons-collections:commons-collections` dependency brought by:
...
Running `./gradlew dependencies --configuration runtimeClasspath` showcases how `commons-collections:commons-collections` is brought in:
...
This example actually goes a step further as `com.opencsv:opencsv` brings in `commons-collections4`.
...
** **Use Dependency Constraints** - If a dependency version conflict occurs, **adjust the version using a <<dependency_constraits.adoc#dependency-constraints,constraint>>** rather than excluding the dependency entirely.
