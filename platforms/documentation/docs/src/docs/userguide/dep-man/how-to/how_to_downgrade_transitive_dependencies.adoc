...
Additionally, you should make sure there aren't existing <<dependency_constraints.adoc#dependency-constraints,dependency constraint>>, <<platforms.adoc#platforms,platforms>>, or <<version_catalog.adoc#version-catalog,version catalogs>>, that are setting the dependency to an undesired version in your project.
