...
In Gradle, it's important to separate the configuration where you declare dependencies from the one you resolve artifacts.
This avoids ambiguity, improves maintainability, and ensures compatibility with Gradle's evolving configuration model.

First, the consumer project creates two configurations:

* `instrumentedRuntimeDependencies` is used to declare that the consumer depends on the producer's `instrumentedJars`.
* `instrumentedRuntime` is a resolvable configuration that extends the dependency declaration and is used by the `runWithInstrumentation` task.
...
Now, the consumer project requests the instrumented JAR by defining matching attributes.
