...
As explained in the <<declaring_configurations.adoc#sec:resolvable-consumable-configs, configurations chapter>>, this corresponds to a _consumable configuration_.
...
In Gradle terminology, this is called a <<declaring_configurations.adoc#sec:resolvable-consumable-configs,resolvable configuration>>, which is defined this way:
...
In fact, we also <<variant_model.adoc#sec:understanding-variant-selection, have explained>> that it is possible for a consumer to express requirements using _attributes_ and that the producer should provide the appropriate outgoing variants using attributes too.
...
More details can be found in the <<variant_attributes.adoc#variant-attributes, variant aware plugins section of the documentation>>.
