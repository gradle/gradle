...
You can add repositories for your dependencies by configuring the repositories block in your `build.gradle(.kts)` file:
...
** a wide variety of <<#sec:supported_transport_protocols,remote protocols>> such as HTTPS, SFTP, AWS S3 and Google Cloud Storage based on the presence of artifacts.
