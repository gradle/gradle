...
When the same library is declared multiple times or when two different libraries are used with the same functionality, a conflict can occur during your build.
...
2. *Capability conflicts*: That is when the dependency graph contains multiple components that provide the same implementation.
...
The configuration API provides additional solutions.
