...
In the metadata for Jackson Databind, we see that the `runtimeElements` variant is depicted by the `org.gradle.category`, `org.gradle.dependency.bundling`, `org.gradle.libraryelement`, and `org.gradle.usage` attributes:
...
----
<1> This is `"org.gradle.usage": "java-api"` for `apiElements`
...
And their own transitive dependencies including `jackson-databind` which corresponds to `jackson-databind-2.17.2.jar`.
