...
However, note that **link:https://central.sonatype.com/[Maven Central]** is moving away from the legacy protocol used by the link:https://maven.apache.org/plugins/maven-deploy-plugin/[Maven Deploy Plugin].
Publishing to Maven Central now **requires a third-party plugin**.
