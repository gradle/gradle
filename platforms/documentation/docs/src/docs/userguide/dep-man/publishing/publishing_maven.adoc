...
A module published to a Maven repository can be consumed by Maven, Gradle (see <<declaring_dependencies.adoc#declaring-basic-dependencies,Declaring Dependencies>>) and other tools that understand the Maven repository format.
...
To detect such conflicting duplicate dependencies, <<component_capabilities.adoc#declaring-component-capabilities,capabilities>> can be published as part of the <<publishing_gradle_module_metadata.adoc#sec:understanding-gradle-module-md,Gradle Module Metadata>>.  For an example using a <<java_library_plugin.adoc#java_library_plugin,Java Library>> project, see <<component_capabilities.adoc#declaring-component-capabilities,declaring additional capabilities for a local component>>.
...
If you want to verify that the checksum files are created correctly, or use them for later publishing, consider configuring a <<declaring_repositories_adv.adoc#sec:maven_repo,custom Maven repository>> with a `file://` URL and using that as the publishing target instead.
