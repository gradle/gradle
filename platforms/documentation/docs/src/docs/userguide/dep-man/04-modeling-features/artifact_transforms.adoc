...
With Artifact Transforms, you can adjust the files, or artifacts - like JAR files - delivered by a dependency as the last step in resolving it.
...
If the component does not have a matching variant, resolution fails *unless* Gradle can construct a sequence of transformations that will modify an existing variant to create a valid match (without changing its transitive dependencies).

**Artifact Transforms** are a mechanism for converting one type of artifact into another during the build process.
They provide a consumer an efficient and flexible mechanism for transforming the artifacts of a given producer to some required format without needing the producer to expose variants in that format.
...
2. **Register a Transform**: You register the transform by using the link:{javadocPath}/org/gradle/api/artifacts/dsl/DependencyHandler.html#registerTransform(java.lang.Class,org.gradle.api.Action)[`registerTransform()`] method of the link:{javadocPath}/org/gradle/api/Project.html#dependencies(groovy.lang.Closure)[`dependencies`] block.
...
Attributes are key-value pairs used to describe variants, like `org.gradle.usage=java-api` and `org.gradle.usage=java-runtime`.

3. **Use the Transform**: When a resolution requires a variant that isn't already present in the selected component (because none of the actual variants possess compatible attributes to the requested attributes), Gradle doesn't just give up!
...
"To" attributes are supply describe the format or type of the artifacts that this transform can use as an input, and "from" attributes to describe the format or type of the artifacts that it produces as an output.
...
Since no variants exist supplying the requested format (as none contain the `artifactType` attribute with a value of `"transformed-jar"`), Gradle attempts to construct a chain of transformations that will supply it.
...
You can think of the chain of transforms as producing a "virtual variant" that doesn't exist in a component itself, but is instead created on-demand by Gradle as part of a resolution.
...
These variants can each provide a different collections of files, meant to satisfy different use cases, such as compiling code, browsing documentation or running applications.
...
<<variant_attributes.adoc#sec:abm_compatibility_rules,**matching**>>.

Sometimes, a dependency might not have a variant with attributes that match the requested attributes.
In such cases, Gradle can transform one variant into another "vitual variant" by modifying its artifacts without changing its transitive dependencies.

IMPORTANT: Gradle will not ever run Artifact Transforms when a variant of the dependency matching the requested attributes already exists.
...
- Gradle starts with the attributes specified in a dependency resolution request (typically those present on the configuration being resolved).
...
How could multiple chains produce different suitable results?
