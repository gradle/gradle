...
1. **Defining a Transform**: You define an artifact transform by creating a class that implements the `TransformAction` interface.
...
3. **Registering a Transform**: You register the transform in your build script using the `transform()` method of the `dependencies` configuration.
...
The class implements the `TransformAction` interface.
It can optionally have parameters defined in a separate interface.

Each transform has exactly one input artifact.
It must be annotated with the `@InputArtifacts` annotation.

Then, you implement the `transform()` method from the `TransformAction` interface.
This is where you implement the work the transform should do when triggered.
The method has the `TransformOutputs` as an argument which defines what the transform produces.
...
A transform must be registered the same way a task is registered using `dependencies.registerTransform()`:

Here, our transform is registered with the `dependencies` configuration:
...
== Understanding Different Variants of Dependencies and Transforming Artifacts on Resolution
...
For example, the `java-api` variant of a dependency might be identified by the attribute `org.gradle.usage` with the value `java-api`.

=== Resolving Configurations

When Gradle resolves a configuration (a group of dependencies), it uses the attributes defined in the configuration to select the appropriate variant of each dependency.
...
For example, if the requested variant is `java-api,classes`, but the dependency only has `java-api,jar`, Gradle can transform the JAR file into a `classes` directory by unzipping it.
This process is known as an **Artifact Transform**.
...
For example, consider a transform that unzips a JAR file and converts from `org.gradle.usage=java-api, org.gradle.libraryelements=jars` to `org.gradle.usage=java-api, org.gradle.libraryelements=classes`.

=== How Gradle Resolves Variants

1. **Starting with Requested Attributes**:
...
2. **Finding a Path to Existing Variants**:
...
- Transforms that only change `artifactType` are not considered during configuration resolution but can be used in an `ArtifactView`.
...
- For `guava`, Gradle converts `org.gradle.usage=java-runtime, org.gradle.libraryelements=jar, minified=false` to `org.gradle.usage=java-runtime, org.gradle.libraryelements=jar, minified=true`.
- For `producer`, Gradle converts `org.gradle.usage=java-runtime, org.gradle.libraryelements=jar, minified=false` to `org.gradle.usage=java-runtime, org.gradle.libraryelements=jar, minified=true`.
...
To set up the `minified` attribute so that the above works:

1. Register the new attribute in the schema.
2. Add the attribute to all JAR artifacts.
3. Request the attribute on all resolvable configurations.
