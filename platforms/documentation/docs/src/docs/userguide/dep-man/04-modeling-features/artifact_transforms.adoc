...
What if you want to adjust the JAR file of one of the dependencies you get from a repository before you use it?
...
When Gradle resolves a configuration, it looks at each dependency and finds its variant with the requested attributes is selected.
When the dependency does not have a variant, resolving the configuration fails unless Gradle finds Artifact Transforms that transform the dependency's artifacts into the required format (without changing its transitive dependencies).
...
Here's a brief overview of Artifact Transforms:
...
On the command line, Gradle runs tasks and not Artifact Transforms: `./gradlew build`.
So how and when does it run transforms?

A task can declare an input with attributes.
When running this task, Gradle may run an artifact transform to ensure that the input to the task has the proper attributes.
...
Unlike a task, which is invoked once when all its dependencies finished executing, transforms can be requested by many tasks in different parts of the build.
...
Gradle will check if it needs to execute this request.
...
If the project dependency is badly declared (eg missing task input) then the transform execution will happen inside the task.
