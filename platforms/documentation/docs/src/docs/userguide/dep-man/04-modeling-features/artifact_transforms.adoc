...
This is done as the last step in resolving a dependency, before tasks or tools like the IDE uses it.
...
4. **Use the Transform**: When a resolution requires a variant that isn't already present in the selected component (because none of the actual variants possess compatible attributes to the requested attributes), Gradle doesn't just give up!
...
IMPORTANT: Gradle will not ever attempt to select or run Artifact Transforms when a variant of the dependency matching the requested attributes already exists.
