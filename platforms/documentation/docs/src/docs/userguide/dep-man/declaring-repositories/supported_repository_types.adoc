...
See <<supported_repository_protocols.adoc#supported_transport_protocols, Supported Repository Protocols>> for authentication options.
...
NOTE: Gradle manages its own <<dependency_resolution_adv.adoc#sec:dependency_cache,cache>> and doesn't need to declare the local Maven repository even if you resolve dependencies from a remote Maven repository.
