...
Gradle supports additional <<supported_repository_protocols.adoc#supported_transport_protocols,protocols>> beyond `http` and `https`, such as `file`, `sftp`, and `s3` for custom URLs.
