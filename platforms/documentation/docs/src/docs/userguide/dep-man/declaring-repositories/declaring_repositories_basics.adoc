...
Alternatively, you can <<#sec:declaring_custom_repository,explicitly specify the URL of the repository>> for more fine-grained control.
...
For a full coverage see the section on <<supported_repository_types.adoc#repository-types,supported repository types>>.
...
Local repositories differ from flat directory repositories in that local repositories follow a directory structure and contain metadata.

Gradle completely bypasses the <<dependency_resolution.adoc#sec:dependency_cache,dependency cache>> for dependencies sourced from local repositories, as there can be no guarantee that content may not change between executions.
...
In general, external repositories should be preferred over using local repositories. Binary dependencies generally should not be stored in source control.
