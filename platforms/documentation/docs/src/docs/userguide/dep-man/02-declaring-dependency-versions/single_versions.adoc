...
The symbol `]` can be used instead of `(` for an exclusive lower bound and `[` instead of `)` for an exclusive upper bound. e.g. `]1.0, 2.0[`. Note: The range `[1.0,[` is not appropriate, as it lacks an upper bound, and can be rewritten as `[1.0,)`.
