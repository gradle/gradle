...
Gradle treats all entries in the block of a BOM similar to <<dependency_constraints.adoc#sec:adding-constraints-transitive-deps,Adding Constrains On Dependencies>>.
...
Instead, if your reusable component strongly favors specific third-party dependency versions, consider using a <<rich_versions.adoc#sec:strict-version,rich version>> declaration with `strictly`.
