...
- *Conflict Resolution*: Like regular dependency notation, version catalogs declare requested <<#sec:common-version-numbers,versions>> but do not enforce them during <<dependency_resolution_adv#understanding_dependency_resolution,conflict resolution>>.
