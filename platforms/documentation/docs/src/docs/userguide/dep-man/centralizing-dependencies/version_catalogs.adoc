...
In **precompiled script plugins** inside `buildSrc`, the version catalog can be accessed using `extensions.getByType(VersionCatalogsExtension).named("libs")`:
...
However, the plugins block in the **precompiled script plugins** does not allow access to the version catalog.
