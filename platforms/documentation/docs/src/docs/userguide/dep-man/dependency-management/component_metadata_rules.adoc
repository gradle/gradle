...
Each module pulled from a repository includes metadata, such as its group, name, version, and the various variants it provides along with their artifacts and dependencies.
...
The key difference between using metadata rules and defining dependencies/artifacts in a build script is that component metadata rules operate directly on <<variant_model.adoc#understanding-variant-selection,variants>>, whereas build scripts often affect multiple variants at once (e.g., an `api` dependency is applied to both `api` and `runtime` variants of a Java library).
...
- **Attributes**: Use the `attributes {}` block to adjust <<variant_model.adoc#sec:abm_configuration_attributes,attributes>> that identify the variant.
- **Capabilities**: Use the `withCapabilities {}` block to define the <<component_capabilities.adoc#declaring-component-capabilities,capabilities>> the variant provides.
- **Dependencies**: Use the `withDependencies {}` block to manage the variant’s dependencies, including <<rich_versions.adoc#rich-version-constraints,rich version>> constraints.
...
- **Component Attributes**: The only meaningful attribute here is `org.gradle.status`.
...
- **POM Metadata**: For modules published with `.pom` metadata, fixed variants are derived as explained in the <<variant_model.adoc#sub:maven-mapping-to-variants,"Mapping POM Files to Variants">>, section.
- **Ivy Metadata**: If a module was published with an `ivy.xml` file, Ivy configurations can be accessed in place of variants. Their dependencies, constraints, and files can be modified. You can also use `addVariant(name, baseVariantOrConfiguration)` to derive variants from Ivy configurations, such as defining <<variant_model.adoc#sub:ivy-mapping-to-variants,`compile` and `runtime` variants for the Java library plugin>>.
...
- **Modules with Gradle Module Metadata**: These typically have complete metadata, but issues can still occur. Only apply component metadata rules if you've clearly identified a problem with the metadata. For dependency resolution issues, first consider using [dependency constraints with rich versions](rich_versions.adoc#rich-version-constraints). If you're developing a library, note that dependency constraints are published as part of your own library’s metadata, making it easier to share the solution with consumers. In contrast, component metadata rules apply only within your own build.
...
TIP: Ensure that the rule you create works beyond your build and still provides a correct and useful result if applied in other builds that use the affected module(s).
...
Optional dependencies are not processed automatically by Gradle or Maven, as they indicate <<feature_variants.adoc#feature_variants,feature variants>> that require additional dependencies.
However, the `pom` file lacks information about these features and their corresponding dependencies.
This can be represented in Gradle Module Metadata through variants and <<component_capabilities.adoc#declaring-component-capabilities,capabilities>>, which we can add via a component metadata rule.
...
In some cases, variants are published as separate files rather than being packaged together in a single jar.
This is common when the variants are mutually exclusive, offering alternative choices rather than optional features.
An example is found in pom-based libraries, where runtime and compile variants are distinct, and Gradle selects one depending on the task.
A similar scenario often seen in the Java ecosystem is jars targeting different Java versions.
...
This information can influence the <<dependency_resolution_adv.adoc#understanding_dependency_resolution,version selection>> process for a module during dependency resolution, which is performed _before_ one or multiple variants of a component are selected.
...
A module's status is taken into consideration when a <<dependency_versions.adoc#single-version-declarations,_latest_ version selector>> is resolved.
