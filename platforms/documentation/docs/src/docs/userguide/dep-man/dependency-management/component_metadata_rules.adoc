...
- The <<declaring_dependencies.adoc#declaring-basic-dependencies,dependencies>> of the variant, including <<rich_versions.adoc#rich-version-constraints,rich versions>> — `withDependencies {}` block
...
In addition, there are `add(notation, configureAction)` methods accepting the usual notations similar to <<declaring_dependencies.adoc#declaring-basic-dependencies,declaring dependencies>> in the build script.
