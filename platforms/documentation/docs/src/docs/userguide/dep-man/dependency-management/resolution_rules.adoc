...
|<<sec:dependency-resolve-rules,10>>
|Modifying Configuration Dependencies Before Resolution
|Dynamically modify a configuration's dependencies before Gradle resolves them.
|===
...
[[changing_configuration_dependencies_prior_to_resolution]]
== 10. Changing configuration dependencies prior to resolution

The `withDependencies` method in Gradle allows you to dynamically modify a configuration's dependencies before Gradle resolves them.
This is useful when you need to alter, enforce, or remove dependencies programmatically based on some conditions.
