...
<<variant-attributes.adoc#variant-attributes,Variants>> are essentially different forms of a dependency, often based on factors such as platform (e.g., JVM or Android), or configuration (e.g., compile, runtime).

<<variant-attributes.adoc#variant-attributes,Capabilities>> are a way to express mutually exclusive variants of a dependency.
