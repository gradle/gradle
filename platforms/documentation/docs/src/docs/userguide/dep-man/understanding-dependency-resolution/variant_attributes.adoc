...
**Variants** represent different versions or aspects of a component, like `api` vs `implementation` or `debug` vs `release`.
...
To understand how Gradle's dependency management engine works to select the best matching variant, see our <<variant_model.adoc#sec:variant-aware-matching,Understanding Variant Selection>> chapter.
...
.build.gradle.kts
----
// build.gradle.kts in the library project

import org.gradle.api.attributes.Attribute

val jvmVersionAttribute = Attribute.of("org.gradle.jvm.version", String::class.java)

configurations {
    create("apiElements") {
        attributes {
            attribute(jvmVersionAttribute, "17")
        }
    }
}
----
...
dependencies {
    implementation("com.example:your-library:1.0") {
        attributes {
            attribute(TargetJvmVersion.TARGET_JVM_VERSION_ATTRIBUTE, 17) // Shorthand notation for Gradle defined attributes
        }
    }
}
----
...
Published attributes must remain supported or have a <<#sec:abm_compatibility_rules,compatibility layer>> in future versions of the plugin to ensure backward compatibility.
...
dependencies {
    ...
    myConfig("com.example:my-library:1.0") {
        attributes {
            attribute(myAttribute, "special-value")
        }
    }
...
Suppose you have a library published with a variant that has the following attributes:

- `Usage`: `java-runtime`
- `JavaLanguageVersion`: `1.8`

A consumer might request a variant with:

- `Usage`: `java-api`
- `JavaLanguageVersion`: `11`

In this case, the consumer's request does not match the exact attributes of the library's available variant.
However, because of compatibility rules, Gradle can determine that the `java-runtime` variant can be used as a fallback.

To set up attribute compatibility rules, you need to register these rules via the attribute matching strategy.

Here's a simple example of how to define a compatibility rule in a Gradle build script:
...
// Register the compatibility rule
configurations.all {
    attributes {
        // Define a compatibility rule for JavaLanguageVersion
        attributeMatchingStrategy(javaLanguageVersion) { strategy ->
            strategy.compatibleWith("1.8") {
                // Define compatibility rule logic here
                true // This means all versions are compatible
            }
            strategy.compatibleWith("11") {
                // Logic for Java 11 compatibility
                false // No compatibility with Java 11
            }
        }
    }
}
