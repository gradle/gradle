...
After covering how you can declare <<declaring_repositories.adoc#three-declaring-repositories,repositories>> and <<declaring_dependencies.adoc#one-declaring-dependencies,dependencies>>, it makes sense to explain how these declarations come together during dependency resolution.
...
[[sec:handle-conflict-resolution]]
...
<<dependency_versions.adoc#sec:version-ordering,here>>.
...
All the rules of <<dependency_versions.adoc#sec:version-ordering,version ordering>> still apply, but the conflict resolver
...
Learn more about handling these type of conflicts in <<component_capabilities.adoc#sub:selecting-between-candidates,Selecting between candidates>>.
