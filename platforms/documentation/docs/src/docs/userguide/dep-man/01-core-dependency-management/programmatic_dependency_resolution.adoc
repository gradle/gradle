...
These three artifact sets represent the distributable, but in different forms.
...
To perform artifact selection (and implicitly, graph resolution) using these defaults attributes, use the `FileCollection` and `ArtifactCollection` APIs:
...
The `Configuration` directly implements `FileCollection` and can be wire directly.
...
NOTE: The `ArtifactView` can produce results as both `FileCollection`s and `ArtifactCollection`s.
The below examples will only demonstrate using `FileCollection`s as the output.

==== Triggering artifact transforms

`ArtifactView`s can be used to trigger artifact selection using attributes different from those used to resolve the graph.
