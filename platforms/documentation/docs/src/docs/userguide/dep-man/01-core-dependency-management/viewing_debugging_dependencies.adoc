...
By default, the dependency tree renders dependencies for all <<dependency_configurations.adoc#sec:what-are-dependency-configurations,configurations>> within a <<command_line_interface#executing_tasks_in_multi_project_builds,single project>>.
...
- `(n)`: A dependency or dependency configuration that <<declaring_dependencies_adv.adoc#sec:resolvable-consumable-configs,cannot be resolved>>.
...
You can declare dependencies for external tooling with the help of a <<dependency_configurations.adoc#sec:what-are-dependency-configurations,custom dependency configuration>>.
...
Gradle applies <<dependency_resolution_adv.adoc#sec:version-conflict,version conflict resolution>> to ensure that only one version of the dependency exists in the dependency graph.
...
Dependency insights provide information about a single dependency within a single <<dependency_configurations.adoc#sec:what-are-dependency-configurations,configuration>>.
...
For more information about configurations, see the <<dependency_configurations.adoc#sec:what-are-dependency-configurations,dependency configuration documentation>>.
...
| Was requested : <text>
| The dependency appears in the graph, and the inclusion came with a <<declaring_dependencies_adv.adoc#sec:documenting-dependencies, `because` text>>.
...
This resulted in <<dependency_resolution_adv#sec:version-conflict, conflict resolution>> to select the most appropriate version.
