...
You can learn more about the inner workings of <<dependency_resolution_adv.adoc#sec:how-gradle-downloads-deps, dependency downloads>>.
====
...
When such a repository is configured, Gradle totally bypasses its <<dependency_resolution_adv.adoc#sec:dependency_cache,dependency cache>> for it as there can be no guarantee that content may not change between executions.
...
NOTE: Gradle stores resolved dependencies in <<dependency_resolution_adv.adoc#sec:dependency_cache,its own cache>>.
...
WARNING: Before adding Maven local as a repository, you should <<supported_repository_types.adoc#sec:case-for-maven-local, make sure this is really required>>.
...
** For example, if Maven never downloaded the source or javadoc files for a given module, Gradle will not find them either since it <<dependency_resolution_adv.adoc#sec:how-gradle-downloads-deps,searches for files in a single repository>> once a module has been found.
...
* To mitigate the fact that metadata and/or artifacts can be changed, Gradle does not perform <<dependency_resolution_adv.adoc#sec:dependency_cache,any caching>> for <<sub:local-repos,local repositories>>
...
After all these warnings, if you end up using `mavenLocal()`, consider combining it with <<filtering_repository_content.adoc#repository-content-filtering,a repository filter>>.
...
When using a strict version, it is possible to use a version range, using <<dependency_versions.adoc#single-version-declarations,the format supported>> by Gradle.
