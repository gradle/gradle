...
The symbol `]` can be used instead of `(` for an exclusive lower bound (and vice versa).
...
----
dependencies {
    implementation("org.springframework:spring-core:1.1") // This is a higher version than 1.a
    implementation("org.springframework:spring-core:1.a") // This is a lower version than 1.1
}
----
...
** Only the parts are compared, not the separators, so `1.a.1`, `1-a+1`, `1.a-1`, and `1a1` are equivalent. (Note: There are <<dependency_resolution#sec:base-version-comparison,exceptions>> during conflict resolution).
...
+
When defined, it overrides any previous `require` declaration and clears any previous `reject`.
...
Dynamic versions are supported.
+
When defined, it overrides any previous `strictly` declaration and clears any previous `reject`.
...
+
When defined, it overrides any previous `prefer` declaration and clears any previous `reject`.
...
Lines annotated with a lock (🔒) indicate situations where leveraging <<dependency_locking.adoc#dependency-locking,dependency locking>> is recommended.
Another related concept is the ability to publish <<publishing_maven.adoc#publishing_maven:resolved_dependencies,resolved versions>> instead of the declared ones.
...
For more details, refer to the section on <<endorsing_strict_versions.adoc#sec:enforcing_dependency_version,overriding dependency versions>>.
...
If you need to enforce a specific version at the configuration level, use <<resolution_rules.adoc,resolution_rules,Resolution Rules>> instead:
...
Differences between forced and strict dependencies:

- **Forced Dependencies:** Apply to the configuration level and override any transitive versions within that configuration.
- **Strict Dependencies:** Are applied transitively to the subgraph and can lead to build failures if there are conflicts with the consumer's requirements.

Use these approaches with caution to ensure compatibility and avoid unexpected build issues.

[[sec:declaring_without_version]]
== Declaring without version

For larger projects, it's advisable to declare *dependencies without versions* and manage versions using <<dependency_constraints.adoc#dependency-constraints,dependency constraints>>:
...
CAUTION: For reproducible builds, it’s crucial to use <<dependency_locking.adoc#dependency-locking,dependency locking>> when declaring dependencies with dynamic versions. Without this, the module you request may change even for the same version, which is known as a <<#sub:declaring_dependency_with_changing_version,changing version>>.
