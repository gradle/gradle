...
If Gradle detects two components providing the same capability (e.g., different bindings for a logging framework), it will fail the build with an error, indicating the conflicting modules. This ensures that conflicting implementations are resolved, avoiding issues on the classpath.

Here's a simple example using Gradle's capability concept to resolve conflicts between different logging framework bindings:

[source,kotlin]
----
dependencies {
    implementation("org.slf4j:slf4j-api:1.7.30")
    implementation("ch.qos.logback:logback-classic:1.2.3") // logback binding for slf4j
    implementation("org.slf4j:slf4j-simple:1.7.30")        // another slf4j binding
}

configurations.all {
    resolutionStrategy.capabilitiesResolution.withCapability("org.slf4j:slf4j-impl") {
        select("ch.qos.logback:logback-classic")
    }
}
----

In this example, two bindings (`logback-classic` and `slf4j-simple`) provide the same capability for `slf4j-impl`.
Gradle will fail if both are on the classpath.
The resolution strategy tells Gradle to **select** the `logback-classic` binding, resolving the conflict.
...
Gradle will fail by default if two components in the dependency graph provide the same capability.
Since many modules are published without Gradle Module Metadata, capabilities are not always automatically recognized by Gradle.
...
Including both versions on the classpath is illegal because they provide the same feature, just under different coordinates.
...
<<#sub:capabilities,Capabilities>> help _understand_ the conflict, then Gradle provides you with tools to solve the conflicts.
...
For instance, in testing environments, the lightweight `slf4j-simple` logging implementation might be sufficient, while in production, a more robust solution like `slf4j-over-log4j` may be preferable.
