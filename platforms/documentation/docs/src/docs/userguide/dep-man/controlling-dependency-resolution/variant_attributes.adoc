...
class MyCustomDisambiguationRule @Inject constructor(private val objects: ObjectFactory) : AttributeDisambiguationRule<Usage> {
    override fun execute(details: MultipleCandidatesDetails<Usage>) {
        ...
        details.closestMatch(objects.named(Usage::class.java, Usage.JAVA_API))
    }
}
----
