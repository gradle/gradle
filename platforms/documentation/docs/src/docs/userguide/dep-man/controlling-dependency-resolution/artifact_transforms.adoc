...
== Using Artifact Transforms with Artifact Views

To leverage <<artifact_views.adoc#artifact_views,Artifact Views>> for Artifact Transforms, you can configure a custom `ArtifactView` to access the transformed artifacts.
...
The example below demonstrates how to define a custom artifact transform (`TestTransform`), register it, and then use it to manipulate artifacts, allowing the consumer configuration to resolve transformed artifacts (`stub`).
...
abstract class TestTransform : TransformAction<TransformParameters.None> {
    ...
    override fun transform(outputs: TransformOutputs) {
        outputs.file("stub").createNewFile()
    }
}

project.dependencies.registerTransform(TestTransform::class.java) {
...
val resolvedArtifactType = consumer.incoming.artifacts.resolvedArtifacts.map {
    it.single().variant.attributes.getAttribute(ArtifactTypeDefinition.ARTIFACT_TYPE_ATTRIBUTE)
}

tasks.register("testArtifact") {
    doLast {
        println(resolvedArtifactType.get())
    }
}
----
