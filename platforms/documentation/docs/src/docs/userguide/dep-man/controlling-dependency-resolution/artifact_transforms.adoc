...
Since no variants exist supplying artifacts of requested format (as none contain the `artifactType` attribute with a value of `"transformed-jar"`), Gradle attempts to construct a chain of transformations that will supply the proper artifact.
...
TIP: Gradle applies transformations to artifacts, not variants. New variants are *not* produced.

== Leveraging Artifact Views

To leverage <<artifact_views.adoc#artifact-views,Artifact Views>> with Artifact Transforms, you can configure a custom `ArtifactView` to access the transformed artifacts.
This allows you to apply transformations to the artifacts in the view and further process or consume them.
Typically, `artifactView` is used with a specific transform type to modify or manipulate the artifacts as they are resolved.

In the example below, the `TestTransform` class defines a transformation and the transformation is registered to process artifacts of type "jar" into artifacts of type "stub".
The `consumer` configuration is then created to resolve artifacts of type "stub", and the `testArtifact` task queries and prints the resolved artifact's attributes, demonstrating the successful transformation from JAR to STUB:
...
When Gradle resolves a configuration and a dependency does not have a variant with the requested attributes, it attempts to find a chain of one or more Artifact Transforms that can be run sequentially to create the desired artifact.
