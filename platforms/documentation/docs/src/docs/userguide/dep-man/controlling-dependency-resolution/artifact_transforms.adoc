...
To run an Artifact Transform, you can configure a custom <<artifact_views.adoc#artifact-views,Artifact View>> to request artifacts belonging to a variant that doesn't actually exist in a dependency.
...
Gradle will run these transformations on the original artifacts found in a variant of the dependency to produce a result compatible with the attributes in the view.
