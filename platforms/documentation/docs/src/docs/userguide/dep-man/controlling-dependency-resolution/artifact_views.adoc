...
1. Trigger artifact transforms
2. Select _alternative_ variants, such as sources or javadoc, for an entire resolution
3. Perform _lenient_ artifact selection and resolution
4. Filter selected artifacts
...
== 1. Triggering artifact transforms
...
Most commonly, this API is used to request attributes that are not present on the variant that artifacts are being selected from.
...
== 2. Performing variant reselection
...
== 3. Performing lenient artifact resolution
