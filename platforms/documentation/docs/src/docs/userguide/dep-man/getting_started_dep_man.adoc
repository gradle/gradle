...
These _configurations_ are typically available by applying plugins or can be created using APIs.
...
During <<graph_resolution.adoc#dependency-graph-resolution,graph resolution>>, Gradle constructs a _dependency graph_, resolving version conflicts and ensuring a consistent set of dependencies.

=== 3. Variant Selection

<<variant_aware_resolution.adoc#sec:understanding-variant-selection,Variant selection>> refines the resolved dependency graph by choosing the most compatible _variant_ of each module based on _attributes_ and _capabilities_.

=== 4. Artifact Resolution

<<artifact_resolution.adoc#artifact-resolution,Artifact resolution>> maps the selected dependency _variants_ to their corresponding files, determining which _artifacts_ Gradle will download and use during the build.
