...
[[old:what-are-dependency-configurations]]
...
For more examples on the usage of configurations to navigate, inspect and post-process metadata and artifacts of assigned dependencies, have a look at the <<dependency_resolution_basics.adoc#dependency-resolution-basics,resolution result APIs>>.
...
This makes the dependency declarations in your build script and the <<viewing_debugging_dependencies.adoc#sec:identifying-reason-dependency-selection,dependency insight report>> easier to interpret.
