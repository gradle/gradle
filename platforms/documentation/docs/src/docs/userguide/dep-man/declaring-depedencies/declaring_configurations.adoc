...
Now that we've created a _dependency scope_ configuration for declaring dependencies, and a _resolvable_ configuration for resolving those dependencies, we can use Gradle's <<dependency_resolution_basics.adoc#dependency_resolution_basics,dependency resolution APIs>> to access the results of resolution.
