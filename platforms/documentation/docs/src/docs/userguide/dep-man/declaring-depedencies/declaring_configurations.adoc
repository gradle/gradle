...
- **`canBeResolved`:** Indicates that this configuration is intended for resolving a set of dependencies into a dependency graph.
- **`canBeConsumed`:** Indicates that this configuration is intended for exposing artifacts outside this project.
- **`canBeDeclared`:** Indicates that this configuration is intended for declaring dependencies.
...
|===
|Configuration role|Can be resolved|Can be consumed|Can be declared|Explanation
|Dependency Scope|false|false|true|A declarable configuration should not be resolvable or consumable.
|Resolve for certain usage|true|false|false|A resolvable configuration should not be declarable or consumable.
|Exposed to consumers|false|true|false|A consumable configuration should not be declarable or resolvable.
...
These methods help users avoid manually setting various configuration flags, streamlining the process and ensuring consistency:
