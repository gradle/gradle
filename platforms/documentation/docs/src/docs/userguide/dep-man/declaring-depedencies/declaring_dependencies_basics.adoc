...
2. *File Dependencies*: Refer to local files or directories, such as `.jar` or `.aar` files.
3. *Project Dependencies*: Refer to other projects in the same multi-project build.
...
When using file dependencies, it’s important to clearly express both intent and version.
Unlike module dependencies, file dependencies are not subject to Gradle’s <<dependency_resolution.adoc#sec:how-gradle-downloads-deps,version conflict resolution>>, so assigning a version number in the file name is critical for tracking changes.

For example, naming a file `commons-beanutils-1.3.jar` helps maintain a clear record of library versions and their associated release notes.
...
Since Gradle 7, you can use an experimental type-safe API for project dependencies, which provides IDE completion, making it easier to declare dependencies:
...
With this API, incorrect dependencies in Kotlin DSL scripts trigger compilation errors, helping you avoid missed updates.
...
When declaring a dependency or a <<dependency_constraints.adoc#dependency-constraints,dependency constraint>>, you can provide a custom reason to clarify why the dependency is included.
