...
Gradle offers tools to navigate dependency management.
You can render a full dependency graph, identify the origin of each dependency, and see why specific versions were selected.
...
A project may request two different versions of the same dependency either directly or transitively that may result in a <<dependency_resolution.adoc#sec:version-conflict,version conflict>>.
