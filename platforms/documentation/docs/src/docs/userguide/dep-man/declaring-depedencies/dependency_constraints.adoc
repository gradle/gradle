...
In general, a dependency is considered either a **direct** dependency, or a **transitive** dependency:

- *Direct dependencies* are _directly required by a component_.
- *Transitive dependencies* are dependencies that a component needs, but only because another direct or transitive dependency needs them.

== Declaring constraints alongside direct dependencies

**Dependency constraints** allow you to define the version or version range for declared dependencies.
...
**Dependency constraints** allow you to define the version or version range for transitive dependencies.
