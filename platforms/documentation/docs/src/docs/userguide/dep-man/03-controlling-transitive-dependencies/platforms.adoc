...
NOTE: Each catalog will generate an extension applied to all projects for accessing its content.
...
WARNING: Only a single file will be accepted when using the link:{javadocPath}/org/gradle/api/initialization/dsl/VersionCatalogBuilder.html#from-java.lang.Object-[VersionCatalogBuilder.from(Object dependencyNotation)] method.
...
WARNING: Using `enforcedPlatform` needs to be considered with care if your software component can be consumed by others.
