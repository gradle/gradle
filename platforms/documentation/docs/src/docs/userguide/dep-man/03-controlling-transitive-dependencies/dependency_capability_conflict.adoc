...
[[old:handling-mutually-exclusive-deps]]
...
[[old:selecting-between-candidates]]
...
<<component_capabilities.adoc#sub:capabilities,Capabilities>> help _realizing_ that you have a conflict, but Gradle also provides tools to express how to solve the conflicts.

[[old:selecting-preferred-capability-provider]]
