...
** Only the parts are compared, not the separators, so `1.a.1`, `1-a+1`, `1.a-1`, and `1a1` are equivalent. (Note: There are <<dependency_resolution_adv#sec:base-version-comparison,exceptions>> during conflict resolution).
...
For more details, refer to the section on <<dependency_versions.adoc#sec:enforcing_dependency_version,overriding dependency versions>>.
