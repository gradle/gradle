...
<<dependency_resolution_model.adoc#sec:base-version-comparison,"base versions">>).
...
** Only the parts are compared, not the separators, so `1.a.1`, `1-a+1`, `1.a-1`, and `1a1` are equivalent. (Note: There are <<dependency_resolution_model.adoc#sec:base-version-comparison,exceptions>> during conflict resolution).
...
File dependencies are not considered by Gradle's <<dependency_resolution_model.adoc#sec:how-gradle-downloads-deps,version conflict resolution>>.
