...
Although constraints are not <<rich_versions.adoc#sec:strict-version,strict versions>> by default, you can specify a strict version constraint if needed.
...
When Gradle resolves a module version, it considers all relevant factors, including <<rich_versions.adoc#rich-version-constraints,rich versions>>, transitive dependencies, and dependency constraints for that module.
