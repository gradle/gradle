...
The configuration can be declared in `build.gradle` as follows:
...
Now that we've created a _dependency scope_ configuration for declaring dependencies, and a _resolvable_ configuration for resolving those dependencies, we can use Gradle's <<dependency_resolution_basics.adoc#dependency-resolution-basics,dependency resolution APIs>> to access the results of resolution.
...
In most cases, you can resolve unsafe accesses by creating a <<variant_model.adoc#sec:variant-aware-matching,proper cross-project dependency>>.
