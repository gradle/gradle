...
By default, the task shows dependencies for all <<dependency_configurations.adoc#sec:what-are-dependency-configurations,configurations>> within a <<command_line_interface#executing_tasks_in_multi_project_builds,single project>>.
...
A project may request two different versions of the same dependency either directly or transitively that may result in a <<dependency_resolution_adv.adoc#sec:version-conflict,version conflict>>.
...
| The dependency appears with a <<dependency_versions#sub:declaring_dependency_with_dynamic_version, dynamic version>> which did not include the listed versions.
