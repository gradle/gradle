...
A project may request two different versions of the same dependency either directly or transitively that may result in a <<variant_model.adoc#sec:version-conflict,version conflict>>.
...
This resulted in <<variant_model.adoc#sec:version-conflict, conflict resolution>> to select the most appropriate version.
