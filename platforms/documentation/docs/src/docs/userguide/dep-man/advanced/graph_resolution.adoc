...
- The `runtimeElements` *variant* of `com.fasterxml.jackson.core:jackson-databind:2.17.2` depends on `com.fasterxml.jackson.core`.
- The `runtimeElements` *variant* of `com.fasterxml.jackson.core:jackson-databind:2.17.2` is provided as a jar file called `jackson-databind-2.17.2.jar`.

Each *variant* is described by *attributes*.

== Attributes

To describe and differentiate between *variants*, Gradle uses *attributes*.
...
*Attributes* are defined as `key:value` pairs, where the key represents the *attribute*.
...
NOTE: Each resolvable configuration calculates a separate dependency graph.
...
When the process starts, a root node added to the queue.
Otherwise, the queue is empty.

Gradle starts the loop by pulling the root node from the queue.
Gradle examines the *dependencies* of the root node, resolves their conflicts, and downloads their *metadata*.
Based on this metadata, Gradle selects *variants* of those dependencies and adds them to the queue.

At this point, the queue contains all selected *variants* for the root node's *dependencies*, which are then processed one by one.

Note that the root *variant*'s dependencies correspond to the declared *dependencies* of the resolvable configuration.
Essentially, the resolvable configuration becomes the root node of the graph.
...
The process alternates between parallel downloads and single-threaded logic, repeating this pattern for the entire graph.
...
Variant selection is fully described in the <<variant_aware_resolution.adoc#sec:understanding-variant-selection,next section>>.
...
[.text-right]
**Next Step:** <<variant_aware_resolution.adoc#sec:understanding-variant-selection,Learn about Variant Selection>> >>
