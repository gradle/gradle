...
Graph resolution operates on a node-by-node (variant-by-variant) basis.

Each iteration processes one node at a time, starting by de-queuing a node.
Its dependencies are examined, their target versions determined, and metadata for all dependencies is downloaded in parallel.
Version conflict resolution is performed, variants are selected, and those variants are added to the queue for further processing.

The process is single-threaded, with parallel downloads occurring between sequential resolution steps, repeated iteratively for the entire graph.


The graph resolution process works on a node by node (variant by variant) basis.

Each iteration of the loop starts by pulling a node off the queue.
We process the graph one node (variant) at a time.

For each node we look at all its dependencies, determine what versions they target, and download the metadata for all dependencies (for that node) at once.
We look at the node's dependencies, and for each of their dependencies perform version conflict resolution.
Then we, download the metadata for all its dependencies in parallel, then we select variants for each of those dependencies, then those variants get added to the top-level queue.

Graph resolution is single threaded.
it’s a bunch of parallel downloads separated by single threaded logic, over and over again — for a single graph.
