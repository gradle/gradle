...
* Must be explicitly published (normally they are not published and not declared in metadata).
* Are typically used to expose *precomputed transforms* of the main artifact like `.class` files extracted from a JAR, or the `resources` directory.
* May be selected via an `ArtifactView` or `ArtifactResolutionQuery`, if the requested attributes match.
