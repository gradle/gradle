...
*** If the *attribute* value matches the consumers *requested attribute* value exactly,
*** Or if the *attribute* value passes the compatibility test, the candidate is compatible.
*** Else, the candidate is eliminated.

Once the loops completes:

* If only one candidate is compatible, that candidate wins.
* If several candidates are compatible, the candidate with the _most matching_ *attributes* win.
* If several candidates are compatible, and are compatible with an _equal number_ of *attributes*, Gradle needs to _disambiguate_ the candidates.

The *requested attributes* are now prioritized based on _disambiguation precedence_.
If no precedence exists, the *requested attribute* is excluded from the next loop:

* For each candidate,
** For each *requested attribute* (in _disambiguation preference_ order),
*** If a matching candidate *attribute* exists,
**** If the *attribute* has a value matching the _disambiguation rule_, the candidate is compatible.
***** _The loop is broken as soon as a match is found._
** If a candidate has _no matching_ *attributes*, the candidate is eliminated.

Once the loops completes:

* If only one candidate remains, that candidate wins.
* If several candidates still remain, Gradle repeats the process with *extra attributes*.

*Extra attributes* are *attributes* that were not requested by the consumer but are present on at least one candidate.
These extra *attributes* are considered in precedence order:

* For each candidate,
** For each *extra attribute* (in _preference order_),
*** If the *attribute* has a known precedence, the candidate is compatible.
**** _The loop is broken as soon as a match is found._
** After all extra *attributes* with _precedence_ are considered, the remaining candidates can be chosen if they are compatible with all the _non-ordered disambiguation rules_.
