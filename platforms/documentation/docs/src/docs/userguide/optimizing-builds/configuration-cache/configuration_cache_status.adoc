...
The Configuration Cache currently has no API to mark a task field or property as "do not serialize."
Sensitive values stored in a task's state, such as credentials, tokens, and API keys, are captured in the cache entry alongside the rest of the build configuration.
...
TIP: The current workaround is to store secrets in `<<directory_layout.adoc#dir:gradle_user_home,GRADLE_USER_HOME>>/gradle.properties`, whose contents are not included in cache entries; only the file's fingerprint is recorded.
