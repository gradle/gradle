...
Because the configuration phase is skipped on a hit, any logic registered through configuration-phase callbacks, such as `BuildListener`, `Gradle.beforeProject`, `Gradle.afterProject`, or `Gradle.taskGraph.whenReady`, will not execute.
...
- The *bean codec* is the fallback for arbitrary user-defined classes; it walks reflective fields and recursively serializes each one.
