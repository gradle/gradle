...
- The *configuration codec* handles `Configuration` instances by resolving them at store time and serializing the result as a file collection.
...
Task authors should still <<configuration_cache_requirements.adoc#config_cache:requirements:disallowed_types, prefer `FileCollection`>> over `Configuration` in their task inputs.
