...
NOTE: Do not add `-Dorg.gradle.configuration-cache.problems=warn` here.
Warning mode is not recommended as an adoption mechanism; see <<configuration_cache_adoption.adoc#config_cache:adoption_guide, Adopting the Configuration Cache>> for the supported migration path and <<configuration_cache_debugging.adoc#sec:enable_warning_mode, Enable Warning Mode>> for the full caveat.
