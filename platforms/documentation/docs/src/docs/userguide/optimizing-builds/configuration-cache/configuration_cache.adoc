...
If a candidate entry exists, Gradle compares the build's current <<config_cache:intro:how_does_it_work, configuration inputs>> against the entry's recorded fingerprint.
...
On a cache miss (such as the first build with the Configuration Cache enabled, or any build whose configuration inputs have changed), Gradle runs the configuration phase as it would without the cache, then **serializes the resulting task graph and writes it to the cache** before execution begins.
...
On a cache hit, Gradle skips the configuration phase entirely.
It does *not*:
...
On subsequent executions of the same tasks (e.g., `./gradlew check` again), Gradle skips the configuration phase entirely and loads the task graph from the Configuration Cache.
The cache is reused only when none of the build configuration inputs have changed; if any input has changed, Gradle reruns configuration and stores a new entry.
...
The serialized task graph is stored alongside a separate **fingerprint**—the recorded list of configuration inputs the build observed (see <<config_cache:intro:how_does_it_work, the build configuration inputs>>).
...
Built-in codecs also exist for collections, files, file collections, dependency results, build services, lambdas, and many Gradle-specific types. When none of these match, the bean codec is the fallback. Types that can't be safely serialized—`ClassLoader`, `Thread`, `Configuration`, and others—are rejected at store time rather than producing a failure on later load.
...
To share state across tasks under the Configuration Cache, use a <<build_services#build_services, Build Service>>—each task is otherwise its own isolated realm.

==== Detecting configuration inputs

While the task graph is being serialized, Gradle is also recording the build's <<config_cache:intro:how_does_it_work, configuration inputs>>—every environment variable, system property, file read, and file-system check the build performed during configuration.
