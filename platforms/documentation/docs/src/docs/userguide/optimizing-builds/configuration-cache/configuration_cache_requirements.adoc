...
Some (such as reading an environment variable or system property through a code path that bypasses Gradle's bytecode instrumentation), are not reported, but will produce incorrect results on a cache hit because the value is not tracked as a configuration input.
...
See also link:{gradle-issues}22618[gradle/gradle#22618].
