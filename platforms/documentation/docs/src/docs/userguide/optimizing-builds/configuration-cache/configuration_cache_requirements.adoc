...
If you need the value at configuration time, you can wrap the computation in a `ValueSource` and use an opaque provider to preserve laziness:
...
val valueSource = providers.of(MyValueSource::class) {
    parameters {
        // This does NOT work:
        // value = serviceProvider.map { it.getValueFromService() }

        // Workaround — preserves laziness:
        value = provider { serviceProvider.get().getValueFromService() }
    }
}
