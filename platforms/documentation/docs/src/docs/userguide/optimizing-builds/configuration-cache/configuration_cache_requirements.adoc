...
Gradle tracks this read via bytecode instrumentation, so the file's contents do become a build configuration input — changing `config/title.txt` will invalidate the CC entry and re-run the configuration phase. The problem is that the read happens at configuration time when it does not need to, so a value change forces a full CC entry rebuild instead of just re-running the affected task.
...
tasks.register<PublishArtifactTask>("publishArtifact") {
    apiToken = providers.environmentVariable("REGISTRY_TOKEN") // <1> <2>
}
----
=====
[.multi-language-sample]
