...
== Third-party Java Agents Require Gradle's Instrumentation Agent

When the Configuration Cache is enabled and a third-party `-javaagent:` is attached to the build JVM (for example a coverage agent such as Jacoco, IntelliJ's `idea_rt.jar`, or a custom user agent), Gradle requires its own instrumentation agent to be active.
Gradle composes its buildscript-classpath instrumentation with the third-party agent's class-file transformer at class load; that composition is only safe when Gradle's agent is active.

Gradle's instrumentation agent is enabled by default for:

* Regular daemon builds.
* <<test_kit#test_kit, TestKit>>'s default (daemon) mode.
* Tooling API non-embedded clients.

Gradle's instrumentation agent is NOT active when:

* The build runs in embedded mode: TestKit with `withDebug(true)`, or an embedded Tooling API client.
* Gradle's instrumentation agent has been explicitly disabled in the build environment.

In those configurations, attaching a third-party Java agent is unsupported with the Configuration Cache. Either:

* Switch from embedded mode to daemon mode (drop `withDebug(true)` for TestKit; use a non-embedded Tooling API connection). Gradle's instrumentation agent is loaded automatically in daemon mode.
* If Gradle's instrumentation agent has been explicitly disabled in the build environment, restore its default.
