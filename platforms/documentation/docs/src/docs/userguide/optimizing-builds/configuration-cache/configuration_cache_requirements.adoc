...
Parameter types must be Configuration Cache-compatible: standard managed property types (`Property<T>`, `RegularFileProperty`, `ListProperty<T>`, and so on) are safe; raw `Configuration` references, live JVM state, and `Project` references are not.

*The service instance is recreated on every build.*
Only the parameters are cached.
On a cache hit, Gradle reads the recorded parameters from the entry, instantiates the build service afresh, and injects them.
Any work done in the service's constructor or first method invocation runs again on every build, regardless of whether the configuration phase ran.
...
The supported alternatives are `@ServiceReference` for direct injection and storing the build service reference in an `@Internal`-annotated property on a task.
...
A build service registered via link:{javadocPath}/org/gradle/build/event/BuildEventsListenerRegistry.html#onTaskCompletion(org.gradle.api.provider.Provider)[`BuildEventsListenerRegistry.onTaskCompletion`] is the supported way to survive the configuration-to-execution boundary correctly under the Configuration Cache.
Other ways of holding a build service reference may not survive a cache hit cleanly.
