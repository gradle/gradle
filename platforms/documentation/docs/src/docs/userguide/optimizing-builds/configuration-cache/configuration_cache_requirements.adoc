...
| `project.buildDir`
| `project.buildDir` has been deprecated. You should use `project.layout.buildDirectory`. A task input or output property or a script variable to capture the result of using `project.layout.buildDirectory` to calculate the actual parameter.
