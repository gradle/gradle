...
This restriction only applies to build logic (including plugins and dependencies of plugins). Code built with Gradle isn't affected.
...
In environments where the agent is available, Gradle automatically opens any JDK packages that need deep reflective access during serialization, so user code does not need to add `--add-opens` for them.
