...
The Configuration Cache stores the captured build script object, but drops its connection to the build model, so an action that uses only Project-independent
script state keeps working.

Accessing the functions and variables defined in the script, as well as script-provided utilities such as `file(...)`, is allowed.
Note that the script instance is isolated per task, so mutable variables defined in the script will not be shared across tasks; instead, each
task will work with a copy of that state.

Accessing the build model, such as the `Project`, through the script object at execution time fails and reports a problem.
