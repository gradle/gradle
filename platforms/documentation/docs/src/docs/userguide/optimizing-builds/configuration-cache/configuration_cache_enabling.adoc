...
[[config_cache:adoption:corrupted_entries]]
==== Recovering from corrupted entries

Starting with Gradle 9.8.0, when a Configuration Cache entry cannot be loaded — for example because it is corrupted — Gradle discards the broken entry so the requested work is recomputed as a cache miss.
...
To restore the previous behavior and surface corruption as a build failure instead, set the Gradle property:
