...
2. Starting with Gradle 9.7.0, standard output and error listeners registered on a task's logging manager during configuration (for example via
...
+
Prior to Gradle 9.7.0, these listeners were silently discarded and never received output during cache-hit builds.
+
To temporarily revert to the previous behavior, set the Gradle property:
+
[source,properties]
