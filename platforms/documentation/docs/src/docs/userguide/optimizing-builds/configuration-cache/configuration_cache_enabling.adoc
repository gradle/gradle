...
In some builds, such as continuous integration builds in ephemeral environments, it may be desirable that Gradle only runs
in Configuration Cache mode in case of a configuration cache hit. In case of a miss, there would be no benefit in
caching the configuration, because the state in disk is not going to be preserved across builds.
...
Note that enabling it persistently would require disabling it on the command-line in order for configuration to ever be cached.
