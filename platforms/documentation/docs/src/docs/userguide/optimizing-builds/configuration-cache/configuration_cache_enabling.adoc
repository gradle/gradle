...
- Fix issues with link:https://gradle.com/develocity/product/build-scan/[Build Scans].
...
With the evolution of the Configuration Cache, Gradle may also impose additional restrictions on the build logic. In order to make adoption smoother, it is possible to
_temporarily_ opt out of these restriction in the following cases:
...
+
Prior to that, these unsupported providers were silently discarded and never received events in a cache hit build.
+
To temporarily revert to the previous behavior, set the Gradle property `org.gradle.configuration-cache.unsafe.ignore.unsupported-build-events-listeners` to `true`.
