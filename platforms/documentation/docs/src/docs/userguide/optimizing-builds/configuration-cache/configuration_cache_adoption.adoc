...
* **Excessive build inputs that survive but invalidate the cache too often** — reading the environment (`System.getenv` / `System.getProperty` → `providers.environmentVariable` / `providers.systemProperty`) or reading files at configuration time (`File.readText` / `file(...).exists()` → `providers.fileContents`). These do not fail the build; Gradle tracks them as inputs, but the tracking is coarser than the equivalent provider API, so a value change will invalidate more of the cache than necessary. Fix the hard failures first, then clean these up as a follow-up.
...
**Excessive build inputs — these survive but track more coarsely than the equivalent provider API, so a value change will invalidate more of the cache than necessary. Clean these up after the hard failures are gone:**
...
Leave CI in the default **strict** problem mode (`org.gradle.configuration-cache.problems=fail`), not warning mode.
...
CC enables **parallel task execution** regardless of `--parallel`, `--no-parallel`, or `org.gradle.parallel`.
Every task has its own isolated deserialized state and no project-level lock contention, so tasks run in parallel across the entire build — intra- and cross-project — automatically.

`--parallel` is still meaningful under CC, but its effect is narrower than the flag name suggests: it enables **parallel CC store**, so writing the CC entry itself proceeds in parallel where possible.
It does *not* control task execution parallelism — that is already parallel with or without the flag.

`--no-parallel` is effectively ignored for task execution under CC. It still disables the parallel CC store, but task execution stays parallel.
