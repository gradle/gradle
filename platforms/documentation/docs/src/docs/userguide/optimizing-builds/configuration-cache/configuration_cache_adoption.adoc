...
* **Test framework variants** — if your test framework is Groovy-major-locked, a Gradle major bump can break it. For example, Gradle 9.0.0 ships Groovy 4; the Groovy-3-variant Spock release will refuse to load and the test compilation will fail with a clear `IncompatibleGroovyVersionException`.
