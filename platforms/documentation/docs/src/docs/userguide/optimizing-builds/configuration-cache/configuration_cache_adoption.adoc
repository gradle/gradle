...
| 2
| Suppressible inputs: `org.gradle.configuration-cache.inputs.unsafe.ignore.file-system-checks` deprecated-label:[] and `org.gradle.configuration-cache.inputs.unsafe.ignore.in-serialization` deprecated-label:[]. Use sparingly to mitigate noisy invalidation while the underlying plugin is being fixed. Both properties are scheduled for removal in Gradle 11. See <<configuration_cache_enabling.adoc#config_cache:usage:input_opt_outs:input_detection, Input-detection opt-outs>>.
