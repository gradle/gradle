...
| `third-party Java agents with inactive Gradle's instrumentation agent are not supported`
| TestKit debug mode plus a second Java agent (a coverage agent, or one the IDE attaches) on the test JVM
| <<test_kit.adoc#sub:test-kit-debug-config-cache,Debugging with the Configuration Cache or Isolated Projects>>
...
Debug mode has one further constraint under the Configuration Cache when another Java agent is attached to the test JVM.
See <<test_kit.adoc#sub:test-kit-debug-config-cache,Debugging with the Configuration Cache or Isolated Projects>>.
====
