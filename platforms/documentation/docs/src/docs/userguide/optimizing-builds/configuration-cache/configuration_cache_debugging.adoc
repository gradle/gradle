...
| A non-serializable runtime object (HttpClient, Connection, Thread) is stored in a task field
...
| Kotlin DSL `KotlinClosure1` capturing build-script state
| Restructure the closure so it only captures values, not the enclosing script object.

| `Could not find build 'X'`
| Composite-build CC reuse edge case
| link:{gradle-issues}20945[gradle/gradle#20945].

| `Unable to make field ... accessible: module java.base does not "opens" ...`
| JPMS reflective-access blocked during CC serialization (typically lambdas, `SerializedLambda`).
The CC report may show `0 problems` even though the build fails.
| For the Gradle daemon, add `--add-opens` directives via `org.gradle.jvmargs` in `gradle.properties`.
For TestKit, set them as `jvmArgs` on the test task — see <<config_cache:testing,Test your Build Logic>>.

| `Cannot reference a Gradle script object from a Groovy closure`
| Groovy closure captured a `Project`, `Gradle`, or script reference
...
See <<test_kit.adoc#sub:test-kit-configuration-cache, Testing with the Configuration Cache>> for the full list of outcomes exposed by link:{javadocPath}/org/gradle/testkit/runner/BuildResult.html#getConfigurationCacheOutcome--[`BuildResult.getConfigurationCacheOutcome()`].
...
[NOTE]
====
**JPMS and `--add-opens` with TestKit.**
...
The Gradle daemon supplies the required `--add-opens` directives by default.
TestKit's `GradleRunner` does not, so they have to be set explicitly on the test task that runs the functional tests:
...
Without these, the CC report may show `0 problems` even though the build fails on a reflective-access error.
====
