...
In order to improve performance and reduce entry size, the configuration cache performs a minimal amount of integrity checks as it writes or reads the data.
However, it makes it harder to troubleshoot issues caused by concurrency problems or misbehaving serialization logic.
The incorrectly stored object may not be discovered immediately, but it can affect reading data of some other, unrelated task.
For example, reading a value of the property may end up with a type mismatch or break some other invariant, resulting in a potentially misleading or misattributed error message.

In order to make debugging easier, it is possible to enable stricter checks that may help to discover discrepancies earlier,
at a cost of slowing down cache operations and significantly increasing the size of the entry.
To do that, add the following line to your `gradle.properties`:
[source,properties]
.gradle.properties
----
...
When running without the integrity check, you may get a cryptic failure message, perhaps accompanied by some induced configuration cache problems:
----
...
It might be really hard to connect the invalid index to a serialization issue. Rerunning the build with the integrity check enabled gives a much more precise diagnostic:
----
...
----
You can immediately see the name of the offending task and even the field that contains the broken data.
Keep in mind that this attribution is best-effort: it should be accurate in most cases, but rarely can be confused by some byte patterns.

[WARNING]
====
The integrity check relies on extra data stored in the cache, and thus cannot be used to debug an existing corrupted entry written without the check enabled.
====

Note, that the current set of integrity checks primarily aims at detecting issues with serialization protocols more than data corruption.
It may not be as effective against issues caused by hardware failures like bit rotting or bad sectors.
Because of that and of the extra costs the integrity check brings, we don't recommend to have it always enabled, but consider it a troubleshooting tool.
