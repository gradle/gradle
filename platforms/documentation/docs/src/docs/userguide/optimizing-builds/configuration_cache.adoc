...
By default, the configuration cache doesn't perform many integrity checks when reading the entry.
This improves performance and reduces entry size.
However, it makes it harder to pinpoint some store and load issues.
For example, the incorrectly stored object may be discovered later, when reading data of some unrelated task.
It may cause a type mismatch when restoring a value of the property or break some other invariant, resulting in a potentially misleading exception.
...
<2> `readObject` reads only the first field, leaving the lingering data.

Such a type will cause problems when used as part of a task state.
[source,java]
----
...
It might be really hard to connect the invalid index to a serialization issue. Enabling the integrity check gives a much more precise diagnostics:
----
...
Note, however, that the current set of integrity checks primarily aims at detecting issues with serialization protocols more than data corruption.
