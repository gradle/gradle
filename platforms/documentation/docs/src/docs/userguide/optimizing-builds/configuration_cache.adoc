...
The only exceptions are standard streams (`System.in`, `System.out`, `System.err`), which can be used as parameters for link:{groovyDslPath}/org.gradle.api.tasks.Exec.html[Exec] and link:{groovyDslPath}/org.gradle.api.tasks.JavaExec.html[JavaExec] tasks.
...
Reference equality is preserved in both cases:
...
Only user-defined object references remain identical.
