...
* C. Disable incremental compilation on stateless machines
...
Incremental compilation is only useful when Gradle has the previous compilation history.
If that history is not available, incremental compilation provides no benefit, and may add a significant overhead.
This is common on CI agents that start from a clean workspace and do not retain local build state.

Writing the incremental compilation state is I/O intensive.
On machines with slow disks or limited IOPS, it can be useful to disable incremental compilation to avoid the overhead.

You can disable incremental compilation for all `JavaCompile` tasks in the build with one of the following mechanisms:

* System property: `-Dorg.gradle.java.disable-incremental-compile=true`
* Gradle property: `org.gradle.java.disable-incremental-compile=true`
* Environment variable: `ORG_GRADLE_JAVA_DISABLE_INCREMENTAL_COMPILE=true`

This build-wide override takes precedence over any `options.incremental` setting in the build script.
...
To disable incremental Java compilation for all `JavaCompile` tasks in a build, set `org.gradle.java.disable-incremental-compile=true` as a system property, Gradle property, or environment variable (`ORG_GRADLE_JAVA_DISABLE_INCREMENTAL_COMPILE=true`).
This override takes precedence over `options.incremental`.
