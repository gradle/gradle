...
Further optimizations, such as finer-grained caching of configuration results, are possible but are not yet available.
...
Keep the ceiling in mind: Isolated Projects can only speed up the <<build_lifecycle.adoc#configuration,configuration phase>>, so its upper bound is the share of build time that phase represents.
...
Common questions about adopting Isolated Projects.
...
It is driven by the number of projects and the cost of configuring them.
