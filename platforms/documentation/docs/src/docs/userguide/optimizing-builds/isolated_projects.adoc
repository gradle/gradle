...
* Gradle 9.7.0
** The new <<sec:dangerously_ignore_problems,dangerously-ignore-problems>> mode lets you ignore Isolated Projects violations to evaluate parallel build and sync performance without fixing violations first.
** Isolated Projects violation severity is now independent of the Configuration Cache `--configuration-cache-problems=warn` flag.
...
The corresponding <<upgrading_version_9.adoc#deprecated_implicit_project_hierarchy_lookup,upgrade guide entry>> details the cases when it is applicable.
...
Enable it by setting `org.gradle.unsafe.isolated-projects.dangerously-ignore-problems=true` in `gradle.properties`, alongside enabling Isolated Projects:
...
$ ./gradlew help \
    -Dorg.gradle.unsafe.isolated-projects=true \
    -Dorg.gradle.unsafe.isolated-projects.dangerously-ignore-problems=true
----
...
[WARNING]
====
This mode is a migration and troubleshooting aid and not intended as a persistent way of ignoring incompatibilities.
