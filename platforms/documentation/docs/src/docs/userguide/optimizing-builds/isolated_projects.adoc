...
* The same plugin id *is not* usable directly from inside a `gradle.lifecycle.beforeProject` (or `afterProject`) callback registered in the settings script.
The callback fails with `Plugin with id '...' not found.`
