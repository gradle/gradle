...
`getRegistrations` now only permits the `findByName(String)` method. All other methods emit IP violations.
`registerIfAbsent` remains IP compatible.
...
Only `findByName(String)` is permitted on the returned set; all other methods emit IP violations.
