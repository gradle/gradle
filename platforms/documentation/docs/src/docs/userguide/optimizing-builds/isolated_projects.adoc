...
* `gradle.lifecycle.beforeProject` callbacks registered in `settings.gradle(.kts)` cannot directly apply convention plugins from a build registered via `pluginManagement.includeBuild(...)`, even though the same plugins are usable from a `plugins {}` block in the same settings script. See <<sec:lifecycle_callbacks_with_included_plugin_builds>> for working patterns.
...
There is a current asymmetry to be aware of when build logic is provided by a build registered via `pluginManagement.includeBuild(...)`:

* A convention plugin from such a build *is* usable from a `plugins {}` block in `settings.gradle(.kts)` or in any project script.
* The same plugin id *is not* usable directly from inside a `gradle.lifecycle.beforeProject` (or `afterProject`) callback registered in the settings script. The callback fails with `Plugin with id '...' not found.`

The reason is that plugins from `pluginManagement.includeBuild` are resolved lazily: only a `plugins {}` block triggers resolution and exports the resulting classpath into the surrounding script's classloader scope.
A `gradle.lifecycle.beforeProject { apply(plugin = "...") }` callback does not trigger resolution, so the plugin is invisible to it.
By contrast, `buildSrc/` is built eagerly and its classpath is automatically exported to all projects, so this limitation does not apply there.

There are two patterns that make this work today.

[[sec:lifecycle_via_settings_convention_plugin]]
====== Recommended: register `beforeProject` from a settings convention plugin

Move the `gradle.lifecycle.beforeProject` registration into a *settings* convention plugin published from your included build, then apply that settings plugin from `settings.gradle(.kts)`.
