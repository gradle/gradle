...
A child project does not implicitly resolve properties or methods from its parent projects.
This applies to all Gradle modes: the implicit lookup was deprecated in Gradle 9.6.0 and removed in Gradle 10, so the behavior is now the same with and without Isolated Projects.

An unresolved reference fails at the lookup site rather than resolving from an ancestor project.
Note that `Project.findProperty()` returns `null` for a property that is only defined in a parent project, rather than the parent's value.
