...
`-Dorg.gradle.console=(auto,plain,plain-with-color,rich,verbose)`::
...
`--console=(auto,plain,plain-with-color,rich,verbose)`::
...
+
Set to `plain-with-color` to generate colored output without rich status information such as progress bars.
+
