...
+
For Gradle versions after major version 9, the value can be specified with a major version or a minor version only.
In that case, the latest normal Gradle version with that major or minor version will be used.
For example, `9` and `9.1` are valid values.
+
...
* `link:https://services.gradle.org/versions/milestone[milestone]`
