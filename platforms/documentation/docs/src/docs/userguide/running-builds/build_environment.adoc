...
:keywords: properties, system, project, environment, variables, https.protocols, gradle.user.home, gradle.wrapperPassword, gradle.wrapperUser, org.gradle, gradle, java_home, gradle_opts, java_opts, gradle_libs_repo_override
...
When configured, Gradle will fork up to `org.gradle.workers.max` JVMs to execute <<performance.adoc#sec:parallel_execution,projects in parallel>>.
+
_Default is `false`._
