...
----
+
Cross-project configuration usually breaks `Project Isolation` and `Parallel Projects` so you should use convention plugins or the proper APIs whenever possible:
+
[source,groovy]
.settings.gradle
----
...
tasks.register("customTask") {
    // Avoid this! Directly referencing a task or property from another subproject
    dependsOn project(":subprojectB").tasks.named("someTask")
}
----
+
[source,groovy]
...
[[sub:gradle_build]]
Gradle Build::
A Gradle Build is one or more project(s) that use the Gradle Build Tool to compile, test, package, and deploy code. A Gradle Build should contain at least one Gradle build file and is executed using the `gradle` command, such as `gradle build`.
