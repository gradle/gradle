...
When the task runs, it creates or updates the criteria in the `gradle/daemon-jvm.properties` file.
...
This feature is used to locate a JVM installation that matches the criteria specified in the `gradle/daemon-jvm.properties` file.
