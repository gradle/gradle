...
You can also specify a different JVM installation for the build using the [`org.gradle.java.home`](build_environment.html#sec:gradle_configuration_properties) property or programmatically through the Tooling API.
...
This will generate a new `gradle-build.properties` file that captures the currently used JVM requirements.
