...
However, should the need arise, only configuration-cache-compatible APIs described below must be used instead of Java and Groovy standard APIs or
existing `ExecOperations`, `Project.exec`, `Project.javaexec`.
The flexibility of the latter doesn't allow Gradle to figure out how these calls affect the build configuration and check if the configuration cache entry can be safely reused.
