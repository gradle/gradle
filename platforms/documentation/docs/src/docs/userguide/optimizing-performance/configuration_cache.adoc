...
However, should the need arise, only configuration-cache-compatible APIs described below must be used.
Using Java and Groovy standard APIs or existing `ExecOperations`, `Project.exec`, `Project.javaexec`, and their likes in settings and init scripts is not allowed when the configuration cache is enabled.
The flexibility these APIs provide doesn't allow Gradle to figure out how these calls affect the build configuration and check if the configuration cache entry can be safely reused.
