...
<<structuring_software_products_details.adoc#sec:parallel_execution,parallel builds documentation>>.
...
* If multiple subprojects, but not all, use the script, you could refactor the script and
  all surrounding logic into a custom plugin located in <<multi_project_builds.adoc#sec:multiproject_buildsrc,`buildSrc`>>.
