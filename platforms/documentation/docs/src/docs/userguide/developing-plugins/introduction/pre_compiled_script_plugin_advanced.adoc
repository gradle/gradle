...
However, this is not typical or recommended and would require some work as it must first be turned into a binary plugin.
...
To publish them, you must first convert the precompiled script plugin into a binary plugin.
...
Inside `buildSrc/src/main/kotlin` or `buildSrc/src/main/groovy`, write a `.gradle(.kts)` file.
For example:
