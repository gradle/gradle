...
Now, let's define the class that holds the configurable properties for our plugin. We'll use Gradle's `Property<T>` type for these properties, as this makes them lazy and fully compatible with the Configuration Cache.
