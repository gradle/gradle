...
This tells Gradle to treat your `plugin` and `consumer` projects as part of a single, coordinated build.
...
include::sample[dir="snippets/plugins/plugin-tutorial/groovy",files="consumer/settings.gradle[]"]
====
...
That's it! You can now iterate on your plugin in the `plugin` directory and test it immediately in the `consumer` project without any publishing overhead.
