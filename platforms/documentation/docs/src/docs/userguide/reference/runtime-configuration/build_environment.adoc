...
`org.gradle.agent=(true,false)` incubating-label:[]::
When set to `true`, enables <<command_line_interface.adoc#sec:agent_mode,agent mode>>, which writes all build output to a file instead of the console.
+
_Default is `false`._
...
`ORG_GRADLE_AGENT` incubating-label:[]::
When set to `true`, enables <<command_line_interface.adoc#sec:agent_mode,agent mode>>; when set to `false`, disables it.
+
Takes precedence over the `org.gradle.agent` Gradle property, and is overridden by the `--agent` and `--no-agent` command-line options.
