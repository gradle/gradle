...
Its primary benefit is dramatically reducing build times by eliminating the significant overhead associated with JVM startup and initialization on every command execution.
...
| *Gradle Client JVM*
| Short-lived launcher that starts when you run `./gradlew`. It forwards the request to the Daemon and handles console I/O.
| Whatever JVM launches the script (e.g., `JAVA_HOME`, `java` on `PATH`, or settings in your IDE).

| *Gradle Daemon JVM*
| Long-lived process that executes the build and caches state between runs.
| By default, the *same JVM as the Gradle Client*.
Can be overridden by `--java-home` (CLI), `org.gradle.java.home` (properties), the <<tooling_api.adoc#tooling_api,Tooling API>>, or the <<#sec:configuring_daemon_jvm,Daemon JVM toolchains>>.
|===
...
== Configuring the Daemon JVM using Toolchains
