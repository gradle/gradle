...
By default, daemon logs older than 14 days are deleted when the Gradle Daemon shuts down.
This cleanup runs alongside other <<directory_layout.adoc#dir:gradle_user_home:cache_cleanup,cache cleanup operations>>.

The cleanup occurs:

- In the background when the daemon stops or is idle (default behavior)
