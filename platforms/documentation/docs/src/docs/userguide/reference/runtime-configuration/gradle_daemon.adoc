...
| *Gradle Client JVM*
| Process that starts when you run `./gradlew` and stays alive for the *entire build invocation*. +
...
| Uses the JVM set by (in order): +
1) `--java-home` (CLI) +
2) `org.gradle.java.home` (properties) +
3) <<tooling_api.adoc#tooling_api,Tooling API>> requests (e.g., IDE) +
4) <<#sec:configuring_daemon_jvm,Daemon JVM toolchains>> +
4) Falls back to the JVM that launched the Gradle Client or reuses an existing Daemon if conditions are right*.
|===
...
Changing JVM args (like increasing memory) will spawn a new Daemon.
