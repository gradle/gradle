...
Agent mode is intended for builds driven by an AI agent rather than a person.
Gradle renders all build output as with `--console=plain` and writes it to the file `agent/builds/<id>/build-output.log` in the project cache directory (`.gradle` in the root directory by default), where `<id>` is unique to each invocation.
Gradle <<directory_layout.adoc#dir:project_root:cache_cleanup,deletes these files>> once they are older than 7 days.
The only thing printed to standard output is the absolute path of that file.
The path is printed before the build starts, and Gradle only ever appends complete lines to the file, so an agent can follow it as it grows without seeing a partially written line.
While the build is running, Gradle writes a single newline character to standard error every 10 seconds, so that the process does not appear to be hung.
The exit code still reports whether the build succeeded (exit code 0) or failed (exit code 1).

You can also enable it by setting the `org.gradle.agent` <<build_environment.adoc#sec:gradle_configuration_properties,Gradle property>> to `true` (for example in `gradle.properties`, `GRADLE_OPTS`, or via `-D`), or with the `ORG_GRADLE_AGENT` <<build_environment.adoc#environment_variables_reference,environment variable>>.

Agent mode also implies `--non-interactive`, `--warning-mode=none` and `--stacktrace`.
Each of these can still be configured explicitly, for example `--agent --warning-mode=all` writes deprecation warnings to the output file.
The <<#sec:command_line_logging,log level options>> apply as usual and control what is written to the output file.

Agent mode controls the console output, so the `--console` command-line option has no effect while it is enabled.
Gradle prints a warning to the build output file when it ignores an explicit `--console` option.

Agent mode only applies to builds run from the command line.
A build run through the <<tooling_api.adoc#tooling_api,Tooling API>> ignores it, however it was requested, and reports a problem instead.

`--agent`, `--no-agent` incubating-label:[]::
Toggles agent mode. Default is off.
