...
You can disable all interactive prompting by using the `--non-interactive` command-line option or by setting the `NONINTERACTIVE` environment variable to a non-empty value.
...
This can also be enabled by setting the `NONINTERACTIVE` environment variable to a non-empty value.
