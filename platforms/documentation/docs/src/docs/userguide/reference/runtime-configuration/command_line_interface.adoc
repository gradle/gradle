...
`--task-graph-limit=<n>` since-gradle-label:9.7.0[]::
Limit the depth of the task graph printed by `--task-graph`.
The requested task itself is not counted against the limit, so `--task-graph-limit=1` shows the requested task plus its direct dependencies.
Truncated nodes are marked with `(+)`.
Has no effect unless `--task-graph` is also specified.
