...
Gradle renders all build output as with `--console=plain` and writes it to the file `agent/builds/latest/build-output.log` in the project cache directory (`.gradle` in the root directory by default), replacing any previous content.
