...
`--retries`::
The number of retries to attempt when downloading the Gradle distribution. The default value is `0`, meaning retrying is disabled, downloading will be attempted a single time.

`--retry-timeout-ms`::
The timeout in milliseconds to wait between download retries (if enabled, see `--retries`). The default value is `0`, meaning that there won't be any delay between two successive download attempts.
...
You can configure the number of retries and the timeout between attempts in the `gradle-wrapper.properties` file:
