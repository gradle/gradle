...
When the `wrapper` task regenerates an existing `gradle-wrapper.properties` file, values already present in that file are preserved unless the corresponding task properties are explicitly configured.
See <<#sec:preserving_wrapper_properties,Preserving Existing Wrapper Properties>> for details.
...
The resulting distribution URL is validated before it is written to the properties file, unless validation is disabled (see `--no-validate-url`).
+
...
The default value is `bin`, or the type of the `distributionUrl` already present in `gradle-wrapper.properties`.
...
The URL is validated before it is written to the properties file, unless validation is disabled (see `--no-validate-url`).
...
The resulting `validateDistributionUrl=false` is preserved in `gradle-wrapper.properties`, so later `wrapper` runs skip validation as well until `--validate-url` is passed.

`--validate-url`::
Enables the validation of the configured distribution URL. Enabled by default, unless `gradle-wrapper.properties` already contains `validateDistributionUrl=false`.
...
If the distribution URL is configured with `--gradle-version` or `--gradle-distribution-url` and validation is enabled, the URL is validated by sending a HEAD request in the case of the `https` scheme or by checking the existence of the file in the case of the `file` scheme.
The HEAD request is skipped when the build runs with `--offline`.
...
[[sec:preserving_wrapper_properties]]
=== Preserving Existing Wrapper Properties

When the `wrapper` task regenerates an existing `gradle-wrapper.properties` file, values already present in that file are preserved unless the corresponding task properties are explicitly configured.
Preserved properties are `distributionBase`, `distributionPath`, `zipStoreBase`, `zipStorePath`, `networkTimeout`, `validateDistributionUrl`, `retries`, and `retryBackOffMs`.

Explicit task configuration takes precedence over existing values in the file.

The `distributionUrl` is regenerated for the configured Gradle version.
Its distribution type is inferred from the filename of an existing `-bin.zip` or `-all.zip` URL when the task does not configure one.
The host and any other URL components are not preserved.
