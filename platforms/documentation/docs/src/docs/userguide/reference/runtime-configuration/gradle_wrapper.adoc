...
[[gradle_wrapper]]
= Gradle Wrapper
:keywords: wrapper, distributionUrl, wrapperJarUrl, gradle-version, systemProp.gradle.wrapperUser, systemProp.gradle.wrapperPassword, gradle-wrapper.jar, gradle-wrapper.properties, gradlew, gradlew.bat, gradle-version, distribution-type, gradle-distribution-url, gradle-wrapper-jar-url, gradle-distribution-sha256-sum, network-timeout, no-validate-url, validate-url, retries, retry-back-off-ms
...
If your organization does not allow binary files in version control, you can omit `gradle/wrapper/gradle-wrapper.jar`: the `gradlew` and `gradlew.bat` scripts download it on demand using `wrapperJarUrl` from `gradle/wrapper/gradle-wrapper.properties`, or a URL derived from `distributionUrl` if `wrapperJarUrl` is not set — like the Maven Wrapper.
====
...
`--gradle-wrapper-jar-url`::
The full URL pointing to the `gradle-wrapper.jar` file.
If not set, Gradle derives the URL from the distribution URL (preserving a custom host when it uses the standard `-bin.zip`/`-all.zip` layout) or from the official Gradle distribution location for the given Gradle version.
The `gradlew` and `gradlew.bat` scripts use it to download a missing wrapper JAR, so the JAR does not have to be checked into version control.
The URL is validated before it is written to the properties file.
...
It can be omitted from version control: in that case `gradlew`/`gradlew.bat` download it on demand from `wrapperJarUrl` (see below).
...
It also contains `wrapperJarUrl`, used by the scripts to download a missing `gradle-wrapper.jar`.

`gradlew`, `gradlew.bat`::
A shell script and a Windows batch script for executing the build with the Wrapper.
If `gradle-wrapper.jar` is missing, the scripts download it first (using `curl`/`wget` on Unix, `curl`/PowerShell on Windows), so the JAR does not have to be checked into version control.
...
[[sec:omitting_wrapper_jar]]
=== Omitting the Wrapper JAR from version control

If your organization does not allow binary files in version control, you can omit `gradle/wrapper/gradle-wrapper.jar` (for example via `.gitignore`):
...
The generated `gradlew`/`gradlew.bat` scripts detect a missing JAR and download it on demand from `wrapperJarUrl` in `gradle/wrapper/gradle-wrapper.properties`.
When `wrapperJarUrl` is not set explicitly (via `--gradle-wrapper-jar-url`), it is derived from `distributionUrl` for standard `-bin.zip`/`-all.zip` distributions, preserving a custom distribution host.
This works similarly to the Maven Wrapper: only the scripts and the properties file need to be checked in.

TIP: After upgrading to a Gradle version that generates the download logic, run the `wrapper` task once so the scripts and `wrapperJarUrl` are up to date, then remove the JAR from version control.
...
====
include::sample[dir="snippets/reference/runtime-configuration/customized-task/kotlin",files="build.gradle.kts[tags=customized-wrapper-task]"]
...
[source,properties]
----
distributionUrl=https://username:password@your.secure.repo/path/to/gradle-distribution.zip
----
...
NOTE: If a `wrapperToken` is specified, it will take precedence over `wrapperUser` and `wrapperPassword` system properties (and also over user & password values embedded in the URL).
...
NOTE: When `gradle-wrapper.jar` is omitted from version control (see <<#sec:omitting_wrapper_jar>>), the Wrapper scripts download it on demand from `wrapperJarUrl` over HTTPS.
Make sure `wrapperJarUrl` points to a trusted location (by default `https://services.gradle.org`) and review changes to it like any other security-sensitive change.
After the download, you can verify the JAR with the same checksum steps below.
Automatic validation by the Setup Gradle GitHub Action only applies to JARs checked into the repository.
...
[source,bash]
----
$ curl --location https://keys.openpgp.org/vks/v1/by-fingerprint/<FINGERPRINT> | gpg --import
----
