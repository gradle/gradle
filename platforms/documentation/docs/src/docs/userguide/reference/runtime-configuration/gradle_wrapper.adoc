...
`--retries`::
The number of retries to attempt when downloading the Gradle distribution. The default value is `0`.

`--retry-timeout-ms`::
The timeout in milliseconds to wait between download retries. The default value is `2000`.
