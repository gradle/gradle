...
By default it is derived from the distribution URL (preserving a custom host when it uses the standard `-bin.zip`/`-all.zip` layout) or from the official Gradle distribution location for the given Gradle version.
...
This works similar to the Maven Wrapper: only the scripts and the properties file have to be checked in.
