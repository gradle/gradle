...
`--retry-back-off-ms`::
The initial back off in milliseconds to wait between download retries (if enabled, see `--retries`). The delay doubles after each consecutive failure. The default value is `0`, meaning that there won't be any delay between two successive download attempts. Negative values will be ignored and the default will be used.
...
----
retries=3
retryBackOffMs=5000 # initial delay; doubles on each subsequent failure
----
