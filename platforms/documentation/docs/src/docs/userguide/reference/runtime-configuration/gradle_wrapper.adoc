...
`--retry-timeout-ms`::
The timeout in milliseconds to wait between download retries (if enabled, see `--retries`). The default value is `0`, meaning that there won't be any delay between two successive download attempts. Negative values will be ignored and the default will be used.
