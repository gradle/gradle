...
With Bearer Token, the Token itself is send in clear text.
====
...
WARNING: If you use `systemProp.gradle.wrapperUser` and `systemProp.gradle.wrapperPassword` to download the Gradle Wrapper from a private server, your credentials may be leaked if a build attempts to download the wrapper from a different server..
To safeguard your credentials, prefix the system properties with the private server's hostname, replacing all dots (`.`) with underscores (`_`).
...
To authenticate with a static HTTP Bearer Token, add the following line to your system properties file:
...
It is strongly recommended to include the hostname in the property to prevent the token from being accidentally sent to an unintended server.
