...
When the `wrapper` task updates an existing `gradle-wrapper.properties` file, values already present in that file are preserved when the corresponding task properties are not explicitly configured. This includes the distribution and archive paths and bases, the network timeout, URL validation, retries, and retry backoff. Explicit task configuration takes precedence. The distribution URL is regenerated for the configured Gradle version; its distribution type is inferred from the filename of an existing `-bin.zip` or `-all.zip` URL when the task does not configure one. The host and any other URL components are not preserved.
