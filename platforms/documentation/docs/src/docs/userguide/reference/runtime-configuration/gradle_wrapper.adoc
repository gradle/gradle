...
WARNING: If you use systemProp.gradle.wrapperUser and systemProp.gradle.wrapperPassword to download the wrapper from a private server, your credentials can be leaked if you use build that will download the wrapper from another server.
To prevent this, you can take the hostname of the private server, replace all dots with underscores, and use it as a prefix for the system properties.

Example: If the private server hostname is `your.private-server.com`, you can use the following system properties:
...
To use a static HTTP Bearer Token, add the following line to the system properties file:
...
If a wrapperToken is specified, it will be used instead of the username and password.
It is recommended to specify the hostname in the property to prevent the token from being accidentally sent to a foreign server
