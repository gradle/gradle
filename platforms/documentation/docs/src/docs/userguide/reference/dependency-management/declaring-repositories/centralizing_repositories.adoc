...
* *Relocated Maven master passwords.* The master password is only read from the default
  `settings-security.xml` location. Maven supports moving it elsewhere, either with a
  `<relocation>` pointing at another file or with the `settings.security` system property, and
  neither is supported here. Decryption still follows a `<relocation>`, because Maven's own
  dispatcher does, but Gradle does not know about the file it leads to: changes to it, such as
  rotating the master password, will not invalidate the configuration cache.
...
A repository matched by a mirror keeps its Gradle name, but its resolved URL, and therefore its
identity in build scans and the dependency cache, becomes the mirror's. Turning the property on or
off changes that identity and re-downloads artifacts.
