...
|<<sec:dependency-resolve-rules,8>>
|Per-Dependency Substitution Rules and Other Conditionals
...
== 6. Force Failed Resolution Strategies
...
== 7. Disabling Transitive Dependencies
...
== 8. Per-Dependency Substitution Rules and Other Conditionals

Per-dependency substitution rules (`dependencySubstitution { all { ... } }`) execute for every dependency as it's being resolved, providing a powerful API to modify a dependency's coordinates, such as group, name, or version, before the resolution is finalized.

This complements the static substitutions shown in <<sec:dependency-substitution-rules,Dependency Substitution>>: rather than declaring substitutions for specific coordinates up front, you can define rules dynamically based on the requested dependency.

This feature is particularly useful for implementing advanced dependency management patterns.
With per-dependency substitution rules, you can redirect dependencies to specific versions or even different modules entirely, allowing you to enforce consistent versions across a project or override problematic dependencies:
...
For more advanced usage and additional examples, refer to the link:{groovyDslPath}/org.gradle.api.artifacts.DependencySubstitutions.html[DependencySubstitutions] and link:{groovyDslPath}/org.gradle.api.artifacts.ResolutionStrategy.html[ResolutionStrategy] classes in the API documentation.
...
A dependency substitution rule offers an effective way to implement this:
...
- A substitution rule then resolves the `default` version to an approved version, which is retrieved from a corporate catalog of sanctioned modules.
...
In this setup, whenever a developer specifies `default` as the version, the substitution rule replaces it with the approved version from the corporate catalog.
...
Targeted substitution rules offer a powerful mechanism for blocking specific versions of a dependency and substituting them with an alternative.
...
By defining a substitution rule for the specific coordinates, you can automatically replace a problematic version with a stable one.
...
With a substitution rule, you can enforce this replacement: "anytime version `1.2` is requested, it will be replaced with `1.2.1`".
