...
A `strictly` declaration sets an upper bound that transitive dependencies cannot override.
...
// Result: commons-codec resolves to 1.9, not 1.10
----
[source,groovy]
----
dependencies {
...
dependencies {
    ...
    implementation("com.example:my-lib") {
        version {
            require("[1.0, 2.0[")
            reject("1.4")
        }
...
// Result: resolves to the highest available version in [1.0, 2.0) that is not 1.4
----
[source,groovy]
----
dependencies {
...
----
dependencies {
    implementation("org.example:some-lib") {
        version {
...
A consumer of your library that requires `1.8.0` will resolve successfully — the version falls within the strict range.
...
Using a constraint rather than a direct dependency means the constraint only takes effect if `log4j-core` appears somewhere in the graph.
...
// Result: commons-codec resolves to 1.9
----
[source,groovy]
----
dependencies {
...
This distinction matters because dependency constraints only take effect when the module appears in the graph — if no dependency brings in `commons-codec`, the constraint is ignored entirely.
