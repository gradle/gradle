...
| `strictly("[1.9, 1.10[")`
| Strict ranges cannot be overridden by other participants in the graph.
...
NOTE: This downgrade works because most transitive dependencies declare their requirements as `require`, which is satisfied by any version meeting or exceeding the requested one, so the `strictly` constraint can pull the resolved version back to `1.9`.
If a transitive dependency declared `strictly("1.10")` itself, the two strict declarations would conflict and resolution would fail rather than silently downgrade.
