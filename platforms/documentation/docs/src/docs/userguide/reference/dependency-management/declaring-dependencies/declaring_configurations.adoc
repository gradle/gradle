...
== Bucket Configurations added by Plugins

Gradle plugins often create *bucket link:{groovyDslPath}/org.gradle.api.artifacts.Configuration.html[configurations]* (like `implementation`, `api`, `testImplementation`) for you.
These buckets are used to declare a dependency and specify *where* that dependency is used (compile, runtime, tests, consumers, variants, etc.).

=== Java Plugin

Applying `java` or `java-library` wires the *standard JVM* buckets.

* App & Library (common):
** `implementation`
** `compileOnly`
** `runtimeOnly`
* Library-specific (`java-library` only):
** `api`
* Tests:
...
The Android Gradle Plugin (AGP) adds *variant-aware* buckets on top of the Java ones.
You’ll see *per-test-type* and *per-variant* buckets.
...
* Library-specific:
** `api` (only for Android library modules; not for application modules)
* Variant & build-type buckets (created automatically, e.g. for `debug` / `release` or flavors):
** `debugImplementation`, `releaseImplementation`
** `freeImplementation`, `paidImplementation` (for flavors)
** `freeDebugImplementation`, `paidReleaseRuntimeOnly`, etc.
* Optional plugin add-ons introduce more buckets:
** KAPT: `kapt`, `kaptTest`, `kaptAndroidTest`, `kapt<Variant>`
** KSP: `ksp`, `kspTest`, `kspAndroidTest`, `ksp<Variant>`

====
include::sample[dir="snippets/dependencyManagement/declaringConfigurations-android/kotlin",files="app/build.gradle.kts[tags=configurations]"]
include::sample[dir="snippets/dependencyManagement/declaringConfigurations-android/groovy",files="app/build.gradle[tags=configurations]"]
====

=== Kotlin Multiplatform Plugin

Kotlin Multiplatform (KMP) defines buckets per source set, so you attach dependencies under each target’s or common source set’s `dependencies {}` block.

* Common source sets:
** `commonMain.dependencies { implementation(...) }`
** `commonTest.dependencies { implementation(...) }`
* Per-target source sets (examples):
** `jvmMain`, `jvmTest`
**  `androidMain`, `androidUnitTest`, `androidInstrumentedTest` (with AGP)
** `iosMain`, `iosTest` (or more granular like `iosArm64Main`, etc.)

====
include::sample[dir="snippets/dependencyManagement/declaringConfigurations-kmp/kotlin",files="app/build.gradle.kts[tags=configurations]"]
include::sample[dir="snippets/dependencyManagement/declaringConfigurations-kmp/groovy",files="app/build.gradle[tags=configurations]"]
====

Under-the-hood, you’re actually adding dependencies to **Gradle configurations** that the Kotlin plugin wires up for each source set:

* `commonMainImplementation`
* `jvmMainImplementation`
* ...
* (and similarly `api`, `compileOnly`, `runtimeOnly` when applicable)
...
=== Declarable, Resolvable, and Consumable configurations in the Java Plugin

When you apply the Java (or Java Library) plugin, Gradle wires three “roles” of configurations that work together.
