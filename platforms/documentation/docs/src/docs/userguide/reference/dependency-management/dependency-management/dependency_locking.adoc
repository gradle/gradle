...
TIP: This is why updating a direct dependency version without also running `--write-locks` can cause a build failure: your new `require` may be higher than the version pinned by the lock's `strictly`.
