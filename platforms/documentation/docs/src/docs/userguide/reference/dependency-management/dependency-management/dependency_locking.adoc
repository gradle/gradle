...
** If you declare version `1.2.0` (interpreted as `require 1.2.0`) and the lock state contains `1.0.0`, the lock's `strictly 1.0.0` conflicts with your `require 1.2.0` — the declared version is higher than what the lock allows — and Gradle **fails resolution**. To resolve this, update the lock state with `--write-locks`.
...
TIP: This is why updating a direct dependency version without also running `--write-locks` can cause a build failure: your new `require` may be higher than the existing lock's `strictly`.
...
.gradle.lockfile
----
include::{snippetsPath}/dependencyManagement/dependencyLocking-lockingSingleFilePerProject/common/gradle.lockfile[]
----
