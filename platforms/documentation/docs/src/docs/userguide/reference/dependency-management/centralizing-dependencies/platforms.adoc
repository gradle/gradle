...
TIP: You can verify this on your own build with `./gradlew dependencies` or `./gradlew dependencyInsight --dependency <module>` — each converted constraint appears with a `{strictly <version>}` annotation in the report.
...
Consumers who need a different version cannot simply declare it, they must either override with their own `strictly` higher in the dependency graph, or disable endorsement on the platform import with `doNotEndorseStrictVersions()`.
