...
To use a platform, declare a dependency with the link:{javadocPath}/org/gradle/api/artifacts/dsl/DependencyHandler.html#platform(java.lang.Object)[`platform`] keyword:
...
For reference, the local `platform` project looks as follows:
...
- Enables the link:{javadocPath}/org/gradle/api/artifacts/ModuleDependency.html#endorseStrictVersions--[`endorseStrictVersions`] behavior by default, enforcing strict versions defined in the platform. Check out the <<#sec:endorsing-platform-strict-versions,section below>> to learn more.

Custom platforms like the `platform` project created with the <<java_platform_plugin.adoc#java_platform_plugin,`java-platform plugin`>> can be published as Maven BOMs (Bill of Materials) for consumption by other projects, just like the `junit` platform in the example above.
...
When you import a BOM using `platform`, the versions it defines act as recommendations, they can be upgraded by other dependencies in the graph.
To prevent upgrades and lock all versions from a BOM, use `enforcedPlatform`:
...
This means no dependency in the graph can upgrade past the versions defined in the BOM.

[[sec:enforced-platform-vs-strictly]]
==== `enforcedPlatform` vs. `strictly` on individual dependencies
...
Mixing both approaches on the same dependency group can lead to version misalignment.
For example, if you import a BOM with `platform` and then add a `strictly` constraint on a single module from that group, the constrained module may resolve to a different version than its siblings:
