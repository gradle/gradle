...
The IDEA plugin lets you customize how http://www.jetbrains.com/idea/[IntelliJ IDEA] understands your project. When IDEA imports a Gradle build via its https://www.jetbrains.com/help/idea/gradle.html#gradle_import[built-in Gradle integration], it takes the `idea { ... }` configuration into account. The `idea { ... }` configuration covers both external dependencies (including their associated source and Javadoc files) and project dependencies.
...
NOTE: The IDE file generation tasks that this plugin used to contribute (`idea`, `ideaProject`, `ideaModule`, `ideaWorkspace`, `openIdea` and the associated `clean*` tasks) were deprecated in Gradle 9 and have been removed. Use IDEA's built-in Gradle integration to import the project instead. See the <<upgrading_major_version_10.adoc#ide_task_removal,upgrade guide>> for details.

[[sec:idea_configuration]]
== Configuration

The plugin adds configuration options that let you customize how IDEA understands the project. For example, you can add source and resource directories. This configuration is honored by IDEA's import facility.
