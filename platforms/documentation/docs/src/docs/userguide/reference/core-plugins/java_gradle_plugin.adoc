...
It automatically applies the <<java_library_plugin.adoc#java_library_plugin,Java Library (`java-library`)>> plugin, adds the `gradleApi()` dependency to the `compileOnly` configuration and performs validation of plugin metadata during `jar` task execution.
