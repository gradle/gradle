...
`webXmlFile` — link:{javadocPath}/org/gradle/api/file/RegularFileProperty.html[RegularFileProperty]::
The `web.xml` file to copy to `WEB-INF/web.xml`.
When not set, no `web.xml` file is included in the archive.
Introduced in Gradle 9.7.0 as the replacement for the older `webXml` accessor.
