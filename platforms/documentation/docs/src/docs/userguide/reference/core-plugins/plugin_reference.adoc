...
<<java_test_fixtures_plugin.adoc#java_test_fixtures_plugin,Java Test Fixtures>>::
Provides support for producing and consuming test fixtures with the `java-test-fixtures` plugin.

<<jvm_toolchains_plugins.adoc#jvm_toolchains_plugins,JVM Toolchains>>::
Provides support for selecting and provisioning the JDK used to build and test, via the `jvm-toolchains` plugin (project) and `jvm-toolchain-management` plugin (settings).
...
<<groovy_gradle_plugin.adoc#groovy_gradle_plugin,Groovy Gradle Plugin>>::
