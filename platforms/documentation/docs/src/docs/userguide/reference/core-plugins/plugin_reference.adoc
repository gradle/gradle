...
Provides support for generating parsers using https://www.antlr.org/[ANTLR].
...
<<java_testing.adoc#sec:java_test_fixtures,Java Test Fixtures>>::
Provides support for producing and consuming test fixtures with the `java-test-fixtures` plugin.

<<toolchains.adoc#toolchains,JVM Toolchains>>::
Provides support for selecting and provisioning the JDK used to build and test, via the `jvm-toolchains` plugin. Toolchain download repositories are configured in a settings script with the `jvm-toolchain-management` plugin — see <<toolchain_plugins.adoc#toolchain_plugins,Toolchain Resolver Plugins>>.
...
Performs quality checks on your project’s Java source files using https://pmd.github.io/[PMD] and generates associated reports.

<<jacoco_plugin.adoc#jacoco_plugin,JaCoCo>>::
Provides code coverage metrics for your Java project using https://www.jacoco.org/jacoco/[JaCoCo].
...
Customizes how Eclipse/https://projects.eclipse.org/projects/tools.buildship[Buildship] understands your Gradle project during import.
...
Generates Visual Studio solution and project files for a build that can be opened by the IDE.
...
<<build_dashboard_plugin.adoc#build_dashboard_plugin,Build Dashboard Plugin>>::
Generates a single HTML dashboard that provides a single point of access to all of the reports generated by your build.

<<version_catalog_plugin.adoc#version_catalog_plugin,Version Catalog>>::
Provides support for generating and publishing a version catalog with the `version-catalog` plugin.

<<implementing_gradle_plugins_precompiled.adoc#implementing_precompiled_plugins,Precompiled Groovy DSL Script Plugins>>::
Provides support for authoring plugins as precompiled Groovy DSL scripts with the `groovy-gradle-plugin` plugin.
