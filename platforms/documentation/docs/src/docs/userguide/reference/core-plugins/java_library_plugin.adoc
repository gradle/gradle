...
When a dependency triggers this behavior, declare it as an `api` dependency so that it is available on consumers' compile classpath.
API and implementation separation has to account for what the compiler requires, not only which types you consider part of your public API.
====
