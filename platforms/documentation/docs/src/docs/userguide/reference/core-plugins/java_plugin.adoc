...
For information on the `api` and `compileOnlyApi` configurations, which are only added when the
<<java_library_plugin.adoc#sec:java_library_separation,Java Library Plugin>> is applied, please consult its reference documentation and
...
Controls whether Gradle infers the module path (instead of the classpath) for tasks that have a `module-info.java`. See <<java_library_plugin.adoc#sec:java_library_modular, Building Modules for the Java Module System>>. Default value: `true`
