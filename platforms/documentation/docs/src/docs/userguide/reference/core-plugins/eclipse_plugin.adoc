...
The `eclipse-wtp` plugin configures the WTP component and facet models. If a <<java_plugin.adoc#java_plugin,Java>> or <<war_plugin.adoc#war_plugin,War>> is applied, the classpath will be extended to get a proper packaging structure for this utility library or web application project.
