...
The Eclipse plugins provide a DSL for configuring model objects that model the Eclipse view of the project. Buildship consumes this model through the Tooling API when it imports or synchronizes the project. The model objects also expose lower-level `beforeMerged` and `whenMerged` hooks for working with the domain objects before and after they are populated, so you can adjust settings that the plugins do not model.
...
The `whenMerged` hook lets you manipulate the fully populated domain objects. Here is how you would export all the dependencies of an Eclipse project:
