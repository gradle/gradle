...
TIP: To make the Wrapper files available to other developers and execution environments, you need to check them into version control. +
...
TIP: HTTP Basic Authentication should only be used with `HTTPS` URLs and not plain `HTTP` ones. +
