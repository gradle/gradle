...
See <<properties_providers.adoc#managed_properties, Using Gradle Managed Properties>> for the full conventions.
...
TIP: When <<isolated_projects.adoc#sec:shared_build_services,Isolated Projects>> is enabled, most methods on the set returned by `getRegistrations()` are restricted. Use `registerIfAbsent()` to obtain a service provider directly.
...
.DownloadPlugin.java
----
include::{snippetsPath}/reference/plugin-development/buildServiceUsingServiceReference/groovy/buildSrc/src/main/java/DownloadPlugin.java[]
----
...
.Download.java
----
include::{snippetsPath}/reference/plugin-development/buildServiceUsingServiceReference/groovy/buildSrc/src/main/java/Download.java[]
----
...
----
include::{snippetsPath}/reference/plugin-development/buildServiceListener/groovy/buildSrc/src/main/java/TaskEventsService.java[tags=build-service]
----
...
----
include::{snippetsPath}/reference/plugin-development/buildServiceListener/groovy/buildSrc/src/main/java/TaskEventsPlugin.java[]
----
...
[[sec:configuration_cache_interaction]]
== Build Services and the Configuration Cache

When the <<configuration_cache.adoc#config_cache, Configuration Cache>> is enabled, build service parameters are serialized into the cache entry, and the service instance is recreated on every build.
Several caveats apply.
See <<configuration_cache_requirements.adoc#config_cache:requirements:build_services, Using Build Services>> in the Configuration Cache requirements for the full treatment.
