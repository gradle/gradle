...
This typically happens during the <<build_lifecycle.adoc#build_phases,execution phase>>.

The following demonstrates a task with a configurable `greeting` property and a read-only `message` property:

====
include::sample[dir="snippets/reference/task-development/propertyAndProvider/kotlin",files="build.gradle.kts[]"]
include::sample[dir="snippets/reference/task-development/propertyAndProvider/groovy",files="build.gradle[]"]
====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/propertyAndProvider/tests/usePropertyAndProvider.out[]
----
...
====
include::sample[dir="snippets/reference/task-development/connectProperties/kotlin",files="build.gradle.kts[]"]
include::sample[dir="snippets/reference/task-development/connectProperties/groovy",files="build.gradle[]"]
====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/connectProperties/tests/connectProperties.out[]
----
...
====
include::sample[dir="snippets/reference/task-development/fileAndDirectoryProperty/kotlin",files="build.gradle.kts[]"]
include::sample[dir="snippets/reference/task-development/fileAndDirectoryProperty/groovy",files="build.gradle[]"]
====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/fileAndDirectoryProperty/tests/workingWithFilesKotlin.out[]
----
=====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/fileAndDirectoryProperty/tests/workingWithFilesGroovy.out[]
----
=====
====
...
====
include::sample[dir="snippets/reference/task-development/implicitTaskInputFileDependency/kotlin",files="build.gradle.kts[]"]
include::sample[dir="snippets/reference/task-development/implicitTaskInputFileDependency/groovy",files="build.gradle[]"]
====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/implicitTaskInputFileDependency/tests/implicitTaskInputFileDependencyKotlin.out[]
----
=====
[.multi-language-text.lang-groovy]
=====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/implicitTaskInputFileDependency/tests/implicitTaskInputFileDependencyGroovy.out[]
----
=====
====
...
====
include::sample[dir="snippets/reference/task-development/implicitTaskInputDependency/kotlin",files="build.gradle.kts[]"]
include::sample[dir="snippets/reference/task-development/implicitTaskInputDependency/groovy",files="build.gradle[]"]
====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/implicitTaskInputDependency/tests/implicitTaskInputDependencyKotlin.out[]
----
=====
[.multi-language-text.lang-groovy]
=====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/implicitTaskInputDependency/tests/implicitTaskInputDependencyGroovy.out[]
----
=====
====

[[working_with_collections]]
[[nested_output_properties]]
=== Nested output properties

A task can expose outputs through a managed bean in an `@Nested Property<T>`.
Create the bean with `objects.newInstance()` and access its outputs through the task's nested property:

[source,groovy]
.build.gradle
----
abstract class OutputFiles {
    @OutputFile abstract RegularFileProperty getResult()
}

abstract class Generate extends DefaultTask {
    @Nested abstract Property<OutputFiles> getFiles()

    @TaskAction void generate() {
        files.get().result.get().asFile.text = "generated"
    }
}

def outputFiles = objects.newInstance(OutputFiles)
outputFiles.result.set(layout.buildDirectory.file("generated.txt"))
def generate = tasks.register("generate", Generate) {
    files.set(outputFiles)
}

tasks.register("copyResult", Copy) {
    from(generate.flatMap { it.files.flatMap { it.result } })
    into(layout.buildDirectory.dir("copied"))
}
----

Running `copyResult` automatically schedules `generate`.
The same inference works through an aliased container, such as `def files = generate.get().files; files.flatMap { it.result }`, and through several levels of managed nested properties.

Assignment does not inspect the bean or establish its output ownership.
Resolving the nested property establishes ownership before returning the bean to `get()`, `map`, or `flatMap`.
Reading the bean alone leaves the property configurable.
Once Gradle uses an output's producer information or inspects its output declaration, it finalizes the containing property and retains that bean.
This also applies to containing nested properties at each level.
Output locations retain their usual lazy-property lifecycle.

Input-only beans may be shared between tasks.
An output bean must have a single producing task; sharing it across resolved declarations makes automatic dependency inference ambiguous.
Use separate output beans for separate producers.

The bean's structure must be available without executing a task or reading generated file contents.
Its input leaves can still reference other tasks' outputs, and its output filenames can be lazy.
Beans selected from configuration inputs should use configuration-cache-compatible providers.

This ownership support applies to Gradle-created beans reached through a task's managed or decorated `@Nested Property<T>` getter.
It includes eligible final getters returning such a property.
It does not add ownership to standalone properties, plain beans, collection elements, or command-line argument provider lists.
Those forms can still declare output locations for tracking.
A direct reference such as `from(outputFiles.result)` bypasses the nested property and cannot establish ownership on its own.
Use the provider form above to make the producer discoverable.
...
====
include::sample[dir="snippets/reference/task-development/listProperty/kotlin",files="build.gradle.kts[]"]
include::sample[dir="snippets/reference/task-development/listProperty/groovy",files="build.gradle[]"]
====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/listProperty/tests/listPropertyKotlin.out[]
----
=====
[.multi-language-text.lang-groovy]
=====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/listProperty/tests/listPropertyGroovy.out[]
----
=====
====
...
====
include::sample[dir="snippets/reference/task-development/mapProperty/kotlin",files="build.gradle.kts[]"]
include::sample[dir="snippets/reference/task-development/mapProperty/groovy",files="build.gradle[]"]
====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/mapProperty/tests/mapProperty.out[]
----
...
====
include::sample[dir="snippets/reference/task-development/propertyConvention/kotlin",files="build.gradle.kts[tags=conventions]"]
include::sample[dir="snippets/reference/task-development/propertyConvention/groovy",files="build.gradle[tags=conventions]"]
====
...
[source,text]
----
include::{snippetsPath}/reference/task-development/propertyConvention/tests/propertyConvention.out[]
----

[[where_to_apply_conventions_from]]
...
====
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/kotlin",files="build.gradle.kts[tags=convention-callsites]"]
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/groovy",files="build.gradle[tags=convention-callsites]"]
====

[[from_a_plugins_apply_method]]
...
====
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/kotlin",files="build.gradle.kts[tags=convention-callsites-from-plugin]"]
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/groovy",files="build.gradle[tags=convention-callsites-from-plugin]"]
====
...
====
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/kotlin",files="build.gradle.kts[tags=convention-callsites-from-buildscript]"]
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/groovy",files="build.gradle[tags=convention-callsites-from-buildscript]"]
====
...
====
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/kotlin",files="build.gradle.kts[tags=convention-callsites-from-constructor]"]
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/groovy",files="build.gradle[tags=convention-callsites-from-constructor]"]
====

[[next_to_the_property_declaration]]
...
====
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/kotlin",files="build.gradle.kts[tags=convention-callsites-from-declaration]"]
include::sample[dir="snippets/reference/task-development/propertyConventionCallsites/groovy",files="build.gradle[tags=convention-callsites-from-declaration]"]
====
...
[[lazy_collections_api_reference]]
