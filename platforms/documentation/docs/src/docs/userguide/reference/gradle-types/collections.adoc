...
All Gradle collection types extend from link:{javadocPath}/org/gradle/api/DomainObjectCollection.html[`DomainObjectCollection<T>`], which defines the core contract for live collections — including methods like `configureEach()`, `withType()`, `matching()`, `all()`, `whenObjectAdded()`, and `addLater()`.
