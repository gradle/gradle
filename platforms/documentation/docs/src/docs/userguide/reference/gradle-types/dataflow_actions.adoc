...
NOTE: A partially reused entry — possible for example with the parallel configuration cache or under <<isolated_projects.adoc#isolated_projects,isolated projects>> — is reported as `UPDATED`, not `REUSED`. A strict reuse check like the one above should only accept `REUSED`.
