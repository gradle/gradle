...
[[verifying_configuration_cache_reuse]]
== Verifying configuration cache reuse

The link:{javadocPath}/org/gradle/api/flow/FlowProviders.html#getConfigurationCacheOutcome--[`configurationCacheOutcome`] lifecycle event provider carries the outcome of <<configuration_cache.adoc#config_cache,configuration caching>> for the build invocation: whether a cache entry was stored, reused, or storing it failed or was deliberately skipped, expressed as a subtype of
link:{javadocPath}/org/gradle/api/configuration/ConfigurationCacheOutcome.html[`ConfigurationCacheOutcome`].
When the configuration cache is not enabled, the value is `ConfigurationCacheOutcome.NotEnabled`, so the provider is safe to use unconditionally.

This makes it possible to enforce configuration cache reuse from build logic, for example on a CI setup that runs the same build twice and expects the second invocation to reuse the entry stored by the first:

[source,groovy]
----
class VerifyConfigurationCacheReuse implements FlowAction<Parameters> {
    ...
    void execute(Parameters parameters) {
        // Read the toggle inside the action, at execution time: reading it at configuration
        // time would make it a configuration input and invalidate the very entry to verify.
        if (System.getenv('VERIFY_CC_REUSE') != null) {
            def outcome = parameters.outcome.get()
            if (outcome !instanceof ConfigurationCacheOutcome.Reused) {
...
NOTE: Only `ConfigurationCacheOutcome.Reused` means the entry was fully reused. A strict reuse check like the one above should accept nothing else.

Plugin authors can make equivalent assertions in their tests via the TestKit API, see <<test_kit.adoc#sub:test-kit-configuration-cache,Testing with the Configuration Cache>>.
