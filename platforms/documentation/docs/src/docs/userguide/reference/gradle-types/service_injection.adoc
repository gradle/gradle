...
Instead of declaring an `@Inject` point, build and settings scripts, plugins, and task actions can look up a curated set of Gradle services directly with `service(Class)` in the Groovy DSL or `service<Type>()` in the Kotlin DSL.
...
The lookup is a member of link:{javadocPath}/org/gradle/api/Project.html#service-java.lang.Class-[`Project`], link:{javadocPath}/org/gradle/api/initialization/Settings.html#service-java.lang.Class-[`Settings`] and link:{javadocPath}/org/gradle/api/Task.html#service-java.lang.Class-[`Task`], and resolves the services of that project, build, or task.
Script plugins applied with `apply from:`, <<implementing_gradle_plugins_precompiled.adoc#implementing_precompiled_plugins,precompiled script plugins>>, and binary plugins can use it in the same way.

In a build script, `service<T>()` can be called in two places, and the choice matters for when the lookup runs.

*Inside a task action*. The lookup is made on the task and runs each time the task executes.
...
These bounds are enforced at compile time in the Kotlin DSL, and at runtime in the Groovy DSL:

[%header,cols="2,1,1,1"]
|===
| Service | Projects | Tasks | Settings
...
† `ExecOperations` can only be looked up on a task, which keeps external processes out of the configuration phase.
To run a process at configuration time, use link:{javadocPath}/org/gradle/api/provider/ProviderFactory.html#exec-org.gradle.api.Action-[`ProviderFactory.exec`], which the Configuration Cache tracks.

Relative paths passed to a looked-up `FileSystemOperations` or `ArchiveOperations` are resolved the same way as for the enclosing scope: against the project directory for a project or task, and against the build's root directory for settings.

=== Error messages

In the Kotlin DSL, requesting a service outside its scope, or a type that is not a Gradle service at all, is a compile error.
...
=== Scopes and Isolated Projects

`service(...)` resolves against the project, build, or task it is called on: a project and its tasks see the services of that project, and settings those of the build.
There is no lookup on `Gradle`, so a project cannot use it to reach the services of the build or of a parent build.

Calling it on another project, for example inside a `subprojects {}` block or as `project(":other").service(ObjectFactory)`, resolves the services of that other project.
This is cross-project configuration and is reported as a problem under <<isolated_projects.adoc#isolated_projects,Isolated Projects>>, exactly like `project(":other").objects`.
Look services up from the owning project or from its tasks instead.
...
| `service<T>()`
| Writing a script, a plugin, or a task action; the service is in the lookup table above.

| `@Inject`
| You own a task, plugin, or extension type; you want the type unit-testable, or the service is not in the lookup table.
