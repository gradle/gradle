...
The lookup is a member of the script itself and of link:{javadocPath}/org/gradle/api/Task.html#service-java.lang.Class-[`Task`], and a script or task can only look up services of its own scope.
Script plugins applied with `apply from:` and <<implementing_gradle_plugins_precompiled.adoc#implementing_precompiled_plugins,precompiled script plugins>> are scripts too, and can use it in the same way.
...
|===
| Service | Build scripts | Tasks | Settings scripts | Init scripts
...
Relative paths passed to a looked-up `FileSystemOperations` or `ArchiveOperations` are resolved the same way as for the enclosing scope: against the project directory for a build script or task, and against the build's root directory for a settings or init script.
...
`service(...)` always resolves against the scope of the script or task it is called on: a build script and its tasks see the services of their own project, a settings script those of the settings, and an init script those of the build.
A build script cannot use it to reach the services of another project or of the build.

A script plugin applied to an explicit target with `apply from: "...", to: target` resolves services in the scope of that target.

Looking up a service on a task of another project requires reaching that task first, which is reported as a problem under <<isolated_projects.adoc#isolated_projects,Isolated Projects>> like any other cross-project task access.
...
| `service<T>()`
| Writing a script, a script plugin, or a task action; the service is in the lookup table above.

| `@Inject`
| You own a task, plugin, or extension type, such as in a binary plugin; you want the type unit-testable.
