...
<2> TestSuiteName attribute; value is derived from link:{javadocPath}/org/gradle/api/Named.html#getName()[TestSuite#getName()].
...
====
<.> Creates a report named `testAggregateTestReport` of type `AggregateTestReport`, aggregating all test suites from all project dependencies with a given link:{javadocPath}/org/gradle/api/Named.html#getName()[TestSuite#getName()].
