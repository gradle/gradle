...
Non-class-based tests can be filtered in the same way as class-based tests using the link:{javadocPath}/org/gradle/api/tasks/testing/TestFilter.html[`TestFilter`] API or `--tests` command line option.

Non-class-based test filtering is performed by converting the path to the test definition into a package like name by converting file path separators into package separators.
A file with the path `src/test/definitions/sub/MoreTests.xml` would be converted into a class name like `sub.MoreTests.xml` by calculating a path relative to the root of the test definitions.
...
If the previous example was run with the filter: `./gradlew test --info --tests "*ExampleTests.xml"` then only the tests in `ExampleTests.xml` would be executed.
