...
[WARNING]
====
When filtering tests, the default in Gradle is to fail if the filter does not match any test.
That means that using the top level task selector, like `test`, in a multi project has a high chance of failing if your filter matches tests in a single project only.

To change this behavior, you can pass the additional command line option `--no-matching-tests` which will cause the build to succeed even if the filter does not match any test.
====
