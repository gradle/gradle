...
+
You can configure manifest attributes on a `Jar` task:
+
====
...
====
+
Manifest attribute values are evaluated lazily when the manifest is written.
In the example above, `archiveVersion` and `buildCommit` are resolved only when the `jar` task generates the manifest.
+
You can also create a standalone `Manifest` instance with the `java` extension and share it between JARs:
+
====
