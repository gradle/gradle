...
====
include::sample[dir="snippets/reference/platforms/jvm/customizedLayout/kotlin", files="build.gradle.kts[tags=define-main]"]
include::sample[dir="snippets/reference/platforms/jvm/customizedLayout/groovy", files="build.gradle[tags=define-main]"]
====
...
| Resolvable
| Represents the classpath used to run the main sources, which includes dependencies from both `runtimeOnly` and `implementation` configurations. Used by the `run` task.
...
====
include::sample[dir="snippets/reference/platforms/jvm/basic/kotlin",files="build.gradle.kts[tags=java-extension]"]
include::sample[dir="snippets/reference/platforms/jvm/basic/groovy",files="build.gradle[tags=java-extension]"]
====
