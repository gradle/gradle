...
* The class analysis is cached in the project directory, so the first build after a clean checkout can be slower. Consider turning off the incremental compiler on your build server, for example with `org.gradle.java.disable-incremental-compile=true`.
