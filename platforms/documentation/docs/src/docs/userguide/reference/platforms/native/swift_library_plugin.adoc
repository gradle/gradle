...
The Swift Library Plugin adds a link:{groovyDslPath}/org.gradle.nativeplatform.tasks.LinkSharedLibrary.xml[LinkSharedLibrary] instance for each variant of the library containing shared linkage as a dimension - e.g. `linkDebug` and `linkRelease`.
Read the <<building_swift_projects.adoc#sec:introducing_build_variants-swift,introduction to build variants>> for more information.
Some of the most common configuration options are shown below.
