...
[[sec:cpp_tool_chain_freebsd]]
==== FreeBSD

NOTE: FreeBSD support for the native plugins is link:{javadocPath}/org/gradle/api/Incubating.html[incubating].

To build on FreeBSD, install GCC or Clang via the FreeBSD ports collection or `pkg`.
The {cpp} plugins will discover GCC or Clang using the system PATH.
