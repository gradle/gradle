...
2. `__$<<directory_layout.adoc#dir:gradle_user_home,GRADLE_USER_HOME>>__/init.gradle(.kts)`
3. Any `\*.init.gradle(.kts)` in `__$<<directory_layout.adoc#dir:gradle_user_home,GRADLE_USER_HOME>>__/init.d/` (alphabetical)
4. Any `\*.init.gradle(.kts)` in `$<<installation.adoc#sec:linux_macos_users_2,__GRADLE_HOME__>>/init.d/` (alphabetical)
...
Like any Gradle script, an init script implements link:{groovyDslPath}/org.gradle.api.Script.html[`Script`] and talks to the link:{groovyDslPath}/org.gradle.api.invocation.Gradle.html[`Gradle`] instance.

NOTE: Properties from `gradle.properties` are available on `Settings`/`Project`, *not* on `Gradle`.

TIP: Useful `Gradle` callbacks: `settingsEvaluated {}`, `projectsLoaded {}`, `beforeProject {}`, `afterProject {}`.
...
- *Scope*: Runs at init; can access `settingsEvaluated`, `projectsLoaded`, `beforeProject`/`afterProject` callbacks to safely interact with settings/projects when they exist.
- *Distribution*: Applied from an init script (`~/.gradle/init.d/*.init.gradle(.kts)` or `$GRADLE_HOME/init.d`) so it affects every build on that machine/agent.

You can find a simple example in the next section.
