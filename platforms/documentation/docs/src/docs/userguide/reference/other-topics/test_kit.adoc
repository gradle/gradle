...
It's important to note that the mechanism currently _only_ works if the plugin under test is applied using the <<plugins_intermediate.adoc#sec:plugins_block,plugins DSL>>.
...
// keep the version below in sync with the compatibility matrix in tooling_api.adoc#sec:embedding_compatibility
It is possible to use the GradleRunner to execute builds with Gradle 4.0 and later.
