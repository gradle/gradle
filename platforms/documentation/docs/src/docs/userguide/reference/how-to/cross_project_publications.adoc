...
Gradle's <<variant_aware_resolution.adoc#sec:understanding-variant-selection, variant model>> allows consumers to specify requirements using _attributes_, while producers provide appropriate outgoing variants using attributes as well.
