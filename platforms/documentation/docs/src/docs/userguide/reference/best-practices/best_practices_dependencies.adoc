...
Occasionally, you may need to <<how_to_exclude_transitive_dependencies.adoc#how_to_exclude_transitive_dependencies,exclude transitive dependencies>> that are causing conflicts or issues in your project.
Gradle allows you to exclude transitive dependencies in several different ways.

Always excludes transitive dependencies as narrowly as possible:

 - Attach exclusions to *specific dependencies*, instead of every dependency in a configuration.
 - Instead of excluding every dependency available from a particular `group`, only exclude a specific `module` from that `group`.
 - *Never* exclude dependencies from *every configuration* using `configurations.all { ... }` or `configurations.configureEach { ... }`.

Exclusions have a negative impact on the performance of dependency resolution.
Applying exclusions as narrowly as possible minimizes this impact.
It also reduces the risk of inadvertently and silently excluding dependencies that are needed by other parts of your build, and of accidental runtime dependency clashes.
...
====
<1> The `commons-pool2` dependency transitively includes `cglib:cglib` and `org.ow2.asm:asm-util` as optional dependencies, which we want to exclude from this project.
...
If this project's other (or future) dependencies depend on other modules from `cglib`, compilation or runtime errors may result as those dependencies will not be resolved.
<3> This excludes `org.ow2.asm:asm-util` from *every* dependency in the `implementation` configuration.
If future dependencies are added that depend on `org.ow2.asm:asm-util`, compilation or runtime errors may result because those dependencies will be silently excluded.
<4> This excludes `javaassist:javassist` from *every* dependency in *every* configuration, including configurations that may be added by plugins and in the future, which has the same risks as above but amplified.

==== Do This Instead

Exclude transitive dependencies narrowly at the individual dependency level:
...
Though it may seem repetitive to exclude the same transitive dependencies from multiple dependencies, this approach is safer, more performant, and makes it clearer exactly which dependencies are meant to be affected by each exclusion.
