...
Create included build with convention plugins for your build in `build-logic`:
...
Create convention plugins separate in `build-logic`:
...
include::sample[dir="snippets/bestPractices/useConventionPlugins-do/groovy",files="build-logic/src/main/kotlin/my.base-java-library.gradle[tags=do-this];build-logic/src/main/kotlin/my.java-library.gradle[tags=do-this];build-logic/src/main/kotlin/my.java-use-junit5.gradle[tags=do-this]"]
====

<1> Default settings for Java library plugin.
<2> Junit5 configuration in convention plugin.
<3> Convention plugin that aggregates other convention plugins.
