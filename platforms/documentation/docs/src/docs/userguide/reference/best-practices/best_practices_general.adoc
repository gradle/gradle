...
You cannot assume whether `pluginA`, `pluginB`, `pluginC`, or `pluginD` will be applied first because the first thing `pluginC` does could be to apply `pluginD`.
...
Avoid build logic that assumes ordering between different projects, included builds, or applied scripts; the actual order is not guaranteed and is extremely vulnerable to minor structural changes (such as adding a new project or renaming an include).
...
- Apply it yourself as the first action in your `Plugin.apply()` method to ensure it is applied.
- React to the other plugin's application:
