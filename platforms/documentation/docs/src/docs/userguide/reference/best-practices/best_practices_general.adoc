...
Gradle does not guarantee the order in which plugins are applied, especially across multiple build scripts, projects, convention plugins, or included builds.
...
However, in <<multi_project_builds.adoc#multi_project_builds,multi-project builds>>, with multiple `build.gradle(.kts)` files or a <<plugins.adoc#sec:convention_plugins,convention plugin>>, plugin application can be non-deterministic.
Gradle may configure projects in parallel, and the effective order can vary between runs:
...
You cannot assume whether `pluginA`, `pluginB`, `pluginC`, or `pluginD` will be applied first.

==== Build Engineers

Writing build logic that assumes plugin ordering can lead to:

- Fragile builds that break when project structure changes.
- Brittle behavior that works locally but fails in CI or in <<composite_builds.adoc#composite_builds,composite builds>>.
- Hard-to-diagnose bugs when plugins interact inconsistently due to race conditions.

Techniques like `allprojects {}`, `subprojects {}`, or `afterEvaluate {}` do not reliably control plugin order across projects or included builds.

Avoid build logic that assumes ordering between different projects, included builds, or applied scripts, the order is not guaranteed.

==== Plugin Developers

If your plugin requires another plugin, do not assume it has already been applied.
Either:

- Apply it yourself, or
- Fail fast with a clear error if it is missing.

[source,groovy]
----
if (!project.pluginManager.hasPlugin(requiredPlugin)) {
    // Warn or fail with a clear message
}
project.pluginManager.apply('com.example.required-plugin')
----

If your plugin can work with or without another plugin, and behavior changes when that plugin is present, use link:{javadocPath}/org/gradle/api/plugins/PluginManager.html#withPlugin(java.lang.String,org.gradle.api.Action)[`withPlugin()`]: to defer configuration until it is actually applied:

[source,groovy]
----
pluginManager.withPlugin('ivy-publish') {
    // Configure 'ivy-publish' after it is applied
}
----
...
In the plugin implementation, `MyPlugin.kt` also assumes `java` is already applied:
...
In the `app` subproject, the build file uses `plugins.withId("java") {}` to safely configure tasks once `java` is applied.

====
include::sample[dir="snippets/bestPractices/dontAssumePluginOrder-do/kotlin",files="app/build.gradle.kts[tags=do-this]"]
====

In the plugin implementation, `MyPlugin.kt` explicitly applies the `java` plugin:
