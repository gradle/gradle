...
Use Incubating APIs deliberately and with awareness of trade-offs. New or unstable Incubating features can require updates even on minor Gradle upgrades, while some features are close to Stable and already widely adopted by the users. Weigh the benefits against the likely increase in maintenance effort.

=== Explanation

Compared to many other ecosystems, Gradle is very strict with Incubating features. As long as there are outstanding issues related to the feature, it will not be promoted to the Stable state. Incubating state means that breaking changes affecting the feature can happen in non-major releases. However, the closer the feature is to being Stable, the less likely it is to change. There are a number of features that are already widely adopted and recommended as best practices, even though they are still incubating, like https://github.com/gradle/gradle/issues/32443[`dependencyResolutionManagement.repositories`].
