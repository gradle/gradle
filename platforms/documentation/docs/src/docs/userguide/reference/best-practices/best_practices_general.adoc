...
Gradle's plugin application is deterministic, but the effective execution order is complex and not guaranteed to follow a visible sequence, especially across multiple build scripts, projects, convention plugins, or included builds.
...
You cannot assume whether `pluginA`, `pluginB`, `pluginC`, or `pluginD` will be applied first because the first thing `pluginA` does could be to apply `pluginD`.
...
[source, kotlin]
----
// Configure behavior that depends on required-plugin
project.pluginManager.withPlugin("com.example.required-plugin") {  }

project.plugins.configureEach { plugin ->
    when (plugin) {
        // Configure behavior specific to RequiredPlugin
        is com.example.RequiredPlugin -> {  }
        // Configure behavior specific to OtherPlugin
        is com.example.OtherPlugin -> {  }
    }
}
----
=====
...
project.plugins.configureEach { plugin ->
    ...
    // Configure behavior specific to OtherPlugin
    else if (plugin instanceof com.example.OtherPlugin) {  }
}
----
=====
====
