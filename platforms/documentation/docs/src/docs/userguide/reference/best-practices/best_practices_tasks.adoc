...
Running `consumer` task, then `taskB`, the `consumer` task will be invalidated and have to run again despite it not using the output from `taskB`.
...
<1> *Write to a file in the output file*: This task produces a single file to a directly specified `outputFile`.
...
<5> *Wire taskA to consumer*: Makes sure that `taskA` and produces the output file before it is used by `consumer`.
