...
`@PathSensitivity.ABSOLUTE` prevents the <<build_cache.adoc#build_cache,Build Cache>> and <<configuration_cache.adoc#config_cache,Configuration Cache>> from being reused across different machines or directories, making your build non-relocatable.
