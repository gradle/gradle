...
For example, if the contents of a task’s output directory change between builds, Gradle will treat the task as out-of-date and execute it again.

To avoid unnecessary work, ensure that each task uses its own unique output files and directories, both within a project and across the entire build.
...
With this setup, if you run the `consumer` task and then run `greeterB`, the `consumer` task will be invalidated.

The next time you run `consumer`, it will *not* be `UP-TO-DATE` and will execute again, even though it does not use the output from `greeterB`.

This happens because `greeterB` modifies the shared `greetings` output directory, which is also declared as an output of `greeterA` (and therefore an input to `consumer`).
Gradle only sees that the directory contents have changed, so it must assume `consumer`’s inputs are out-of-date, even though the `a.txt` file it actually reads has not changed.

==== Do This Instead

To avoid issues, avoid shared output directories or files across multiple tasks.

Each task should declare only the outputs it actually produces and consume only the inputs it actually reads.

In this example, the _simplest_ fix is to give each `GreetingTask` its own dedicated output directory.
This change alone is enough to prevent `consumer` from being invalidated when `greeterB` runs:
...
Now, when running the `consumer` task, followed by `greeterB`, the `consumer` task remains `UP-TO-DATE`.
Gradle knows it is not using the output from `greeterB`, since `greeterA` and `greeterB` write to distinct output directories.

However, a more _complete_ and _idiomatic_ approach recognizes that:

1. Tasks that produce a single output file should express this clearly by using a single-file type `@Output` property.
2. Tasks that consume a single input file should likewise use a single-file `@Input` property.
...
Now, when you run the `consumer` task and then `greeterB`, the `consumer` task remains UP-TO-DATE. 
Gradle knows that it is not using the output from `greeterB`, since `greeterA` and `greeterB` _produce and consume distinct files_ (that happen to be created in the same directory).
