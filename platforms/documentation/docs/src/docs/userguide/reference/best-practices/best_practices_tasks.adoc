...
[[prefer_path_sensitive]]
== Prefer `@PathSensitivity.NONE` for file inputs

When defining file inputs for tasks, prefer using `@PathSensitivity.NONE` unless your task specifically requires path sensitivity.

=== Explanation

Most of the time, tasks only care about the **contents** of their input files, not their locations.
You should annotate your `@InputFile` and `@InputFiles` properties as `@PathSensitivity.NONE` unless your task's behavior should change based on the location of the input files.

Task inputs that include path sensitivity can result in non-relocatable builds, which can prevent hits using the <<build_cache.adoc#build_cache, build cache>> and <<configuration_cache.adoc#config_cache,configuration cache>>.
It can also cause different task results when the build is run in different directories, which can be confusing and hard to debug.
...
====
include::sample[dir="snippets/bestPractices/preferPathSensitivityNone-avoid/kotlin",files="app/settings.gradle.kts[]"]
include::sample[dir="snippets/bestPractices/preferPathSensitivityNone-avoid/groovy",files="app/settings.gradle[]"]
====
...
====
include::sample[dir="snippets/bestPractices/preferPathSensitivityNone-do/kotlin",files="app/settings.gradle.kts[]"]
include::sample[dir="snippets/bestPractices/preferPathSensitivityNone-do/groovy",files="app/settings.gradle[]"]
====
