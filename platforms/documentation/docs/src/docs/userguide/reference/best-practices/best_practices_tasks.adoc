...
Tasks should generally only care about the **contents** of their input files, not their location on disk.

When annotating input properties that contain files (such as `@InputFile` and `@InputFiles` collections) use `@PathSensitivity.NONE`.
This tells Gradle to consider only the files' contents when determining if a task is up-to-date.

For input properties that contain directories (such as `@InputDirectory` and `@InputFiles` collections) use `@PathSensitivity.RELATIVE`.
This tells Gradle to also consider the path to each directory relative to the build's root directory (ignoring its absolute location) for up-to-date checks.

Using `PathSensitivity.NAME_ONLY` or `@PathSensitivity.ABSOLUTE` is generally incorrect.

`PathSensitivity.NAME_ONLY` tells Gradle to also consider a file's name in addition to its contents, which is rarely useful.
`@PathSensitivity.ABSOLUTE` tells Gradle to consider a files' complete absolute path.
This prevents <<build_cache.adoc#build_cache,Build Cache>> and <<configuration_cache.adoc#config_cache,Configuration Cache>> hits across different machines or directories, making your build non-relocatable.
