...
The _Daemon JVM criteria_ is controlled by a task, similar to how the <<gradle_wrapper.adoc#customizing_wrapper, `wrapper` task>> updates the wrapper properties file.
...
WARNING: With the addition of auto-provisioning support, the default setup requires that <<toolchains.adoc#sub:download_repositories, toolchain download repositories>> be configured.
...
See <<#daemon_jvm_discovery,Daemon JVM Discovery>> for an explanation of the URLs generated in this file and how to modify them.

The same properties file can be generated without configuring the task in the build script by using a command-line argument:

----
$ ./gradlew updateDaemonJvm --jvm-version=17
----

If you run the task without any arguments, and the properties file does not exist, then the version of the current JVM used by the daemon will be used and the file will be created.
...
NOTE: Gradle currently supports only the major JVM version (`--jvm-version`) and JVM vendor (`--jvm-vendor`) as criteria. Support for additional criteria may be added in a future release.

==== Specifying a JVM vendor

The JVM vendor, like the JVM version, can be used as a criterion to select a compatible JVM installation for the build.
...
You can view the list of recognized vendors by running `gradle help --task updateDaemonJvm`.

If the specified vendor is not one of the recognized equivalents, Gradle will match it exactly.
For example, "My Custom JVM" would require an exact match of the vendor name.

The vendor can be configured on the task using <<toolchains.adoc#sec:vendors,the same syntax as the Java toolchain spec>>.

[[sec:daemon_jvm_discovery]]
...
If no such repositories are configured, the `updateDaemonJvm` task will fail.
