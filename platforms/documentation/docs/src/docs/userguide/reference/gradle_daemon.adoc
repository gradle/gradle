...
If the Daemon JVM criteria configuration is available, it takes precedence over `JAVA_HOME` and `org.gradle.java.home`.
...
The _daemon JVM criteria_ is controlled by a task, similarly to how <<gradle_wrapper.adoc#customizing_wrapper,`wrapper` task>> updates the wrapper properties file.
...
NOTE: With the addition of the support for auto-provisioning, the default setup mandates that <<toolchains.adoc#sub:download_repositories,toolchain download repositories>> are configured.
See <<#daemon_jvm_provisioning,below>> for details.
...
toolchainUrl.FREE_BSD.AARCH64=https\://exmaple.com/...
toolchainUrl.FREE_BSD.X86_64=https\://exmaple.com/...
toolchainUrl.LINUX.AARCH64=https\://exmaple.com/...
toolchainUrl.LINUX.X86_64=https\://exmaple.com/...
toolchainUrl.MAC_OS.AARCH64=https\://exmaple.com/...
toolchainUrl.MAC_OS.X86_64=https\://exmaple.com/...
toolchainUrl.UNIX.AARCH64=https\://exmaple.com/...
toolchainUrl.UNIX.X86_64=https\://exmaple.com/...
toolchainUrl.WINDOWS.X86_64=https\://exmaple.com/...
toolchainVersion=17
----
====

See <<#daemon_jvm_discovery,Daemon JVM discovery>> for an explanation of the URLs generated in this file and how to influence them.
...
Like the JVM version, the JVM vendor can be used as criteria to select a compatible JVM installation for the build.
When no JVM vendor is specified, Gradle will consider all vendors compatible.

By default, running `updateDaemonJvm` to create the `gradle-daemon-jvm.properties` file will not generate a JVM vendor criterion.
You must either explicitly specify a JVM vendor for the `updateDaemonJvm` task in the build script or pass a JVM vendor on the command-line with `--jvm-vendor=<value>`.

Gradle recognizes a small number of JVM vendor strings as special and equivalent.
For example, "Adoptium" and "Temurin" are considered the same vendor.
You can see the list of special vendors by running `gradle help --task updateDaemonJvm`.

If the JVM vendor you specify is not treated as a special value, Gradle considers the value as an exact match.
For example, to match the vendor "My Custom JVM", the vendor criterion must be "My Custom JVM".

Configuring the vendor on the task itself uses the <<toolchains.adoc#sec:vendors,same syntax as the Java toolchain spec>>.

[[daemon_jvm_discovery]]
...
This process fully supports both <<toolchains.adoc#sec:auto_detection,auto-discovery>> and <<toolchains.adoc#sec:provisioning,auto-provisioning>>.

[[daemon_jvm_provisioning]]
==== Controlling auto-provisioning URLs

The `updateDaemonJvm` task will, by default, attempt to generate download URLs for JDKs on different platforms (OS and architecture) that match the criteria.

By default, Gradle will consider the following build platforms:
...
These can be configured through the `toolchainPlatforms` property on the `UpdateDaemonJvm` task.
They do not have a CLI configuration option.

TODO Add a snippet showing how to configure those and one on how to disable that feature

Gradle will attempt to resolve JDKs URLs for these platforms by leveraging the <<toolchains.adoc#sub:download_repositories,configured toolchain download repositories>>.
If no such repositories are configured, then the `updateDaemonJvm` task will fail.

Instead of using toolchain download repositories, users may chose to configure the JDK URLs on the task itself, using the `toolchainDownloadUrls` property.
This is a `Map<BuildPlatform, URI>` that can be configured as the following example shows:

TODO add example of hardcoding URLs in task configuration
