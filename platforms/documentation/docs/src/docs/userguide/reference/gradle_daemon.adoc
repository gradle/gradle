...
The daemon JVM is located using the same underlying logic as for <<toolchains.adoc#sec:auto_detection,project JVM toolchains>>.

The auto-provisioning is simpler, as Gradle can only look up a download URL matching the current platform inside the `gradle-daemon-jvm.properties` file, which is then used to download a JDK if none can be found locally.

Both properties for disabling auto-detection and auto-provisioning will impact the daemon toolchain resolution logic.
