...
To use the repository in another build, add it to <<plugins_intermediate.adoc#sec:custom_plugin_repositories,the repositories of the `pluginManagement {}` block>> in your `settings.gradle(.kts)` file.
...
If your plugin was published without using the <<java_gradle_plugin.adoc#java_gradle_plugin,Java Gradle Plugin Development Plugin>>, the publication will be lacking <<plugins_intermediate.adoc#sec:plugin_markers,Plugin Marker Artifact>>, which is needed for <<plugins_intermediate.adoc#sec:plugins_block,plugins DSL>> to locate the plugin.
