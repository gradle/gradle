...
This occurs because the `implementation` configuration forces the exact version you used onto the user's buildscript classpath, potentially overriding a newer or different version they require.

To prevent buildscript classpath leakage and maintain version flexibility for your users, you must use the `compileOnly` configuration for any dependency that represents an underlying plugin's API.

[source,text]
----
dependencies {
    // BAD: Can cause conflict by forcing this version on the user's classpath
...
If you need to *enforce* a specific version (e.g., for internal <<implementing_convention_plugins.adoc#implementing_convention_plugins,Convention Plugins>>), the `implementation` approach may be acceptable, but this should be done with clear awareness of the potential user conflicts.

WARNING: Using the `compileOnly` configuration will cause the standard plugin testing method, `GradleRunner.withPluginClasspath()`, to fail. See to learn more.
