...
|Initialization, configuration, and execution of the Gradle runtime, integration of built-in and third-party plugins with Gradle. This group is closed: plugins can report into its subgroups but cannot add subgroups. Gradle reports its own deprecations under the `Deprecation` subgroup; plugins cannot report into it. Consumers of reports and Tooling API events identify it, like any group, by its name path, `Gradle` then `Deprecation`.
