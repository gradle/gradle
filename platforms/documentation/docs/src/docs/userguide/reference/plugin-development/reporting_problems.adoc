...
It warns if no recipient is set and fails if the recipient is `fail`.
...
=== Getting a `ProblemReporter`

Get the link:{javaDocPath}/org/gradle/api/problems/ProblemReporter.html[`ProblemReporter`] using `problem.getReporter()`:
...
When reporting a problem, you can provide a wide variety of metadata such as descriptions, severity, solutions, and additional data using link:{javaDocPath}/org/gradle/api/problems/ProblemSpec.html[`ProblemSpec`]:
