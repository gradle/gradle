...
Group names must not be blank, must not have leading or trailing whitespace, must not contain control or format characters (other than the zero width joiner and non-joiner), must not contain `>` or `"`, and are limited to 50 characters.
Problem names follow the same rules with a limit of 2000 characters, and may contain `>` and `"`.
