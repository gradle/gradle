...
- link:{javadocPath}/org/gradle/api/problems/ProblemReporter.html#report(java.util.Collection)[`report()`] — use for recoverable problems when the build should continue.
- link:{javadocPath}/org/gradle/api/problems/ProblemReporter.html#throwing(java.lang.Throwable,java.util.Collection)[`throwing()`] — use for non-recoverable problems that should fail the build.
...
> Message from runtime exception
  ...
  This is a demonstration of how to add detailed information to a build failure
