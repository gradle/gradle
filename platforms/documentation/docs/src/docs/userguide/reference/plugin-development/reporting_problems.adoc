...
Gradle’s Problems API lets plugins report rich, structured information about issues that occur during a build.

This data powers Gradle’s console output, Build Scans, and IDE integrations so users see clear, actionable problem reports.
...
====
include::sample[dir="snippets/plugins/reportingProblems/kotlin",files="plugin/src/main/kotlin/org/example/PublishingGuardPlugin.kt[]"]
include::sample[dir="snippets/plugins/reportingProblems/groovy",files="plugin/src/main/groovy/org/example/PublishingGuardPlugin.groovy[]"]
====

To use the Problems API, you must:

1. Inject Problems — Gradle gives you a link:{javaDocPath}/org/gradle/api/problems/ProblemReporter.html[`ProblemReporter`] from link:{javaDocPath}/org/gradle/api/problems/Problems.html#getReporter()[`Problems.getReporter()`].
2. Create a link:{javaDocPath}/org/gradle/api/problems/ProblemId.html[`ProblemId`] — unique name + display name.
3. Call link:{javaDocPath}/org/gradle/api/problems/ProblemReporter.html#report(org.gradle.api.problems.Problem)[`report(...)`] for warnings; use link:{javaDocPath}/org/gradle/api/problems/ProblemReporter.html#throwing(java.lang.Throwable,org.gradle.api.problems.Problem)[`throwing(...)`] for fatal.

=== Adding the Problems service with `ProblemReporter`

Inject the `ProblemReporter` service into your plugin:

====
include::sample[dir="snippets/plugins/reportingProblems/kotlin",files="plugin/src/main/kotlin/org/example/PublishingGuardPlugin.kt[tags=problem_reporter]"]
include::sample[dir="snippets/plugins/reportingProblems/groovy",files="plugin/src/main/groovy/org/example/PublishingGuardPlugin.groovy[tags=problem_reporter]"]
====

The link:{javaDocPath}/org/gradle/api/problems/ProblemReporter.html[`ProblemReporter`] is a Gradle service; add an abstract accessor and Gradle will inject it.

=== Creating a `ProblemId`

A `ProblemId` is the identity of the problem you’re reporting.

====
include::sample[dir="snippets/plugins/reportingProblems/kotlin",files="plugin/src/main/kotlin/org/example/PublishingGuardPlugin.kt[tags=problem_id]"]
include::sample[dir="snippets/plugins/reportingProblems/groovy",files="plugin/src/main/groovy/org/example/PublishingGuardPlugin.groovy[tags=problem_id]"]
====

The `Id` is not required but helps:

- *Group* related problem instances together
- *Deduplicate messages* (avoid spamming the same warning 100 times)
- *Link* back to docs or solutions
- Surface in Build Scans, IDEs, and the HTML report in a structured way

=== Reporting a problem with `ProblemSpec`
...
====
include::sample[dir="snippets/plugins/reportingProblems/kotlin",files="plugin/src/main/kotlin/org/example/PublishingGuardPlugin.kt[tags=problem_spec]"]
include::sample[dir="snippets/plugins/reportingProblems/groovy",files="plugin/src/main/groovy/org/example/PublishingGuardPlugin.groovy[tags=problem_spec]"]
====

See link:{javaDocPath}/org/gradle/api/problems/ProblemSpec.html[`ProblemSpec`] for all available fields.

=== Choosing a reporting mode

Plugins can report problems in two ways:

- link:{javadocPath}/org/gradle/api/problems/ProblemReporter.html#reporting-org.gradle.api.Action-[`reporting()`] — use for recoverable problems when the build should continue.
...
For details, see the link:{javaDocPath}/org/gradle/api/problems/ProblemReporter.html[`ProblemReporter`] documentation.

=== Reporting a recoverable problem

Use `problems.reporting {}` to signal something went wrong but allow the build to continue:

====
include::sample[dir="snippets/plugins/reportingProblems/kotlin",files="plugin/src/main/kotlin/org/example/PublishingGuardPlugin.kt[tags=problem_report]"]
include::sample[dir="snippets/plugins/reportingProblems/groovy",files="plugin/src/main/groovy/org/example/PublishingGuardPlugin.groovy[tags=problem_report]"]
====

=== Reporting a fatal problem

Use `problems.throwing {}` to fail the build with a structured error:

====
include::sample[dir="snippets/plugins/reportingProblems/kotlin",files="plugin/src/main/kotlin/org/example/PublishingGuardPlugin.kt[tags=problem_throw]"]
include::sample[dir="snippets/plugins/reportingProblems/groovy",files="plugin/src/main/groovy/org/example/PublishingGuardPlugin.groovy[tags=problem_throw]"]
====
