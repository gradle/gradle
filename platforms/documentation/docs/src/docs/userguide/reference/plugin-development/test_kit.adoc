...
[source,groovy]
----
def result = runner.withArguments('myTask', '--configuration-cache').build()
assert result.configurationCacheOutcome == ConfigurationCacheOutcome.STORED

result = runner.withArguments('myTask', '--configuration-cache').build()
assert result.configurationCacheOutcome == ConfigurationCacheOutcome.REUSED
----

See link:{javadocPath}/org/gradle/testkit/runner/ConfigurationCacheOutcome.html[`ConfigurationCacheOutcome`] for all possible outcomes, including link:{javadocPath}/org/gradle/testkit/runner/ConfigurationCacheOutcome.html#DISCARDED[`DISCARDED`], which indicates that the entry was not kept, e.g. because of Configuration Cache problems.

Inspecting the outcome requires the build under test to run with Gradle 9.8 or later; for older versions (via link:{javadocPath}/org/gradle/testkit/runner/GradleRunner.html#withGradleVersion-java.lang.String-[`GradleRunner.withGradleVersion(java.lang.String)`]) accessing it throws link:{javadocPath}/org/gradle/testkit/runner/UnsupportedFeatureException.html[`UnsupportedFeatureException`].
