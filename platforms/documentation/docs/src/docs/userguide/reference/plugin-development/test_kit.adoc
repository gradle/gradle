...
A debugger on its own is fine: the JDWP agent is not treated as a third-party agent, so a debugging session takes the same instrumentation path as a regular build.
...
With <<configuration_cache.adoc#config_cache,Configuration Cache>> or <<isolated_projects.adoc#isolated_projects,Isolated Projects>> enabled in the build under test, that combination is reported as a Configuration Cache problem:
