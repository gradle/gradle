...
[[sub:test-kit-debug-config-cache]]
=== Debugging with the Configuration Cache or Isolated Projects

In debug mode the build runs in the test process, where Gradle's instrumentation agent is not installed.
The agent is applied through a JVM command-line switch, so it is available only in a process Gradle starts itself.

Attaching a debugger alone is supported.
The JDWP agent is not treated as a third-party agent, so a debugging session takes the same instrumentation path as a regular build.
The combination that is not supported is debug mode together with _another_ Java agent on the test JVM: a coverage agent such as JaCoCo, or the agent an IDE attaches when it runs tests.
With <<configuration_cache.adoc#config_cache,Configuration Cache>> or <<isolated_projects.adoc#isolated_projects,Isolated Projects>> enabled in the build under test, Gradle reports that combination as a Configuration Cache problem when the plugin under test is applied through TestKit's injected classpath:

[source,text]
----
third-party Java agents with inactive Gradle's instrumentation agent are not supported by the configuration cache.
----

When the plugin under test is resolved from a repository instead, the same combination is unsafe but is not currently detected.

To debug such a build, keep it in its own process and attach to that process instead:
...
The build then takes the same instrumentation path as a regular build.
Support for the instrumentation agent in debug mode is tracked in link:https://github.com/gradle/gradle/issues/23817[gradle/gradle#23817].
