...
[[sub:test-kit-configuration-cache]]
== Testing with the Configuration Cache

To verify that your plugin is compatible with the <<configuration_cache.adoc#config_cache,Configuration Cache>>, enable it in your tests by passing the `--configuration-cache` argument to link:{javadocPath}/org/gradle/testkit/runner/GradleRunner.html[`GradleRunner`] or use one of the other methods described in <<configuration_cache_enabling.adoc#config_cache:usage:enable,Enabling the Configuration Cache>>.

The outcome of configuration caching is available via link:{javadocPath}/org/gradle/testkit/runner/BuildResult.html#getConfigurationCacheOutcome--[`BuildResult.getConfigurationCacheOutcome()`].
For example, running the same build twice lets you assert that the cache entry was stored and then reused:

====
include::sample[dir="snippets/reference/plugin-development/testKitFunctionalTestConfigurationCache/kotlin",files="src/test/kotlin/org/gradle/sample/BuildLogicFunctionalTest.kt[tags=functional-test-configuration-cache-outcome]"]
include::sample[dir="snippets/reference/plugin-development/testKitFunctionalTestConfigurationCache/groovy",files="src/test/groovy/org/gradle/sample/BuildLogicFunctionalTest.groovy[tags=functional-test-configuration-cache-outcome]"]
====
<1> First run primes the Configuration Cache.
<2> Second run reuses the Configuration Cache.

See link:{javadocPath}/org/gradle/testkit/runner/ConfigurationCacheOutcome.html[`ConfigurationCacheOutcome`] for all possible outcomes, including link:{javadocPath}/org/gradle/testkit/runner/ConfigurationCacheOutcome.html#STORE_FAILED[`STORE_FAILED`], which indicates that storing an entry failed, for example because of Configuration Cache problems, and link:{javadocPath}/org/gradle/testkit/runner/ConfigurationCacheOutcome.html#STORE_SKIPPED[`STORE_SKIPPED`], which indicates that storing was deliberately skipped, for example because an incompatible task was scheduled.

Inspecting the outcome requires the build under test to run with Gradle 9.8 or later.
For older Gradle versions selected via link:{javadocPath}/org/gradle/testkit/runner/GradleRunner.html#withGradleVersion-java.lang.String-[`GradleRunner.withGradleVersion(java.lang.String)`], calling `getConfigurationCacheOutcome()` throws link:{javadocPath}/org/gradle/testkit/runner/UnsupportedFeatureException.html[`UnsupportedFeatureException`].
