...
Gradle associates the value of an output property with its task when the value is returned from a getter that Gradle can override, on a property that has no setter.
A final getter, such as a Kotlin `val`, cannot be overridden, and a property with a setter may be replaced at any time, so their values stay unassociated.
...
In Groovy, mark the field `final`, which gives it a non-final getter and no setter:

[source,groovy]
----
@OutputFile
final Provider<RegularFile> outputFile = outputDir.map { it.file("result.txt") }
----

In Java, make sure the getter is not `final` and that there is no setter for the property.
