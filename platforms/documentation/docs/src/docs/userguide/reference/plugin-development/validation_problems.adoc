...
Remove the setter and make the property final:
...
Or let Gradle inject the property using an abstract getter, which is the recommended shape:
...
----
someTask {
    inputFile.from(jar.archiveFile)   // Provider<RegularFile> — carries the dependency
}
----
