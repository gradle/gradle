...
| *`my-composite`*
| *Composite Root*
| The entry point. It doesn't contain code but "knits" other builds together via `settings.gradle(.kts)`.

| *`my-app`*
| *Consumer Build*
| A standalone build that produces an executable. It declares dependencies on the libraries produced by `my-utils`.

| *`my-utils`*
| *Producer Build*
| A standalone multi-project build that provides two distinct libraries: `number-utils` and `string-utils`.
|===
...
[source,bash]
----
$ ./gradlew checkAll -PpropertiesMessage="Hello from the root build -P CLI flag" -DsystemMessage="Hello from the root build -D CLI flag"
----
...
[source,text]
----
include::{samplesPath}/build-organization/composite-builds/basic/tests/inheritance.out[]
----
...
====
include::sample[dir="samples/build-organization/composite-builds/basic/kotlin/my-app",files="settings.gradle.kts[tags=mag]"]
include::sample[dir="samples/build-organization/composite-builds/basic/groovy/my-app",files="settings.gradle[tags=mag]"]
====

Then the plugin is used as follows:

====
include::sample[dir="samples/build-organization/composite-builds/basic/kotlin",files="my-app/app/build.gradle.kts[tags=plugin_dependencies]"]
include::sample[dir="samples/build-organization/composite-builds/basic/groovy",files="my-app/app/build.gradle[tags=plugin_dependencies]"]
====
