...
They also allow you to decompose a large multi-project build into smaller, more isolated chunks that can be worked on independently or together as needed.
...
To execute the `run` task in the `my-app` build from `my-composite`:
...
NOTE: While the `--include-build` CLI flag can work for plugins applied via the `buildscript {}` block, it may not work reliably for plugins applied using the modern `plugins {}` block, especially for unpublished plugins. For consistent behavior, use `includeBuild` in your settings file. For settings plugins specifically (plugins applied in the settings file itself), use `includeBuild` within the `pluginManagement {}` block.
