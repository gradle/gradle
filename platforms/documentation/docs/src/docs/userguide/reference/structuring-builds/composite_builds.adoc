...
Composite builds allow you to decompose a large multi-project build into smaller, more isolated chunks that can be worked on independently or together as needed.
They also allow you to combine builds that are usually developed independently, for instance, when trying out a bug fix in a library that your application uses.

Included builds do not share any configuration with other included builds.

IMPORTANT: Each included build is configured and executed in isolation.
...
| *`my-composite`*
| *Composite Root*
| The entry point. It doesn't contain code but "knits" other builds together via `settings.gradle`.
