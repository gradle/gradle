...
Using Configuration-on-Demand with composite builds can significantly impact performance.
The behavior changes depending on whether you rely on default substitutions or explicitly declared substitutions.

1. The "All-or-Nothing" Rule for Substitutions When you explicitly declare a substitution rule for an included build, Gradle disables its automatic discovery logic for that specific build.
** The Result: You must manually declare all substitutions for every project within that included build that you wish to use. Any project not explicitly mapped will be treated as an external dependency.
2. Performance Implications with Configuration-on-Demand The way Gradle discovers what an included build "provides" dictates how much of that build must be configured at startup:
+
...
| **Explicit Rules** (Using `substitute...using`)
