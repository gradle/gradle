...
Gradle @version@ introduces [improved error messages](#error) for version conflicts and enhancements to the [Configuration Cache](#configuration-cache), including a new read-only mode, finer grained tracking of `-P` command-line properties and improved handling of keystore types.
...
Previously, any changes to `-P` command-line properties would invalidate the configuration cache, regardless of whether the changed properties were actually used at configuration-time or not.
...
Since the `value` property is never mentioned at configuration time, this release is able to reuse the configuration cache for faster builds:
...
In addition, the Configuration Cache report will also include properties used at configuration-time in the _Build configuration inputs_ tab.
