...
Gradle provides APIs for testing build logic and plugins with [TestKit](userguide/test_kit.html), and a dedicated [`Test` task](userguide/java_testing.html) for testing JVM code.

#### Testing refactoring

The [HTML report](java_testing.html#test_reporting) generated by the `test` task, `TestReport`, and other `AbstractTestTask` users now provides a more detailed view of the test results.
...
For JUnit 4 and JUnit Jupiter, nested test classes are now shown nested under their enclosing class in the HTML report.
...
You can switch between roots in the aggregate HTML report:
...
A new [`BuildResult.getOutputReader()`](javadoc/org/gradle/testkit/runner/BuildResult.html#getOutput()) method is available, returning a `BufferedReader` for streaming the build output incrementally.
