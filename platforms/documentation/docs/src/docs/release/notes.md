...
The aggregate report generated by the [Test Report Aggregation Plugin](userguide/test_report_aggregation_plugin.html) or manually with `TestReport` now also supports overlapping test structures,
e.g. if there is a suite with the same name in multiple subprojects. Each individual report source is now represented as a "root", allowing you to switch between them in the aggregate HTML report:
![new-aggregate-report.png](release-notes-assets/new-aggregate-report.png)

This change does mean that not all children are listed on the first tab now, you must select the appropriate root tab to see the tests contained in that source.
