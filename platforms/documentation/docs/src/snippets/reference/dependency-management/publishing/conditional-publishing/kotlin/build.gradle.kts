...
tasks.withType<PublishToMavenRepository>().configureEach {
    val predicate = repositoryName.map { repoName ->
        (repoName == "external" && publication == publishing.publications["binary"]) ||
        (repoName == "internal" && publication == publishing.publications["binaryAndSources"])
    }
...
tasks.register("publishToExternalRepository") {
    ...
    dependsOn(tasks.withType<PublishToMavenRepository>().matching {
        it.repositoryName.get() == "external"
    })
}
// end::shorthand-tasks[]
...
tasks.register("publishForDevelopment") {
    ...
    dependsOn(tasks.withType<PublishToMavenRepository>().matching {
        it.repositoryName.get() == "internal"
    })
