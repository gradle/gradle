...
The report aggregation logic does not automatically inspect all subprojects for coverage data to aggregate.  Instead, the direct and transitive <<declaring_dependencies_basics.adoc#sec:project-dependencies,project dependencies>> of the `jacocoAggregation` configuration are selected for potential aggregation.
