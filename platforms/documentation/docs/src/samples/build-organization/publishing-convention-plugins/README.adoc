...
In this sample, convention plugins are implemented as link:{userManualPath}/plugins.html#sec:precompile_script_plugin[precompiled script plugins] -
...
Convention plugins are regular Gradle plugins - thus they can be link:{userManualPath}/publishing_gradle_plugins.html#sec:publishing_your_plugin[published to an external repository like any other Gradle plugin].
...
SpotBugs is an external plugin - external plugins link:{userManualPath}/implementing_gradle_plugins_precompiled.html#sec:applying_external_plugins[need to be added as implementation dependencies] before they can be applied in a convention plugin:
====
