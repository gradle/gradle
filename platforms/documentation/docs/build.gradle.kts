...
tasks.withType<AsciidoctorTask>().configureEach {
    if (name == "userguideSinglePagePdf") {
        ...
        asciidoctorj.docExtensions(
            ...
            project.dependencies.create(project.files("src/main/resources"))
        )
    }
}
...
tasks.withType<Sync>().named("stageDocs") {
...
samples {
    ...
    val topLevelDirs = File("src/snippets").listFiles(directoriesOnly)
    val snippetDirs = topLevelDirs?.flatMap { it.listFiles(directoriesOnly)?.toList() ?: emptyList() }
        ?.filter {
            File(it, "kotlin").exists() || File(it, "groovy").exists()
        }
...
tasks.withType<Test>().named("docsTest") {
    ...
    if (project.configurationCacheEnabledForDocsTests) {
        ...
        filter {
            ...
            val testsForUnsupportedFeatures = arrayOf(
            ...
            val testsWithThirdPartyFailures = arrayOf(
            ...
            val testsForNotYetSupportedFeatures = arrayOf(
            ...
            val testsToBeFixedForConfigurationCache = arrayOf(
...
@Inject constructor(private val project: Project, private val testTask: Test) : CommandLineArgumentProvider {
    ...
    override fun asArguments(): Iterable<String> {
        val distributionName = testTaskClasspath.single { it.name.startsWith("gradle-runtime-api-info") }.parentFile.parentFile.parentFile.name
        return listOf(
