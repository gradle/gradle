...
description: "Migrating a Gradle 6.x build to Gradle 7.0, covering deprecations, removals and behaviour changes."
---
...
For migrating from Gradle 5.x, see the <Xref page="upgrading-version-5" section="Upgrading from Gradle 5.x to 6.0">older migration guide</Xref> first.
...
3. Run `gradle :wrapper --gradle-version 7.0.2` to update the project to 7.0.2.
...
Head over to <Xref page="dependency-locking" section="Migrating your legacy lockfile">the documentation</Xref> to learn how to migrate to the new format.
...
Users can still opt in to have this unique identifier part of the produced metadata if they want to, see <Xref page="publishing-gradle-module-metadata" section="Gradle Module Metadata reproducibility">the documentation</Xref>.
...
We recommend that you consider <Xref page="declaring-repositories" section="3. Declaring repositories">using</Xref> `mavenCentral()`, `google()` or a private `maven` repository instead.
...
- Kotlin has been updated to [Kotlin 1.4.31](https://kotlinlang.org/docs/reference/whatsnew1430.html).
- Groovy has been updated to [Groovy 3.0.7](https://groovy-lang.org/releasenotes/groovy-3.0.html).
...
If you encounter syntax errors, check the [Groovy issue tracker](https://groovy.apache.org/#reporting-issues) and [Groovy 3 release highlights](https://blogs.apache.org/groovy/entry/groovy-3-highlights).

[Some very specific regressions](https://issues.apache.org/jira/browse/GROOVY-9936) have already been fixed in the next minor version of Groovy.
...
Please refer to [#sec:configuration_removal](#sec:configuration_removal) for more details.
...
- PMD has been updated to [PMD 6.31.0](https://github.com/pmd/pmd/releases/tag/pmd_releases%2F6.31.0).
- Groovy and GroovyDoc have been updated to [Groovy 3.0.7](https://groovy-lang.org/releasenotes/groovy-3.0.html).
...
You can find more details about the benefits of the new configurations and which one to use in place of `compile` and `runtime` by reading the <Xref page="java-library-plugin" section="The Java Library Plugin">Java Library plugin</Xref> documentation.
...
Tests that use the <Xref page="test-kit" section="Testing Build Logic with TestKit">TestKit</Xref> API used to create temporary files under the system temporary directory as defined by `java.io.tmpdir`.
...
To avoid problems with these file locks, `<Xref page="test-kit" section="Testing Build Logic with TestKit">TestKit</Xref>` disables file system watching for builds executed on Windows via `GradleRunner`.
...
Please refer to the <Xref page="publishing-maven" section="The Maven Publish Plugin">documentation of the Maven Publish plugin</Xref> for more details.
...
Please refer to the <Xref page="publishing-maven" section="The Maven Publish Plugin">documentation of the Maven Publish plugin</Xref> for publishing on Maven repositories.
Please refer to the <Xref page="publishing-ivy" section="The Ivy Publish Plugin">documentation of the Ivy Publish plugin</Xref> for publishing on Ivy repositories.
...
Have a look at <Xref page="dependency-versions" section="Understanding version declaration">the documentation</Xref> for all the rules Gradle applies.
...
Please use the stable <Xref page="java-library-plugin" section="The Java Library Plugin">Java Library</Xref> and <Xref page="scala-plugin" section="The Scala Plugin">Scala</Xref> plugins instead.
...
The `layout` method taking a configuration block has been removed and is replaced by <GroovyDocLink class="org.gradle.api.artifacts.repositories.IvyArtifactRepository" method="patternLayout(org.gradle.api.Action)">patternLayout</GroovyDocLink>.
...
To fix this error, <Xref page="settings-file-basics" section="Settings script">create a `settings.gradle(.kts)` file</Xref> for the build.
...
You can use <JavaDocLink class="org.gradle.api.DefaultTask" /> instead.
...
Please use <JavaDocLink class="org.gradle.BuildListener" method="beforeSettings-org.gradle.api.initialization.Settings-">BuildListener.beforeSettings(Settings)</JavaDocLink> instead.
...
You can find more information about <Xref page="multi-project-builds" section="Multi-Project Builds">how to structure a Gradle build</Xref> in the user manual.
Alternatively, you can still run tasks in builds like this by invoking the build from the `master` directory only using a
<Xref page="multi-project-builds-intermediate" section="Executing tasks by fully qualified name">fully qualified path to the task</Xref>.

#### `modularity.inferModulePath` defaults to 'true'

<Xref page="java-library-plugin" section="Building Modules for the Java Module System">Compiling</Xref>,
<Xref page="java-testing" section="Testing Java Modules">testing</Xref> and
<Xref page="application-plugin" section="Building applications using the Java Module System">executing</Xref>
...
The `ValidateTaskProperties` task has been removed and replaced by the <JavaDocLink class="org.gradle.plugin.devel.tasks.ValidatePlugins" /> task.

#### Removal of `ImmutableFileCollection`

The `ImmutableFileCollection` type has been removed.
Use the <JavaDocLink class="org.gradle.api.file.ProjectLayout" method="files-java.lang.Object&#8230;&#8203;-">factory method</JavaDocLink> instead.
A handle to the project layout can be obtained via <JavaDocLink class="org.gradle.api.Project" method="getLayout--">Project.layout</JavaDocLink>.
...
The local build cache configuration now needs to be done via <JavaDocLink class="org.gradle.caching.configuration.BuildCacheConfiguration" method="local-org.gradle.api.Action-">BuildCacheConfiguration.local()</JavaDocLink>.

#### Removal of DefaultVersionSelectorScheme constructor

This internal API was used in plugins, amongst other the [Nebula plugins](https://github.com/nebula-plugins), and was deprecated in the Gradle 5.x timeline and is now removed.
...
See [this issue](https://github.com/gradle/gradle/issues/16706) for more context.
...
A fix for this problem is to add a dependency from the consumer to the producer.
...
Please look at <JavaDocLink class="org.gradle.api.file.CopySpec" method="setDuplicatesStrategy-org.gradle.api.file.DuplicatesStrategy-">the CopySpec docs</JavaDocLink> for details.
...
- JaCoCo has been updated to [0.8.6](https://www.jacoco.org/jacoco/trunk/doc/changes.html).
- Checkstyle has been updated to [Checkstyle 8.37](https://checkstyle.sourceforge.io/releasenotes.html#Release_8.37).
- CodeNarc has been updated to [CodeNarc 2.0.0](https://github.com/CodeNarc/CodeNarc/blob/v2.0.0/CHANGELOG.md).

#### Updates to bundled Gradle dependencies

- Kotlin has been updated to [Kotlin 1.4.20](https://blog.jetbrains.com/kotlin/2020/08/kotlin-1-4-released-with-a-focus-on-quality-and-performance/).
...
If, for some reason, you still want to publish components with dependencies on enforced platforms, you can disable the validation following the <Xref page="publishing-setup" section="Suppressing validation errors">documentation</Xref>.
...
See the <Xref page="working-with-files" section="Using `FileTree`">user manual</Xref> for more information.
...
As a consequence, you are only allowed to change Gradle’s default excludes in the settings script, see the <Xref page="working-with-files" section="Using `FileTree`">user manual</Xref> for an example.
...
You are only allowed to change Gradle’s default excludes in the settings script, see the <Xref page="working-with-files" section="Using `FileTree`">user manual</Xref> for an example.
...
- Ant has been updated to [1.10.8](https://archive.apache.org/dist/ant/RELEASE-NOTES-1.10.8.html).
...
While adding support for expressing <Xref page="resolution-rules" section="Substituting a dependency with another variant">variant support</Xref> in dependency substitutions, a bug fix introduced a behaviour change that some builds may rely upon.
Previously a substituted dependency would still use the <Xref page="variant-attributes" section="Variants and Attributes">attributes</Xref> of the original selector instead of the ones from the replacement selector.
...
- _and_ you use <Xref page="resolution-rules" section="Using Resolution Rules">resolution rules</Xref> on these dependencies.

See the <Xref page="resolution-rules" section="Substituting a dependency with another variant">documentation</Xref> for resolving issues if you are impacted.
...
With Gradle 6.4, the incubating API for <Xref page="dependency-locking" section="Fine-tuning dependency locking behaviour with lock mode">dependency locking `LockMode`</Xref> has changed.
...
Before Gradle introduced <Xref page="java-plugin" section="Incremental annotation processing">incremental annotation processing support</Xref>, IDEA required all annotation processors to be on the compilation classpath to be able to run annotation processing when compiling in IDEA.
This is no longer necessary because Gradle has a separate <Xref page="java-plugin" section="Dependency management">annotation processor classpath</Xref>.
...
- PMD has been updated to [PMD 6.21.0](https://pmd.github.io/2020/01/24/PMD-6.21.0/#24-january-2020---6210).
...
Gradle 6.3 does not support the <Xref page="command-line-interface" section="Rich console">rich console</Xref> for 32-bit Unix systems and for old FreeBSD versions (older than FreeBSD 10). Microsoft Windows 32-bit is unaffected.
...
These configurations are no longer used in modern Gradle builds that use <Xref page="variant-aware-resolution" section="Variant Selection and Attribute Matching">variant aware dependency management</Xref> and the <Xref page="publishing-setup" section="Publishing a project as module">new publishing plugins</Xref>.
...
For declaring dependencies, please use the configurations provided by the plugins you use, for example by the <Xref page="java-library-plugin" section="Dependency configurations">Java Library plugin</Xref>.
...
The practical effect is that now all <Xref page="java-platform-plugin" section="Consuming platforms">platform dependencies</Xref> have to be declared as such.
...
If you are still using one of these old plugins (`java-lang`, `scala-lang`, `jvm-component`, `jvm-resources`, `junit-test-suite`) please consult the documentation on <Xref page="building-java-projects" section="Building Java & JVM projects">Building Java & JVM projects</Xref> to determine which of the stable JVM plugins are appropriate for your project.
...
- PMD has been updated to [PMD 6.20.0](https://pmd.github.io/2019/11/29/PMD-6.20.0/#29-november-2019---6200).
