...
description: "Migrating a Gradle 5.x build to Gradle 6.0, covering deprecations, removals and behaviour changes."
---
...
For migrating from Gradle 4.x, see the <Xref page="upgrading-version-4" section="Upgrading from Gradle 4.x to 5.0">4.x to 5.0 guide</Xref> first.
...
3. Run `gradle :wrapper --gradle-version 6.0.1` to update the project to 6.0.1.
...
See <Xref page="java-library-plugin" section="Dependency configurations">the relationship of these configurations</Xref>.
...
Users should migrate to the <Xref page="publishing-setup" section="Publishing a project as module">publishing system</Xref> of Gradle by using either the <Xref page="publishing-maven" section="The Maven Publish Plugin">`maven-publish`</Xref> or <Xref page="publishing-ivy" section="The Ivy Publish Plugin">`ivy-publish`</Xref> plugins.
These plugins have been stable since Gradle 4.8.

The publishing system is also the only way to ensure the publication of <Xref page="publishing-gradle-module-metadata" section="Gradle Module Metadata">Gradle Module Metadata</Xref>.
...
In Gradle 5.4 we introduced a new API for implementing <Xref page="custom-tasks" section="Incremental tasks">incremental tasks</Xref>: <GroovyDocLink class="org.gradle.work.InputChanges" />. The old API based on `IncrementalTaskInputs` has been deprecated.

#### Forced dependencies

Forcing dependency versions using `force = true` on a first-level dependency has been deprecated.

Force has both a semantic and ordering issue which can be avoided by using a <Xref page="dependency-versions" section="Declaring rich versions">strict version constraint</Xref>.
...
This can cause ["grossly invalid zip files" that can trigger zip bomb detection](https://github.com/gradle/gradle/issues/9990).
...
Please see [https://gradle.com/help/gradle-6-build-scan-plugin](https://gradle.com/help/gradle-6-build-scan-plugin) for more information.

#### Updates to bundled Gradle dependencies

- Groovy has been updated to [Groovy 2.5.8](http://groovy-lang.org/changelogs/changelog-2.5.8.html).
- Kotlin has been updated to [Kotlin 1.3.50](https://blog.jetbrains.com/kotlin/2019/08/kotlin-1-3-50-released/).
- Ant has been updated to [Ant 1.10.7](https://archive.apache.org/dist/ant/RELEASE-NOTES-1.10.7.html).

#### Updates to default integration versions

- Checkstyle has been updated to [Checkstyle 8.24](https://checkstyle.org/releasenotes.html#Release_8.24).
- CodeNarc has been updated to [CodeNarc 1.4](https://github.com/CodeNarc/CodeNarc/blob/master/CHANGELOG.md#version-14---may-2019).
- PMD has been updated to [PMD 6.17.0](https://pmd.github.io/2019/07/28/PMD-6.17.0/#28-july-2019---6170).
- JaCoCo has been updated to [0.8.5](https://www.jacoco.org/jacoco/trunk/doc/changes.html). Contributed by [Evgeny Mandrikov](https://github.com/Godin)
...
This means, if you are publishing libraries with Gradle and using the <Xref page="publishing-maven" section="The Maven Publish Plugin">maven-publish</Xref> or <Xref page="publishing-ivy" section="The Ivy Publish Plugin">ivy-publish</Xref> plugin, the Gradle Module Metadata file is always published **in addition** to traditional metadata.
...
These are documented in the [specification](https://github.com/gradle/gradle/blob/master/platforms/documentation/docs/src/docs/design/gradle-module-metadata-latest-specification.md) as well.
...
You can opt into the old behavior for selected repositories by adding the `artifact()` <Xref page="supported-metadata-formats" section="Supported metadata sources">metadata source</Xref>.
...
- The <JavaDocLink class="org.gradle.plugin.devel.tasks.ValidatePlugins" method="getFailOnWarning--">failOnWarning</JavaDocLink> option is now enabled by default.
- The following task validation errors now fail the build at runtime and are promoted to errors for <JavaDocLink class="org.gradle.plugin.devel.tasks.ValidatePlugins" />:
...
In Gradle 6.0, we introduced a new progress event (<JavaDocLink class="org.gradle.tooling.events.test.TestOutputEvent">org.gradle.tooling.events.test.TestOutputEvent</JavaDocLink>) to expose the output of test execution.
...
As an alternative, you can use the [SpotBugs plugin](https://plugins.gradle.org/plugin/com.github.spotbugs) from the [Gradle Plugin Portal](https://plugins.gradle.org/search?term=spotbugs).

#### The JDepend plugin has been removed

The deprecated JDepend plugin has been removed.
There are a number of community-provided plugins for code and architecture analysis available on the [Gradle Plugin Portal](https://plugins.gradle.org).

#### The OSGI plugin has been removed

The deprecated OSGI plugin has been removed.
There are a number of community-provided OSGI plugins available on the [Gradle Plugin Portal](https://plugins.gradle.org/search?term=osgi).
...
Please use a [Build Scan](https://scans.gradle.com/) for build analysis and comparison.

#### The Play plugins have been removed

The deprecated Play plugin has been removed.
An external replacement, the [Play Framework plugin](https://gradle.github.io/playframework), is available from the plugin portal.
...
Use <JavaDocLink class="org.gradle.process.CommandLineArgumentProvider">org.gradle.process.CommandLineArgumentProvider</JavaDocLink> instead.
- The deprecated class `org.gradle.api.ConventionProperty` has been removed.
Use <JavaDocLink class="org.gradle.api.provider.Provider">Providers</JavaDocLink> instead of convention properties.
...
- The following setters have been removed from `JacocoReportBase`:
  - <GroovyDocLink class="org.gradle.testing.jacoco.tasks.JacocoReport" method="executionData">executionData</GroovyDocLink> - use `getExecutionData().setFrom()` instead.
  - <GroovyDocLink class="org.gradle.testing.jacoco.tasks.JacocoReport" method="sourceDirectories">sourceDirectories</GroovyDocLink> - use `getSourceDirectories().setFrom()` instead.
  - <GroovyDocLink class="org.gradle.testing.jacoco.tasks.JacocoReport" method="classDirectories">classDirectories</GroovyDocLink> - use `getClassDirectories().setFrom()` instead.
  - <GroovyDocLink class="org.gradle.testing.jacoco.tasks.JacocoReport" method="additionalClassDirs">additionalClassDirs</GroovyDocLink> - use `getAdditionalClassDirs().setFrom()` instead.
  - <GroovyDocLink class="org.gradle.testing.jacoco.tasks.JacocoReport" method="additionalSourceDirs">additionalSourceDirs</GroovyDocLink> - use `getAdditionalSourceDirs().setFrom()` instead.
...
- File paths in <JavaDocLink class="org.gradle.plugins.ear.descriptor.DeploymentDescriptor" method="getFileName--">deployment descriptor file name</JavaDocLink> for the ear plugin are not allowed any more.
...
- When <Xref page="groovy-plugin" section="Incremental Groovy compilation">incremental Groovy compilation</Xref> is enabled, a wrong configuration of the source roots or enabling Java annotation for Groovy now fails the build.
...
Use the methods on the <Xref page="resolution-rules" section="4. Component Selection Rules">`ComponentSelection` parameter</Xref> instead.
- Declaring an <Xref page="custom-tasks" section="Incremental tasks">incremental task</Xref> without declaring outputs is now an error.
Declare file outputs or use <JavaDocLink class="org.gradle.api.tasks.TaskOutputs" method="upToDateWhen-groovy.lang.Closure-">TaskOutputs.upToDateWhen()</JavaDocLink> instead.
...
- There are slight changes in the incubating <Xref page="component-capabilities" section="Selecting between candidates">capabilities resolution</Xref> API, which has been introduced in 5.6, to also allow variant selection based on variant name
...
Declaring an <Xref page="custom-tasks" section="Incremental tasks">incremental task</Xref> without declaring outputs is now deprecated.
Declare file outputs or use <JavaDocLink class="org.gradle.api.tasks.TaskOutputs" method="upToDateWhen-groovy.lang.Closure-">TaskOutputs.upToDateWhen()</JavaDocLink> instead.
...
This fixes [an issue](https://github.com/gradle/gradle/issues/3698) where a worker needs to use a dependency that is also used by Gradle internally.
...
<Xref page="pmd-plugin" section="The PMD Plugin">The PMD plugin</Xref> has been upgraded to use [PMD version 6.15.0](https://pmd.github.io/2019/05/26/PMD-6.15.0/) instead of 6.8.0 by default.

Contributed by [wreulicke](https://github.com/wreulicke)
...
Instead, it provides the attributes for [Eclipse test sources](https://www.eclipse.org/eclipse/news/4.8/jdt.php#jdt-test-sources).
...
Please see the Kotlin [blog post](https://blog.jetbrains.com/kotlin/2019/06/kotlin-1-3-40-released/) and [changelog](https://github.com/JetBrains/kotlin/blob/1.3.40/ChangeLog.md) for more information about the included changes.
...
See <Xref page="component-capabilities" section="Capabilities">the capabilities section of the documentation</Xref> for more options.
...
The built-in Play plugin has been deprecated and will be replaced by a new [Play Framework plugin](https://gradle.github.io/playframework) available from the plugin portal.

#### Build Comparison

The _build comparison_ plugin has been deprecated and will be removed in the next major version of Gradle.

A [Build Scan](https://develocity.ai/product/build-scan/) shows much deeper insights into your build and you can use [Develocity](https://gradle.com/develocity) to directly compare two build’s build-scans.

### Potential breaking changes

#### User supplied Eclipse project names may be ignored on conflict

Project names configured via <JavaDocLink class="org.gradle.plugins.ide.eclipse.model.EclipseProject">`EclipseProject.setName(…​)`</JavaDocLink> were honored by Gradle and Buildship in all cases, even when the names caused conflicts and import/synchronization errors.
...
<Xref page="jacoco-plugin" section="The JaCoCo Plugin">The JaCoCo plugin</Xref> has been upgraded to use [JaCoCo version 0.8.4](https://www.jacoco.org/jacoco/trunk/doc/changes.html) instead of 0.8.3 by default.

Contributed by [Evgeny Mandrikov](https://github.com/Godin)

#### Embedded Ant version upgraded to 1.9.14

The version of Ant distributed with Gradle has been upgraded to [1.9.14](https://archive.apache.org/dist/ant/RELEASE-NOTES-1.9.14.html) from 1.9.13.
...
- There should not be setters for lazy properties like <JavaDocLink class="org.gradle.api.file.ConfigurableFileCollection" />.  Use `setFrom` instead. For example,
...
Please pass files and directories as arguments instead. See examples in the Worker API documentation.

#### Changes to native linking tasks

To expand our idiomatic <Xref page="lazy-configuration" section="Configuring Tasks Lazily">Provider API</Xref> practices, the install name property from `org.gradle.nativeplatform.tasks.LinkSharedLibrary` is affected by this change.
...
To expand our idiomatic <Xref page="lazy-configuration" section="Configuring Tasks Lazily">Provider API</Xref> practices, the `WindowsResourceCompile` task has been converted to use the Provider API.
...
The incubating `operatingSystems` property on native components has been replaced with the <JavaDocLink class="org.gradle.language.cpp.CppComponent" method="getTargetMachines()">targetMachines</JavaDocLink> property.

#### Changes for archive tasks (`Zip`, `Jar`, `War`, `Ear`, `Tar`)

##### Change in behavior for tasks extending `AbstractArchiveTask`

The `AbstractArchiveTask` has several new properties using the <Xref page="lazy-configuration" section="Provider Files API Reference">Provider API</Xref>.
