...
For migrating from Gradle 6.x, see the  [older migration guide](upgrading_version_6.mdx#upgrading_version_6) first.
...
See [user manual](toolchains.mdx#sec:configuring_toolchain_specifications) for more information.
...
See [user manual](toolchains.mdx#sub:download_repositories) for more information.
...
- migrate to the [JVM Test Suite Plugin](jvm_test_suite_plugin.mdx#jvm_test_suite_plugin)
...
Gradle uses runtime class decoration to implement abstract methods as long as the object is instantiated via `ObjectFactory` or some other automatic mechanism (like [managed properties](properties_providers.mdx#managed_properties)).
...
Prior to Gradle 8.0, the task dependencies for `[SourceSetOutput.classesDirs]({groovyDslPath}/org.gradle.api.tasks.SourceSetOutput.html#org.gradle.api.tasks.SourceSetOutput:classesDirs)`
included tasks that did not produce class files.
This meant that a task which depends on `classesDirs` would also depend on `classes`, `processResources`, and any other task dependency added to `[SourceSetOutput]({groovyDslPath}/org.gradle.api.tasks.SourceSetOutput.html)`.
...
See [Cannot use an input with an unknown implementation](validation_problems.mdx#implementation_unknown).
- Missing dependencies between tasks.
See [Implicit dependencies between tasks](validation_problems.mdx#implicit_dependency).
...
The `outputLocation` property of the [Report]({groovyDslPath}/org.gradle.api.reporting.Report.html#org.gradle.api.reporting.Report) now returns a value of type `Property&lt;? extends FileSystemLocation&gt;`.
...
You should [configure a Java Toolchain](kotlin_dsl.mdx#sec:kotlin-dsl_plugin) instead.
...
[destinationDirectory]({groovyDslPath}/org.gradle.api.tasks.bundling.AbstractArchiveTask.html#org.gradle.api.tasks.bundling.AbstractArchiveTask:destinationDirectory) of
[Jar]({groovyDslPath}/org.gradle.api.tasks.bundling.Jar.html), [War]({groovyDslPath}/org.gradle.api.tasks.bundling.War.html), and
[Ear]({groovyDslPath}/org.gradle.plugins.ear.Ear.html) tasks to the directory specified by
[BasePluginExtension#getLibsDirectory]({groovyDslPath}/org.gradle.api.plugins.BasePluginExtension.html#org.gradle.api.plugins.BasePluginExtension:libsDirectory).
...
The [`outgoingVariants` report](variant_aware_resolution.mdx#outgoing_variants_report) will warn about this for impacted configurations.
...
See the [Scala plugin documentation](scala_plugin.mdx#sec:scala_target) for details.
...
Documentation on configuring plugin publication can be found both [on the Portal](https://plugins.gradle.org/docs/publish-plugin) and [in the user manual](publishing_gradle_plugins.mdx#configure_the_plugin_publishing_plugin).

## Upgrading from 7.5 and earlier

### Updates to Attribute Disambiguation Rules related methods

The `[AttributeSchema.setAttributeDisambiguationPrecedence(List)]({javadocPath}/org/gradle/api/attributes/AttributesSchema.html#setAttributeDisambiguationPrecedence(List)--)` and `[AttributeSchema.getAttributeDisambiguationPrecedence()]({javadocPath}/org/gradle/api/attributes/AttributesSchema.html#getAttributeDisambiguationPrecedence()--)` methods now accept and return `List` instead of `Collection` to better indicate that the order of the elements in those collection is significant.
...
When configuring an executable explicitly for [`JavaCompile`]({groovyDslPath}/org.gradle.api.tasks.compile.ForkOptions.html#org.gradle.api.tasks.compile.ForkOptions:executable) or [`Test`]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html#org.gradle.api.tasks.testing.Test:executable) tasks, Gradle will now emit an error if this executable does not exist.
...
As part of the ongoing effort to evolve Test Suites, dependency declarations in the Test Suites `dependencies` block are [now strongly typed](jvm_test_suite_plugin.mdx#sec:differences_with_top_level_dependencies).
...
For more information, see the updated [declare an additional test suite](jvm_test_suite_plugin.mdx#sec:declare_an_additional_test_suite) example in the JVM Test Suite Plugin section of the user guide.

### Deprecations

#### Usage of invalid Java toolchain specifications is now deprecated

Along with the Java language version, the [Java toolchain](toolchains.mdx#toolchains) DSL allows configuring other criteria such as specific vendors or VM implementations.
...
See more details about toolchain configuration in the [user manual](toolchains.mdx#sec:configuring_toolchain_specifications).
...
Please refer to the [userguide section about implementing incremental tasks for more details](custom_tasks.mdx#sec:implementing_an_incremental_task).
...
This means that notations, which resolve to multiple files (e.g. the [Project.files(java.lang.Object&#8230;&#8203;)]({groovyDslPath}/org.gradle.api.Project.html#org.gradle.api.Project:files(java.lang.Object[])) method, when more then one file is passed) will result in a build failure.
...
The [Checkstyle plugin](checkstyle_plugin.mdx#checkstyle_plugin) now uses the Gradle worker API to run Checkstyle as an external worker process.
...
You can [increase the maximum memory for the Checkstyle process](checkstyle_plugin.mdx#sec:checkstyle_customize_memory) by setting the `maxHeapSize` for the Checkstyle task.
...
Gradle 7.5 consistently sets the current working directory for the Checkstyle task to `$[GRADLE_USER_HOME](directory_layout.mdx#dir:gradle_user_home)/workers`.
...
In version 7.5 and above, Gradle consistently sets the current working directory for the Checkstyle task to `$[GRADLE_USER_HOME](directory_layout.mdx#dir:gradle_user_home)/workers`.
...
Consequently, using
`@[FileCollection.getAsPath()]({javadocPath}/org/gradle/api/file/FileCollection.html#getAsPath--)` for files with paths that contain a path separator has been deprecated, and it will be an error in Gradle 8.0 and later.
...
There is a new `[access]({groovyDslPath}/org.gradle.api.tasks.javadoc.Groovydoc.html#org.gradle.api.tasks.javadoc.Groovydoc:access)` property that allows finer control over what is included in the Groovydoc.
...
Gradle 7.5 introduces configuration cache-compatible ways to execute and obtain output of an external process with the [provider-based APIs]({groovyDslPath}/org.gradle.api.provider.ProviderFactory.html) or a custom implementation of the [`ValueSource`]({javadocPath}/org/gradle/api/provider/ValueSource.html) interface.
The [configuration cache chapter](configuration_cache_requirements.mdx#config_cache:requirements:external_processes) has more details to help with the migration to the new APIs.
...
Gradle 7.4+ will now emit a deprecation warning when the AdoptOpenJDK vendor is specified in the [toolchain specification](toolchains.mdx#sec:vendors) and it is used by auto provisioning.
...
When using `@[SkipWhenEmpty]({javadocPath}/org/gradle/api/tasks/SkipWhenEmpty.html)` on an input file collection, Gradle skips the task when it determines that the input is empty.
If the input file collection consists only of file trees, Gradle ignores directories for the emptiness check.
Though when checking for changes to the input file collection, Gradle only ignores directories when the `@[IgnoreEmptyDirectories]({javadocPath}/org/gradle/api/tasks/IgnoreEmptyDirectories.html)` annotation is present.
...
Finally, using `@[InputDirectory]({javadocPath}/org/gradle/api/tasks/InputDirectory.html)` implies `@IgnoreEmptyDirectories`, so no changes are necessary when using this annotation.
The same is true for `[inputs.dir()]({javadocPath}/org/gradle/api/tasks/TaskInputs.html#dir-java.lang.Object-)` when registering an input directory via the runtime API.
...
The set of [Attribute]({javadocPath}/org/gradle/api/attributes/Attribute.html)s associated with a _consumable_ configuration within a project, must be unique across all other configurations within that project which share the same set of [Capability]({javadocPath}/org/gradle/api/capabilities/Capability.html)s.
...
[Provider#forUseAtConfigurationTime]({javadocPath}/org/gradle/api/provider/Provider.html#forUseAtConfigurationTime--) is now deprecated and scheduled for removal in Gradle 9.0.0.
Clients should simply remove the call.

The call was mandatory on providers of external values such as [system properties]({groovyDslPath}/org.gradle.api.provider.ProviderFactory.html#org.gradle.api.provider.ProviderFactory:systemProperty(java.lang.String)), [environment variables]({groovyDslPath}/org.gradle.api.provider.ProviderFactory.html#org.gradle.api.provider.ProviderFactory:environmentVariable(java.lang.String)),
[Gradle properties]({groovyDslPath}/org.gradle.api.provider.ProviderFactory.html#org.gradle.api.provider.ProviderFactory:gradleProperty(java.lang.String)) and [file contents]({groovyDslPath}/org.gradle.api.provider.ProviderFactory.html#org.gradle.api.provider.ProviderFactory:fileContents(org.gradle.api.file.RegularFile)) meant to be used at configuration time together with the configuration cache feature.
...
Clients are also free to use standard Java APIs such as `System#getenv` to read environment variables, `System#getProperty` to read system properties as well as Gradle APIs such as [`Project#property(String)`]({groovyDslPath}/org.gradle.api.provider.ProviderFactory.html#org.gradle.api.provider.ProviderFactory:systemProperty(java.lang.String)) and [`Project#findProperty(String)`]({javadocPath}/org/gradle/api/Project.html#findProperty-java.lang.String-) to read Gradle properties at configuration time.
...
[`ConfigurableReport#setDestination(org.gradle.api.provider.Provider&lt;java.io.File&gt;)`]({javadocPath}/org/gradle/api/reporting/ConfigurableReport.html#setDestination-org.gradle.api.provider.Provider-) is now deprecated and scheduled for removal in Gradle 8.0.

Use [`Report#getOutputLocation().set(&#8230;&#8203;)`]({javadocPath}/org/gradle/api/reporting/Report.html#getOutputLocation--) instead.
...
- Interface [TaskExecutionListener]({javadocPath}/org/gradle/api/execution/TaskExecutionListener.html)
- Interface [TaskActionListener]({javadocPath}/org/gradle/api/execution/TaskActionListener.html)
- Method [TaskExecutionGraph.addTaskExecutionListener()]({javadocPath}/org/gradle/api/execution/TaskExecutionGraph.html#addTaskExecutionListener-org.gradle.api.execution.TaskExecutionListener-)
- Method [TaskExecutionGraph.removeTaskExecutionListener()]({javadocPath}/org/gradle/api/execution/TaskExecutionGraph.html#removeTaskExecutionListener-org.gradle.api.execution.TaskExecutionListener-)
- Method [TaskExecutionGraph.beforeTask()]({javadocPath}/org/gradle/api/execution/TaskExecutionGraph.html#beforeTask-org.gradle.api.Action-)
- Method [TaskExecutionGraph.afterTask()]({javadocPath}/org/gradle/api/execution/TaskExecutionGraph.html#afterTask-org.gradle.api.Action-)
- Registering TaskExecutionListener, TaskActionListener, TestListener, TestOutputListener via [Gradle.addListener()]({javadocPath}/org/gradle/api/invocation/Gradle.html#addListener-java.lang.Object-)

See the [configuration cache chapter](configuration_cache_requirements.mdx#config_cache:requirements:build_listeners) for details on how to migrate these usages to APIs that are supported by the configuration cache.
...
- Method [Gradle.buildFinished()]({javadocPath}/org/gradle/api/invocation/Gradle.html#buildFinished-org.gradle.api.Action-)
- Method [BuildListener.buildFinished()]({javadocPath}/org/gradle/BuildListener.html#buildFinished-org.gradle.BuildResult-)

See the [configuration cache chapter](configuration_cache_requirements.mdx#config_cache:requirements:build_listeners) for details on how to migrate these usages to APIs that are supported by the configuration cache.

#### Calling `Task.getProject()` from a task action

Calling [Task.getProject()]({javadocPath}/org/gradle/api/Task.html#getProject--) from a task action at execution time is now deprecated and will be made an error in Gradle 8.0.
This method can be used during configuration time, but it is recommended to avoid doing this.

See the [configuration cache chapter](configuration_cache_requirements.mdx#config_cache:requirements:use_project_during_execution) for details on how to migrate these usages to APIs that are supported by the configuration cache.

#### Calling `Task.getTaskDependencies()` from a task action

Calling [Task.getTaskDependencies()]({javadocPath}/org/gradle/api/Task.html#getTaskDependencies--) from a task action at execution time is now deprecated and will be made an error in Gradle 8.0.
This method can be used during configuration time, but it is recommended to avoid doing this.
...
Check the [Shared Build Services documentation](build_services.mdx#using_a_build_service_from_a_task) for more information.

#### VersionCatalog and VersionCatalogBuilder deprecations

Some methods in [VersionCatalog]({javadocPath}/org/gradle/api/artifacts/VersionCatalog.html) and [VersionCatalogBuilder]({javadocPath}/org/gradle/api/initialization/dsl/VersionCatalogBuilder.html) are now deprecated and scheduled for removal in Gradle 8.0.
...
You have to use the [version catalog extension](version_catalogs.mdx#sec:type-unsafe-access-to-catalog) instead.

#### Toolchain support in Scala

When using [toolchains in Scala](scala_plugin.mdx#sec:scala_tasks), the `-target` option of the Scala compiler will now be set automatically.
...
Use the @[UntrackedTask]({javadocPath}/org/gradle/api/tasks/UntrackedTask.html) annotation or the [Task.doNotTrackState()]({groovyDslPath}/org.gradle.api.Task.html#org.gradle.api.Task:doNotTrackState(java.lang.String)) method to declare a task as untracked.

When you are using a `[Copy]({groovyDslPath}/org.gradle.api.tasks.Copy.html)` task for copying single files into a directory which contains unreadable files, use the method [Task.doNotTrackState()]({groovyDslPath}/org.gradle.api.Task.html#org.gradle.api.Task:doNotTrackState(java.lang.String)).
...
Due to [CVE-2021-32751](https://github.com/gradle/gradle/security/advisories/GHSA-6j2p-252f-7mw8), `gradle`, `gradlew` and start scripts generated by Gradle&#8217;s [application plugin](application_plugin.mdx#application_plugin) have been updated to avoid situations where these
...
See [Validation problems](validation_problems.mdx#implementation_unknown) for more details how to fix the issue.
...
- JaCoCo has been updated to [0.8.7](http://www.jacoco.org/jacoco/trunk/doc/changes.html).
...
- `groovy`: [GroovySourceDirectorySet]({groovyDslPath}/org.gradle.api.tasks.GroovySourceDirectorySet.html)
- `antlr`: [AntlrSourceDirectorySet]({groovyDslPath}/org.gradle.api.plugins.antlr.AntlrSourceDirectorySet.html)
- `scala`: [ScalaSourceDirectorySet]({groovyDslPath}/org.gradle.api.tasks.ScalaSourceDirectorySet.html)
...
Convention mapping is an internal feature that is been replaced by the [Provider API](lazy_configuration.mdx#lazy_configuration).
...
[buildFile]({groovyDslPath}/org.gradle.api.tasks.GradleBuild.html#org.gradle.api.tasks.GradleBuild:buildFile)
property in [GradleBuild]({groovyDslPath}/org.gradle.api.tasks.GradleBuild.html) task has been deprecated.

Please use the [dir]({groovyDslPath}/org.gradle.api.tasks.GradleBuild.html#org.gradle.api.tasks.GradleBuild:dir)
property instead to specify the root of the nested build.

Setting custom build layout using
[StartParameter]({javadocPath}/org/gradle/StartParameter.html) methods
[setBuildFile(File)]({javadocPath}/org/gradle/StartParameter.html#setBuildFile-java.io.File-)
and
[setSettingsFile(File)]({javadocPath}/org/gradle/StartParameter.html#setSettingsFile-java.io.File-)
as well as the counterpart getters
[getBuildFile()]({javadocPath}/org/gradle/StartParameter.html#getBuildFile--)
and
[getSettingsFile()]({javadocPath}/org/gradle/StartParameter.html#getSettingsFile--)
have been deprecated.
...
For the use case where custom settings or build files are used to model different behavior (similar to Maven profiles), consider using [system properties](build_environment.mdx#sec:gradle_system_properties) with conditional logic.
...
[Dependency substitutions](resolution_rules.mdx#sec:dependency-substitution-rules) using `with` method have been deprecated and are replaced with `using` method that also allows chaining.
...
- The [main]({groovyDslPath}/org.gradle.api.tasks.JavaExec.html#org.gradle.api.tasks.JavaExec:main) getters and setters
in [JavaExec]({groovyDslPath}/org.gradle.api.tasks.JavaExec.html) task have been deprecated.
Use the [mainClass]({groovyDslPath}/org.gradle.api.tasks.JavaExec.html#org.gradle.api.tasks.JavaExec:mainClass) property instead.

#### Deprecated properties in `compile` task

- The [JavaCompile.destinationDir]({groovyDslPath}/org.gradle.api.tasks.compile.JavaCompile.html#org.gradle.api.tasks.compile.JavaCompile:destinationDir)
property has been deprecated.
Use the [JavaCompile.destinationDirectory]({groovyDslPath}/org.gradle.api.tasks.compile.JavaCompile.html#org.gradle.api.tasks.compile.JavaCompile:destinationDirectory)
property instead.
- The [GroovyCompile.destinationDir]({groovyDslPath}/org.gradle.api.tasks.compile.GroovyCompile.html#org.gradle.api.tasks.compile.GroovyCompile:destinationDir)
property has been deprecated.
Use the [GroovyCompile.destinationDirectory]({groovyDslPath}/org.gradle.api.tasks.compile.GroovyCompile.html#org.gradle.api.tasks.compile.GroovyCompile:destinationDirectory)
property instead.
- The [ScalaCompile.destinationDir]({groovyDslPath}/org.gradle.api.tasks.scala.ScalaCompile.html#org.gradle.api.tasks.scala.ScalaCompile:destinationDir)
property has been deprecated.
Use the [ScalaCompile.destinationDirectory]({groovyDslPath}/org.gradle.api.tasks.scala.ScalaCompile.html#org.gradle.api.tasks.scala.ScalaCompile:destinationDirectory)
property instead.
...
As a consequence, the [Settings.includeFlat()]({groovyDslPath}/org.gradle.api.initialization.Settings.html#org.gradle.api.initialization.Settings:includeFlat(java.lang.String&#91;&#93;)) method is deprecated in Gradle 7.1, 7.2, and 7.3 only.
...
Existing clients should migrate to the [`maven-publish` plugin](publishing_maven.mdx#publishing_maven).
...
- [org.gradle.api.Project.getConvention()]({javadocPath}/org/gradle/api/Project.html#getConvention--)
...
- `antlr`: [AntlrSourceDirectorySet]({javadocPath}/org/gradle/api/plugins/antlr/AntlrSourceDirectorySet.html)
...
Using the registration method [DependencyHandler.registerTransform(Action)]({groovyDslPath}/org.gradle.api.artifacts.dsl.DependencyHandler.html#org.gradle.api.artifacts.dsl.DependencyHandler:registerTransform(org.gradle.api.Action)) for `ArtifactTransform` has been deprecated.
Migrate your artifact transform to use `TransformAction` and use [DependencyHandler.registerTransform(Class, Action)]({groovyDslPath}/org.gradle.api.artifacts.dsl.DependencyHandler.html#org.gradle.api.artifacts.dsl.DependencyHandler:registerTransform(java.lang.Class,%20org.gradle.api.Action)) instead.
See the [user manual](artifact_transforms.mdx#sec:abm-artifact-transforms) for more information on implementing `TransformAction`.