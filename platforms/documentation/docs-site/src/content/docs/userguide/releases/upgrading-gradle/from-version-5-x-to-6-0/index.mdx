...
For migrating from Gradle 4.x, see the [4.x to 5.0 guide](upgrading_version_4.mdx#upgrading_version_4) first.
...
See [the relationship of these configurations](java_library_plugin.mdx#sec:java_library_configurations_graph).
...
Users should migrate to the [publishing system](publishing_setup.mdx#publishing_components) of Gradle by using either the [`maven-publish`](publishing_maven.mdx#publishing_maven) or [`ivy-publish`](publishing_ivy.mdx#publishing_ivy) plugins.
These plugins have been stable since Gradle 4.8.

The publishing system is also the only way to ensure the publication of [Gradle Module Metadata](publishing_gradle_module_metadata.mdx#sec:understanding-gradle-module-md).
...
If you own the code of the tasks in question, you can fix them by [following the suggestions](incremental_build.mdx#sec:task_input_validation).
...
In Gradle 5.4 we introduced a new API for implementing [incremental tasks](custom_tasks.mdx#incremental_tasks): [InputChanges]({groovyDslPath}/org.gradle.work.InputChanges.html). The old API based on `IncrementalTaskInputs` has been deprecated.

#### Forced dependencies

Forcing dependency versions using `force = true` on a first-level dependency has been deprecated.

Force has both a semantic and ordering issue which can be avoided by using a [strict version constraint](dependency_versions.mdx#sec:rich-version-constraints).
...
This means, if you are publishing libraries with Gradle and using the [maven-publish](publishing_maven.mdx#publishing_maven) or [ivy-publish](publishing_ivy.mdx#publishing_ivy) plugin, the Gradle Module Metadata file is always published **in addition** to traditional metadata.
...
These are documented in the [specification](https://github.com/gradle/gradle/blob/master/platforms/documentation/reference-docs/src/docs/design/gradle-module-metadata-latest-specification.md) as well.
...
You can opt into the old behavior for selected repositories by adding the `artifact()` [metadata source](supported_metadata_formats.mdx#sec:supported-metadata-sources).
...
- The [failOnWarning]({javadocPath}/org/gradle/plugin/devel/tasks/ValidatePlugins.html#getFailOnWarning--) option is now enabled by default.
- The following task validation errors now fail the build at runtime and are promoted to errors for [ValidatePlugins]({javadocPath}/org/gradle/plugin/devel/tasks/ValidatePlugins.html):
...
In Gradle 6.0, we introduced a new progress event ([org.gradle.tooling.events.test.TestOutputEvent]({javadocPath}/org/gradle/tooling/events/test/TestOutputEvent.html)) to expose the output of test execution.
...
Use [org.gradle.process.CommandLineArgumentProvider]({javadocPath}/org/gradle/process/CommandLineArgumentProvider.html) instead.
- The deprecated class `org.gradle.api.ConventionProperty` has been removed.
Use [Providers]({javadocPath}/org/gradle/api/provider/Provider.html) instead of convention properties.
...
- File paths in [deployment descriptor file name]({javadocPath}/org/gradle/plugins/ear/descriptor/DeploymentDescriptor.html#getFileName--) for the ear plugin are not allowed any more.
...
- When [incremental Groovy compilation](groovy_plugin.mdx#sec:incremental_groovy_compilation) is enabled, a wrong configuration of the source roots or enabling Java annotation for Groovy now fails the build.
...
Use the methods on the [`ComponentSelection` parameter](resolution_rules.mdx#sec:component-selection-rules) instead.
- Declaring an [incremental task](custom_tasks.mdx#incremental_tasks) without declaring outputs is now an error.
Declare file outputs or use [TaskOutputs.upToDateWhen()]({javadocPath}/org/gradle/api/tasks/TaskOutputs.html#upToDateWhen-groovy.lang.Closure-) instead.
...
- There are slight changes in the incubating [capabilities resolution](component_capabilities.mdx#sec:selecting-between-candidates) API, which has been introduced in 5.6, to also allow variant selection based on variant name
...
Declaring an [incremental task](custom_tasks.mdx#incremental_tasks) without declaring outputs is now deprecated.
Declare file outputs or use [TaskOutputs.upToDateWhen()]({javadocPath}/org/gradle/api/tasks/TaskOutputs.html#upToDateWhen-groovy.lang.Closure-) instead.
...
See [the section on the Worker API](worker_api.mdx#tasks_parallel_worker) for more information on using these methods.
...
[The PMD plugin](pmd_plugin.mdx#pmd_plugin) has been upgraded to use [PMD version 6.15.0](https://pmd.github.io/pmd-6.15.0/pmd_release_notes.html) instead of 6.8.0 by default.
...
See [the capabilities section of the documentation](component_capabilities.mdx#capabilities) for more options.
...
The built-in [Play plugin](play_plugin.mdx#play_plugin) has been deprecated and will be replaced by a new [Play Framework plugin](https://gradle.github.io/playframework) available from the plugin portal.
...
Project names configured via [`EclipseProject.setName(&#8230;&#8203;)`]({javadocPath}/org/gradle/plugins/ide/eclipse/model/EclipseProject.html) were honored by Gradle and Buildship in all cases, even when the names caused conflicts and import/synchronization errors.
...
[The JaCoCo plugin](jacoco_plugin.mdx#jacoco_plugin) has been upgraded to use [JaCoCo version 0.8.4](http://www.jacoco.org/jacoco/trunk/doc/changes.html) instead of 0.8.3 by default.
...
The system classpath for worker daemons started by the [Worker API](worker_api.mdx#worker_api) when using `PROCESS` isolation has been reduced to a minimum set of Gradle infrastructure.
...
- There should not be setters for lazy properties like [`ConfigurableFileCollection`]({javadocPath}/org/gradle/api/file/ConfigurableFileCollection.html).  Use `setFrom` instead. For example,
...
Please pass files and directories as arguments instead. See examples in the [Worker API documentation](worker_api.mdx#worker_api).

#### Changes to native linking tasks

To expand our idiomatic [Provider API](lazy_configuration.mdx#lazy_configuration) practices, the install name property from `org.gradle.nativeplatform.tasks.LinkSharedLibrary` is affected by this change.
...
To expand our idiomatic [Provider API](lazy_configuration.mdx#lazy_configuration) practices, the `WindowsResourceCompile` task has been converted to use the Provider API.
...
The incubating `operatingSystems` property on native components has been replaced with the [targetMachines]({javadocPath}/org/gradle/language/cpp/CppComponent.html#getTargetMachines()) property.

#### Changes for archive tasks (`Zip`, `Jar`, `War`, `Ear`, `Tar`)

##### Change in behavior for tasks extending `AbstractArchiveTask`

The `AbstractArchiveTask` has several new properties using the [Provider API](lazy_configuration.mdx#lazy_configuration_reference).
