...
For migrating from Gradle 5.x, see the [older migration guide](upgrading_version_5.mdx#upgrading_version_5) first.
...
Head over to [the documentation](dependency_locking.mdx#sec:migrate-single-lockfile) to learn how to migrate to the new format.
...
Users can still opt in to have this unique identifier part of the produced metadata if they want to, see [the documentation](publishing_gradle_module_metadata.mdx#sub:gmm-reproducible).
...
We recommend that you consider [using](declaring_repositories.mdx#three-declaring-repositories) `mavenCentral()`, `google()` or a private `maven` repository instead.
...
You can find more details about the benefits of the new configurations and which one to use in place of `compile` and `runtime` by reading the [Java Library plugin](java_library_plugin.mdx#java_library_plugin) documentation.
...
Tests that use the [TestKit](test_kit.mdx#test_kit) API used to create temporary files under the system temporary directory as defined by `java.io.tmpdir`.
...
To avoid problems with these file locks, `[TestKit](test_kit.mdx#test_kit)` disables file system watching for builds executed on Windows via `GradleRunner`.
...
Please refer to the [documentation of the Maven Publish plugin](publishing_maven.mdx#publishing_maven) for more details.
...
Please refer to the [documentation of the Maven Publish plugin](publishing_maven.mdx#publishing_maven) for publishing on Maven repositories.
Please refer to the [documentation of the Ivy Publish plugin](publishing_ivy.mdx#publishing_ivy) for publishing on Ivy repositories.
...
Have a look at [the documentation](dependency_versions.mdx#sec:single-version-declarations) for all the rules Gradle applies.
...
Please use the stable [Java Library](java_library_plugin.mdx#java_library_plugin) and [Scala](scala_plugin.mdx#scala_plugin) plugins instead.
...
The `layout` method taking a configuration block has been removed and is replaced by [patternLayout]({groovyDslPath}/org.gradle.api.artifacts.repositories.IvyArtifactRepository.html#org.gradle.api.artifacts.repositories.IvyArtifactRepository:patternLayout(org.gradle.api.Action)).
...
To fix this error, [create a `settings.gradle(.kts)` file](settings_file_basics.mdx#sec:settings_file_script) for the build.
...
You can use [DefaultTask]({javadocPath}/org/gradle/api/DefaultTask.html) instead.
...
Please use [BuildListener.beforeSettings(Settings)]({javadocPath}/org/gradle/BuildListener.html#beforeSettings-org.gradle.api.initialization.Settings-) instead.
...
You can find more information about [how to structure a Gradle build](multi_project_builds.mdx#multi_project_builds) in the user manual.
Alternatively, you can still run tasks in builds like this by invoking the build from the `master` directory only using a
[fully qualified path to the task](multi_project_builds_intermediate.mdx#sec:executing_tasks_by_fully_qualified_name).

#### `modularity.inferModulePath` defaults to 'true'

[Compiling](java_library_plugin.mdx#sec:java_library_modular),
[testing](java_testing.mdx#sec:java_testing_modular) and
[executing](application_plugin.mdx#sec:application_modular)
...
The `ValidateTaskProperties` task has been removed and replaced by the [ValidatePlugins]({javadocPath}/org/gradle/plugin/devel/tasks/ValidatePlugins.html) task.

#### Removal of `ImmutableFileCollection`

The `ImmutableFileCollection` type has been removed.
Use the [factory method]({javadocPath}/org/gradle/api/file/ProjectLayout.html#files-java.lang.Object&#8230;&#8203;-) instead.
A handle to the project layout can be obtained via [Project.layout]({javadocPath}/org/gradle/api/Project.html#getLayout--).
...
The local build cache configuration now needs to be done via [BuildCacheConfiguration.local()]({javadocPath}/org/gradle/caching/configuration/BuildCacheConfiguration.html#local-org.gradle.api.Action-).
...
A fix for this problem is to [add a dependency from the consumer to the producer](incremental_build.mdx#sec:link_output_dir_to_input_files).
...
Please look at [the CopySpec docs]({javadocPath}/org/gradle/api/file/CopySpec.html#setDuplicatesStrategy-org.gradle.api.file.DuplicatesStrategy-) for details.
...
If, for some reason, you still want to publish components with dependencies on enforced platforms, you can disable the validation following the [documentation](publishing_setup.mdx#sec:suppressing_validation_errors).
...
See the [user manual](working_with_files.mdx#sec:file_trees) for more information.
...
As a consequence, you are only allowed to change Gradle&#8217;s default excludes in the settings script, see the [user manual](working_with_files.mdx#sec:change_default_excludes) for an example.
...
You are only allowed to change Gradle&#8217;s default excludes in the settings script, see the [user manual](working_with_files.mdx#sec:change_default_excludes) for an example.
...
While adding support for expressing [variant support](resolution_rules.mdx#sec:variant-aware-substitutions) in dependency substitutions, a bug fix introduced a behaviour change that some builds may rely upon.
Previously a substituted dependency would still use the [attributes](variant_attributes.mdx#variant-attributes) of the original selector instead of the ones from the replacement selector.
...
- _and_ you use [resolution rules](resolution_rules.mdx#using-resolution-rules) on these dependencies.

See the [documentation](resolution_rules.mdx#sec:variant-aware-substitutions) for resolving issues if you are impacted.
...
With Gradle 6.4, the incubating API for [dependency locking `LockMode`](dependency_locking.mdx#sec:fine-tuning-dependency-locking-behaviour-with-lock-mode) has changed.
...
Before Gradle introduced [incremental annotation processing support](java_plugin.mdx#sec:incremental_annotation_processing), IDEA required all annotation processors to be on the compilation classpath to be able to run annotation processing when compiling in IDEA.
This is no longer necessary because Gradle has a separate [annotation processor classpath](java_plugin.mdx#tab:configurations).
...
Gradle 6.3 does not support the [rich console](command_line_interface.mdx#sec:rich_console) for 32-bit Unix systems and for old FreeBSD versions (older than FreeBSD 10). Microsoft Windows 32-bit is unaffected.
...
These configurations are no longer used in modern Gradle builds that use [variant aware dependency management](variant_aware_resolution.mdx#sec:understanding-variant-selection) and the [new publishing plugins](publishing_setup.mdx#publishing_components).
...
For declaring dependencies, please use the configurations provided by the plugins you use, for example by the [Java Library plugin](java_library_plugin.mdx#sec:java_library_configurations_graph).
...
The practical effect is that now all [platform dependencies](java_platform_plugin.mdx#sec:java_platform_consumption) have to be declared as such.
...
If you are still using one of these old plugins (`java-lang`, `scala-lang`, `jvm-component`, `jvm-resources`, `junit-test-suite`) please consult the documentation on [Building Java &amp; JVM projects](building_java_projects.mdx#building_java_projects) to determine which of the stable JVM plugins are appropriate for your project.
