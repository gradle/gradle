...
description: "Migrating an Apache Maven build to Gradle, covering the automated conversion and how Maven concepts map across."
---
...
And to make the transition to Gradle easier for Maven users, the <Xref page="base-plugin" section="Tasks">Base Plugin</Xref> — applied by all the JVM language plugins like the <Xref page="java-library-plugin" section="The Java Library Plugin">Java Library Plugin</Xref> — provides a set of lifecycle tasks that correspond to the main Maven phases.
...
It’s one of the standard dependency configurations provided by the <Xref page="java-plugin" section="Dependency management">Java Plugin</Xref> and is often used as a substitute for Maven’s default `compile` scope.
...
You can learn more about this feature and the difference between `platform()` and `enforcedPlatform()` in the section on <Xref page="platforms" section="Importing a Platform (BOM)">importing version recommendations from a Maven BOM</Xref>.
