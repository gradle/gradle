...
description: "The Java plugin: compilation, testing and packaging for Java projects, and the source sets and tasks it adds."
---
...
<SampleScripts sample_name="java-quickstart" basename="build" tag="use-plugin" source_dir="snippets/reference/platforms/jvm/javaQuickstart" />
...
The Java plugin attaches some of its tasks to the lifecycle tasks defined by the <Xref page="base-plugin" section="Tasks">Base Plugin</Xref> — which the Java Plugin applies automatically — and it also adds a few other lifecycle tasks:
...
<SampleScripts sample_name="customized-layout" basename="build" tag="define-main" source_dir="snippets/reference/core-plugins/customizedLayout" />
...
<SampleScripts sample_name="source-sets" basename="build" tag="jar" source_dir="snippets/reference/platforms/jvm/sourceSets" />

Generating Javadoc for a source set:

<SampleScripts sample_name="source-sets" basename="build" tag="javadoc" source_dir="snippets/reference/platforms/jvm/sourceSets" />

Adding a test suite to run the tests in a source set:

<SampleScripts sample_name="source-sets" basename="build" tag="test" source_dir="snippets/reference/platforms/jvm/sourceSets" />
...
| `runtimeClasspath` (extends `runtimeOnly, implementation`) | Resolvable | Represents the classpath used to run the main sources, which includes dependencies from both `runtimeOnly` and `implementation` configurations. |
...
<SampleScripts sample_name="java-basic" basename="build" tag="java-extension" source_dir="snippets/reference/core-plugins/java-basic" />
...
<dl>
<dt>`docsDir` — <JavaDocLink class="org.gradle.api.file.DirectoryProperty" /></dt>
<dd>

The directory to generate documentation into. Default value: `layout.buildDirectory.dir("docs")`

</dd>
<dt>`testResultsDir` — <JavaDocLink class="org.gradle.api.file.DirectoryProperty" /></dt>
<dd>

The directory to generate test result `.xml` files into. Default value: `layout.buildDirectory.dir("test-results")`

</dd>
<dt>`testReportDir` — <JavaDocLink class="org.gradle.api.file.DirectoryProperty" /></dt>
<dd>
...
:::note
The directories for library archives (`libs`) and distributions (`distributions`), along with the archive base name, are configured on the <Xref page="base-plugin" section="Contributed extensions">`base` extension</Xref> — via `base.libsDirectory`, `base.distsDirectory`, and `base.archivesName` respectively.
:::
...
See <Xref page="building-java-projects" section="Modifying the JAR manifest">Modifying the JAR manifest</Xref> for examples of configuring attributes and using Provider values.

</dd>
<dt>`modularity.inferModulePath`</dt>
<dd>

Controls whether Gradle infers the module path (instead of the classpath) for tasks that have a `module-info.java`. See <Xref page="java-library-plugin" section="Building Modules for the Java Module System">Building Modules for the Java Module System</Xref>. Default value: `true`

</dd>
<dt>`consistentResolution(Action)`</dt>
<dd>
...
<dt>`(read-only) boolean autoTargetJvmDisabled`</dt>
<dd>

Tells whether automatic JVM targeting is disabled. When enabled (the default), Gradle refuses to use a dependency built for a more recent JVM than this module’s target compatibility. Default value: `false`
...
| [Hibernate Processor](https://docs.hibernate.org/orm/7.2/userguide/html_single/#tooling-modelgen) (formerly [JPA Static Metamodel Generator](https://docs.hibernate.org/orm/5.4/topical/html_single/metamodelgen/MetamodelGenerator.html), now also supports Jakarta Data) | [5.4.11](https://github.com/hibernate/hibernate-orm/releases/tag/5.4.11) | N/A |
...
<SampleScripts sample_name="apt" basename="build" tag="annotation-processing" source_dir="snippets/reference/platforms/jvm/apt" />
