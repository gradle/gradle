...
description: "Gradle's dependency cache: how downloaded metadata and artifacts are stored, reused and refreshed."
---
...
The Gradle dependency cache consists of two storage types located under `$<Xref page="directory-layout" section="1. Gradle User Home directory">GRADLE_USER_HOME</Xref>/caches`:
...
This will always involve a remote lookup on the repository, however in many cases [no download will be required](#sec:cache-artifact-reuse).
...
You can learn more about cache cleanup in <Xref page="directory-layout" section="Cleanup of caches and distributions">Gradle-managed Directories</Xref>.
...
<SampleScripts sample_name="dependency-caching-changing-deps" basename="build" tag="changing-deps" source_dir="snippets/reference/dependency-management/dependency-management/dependencyCaching-changingDeps" />
...
You can fine-tune certain aspects of caching programmatically using the <JavaDocLink class="org.gradle.api.artifacts.ResolutionStrategy" /> for a configuration.
...
<SampleScripts sample_name="troubleshooting-cache-dynamic" basename="build" tag="dynamic-version-cache-control" source_dir="snippets/reference/dependency-management/dependency-management/troubleshooting-cache-dynamic" />

To change how long Gradle will cache the metadata and artifacts for a changing module, use:

<SampleScripts sample_name="troubleshooting-cache-changing" basename="build" tag="changing-module-cache-control" source_dir="snippets/reference/dependency-management/dependency-management/troubleshooting-cache-changing" />
...
- [copying the dependency cache](#sec:cache-copy) into each container
- [sharing a read-only dependency cache](#sec:shared-readonly-cache) between multiple containers
...
Instead of [copying the dependency cache into each container](#sec:cache-copy), it’s possible to mount a shared, read-only directory that will act as a dependency cache for all containers.
...
For this you need to follow the [instructions above](#sec:cache-copy).
