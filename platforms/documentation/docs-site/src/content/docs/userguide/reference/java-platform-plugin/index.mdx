...
description: "The Java Platform plugin: declaring a platform that shares dependency versions across projects."
---
...
<SampleScripts sample_name="java-platform-quickstart" basename="build" tag="use-plugin" source_dir="snippets/reference/platforms/jvm/javaPlatformQuickstart" />
...
<SampleScripts sample_name="java-platform-quickstart" basename="build" tag="constraints" source_dir="snippets/reference/platforms/jvm/javaPlatformQuickstart" />
...
<SampleScripts sample_name="java-platform-quickstart" basename="build" tag="allow-dependencies" source_dir="snippets/reference/platforms/jvm/javaPlatformQuickstart" />
...
| --- | --- | --- |
| `api` | Declarable | This is where you declare constraints and dependencies that apply when compiling against the platform. They are exported to consumers. |
| `runtime` | Declarable | This is where you declare constraints and dependencies that are only visible at runtime. |

### Consumable Configurations

| Configuration name | Role | Description |
| --- | --- | --- |
| `apiElements` | Consumable | The elements exposed to consumers that depend on the platform via `platform(…​)` for compilation. |
| `runtimeElements` | Consumable | The elements exposed to consumers that depend on the platform via `platform(…​)` at runtime. |
| `enforcedApiElements` | Consumable | Like `apiElements`, but exposes the platform’s constraints as _forced_. Used when consumers depend on the platform via `enforcedPlatform(…​)`. |
| `enforcedRuntimeElements` | Consumable | Like `runtimeElements`, but exposes the platform’s constraints as _forced_. Used when consumers depend on the platform via `enforcedPlatform(…​)`. |
...
<SampleScripts sample_name="multiproject" basename="build" subpath="platform" tag="project-constraints" source_dir="snippets/reference/platforms/jvm/multiproject" />
...
<SampleScripts sample_name="java-platform-quickstart" basename="build" tag="platform" source_dir="snippets/reference/platforms/jvm/javaPlatformQuickstart" />
...
<SampleScripts sample_name="multiproject" basename="build" subpath="platform" tag="publishing" source_dir="snippets/reference/platforms/jvm/multiproject" />
...
Because a Java Platform is a special kind of component, a dependency on a Java platform has to be declared using the `platform` or `enforcedPlatform` keyword, as explained in the <Xref page="platforms" section="Importing a Platform (BOM)">managing transitive dependencies</Xref> section.
...
<SampleScripts sample_name="recommender" basename="build" subpath="platform" tag="define-platform" source_dir="snippets/reference/platforms/jvm/recommender" />

And then have subprojects depend on the platform to get recommendations:

<SampleScripts sample_name="recommender" basename="build" subpath="consumer" tag="get-recommendations" source_dir="snippets/reference/platforms/jvm/recommender" />