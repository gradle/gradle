...
description: "Version catalogs: a central, type-safe list of dependency coordinates and versions shared across a build."
---

import { GroovyDocLink, JavaDocLink, SampleFile, SampleScripts, Xref, YouTube } from '#/components';
...
<YouTube id="dZmlSsRdctE" />

Instead of specifying dependencies directly using string notation, you can pick them from a version catalog:

<SampleScripts sample_name="catalogs-settings" basename="build" tag="simple_dependency_use" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-settings" />
...
- **Dependency Bundles**: Catalogs can group commonly used dependencies into [bundles](#sec:dependency-bundles).
...
- **Conflict Resolution**: Like regular dependency notation, version catalogs declare requested [versions](#sec:common-version-numbers) but do not enforce them during <Xref page="dependency-constraints" section="Declaring Dependency Constraints">conflict resolution</Xref>.

:::note
Version catalogs declare _requested_ versions but do not enforce them.
Gradle may still select different versions due to dependency graph conflicts or constraints applied through <Xref page="platforms" section="Platforms">platforms</Xref> or other dependency management APIs.
:::
...
<SampleScripts sample_name="catalogs-settings" basename="build" tag="use_catalog" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-settings" />

Is the same as:

<SampleScripts sample_name="catalogs-settings" basename="build" tag="use_catalog_equiv" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-settings" />

If the catalog looks as follows:

<SampleFile sample_name="catalogs-settings" path="common/gradle/libs.versions.toml" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-settings" />

Where the catalog can be imported from a [local file](#sec:importing-catalog-from-file) or a [repository](#sec:importing-published-catalog).

Version catalogs use a [TOML format](#sec::toml-dependencies-format) in which accessors map directly to the aliases and versions defined in it, providing type-safe access to dependencies and plugins.
...
| `ktor-client-cio` | `libs.ktor.client.cio` |
...
They do not support classifiers, artifact types (`@aar`, `@zip`), excludes, or capabilities directly in the [TOML](#sec::toml-dependencies-format) file.

<SampleFile sample_name="catalogs-classifiers" path="common/gradle/libs.versions.toml" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-classifiers" />
...
<SampleScripts sample_name="catalogs-classifiers" basename="build" tag="use_classifier" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-classifiers" />

For example, to depend on the sources or Javadoc JAR:

<SampleScripts sample_name="catalogs-classifiers" basename="build" tag="use_classifier_sources" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-classifiers" />
...
<SampleScripts sample_name="catalogs-classifiers" basename="build" tag="use_artifact_type" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-classifiers" />

See [TOML Limitations](#sec:toml-limitations) to learn more.

## Importing a published catalog

A catalog published with the [`version-catalog` plugin](#sec:version-catalog-plugin) can be imported in a consumer’s `settings.gradle(.kts)` file using the <JavaDocLink class="org.gradle.api.initialization.resolve.DependencyResolutionManagement" method="getVersionCatalogs()">Settings API</JavaDocLink>:

<SampleScripts sample_name="catalogs-java-platform-catalog" basename="settings" subpath="consumer" tag="consume_catalog" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-javaPlatformCatalog" />

The `from()` method accepts standard dependency notation (`"group:artifact:version"`).
Gradle downloads the [TOML](#sec::toml-dependencies-format) file from whatever repositories are configured in the `dependencyResolutionManagement` block and makes its entries available as type-safe accessors.
...
The <JavaDocLink class="org.gradle.api.initialization.dsl.VersionCatalogBuilder">version catalog builder API</JavaDocLink> allows importing a catalog from an external file, enabling reuse across different parts of a build, such as sharing the main build’s catalog with `buildSrc`.

For example, you can include a catalog in the `buildSrc/settings.gradle(.kts)` file as follows:

<SampleScripts sample_name="catalogs-toml" basename="settings" subpath="buildSrc" tag="import_main_catalog" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-toml" />

The <JavaDocLink class="org.gradle.api.initialization.dsl.VersionCatalogBuilder" method="from-java.lang.Object-">VersionCatalogBuilder.from(Object dependencyNotation)</JavaDocLink> method accepts only a single file, meaning that notations like <GroovyDocLink class="org.gradle.api.Project" method="files(java.lang.Object[])">Project.files(java.lang.Object…​)</GroovyDocLink> must refer to one file.
...
You can declare multiple catalogs using the <JavaDocLink class="org.gradle.api.initialization.Settings" method="dependencyResolutionManagement(org.gradle.api.Action)">Settings API</JavaDocLink>.
...
<SampleScripts sample_name="catalogs-multiple-catalogs" basename="settings" tag="multiple-catalogs" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-multipleCatalogs" />

Each catalog generates its own extension, so you reference them by name in build scripts:

<SampleScripts sample_name="catalogs-multiple-catalogs" basename="build" tag="use-multiple-catalogs" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-multipleCatalogs" />
...
<SampleFile sample_name="catalogs-toml" path="common/gradle/libs.versions.toml" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-toml" />

However, they can reside in other folders and use different names.
See [#sec:importing-catalog-from-file](#sec:importing-catalog-from-file) for importing a catalog from a custom location, or [#sec:multiple-catalogs](#sec:multiple-catalogs) for registering additional catalogs.

### The TOML format

The version catalog [TOML](https://toml.io/) file has four sections:
...
Versions can be declared either as a single string, in which case they are interpreted as a _required_ version, or as a <Xref page="dependency-versions" section="Declaring rich versions">rich version</Xref>:
...
- `require`: the <Xref page="dependency-versions" section="`require`">required version</Xref>
- `strictly`: the <Xref page="dependency-versions" section="`strictly` or `!!`">strict version</Xref>
- `prefer`: the <Xref page="dependency-versions" section="`prefer`">preferred version</Xref>
- `reject`: the list of <Xref page="dependency-versions" section="`reject`">rejected versions</Xref>
- `rejectAll`: a boolean to reject all <Xref page="dependency-versions" section="`reject`">versions</Xref>
...
<SampleFile sample_name="catalogs-toml" path="common/gradle/test-libs.versions.toml" tag="catalog" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-toml" />
...
<SampleScripts sample_name="catalogs-settings" basename="build" tag="use_dependency_bundle" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-settings" />
...
<SampleScripts sample_name="catalogs-settings" basename="build" tag="use_plugin" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-settings" />
...
| Aliases | Accessors |
| --- | --- |
| `groovyCore` | `libs.groovyCore` |
| `groovyJson-core` | `libs.groovyJson.core` |
...
<SampleScripts sample_name="catalogs-asprovider" basename="build" tag="use_version_asprovider" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-asprovider" />
...
**Workarounds**: for classifiers and artifact types, see [Classifiers, artifact types, and capabilities](#sec:classifiers_artifact_types_and_capabilities).
...
Version catalogs can be declared programmatically in the `settings.gradle(.kts)` file using the <JavaDocLink class="org.gradle.api.initialization.Settings">Settings API</JavaDocLink>:

<SampleScripts sample_name="catalogs-settings" basename="settings" tag="catalog_with_versions" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-settings" />
...
2. ***Write a <Xref page="implementing-gradle-plugins-precompiled" section="Example of a precompiled script plugin in the `Settings` scope">settings plugin</Xref>*** — Create a plugin that programmatically declares catalog entries, publish it to the Gradle Plugin Portal or an internal repository, and have consumers apply it in their settings file.
...
If you already have a `gradle/libs.versions.toml` file, you can publish it directly by importing it into the <JavaDocLink class="org.gradle.api.plugins.catalog.CatalogPluginExtension">catalog extension</JavaDocLink>:
...
This artifact can then be [imported by other Gradle builds](#sec:importing-published-catalog).
...
:::tip
For the full plugin reference — apply syntax, tasks, dependency configurations, extension and publishing component — see the <Xref page="version-catalog-plugin" section="The Version Catalog Plugin">Version Catalog Plugin</Xref> page.
:::
...
<SampleScripts sample_name="catalogs-toml" basename="settings" tag="change_default_extension_name" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-toml" />

## Overwriting catalog versions

You can overwrite versions when importing a catalog:

<SampleScripts sample_name="catalogs-java-platform-catalog" basename="settings" subpath="consumer" tag="overwrite_version" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-javaPlatformCatalog" />

In the examples above, any dependency referencing the `groovy` version will automatically be updated to use `3.0.6`.
You can see how this is used in [Working with multiple version catalogs](#sec:working-multiple-version-catalogs).
...
<SampleScripts sample_name="catalogs-toml-build-src" basename="settings" subpath="buildSrc" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-toml-buildSrc" />

Once the version catalog is imported, dependencies can be referenced using a few tricks:

<SampleScripts sample_name="catalogs-toml-build-src" basename="build" subpath="buildSrc" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-toml-buildSrc" />
...
<SampleScripts sample_name="catalogs-toml-build-src" basename="java-commons-convention" subpath="buildSrc/src/main/{lang}" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-toml-buildSrc" />
...
<SampleFile sample_name="catalogs-duplicate-and-verify" path="{lang}/gradle/distribution.versions.toml" tag="versions" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-duplicateAndVerify" />

<SampleFile sample_name="catalogs-duplicate-and-verify" path="{lang}/gradle/test.versions.toml" tag="versions" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-duplicateAndVerify" />

Add a small verification task (or a CI script) that parses both files and fails the build if the values diverge:

<SampleScripts sample_name="catalogs-duplicate-and-verify" basename="build" tag="verify_shared_versions" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-duplicateAndVerify" />
...
<SampleFile sample_name="catalogs-external-properties" path="{lang}/gradle/shared-versions.properties" tag="shared_versions" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-externalProperties" />

<SampleScripts sample_name="catalogs-external-properties" basename="settings" tag="inject_versions" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-externalProperties" />

The TOML files use `version.ref` to reference these versions as if they were declared locally:

<SampleFile sample_name="catalogs-external-properties" path="{lang}/gradle/distribution.versions.toml" tag="catalog" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-externalProperties" />
...
<SampleScripts sample_name="catalogs-published-catalog-overrides" basename="build" subpath="catalog" tag="publish_catalog" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-publishedCatalogOverrides" />

Consuming project (`settings.gradle(.kts)`):

<SampleScripts sample_name="catalogs-published-catalog-overrides" basename="settings" subpath="consumer" tag="consume_with_overrides" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-publishedCatalogOverrides" />
...
<SampleScripts sample_name="catalogs-combine-from-with-version" basename="settings" tag="combine_from_with_version" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-combineFromWithVersion" />
...
<SampleFile sample_name="catalogs-kotlin-version-sharing" path="common/gradle/catalog.versions.toml" tag="kotlin_catalog" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-kotlinVersionSharing" />

<SampleScripts sample_name="catalogs-kotlin-version-sharing" basename="settings" tag="kotlin_version_sharing" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-kotlinVersionSharing" />
...
<SampleScripts sample_name="catalogs-gradle-apis" basename="build" tag="use_catalog_entries" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-gradleApis" />
...
<SampleScripts sample_name="catalogs-gradle-apis" basename="build" tag="use_provider_in_apis" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-gradleApis" />
