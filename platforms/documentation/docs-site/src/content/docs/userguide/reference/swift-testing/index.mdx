...
description: "Testing Swift projects with Gradle, covering XCTest integration and the native testing model."
---
...
Gradle supports deep integration with XCTest testing framework for the Swift language and revolves around the <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" /> task type.
...
You can then turn those results into a report via an instance of the <GroovyDocLink class="org.gradle.api.tasks.testing.TestReport" /> task type.

In order to operate, the <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" /> task type requires three pieces of information:
- Where to find the built testable bundle (on macOS) or executable (on Linux) (property: <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" method="testInstallDirectory">XCTest.getTestInstalledDirectory()</GroovyDocLink>)
- The run script for executing the bundle or executable (property: <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" method="runScriptFile">XCTest.getRunScriptFile()</GroovyDocLink>)
- The working directory to execution the bundle or executable (property: <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" method="workingDirectory">XCTest.getWorkingDirectory()</GroovyDocLink>)

When you’re using the <Xref page="xctest-plugin" section="XCTest">XCTest Plugin</Xref> you will automatically get the following:
- A dedicated `xctest` extension of type <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.SwiftXCTestSuite" /> for configuring test component and its variants
- A `xcTest` task of type <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" /> that runs those unit tests
...
The <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" /> task has many configuration options.
...
You can control how the test process is launched via several properties on the <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" /> task, including the following:
...
<dt>`testLogging` - default: not set</dt>
<dd>
...
Set <JavaDocLink class="org.gradle.api.tasks.testing.logging.TestLoggingContainer" /> for more detail.

</dd>
</dl>

See <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" /> for details on all the available configuration options.
...
<SampleScripts sample_name="test-filtering" basename="build" tag="test-filtering" source_dir="snippets/reference/platforms/native/testFiltering" />
...
You can also combine filters defined at the command line with <Xref page="command-line-interface" section="Continuous build">continuous build</Xref> to re-execute a subset of tests immediately after every change to a production or test source file.
...
The <GroovyDocLink class="org.gradle.nativeplatform.test.xctest.tasks.XCTest" /> task generates the following results by default:
...
There is also a standalone <GroovyDocLink class="org.gradle.api.tasks.testing.TestReport" /> task type that you can use to generate a custom HTML test report.
...
<SampleScripts sample_name="test-report" basename="myproject.xctest-conventions" subpath="buildSrc/src/main/{lang}" tag="test-report" source_dir="snippets/reference/platforms/native/testReport" />
<SampleScripts sample_name="test-report" basename="build" tag="test-report" source_dir="snippets/reference/platforms/native/testReport" />

In this example, we use a convention plugin `myproject.xctest-conventions` to expose the test results from a project to Gradle’s <Xref page="variant-aware-resolution" section="Variant Selection and Attribute Matching">variant aware dependency management engine</Xref>.
...
You should note that the <GroovyDocLink class="org.gradle.api.tasks.testing.TestReport" /> type combines the results from multiple test tasks and needs to aggregate the results of individual test classes.
