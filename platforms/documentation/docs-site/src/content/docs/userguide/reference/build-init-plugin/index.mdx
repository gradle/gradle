...
description: "The Build Init plugin: generating a new Gradle build with gradle init, including available project types."
---
...
| [swift-application](#sec:swift_application) | A command-line application implemented in Swift |
| [swift-library](#sec:swift_library) | A Swift library |
...
| `--into` | Set the directory where the project is generated. Defaults to the directory where `init` is run. | `./my-project` |
...
| --- | --- | --- |
| `java-application`, `java-library` | `junit-jupiter` | `junit`, `testng`, `spock` |
| `java-gradle-plugin` | `junit-jupiter` | None |
| `kotlin-application`, `kotlin-library` | `kotlintest` | `junit-jupiter` |
| `kotlin-gradle-plugin` | `kotlintest` | None |
| `groovy-application`, `groovy-library`, `groovy-gradle-plugin` | `spock` | None |
| `scala-application`, `scala-library` | `scalatest` | None |
| `cpp-application`, `cpp-library` | `cpptest` | None |
| `swift-application`, `swift-library` | `xctest` | None |
| `basic`, `pom` | None | None |

:::note
`junit` selects [JUnit 4](https://junit.org/junit4/) and `junit-jupiter` selects [JUnit Jupiter](https://junit.org) (JUnit 5). For JVM build types generated with `--split-project`, `junit-jupiter` is the only supported framework.
:::
...
This option tells the conversion process how to handle Maven repositories located at insecure `http` URLs. It is set via the <GroovyDocLink class="org.gradle.buildinit.tasks.InitBuild" method="getInsecureProtocol">--insecure-protocol</GroovyDocLink> option. The default value is `warn`.
...
- Uses [JUnit Jupiter](https://junit.org) for testing
...
- `gradle init --type java-application --test-framework junit`: Uses [JUnit 4](https://junit.org/junit4/) for testing instead of JUnit Jupiter
- `gradle init --type java-application --test-framework spock`: Uses [Spock](https://spockframework.org) for testing instead of JUnit Jupiter
- `gradle init --type java-application --test-framework testng`: Uses [TestNG](https://testng.org) for testing instead of JUnit Jupiter
...
- `gradle init --type java-library --test-framework junit`: Uses [JUnit 4](https://junit.org/junit4/) for testing instead of JUnit Jupiter
- `gradle init --type java-library --test-framework spock`: Uses [Spock](https://spockframework.org) for testing instead of JUnit Jupiter
- `gradle init --type java-library --test-framework testng`: Uses [TestNG](https://testng.org) for testing instead of JUnit Jupiter
...
- Uses the mavenCentral dependency repository
- Uses [JUnit Jupiter](https://junit.org) and TestKit for testing
- Has directories in the conventional locations for source code
- Contains a sample class and unit test, if there are no existing source or test files
...
- `gradle init --type kotlin-application --test-framework junit-jupiter`: Uses [JUnit Jupiter](https://junit.org) for testing instead of the Kotlin test library
...
- `gradle init --type kotlin-library --test-framework junit-jupiter`: Uses [JUnit Jupiter](https://junit.org) for testing instead of the Kotlin test library
...
- Uses the mavenCentral dependency repository
- Uses Scala 2.13
- Uses [ScalaTest](https://www.scalatest.org) for testing
- Has directories in the conventional locations for source code
...
- Uses the mavenCentral dependency repository
- Uses Groovy 4.x
- Uses [Spock testing framework](https://spockframework.org) for testing
- Has directories in the conventional locations for source code
