...
description: "Configuring the build environment: Gradle properties, JVM arguments, environment variables and their precedence."
---

import { GroovyDocLink, Incubating, JavaDocLink, SampleScripts, Xref } from '#/components';
...
For the project property `myProperty`, the system property created is called `org.gradle.project._myProperty_`.
...
If the environment variable name looks like `ORG_GRADLE_PROJECT__myProperty_='Hi, world'`, then Gradle will set a `myProperty` property on your project object, with the value of `Hi, world`.
...
Use <JavaDocLink class="org.gradle.api.provider.ProviderFactory" method="gradleProperty(java.lang.String)">`providers.gradleProperty()`</JavaDocLink> when the value comes from the build environment and should remain lazy.
It searches build-level Gradle properties only.
It does not include `gradle.properties` files in subproject directories, extra properties, or other values added dynamically to a <GroovyDocLink class="org.gradle.api.Project" /> instance.

<JavaDocLink class="org.gradle.api.Project" method="findProperty(java.lang.String)">`Project.findProperty()`</JavaDocLink>, <JavaDocLink class="org.gradle.api.Project" method="property(java.lang.String)">`Project.property()`</JavaDocLink>, and <JavaDocLink class="org.gradle.api.Project" method="hasProperty(java.lang.String)">`Project.hasProperty()`</JavaDocLink> search the dynamic project model in this order:

1. The current `Project` object’s own properties.
...
2. The current project’s extra properties.
...
3. An extension with the requested name.
4. A task with the requested name.
5. Extra properties and extensions on ancestor projects, starting with the parent project and continuing to the root project.
...
See <Xref page="upgrading-version-9" section="Deprecation of implicit lookup of properties and methods in parent projects">the upgrade guide entry</Xref> and <Xref page="isolated-projects" section="No implicit lookup of properties and methods in parent projects">No implicit lookup of properties and methods in parent projects</Xref>.
...
1. Command-line project properties passed with `-P`.
...
5. `gradle.properties` in the current project directory.
6. `gradle.properties` in the build root directory.
7. `gradle.properties` in the Gradle installation directory.
...
| 3 | `gradle.properties` file | Build Root Dir | Stored in the `gradle.properties` file in the build root directory. |
...
Prefer setting an explicit value; relying on the default is <Xref page="upgrading-version-9" section="Deprecation of implicit opt-in to parallel model building">deprecated</Xref>.
