...
description: "How version catalogs and platforms differ, and when to use each to centralise dependency versions."
---
...
<SampleScripts sample_name="catalogs-platforms-platform-example" basename="build" tag="platform-example" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogsPlatforms-platformExample" />

## Using a catalog with a platform

Even if a version catalog defines a version for a dependency, Gradle might pick a different version during resolution if another component (e.g., a platform or a transitive dependency) suggests a different version (unless <Xref page="platforms" section="Enforced Platform">`enforcedPlatform`</Xref> is used).
...
<SampleFile sample_name="catalogs-platforms" path="common/gradle/libs.versions.toml" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-platforms" />

Platform:

<SampleScripts sample_name="catalogs-platforms" basename="build" subpath="platform" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-platforms" />

Consumer:

<SampleScripts sample_name="catalogs-platforms" basename="build" subpath="consumer" tag="usage" source_dir="snippets/reference/dependency-management/centralizing-dependencies/catalogs-platforms" />
