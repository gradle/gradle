...
description: "The Scala plugin: compiling and testing Scala sources, extending the Java plugin's conventions."
---
...
The Scala plugin defaults to incremental compilation by integrating with [Zinc](https://github.com/sbt/zinc), a standalone version of [sbt](https://www.scala-sbt.org/)'s incremental Scala compiler. If you want to disable the incremental compilation, set `force = true` in your build file:
...
:::note
The `targetVersion` property on `IdeaModel` is deprecated and will be removed in Gradle 10.
:::