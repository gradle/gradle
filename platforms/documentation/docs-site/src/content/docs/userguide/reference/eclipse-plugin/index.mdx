...
description: "The Eclipse plugins: generating project and classpath files so a Gradle build can be imported into Eclipse."
---

import { GroovyDocLink, JavaDocLink, SampleScripts, Xref } from '#/components';

The Eclipse plugins generate files that are used by the [Eclipse IDE](https://eclipse.org), thus making it possible to import the project into Eclipse (`File` - `Import…​` - `Existing Projects into Workspace`).
