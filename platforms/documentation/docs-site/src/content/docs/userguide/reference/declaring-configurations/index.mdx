...
description: "Creating dependency configurations, and the roles a configuration can take: declarable, resolvable and consumable."
---

import { GroovyDocLink, JavaDocLink, KotlinDocLink, SampleScripts, Xref } from '#/components';
import { Image } from 'astro:assets';
import dependencyManagementConfigurations from './dependency-management-configurations.png';
import dependencyManagementJavaConfigurations from './dependency-management-java-configurations.png';
...
<Image src={dependencyManagementConfigurations} alt="dependency management configurations" />
...
| --- | --- | --- | --- | --- |
| 1 | Bucket Role | Declarable configuration | Define a set of dependencies. | `implementation` |
| 2 | Consumer Role | Resolvable configuration | Resolve dependencies into concrete artifacts/files. | `runtimeClasspath` |
| 3 | Producer Role | Consumable configuration | Expose built artifacts for other projects to consume. | `runtimeElements` |
...
<SampleScripts sample_name="attribute-matching" basename="build" tag="declare-configuration" source_dir="snippets/reference/dependency-management/declaring-dependencies/attributeMatching" />
...
<SampleScripts sample_name="attribute-matching" basename="build" tag="concrete-classpath" source_dir="snippets/reference/dependency-management/declaring-dependencies/attributeMatching" />
...
<SampleScripts sample_name="attribute-matching" basename="build" tag="setup-configurations" source_dir="snippets/reference/dependency-management/declaring-dependencies/attributeMatching" />
...
Gradle plugins often create <GroovyDocLink class="org.gradle.api.artifacts.Configuration">**configurations**</GroovyDocLink> (like `implementation`, `api`, `testImplementation`) for you.
...
<SampleScripts sample_name="dependency-basics" basename="build" tag="configurations" source_dir="snippets/reference/dependency-management/declaring-dependencies/dependencyBasics" />

There is a relationship between these configurations.

<Image src={dependencyManagementJavaConfigurations} alt="dependency management java configurations" />
...
<SampleScripts sample_name="declaring-configurations-android" basename="build" tag="configurations" source_dir="snippets/reference/dependency-management/declaring-dependencies/declaringConfigurations-android" />
...
<SampleScripts sample_name="declaring-configurations-kmp" basename="build" subpath="shared" tag="configurations" source_dir="snippets/reference/dependency-management/declaring-dependencies/declaringConfigurations-kmp" />
...
This example demonstrates how to manually declare the core Java configurations (normally provided by the <Xref page="java-plugin" section="The Java Plugin">Java plugin</Xref>) in Gradle:

<SampleScripts sample_name="attribute-matching" basename="build" tag="java-configuration-example" source_dir="snippets/reference/dependency-management/declaring-dependencies/attributeMatching" />
...
This setup mimics the behavior of the `implementation`, `compileClasspath`, `runtimeClasspath`, `apiElements`, and `runtimeElements` configurations in the <Xref page="java-plugin" section="The Java Plugin">Java plugin</Xref>.
...
<SampleScripts sample_name="declaring-configurations-lazy-config" basename="build" tag="lazy-config" source_dir="snippets/reference/dependency-management/declaring-dependencies/declaringConfigurations-lazyConfig" />
...
<SampleScripts sample_name="asciidoclet" basename="build" tag="create-custom-doclet" source_dir="snippets/reference/dependency-management/declaring-dependencies/asciidoclet" />
...
For more details, refer to the <GroovyDocLink class="org.gradle.api.artifacts.ConfigurationContainer" /> API.
...
Several incubating factory methods—`resolvable()`, `consumable()`, and `dependencyScope()`—within the <JavaDocLink class="org.gradle.api.artifacts.ConfigurationContainer" /> API can be used to simplify the creation of configurations with specific roles.
...
Configurations form an inheritance hierarchy using the <GroovyDocLink class="org.gradle.api.artifacts.Configuration" method="extendsFrom)">`Configuration.extendsFrom(Configuration…​)`</GroovyDocLink> method.
A configuration can extend any other configuration other than a <KotlinDocLink class="org.gradle.api.artifacts.ConfigurationContainer" method="detached-configuration">detached configuration</KotlinDocLink>, regardless of how it is defined in the build script or plugin.
...
<SampleScripts sample_name="defining-using-configurations-inheritance" basename="build" tag="configuration-definition" source_dir="snippets/reference/dependency-management/declaring-dependencies/definingUsingConfigurations-inheritance" />
...
The entrypoint to all dependency resolution APIs is a _resolvable_ <JavaDocLink class="org.gradle.api.artifacts.Configuration" />.
...
<SampleScripts sample_name="performing-resolution-resolution-result" basename="build" tag="declare-configurations" source_dir="snippets/reference/dependency-management/declaring-dependencies/performingResolution-resolutionResult" />

Dependencies can be declared on the `implementation` configuration using the dependencies block. See the <Xref page="declaring-dependencies" section="1. Declaring dependencies">Declaring Dependencies</Xref> chapter for more information on the types of dependencies that can be declared, and the various options for customizing dependency declarations.

<SampleScripts sample_name="performing-resolution-resolution-result" basename="build" tag="declaring-dependencies" source_dir="snippets/reference/dependency-management/declaring-dependencies/performingResolution-resolutionResult" />

Now that we’ve created a _dependency scope_ configuration for declaring dependencies, and a _resolvable_ configuration for resolving those dependencies, we can use Gradle’s <Xref page="dependency-resolution" section="Dependency Resolution">dependency resolution APIs</Xref> to access the results of resolution.
...
You should <Xref page="command-line-interface" section="Showing or hiding warnings">fix unsafe access warnings</Xref> in your build.

In most cases, you can resolve unsafe accesses by creating a <Xref page="variant-aware-resolution" section="Variant Attribute Matching">proper cross-project dependency</Xref>.