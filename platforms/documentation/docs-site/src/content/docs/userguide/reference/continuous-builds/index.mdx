...
description: "Continuous build, which re-runs the requested tasks automatically when their file inputs change."
---
...
Continuous build uses <Xref page="file-system-watching" section="File System Watching">file system watching</Xref> to detect changes to the inputs.
...
You can configure the quiet period in milliseconds by the Gradle property `<Xref page="build-environment" section="Gradle properties">org.gradle.continuous.quietperiod</Xref>`.
...
Similarly, if the input is a <Xref page="working-with-files" section="Filtering files">filtered file tree</Xref> and no files are matching the filter, the creation of matching files may not trigger a build.
