...
[Build Scan](https://gradle.com/develocity/product/build-scan) is part of that toolchain and will be used throughout this guide.

## Build cache key

Artifacts in the build cache are uniquely identified by a [build cache key](build_cache.mdx#sec:task_output_caching_details).
...
One example is Oracle&#8217;s Java compiler, which, [due to a bug](https://bugs.java.com/bugdatabase/view_bug.do?bug_id=8067422), was producing different bytecode depending on the order source files to be compiled were presented to it.
...
{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/timestamp-in-manifest/kotlin" files="build.gradle.kts[tags=timestamp]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/timestamp-in-manifest/groovy" files="build.gradle[tags=timestamp]" */}
...
This is where [input normalization](incremental_build.mdx#sec:configure_input_normalization) comes into play.
...
Tasks having files as inputs can declare the parts of a file&#8217;s path what are essential to them: this is called the [_path sensitivity_]({javadocPath}/org/gradle/api/tasks/PathSensitive.html) of the input.
...
For more information on compile avoidance see the [corresponding section](java_plugin.mdx#sec:java_compile_avoidance).
...
For details on what level of understanding Gradle has for detecting changes to classpaths and what is considered as a classpath see [this section](incremental_build.mdx#sec:task_input_using_classpath_annotations).

##### Filtering runtime classpaths

For a runtime classpath it is possible to provide better insights to Gradle which files are essential to the input by [configuring input normalization](incremental_build.mdx#sec:configure_input_normalization).
...
{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/normalization/kotlin" files="build.gradle.kts[tags=normalization]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/normalization/groovy" files="build.gradle[tags=normalization]" */}
...
For the same reason, Gradle cannot remove [stale output files](incremental_build.mdx#sec:stale_task_outputs) for these tasks.
...
As [discussed previously](build_cache_use_cases.mdx#share_results_between_ci_builds), you can use Develocity to diagnose the source build of these unexpected cache-hits.
