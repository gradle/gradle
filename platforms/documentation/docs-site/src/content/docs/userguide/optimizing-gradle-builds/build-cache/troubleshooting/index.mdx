...
{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/integration-tests/kotlin" files="build.gradle.kts[tags=environment]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/integration-tests/groovy" files="build.gradle[tags=environment]" */}
...
{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/conditional-action/kotlin" files="build.gradle.kts[tags=conditionalAction]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/conditional-action/groovy" files="build.gradle[tags=conditionalAction]" */}

You should always add the action unconditionally:

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/conditional-action/kotlin" files="build.gradle.kts[tags=unconditionalAction]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/conditional-action/groovy" files="build.gradle[tags=unconditionalAction]" */}
...
:::note
If you&#8217;re using [Java Toolchains](toolchains.mdx#toolchains), the Java major version, the vendor (if specified) and implementation (if specified) will be tracked automatically as an input for compilation and test execution.
:::
...
This can be achieved by using the [runtime API](incremental_build.mdx#sec:task_input_output_runtime_api) as shown in the following snippet.

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/java-version-tracking/kotlin" files="build.gradle.kts[tags=trackVendor]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/java-version-tracking/groovy" files="build.gradle[tags=trackVendor]" */}
...
:::note
[Java compile avoidance](java_plugin.mdx#sec:java_compile_avoidance) will treat this bytecode the same since it extracts the ABI.
:::
...
Using `SNAPSHOT`s or other changing dependencies in your build by design violates the [stable task inputs](build_cache_concepts.mdx#stable_task_inputs) principle.
To use the build cache effectively, you should depend on fixed dependencies.
You may want to look into [dependency locking](dependency_locking.mdx#locking-versions) or switch to using [composite builds](composite_builds.mdx#composite_builds) instead.
...
{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/custom-action/kotlin" files="build.gradle.kts[tags=customAction]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/custom-action/groovy" files="build.gradle[tags=customAction]" */}

do

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/configure-task/kotlin" files="build.gradle.kts[tags=configureTask]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/configure-task/groovy" files="build.gradle[tags=configureTask]" */}

:::caution
Note that configuring a task from other task is not supported when using the [configuration cache](configuration_cache_requirements.mdx#config_cache:requirements:task_access).
:::
...
You already saw that [overlapping outputs are a problem for task output caching](build_cache_concepts.mdx#concepts_overlapping_outputs).
...
It is crucial to have [stable task inputs](build_cache_concepts.mdx#stable_task_inputs) for every cacheable task.
...
Having tasks which generate different outputs for the same inputs can pose a challenge for the effective use of task output caching as seen in [repeatable task outputs](build_cache_concepts.mdx#concepts_repeatable_task_outputs).
...
As described in the [stable task inputs section](build_cache_concepts.mdx#stable_task_inputs), you can either make the task outputs repeatable or use input normalization.
You already learned about the possibilities with [configurable input normalization](build_cache_concepts.mdx#filter_runtime_classpath).

Tasks that produce archives (`tar`, `zip`) are [reproducible](working_with_files.mdx#sec:reproducible_archives) by default, starting with Gradle 9.0.0.
...
{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/reproducible-archives/kotlin" files="build.gradle.kts[]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/reproducible-archives/groovy" files="build.gradle[]" */}
...
Special care should also be taken for code generation tasks which may not have [repeatable task outputs](build_cache_concepts.mdx#concepts_repeatable_task_outputs).
