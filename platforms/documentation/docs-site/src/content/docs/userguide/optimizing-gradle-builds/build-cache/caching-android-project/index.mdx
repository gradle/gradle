---
title: "Caching Android projects"
---
...
This defeats [incremental build](incremental_build.mdx#incremental_build) (i.e., `UP-TO-DATE` checks), but this issue is mitigated via the use of the local build cache.
...
The [discussion](caching_java_projects.mdx#java_compilation) above in "Caching Java projects" is equally relevant here, with the caveat that, for projects that include Kotlin source code, the Kotlin compiler does not currently support [compile avoidance](java_plugin.mdx#sec:java_compile_avoidance) in the way that the Java compiler does.

## Annotation processors and Kotlin

The [advice above](caching_java_projects.mdx#annotation_processors) for pure Java projects also applies to Android projects.
However, if you are using annotation processors (such as Dagger2 or Butterknife) in conjunction with Kotlin and the kotlin-kapt plugin, you should know that before Kotlin 1.3.30 kapt [was not cached by default](https://youtrack.jetbrains.com/issue/KT-27675).

[You can opt into it](https://blog.jetbrains.com/kotlin/2018/01/kotlin-1-2-20-is-out/) (which is recommended) by adding the following to build scripts:

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/caching-android-projects/kotlin" files="build.gradle.kts[tags=cacheKapt]" */}

{/* converter-gap: node=sample dir="snippets/optimizing-builds/build-cache/caching-android-projects/groovy" files="build.gradle[tags=cacheKapt]" */}
...
Please see [this issue](https://issuetracker.google.com/issues/115873051).
...
The [Fabric](https://docs.fabric.io/android/fabric/overview.html) plugin, which is used to integrate the Crashlytics crash-reporting tool (among others), is very popular, yet imposes some hefty performance penalties during the build process.
...
This defeats [incremental build](incremental_build.mdx#incremental_build), because each build will be unique.
...
You may find instructions for that in the [Crashlytics documentation](https://docs.fabric.io/android/crashlytics/build-tools.html).
...
There is a simple workaround for this, based on [this advice](kotlin_dsl.mdx#using_a_groovy_script) from the Kotlin DSL primer.
...
```groovy
Unresolved directive in caching_android_projects.adoc - include::{snippetsPath}/optimizing-builds/build-cache/caching-android-projects/groovy/build.gradle[tags=fabricGroovy]
```

And then, in the module’s `build.gradle.kts` file, apply this script plugin:

```kotlin
Unresolved directive in caching_android_projects.adoc - include::{snippetsPath}/optimizing-builds/build-cache/caching-android-projects/kotlin/build.gradle.kts[tags=fabricKotlin]
```