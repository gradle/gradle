...
description: "Dependency verification: checking the checksums and signatures of dependencies and plugins before a build uses them."
---
...
:::note
Dependency verification is meant to protect yourself from compromised dependencies, not to prevent you from including _vulnerable_ dependencies.
:::
...
:::caution
Signatures can also be used to assess integrity like checksums, but signatures are an _encrypted hash_ of the artifact contents and some metadata, not artifacts themselves.
...
:::caution
Using such a minimal configuration file will cause a project with external dependencies or plugins to fail immediately, as the file does not yet contain the required checksums for verification. See [#sec:bootstrapping-verification](#sec:bootstrapping-verification) to learn more.
:::
...
<SampleScripts sample_name="dependency-verification-disable-verification" basename="build" tag="disable-verification" source_dir="snippets/securing-builds/dependencyVerification-disableVerification" />
...
You have two options for updating the verification metadata: [auto-update](#sec:manual-update-metadata) or [manual update](#sec:verification-update).
...
4. **Handle ignored keys** - If any ignored keys were added, attempt to download them from a keyserver and add them to the keyring if found. See [adding keys to the keyring](#sec:add-ascii-keyring).
...
:::caution
There are dependencies that Gradle _cannot_ discover automatically. In particular, dependencies that are only resolved during task execution or in custom dependency resolution logic may not be included in the generated file. You may need to add these manually.
:::
...
For example, to add verification for [Apache PDFBox](https://pdfbox.apache.org) with GAV coordinates:
...
You need to declare checksums for both (unless you [disabled metadata verification](#sec:disabling-metadata-verification)):
...
See [#sec:dealing-verification-failure](#sec:dealing-verification-failure) for more details.
...
However, passing signature verification doesn’t automatically mean you can trust the artifact—you must explicitly trust the keys by adding them to your verification metadata as shown in [#sec:manual-update-metadata](#sec:manual-update-metadata).
...
Alternatively, ask Gradle to generate the missing information using [bootstrapping](#sec:bootstrapping-verification).
...
- ***Dependency available in multiple sources with slightly different binaries*** - Additional whitespace, timestamps, etc.
  ...
  - Use [`also-trust`](#sec:trusting-several-checksums) to accept additional checksums
...
You need to verify if the key can be trusted, then refer to [Understanding Signature Verification](#sec:understanding-signature-verification) to declare trusted keys.
