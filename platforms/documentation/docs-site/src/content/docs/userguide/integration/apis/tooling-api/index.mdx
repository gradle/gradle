...
- [Gradle TestKit](test_kit.mdx#test_kit) uses the Tooling API for functional testing of your Gradle plugins.
...
[Gradle Daemon](gradle_daemon.mdx#gradle_daemon) contains more details about the daemon, specifically information on situations when new daemons are forked.
...
{/* converter-gap: node=sample dir="snippets/integration/runBuild/kotlin" files="build.gradle.kts[tags=use-tooling-api]" */}

{/* converter-gap: node=sample dir="snippets/integration/runBuild/groovy" files="build.gradle[tags=use-tooling-api]" */}

The main entry point to the Tooling API is the [GradleConnector]({javadocPath}/org/gradle/tooling/GradleConnector.html).
You can navigate from there to find code samples and explore the available Tooling API models.
You can use [GradleConnector.connect()]({javadocPath}/org/gradle/tooling/GradleConnector.html#connect--) to create a [ProjectConnection]({javadocPath}/org/gradle/tooling/ProjectConnection.html).
...
The upper bound is defined in the [compatibility matrix](compatibility.mdx#compatibility).
