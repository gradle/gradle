...
:::note
A new configuration DSL for modeling test execution phases is available via the incubating [JVM Test Suite](jvm_test_suite_plugin.mdx#jvm_test_suite_plugin) plugin.
:::

## The basics

All JVM testing revolves around a single task type: [Test]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html). This runs a collection of test cases using any supported test library — JUnit, JUnit Platform or TestNG — and collates the results. You can then turn those results into a report via an instance of the [TestReport]({groovyDslPath}/org.gradle.api.tasks.testing.TestReport.html) task type.

In order to operate, the `Test` task type requires just two pieces of information:

- Where to find the compiled test classes (property: [Test.getTestClassesDirs()]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html#org.gradle.api.tasks.testing.Test:testClassesDirs))
- The execution classpath, which should include the classes under test as well as the test library that you&#8217;re using (property: [Test.getClasspath()]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html#org.gradle.api.tasks.testing.Test:classpath))

When you&#8217;re using a JVM language plugin — such as the [Java Plugin](java_plugin.mdx#java_plugin) — you will automatically get the following:

- A dedicated `test` source set for unit tests
- A `test` task of type `Test` that runs those unit tests

The JVM language plugins use the source set to configure the task with the appropriate execution classpath and the directory containing the compiled test classes. In addition, they attach the `test` task to the `check` [lifecycle task](organizing_tasks.mdx#sec:lifecycle_tasks).

It&#8217;s also worth bearing in mind that the `test` source set automatically creates [corresponding dependency configurations](java_plugin.mdx#java_source_set_configurations) — of which the most useful are `testImplementation` and `testRuntimeOnly` — that the plugins tie into the `test` task&#8217;s classpath.
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/basic/kotlin" files="build.gradle.kts[tags=java-basic-test-config]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/basic/groovy" files="build.gradle[tags=java-basic-test-config]" */}

The [Test]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html) task has many generic configuration options as well as several framework-specific ones that you can find described in [JUnitOptions]({javadocPath}/org/gradle/api/tasks/testing/junit/JUnitOptions.html), [JUnitPlatformOptions]({javadocPath}/org/gradle/api/tasks/testing/junitplatform/JUnitPlatformOptions.html) and [TestNGOptions]({javadocPath}/org/gradle/api/tasks/testing/testng/TestNGOptions.html). We cover a significant number of them in the rest of the chapter.
...
See [Test]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html) for details on all the available configuration options.
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/filtering/kotlin" files="build.gradle.kts[tags=test-filtering]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/filtering/groovy" files="build.gradle[tags=test-filtering]" */}

For more details and examples of declaring filters in the build script, please see the [TestFilter]({javadocPath}/org/gradle/api/tasks/testing/TestFilter.html) reference.
...
:::note
To aggregate test results across multiple subprojects, see the [Test Report Aggregation Plugin](test_report_aggregation_plugin.mdx#test_report_aggregation_plugin).
:::
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/customDirs/kotlin" files="build.gradle.kts[tags=custom-report-dirs]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/customDirs/groovy" files="build.gradle[tags=custom-report-dirs]" */}
...
```text
Unresolved directive in java_testing.adoc - include::{snippetsPath}/reference/platforms/jvm/customDirs/tests/javaCustomReportDirs.out[]
```

Follow the link to the convention properties for more details.

There is also a standalone [TestReport]({groovyDslPath}/org.gradle.api.tasks.testing.TestReport.html) task type that you can use to generate a custom HTML test report. All it requires are a value for `destinationDir` and the test results you want included in the report. Here is a sample which generates a combined report for the unit tests from all subprojects:

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/testReport/kotlin" files="buildSrc/src/main/kotlin/myproject.java-conventions.gradle.kts[tags=test-report];build.gradle.kts[tags=test-report]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/testReport/groovy" files="buildSrc/src/main/groovy/myproject.java-conventions.gradle[tags=test-report];build.gradle[tags=test-report]" */}

In this example, we use a convention plugin `myproject.java-conventions` to expose the test results from a project to Gradle&#8217;s [variant aware dependency management engine](variant_aware_resolution.mdx#sec:understanding-variant-selection).
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junit-xml-results/kotlin" files="build.gradle.kts[tags=configure-location-convention]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junit-xml-results/groovy" files="build.gradle[tags=configure-location-convention]" */}

With the above configuration, the XML files will be written to `layout.buildDirectory.dir("junit-xml/$testTaskName")`.

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junit-xml-results/kotlin" files="build.gradle.kts[tags=configure-location-task]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junit-xml-results/groovy" files="build.gradle[tags=configure-location-task]" */}
...
[JUnitXmlReport]({javadocPath}/org/gradle/api/tasks/testing/JUnitXmlReport.html) options.

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junit-xml-results/kotlin" files="build.gradle.kts[tags=configure-content]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junit-xml-results/groovy" files="build.gradle[tags=configure-content]" */}
...
If you don&#8217;t want to use test class detection, you can disable it by setting the `scanForTestClasses` property on [Test]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html) to `false`. When you do that, the test task uses only the `includes` and `excludes` properties to find test classes.
...
Test logging can be modified on a per-log-level basis by adjusting the appropriate [TestLogging]({javadocPath}/org/gradle/api/tasks/testing/logging/TestLogging.html) instances in the [testLogging]({javadocPath}/org/gradle/api/tasks/testing/AbstractTestTask.html#getTestLogging--) property of the test task.
For example, to adjust the `INFO` level test logging configuration, modify the
[TestLoggingContainer.getInfo()]({javadocPath}/org/gradle/api/tasks/testing/logging/TestLoggingContainer.html#getInfo--) property.
...
For dividing test classes based upon their purpose, see the incubating [JVM Test Suite](jvm_test_suite_plugin.mdx#jvm_test_suite_plugin) plugin.
:::

JUnit 4.8 introduced the concept of categories for grouping JUnit 4 tests classes and methods.{/* converter-gap: node=inline_footnote */}(https://github.com/junit-team/junit/wiki/Categories).] [Test.useJUnit(org.gradle.api.Action)]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html#org.gradle.api.tasks.testing.Test:useJUnit(org.gradle.api.Action)) allows you to specify the JUnit categories you want to include and exclude. For example, the following configuration includes tests in `CategoryA` and excludes those in `CategoryB` for the `test` task:

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junit-categories/kotlin" files="build.gradle.kts[tags=test-categories]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junit-categories/groovy" files="build.gradle[tags=test-categories]" */}

[JUnit Platform](http://junit.org/junit5/docs/current/user-guide) introduced [tagging](http://junit.org/junit5/docs/current/user-guide/#writing-tests-tagging-and-filtering) to replace categories. You can specify the included/excluded tags via [Test.useJUnitPlatform(org.gradle.api.Action)]({javadocPath}/org/gradle/api/tasks/testing/Test.html#useJUnitPlatform-org.gradle.api.Action-), as follows:

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-tagging/kotlin" files="build.gradle.kts[tags=test-tags]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-tagging/groovy" files="build.gradle[tags=test-tags]" */}

The TestNG framework uses the concept of test groups for a similar effect.{/* converter-gap: node=inline_footnote */}(https://testng.org/#_test_groups).] You can configure which test groups to include or exclude during the test execution via the [Test.useTestNG(org.gradle.api.Action)]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html#org.gradle.api.tasks.testing.Test:useTestNG(org.gradle.api.Action)) setting, as seen here:

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/testng-groups/kotlin" files="build.gradle.kts[tags=test-config]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/testng-groups/groovy" files="build.gradle[tags=test-config]" */}
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-jupiter/kotlin" files="build.gradle.kts[tags=enabling-junit-platform]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-jupiter/groovy" files="build.gradle[tags=enabling-junit-platform]" */}

See [Test.useJUnitPlatform()]({javadocPath}/org/gradle/api/tasks/testing/Test.html#useJUnitPlatform--) for more details.
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-jupiter/kotlin" files="build.gradle.kts[tags=jupiter-dependencies]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-jupiter/groovy" files="build.gradle[tags=jupiter-dependencies]" */}
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-mix/kotlin" files="build.gradle.kts[tags=vintage-dependencies]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-mix/groovy" files="build.gradle[tags=vintage-dependencies]" */}
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-engine/kotlin" files="build.gradle.kts[tags=filter-engine]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/junitplatform-engine/groovy" files="build.gradle[tags=filter-engine]" */}

## Test execution order in TestNG

TestNG allows explicit control of the execution order of tests when you use a _testng.xml_ file. Without such a file — or an equivalent one configured by [TestNGOptions.getSuiteXmlBuilder()]({javadocPath}/org/gradle/api/tasks/testing/testng/TestNGOptions.html#getSuiteXmlBuilder--) — you can&#8217;t specify the test execution order. However, what you _can_ do is control whether all aspects of a test — including its associated `@BeforeXXX` and `@AfterXXX` methods, such as those annotated with `@Before/AfterClass` and `@Before/AfterMethod` — are executed before the next test starts. You do this by setting the [TestNGOptions.getPreserveOrder()]({javadocPath}/org/gradle/api/tasks/testing/testng/TestNGOptions.html#getPreserveOrder--) property to `true`. If you set it to `false`, you may encounter scenarios in which the execution order is something like: `TestA.doBeforeClass()` &#8594; `TestB.doBeforeClass()` &#8594; `TestA` tests.

While preserving the order of tests is the default behavior when directly working with _testng.xml_ files, the [TestNG API](https://javadoc.io/doc/org.testng/testng/latest/org/testng/TestNG.html) that is used by Gradle&#8217;s TestNG integration executes tests in unpredictable order by default.{/* converter-gap: node=inline_footnote */}(http://testng.org/doc/documentation-main.html#testng-xml).] The ability to preserve test execution order was introduced with TestNG version 5.14.5. Setting the `preserveOrder` property to `true` for an older TestNG version will cause the build to fail.

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/testng-preserveorder/kotlin" files="build.gradle.kts[tags=test-config]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/testng-preserveorder/groovy" files="build.gradle[tags=test-config]" */}
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/testng-groupbyinstances/kotlin" files="build.gradle.kts[tags=test-config]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/testng-groupbyinstances/groovy" files="build.gradle[tags=test-config]" */}
...
The simplest way to add integration tests to your build is by leveraging the incubating [JVM Test Suite](jvm_test_suite_plugin.mdx#jvm_test_suite_plugin) plugin.
If an incubating solution is not something for you, here are the steps you need to take in your build:

1. Create a new [source set](building_java_projects.mdx#sec:java_source_sets) for them
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/basic/kotlin" files="build.gradle.kts[tags=practical-integ-test-source-set]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/basic/groovy" files="build.gradle[tags=practical-integ-test-source-set]" */}

This will set up a new source set called `intTest` that automatically creates:

- `intTestImplementation`, `intTestCompileOnly`, `intTestRuntimeOnly` configurations (and [a few others](java_plugin.mdx#java_source_set_configurations) that are less commonly needed)
- A `compileIntTestJava` task that will compile all the source files under _src/intTest/java_

:::note
If you are working with the IntelliJ IDE, you may wish to flag the directories in these additional source sets as containing test source rather than production source as explained in the [Idea Plugin](idea_plugin.mdx#sec:idea_identify_additional_source_sets) documentation.
:::

The example also does the following, not all of which you may need for your specific integration tests:

- Adds the production classes from the `main` source set to the compilation and runtime classpaths of the integration tests — `sourceSets.main.output` is a [file collection](working_with_files.mdx#sec:file_collections) of all the directories containing compiled production classes and resources
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/basic/kotlin" files="build.gradle.kts[tags=integ-test-task]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/basic/groovy" files="build.gradle[tags=integ-test-task]" */}
...
For information on how to determine code coverage for tests in additional source sets, see the [JaCoCo Plugin](jacoco_plugin.mdx#jacoco_plugin) and the [JaCoCo Report Aggregation Plugin](jacoco_report_aggregation_plugin.mdx#jacoco_report_aggregation_plugin) chapters.
...
The [JVM Test Suite](jvm_test_suite_plugin.mdx#jvm_test_suite_plugin) plugin automatically configures all necessary dependencies, but if you are manually creating a `Test` task or using the built-in `test` task, you need to ensure that all required dependencies are declared on the appropriate configuration.
...
Prior to Gradle 9.0.0, Gradle [leaked internal dependencies](upgrading_version_8.mdx#test_framework_implementation_dependencies) into the test&#8217;s runtime classpath which may have masked missing dependencies.
...
Prior to Gradle 9.0.0, all `Test` tasks were [implicitly configured](upgrading_version_8.mdx#test_task_default_classpath) to use the `test` source set.
...
For example, tests that rely on modifying the [`SecurityManager`]({javaApi}/java/lang/SecurityManager.html) may interfere with Gradle&#8217;s internal messaging communication between the build and test worker.
...
If you are [developing Java Modules](java_library_plugin.mdx#java_library_plugin), everything described in this chapter still applies and any of the supported test frameworks can be used.
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/patch-module/kotlin" files="build.gradle.kts[tags=patchArgs]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/patch-module/groovy" files="build.gradle[tags=patchArgs]" */}
...
If you want to skip the tests when running a build, you have a few options. You can either do it via [command line arguments](command_line_interface.mdx#sec:excluding_tasks_from_the_command_line) or [in the build script](controlling_task_execution.mdx#sec:skipping_tasks). To do it on the command line, you can use the `-x` or `--exclude-task` option like so:
...
Skipping a test via the build script can be done a few ways. One common approach is to make test execution conditional via the [Task.onlyIf(String, org.gradle.api.specs.Spec)]({groovyDslPath}/org.gradle.api.Task.html#org.gradle.api.Task:onlyIf(java.lang.String,org.gradle.api.specs.Spec)) method. The following sample skips the `test` task if the project has a property called `mySkipTests`:

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/basic/kotlin" files="build.gradle.kts[tags=skip-tests-condition]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/basic/groovy" files="build.gradle[tags=skip-tests-condition]" */}
...
You can always use the `--rerun` [built-in task option](command_line_interface.mdx#sec:builtin_task_options) to force a task to rerun.

```bash
$ ./gradlew test --rerun
```

Alternatively, if [build caching](build_cache.mdx#sec:build_cache_enable) is not enabled, you can also force tests to run by cleaning the output of the relevant `Test` task — say `test` — and running the tests again, like so:

```bash
$ ./gradlew cleanTest test
```

`cleanTest` is based on a task rule provided by the [Base Plugin](base_plugin.mdx#sec:base_tasks). You can use it for _any_ task.

## Debugging when running tests

On the few occasions that you want to debug your code while the tests are running, it can be helpful if you can attach a debugger at that point. You can either set the [Test.getDebug()]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html#org.gradle.api.tasks.testing.Test:debug) property to `true` or use the `--debug-jvm` command line option, or use `--no-debug-jvm` to set it to false.
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/kotlin" files="lib/build.gradle.kts[tags=use-plugin]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/groovy" files="lib/build.gradle[tags=use-plugin]" */}
...
```java
Unresolved directive in java_testing.adoc - include::{snippetsPath}/reference/platforms/jvm/fixtures/groovy/lib/src/main/java/com/acme/Person.java[tags=sample]
```

A test fixture can be written in `src/testFixtures/java`:

```java
Unresolved directive in java_testing.adoc - include::{snippetsPath}/reference/platforms/jvm/fixtures/groovy/lib/src/testFixtures/java/com/acme/Simpsons.java[tags=sample]
```
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/kotlin" files="lib/build.gradle.kts[tags=test_fixtures_deps]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/groovy" files="lib/build.gradle[tags=test_fixtures_deps]" */}
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/kotlin" files="build.gradle.kts[tags=consumer_dependencies]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/groovy" files="build.gradle[tags=consumer_dependencies]" */}
...
However, if you use the `maven-publish` or `ivy-publish` plugin, test fixtures are published as additional variants in [Gradle Module Metadata](https://github.com/gradle/gradle/blob/master/platforms/documentation/reference-docs/src/docs/design/gradle-module-metadata-1.0-specification.md) and you can directly depend on test fixtures of external libraries in another Gradle project:

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/kotlin" files="build.gradle.kts[tags=external-test-fixtures-dependency]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/groovy" files="build.gradle[tags=external-test-fixtures-dependency]" */}
...
```text
Unresolved directive in java_testing.adoc - include::{snippetsPath}/reference/platforms/jvm/fixtures/tests/dependencyInsight.out[]
```
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/kotlin/lib" files="build.gradle.kts[tags=disable-test-fixtures-publishing]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/fixtures/groovy/lib" files="build.gradle[tags=disable-test-fixtures-publishing]" */}
...
- Tell the `TestEngine` where to find the tests using the [`testDefinitionDirs`]({groovyDslPath}/org.gradle.api.tasks.testing.Test.html#org.gradle.api.tasks.testing.Test:testDefinitionDirs) property of the Test task.
...
:::note
Though possible, it is not recommended to mix class-based and non-class-based tests in the same `Test` task.  Create a separate `Test` task for each type of test instead.  Using [Java Test Suites](jvm_test_suite_plugin.mdx#jvm_test_suite_plugin) makes this easy to do.
:::
...
This example uses [Java Test Suites](jvm_test_suite_plugin.mdx#jvm_test_suite_plugin), but the same process can be applied to manually configured `Test` tasks as well.
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/non-class-based-testing/kotlin/lib" files="build.gradle.kts[tags=test-setup]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/non-class-based-testing/groovy/lib" files="build.gradle[tags=test-setup]" */}
...
{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/non-class-based-testing/common/lib/src/test/definitions" files="ExampleTests.xml[]" */}

{/* converter-gap: node=sample dir="snippets/reference/platforms/jvm/non-class-based-testing/common/lib/src/test/definitions/sub" files="MoreTests.xml[]" */}
...
Non-class-based tests can be filtered in the same way as [class-based tests](#test_filtering) using the [`TestFilter`]({javadocPath}/org/gradle/api/tasks/testing/TestFilter.html) API or `--tests` command line option.

:::note
Non-class-based filtering is an [incubating](feature_lifecycle.mdx#sec:incubating_state) feature that may change.
:::
...
Parallelism in non-class-based tests is controlled via the [`Test.getMaxParallelForks()`]({javadocPath}/org/gradle/api/tasks/testing/Test.html#getMaxParallelForks--) property of the `Test` task.
...
Non-class-based tests can be run through the [Tooling API](tooling_api.mdx#tooling_api) in the same manner as class-based tests.