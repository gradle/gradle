...
import {SampleScripts, GitHubButton, JavaDocLink} from '#/components';

<div id="sharing_build_logic_between_subprojects" />
...
![Sharing build logic overview](./structuring-builds-8.png)

<div id="sec:sharing_logic_via_convention_plugins" />
...
![buildSrc structure](./structuring-builds-9.png)
...
├── api
│   ├── src/
│   └── build.gradle.kts    // <1> A build script that contains build logic, including parts that are shared with other subprojects.
├── services
│   ├── src/
│   └── build.gradle.kts    // <1> A build script that contains build logic, including parts that are shared with other subprojects.
├── shared
...
├── api
│   ├── src/
│   └── build.gradle    // <1> A build script that contains build logic, including parts that are shared with other subprojects.
├── services
│   ├── src/
│   └── build.gradle    // <1> A build script that contains build logic, including parts that are shared with other subprojects.
├── shared
...
<GitHubButton sample="gradle-reference/structuring-builds/sharing-build-logic-using-buildsrc/with-buildsrc" />

`api/build.gradle(.kts)`:
<SampleScripts sample_name="no-buildsrc" basename="build" subpath="api/" />

`services/build.gradle(.kts)`:
<SampleScripts sample_name="no-buildsrc" basename="build" subpath="services/" />

`shared/build.gradle(.kts)`:
<SampleScripts sample_name="no-buildsrc" basename="build" subpath="shared/" />
...
│   │   └──main
│   │      └──kotlin
│   │         └──java-common-conventions.gradle.kts  // <1> A shared build script.
│   └── build.gradle.kts
├── api
│   ├── src/
│   └── build.gradle.kts            // <2> Applies the shared build script.
├── services
│   ├── src/
│   └── build.gradle.kts            // <2> Applies the shared build script.
├── shared
...
│   │   └──main
│   │      └──groovy
│   │         └──java-common-conventions.gradle  // <1> A shared build script.
│   └── build.gradle
├── api
│   ├── src/
│   └── build.gradle            // <2> Applies the shared build script.
├── services
│   ├── src/
│   └── build.gradle            // <2> Applies the shared build script.
├── shared
...
When a `buildSrc` directory is present at the root of a Gradle build, Gradle treats it as a [Composite Build](/gradle-reference/structuring-builds/composite-builds/#composite_builds).
...
<SampleScripts sample_name="buildsrc-build" basename="build" subpath="buildSrc/" />
...
<SampleScripts sample_name="buildsrc-convention" basename="java-common-conventions" subpath="buildSrc/src/main/" />
...
`api/build.gradle(.kts)`:
<SampleScripts sample_name="with-buildsrc" basename="build" subpath="api/" />

`services/build.gradle(.kts)`:
<SampleScripts sample_name="with-buildsrc" basename="build" subpath="services/" />

`shared/build.gradle(.kts)`:
<SampleScripts sample_name="with-buildsrc" basename="build" subpath="shared/" />
...
`buildSrc` follows the same [source layout conventions](https://docs.gradle.org/current/userguide/java_plugin.html#javalayout) as regular Java, Groovy, or Kotlin projects and has direct access to the Gradle API.
...
```kotlin
.
├── build-logic/                   // <1> Standalone build logic project that defines convention plugins.
│   ├── build.gradle.kts
│   ├── settings.gradle.kts
│   └── src/main/kotlin
│       └── java-common-conventions.gradle.kts
├── api/
│   └── build.gradle.kts           // <2> Applies shared plugin(s) from the composite build.
├── services/
│   └── build.gradle.kts           // <2> Applies shared plugin(s) from the composite build.
├── shared/
│   └── build.gradle.kts           // <2> Applies shared plugin(s) from the composite build.
└── settings.gradle.kts            // <3> Includes build-logic as an included build.
```
...
├── api/
│   └── build.gradle            // <2> Applies shared plugin(s) from the composite build.
├── services/
│   └── build.gradle            // <2> Applies shared plugin(s) from the composite build.
├── shared/
│   └── build.gradle            // <2> Applies shared plugin(s) from the composite build.
└── settings.gradle             // <3> Includes `build-logic` as an included build.
```

You can learn more in [Composite Builds](/gradle-reference/structuring-builds/composite-builds/#composite_builds).

<div id="sec:convention_plugins_vs_cross_configuration" />

## Avoid cross-project configuration using `subprojects` and `allprojects`

An improper way to share build logic between subprojects is _cross-project configuration_ via the <JavaDocLink class="org.gradle.api.Project" method="subprojects(groovy.lang.Closure)" text="`subprojects {}`" /> and <JavaDocLink class="org.gradle.api.Project" method="allprojects(groovy.lang.Closure)" text="`allprojects {}`" /> DSL constructs.

With [cross-project configuration](/additional/glossary/#sub:terminology_cross_configuration), build logic can be injected into a subproject which is not obvious when looking at its build script.
...
2. Extracting information from subprojects of a certain type.
   This use case can be modeled using [outgoing configuration variants](/gradle-reference/dependency-management/advanced-concepts/variant-selection/#sec:variant-aware-matching).
