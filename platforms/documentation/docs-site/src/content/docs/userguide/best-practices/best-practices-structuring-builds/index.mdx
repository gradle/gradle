...
description: "Recommendations for organising a build into subprojects and composite builds as a codebase grows."
---

import { CalloutList, KotlinDocLink, SampleFile, SampleScripts, Xref } from '#/components';
...
```kotlin title="A common way to structure new builds"
...
```groovy title="A common way to structure new builds"
├── app // This project contains a mix of classes
│    ├── build.gradle
│    └── src
│        └── main
...
<SampleScripts sample_name="modularize-your-build-avoid" basename="settings" tag="avoid-this" source_dir="snippets/best-practices/modularizeYourBuild-avoid" />

<SampleScripts sample_name="modularize-your-build-avoid" basename="build" subpath="app" tag="avoid-this" source_dir="snippets/best-practices/modularizeYourBuild-avoid" />
...
```kotlin title="A better way to structure this build"
├── app
│    ├── build.gradle.kts
│    └── src
│        └── main
...
```groovy title="A better way to structure this build"
...
<SampleScripts sample_name="modularize-your-build-do" basename="settings" tag="do-this" source_dir="snippets/best-practices/modularizeYourBuild-do" />

<SampleScripts sample_name="modularize-your-build-do" basename="build" subpath="app" tag="do-this" source_dir="snippets/best-practices/modularizeYourBuild-do" />

<SampleScripts sample_name="modularize-your-build-do" basename="build" subpath="util-commons" tag="do-this" source_dir="snippets/best-practices/modularizeYourBuild-do" />

<SampleScripts sample_name="modularize-your-build-do" basename="build" subpath="util-guava" tag="do-this" source_dir="snippets/best-practices/modularizeYourBuild-do" />
...
5. Each project only adds the dependencies it needs.  Projects can effectively use <Xref page="java-library-plugin" section="API and implementation separation">API vs. Implementation separation</Xref>.

</CalloutList>

### References

- <Xref page="multi-project-builds" section="Multi-Project Builds">Structuring Projects with Gradle</Xref>
- <Xref page="organizing-gradle-projects" section="Use a Settings file">Organizing Gradle Projects</Xref>
...
The root project is a special <KotlinDocLink class="org.gradle.api.Project" /> in Gradle that serves as the entry point for your build.

It is the place to configure some settings and conventions that apply globally to the entire build, that are not configured via <KotlinDocLink class="org.gradle.api.initialization.Settings" />.
...
<SampleScripts sample_name="no-source-in-root-avoid" basename="build" tag="avoid-this" source_dir="snippets/best-practices/noSourceInRoot-avoid" />
...
```kotlin title="A better way to structure new builds"
├── core
...
```groovy title="A better way to structure new builds"
├── core
...
<SampleScripts sample_name="no-source-in-root-do" basename="settings" tag="do-this" source_dir="snippets/best-practices/noSourceInRoot-do" />

<SampleScripts sample_name="no-source-in-root-do" basename="build" subpath="core" tag="do-this" source_dir="snippets/best-practices/noSourceInRoot-do" />
...
You should set up a <Xref page="composite-builds" section="Composite Build Layout">Composite Build</Xref> (often called an "included build") to hold your build logic—including any custom plugins, convention plugins, and other build-specific customizations.
...
Using different plugin combinations can cause features like <Xref page="build-services" section="Using Shared Build Services">Build Services</Xref> to break in difficult to diagnose ways.
...
<SampleScripts sample_name="favor-composite-builds-avoid" basename="build" subpath="buildSrc" tag="avoid-this" source_dir="snippets/best-practices/favorCompositeBuilds-avoid" />

**Set up a Plugin Build**: This is the same using either method.

<SampleScripts sample_name="favor-composite-builds-avoid" basename="settings" tag="avoid-this" source_dir="snippets/best-practices/favorCompositeBuilds-avoid" />
...
<SampleScripts sample_name="favor-composite-builds-do" basename="build" subpath="build-logic/plugin" tag="do-this" source_dir="snippets/best-practices/favorCompositeBuilds-do" />

**Set up a Plugin Build**: This is the same using either method.

<SampleScripts sample_name="favor-composite-builds-do" basename="settings" tag="do-this" source_dir="snippets/best-practices/favorCompositeBuilds-do" />

<SampleScripts sample_name="favor-composite-builds-do" basename="settings" subpath="build-logic" tag="do-this" source_dir="snippets/best-practices/favorCompositeBuilds-do" />
...
- <Xref page="part3-multi-project-builds" section="Step 3. Understand Composite Builds">Composite Builds in the Multi-Project Builds Tutorial</Xref>
- <Xref page="composite-builds" section="Composite Builds (Included Builds)">Composite Builds reference documentation</Xref>
- [Gradle Issue #6045: buildSrc vs. included builds](https://github.com/gradle/gradle/issues/6045)

### Tags

`<Xref page="tags-reference" section="`#structuring-builds`">#structuring-builds</Xref>`, `<Xref page="tags-reference" section="`#composite-builds`">#composite-builds</Xref>`
...
When you use the <KotlinDocLink class="org.gradle.api.initialization.Settings" method="include">Settings.include()</KotlinDocLink> method to include a project in your Grade settings file, you typically include projects by supplying the directory name like `include("featureA")`.
...
To avoid these downsides when using a hierarchical project structure, you can provide a flat name when including the project and explicitly set the <KotlinDocLink class="org.gradle.api.Project" method="get-project-dir">Project.projectDir</KotlinDocLink> property for any projects located in nested directories:
...
:::note
Always use an identical _logical_ project ***name*** and _physical_ project ***location*** to avoid confusion.
...
You can also run the <Xref page="project-report-plugin" section="The Project Report Plugin">projects report</Xref> for more information about the projects in your build and their locations.
...
<SampleScripts sample_name="avoid-empty-projects-avoid" basename="settings" tag="avoid-this" source_dir="snippets/best-practices/avoidEmptyProjects-avoid" />
...
<SampleFile sample_name="avoid-empty-projects-avoid" path="tests/avoidEmptyProjects-avoid.out" source_dir="snippets/best-practices/avoidEmptyProjects-avoid" />
...
<SampleScripts sample_name="avoid-empty-projects-do" basename="settings" tag="do-this" source_dir="snippets/best-practices/avoidEmptyProjects-do" />
...
<SampleFile sample_name="avoid-empty-projects-do" path="tests/avoidEmptyProjects-do.out" source_dir="snippets/best-practices/avoidEmptyProjects-do" />
...
- <Xref page="multi-project-builds" section="Multi-Project Builds">Multi-Project Builds</Xref>
...
Use <Xref page="implementing-gradle-plugins-precompiled" section="Pre-compiled Script Plugins">convention plugins</Xref> to encapsulate and reuse shared build logic across multiple projects in your build.
...
Convention plugins are quicker to create than <Xref page="implementing-gradle-plugins-binary" section="Binary Plugins">typed binary plugins</Xref> extending the `Plugin` class.
...
<SampleScripts sample_name="use-convention-plugins-avoid" basename="build" subpath="project-a" tag="avoid-this" source_dir="snippets/best-practices/useConventionPlugins-avoid" />
<SampleScripts sample_name="use-convention-plugins-avoid" basename="build" subpath="project-b" tag="avoid-this" source_dir="snippets/best-practices/useConventionPlugins-avoid" />
...
<SampleScripts sample_name="use-convention-plugins-do" basename="settings" tag="do-this" source_dir="snippets/best-practices/useConventionPlugins-do" />
<SampleScripts sample_name="use-convention-plugins-do" basename="build" subpath="build-logic" tag="do-this" source_dir="snippets/best-practices/useConventionPlugins-do" />
...
<SampleScripts sample_name="use-convention-plugins-do" basename="my.base-java-library" subpath="build-logic/src/main/{lang}" tag="do-this" source_dir="snippets/best-practices/useConventionPlugins-do" />
<SampleScripts sample_name="use-convention-plugins-do" basename="my.java-library" subpath="build-logic/src/main/{lang}" tag="do-this" source_dir="snippets/best-practices/useConventionPlugins-do" />
<SampleScripts sample_name="use-convention-plugins-do" basename="my.java-use-junit5" subpath="build-logic/src/main/{lang}" tag="do-this" source_dir="snippets/best-practices/useConventionPlugins-do" />
...
<SampleScripts sample_name="use-convention-plugins-do" basename="build" subpath="project-a" tag="do-this" source_dir="snippets/best-practices/useConventionPlugins-do" />
<SampleScripts sample_name="use-convention-plugins-do" basename="build" subpath="project-b" tag="do-this" source_dir="snippets/best-practices/useConventionPlugins-do" />

### References

- <Xref page="plugins" section="Introduction to Plugins">Developing Custom Gradle Plugins</Xref>
- <Xref page="implementing-gradle-plugins-precompiled" section="Pre-compiled Script Plugins">Implementing Pre-compiled Script Plugins</Xref>
- <Xref page="plugins" section="Types of Plugins">Types of Plugins</Xref>
