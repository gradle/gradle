...
description: "General recommendations for authoring Gradle builds, covering DSL choice, project layout and conventions that keep builds maintainable."
---
...
<SampleScripts sample_name="use-plugins-block-avoid" basename="build" tag="avoid-this" source_dir="snippets/best-practices/usePluginsBlock-avoid" />
...
<SampleScripts sample_name="use-plugins-block-do" basename="build" tag="do-this" source_dir="snippets/best-practices/usePluginsBlock-do" />
...
<SampleScripts sample_name="dont-assume-plugin-order-avoid" basename="build" tag="avoid-this" source_dir="snippets/best-practices/dontAssumePluginOrder-avoid" />

In the `app` subproject, the build file uses `extensions.getByType(…​)` which assumes `java` has already been applied:

<SampleScripts sample_name="dont-assume-plugin-order-avoid" basename="build" subpath="app" tag="avoid-this" source_dir="snippets/best-practices/dontAssumePluginOrder-avoid" />

In the plugin implementation, `MyPlugin.kt` or `MyPlugin.groovy` also assumes `java` is already applied:

<SampleFile sample_name="dont-assume-plugin-order-avoid" path="{lang}/buildSrc/src/main/{lang}/MyPlugin.{srcExt}" tag="avoid-this" source_dir="snippets/best-practices/dontAssumePluginOrder-avoid" />
...
<SampleScripts sample_name="dont-assume-plugin-order-do" basename="build" subpath="app" tag="do-this" source_dir="snippets/best-practices/dontAssumePluginOrder-do" />

In the plugin implementation, `MyPlugin.kt` or `MyPlugin.groovy` explicitly applies the `java` plugin:

<SampleFile sample_name="dont-assume-plugin-order-do" path="{lang}/buildSrc/src/main/{lang}/MyPlugin.{srcExt}" tag="do-this" source_dir="snippets/best-practices/dontAssumePluginOrder-do" />
...
<SampleScripts sample_name="avoid-internal-avoid" basename="build" tag="avoid-this" source_dir="snippets/best-practices/avoidInternal-avoid" />
...
<SampleScripts sample_name="avoid-internal-do" basename="build" tag="do-this" source_dir="snippets/best-practices/avoidInternal-do" />
...
<SampleScripts sample_name="use-the-gradle-properties-file-avoid" basename="build" tag="avoid-this" source_dir="snippets/best-practices/useTheGradlePropertiesFile-avoid" />
...
<SampleScripts sample_name="name-your-root-project-avoid" basename="settings" source_dir="snippets/best-practices/nameYourRootProject-avoid" />
...
<SampleScripts sample_name="name-your-root-project-do" basename="settings" source_dir="snippets/best-practices/nameYourRootProject-do" />
...
<SampleFile sample_name="name-your-root-project-do" path="tests/nameYourRootProject-do.out" source_dir="snippets/best-practices/nameYourRootProject-do" />
...
<SampleFile sample_name="avoid-gradle-properties-in-sub-projects-avoid" path="common/app/gradle.properties" source_dir="snippets/best-practices/avoidGradlePropertiesInSubProjects-avoid" />

<SampleScripts sample_name="avoid-gradle-properties-in-sub-projects-avoid" basename="build" subpath="app" source_dir="snippets/best-practices/avoidGradlePropertiesInSubProjects-avoid" />

<SampleFile sample_name="avoid-gradle-properties-in-sub-projects-avoid" path="common/util/gradle.properties" source_dir="snippets/best-practices/avoidGradlePropertiesInSubProjects-avoid" />

<SampleScripts sample_name="avoid-gradle-properties-in-sub-projects-avoid" basename="build" subpath="util" source_dir="snippets/best-practices/avoidGradlePropertiesInSubProjects-avoid" />
...
<SampleFile sample_name="avoid-gradle-properties-in-sub-projects-do" path="common/gradle.properties" source_dir="snippets/best-practices/avoidGradlePropertiesInSubProjects-do" />

<SampleFile sample_name="avoid-gradle-properties-in-sub-projects-do" path="{lang}/buildSrc/src/main/{lang}/ProjectProperties.{srcExt}" source_dir="snippets/best-practices/avoidGradlePropertiesInSubProjects-do" />

<SampleScripts sample_name="avoid-gradle-properties-in-sub-projects-do" basename="project-properties" subpath="buildSrc/src/main/{lang}" source_dir="snippets/best-practices/avoidGradlePropertiesInSubProjects-do" />

<SampleScripts sample_name="avoid-gradle-properties-in-sub-projects-do" basename="build" subpath="app" source_dir="snippets/best-practices/avoidGradlePropertiesInSubProjects-do" />

<SampleScripts sample_name="avoid-gradle-properties-in-sub-projects-do" basename="build" subpath="util" source_dir="snippets/best-practices/avoidGradlePropertiesInSubProjects-do" />
...
<SampleFile sample_name="avoid-after-evaluate-avoid" path="{lang}/buildSrc/src/main/{lang}/AppInfoPlugin.{srcExt}" tag="avoid-this" source_dir="snippets/best-practices/avoidAfterEvaluate-avoid" />
...
<SampleScripts sample_name="avoid-after-evaluate-avoid" basename="build" tag="avoid-this" source_dir="snippets/best-practices/avoidAfterEvaluate-avoid" />
...
<SampleFile sample_name="avoid-after-evaluate-do" path="{lang}/buildSrc/src/main/{lang}/AppInfoPlugin.{srcExt}" tag="do-this" source_dir="snippets/best-practices/avoidAfterEvaluate-do" />
...
<SampleScripts sample_name="avoid-after-evaluate-do" basename="build" tag="do-this" source_dir="snippets/best-practices/avoidAfterEvaluate-do" />
...
<Xref page="tags-reference" section="`#plugins`">`#plugins`</Xref>, <Xref page="tags-reference" section="`#structuring-builds`">`#structuring-builds`</Xref>
...
Always obtain a <JavaDocLink class="org.gradle.api.logging.Logger" /> by calling `Logging.getLogger(MyClass.class)` from inside each non-`Task` class that logs — plugins, build services, project extensions, and any other helper class — rather than reusing `project.logger` or `script.logger`.
Inside a `Task`, use the inherited <GroovyDocLink class="org.gradle.api.Task" method="logger">`Task.getLogger()`</GroovyDocLink> instead.
...
<GroovyDocLink class="org.gradle.api.Task" method="logger">`Task.getLogger()`</GroovyDocLink> returns a logger whose underlying name is `org.gradle.api.Task` (shared across all tasks), but Gradle wraps it with per-task build-operation context, so console output is attributed to the specific task automatically.
...
- `project.logger` accessed during task execution is not compatible with the <Xref page="configuration-cache" section="Configuration Cache">Configuration Cache</Xref>, causing the same problems as noted in <Xref page="best-practices-tasks" section="Don't access a `Project` instance during Task Execution">Don’t access a `Project` instance during Task Execution</Xref>.
...
`Logger` is not among the <Xref page="service-injection" section="Available services">services available for injection</Xref> into Gradle-managed types (plugins, build services).
...
:::note
Inline build scripts (`build.gradle(.kts)`) should use the script-scoped `logger` directly if there is no class to attach a static logger to.
...
<SampleScripts sample_name="obtain-loggers-via-logging-get-logger-avoid" basename="build" tag="avoid-this" source_dir="snippets/best-practices/obtainLoggersViaLoggingGetLogger-avoid" />
...
<SampleScripts sample_name="obtain-loggers-via-logging-get-logger-do" basename="build" tag="do-this" source_dir="snippets/best-practices/obtainLoggersViaLoggingGetLogger-do" />
...
- <Xref page="logging" section="Logging and Output">Logging</Xref>
- <Xref page="build-services" section="Using Shared Build Services">Shared Build Services</Xref>
- <Xref page="service-injection" section="Available services">Services Available for Injection</Xref>
- <Xref page="configuration-cache" section="Configuration Cache">Configuration Cache</Xref>

### Tags

<Xref page="tags-reference" section="`#tasks`">`#tasks`</Xref>, <Xref page="tags-reference" section="`#plugins`">`#plugins`</Xref>, <Xref page="tags-reference" section="`#logging`">`#logging`</Xref>