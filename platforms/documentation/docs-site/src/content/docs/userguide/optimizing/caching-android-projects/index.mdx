...
description: "Making Android builds cacheable, and the differences from plain Java projects that affect caching."
---
...
The <Xref page="caching-java-projects" section="Java compilation">discussion</Xref> above in "Caching Java projects" is equally relevant here, with the caveat that, for projects that include Kotlin source code, the Kotlin compiler does not currently support <Xref page="java-plugin" section="Compilation avoidance">compile avoidance</Xref> in the way that the Java compiler does.

## Annotation processors and Kotlin

The <Xref page="caching-java-projects" section="Annotation processors">advice above</Xref> for pure Java projects also applies to Android projects.
...
<SampleScripts sample_name="caching-android-projects" basename="build" tag="cacheKapt" source_dir="snippets/optimizing-builds/build-cache/caching-android-projects" />
...
This defeats incremental build, because each build will be unique.
...
There is a simple workaround for this, based on <Xref page="kotlin-dsl" section="Using a Groovy script">this advice</Xref> from the Kotlin DSL primer.
