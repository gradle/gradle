...
description: "Practical steps for making builds faster, covering parallelism, caching, daemon settings and common bottlenecks."
---
...
Before making any changes, <Xref page="inspect" section="Build Scan">inspect your build</Xref> with a [**Build Scan**](https://develocity.ai/product/build-scan/) or **profile report**.
...
Set `org.gradle.tooling.parallel` explicitly; relying on the default derived from `org.gradle.parallel` is <Xref page="upgrading-version-9" section="Deprecation of implicit opt-in to parallel model building">deprecated</Xref>.
