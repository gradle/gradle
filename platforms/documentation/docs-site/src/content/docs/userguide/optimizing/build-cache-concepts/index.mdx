...
description: "The concepts behind build cache hits and misses: task inputs, cache keys, and what makes a task cacheable."
---

import { JavaDocLink, SampleScripts, Xref } from '#/components';
import { Image } from 'astro:assets';
import overlappingOutputsTimeline from './overlapping-outputs-timeline.png';
...
Artifacts in the build cache are uniquely identified by a <Xref page="build-cache" section="Cacheable tasks">build cache key</Xref>.
...
<SampleScripts sample_name="timestamp-in-manifest" basename="build" tag="timestamp" source_dir="snippets/optimizing-builds/build-cache/timestamp-in-manifest" />
...
This is where input normalization comes into play.
...
Tasks having files as inputs can declare the parts of a file’s path what are essential to them: this is called the <JavaDocLink class="org.gradle.api.tasks.PathSensitive">_path sensitivity_</JavaDocLink> of the input.
...
For more information on compile avoidance see the <Xref page="java-plugin" section="Compilation avoidance">corresponding section</Xref>.
...
For details on what level of understanding Gradle has for detecting changes to classpaths and what is considered as a classpath see this section.
...
<SampleScripts sample_name="normalization" basename="build" tag="normalization" source_dir="snippets/optimizing-builds/build-cache/normalization" />
...
For the same reason, Gradle cannot remove stale output files for these tasks.
...
<Image src={overlappingOutputsTimeline} alt="overlapping outputs timeline" />
...
As <Xref page="build-cache-use-cases" section="Share results between CI builds">discussed previously</Xref>, you can use Develocity to diagnose the source build of these unexpected cache-hits.
