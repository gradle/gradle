...
description: "Configuration Cache support status across core Gradle plugins and the wider plugin ecosystem."
---
...
<Xref page="configuration-cache-enabling" section="Enabling Parallel Configuration Caching">Parallel storing</Xref> writes the configuration state of each project concurrently, and writing a project’s state requires access to that project.
...
Task execution is parallel under the Configuration Cache regardless of `--parallel`, as is loading a cache entry; see <Xref page="configuration-cache" section="Parallel Task Execution">Parallel Task Execution</Xref>.

See [gradle/gradle#36002](https://github.com/gradle/gradle/issues/36002).
...
<SampleScripts sample_name="top-level" basename="build" tag="not-supported" source_dir="snippets/optimizing-builds/configuration-cache/topLevel" />
...
<SampleScripts sample_name="top-level" basename="build" tag="workaround" source_dir="snippets/optimizing-builds/configuration-cache/topLevel" />
...
See <Xref page="configuration-cache-enabling" section="Serialization-behavior opt-outs">Serialization-behavior opt-outs</Xref> to learn how to _temporarily_ suppress these problems if discarded listeners do not affect your
build.
