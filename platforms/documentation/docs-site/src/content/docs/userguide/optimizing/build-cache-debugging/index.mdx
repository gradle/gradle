...
description: "Diagnosing build cache misses by comparing task inputs between builds and finding undeclared or unstable inputs."
---

import { CalloutList, SampleScripts, Xref } from '#/components';
import { Image } from 'astro:assets';
import fullyCachedTaskExecution from './fully-cached-task-execution.png';
import firstNonCachedTask from './first-non-cached-task.png';
...
Use the methods as described below to [diagnose](#diagnosing_cache_miss) and <Xref page="common-caching-problems" section="Solving common problems">fix</Xref> tasks that should be up-to-date but aren’t.
...
The goal is to achieve <Xref page="build-cache-concepts" section="Stable task inputs">stable task inputs</Xref> for cacheable tasks.
...
<Image src={fullyCachedTaskExecution} alt="fully cached task execution" />

Again, use the below methods to [diagnose](#diagnosing_cache_miss) and <Xref page="common-caching-problems" section="Solving common problems">fix</Xref> cacheability issues.
...
(More on this in <Xref page="build-cache-concepts" section="Path sensitivity and relocatability">path sensitivity and relocatability</Xref>.)
...
To test cross-platform cache reuse, set up a `remote` cache (see <Xref page="build-cache-use-cases" section="Share results between CI builds">share results between CI builds</Xref>) and populate it from one platform and consume it from the other.
...
A list of all inputs to the computed build cache key can be found in the <Xref page="build-cache" section="Cacheable tasks">section on cacheable tasks</Xref>.
...
If you want information about the build cache key and individual input property hashes, use <Xref page="build-environment" section="Gradle properties">`-Dorg.gradle.caching.debug=true`</Xref>:
...
<Image src={firstNonCachedTask} alt="first non cached task" />
...
This can be problematic, see <Xref page="common-caching-problems" section="Review usages of `doFirst` and `doLast`">section about `doFirst` and `doLast`</Xref>.
:::
...
As described in <Xref page="common-caching-problems" section="Java version tracking">Java version tracking</Xref>, the Java version can also influence the output of the Java compiler.
...
<SampleScripts sample_name="normalization" basename="build" tag="versionInfo" source_dir="snippets/optimizing-builds/build-cache/normalization" />
...
In order to solve this problem see <Xref page="common-caching-problems" section="Non-repeatable task outputs">non-repeatable task outputs</Xref> or use <Xref page="build-cache-concepts" section="Better reuse via input normalization">input normalization</Xref>.
