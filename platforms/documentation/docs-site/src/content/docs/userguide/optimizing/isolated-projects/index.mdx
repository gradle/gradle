...
It can also be <Xref page="performance" section="Configure Tooling API actions parallelism">allowed without Isolated Projects</Xref>
...
Isolated Projects makes this safe by enforcing [constraints](#sec:constraints) that prevent projects from sharing mutable state.
:::
...
- Make your build <Xref page="configuration-cache-enabling" section="Adopting the Configuration Cache">compatible with Configuration Cache</Xref>
...
To guarantee reliable build results, Gradle will fail the build when any violations of [Isolated Project constraints](#sec:constraints) are detected.
...
3. Follow the [build-logic refactoring strategies](#sec:migration_strategies) to refactor your build logic and make it compatible.
...
<Xref page="sharing-build-logic-between-subprojects" section="Using `buildSrc`">Convention plugins</Xref> address the same need from a different perspective.
The configuration logic is defined once in a <Xref page="plugins" section="Convention plugins">convention plugin</Xref> and then explicitly applied in projects that need it.
...
You should make your build <Xref page="configuration-cache-enabling" section="Adopting the Configuration Cache">compatible with Configuration Cache</Xref> first, before migrating to Isolated Projects.
