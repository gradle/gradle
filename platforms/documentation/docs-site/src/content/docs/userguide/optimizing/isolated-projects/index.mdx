...
description: "Isolated Projects, an incubating feature that improves scalability by isolating project configuration from other projects."
---
...
When the Isolated Projects feature is enabled, the projects in a <Xref page="multi-project-builds" section="Multi-Project Builds">multi-project build</Xref> are "isolated" from each other.
...
Follow the [recommended migration steps](#sec:migration_path) to make your build logic compatible with [Isolated Projects constraints](#sec:constraints).

:::caution
Isolated Projects is an <Xref page="feature-lifecycle" section="2. Incubating">incubating</Xref> feature.
...
- Gradle 9.7.0
  - Isolated Projects is promoted from experimental to <Xref page="feature-lifecycle" section="2. Incubating">incubating</Xref>.
  - The stable `org.gradle.isolated-projects` property names are now preferred, and the earlier `.unsafe` names are [deprecated](#sec:enable_ip).
  - The build now [fails immediately](#sec:enable_ip) on the first violation by default. Use the [Diagnostics mode](#sec:diagnostics_mode) to discover as many violations as possible in a single run.
  - Many `Gradle` APIs are now treated as incompatible when accessed during project configuration, protecting the [mutable build-scoped state](#sec:project_to_build_access).
  - The new [temporarily ignore violations](#sec:dangerously_ignore_problems) mode reports violations without failing the build, so you can evaluate performance before fixing them.
...
- Gradle 9.6.0
  ...
  - Properties and methods are [no longer looked up in parent projects](#sec:no_implicit_parents_lookup), avoiding false-positive violations and reducing project coupling.
  - The `Project.getProperties()` API is now treated as incompatible, while being <Xref page="upgrading-version-9" section="Deprecation of `getProperties()`">generally deprecated</Xref>.
  - The new [Diagnostics mode](#sec:diagnostics_mode) runs without parallelism and deterministically reports constraint violations.
  - Editor support for Kotlin build scripts and precompiled plugins in IDEA-based IDEs has been fixed.
  - The minimum supported version of the [Develocity plugin](https://scans.gradle.com/plugin/) is 4.0.
- Gradle 9.4.0
...
The main goal of Isolated Projects is to **speed up or entirely avoid running the <Xref page="build-lifecycle" section="Phase 2. Configuration">configuration phase</Xref>**.
This is [similar to the Configuration Cache](#sec:vs_cc) feature, which allows skipping configuration when you rerun a task without changing the build configuration inputs.
...
Here is how Isolated Projects affects the <Xref page="build-lifecycle" section="Build Phases">lifecycle of the build</Xref>:
...
:::note
In case of a Configuration Cache hit, the initialization and configuration are <Xref page="configuration-cache" section="Cache Hit">skipped entirely</Xref>.
...
2. Building <Xref page="tooling-api" section="Tooling API">tooling models</Xref> (this happens behind the scenes when you do an IDE sync)
...
1. Configure projects
   ...
   Configuring a project includes compiling its build script, so project build scripts are [compiled in parallel](#sec:faq_parallel_script_compilation) as well.
...
2. Discover the work graph
   ...
   An example is running checks or tests on <Xref page="command-line-interface" section="Executing tasks in multi-project builds">all projects via an unqualified task</Xref>, such as `./gradlew check`, which is typical on CI.
3. Store the work graph (Configuration Cache)

   Isolated Projects makes this step run in ***parallel***.
   This is the [same as with Parallel Configuration Cache](#sec:vs_parallel_cc),
   but with the additional safety provided by the [Isolated Projects constraints](#sec:constraints).
...
For scenarios such as IDE sync, Gradle provides a special mechanism of building <Xref page="tooling-api" section="Tooling API">tooling models</Xref>.
...
If you have already been using <Xref page="performance" section="Configure Tooling API actions parallelism">parallel model building</Xref> for IDE sync, then Isolated Projects will not provide a performance benefit in that part of the configuration phase.
...
Isolated Projects makes this safe by enforcing additional [constraints](#sec:constraints).
:::

### Parallelism controls

Much of the performance benefit of Isolated Projects comes from running project configuration in parallel.
The degree of parallelism is controlled by the <Xref page="build-environment" section="`org.gradle.workers.max=(max # of worker processes)`">`org.gradle.workers.max` Gradle property</Xref>,
which sets the maximum number of parallel units of work across all <Xref page="build-lifecycle" section="Build Lifecycle">build phases</Xref>,
...
- All projects always get configured, and [Configuration on Demand has no effect](#sec:vs_cod).
...
See [#sec:applying_plugins_from_a_lifecycle_callback](#sec:applying_plugins_from_a_lifecycle_callback) for working patterns.
- Isolated Projects violations are reported as Configuration Cache problems.

## Evaluating Isolated Projects for your build

The benefit Isolated Projects brings depends on the size and shape of your build, your hardware, and your workflow (see [performance benefits and limitations](#sec:performance)).
...
1. Enable the feature
   ...
   Make the build <Xref page="configuration-cache-adoption" section="Adopting the Configuration Cache">compatible with Configuration Cache</Xref> first, then enable Isolated Projects.
2. Get an early estimate before completing the migration

   You don’t have to fix every [violation](#sec:constraint_violations) before measuring.
   Use the [_dangerously ignore problems_ mode](#sec:dangerously_ignore_problems) to gauge the speed-up for your actual workflows — both running tasks and IDE sync — while violations are still present.
3. Compare configuration-phase times

   Measure the configuration phase with and without the feature using a <Xref page="inspect" section="Build Scan">Build Scan or profile report</Xref>.
   Keep in mind that Isolated Projects can only speed up the <Xref page="build-lifecycle" section="Phase 2. Configuration">configuration phase</Xref>, so it can only improve the build time that phase represents: the potential gain is bounded by how much of the total build time it takes up.
   The <Xref page="build-lifecycle" section="Phase 3. Execution">execution phase</Xref> duration is unaffected.
...
Isolated Projects doesn’t change how individual projects or <Xref page="composite-builds" section="Composite Builds (Included Builds)">included builds</Xref> are configured;
<Xref page="plugins" section="Scope of Plugins">plugins</Xref> still mutate the state of the target they’re applied to.
...
- In project-scope, mutable state of other projects ([cross-project access](#sec:cross_project_access))
- In project-scope, mutable state of the owner build ([project-to-build access](#sec:project_to_build_access))
- In build-scope, mutable state of other builds ([cross-build access](#sec:cross_build_access))
- In build-scope, mutable state of the build that is passed into project-scope via callbacks ([build-to-project access](#sec:build_to_project_access))

Among the build logic patterns described by these constraints, [cross-project access](#sec:cross_project_access) is by far the most common.
...
<dl>
<dt>Examples of mutable state (not exhaustive)</dt>
<dd>
...
<dt>Project hierarchy navigation</dt>
<dd>
...
<JavaDocLink class="org.gradle.api.invocation.Gradle" /> object, reachable through `project.gradle`.
...
<dl>
<dt>Examples of restricted members (not exhaustive)</dt>
<dd>
...
:::note
The `gradle.lifecycle` (<JavaDocLink class="org.gradle.api.invocation.GradleLifecycle" />) callbacks are the recommended, Isolated Projects-compatible way to register `beforeProject` and `afterProject` logic.
...
See [state-isolating callbacks](#sec:isolating_lifecycle_callbacks).
:::
...
<Xref page="build-services" section="Using Shared Build Services">Shared Build Services</Xref> are also part of the build-scoped state.
...
See the section on [state-isolating callbacks](#sec:isolating_lifecycle_callbacks) for a better alternative.
...
The Isolated Projects [constraints](#sec:constraints) validate the usages of APIs to prevent cases where intermediate results or the build outcome could have been different.
...
The corresponding <Xref page="upgrading-version-9" section="Deprecation of implicit lookup of properties and methods in parent projects">upgrade guide entry</Xref> details the cases when it is applicable.
...
Under Isolated Projects, the <Xref page="command-line-interface" section="Listing project properties">properties report</Xref>
...
This is a consequence of the [no implicit parent lookup](#sec:no_implicit_parents_lookup) behavior.

Direct uses of <JavaDocLink class="org.gradle.api.Project" method="getProperties--">`Project.getProperties()`</JavaDocLink>
continue to be reported as an Isolated Projects violation, as that API is being <Xref page="upgrading-version-9" section="Deprecation of `getProperties()`">phased out</Xref>.
...
- Make your build <Xref page="configuration-cache-adoption" section="Adopting the Configuration Cache">compatible with Configuration Cache</Xref>
...
- Minimize [behavior changes](#sec:behavior_changes) by addressing relevant deprecations
...
If you have never enabled the feature in the build before, it’s likely that the build is not compatible and contains Isolated Projects [constraint violations](#sec:constraint_violations).
In that case the build fails immediately and reports one or more violations in the error message.
To discover as many violations as possible in a single run, use the [Diagnostics mode](#sec:diagnostics_mode) described below.

### Diagnostics mode

_Diagnostics_ mode is an **opt-in** mode that helps you discover Isolated Projects [constraint violations](#sec:constraint_violations) more efficiently during migration.
...
- All projects are configured, ignoring any <Xref page="configuration-on-demand" section="Enable `configure-on-demand`">Configuration on Demand</Xref> setting.
...
To guarantee reliable build results, Gradle will fail the build when any violations of [Isolated Projects constraints](#sec:constraints) are detected.
...
This mode can be combined with [Diagnostics mode](#sec:diagnostics_mode) to prevent concurrency errors,
...
If you haven’t already, complete the [before the migration](#sec:before_migration) steps first to get the best adoption experience.

1. Learn about incompatibilities in basic workflows

   Run the `help` task with Isolated Projects in the [Diagnostics mode](#sec:diagnostics_mode):
...
3. Follow the strategies for [making build logic compatible](#sec:migration_strategies) to refactor the incompatible parts of your build
...
4. Migrate the IDE sync workflow
   ...
   For IDE sync, the feature flag and [Diagnostics mode](#sec:diagnostics_mode) must be enabled in the `gradle.properties` file to take effect.
...
<Xref page="plugins" section="Convention plugins">Convention plugins</Xref> package shared build logic so that it is defined once and then explicitly applied in each project that needs it.
This is already the Gradle <Xref page="best-practices-structuring-builds" section="Use Convention Plugins for Common Build Logic">best practice for sharing build logic</Xref>, independent of Isolated Projects.
...
[Convention plugins](#sec:convention_plugins_ip) cover most cases, but sometimes it is still desirable to apply configuration to many projects at once, from a single place.
...
The <JavaDocLink class="org.gradle.api.invocation.GradleLifecycle" /> API,
accessible via `gradle.lifecycle`, offers `beforeProject` and `afterProject` callbacks.
Actions registered as `GradleLifecycle` callbacks are **<JavaDocLink class="org.gradle.api.IsolatedAction">isolated</JavaDocLink>** and will run in an isolated context that is private to each project.

The example below shows how this API could be used in a settings script or <Xref page="plugins" section="Scope of Plugins">settings plugins</Xref> to apply configuration to all projects,
while avoiding <Xref page="sharing-build-logic-between-subprojects" section="Avoid cross-project configuration using `subprojects` and `allprojects`">cross-project configuration</Xref>:

<SampleScripts sample_name="isolated-projects-lifecycle-api" basename="settings" tag="lifecycle-before-project" source_dir="snippets/structuring-builds/isolatedProjects-lifecycleApi" />
...
Applying a plugin that is resolved lazily, such as a convention plugin from a build registered via `pluginManagement.includeBuild(…​)` or a third-party plugin from a repository, is subject to a current limitation; see [#sec:applying_plugins_from_a_lifecycle_callback](#sec:applying_plugins_from_a_lifecycle_callback).
:::
...
Moving build logic into `buildSrc` solely to sidestep this limitation is not recommended, however; Gradle best practice is to <Xref page="best-practices-structuring-builds" section="Favor `build-logic` Composite Builds for Build Logic">favor `build-logic` composite builds over `buildSrc`</Xref>.

Two workarounds are available:

- [Register `beforeProject` from a settings convention plugin](#sec:settings_convention_plugin_workaround) — move the callback registration into a settings convention plugin, where the included build’s classpath is already available. This is the recommended approach.
- [Pre-resolve in the settings `plugins {}` block](#sec:pre_resolve_in_settings_plugins_block) — declare the plugin in the settings `plugins {}` block with `apply false` to force resolution and export its classpath to every project.
...
Publish two precompiled script plugins from your included build (named `build-logic` here): the project convention plugin you want to apply, and a <Xref page="implementing-gradle-plugins-precompiled" section="Example of a precompiled script plugin in the `Settings` scope">settings convention plugin</Xref> that registers the `gradle.lifecycle.beforeProject` callback which applies it.
...
<SampleScripts sample_name="lifecycle-settings-plugin" basename="settings" tag="settings-file" source_dir="snippets/isolatedProjects/lifecycleSettingsPlugin" />
...
<SampleScripts sample_name="lifecycle-apply-false" basename="settings" tag="settings-file" source_dir="snippets/isolatedProjects/lifecycleApplyFalse" />
...
:::note
These patterns are workarounds for the current implementation limitation listed under [#sec:current_limitations](#sec:current_limitations).
:::
...
You can get a simplified view of a project by calling <JavaDocLink class="org.gradle.api.Project" method="getIsolated--">`project.isolated`</JavaDocLink>,
which returns <JavaDocLink class="org.gradle.api.project.IsolatedProject" /> type.
...
To detect whether Isolated Projects is active, inject the <JavaDocLink class="org.gradle.api.configuration.BuildFeatures" /> service and read `isolatedProjects.active`.
See <Xref page="implementing-gradle-plugins-binary" section="Reacting to build features">Reacting to build features</Xref> for details on this API.
...
<Xref page="configuration-cache" section="Configuration Cache">Configuration Cache</Xref> allows skipping the entire configuration phase when no build configuration inputs have changed.
...
You should make your build <Xref page="configuration-cache-adoption" section="Adopting the Configuration Cache">compatible with Configuration Cache</Xref> first, before migrating to Isolated Projects.
...
Configuration Cache allows enabling some parallelism in the configuration phase with <Xref page="configuration-cache-enabling" section="Enabling Parallel Configuration Caching">Parallel Configuration Cache</Xref>.
...
<Xref page="performance" section="2. Enable Parallel Execution">Parallel Execution</Xref> has to do with parallel execution of tasks.
This requires an opt-in because not all builds are compatible with this behavior, and there is no explicit validation of violations.
...
<Xref page="configuration-on-demand" section="Enable `configure-on-demand`">Configuration on Demand</Xref> lets Gradle skip configuring projects that aren’t needed for the requested tasks.
...
The <Xref page="configuration-on-demand" section="Enable `configure-on-demand`">Configuration on Demand</Xref> flag is _ignored_ when Isolated Projects is enabled.
...
The benefit comes from [parallelizing the configuration phase](#sec:performance).
...
All projects are always configured regardless of how they depend on each other, and the configuration parallelism is bounded by the project hierarchy (see [current limitations](#sec:current_limitations)).

The benefit varies from build to build, so the best way to find out what you’ll gain is to [measure it for your build](#sec:evaluating).

### Does Isolated Projects compile build scripts in parallel?

Yes.
Compiling a project’s build script is part of [configuring that project](#sec:perf_tasks).
...
This can be a noticeable cost in large multi-project builds, especially when a change to shared build logic (such as `buildSrc` or a <Xref page="composite-builds" section="Composite Builds (Included Builds)">build-logic composite build</Xref>) invalidates the compiled scripts and forces them all to be recompiled.
...
1. Violation: `tasks` is [mutable state](#sec:mutable_cross_project) of another project.
...
Use this with care.
A task path only orders the two tasks, so it fits an aggregating <Xref page="organizing-tasks" section="Lifecycle tasks">lifecycle task</Xref> that schedules work it never consumes.
If the depending task consumes what the other one produces, the data dependency is still missing, and Isolated Projects does not change the usual advice to <Xref page="best-practices-tasks" section="Avoid DependsOn">avoid `dependsOn`</Xref> and wire outputs into inputs instead.
Across project boundaries, that wiring should always go through <Xref page="multi-project-builds" section="Depending on Artifacts produced by another Project">dependency management</Xref>.