...
<SampleFile sample_name="downgrade-transitives" path="tests/dependencies.out" />
...
<SampleFile sample_name="downgrade-transitives" path="tests/dependencies-downgrade.out" />
...
<SampleScripts sample_name="downgrade-transitives" basename="build" tag="dependency-full-prefer" />

If another dependency requires ***1.10***, Gradle can resolve it without failure, as long as it falls within the specified range.

<SampleFile sample_name="downgrade-transitives" path="tests/dependencies-extra.out" />
...
<SampleScripts sample_name="downgrade-transitives" basename="build" tag="dependency-configs" />

Running `./gradlew dependencies --configuration compileClasspath` showcases the results:

<SampleFile sample_name="downgrade-transitives" path="tests/dependencies-config.out" />
