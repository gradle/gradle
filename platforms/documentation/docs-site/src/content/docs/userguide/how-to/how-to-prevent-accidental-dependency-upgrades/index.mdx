...
<SampleScripts sample_name="accidental-upgrades" basename="build" tag="dependency-full" />

Running `./gradlew dependencies --configuration runtimeClasspath` showcases the results:

<SampleFile sample_name="accidental-upgrades" path="tests/dependencies.out" />
...
<SampleScripts sample_name="accidental-upgrades" basename="build" tag="dependency-full-fail" />
...
<SampleFile sample_name="accidental-upgrades" path="tests/dependencies-fail.out" />
...
<SampleScripts sample_name="accidental-upgrades" basename="build" tag="dependency-const" />

With `strictly("3.1")`, Gradle ensures that no other version can override the specified dependency.

Running `./gradlew dependencies --configuration runtimeClasspath` showcases the results:

<SampleFile sample_name="accidental-upgrades" path="tests/dependencies-const.out" />
...
<SampleScripts sample_name="accidental-upgrades" basename="build" tag="dependency-lock" />
