...
<SampleScripts sample_name="java-platform-quickstart" basename="build" tag="use-plugin" />
...
For this purpose, the plugin exposes two <Xref page="dependency-configurations" section="Understanding dependency configurations">configurations</Xref> that can be used to declare dependencies: `api` and `runtime`.
...
<SampleScripts sample_name="java-platform-quickstart" basename="build" tag="constraints" />
...
<SampleScripts sample_name="java-platform-quickstart" basename="build" tag="allow-dependencies" />
...
<SampleScripts sample_name="multiproject" basename="build" subpath="platform" tag="project-constraints" />
...
<SampleScripts sample_name="java-platform-quickstart" basename="build" tag="platform" />
...
<SampleScripts sample_name="multiproject" basename="build" subpath="platform" tag="publishing" />
...
Because a Java Platform is a special kind of component, a dependency on a Java platform has to be declared using the `platform` or `enforcedPlatform` keyword, as explained in the <Xref page="platforms" section="Importing a platform">managing transitive dependencies</Xref> section.
...
<SampleScripts sample_name="recommender" basename="build" subpath="platform" tag="define-platform" />

And then have subprojects depend on the platform to get recommendations:

<SampleScripts sample_name="recommender" basename="build" subpath="consumer" tag="get-recommendations" />