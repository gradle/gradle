// Substitutes `%%name%%` tokens with build-time values in fenced code blocks,
// inline code, and prose text. In an MDX page:
//
//     ```bash
...
// build was given).
//
// Prose substitution exists because the converter injects tokens wherever the
// AsciiDoc source used {gradleVersion} — including sentences — and can't know
// the destination context. Hand-written (stage_2) pages may still prefer a JSX
// expression from src/config/variables.ts. Tokens in link/image URLs and raw
// HTML stay errors: substitution there is easy to get wrong silently. To show
// a token literally inside a code block, opt the fence out with
// ```bash no-substitute.
//
...
export function remarkSubstituteVariables(values: Record<string, string> = {}) {
  ...
  // Fail loudly where substitution is unsafe (URLs, raw HTML) so a stray
  // token can't ship.
  ...
  function walk(node: any): void {
    if (node.type === "code" || node.type === "inlineCode") {
      ...
      rejectTokens(node.value, "raw HTML");
    } else if (
