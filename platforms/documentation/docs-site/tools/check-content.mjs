...
/**
 * Pages with no `description` in their frontmatter.
 *
 * Descriptions are authored in `sidebar-structure.json` and copied into the
 * generated frontmatter by the converter, so a missing one is a gap in the
 * sidebar rather than in the page. They matter because `llms.txt` is built
 * from them: without a description an entry is a bare link that tells an agent
 * nothing about what the page covers.
 *
 * The corpus is complete, so this is enforced: a page without a description
 * fails the check rather than warning.
 */
function checkDescriptions(files) {
  const missing = [];
  for (const file of files) {
    const text = readFileSync(file, "utf8");
    const frontmatter = /^---\r?\n([\s\S]*?)\r?\n---/.exec(text);
    const has = frontmatter && /^description:\s*\S/m.test(frontmatter[1]);
    if (!has) missing.push(relative(docsRoot, file));
  }
  return missing;
}
...
const missingDescriptions = checkDescriptions(files);
...
if (missingDescriptions.length) {
  console.error(
    `\nMISSING DESCRIPTIONS (${missingDescriptions.length} of ${files.length}) — add them to sidebar-structure.json ` +
      `(or, for a stage_2 page, to its frontmatter):`,
  );
  for (const m of missingDescriptions.slice(0, 10)) console.error("  " + m);
  if (missingDescriptions.length > 10) console.error(`  … and ${missingDescriptions.length - 10} more`);
}
