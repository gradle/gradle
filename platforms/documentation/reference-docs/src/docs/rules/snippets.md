...
```bash
# All snippets
./gradlew :reference-docs:docsTest

# A specific snippet (both DSLs)
./gradlew :reference-docs:docsTest --tests '*catalogs-gradle-apis*'

# Kotlin DSL only
./gradlew :reference-docs:docsTest --tests '*catalogs-gradle-apis_kotlin_*'
```
...
```bash
./gradlew :reference-docs:docsTest --tests '*snippet-name*' -PenableConfigurationCacheForDocsTests=true
```
