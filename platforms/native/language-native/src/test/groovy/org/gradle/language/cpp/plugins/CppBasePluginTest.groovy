...
class CppBasePluginTest extends Specification {
    ...
    def "adds link and install task for executable"() {
        ...
        link.linkedFile.get().asFile == projectDir.file("build/exe/$exeDir" + OperatingSystemFileResolver.current().getExecutableName("test_app"))
    ...
    def "adds link task for shared library"() {
        ...
        link.linkedFile.get().asFile == projectDir.file("build/lib/${libDir}" + OperatingSystemFileResolver.current().getSharedLibraryName("test_lib"))
