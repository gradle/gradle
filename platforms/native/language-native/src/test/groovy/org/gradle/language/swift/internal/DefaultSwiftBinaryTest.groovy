...
class DefaultSwiftBinaryTest extends Specification {
    ...
    def setup() {
        _ * configurations.resolvableDependencyScope("swiftCompileDebug") >> Providers.ofNamed(compile)
        _ * configurations.resolvableDependencyScope("nativeLinkDebug") >> Providers.ofNamed(link)
        _ * configurations.resolvable('nativeRuntimeDebug', _) >> Providers.ofNamed(runtime)
