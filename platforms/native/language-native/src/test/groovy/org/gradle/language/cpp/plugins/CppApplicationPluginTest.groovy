...
class CppApplicationPluginTest extends Specification {
    ...
    def "adds compile, link and install tasks"() {
        ...
        linkDebug.linkedFile.get().asFile == projectDir.file("build/exe/main/debug/" + OperatingSystemFileResolver.current().getExecutableName("testApp"))
        ...
        installDebug.runScriptFile.get().getAsFile().name == OperatingSystemFileResolver.current().getScriptName("testApp")
        ...
        linkRelease.linkedFile.get().asFile == projectDir.file("build/exe/main/release/" + OperatingSystemFileResolver.current().getExecutableName("testApp"))
        ...
        installRelease.runScriptFile.get().getAsFile().name == OperatingSystemFileResolver.current().getScriptName("testApp")
    }
    ...
    def "output locations are calculated using base name defined on extension"() {
        ...
        link.linkedFile.get().asFile == projectDir.file("build/exe/main/debug/" + OperatingSystemFileResolver.current().getExecutableName("test_app"))

        def install = project.tasks.installDebug
        install.installDirectory.get().asFile == projectDir.file("build/install/main/debug")
        install.runScriptFile.get().getAsFile().name == OperatingSystemFileResolver.current().getScriptName("test_app")
    }
    ...
    def "output locations reflects changes to buildDir"() {
        ...
        def link = project.tasks.linkDebug
        link.linkedFile.get().asFile == projectDir.file("output/exe/main/debug/" + OperatingSystemFileResolver.current().getExecutableName("testApp"))
