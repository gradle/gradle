...
public class NativeBinaries {
    ...
    public static void createNativeBinaries(
        ...
        if (component instanceof NativeLibrarySpec) {
            ...
            createNativeBinary(
                    NativeExecutableBinarySpec.class,
                    binaries,
                    resolver,
                    fileCollectionFactory,
    ...
    private static <T extends NativeBinarySpec> void createNativeBinary(
        ...
        // We can't just assign the toolchain here because the initializer would be closing over the toolchain which is
        // not reusable, and this breaks model reuse.
        // So here we are just closing over the safely reusable things and then using proper dependencies for the tool
        // chain registry.
