...
public abstract class PrefixHeaderFileGenerateTask extends DefaultTask {
    ...
    abstract static class GeneratePrefixHeaderFile implements WorkAction<PrefixHeaderFileParameters> {
        @Inject
        public GeneratePrefixHeaderFile() {}

        @Override
        public void execute() {
            PCHUtils.generatePrefixHeaderFile(
                    Collections.singletonList(getParameters().getHeader().get()),
                    getParameters().getPrefixHeaderFile().getAsFile().get());
        }
    }
}
