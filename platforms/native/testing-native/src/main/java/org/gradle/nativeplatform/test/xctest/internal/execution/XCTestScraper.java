...
class XCTestScraper implements TextStream {
    ...
    XCTestScraper(
            ...
            Deque<XCTestDescriptor> testDescriptors,
            Map<String, Object> testSuiteIds) {
        ...
        synchronized (testDescriptors) {
            ...
            if (scanner.hasNext()) {
                ...
                if (token.equals("Test Suite")) {
                    ...
                    if (testSuite.equals("All tests")
                            || testSuite.equals("Selected tests")
                            || testSuite.endsWith(".xctest")) {
                    ...
                    if (started) {
                        TestDescriptorInternal testDescriptor = new DefaultTestClassDescriptor(
                                idGenerator.generateId(),
                                testSuite); // Using DefaultTestClassDescriptor to fake JUnit test
                        ...
                        processor.completed(
                                testDescriptor.getId(), new TestCompleteEvent(clock.getCurrentTime(), resultType));
                    ...
                    // (macOS) Looks like: Test Case '-[AppTest.PassingTestSuite testCanPassTestCaseWithAssertion]'
                    // started.
                    // (Linux) Looks like: Test Case 'PassingTestSuite.testCanPassTestCaseWithAssertion' started.
                    String testSuiteAndCase = scanner.next();
                    String[] splits =
                            testSuiteAndCase.replace('[', ' ').replace(']', ' ').split("[. ]");
                    String testSuite;
                    String testCase;
                    ...
                    if (started) {
                        TestDescriptorInternal testDescriptor =
                                new DefaultTestMethodDescriptor(idGenerator.generateId(), testSuite, testCase);
                        ...
                        if (failed) {
                            resultType = TestResult.ResultType.FAILURE;
                            Throwable failure = new Throwable(Joiner.on(TextUtil.getPlatformLineSeparator())
                                    .join(xcTestDescriptor.getMessages()));
                    ...
                    if (xcTestDescriptor != null) {
                        ...
                        processor.output(
                                testDescriptor.getId(),
                                new DefaultTestOutputEvent(clock.getCurrentTime(), destination, text));
                        ...
                        if (failureMessageMatcher.find()) {
                            ...
                            if (testDescriptor.getClassName().equals(testSuite)
                                    && testDescriptor.getName().equals(testCase)) {
                        ...
                        processor.output(
                                lastDescriptor.getId(),
                                new DefaultTestOutputEvent(clock.getCurrentTime(), destination, text));
                    } else {
                        // If there is no known last descriptor, associate it with the root test suite
                        processor.output(
                                rootTestSuiteId, new DefaultTestOutputEvent(clock.getCurrentTime(), destination, text));
                    }
                }
            }
        }
