...
public abstract class JvmProjectInitDescriptor extends LanguageLibraryProjectInitDescriptor {
    ...
    public Optional<String> getFurtherReading(InitSettings settings) {
        ...
        return Optional.of(documentationRegistry.getSampleForMessage(
                "building_" + getLanguage().getName() + "_" + getComponentType().pluralName() + multi));
    }
    ...
    public void generateProjectBuildScript(
        ...
        description.getPluginName().ifPresent(languagePlugin -> {
            ...
            String pluginVersion =
                    pluginVersionProperty == null ? null : libraryVersionProvider.getVersion(pluginVersionProperty);
            buildScriptBuilder.plugin(
                    "Apply the " + languagePlugin + " Plugin to add support for " + getLanguage() + ".",
                    languagePlugin,
                    pluginVersion,
                    description.getExplicitPluginAlias());
        });
        ...
        buildScriptBuilder
                .fileComment("This generated file contains a sample " + getLanguage() + " " + getComponentType()
                        + " project to get you started.")
                .fileComment(documentationRegistry.getDocumentationRecommendationFor(
                        "details on building Java & JVM projects", "building_java_projects"));
        ...
        if (settings.isUseTestSuites()) {
            ...
            if (getLanguage() != Language.GROOVY && settings.getTestFramework() == BuildInitTestFramework.SPOCK) {
                buildScriptBuilder.plugin(
                        "Apply the groovy plugin to also add support for Groovy (needed for Spock)", "groovy");
            }
    ...
    public void generateSources(InitSettings settings, TemplateFactory templateFactory) {
        for (String subproject : settings.getSubprojects()) {
            ...
            List<TemplateOperation> templateOps = sourceTemplates.stream()
                    .map(t -> templateFactory.fromSourceTemplate(
                            templatePath(t), "main", subproject, templateLanguage(t)))
                    .collect(toList());
            testSourceTemplates.stream()
                    .map(t -> templateFactory.fromSourceTemplate(
                            templatePath(t), "test", subproject, templateLanguage(t)))
    ...
    protected abstract List<String> getSourceTemplates(
            String subproject, InitSettings settings, TemplateFactory templateFactory);

    protected abstract List<String> getTestSourceTemplates(
            String subproject, InitSettings settings, TemplateFactory templateFactory);

    protected void applyApplicationPlugin(BuildScriptBuilder buildScriptBuilder) {
        buildScriptBuilder.plugin(
                "Apply the application plugin to add support for building a CLI application in Java.", "application");
    }

    protected void applyLibraryPlugin(BuildScriptBuilder buildScriptBuilder) {
        buildScriptBuilder.plugin(
                "Apply the java-library plugin for API and implementation separation.", "java-library");
    }
            ...
            case GROOVY:
                ...
                buildScriptBuilder.implementationDependency(
                        ...
                        BuildInitDependency.of(
                                groovyGroupName(groovyVersion) + ":groovy-all",
                                constraintsDefined ? null : groovyVersion));
                break;
            case SCALA:
                ...
                buildScriptBuilder.implementationDependency(
                        ...
                        BuildInitDependency.of(
                                "org.scala-lang:scala-library", constraintsDefined ? null : scalaLibraryVersion));
                break;
            default:
                break;
        }
    ...
    private void addDependencyConstraints(BuildScriptBuilder buildScriptBuilder) {
        ...
        if (getLanguage() == Language.GROOVY) {
            String groovyVersion = libraryVersionProvider.getVersion("groovy");
            buildScriptBuilder.implementationDependencyConstraint(
                    null, BuildInitDependency.of(groovyGroupName(groovyVersion) + ":groovy-all", groovyVersion));
        }
        if (getLanguage() == Language.SCALA) {
            String scalaLibraryVersion = libraryVersionProvider.getVersion("scala-library");
            buildScriptBuilder.implementationDependencyConstraint(
                    null, BuildInitDependency.of("org.scala-lang:scala-library", scalaLibraryVersion));
        }
    }
        ...
        switch (testFramework) {
            case SPOCK:
                ...
                buildScriptBuilder.taskMethodInvocation(
                        "Use JUnit Platform for unit tests.", "test", "Test", "useJUnitPlatform");
                break;
            case TESTNG:
                buildScriptBuilder
                        .testImplementationDependency(
                        ...
                        .taskMethodInvocation("Use TestNG for unit tests.", "test", "Test", "useTestNG");
                break;
            case JUNIT_JUPITER:
                buildScriptBuilder.testImplementationDependency(
