 ...
 *
 * <p>If your organization does not allow binary files in version control, you may omit the JAR: the generated
 * scripts download it on demand from {@link #getWrapperJarUrl()} (see
 * <a href="https://github.com/gradle/gradle/issues/11816">issue #11816</a>), similar to the Maven Wrapper.
 * @since 0.7
 */
...
public abstract class Wrapper extends DefaultTask {
    ...
    void generate() {
        ...
        validateWrapperJarUrl(propertiesFile.getParentFile());
    ...
    private static final String WRAPPER_JAR_URL_EXCEPTION_MESSAGE = "Test of wrapper jar url %s failed. Please check the value set with --gradle-wrapper-jar-url.";
    ...
    private void validateWrapperJarUrl(File uriRoot) {
        if (wrapperJarUrlConfigured && getValidateDistributionUrl().get()) {
            ...
            if (uri.getScheme().equals("file")) {
                if (!Files.exists(Paths.get(uri).toAbsolutePath())) {
                    throw UncheckedException.throwAsUncheckedException(new IOException(String.format(WRAPPER_JAR_URL_EXCEPTION_MESSAGE, url)), true);
                }
            } else if (uri.getScheme().startsWith("http") && !isOffline) {
                try {
                    new Download(new Logger(true), "gradlew", Download.UNKNOWN_VERSION).sendHeadRequest(uri);
                } catch (Exception e) {
                    throw UncheckedException.throwAsUncheckedException(new IOException(String.format(WRAPPER_JAR_URL_EXCEPTION_MESSAGE, url), e), true);
                }
            }
        }
    }
     ...
     *
     * <p>If not set, the download URL is derived from the {@link #getDistributionUrl()} (preserving a custom
     * distribution host when it uses the standard {@code -bin.zip}/{@code -all.zip} layout) or defaults to the
     * official Gradle distribution location for the specified {@link #getGradleVersion()}.
     *
     * <p>The wrapper scripts ({@code gradlew}/{@code gradlew.bat}) use this URL to download a missing
     * {@code gradle-wrapper.jar}, so the JAR does not have to be checked into version control.
     * See <a href="https://github.com/gradle/gradle/issues/11816">issue #11816</a>.
     *
     * @since 9.9.0
     */
    @Input
