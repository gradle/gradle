...
public abstract class JvmGradlePluginProjectInitDescriptor extends LanguageLibraryProjectInitDescriptor {
    ...
    public JvmGradlePluginProjectInitDescriptor(
            DocumentationRegistry documentationRegistry, TemplateLibraryVersionProvider libraryVersionProvider) {
    ...
    public void generateProjectBuildScript(
        ...
        String pluginClassName = StringUtils.capitalize(TextUtil.toCamelCase(settings.getProjectName())) + "Plugin";
        ...
        buildScriptBuilder
                ...
                .fileComment(documentationRegistry.getDocumentationRecommendationFor(
                        "details on writing Custom Plugins", "custom_plugins"));

        buildScriptBuilder.plugin(
                "Apply the Java Gradle plugin development plugin to add support for developing Gradle plugins",
                "java-gradle-plugin");

        buildScriptBuilder.block(
                null,
                "gradlePlugin",
                b -> b.containerElement("Define the plugin", "plugins", "greeting", null, g -> {
        ...
        if (settings.isUseTestSuites()) {
            ...
            functionalTestSourceSet = buildScriptBuilder.createContainerElement(
                    "Add a source set for the functional test suite",
                    "sourceSets",
                    "functionalTest",
                    "functionalTestSourceSet");

            BuildScriptBuilder.Expression functionalTestImplementation =
                    buildScriptBuilder.containerElementExpression("configurations", "functionalTestImplementation");
            BuildScriptBuilder.Expression testImplementation =
                    buildScriptBuilder.containerElementExpression("configurations", "testImplementation");
            buildScriptBuilder.methodInvocation(null, functionalTestImplementation, "extendsFrom", testImplementation);

            BuildScriptBuilder.Expression functionalTestRuntimeOnly =
                    buildScriptBuilder.containerElementExpression("configurations", "functionalTestRuntimeOnly");
            BuildScriptBuilder.Expression testRuntimeOnly =
                    buildScriptBuilder.containerElementExpression("configurations", "testRuntimeOnly");
            buildScriptBuilder.methodInvocation(null, functionalTestRuntimeOnly, "extendsFrom", testRuntimeOnly);

            BuildScriptBuilder.Expression functionalTest = buildScriptBuilder.taskRegistration(
                    "Add a task to run the functional tests", "functionalTest", "Test", b -> {
                        b.propertyAssignment(
                                null,
                                "testClassesDirs",
                                buildScriptBuilder.propertyExpression(functionalTestSourceSet, "output.classesDirs"),
                                true);
                        b.propertyAssignment(
                                null,
                                "classpath",
                                buildScriptBuilder.propertyExpression(functionalTestSourceSet, "runtimeClasspath"),
                                true);
                        b.methodInvocation(null, "useJUnitPlatform");
                    });
            buildScriptBuilder.taskMethodInvocation(
                    "Run the functional tests as part of `check`", "check", "Task", "dependsOn", functionalTest);
            buildScriptBuilder.taskMethodInvocation(
                    "Use JUnit Jupiter for unit tests.", "test", "Test", "useJUnitPlatform");
        }
    ...
    public void generateSources(InitSettings settings, TemplateFactory templateFactory) {
        ...
        TemplateOperation functionalTestTemplate =
                functionalTestTemplate(settings, templateFactory, pluginId, functionalTestClassName);
        templateFactory
                .whenNoSourcesAvailable(sourceTemplate, testTemplate, functionalTestTemplate)
                .generate();
    }
