...
public abstract class AntlrPlugin implements Plugin<Project> {
    ...
    public void apply(final Project project) {
        ...
        NamedDomainObjectProvider<? extends Configuration> antlrConfiguration = ((ProjectInternal) project).getConfigurations().resolvableDependencyScope(ANTLR_CONFIGURATION_NAME, conf -> {
            conf.defaultDependencies(dependencies -> dependencies.add(project.getDependencies().create("antlr:antlr:2.7.7@jar")));
        });

        JavaPluginHelper.getJavaComponent(project).getMainFeature().getApiConfiguration().extendsFrom(antlrConfiguration.get());

        // Wire the antlr configuration into all antlr tasks
        project.getTasks().withType(AntlrTask.class).configureEach(antlrTask -> antlrTask.getConventionMapping().map("antlrClasspath", antlrConfiguration::get));
