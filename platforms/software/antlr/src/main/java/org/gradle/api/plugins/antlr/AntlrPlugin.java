...
public abstract class AntlrPlugin implements Plugin<Project> {
    ...
    public void apply(final Project project) {
        ...
        Configuration antlrConfiguration = ((ProjectInternal) project)
                .getConfigurations()
                .resolvableDependencyScopeLocked(ANTLR_CONFIGURATION_NAME, conf -> {
        ...
        JavaPluginHelper.getJavaComponent(project)
                .getMainFeature()
                .getApiConfiguration()
                .extendsFrom(antlrConfiguration);

        // Wire the antlr configuration into all antlr tasks
        project.getTasks().withType(AntlrTask.class).configureEach(antlrTask -> antlrTask
                .getConventionMapping()
                .map("antlrClasspath", () -> project.getConfigurations().getByName(ANTLR_CONFIGURATION_NAME)));
        ...
        project.getExtensions()
                ...
                .all(new Action<SourceSet>() {
                    ...
                    public void execute(final SourceSet sourceSet) {
                        ...
                        AntlrSourceDirectorySet antlrSourceSet = createAntlrSourceDirectorySet(
                                ((DefaultSourceSet) sourceSet).getDisplayName(), objectFactory);
                        sourceSet
                                .getExtensions()
                                .add(AntlrSourceDirectorySet.class, AntlrSourceDirectorySet.NAME, antlrSourceSet);
                        ...
                        final String outputDirectoryName =
                                project.getBuildDir() + "/generated-src/antlr/" + sourceSet.getName();
                        final File outputDirectory = new File(outputDirectoryName);

                        // 4) Register a source-generating task, and
                        TaskProvider<AntlrTask> antlrTask = project.getTasks()
                                .register(taskName, AntlrTask.class, task -> {
                                    task.setDescription("Processes the " + sourceSet.getName() + " Antlr grammars.");
                                    task.setGroup("antlr");
                                    // 4.1) set up convention mapping for default sources (allows user to not have to
                                    // specify)
                        ...
                        sourceSet.getJava().srcDir(antlrTask.map(task -> {
                            ...
                            return project.file(
                                    deriveGeneratedSourceRootDirectory(relativeOutputDirectory, task.getArguments()));
                        }));
                    }
                });
    }
    ...
    private static AntlrSourceDirectorySet createAntlrSourceDirectorySet(
        ...
        AntlrSourceDirectorySet antlrSourceSet = objectFactory.newInstance(
                DefaultAntlrSourceDirectorySet.class, objectFactory.sourceDirectorySet(name, displayName));
