...
class TestReportDataCollectorSpec extends Specification {
    ...
    def collector = new TestReportDataCollector(writer)
    ...
    def "keeps track of test results"() {
        ...
        def result2 = new DefaultTestResult(FAILURE, 250, 300, 1, 0, 1, asList(TestFailure.fromTestFrameworkFailure(new RuntimeException("Boo!"))))

        when:
        collector.beforeSuite(root)
        collector.beforeSuite(clazz)
        ...
        collector.afterSuite(clazz, new DefaultTestResult(FAILURE, 0, 500, 2, 1, 1, []))
        collector.afterSuite(root, new DefaultTestResult(FAILURE, 0, 500, 2, 1, 1, []))
        PersistentTestResultTree results = collector.rootTree

        then:
        results.children.size() == 1
        def fooTest = results.children.first()
        fooTest.result.name == 'FooTest'
        fooTest.result.startTime == 0
        fooTest.result.endTime == 500
        fooTest.children.size() == 2
        fooTest.children[0].result.name == 'testMethod'
        fooTest.children[0].result.startTime == 100
        fooTest.children[0].result.endTime == 200
        fooTest.children[0].result.resultType == SUCCESS
        fooTest.children[1].result.name == 'testMethod2'
        fooTest.children[1].result.startTime == 250
        fooTest.children[1].result.endTime == 300
        fooTest.children[1].result.resultType == FAILURE
    }
    ...
    def "writes test outputs for interleaved tests"() {
        ...
        then:
        1 * writer.onOutput(0, new DefaultTestOutputEvent(1, StdErr, "err-1"))
        1 * writer.onOutput(1, new DefaultTestOutputEvent(2, StdOut, "out-2"))
        1 * writer.onOutput(0, new DefaultTestOutputEvent(3, StdOut, "out-1"))
        0 * writer._
    }
    ...
    def "writes test outputs for class"() {
        ...
        when:
        collector.beforeSuite(testClass)
        collector.onOutput(testClass, new DefaultTestOutputEvent(2, StdErr, "err-1"))
        collector.onOutput(testClass, new DefaultTestOutputEvent(3, StdErr, "err-2"))

        then:
        1 * writer.onOutput(0, new DefaultTestOutputEvent(2, StdErr, "err-1"))
        1 * writer.onOutput(0, new DefaultTestOutputEvent(3, StdErr, "err-2"))
        0 * writer._
    }

    def "writes test outputs for failed suite"() {
        def suite = new DefaultTestSuiteDescriptor("1", "Suite")
        def failure = TestFailure.fromTestFrameworkFailure(new RuntimeException("failure"))
        def result = new DefaultTestResult(FAILURE, 0, 0, 0, 0, 0, [failure])

        when:
        collector.beforeSuite(suite)
        collector.onOutput(suite, new DefaultTestOutputEvent(1, StdOut, "suite-out"))
        collector.afterSuite(suite, result)

        then:
        1 * writer.onOutput(_, new DefaultTestOutputEvent(1, StdOut, "suite-out"))
        0 * writer._
    }

    def "collects failures for test"() {
        def test = new DefaultTestDescriptor("1.1.1", "FooTest", "testMethod")
        def failure1 = TestFailure.fromTestFrameworkFailure(new RuntimeException("failure1"))
        def failure2 = TestFailure.fromTestFrameworkFailure(new IOException("failure2"))
        ...
        then:
        def failures = results.result.failures
        failures.size() == 2
    ...
    def "handle PlaceholderExceptions for test failures"() {
        def test = new DefaultTestDescriptor("1.1.1", "FooTest", "testMethod")
        def failure = TestFailure.fromTestFrameworkFailure(new PlaceholderException("OriginalClassName", "failure2", null, "toString()", null, null))
    ...
    def "handles exception whose toString() method fails"() {
        def test = new DefaultTestDescriptor("1.1.1", "FooTest", "testMethod")
        def failure2 = TestFailure.fromTestFrameworkFailure(new RuntimeException("failure2"))
        def failure1 = TestFailure.fromTestFrameworkFailure(new RuntimeException("failure1") {
            @Override
            String toString() {
    ...
    def "reports suite failures"() {
        def root = new DefaultTestSuiteDescriptor("1", "Suite")
        def testWorker = new DecoratingTestDescriptor(new DefaultTestSuiteDescriptor("2", "Test Worker 1"), root)

        when:
        //simulating a scenario with suite failing badly enough so that no tests are executed
        collector.beforeSuite(root)
        collector.beforeSuite(testWorker)
        collector.afterSuite(testWorker, new DefaultTestResult(FAILURE, 50, 450, 2, 1, 1, [TestFailure.fromTestFrameworkFailure(new RuntimeException("Boo!"))]))
        collector.afterSuite(root, new DefaultTestResult(FAILURE, 0, 500, 2, 1, 1, []))
        PersistentTestResultTree results = collector.rootTree

        then:
        def result = results.children.first().result
        result.name == 'Test Worker 1'
        result.startTime == 50
        result.duration == 400
