...
abstract class GenericPageRenderer<T extends GenericCompositeTestResults> extends TabbedPageRenderer<T> {
    private static final URL STYLE_URL = GenericPageRenderer.class.getResource("style.css");
    ...
    private void renderTestResultList(SimpleHtmlWriter htmlWriter, Set<GenericTestResult> failures) throws IOException {
        htmlWriter.startElement("ul").attribute("class", "linkList");
        for (GenericTestResult test : failures) {
            htmlWriter.startElement("li");
            htmlWriter.startElement("a")
                .attribute("href", asHtmlLinkEncoded(getResults().getUrlTo(test.getParentResults())))
                .characters(test.getParentResults().getReportName())
                .endElement();
            htmlWriter.characters(".");
            String link = asHtmlLinkEncoded(getResults().getUrlTo(test.getParentResults())) + "#" + test.getName();
            htmlWriter.startElement("a")
                .attribute("href", link)
    ...
    protected ReportRenderer<T, SimpleHtmlWriter> getHeaderRenderer() {
        return new ReportRenderer<T, SimpleHtmlWriter>() {
            ...
            public void render(T model, SimpleHtmlWriter htmlWriter) throws IOException {
                GenericPageRenderer.this.results = model;
