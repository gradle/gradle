...
class DefaultTestEventReporter implements TestEventReporterInternal {
    ...
    public void failed(Instant endTime, List<TestFailure> failures) {
        ...
        listener.completed(testDescriptor, new DefaultTestResult(TestResult.ResultType.FAILURE, startTime, endTime.toEpochMilli(), testResultState.getTotalCount(), testResultState.getSuccessfulCount(), testResultState.getFailureCount(), failures), new TestCompleteEvent(endTime.toEpochMilli(), TestResult.ResultType.FAILURE));
    }
