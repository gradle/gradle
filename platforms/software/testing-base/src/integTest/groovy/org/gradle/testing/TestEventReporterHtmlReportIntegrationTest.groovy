...
class TestEventReporterHtmlReportIntegrationTest extends AbstractIntegrationSpec implements VerifiesGenericTestReportResults {
    ...
    def "prints aggregate test results URL if multiple test tasks fail"() {
        ...
        then:
        failure.assertHasDescription("Execution failed for task ':failing1' (created in build file 'build.gradle').")
        ...
        failure.assertHasDescription("Execution failed for task ':failing2' (created in build file 'build.gradle').")
