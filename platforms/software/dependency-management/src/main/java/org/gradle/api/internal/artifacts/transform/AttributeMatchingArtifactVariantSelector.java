...
public class AttributeMatchingArtifactVariantSelector implements ArtifactVariantSelector {
    ...
    public ResolvedArtifactSet select(ResolvedVariantSet producer, ImmutableAttributes requestAttributes, boolean allowNoMatchingVariants, ResolvedArtifactTransformer resolvedArtifactTransformer) {
        try {
            ...
            return new BrokenResolvedArtifactSet(failureProcessor.unknownArtifactVariantSelectionFailure(schema, producer.getComponentIdentifier(), producer.asDescribable().getDisplayName(), requestAttributes, t));
        }
    }
    ...
    private ResolvedArtifactSet doSelect(ResolvedVariantSet producer, boolean allowNoMatchingVariants, ResolvedArtifactTransformer resolvedArtifactTransformer, AttributeMatchingExplanationBuilder explanationBuilder, ImmutableAttributes requestAttributes) {
        ...
        if (matches.size() == 1) {
            ...
            throw failureProcessor.ambiguousArtifactsFailure(schema, matcher, producer.getComponentIdentifier(), producer.asDescribable().getDisplayName(), componentRequested, matches);
        }
        ...
        if (!transformedVariants.isEmpty()) {
            throw failureProcessor.ambiguousArtifactTransformsFailure(schema, producer.getComponentIdentifier(), producer.asDescribable().getDisplayName(), componentRequested, transformedVariants);
        }
        ...
        throw failureProcessor.noCompatibleArtifactFailure(schema, matcher, producer.getComponentIdentifier(), producer.asDescribable().getDisplayName(), componentRequested, variants);
    }
