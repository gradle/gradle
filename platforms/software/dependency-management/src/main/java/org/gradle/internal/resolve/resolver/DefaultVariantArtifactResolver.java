...
public class DefaultVariantArtifactResolver implements VariantArtifactResolver {
    ...
    @Override
    public ResolvedVariant resolveAdhocVariant(
        ...
        VariantResolveMetadata.Identifier identifier = artifacts.size() == 1
                ? new SingleArtifactVariantIdentifier(
                        artifacts.iterator().next().getId())
                : null;
    ...
    public ResolvedVariant resolveVariantArtifactSet(
        ...
        // TODO #31538: In order to apply the artifact type registry, we need to realize the artifacts now, earlier than
        // we should.
        // Since the artifact type registry must be applied before artifact selection, which occurs before task
        // dependencies
        // execute, and since the artifact type registry is a function of the artifacts themselves, which are only known
        // after task
        ...
        if (artifactSetId == null || !variantArtifacts.isEligibleForCaching()) {
            return createResolvedVariant(
                    artifactSetId, sourceVariantId, component, variantArtifacts, artifactTypeRegistry, artifacts);
        }

        // We use the artifact type registry as a key here, since for each consumer the registry may be different.
        // The registry is interned and is safe to be used as a cache key. Ideally, we would do away with the concept of
        // the
        // artifact type registry entirely, as by design it means we need to look at the artifacts of a variant in order
        // to perform
        ...
        return resolvedVariantCache.computeIfAbsent(
                key,
                k -> createResolvedVariant(
                        k.variantIdentifier,
                        sourceVariantId,
                        component,
                        variantArtifacts,
                        k.artifactTypeRegistry,
                        artifacts));
    }
    ...
    private ResolvedVariant createResolvedVariant(
        ...
        ImmutableAttributes attributes =
                artifactTypeRegistry.mapAttributesFor(artifactVariant.getAttributes(), artifacts);
        ImmutableCapabilities capabilities = withImplicitCapability(artifactVariant.getCapabilities(), component);

        // TODO: This value gets cached in a build-tree-scoped cache. It captures a project-scoped `artifactResolver`,
        // which
        // is bound to the repositories of the consumer. That means subsequent resolutions of this artifact from a
        // different
        // project will use the same resolver as the first resolution -- leading us to use repositories from another
        // project
        // when resolving artifacts in this project. We disallow caching of external artifacts above with
        // `isEligibleForCaching`,
        // but if we enable caching for external artifacts, we need to make sure that the `artifactResolver` is included
        // as a
        // cache key or is provided later on during artifact resolution.

        // Better yet, the state required to resolve a variant (the `artifactResolver`) should be captured in the
        // original
        // component artifact metadata, meaning the variant identity is tied to where it is resolved from. This way, we
        // do
        // not need to track the resolver separately, and we can ensure the resolver that resolved a component's
        // metadata is
        // the same one that resolves its artifacts. This would benefit greatly from "repository deduplication", where
        // we could
        // consider repositories from multiple projects as equivalent as long as they are configured the same (same url,
        // cache policy,
        // component metadata rules, metadata sources, etc.). We should probably leverage
        // ComponentArtifactResolveMetadata#getSources() for this.
    ...
    private static ImmutableCapabilities withImplicitCapability(
            ImmutableCapabilities capabilities, ComponentArtifactResolveMetadata component) {
        // TODO: This doesn't seem right. We should know the capability of the variant before we get here instead of
        // assuming that it's the same as the owner
        if (capabilities.asSet().isEmpty()) {
            return ImmutableCapabilities.of(
                    DefaultImmutableCapability.defaultCapabilityForComponent(component.getModuleVersionId()));
        } else {
            return capabilities;
        }
    }
