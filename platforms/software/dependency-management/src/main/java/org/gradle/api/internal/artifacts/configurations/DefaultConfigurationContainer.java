...
// TODO: We should eventually consider making the DefaultConfigurationContainer extend
...
public class DefaultConfigurationContainer extends AbstractValidatingNamedDomainObjectContainer<Configuration>
    ...
    private static final Pattern RESERVED_NAMES_FOR_DETACHED_CONFS =
            Pattern.compile(DETACHED_CONFIGURATION_DEFAULT_NAME + "\\d*");
    ...
    protected Configuration doCreate(String name) {
        ...
        return defaultConfigurationFactory.create(
                name, false, resolver, resolutionStrategyFactory, ConfigurationRoles.ALL);
    }

    @Override
    protected NamedDomainObjectProvider<Configuration> createDomainObjectProvider(
            String name, @Nullable Action<? super Configuration> configurationAction) {
        ...
        NamedDomainObjectProvider<Configuration> provider = Cast.uncheckedCast(getInstantiator()
                .newInstance(
                        AbstractNamedDomainObjectContainer.NamedDomainObjectCreatingProvider.class,
                        DefaultConfigurationContainer.this,
                        name,
    ...
    public void visitConsumable(Consumer<ConfigurationInternal> visitor) {
        ...
        // We need to copy the configurations in case visiting the configuration causes more configurations to be
        // realized.
        Collection<ConsumableConfiguration> availableConsumableConfigurations =
                new ArrayList<>(withType(ConsumableConfiguration.class));
        availableConsumableConfigurations.forEach(
                configuration -> visitor.accept((ConfigurationInternal) configuration));

        // Then, copy and visit any configuration with unknown role, checking if it is consumable
        // We need to copy the configurations in case visiting the configuration causes more configurations to be
        // realized.
        Collection<LegacyConfiguration> availableLegacyConfigurations =
                new ArrayList<>(withType(LegacyConfiguration.class).matching(Configuration::isCanBeConsumed));
    ...
    public boolean add(@Nullable Configuration o) {
        throw failOnAttemptToAdd(
                "Adding a configuration directly to the configuration container is not allowed.  Use a factory method instead to create a new configuration in the container.");
    }

    @Override
    public boolean addAll(Collection<? extends Configuration> c) {
        throw failOnAttemptToAdd(
                "Adding a collection of configurations directly to the configuration container is not allowed.  Use a factory method instead to create a new configuration in the container.");
    }

    @Override
    public void addLater(Provider<? extends Configuration> provider) {
        throw failOnAttemptToAdd(
                "Adding a configuration provider directly to the configuration container is not allowed.  Use a factory method instead to create a new configuration in the container.");
    }

    @Override
    public void addAllLater(Provider<? extends Iterable<Configuration>> provider) {
        throw failOnAttemptToAdd(
                "Adding a provider of configurations directly to the configuration container is not allowed.  Use a factory method instead to create a new configuration in the container.");
    }
    ...
    public ConfigurationInternal detachedConfiguration(Dependency... dependencies) {
        ...
        DefaultLegacyConfiguration detachedConfiguration =
                defaultConfigurationFactory.create(name, true, resolver, resolutionStrategyFactory, role);
    ...
    public NamedDomainObjectProvider<ResolvableConfiguration> resolvable(String name) {
        ...
        return registerConfiguration(
                name, ResolvableConfiguration.class, this::doCreateResolvable, Actions.doNothing());
    }

    @Override
    public NamedDomainObjectProvider<ResolvableConfiguration> resolvable(
    ...
    public NamedDomainObjectProvider<ConsumableConfiguration> consumable(String name) {
        ...
        return registerConfiguration(
                name, ConsumableConfiguration.class, this::doCreateConsumable, Actions.doNothing());
    }

    @Override
    public NamedDomainObjectProvider<ConsumableConfiguration> consumable(
    ...
    public NamedDomainObjectProvider<DependencyScopeConfiguration> dependencyScope(String name) {
        ...
        return registerConfiguration(
                name, DependencyScopeConfiguration.class, this::doCreateDependencyScope, Actions.doNothing());
    }

    @Override
    public NamedDomainObjectProvider<DependencyScopeConfiguration> dependencyScope(
    ...
    @Override
    public DefaultDependencyScopeConfiguration dependencyScopeLocked(
            String name, Action<? super Configuration> action) {
    ...
    public Configuration resolvableDependencyScopeLocked(String name) {
        ...
        return createLockedLegacyConfiguration(
                name, ConfigurationRoles.RESOLVABLE_DEPENDENCY_SCOPE, Actions.doNothing());
    }
    ...
    private RuntimeException failOnReservedName(String confName) {
        GradleException ex = new GradleException("The configuration " + confName
                + " was created explicitly. This configuration name is reserved for creation by Gradle.");
        ProblemId id = ProblemId.create(
                "unexpected configuration usage",
                "Unexpected configuration usage",
    ...
    private ConfigurationInternal createLockedLegacyConfiguration(
            String name, ConfigurationRole role, Action<? super Configuration> configureAction) {
    ...
    private <T extends ConfigurationInternal> T createLockedConfiguration(
            String name, Function<String, T> factory, Action<? super Configuration> configureAction) {
        assertElementNotPresent(name);
        validateNameIsAllowed(name);
    ...
    private <T extends Configuration> NamedDomainObjectProvider<T> registerConfiguration(
            String name, Class<T> publicType, Function<String, T> factory, Action<? super T> configureAction) {
        assertElementNotPresent(name);
        validateNameIsAllowed(name);

        NamedDomainObjectProvider<T> configuration = Cast.uncheckedCast(getInstantiator()
                .newInstance(
                        NamedDomainObjectCreatingProvider.class, this, name, publicType, configureAction, factory));
    ...
    private void validateNameIsAllowed(String name) {
        if (RESERVED_NAMES_FOR_DETACHED_CONFS.matcher(name).matches()) {
            GradleException ex = new GradleException(String.format(
                    "Creating a configuration with a name that starts with 'detachedConfiguration' is not allowed.  Use a different name for the configuration '%s'",
                    name));
            ProblemId id = ProblemId.create(
                    "name-not-allowed", "Configuration name not allowed", GradleCoreProblemGroup.configurationUsage());
            throw problemsService.getInternalReporter().throwing(ex, id, spec -> {
                spec.contextualLabel(ex.getMessage());
                spec.severity(Severity.ERROR);
            });
    ...
    public class NamedDomainObjectCreatingProvider<I extends Configuration>
            extends AbstractDomainObjectCreatingProvider<I> {

        private final Function<String, I> factory;

        public NamedDomainObjectCreatingProvider(
                String name, Class<I> type, @Nullable Action<? super I> configureAction, Function<String, I> factory) {
