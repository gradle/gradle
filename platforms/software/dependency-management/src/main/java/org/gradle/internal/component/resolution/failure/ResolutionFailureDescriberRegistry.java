...
public final class ResolutionFailureDescriberRegistry {
    ...
    private final LinkedHashMap<Class<? extends ResolutionFailure>, List<ResolutionFailureDescriber<?>>> describers =
            new LinkedHashMap<>();
    ...
    public static ResolutionFailureDescriberRegistry standardRegistry(InstanceGenerator instanceGenerator) {
        ...
        registry.registerDescriber(
                ModuleRejectedFailure.class, ModuleRejectedIncompatibleConstraintsFailureDescriber.class);
        registry.registerDescriber(ModuleRejectedFailure.class, ModuleRejectedFailureDescriber.class);

        // Variant Selection failure
        registry.registerDescriber(
                AmbiguousVariantsFailure.class,
                MissingAttributeAmbiguousVariantsFailureDescriber
                        .class); // Added ahead of AmbiguousVariantsFailureDescriber so the more specific ambiguity case
        // is checked first
        ...
        registry.registerDescriber(
                ConfigurationNotCompatibleFailure.class, ConfigurationNotCompatibleFailureDescriber.class);
        registry.registerDescriber(
                ConfigurationDoesNotExistFailure.class, ConfigurationDoesNotExistFailureDescriber.class);

        // Graph Validation failures
        registry.registerDescriber(
                NoVariantsWithMatchingCapabilitiesFailure.class,
                NoVariantsWithMatchingCapabilitiesFailureDescriber.class);
        ...
        registry.registerDescriber(
                IncompatibleMultipleNodesValidationFailure.class,
                IncompatibleMultipleNodesValidationFailureDescriber.class);
        registry.registerDescriber(
                AmbiguousArtifactTransformsFailure.class, AmbiguousArtifactTransformsFailureDescriber.class);
        registry.registerDescriber(
                UnknownArtifactSelectionFailure.class, UnknownArtifactSelectionFailureDescriber.class);
     ...
     */
    public <FAILURE extends ResolutionFailure> List<ResolutionFailureDescriber<FAILURE>> getDescribers(
        ...
        describers.getOrDefault(failureType, Collections.emptyList()).forEach(d -> {
            ...
            ResolutionFailureDescriber<FAILURE> typedDescriber = (ResolutionFailureDescriber<FAILURE>) d;
     ...
     * @param <FAILURE> The type of failure to describe
     */
    public <FAILURE extends ResolutionFailure> void registerDescriber(
