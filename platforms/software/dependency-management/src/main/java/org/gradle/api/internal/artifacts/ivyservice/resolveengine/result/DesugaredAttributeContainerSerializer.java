...
public class DesugaredAttributeContainerSerializer extends AbstractSerializer<AttributeContainer>
    ...
    public DesugaredAttributeContainerSerializer(
            AttributesFactory attributesFactory, NamedObjectInstantiator instantiator) {
    ...
    public ImmutableAttributes read(Decoder decoder) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (type == BOOLEAN_ATTRIBUTE) {
                attributes =
                        attributesFactory.concat(attributes, Attribute.of(name, Boolean.class), decoder.readBoolean());
                ...
                attributes = attributesFactory.concat(
                        attributes,
                        Attribute.of(name, String.class),
                        new CoercingStringValueSnapshot(value, instantiator));
            }
        }
        return attributes;
    }
        ...
        for (Attribute<?> attribute : container.keySet()) {
            ...
            if (type == Boolean.class) {
                ...
                assert type == String.class
                        : "Unexpected attribute type " + type + " : should be " + String.class.getSimpleName();
