...
public abstract class ModuleRejectedIncompatibleConstraintsFailureDescriber
    ...
    private static final String DEBUGGING_WITH_DEPENDENCY_INSIGHT_PREFIX =
            "Debugging using the " + DEPENDENCY_INSIGHT_TASK_NAME + " report is described in more detail at: ";
    private static final String DEBUGGING_WITH_DEPENDENCY_INSIGHT_ID = "viewing_debugging_dependencies";
    private static final String DEBUGGING_WITH_DEPENDENCY_INSIGHT_SECTION =
    ...
    private List<AssessedSelectionReason> findConflictingConstraints(ModuleRejectedFailure failure) {
        Map<String, List<AssessedSelectionReason>> versionsByReasons =
                failure.getAssessedSelection().getReasons().stream()
    ...
    private List<String> buildResolutions(ModuleRejectedFailure failure) {
        ...
        resolutions.add(DEBUGGING_WITH_DEPENDENCY_INSIGHT_PREFIX
                + getDocumentationRegistry()
                        .getDocumentationFor(
                                DEBUGGING_WITH_DEPENDENCY_INSIGHT_ID, DEBUGGING_WITH_DEPENDENCY_INSIGHT_SECTION)
                + ".");
        return resolutions;
    }
    ...
    private String summarizeFailure(ModuleRejectedFailure failure) {
        ...
        conflictingVersionsWithExplanations.keySet().stream().sorted().forEach(version -> {
            List<String> explanations = conflictingVersionsWithExplanations.get(version).stream()
                    .sorted()
