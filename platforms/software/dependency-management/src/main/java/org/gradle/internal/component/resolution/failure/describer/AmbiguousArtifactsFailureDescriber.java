 ...
 */
public abstract class AmbiguousArtifactsFailureDescriber
        extends AbstractResolutionFailureDescriber<AmbiguousArtifactsFailure> {
    ...
    @Inject
    public AmbiguousArtifactsFailureDescriber(AttributeDescriberRegistry attributeDescribers) {
    ...
    private String buildFailureMsg(AmbiguousArtifactsFailure failure, AttributeDescriber describer) {
        ...
        if (failure.getRequestedAttributes().isEmpty()) {
            formatter.node(
                    "More than one variant of " + failure.describeRequestTarget() + " matches the consumer attributes");
        } else {
            formatter.node("The consumer was configured to find "
                    + describer.describeAttributeSet(
                            failure.getRequestedAttributes().asMap())
                    + ". However we cannot choose between the following variants of "
