...
public class NormalizingExcludeFactory extends DelegatingExcludeFactory {
    ...
    // Simplifies (A ∪ ...) ∩ A = A
    // and  (A ∩ ...) ∪ A = A
    private ExcludeSpec simplify(
            Class<? extends CompositeExclude> clazz,
            ExcludeSpec one,
            ExcludeSpec two,
            BiFunction<ExcludeSpec, ExcludeSpec, ExcludeSpec> orElse) {
    ...
    private ExcludeSpec doUnion(Set<ExcludeSpec> specs) {
        ...
        FlattenOperationResult flattened = flatten(
                ExcludeAnyOf.class, specs, ExcludeEverything.class::isInstance, ExcludeNothing.class::isInstance);
        ...
        Map<UnionOf, List<ExcludeSpec>> byType =
                flattened.result.stream().collect(Collectors.groupingBy(UnionOf::typeOf));
        ...
        if (!moduleIdExcludes.isEmpty()) {
            ...
            if (moduleIdExcludes.size() > 1 || !moduleIdSetsExcludes.isEmpty()) {
                ModuleIdSetExclude excludeSpec = delegate.moduleIdSet(moduleIdExcludes.stream()
                        .map(ModuleIdExclude::getModuleId)
                        .collect(toSet()));
        ...
        if (!groupExcludes.isEmpty()) {
            ...
            if (groupExcludes.size() > 1 || !groupSetExcludes.isEmpty()) {
                GroupSetExclude excludeSpec = delegate.groupSet(
                        groupExcludes.stream().map(GroupExclude::getGroup).collect(toSet()));
        ...
        if (!moduleExcludes.isEmpty()) {
            ...
            if (moduleExcludes.size() > 1 || !moduleSetExcludes.isEmpty()) {
                ModuleSetExclude excludeSpec = delegate.moduleSet(
                        moduleExcludes.stream().map(ModuleExclude::getModule).collect(toSet()));
        ...
        if (moduleIdSetsExcludes.size() > 1) {
            moduleIdSetsExcludes = ImmutableList.of(delegate.moduleIdSet(moduleIdSetsExcludes.stream()
                    .flatMap(e -> e.getModuleIds().stream())
                    .collect(toSet())));
        }
        if (groupSetExcludes.size() > 1) {
            groupSetExcludes = ImmutableList.of(delegate.groupSet(groupSetExcludes.stream()
                    .flatMap(e -> e.getGroups().stream())
                    .collect(toSet())));
        }
        if (moduleSetExcludes.size() > 1) {
            moduleSetExcludes = ImmutableList.of(delegate.moduleSet(moduleSetExcludes.stream()
                    .flatMap(e -> e.getModules().stream())
                    .collect(toSet())));
        }
        ImmutableSet.Builder<ExcludeSpec> builder = ImmutableSet.builderWithExpectedSize(moduleIdExcludes.size()
                + groupExcludes.size()
                + moduleExcludes.size()
                + moduleIdSetsExcludes.size()
                + groupSetExcludes.size()
                + moduleSetExcludes.size()
                + other.size());
     ...
     */
    private <T extends ExcludeSpec> FlattenOperationResult flatten(
            Class<T> flattenType,
            Set<ExcludeSpec> specs,
            Predicate<ExcludeSpec> fastExit,
            Predicate<ExcludeSpec> ignoreSpec) {
    ...
    private FlattenOperationResult filterOnly(Set<ExcludeSpec> specs, Predicate<ExcludeSpec> ignoreSpec) {
        return FlattenOperationResult.of(
                specs.stream().filter(e -> !ignoreSpec.test(e)).collect(toSet()));
    }

    private Stream<ExcludeSpec> maybeFilter(
            Set<ExcludeSpec> specs, Predicate<ExcludeSpec> ignoreSpec, boolean filtered) {
    ...
    private <T extends ExcludeSpec> FlattenOperationResult expensiveFlatten(
            Class<T> flattenType, Stream<ExcludeSpec> stream, int size) {
    ...
    private ExcludeSpec doIntersect(Set<ExcludeSpec> specs) {
        ...
        FlattenOperationResult flattened = flatten(
                ExcludeAllOf.class, specs, ExcludeNothing.class::isInstance, ExcludeEverything.class::isInstance);
        if (flattened.fastExit) {
            return nothing();
        }
