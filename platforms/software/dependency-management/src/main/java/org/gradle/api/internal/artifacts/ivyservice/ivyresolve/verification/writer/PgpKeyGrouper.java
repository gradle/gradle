...
class PgpKeyGrouper {
    ...
    private void processRemainingGroups(Map.Entry<String, Collection<PgpEntry>> e, List<String> groups) {
        ...
        List<PgpEntry> remainingUntouched = e.getValue().stream()
                .filter(p -> p.doesNotDeclareKeyGlobally(keyId))
                .collect(Collectors.toList());
        for (String group : groups) {
            long count = remainingUntouched.stream()
                    .filter(p -> p.getGroup().equals(group))
                    .count();
            if (count > 1) {
                // a key is at least used in 2 artifacts
                verificationsBuilder.addTrustedKey(keyId, group, null, null, null, false);
                remainingUntouched.stream()
                        .filter(p -> p.getGroup().equals(group))
    ...
    private void groupUsingRegex(Map.Entry<String, Collection<PgpEntry>> e, List<String> groups) {
        ...
        for (List<String> prefix : commonPrefixes) {
            ...
            verificationsBuilder.addTrustedKey(e.getKey(), groupRegex, null, null, null, true);
    ...
    private static List<List<String>> samePrefix(int prefixLen, List<String> prefix, List<List<String>> candidates) {
        return candidates.stream()
                .filter(groups -> groups.subList(0, prefixLen).equals(prefix))
                .collect(Collectors.toList());
    }
    ...
    private void groupByGroupOnly(Map.Entry<String, Collection<PgpEntry>> e, List<String> groups) {
        ...
        verificationsBuilder.addTrustedKey(e.getKey(), group, null, null, null, false);
    ...
    private void groupByModuleComponentId(
            Map.Entry<String, Collection<PgpEntry>> e, List<ModuleComponentIdentifier> moduleComponentIds) {
