...
public class JavaEcosystemVariantDerivationStrategy extends AbstractStatelessDerivationStrategy {
    ...
    public ImmutableList<? extends ModuleConfigurationMetadata> derive(ModuleComponentResolveMetadata metadata) {
        if (metadata instanceof DefaultMavenModuleResolveMetadata) {
            ...
            DefaultConfigurationMetadata compileConfiguration =
                    (DefaultConfigurationMetadata) md.getConfiguration("compile");
            DefaultConfigurationMetadata runtimeConfiguration =
                    (DefaultConfigurationMetadata) md.getConfiguration("runtime");
            ...
            return ImmutableList.of(
                    // When deriving variants for the Java ecosystem, we actually have 2 components "mixed together":
                    // the library and the platform
                    // and there's no way to figure out what was the intent when it was published. So we derive variants
                    // for both.
                    ...
                    platformWithUsageAttribute(
                            compileConfiguration, attributes, Usage.JAVA_API, false, shadowedPlatformCapability),
                    platformWithUsageAttribute(
                            runtimeConfiguration, attributes, Usage.JAVA_RUNTIME, false, shadowedPlatformCapability),
                    platformWithUsageAttribute(
                            compileConfiguration, attributes, Usage.JAVA_API, true, shadowedEnforcedPlatformCapability),
                    platformWithUsageAttribute(
                            runtimeConfiguration,
                            attributes,
                            Usage.JAVA_RUNTIME,
                            true,
                            shadowedEnforcedPlatformCapability));
        }
        return null;
    }
     ...
     */
    private DefaultConfigurationMetadata libraryWithSourcesVariant(
            DefaultConfigurationMetadata runtimeConfiguration,
     ...
     */
    private ModuleConfigurationMetadata libraryWithJavadocVariant(
            DefaultConfigurationMetadata runtimeConfiguration,
            ImmutableAttributes originAttributes,
            ModuleComponentResolveMetadata metadata) {
        return runtimeConfiguration
    ...
    private ImmutableCapabilities buildShadowPlatformCapability(
            ModuleComponentIdentifier componentId, boolean enforced) {
        return ImmutableCapabilities.of(new ShadowedImmutableCapability(
    ...
    private ModuleConfigurationMetadata libraryCompileScope(
            DefaultConfigurationMetadata conf, ImmutableAttributes originAttributes) {
    ...
    private ModuleConfigurationMetadata libraryRuntimeScope(
            DefaultConfigurationMetadata conf, ImmutableAttributes originAttributes) {
        ImmutableAttributes attributes = mavenAttributesFactory.runtimeScope(originAttributes);
        return conf.mutate().withAttributes(attributes).withoutConstraints().build();
    }

    private ModuleConfigurationMetadata platformWithUsageAttribute(
            DefaultConfigurationMetadata conf,
            ImmutableAttributes originAttributes,
            String usage,
            boolean enforcedPlatform,
            ImmutableCapabilities shadowedPlatformCapability) {
        ImmutableAttributes attributes =
                mavenAttributesFactory.platformWithUsage(originAttributes, usage, enforcedPlatform);
