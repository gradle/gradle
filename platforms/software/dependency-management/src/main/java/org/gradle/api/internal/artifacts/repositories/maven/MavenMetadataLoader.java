    ...
    public MavenMetadata load(ExternalResourceName metadataLocation) throws ResourceException {
        ...
        try {
            ...
            throw new ResourceException(
                    metadataLocation.getUri(),
                    String.format("Unable to load Maven meta-data from %s.", metadataLocation),
                    e);
        }
        return metadata;
    }

    private void parseMavenMetadataInfo(final ExternalResourceName metadataLocation, final MavenMetadata metadata)
            throws IOException {
        ExternalResource resource = cacheAwareExternalResourceAccessor.getResource(
                metadataLocation,
                null,
                new DefaultResourceFileStore<String>(resourcesFileStore) {
                    @Override
        ...
        if (resource == null) {
            throw new MissingResourceException(
                    metadataLocation.getUri(), String.format("Maven meta-data not available at %s", metadataLocation));
        }
