    ...
    private void traverseGraph(final ResolveState resolveState) {
        ...
        while (resolveState.peek() != null || moduleConflictHandler.hasConflicts() || capabilitiesConflictHandler.hasConflicts()) {
            if (resolveState.peek() != null) {
                ...
                if (!node.isSelected() || node.isInCapabilityConflict() || (node.getComponent().getModule().isInModuleConflict() && !node.isRoot())) {
                    // This node has no incoming edges, or is in conflict and should temporarily not contribute to the graph.
                    // We need special handling for root since it does not yet have its own module, but should never be considered a conflict participant.
