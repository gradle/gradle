...
class ArtifactGraphIntegrationTest extends AbstractHttpDependencyResolutionTest {
    ...
    // TODO: Test for node identity.
    // Every way you get a node should return a node that is == to any other way to get that node.

    // TODO: Verify that an ArtifactGraph-derived value (e.g. ArtifactCollection / FileCollection /
    // Provider) wired as a task input drives parallel artifact resolution through
    // ParallelResolveArtifactSet. This is the canonical consumer path: the task graph triggers
    // resolution, and the worker pool downloads all selected artifacts concurrently. Without this
    // regression guard, consumers calling .get() outside a task action (e.g. the IDE model-builder
    // path) silently lose parallelism — see the parallel-download hack in IdeDependencySet's
    // resolveNodeArtifacts for context.
