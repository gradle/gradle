...
"""
    ...
    def "transform does not execute when dependencies cannot be found"() {
        ...
        then:
        assertTransformationsExecuted()
        failure.assertHasDescription("Execution failed for task ':app:resolveGreen' (created in build file 'build.gradle').") // failure is reported for task that takes the files as input
    ...
    def "transform does not execute when dependencies cannot be built"() {
        ...
        then:
        assertTransformationsExecuted()
        failure.assertHasDescription("Execution failed for task ':common:producer' (created in build file 'build.gradle').")
        failure.assertHasFailures(1)
