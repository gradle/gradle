...
class PublishedRichVersionConstraintsIntegrationTest extends AbstractModuleDependencyResolveTest {
   ...
   Dependency path: 'root' (conf) --> 'org:bar:1.0' (runtime) --> 'org:foo:{strictly 15}' because of the following reason: what not""")
   ...
   Dependency path: 'root' (conf) --> 'org:bar:1.0' (runtime) --> 'org:foo:{require 1.0; reject 1.1}'""")
    }
    ...
    void "honors multiple rejections #rejects using dynamic versions using dependency notation #rejects"() {
        ...
        failure.assertHasCause("""Cannot find a version of 'org:foo' that satisfies the version constraints:
   Dependency path: 'root' (conf) --> 'org:foo:1.1'
   Dependency path: 'root' (conf) --> 'org:bar:1.0' (runtime) --> 'org:foo:{require 1.0; reject ${rejects.join(' & ')}}'""")
   ...
   Dependency path: 'root' (conf) --> 'org:foo:1.0'
   Constraint path: 'root' (conf) --> 'org:bar:1.0' (runtime) --> 'org:foo:{reject all versions}'""")
   ...
   Dependency path: 'root' (conf) --> 'org:a:1.0' (runtime) --> 'org:b:{strictly 1.0}' because of the following reason: Not following semantic versioning
   Dependency path: 'root' (conf) --> 'org:c:1.0' (runtime) --> 'org:b:1.1'""")

        and:
        failure.assertHasNoCause("Dependency path: 'root' (conf) --> 'org:d:1.0' --> 'org:c:1.0' --> 'org:b:1.1'")
    }
