...
class PublishedCapabilitiesIntegrationTest extends AbstractModuleDependencyResolveTest {
   ...
   Cannot select module because of conflict with cglib:cglib:3.2.5 (runtime). Both provide capability 'cglib:cglib:3.2.5'.""")
        failure.assertHasCause("""Module 'cglib:cglib' has been rejected:
   Cannot select module because of conflict with cglib:cglib-nodep:3.2.5 (runtime). Both provide capability 'cglib:cglib:3.2.5'.""")
    }
   ...
   Cannot select module because of conflict with root project 'test' (conf). Both provide capability 'org:capability:1.0'.""")
    }
