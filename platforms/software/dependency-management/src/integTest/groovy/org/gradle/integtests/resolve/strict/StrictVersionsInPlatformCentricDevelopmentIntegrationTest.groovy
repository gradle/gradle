...
class StrictVersionsInPlatformCentricDevelopmentIntegrationTest extends AbstractModuleDependencyResolveTest {
   ...
   Dependency path: 'root' (conf) --> 'org:bar:2.0' (runtime) --> 'org:foo:3.1'
   Constraint path: 'root' (conf) --> 'org:platform:1.1' (${platformVariant}) --> 'org:foo:{strictly 3.1.1; reject 3.1 & 3.2}'
   Constraint path: 'root' (conf) --> 'org:foo:3.2'""")
   ...
   Dependency path: 'root' (conf) --> 'org:bar:2.0' (runtime) --> 'org:foo:3.1'
   Constraint path: 'root' (conf) --> 'org:platform:1.1' (enforcedApiElements) --> 'org:foo:{require 3.1.1; reject 3.1 & 3.2}'
   Constraint path: 'root' (conf) --> 'org:foo:{strictly 3.2}'"""
        } else {
            resolve.expectGraph {
                root(':', ':test:') {
    ...
    void "(5) if two libraries are combined without agreeing on an override, the original platform constraint is brought back [#platformType]"() {
        ...
        if (platformType == ENFORCED_PLATFORM) {
            failure.assertHasCause """Cannot find a version of 'org:foo' that satisfies the version constraints:
   Dependency path: 'root' (conf) --> 'project :recklessLibrary' (conf) --> 'org:bar:2.0' (runtime) --> 'org:foo:3.1'
   Constraint path: 'root' (conf) --> 'project :recklessLibrary' (conf) --> 'org:platform:1.1' (enforcedApiElements) --> 'org:foo:{require 3.1.1; reject 3.1 & 3.2}'
   Constraint path: 'root' (conf) --> 'project :recklessLibrary' (conf) --> 'org:foo:{strictly 3.2}'"""
        } else {
            def platformVariant = platformType == MODULE ? 'runtime' : 'apiElements'
            failure.assertHasCause(
                """Cannot find a version of 'org:foo' that satisfies the version constraints:
   Dependency path: 'root' (conf) --> 'project :recklessLibrary' (conf) --> 'org:bar:2.0' (runtime) --> 'org:foo:3.1'
   Constraint path: 'root' (conf) --> 'project :recklessLibrary' (conf) --> 'org:platform:1.1' (${platformVariant}) --> 'org:foo:{strictly 3.1.1; reject 3.1 & 3.2}'
   Constraint path: 'root' (conf) --> 'project :recklessLibrary' (conf) --> 'org:foo:{strictly 3.2}'""")
        }
