...
class ExternalModuleComponentResolverFactoryTest extends Specification {
    ...
    def "returns an empty resolver when no repositories are configured"() {
        when:
        def resolver = newFactory().createResolvers(Collections.emptyList(), Stub(ComponentMetadataProcessorFactory), Stub(ComponentSelectionRulesInternal), false, Mock(ImmutableCachePolicy), ImmutableAttributes.EMPTY, ImmutableAttributesSchema.EMPTY)
    ...
    def "sets parent resolver with different selection rules when repository is external"() {
        ...
        when:
        def resolver = newFactory().createResolvers(repositories, Stub(ComponentMetadataProcessorFactory), componentSelectionRules, false, Mock(ImmutableCachePolicy), ImmutableAttributes.EMPTY, ImmutableAttributesSchema.EMPTY)
