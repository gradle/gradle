...
public abstract class MavenPublicationErrorChecker extends PublicationErrorChecker {
    ...
    public static void checkThatArtifactIsPublishedUnmodified(
        ...
        for (MavenArtifact mavenArtifact : mainArtifacts) {
            ...
            if (!Strings.nullToEmpty(source.getClassifier())
                    .equals(Strings.nullToEmpty(mavenArtifact.getClassifier()))) {
    ...
    private static String formatDifferences(
            ...
            Path buildDir,
            PublishArtifact source,
            Map<MavenArtifact, Set<ArtifactDifference>> differencesByArtifact) {
    ...
    private static String formatDifferenceSet(
            String projectDisplayName,
            Path buildDir,
            PublishArtifact expected,
            MavenArtifact actual,
            Set<ArtifactDifference> differenceSet) {
        return differenceSet.stream()
                .map(diff -> {
                    switch (diff) {
                        case FILE: {
                            Path expectedFile =
                                    buildDir.relativize(expected.getFile().toPath());
                            Path actualFile =
                                    buildDir.relativize(actual.getFile().toPath());
                            return "\t- file differs (relative to " + projectDisplayName + "): (expected) "
                                    + expectedFile + " != (actual) " + actualFile;
                        }
                        case CLASSIFIER:
                            return "\t- classifier differs: (expected) " + expected.getClassifier() + " != (actual) "
                                    + actual.getClassifier();
                        case EXTENSION:
                            return "\t- extension differs: (expected) " + expected.getExtension() + " != (actual) "
                                    + actual.getExtension();
                        default:
