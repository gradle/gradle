...
public final class BuildInitSpecRegistry {
    ...
    public void register(Class<? extends BuildInitGenerator> generatorType, List<BuildInitSpec> specs) {
        List<BuildInitSpec> currentSpecsForGenerator =
                specsByGeneratorType.computeIfAbsent(generatorType, k -> new ArrayList<>());
        specs.forEach(spec -> {
            doGetGeneratorForSpec(spec).ifPresent(g -> {
                throw new IllegalStateException(String.format(
                        "Spec: '%s' with type: '%s' cannot use same type as another spec already registered!",
                        spec.getDisplayName(), spec.getType()));
            });
            currentSpecsForGenerator.add(spec);
            LOGGER.info(
                    "Loaded project spec: '{} ({})', generated via: '{}'",
                    spec.getDisplayName(),
                    spec.getType(),
    ...
    public BuildInitSpec getSpecByType(String type) {
        ...
        switch (matchingSpecs.size()) {
            case 0:
                throw new IllegalStateException("Build init spec with type: '" + type + "' was not found!"
                        + System.lineSeparator() + "Known types:"
            ...
            default:
                throw new IllegalStateException("Multiple project specs: "
                        + matchingSpecs.stream()
                                .map(BuildInitSpec::getDisplayName)
    ...
    public Class<? extends BuildInitGenerator> getGeneratorForSpec(BuildInitSpec spec) {
        return doGetGeneratorForSpec(spec)
                .orElseThrow(() -> new IllegalStateException("Spec: '" + spec.getDisplayName() + "' with type: '"
                        + spec.getType() + "' is not registered!"));
    }
