...
public final class InitProjectSpecLoader {
     ...
     */
    public InitProjectSpecLoader(Map<String, ClassLoader> classLoaders, Logger logger) {
    ...
    public Map<Class<? extends InitProjectGenerator>, List<InitProjectSpec>> loadProjectSpecs() {
        ...
        for (Map.Entry<String, ClassLoader> entry : classLoaders.entrySet()) {
            ServiceLoader<InitProjectSource> loadedSources = ServiceLoader.load(InitProjectSource.class, entry.getValue());
            ...
            loadedSources.forEach(supplier -> {
                ...
                count.addAndGet(specs.size());
            });

            logger.info("Loaded: " + count.get() + " sources from: '" + entry.getKey() + "' using: " + entry.getValue());
        }
