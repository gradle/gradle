    ...
    public TypeBuilderInternal<T> defaultImplementation(Class<?> implementation) {
        if (this.schema instanceof ManagedImplSchema) {
            throw new InvalidModelException(String.format(
                    "Method annotated with @%s cannot set default implementation for managed type %s.",
                    markerAnnotation.getSimpleName(), schema.getType().getName()));
        }
    ...
    public TypeBuilder<T> internalView(Class<?> internalView) {
        if (internalViews.contains(internalView)) {
            throw new InvalidModelException(
                    String.format("Internal view '%s' must not be specified multiple times.", internalView.getName()));
        }
