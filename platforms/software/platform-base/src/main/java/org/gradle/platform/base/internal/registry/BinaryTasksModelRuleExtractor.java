...
public class BinaryTasksModelRuleExtractor extends AbstractAnnotationDrivenComponentModelRuleExtractor<BinaryTasks> {
    ...
    private static final ModelType<NamedEntityInstantiator<Task>> TASK_FACTORY =
            new ModelType<NamedEntityInstantiator<Task>>() {};
    private static final ModelType<Task> TASK = ModelType.of(Task.class);
    private static final ModelReference<BinaryContainer> BINARIES_CONTAINER =
            ModelReference.of("binaries", ModelType.of(BinaryContainer.class));
    ...
    private static class BinaryTaskRule<T extends BinarySpec> extends ModelMapBasedRule<T, T> {
        ...
        protected void execute(ModelRuleInvoker<?> invoker, final T binary, List<ModelView<?>> inputs) {
            NamedEntityInstantiator<Task> taskFactory =
                    Cast.uncheckedCast(ModelViews.getInstance(inputs.get(0), TASK_FACTORY));
            ModelMap<Task> cast = DomainObjectCollectionBackedModelMap.wrap(
                    "tasks", Task.class, binary.getTasks(), taskFactory, Named.Namer.INSTANCE, new Action<Task>() {
                        @Override
            ...
            context.getRegistry()
                    .configure(
                            ...
                            DirectNodeNoInputsModelAction.of(
                                    BINARIES_CONTAINER, ruleDefinition.getDescriptor(), new Action<MutableModelNode>() {
                                        @Override
