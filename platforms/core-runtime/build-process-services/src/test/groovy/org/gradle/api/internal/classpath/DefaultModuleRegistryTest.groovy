...
class DefaultModuleRegistryTest extends Specification {
    ...
    def "fails when properties does not contain jar file name"() {
        given:
        def propertiesFile = storeProperties(properties(dependencies: ""), "foo", libDir)
        ...
        then:
        IllegalArgumentException e = thrown()
        e.message == "Missing required property 'jarFile' in module properties file '$propertiesFile'."
    }

    def "fails when module does not have jar file"() {
        given:
