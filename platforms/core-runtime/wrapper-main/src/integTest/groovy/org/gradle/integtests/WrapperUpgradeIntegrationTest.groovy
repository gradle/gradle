...
class WrapperUpgradeIntegrationTest extends AbstractWrapperIntegrationSpec {
    ...
    def "can update wrapper to a new version via placeholder #placeholder"() {
        given:
        def version = "9.1.0"
        server.expect(server.get("/versions/$endpoint").send(versionJson(version)))

        when:
        updateWrapperTo(placeholder).run()
    ...
    def "can handle missing endpoint for placeholder"() {
        given:
    ...
    private void assertHasResolution(ExecutionFailure failure) {
        ...
        assert failure.assertHasResolution("Use one of the following dynamic version specifications: 'latest', 'release-candidate', 'milestone', 'release-nightly', 'nightly'.")
    }
}
