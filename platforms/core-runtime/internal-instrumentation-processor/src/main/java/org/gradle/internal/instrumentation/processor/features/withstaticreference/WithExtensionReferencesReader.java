...
public class WithExtensionReferencesReader implements RequestPostProcessorExtension {
    ...
    public Collection<CallInterceptionRequest> postProcessRequest(CallInterceptionRequest originalRequest) {
        if (shouldPostProcess(originalRequest)) {
            originalRequest
                    .getRequestExtras()
                    .getByType(RequestExtra.OriginatingElement.class)
                    .ifPresent(originatingElement -> {
                        ExecutableElement element = originatingElement.getElement();
                        AnnotationUtils.findAnnotationMirror(element, WithExtensionReferences.class)
                                .ifPresent(annotation -> AnnotationUtils.findAnnotationValue(annotation, "toClass")
                                        .ifPresent(value -> {
                                            ...
                                            originalRequest
                                                    .getRequestExtras()
                                                    .add(new WithExtensionReferencesExtra(type, methodName));
                                        }));
                    });
        }
    ...
    private static boolean shouldPostProcess(CallInterceptionRequest request) {
        ...
        return kind == CallableKindInfo.INSTANCE_METHOD
