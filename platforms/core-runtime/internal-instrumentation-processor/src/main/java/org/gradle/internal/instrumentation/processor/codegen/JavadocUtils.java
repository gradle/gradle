...
public class JavadocUtils {
    ...
    public static String callableKindForJavadoc(CallInterceptionRequest request) {
        ...
        return kind == CallableKindInfo.STATIC_METHOD
                ? "static method"
                : kind == CallableKindInfo.INSTANCE_METHOD
                        ? "instance method"
                        : kind == CallableKindInfo.AFTER_CONSTRUCTOR
                                ? "constructor (getting notified after it)"
                                : kind == CallableKindInfo.GROOVY_PROPERTY_GETTER
                                        ? "Groovy property getter"
                                        : kind == CallableKindInfo.GROOVY_PROPERTY_SETTER
    ...
    public static CodeBlock interceptedCallableLink(CallInterceptionRequest request) {
        ...
        CallableInfo interceptedCallable = request.getInterceptedCallable();
        ClassName className =
                ClassName.bestGuess(interceptedCallable.getOwner().getType().getClassName());
        String callableNameForDocComment = interceptedCallable.getKind() == CallableKindInfo.AFTER_CONSTRUCTOR
                ? className.simpleName()
                : interceptedCallable.getCallableName();
        List<ParameterInfo> params = request.getInterceptedCallable().getParameters();
        List<ParameterInfo> methodParameters = params.stream()
                .filter(parameter -> parameter.getKind().isSourceParameter())
    ...
    public static CodeBlock interceptorImplementationLink(CallInterceptionRequest request) {
        ...
        result.add(
                "{@link $T#$L(",
                NameUtil.getClassName(request.getImplementationInfo().getOwner().getClassName()),
                request.getImplementationInfo().getName());
    ...
    private static CodeBlock parameterTypeForJavadoc(ParameterInfo parameterInfo, boolean renderVararg) {
        if (parameterInfo.getKind() == ParameterKindInfo.VARARG_METHOD_PARAMETER && renderVararg) {
            return CodeBlock.of(
                    "$T...", typeName(parameterInfo.getParameterType().getElementType()));
        }
