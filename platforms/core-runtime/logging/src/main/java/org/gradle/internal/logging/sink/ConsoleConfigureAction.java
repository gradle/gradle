...
public class ConsoleConfigureAction {
    ...
    private static Console consoleForStdOut(OutputStream stdout, ConsoleMetaData consoleMetaData, ColorMap colourMap) {
        return consoleFor(stdout, new Supplier<OutputStream>() {
            @Override
            public OutputStream get() {
                return installJansiStream(org.fusesource.jansi.AnsiConsole.out());
            }
        }, consoleMetaData, colourMap);
    }

    private static Console consoleForStdErr(OutputStream stderr, ConsoleMetaData consoleMetaData, ColorMap colourMap) {
        return consoleFor(stderr, new Supplier<OutputStream>() {
            @Override
            public OutputStream get() {
                return installJansiStream(org.fusesource.jansi.AnsiConsole.err());
            }
        }, consoleMetaData, colourMap);
    }
