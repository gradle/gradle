...
.*"""
    ...
    def "outcome for failed build is logged with appropriate styling for log level #level"() {
        ...
        if (level == LogLevel.DEBUG) {
            failure.assertThatDescription(containsString("Execution failed for task ':broken' (created in build file 'build.gradle')."))
        } else {
        ...
        outputWithFailureAndNoDebugging.contains("Execution failed for task ':broken' (created in build file 'build.gradle').")
