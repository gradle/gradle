...
public class TransformReplacer implements Closeable {
    ...
    private Loader createLoaderForDomain(ProtectionDomain domain) {
        ...
        if (transformedJarPath == null) {
            return SKIP_INSTRUMENTATION;
        }
        return transformedJarPath.isFile()
            ? new JarLoader(originalJarPath, transformedJarPath)
            : new DirLoader(originalJarPath, transformedJarPath);
    }
    ...
    private class DirLoader extends Loader {
        ...
        public DirLoader(File originalJarFilePath, File transformedJarFile) {
        ...
        public byte[] loadTransformedClass(String className) throws IOException {
            ...
            File classEntry = new File(jarFilePath, classNameToPath(className));
            ...
            InputStream classBytes = FileUtils.openInputStream(classEntry);
            try {
