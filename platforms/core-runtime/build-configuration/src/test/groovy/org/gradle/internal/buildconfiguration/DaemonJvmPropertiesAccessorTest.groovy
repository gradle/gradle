...
class DaemonJvmPropertiesAccessorTest extends Specification {
    ...
    def "Given valid properties When parse them Then expected values are returned"() {
        ...
        then:
        propertiesAccessor.version == JavaVersion.VERSION_12
    ...
    def "Given invalid vendor property When parse value Then fails with excepted exception message"() {
        given:
        def properties = ["toolchainVendor": "invalid vendor",]

        when:
        def propertiesAccessor = new DaemonJvmPropertiesAccessor(properties)
        propertiesAccessor.vendor

        then:
        def e = thrown(IllegalArgumentException)
        e.message == "Value 'invalid vendor' given for toolchainVendor is an invalid Java vendor. Possible values are " +
