...
public class TransformReplacer implements Closeable {
     ...
     */
    public byte @Nullable [] getInstrumentedClass(
            @Nullable String className, @Nullable ProtectionDomain protectionDomain) {
        // Even though some classes don't need instrumentation, trying to query the closed TransformReplacer is still an
        // error.
        ensureOpened();

        if (className == null || protectionDomain == null) {
            // JVM allows to define a class with "null" name through Unsafe. LambdaMetafactory in Java 8 defines a SAM
            // implementation for method handle this way.
    ...
    private Loader getLoader(ProtectionDomain domain) {
        ...
        if (transformLoader == null) {
            ...
            if (closed) {
                ...
                // The transformLoader might be inserted into the loaders map after close(), so let's close it for sure
                // to
                // avoid leaks.
    ...
    private Loader createLoaderForDomain(ProtectionDomain domain) {
        ...
        if (transformedPath == null) {
            ...
            return new JarLoader(
                    Objects.requireNonNull(originalPath, "Must not be null because transformed path isn't"),
                    transformedPath);
    ...
    private class JarLoader extends Loader {
        ...
        public synchronized byte @Nullable [] loadTransformedClass(String className) throws IOException {
            ...
            if (classEntry == null) {
                // This can happen if the class was "injected" into the classloader, e.g. when decorated class is
                // generated by the ObjectFactory.
            ...
            if (jarFile == null) {
                ...
                if (jarFile.isMultiRelease() && !isTransformed(jarFile.getJarFile())) {
                    throw new GradleException(String.format(
                            ...
                            originalPath.getAbsolutePath(), jarFile.javaRuntimeVersionUsed()));
                }
            }
    ...
    private static class DirectoryLoader extends Loader {
        ...
        public byte @Nullable [] loadTransformedClass(String className) throws IOException {
            ...
            if (!classFile.exists()) {
                ...
                // generated by the ObjectFactory.
                // Injected classes reuse the protection domain. See ClassLoaderUtils.define and defineDecorator.
                // TODO(mlopatkin) we don't do any kind of integrity checks for the directory. If the user deletes
                // transformed classes but not the receipt
