...
class DaemonToolchainDownloadIntegrationTest extends AbstractIntegrationSpec implements DaemonJvmPropertiesFixture, JavaToolchainFixture {
    ...
    def "toolchain selection that requires downloading fails when it is disabled"() {
        def javaVersion = AvailableJavaHomes.differentVersion.javaVersion
        given:
    ...
    def "toolchain downloaded is checked against the spec"() {
        ...
        then:
        failure.assertHasDescription("Unable to download toolchain matching the requirements ({languageVersion=${otherJavaVersion.majorVersion}, vendor=any vendor, implementation=vendor-specific}) from '$uri', " +
            "due to: Toolchain provisioned from '$uri' doesn't satisfy the specification: {languageVersion=${otherJavaVersion.majorVersion}, vendor=any vendor, implementation=vendor-specific}")
    }
