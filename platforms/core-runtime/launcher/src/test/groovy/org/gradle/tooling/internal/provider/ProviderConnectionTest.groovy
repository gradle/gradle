...
class ProviderConnectionTest extends Specification {
    ...
    def "ignores unknown operation types"() {
        ...
        when:
        def configuration = ProviderConnection.ProgressListenerConfiguration.from(parameters, GradleVersion.version("12.7"), payloadSerializer, isolatableSerializerRegistry)
    ...
    def "adds specific types when generic type requested by old consumer versions"() {
        ...
        when:
        def configuration = ProviderConnection.ProgressListenerConfiguration.from(parameters, GradleVersion.version("5.0"), payloadSerializer, isolatableSerializerRegistry)
