...
class ProviderStartParameterConverter {
    ...
    private List<TaskExecutionRequest> unpack(final List<InternalLaunchable> launchables) {
        // Important that the launchables are unpacked on the client side, to avoid sending back any additional internal
        // state that
        // the launchable may hold onto. For example, GradleTask implementations hold onto every task for every project
        // in the build
        ...
        for (InternalLaunchable launchable : launchables) {
            if (launchable instanceof TaskExecutionRequest) {
                ...
                TaskExecutionRequest launchableImpl = DefaultTaskExecutionRequest.of(
                        originalLaunchable.getArgs(),
                        originalLaunchable.getProjectPath(),
                        originalLaunchable.getRootDir());
                requests.add(launchableImpl);
            } else {
                throw new InternalUnsupportedBuildArgumentException("Problem with provided launchable arguments: "
    ...
    public StartParameterInternal toStartParameter(
            ProviderOperationParameters parameters, BuildLayoutResult buildLayout, AllProperties properties) {
        // Important that this is constructed on the client so that it has the right gradleHomeDir and other state
        // internally
