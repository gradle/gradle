...
This page describes the approach to nullability of types in the Gradle Build Tool codebase.
...
1. **Use `Objects.requireNonNull()`** - For cases where you know a value is non-null but NullAway cannot verify it. With a small performance penalty, we get fail-fast behavior at runtime:
    ...
    ```
2. **Use `Cast.unsafeStripNullable`** - For cases where you know the value conforms to the type but NullAway cannot infer, mostly happens with creative generic usage. Use sparingly.
