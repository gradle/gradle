...
Instead of declaring the repository in every affected test, register it once in `EXTRA_REPOSITORIES` in [RepoScriptBlockUtil](../testing/internal-distribution-testing/src/main/groovy/org/gradle/integtests/fixtures/RepoScriptBlockUtil.groovy):
...
Add the URL to the `MirroredRepository` enum in the same file too, so that CI can mirror it.
...
- An init script, passed to every build started through a `GradleExecuter` or a smoke test runner.
  It adds the repository to the settings `pluginManagement` and `dependencyResolutionManagement` blocks, and to buildscript and project repositories that already contain one — adding a repository to a project that declares none would stop that project from using the settings repositories.
...
While a repository is active, tests asserting on the repositories a build used can fail, e.g. those checking a plugin resolution failure message or a resolve build operation.
