...
public class DeclarativeDslDependencyTest {
    ...
    public static final ArchRule schema_types_do_not_have_binary_dependencies_on_kotlin_stdlib =
            ...
            classes()
                    .that()
                    .resideInAPackage("org.gradle.declarative.dsl.schema")
                    .or()
                    .resideInAPackage("org.gradle.declarative.dsl.evaluation")
                    .should()
                    .onlyDependOnClassesThat()
                    .haveNameNotMatching("kotlin\\.(?!Metadata$).*");

    @ArchTest
    public static final ArchRule evaluator_classes_do_not_depend_on_generic_gradle_code = classes()
            .that()
            .haveNameMatching("org\\.gradle\\.internal\\.declarativedsl\\.evaluator.*")
            .should()
            .onlyDependOnClassesThat(or(not(new GradleCode()), new DeclarativeGradleCode()))
            .because(
                    "The `declarative-dsl-evaluator` module is meant to be used outside the context of Gradle, so it should not depend on Gradle API/internals.");
}
