...
public interface ArchUnitFixture {
    DescribedPredicate<JavaClass> classes_not_written_in_kotlin = not(annotatedOrInPackageAnnotatedWith(
                    kotlin.Metadata.class))
            .and(resideOutsideOfPackages(
                    "org.gradle.kotlin..")) // a few relocated kotlinx-metadata classes violate the nullability
            // annotation rules
    ...
    DescribedPredicate<JavaMember> not_written_in_kotlin =
            declaredIn(classes_not_written_in_kotlin).as("written in Java or Groovy");

    DescribedPredicate<JavaMember> not_from_fileevents =
            declaredIn(resideOutsideOfPackages("org.gradle.fileevents..")).as("not from fileevents");

    DescribedPredicate<JavaClass> not_from_fileevents_classes =
            resideOutsideOfPackages("org.gradle.fileevents..").as("not from fileevents");

    DescribedPredicate<JavaMember> kotlin_internal_methods = declaredIn(gradlePublicApi())
            .and(not(not_written_in_kotlin))
            .and(modifier(PUBLIC))
            .and(nameMatching(
                    ".+\\$[a-z_]+")) // Kotlin internal methods have `$kotlin_module_name` appended to their name
    ...
    static DescribedPredicate<JavaClass> gradleMaintainedExternalDependency() {
        return resideInAnyPackage("net.rubygrapefruit..", "org.gradle.fileevents..")
        ...
        return new ArchCondition<JavaClass>(" override method " + getMethodDescription(name, parameterTypes)) {
            ...
            public void check(JavaClass javaClass, ConditionEvents events) {
                ...
                events.add(new SimpleConditionEvent(
                        javaClass,
                        false,
                        javaClass.getFullName() + " doesn't override default method "
                                + getMethodDescription(name, parameterTypes)));
            }
        ...
        return new ArchCondition<JavaMethod>("use org.jspecify.annotations.Nullable") {
            ...
            public void check(JavaMethod method, ConditionEvents events) {
                ...
                if (!method.isAnnotatedWith(Nullable.class)
                        && (reflected == null
                                || reflected.getAnnotatedReturnType().getAnnotation(Nullable.class) == null)) {
                    ...
                    if (!nullableAnnotations.isEmpty()) {
                        events.add(new SimpleConditionEvent(
                                method,
                                false,
                                method.getFullName() + " is using forbidden Nullable annotations: "
                                        + nullableAnnotations.stream()
                                                .map(a -> a.getType().getName())
                                                .collect(Collectors.joining(","))));
                    } else {
                        events.add(new SimpleConditionEvent(
                                method, true, method.getFullName() + " is not using forbidden Nullable"));
                    }
                }
                ...
                for (int idx = 0; idx < method.getParameters().size(); idx++) {
                    ...
                    if (!parameter.isAnnotatedWith(Nullable.class)
                            && (reflected == null
                                    || reflected.getAnnotatedParameterTypes()[idx].getAnnotation(Nullable.class)
                                            == null)) {
                        ...
                        if (!nullableAnnotations.isEmpty()) {
                            events.add(new SimpleConditionEvent(
                                    method,
                                    false,
                                    "parameter " + parameter.getIndex() + " for " + method.getFullName()
                                            + " is using forbidden Nullable annotations: "
                                            + nullableAnnotations.stream()
                                                    .map(a -> a.getType().getName())
                                                    .collect(Collectors.joining(","))));
                        } else {
            ...
            private <T extends HasDescription> Set<JavaAnnotation<T>> extractPossibleNullable(
                    Set<JavaAnnotation<T>> annotations) {
                return annotations.stream()
                        .filter(annotation -> annotation.getType().getName().endsWith("Nullable"))
                        .collect(toSet());
            }
        };
    }

    static DescribedPredicate<JavaMember> annotatedMaybeInSupertypeWith(
    ...
    static DescribedPredicate<JavaMember> annotatedMaybeInSupertypeWith(final String annotationTypeName) {
        DescribedPredicate<HasType> typeNameMatches =
                GET_RAW_TYPE.then(GET_NAME).is(equalTo(annotationTypeName));
    ...
    class HaveGradleTypeEquivalent extends ArchCondition<JavaMethod> {
        ...
        public void check(JavaMethod method, ConditionEvents events) {
            if (method.isAnnotatedWith(Deprecated.class)) {
                ...
                events.add(
                        new SimpleConditionEvent(method, true, method.getDescription() + " is deprecated, skipping"));
                return;
            }
            ...
            if (!parameters.isEmpty()) {
                ...
                if (lastParameter.getRawType().isEquivalentTo(Closure.class)) {
                    ...
                    if (similarMethods.stream().noneMatch(m -> {
                        ...
                        return last.getRawType().isEquivalentTo(Action.class)
                                || last.getRawType().isEquivalentTo(Spec.class)
                                || last.getRawType().isEquivalentTo(Transformer.class);
                    })) {
                        // missing
                        String message = String.format(
                                "%s has Closure but does not have equivalent Gradle type method in %s",
        ...
        private static List<JavaMethod> findSimilarMethods(JavaMethod method) {
            // This is taking a shortcut and assuming that we do not have multiple methods with the same name and number
            // of parameters taking Closure
    ...
    class HaveOnlyArgumentsOrReturnTypesThatAre extends ArchCondition<JavaMethod> {
        ...
        public void check(JavaMethod method, ConditionEvents events) {
            ...
            String message = fulfilled
                    ? String.format(
                            "%s has only arguments/return type that are %s in %s",
                            method.getDescription(), types.getDescription(), method.getSourceCodeLocation())
                    : String.format(
                            "%s has arguments/return type %s that %s not %s in %s",
        ...
        private void unpackJavaType(JavaType type, Set<JavaClass> referencedTypes, Set<JavaType> visited) {
            ...
            if (type instanceof JavaTypeVariable) {
                ...
                ((JavaParameterizedType) type)
                        ...
                        .forEach(argument -> unpackJavaType(argument, referencedTypes, visited));
            }
        }
    }

    class InGradlePublicApiPackages extends DescribedPredicate<JavaClass> {
        private static final PackageMatchers INCLUDES =
                PackageMatchers.of(parsePackageMatcher(System.getProperty("org.gradle.public.api.includes")));
        private static final PackageMatchers EXCLUDES =
                PackageMatchers.of(parsePackageMatcher(System.getProperty("org.gradle.public.api.excludes")));
    ...
    class GradlePublicApi extends DescribedPredicate<JavaClass> {
        ...
        @Override
        public boolean test(JavaClass input) {
            return packages.test(input)
                    && !TEST_FIXTURES.test(input)
                    && input.getModifiers().contains(JavaModifier.PUBLIC);
        }
    }
    ...
    class HaveDirectSuperclassOrInterfaceThatAre extends ArchCondition<JavaClass> {
        ...
        public void check(JavaClass item, ConditionEvents events) {
            Optional<JavaClass> matchingSuperclass = item.getRawSuperclass().filter(types);
            Stream<JavaClass> matchingInterfaces =
                    item.getRawInterfaces().stream().filter(types);
            List<String> implementedClasses = Stream.concat(
                            matchingSuperclass.map(Stream::of).orElse(Stream.empty()), matchingInterfaces)
                    .map(JavaClass::getName)
                    .collect(Collectors.toList());
            ...
            String message = String.format(
                    "%s extends/implements %s that %s %s in %s",
                    item.getDescription(),
                    classesDescription,
                    verb,
                    types.getDescription(),
    ...
    class AnnotatedMaybeInSupertypePredicate extends DescribedPredicate<JavaMember> {
        ...
        public boolean test(JavaMember input) {
            ...
            return ownerAndSupertypes.anyMatch(classInHierarchy -> findMatchingCallableMember(classInHierarchy, input)
        ...
        private Optional<? extends JavaMember> findMatchingCallableMember(JavaClass owner, JavaMember memberToFind) {
            ...
            if (memberToFind instanceof JavaMethod) {
                String[] parameterFqNames = ((JavaMethod) memberToFind)
        ...
        public boolean test(JavaClass input) {
            try {
                ...
                return clazz.getAnnotation(annotationType) != null
                        || clazz.getPackage().getAnnotation(annotationType) != null;
            } catch (NoClassDefFoundError e) {
                // Fall back to ArchUnit query
                return input.isAnnotatedWith(annotationType)
                        || input.getPackage().isAnnotatedWith(annotationType);
            }
        }
    }
