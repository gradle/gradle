...
public class PropertyUsageArchitectureTest {
    ...
    private static final DescribedPredicate<JavaMethod> hasRichPropertyReturnType =
            new DescribedPredicate<JavaMethod>("hasRichPropertyReturnType") {
                @Override
    ...
    private static final DescribedPredicate<JavaMethod> rich_task_property_getters = ArchPredicates.<JavaMethod>are(
                    declaredIn(assignableTo(Task.class)))
    ...
    @ArchTest
    public static final ArchRule task_implementations_should_define_properties_as_abstract_getters =
            freeze(methods().that(are(rich_task_property_getters)).should(beAbstractMethod()));
}
