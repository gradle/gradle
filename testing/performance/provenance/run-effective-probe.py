...
"""Run the bounded S3 binding, replace, copy, finalization and retention probes against the S0 distribution."""
import argparse
import hashlib
import json
import os
...
with tempfile.TemporaryDirectory(prefix='shared-provenance-probe-') as temporary:
    ...
    run(['javac', '-cp', baseline_classpath, '-d', baseline_classes, source / 'ProvenanceAllocationProbe.java', source / 'EffectiveBaselineProbe.java'])
    ...
    run(['javac', '-cp', new_classpath, '-d', metadata_classes, source / 'EffectiveEnabledProbe.java'])
