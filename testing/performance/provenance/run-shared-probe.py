...
"""Run bounded independent JVM allocation/layout probes against a preserved baseline distribution."""
import argparse
import hashlib
import json
import os
...
with tempfile.TemporaryDirectory(prefix='shared-provenance-probe-') as temporary:
    ...
    run(['javac', '-cp', baseline_classpath, '-d', baseline_classes, source / 'ProvenanceAllocationProbe.java'])
    ...
    new_classpath = os.pathsep.join([str(main), str(baseline_classes), baseline_classpath])
    run(['javac', '-cp', new_classpath, '-d', metadata_classes, source / 'SharedMetadataAllocationProbe.java'])
    # Rotation avoids giving every variant the same machine-load position; JVMs never share JIT state.
    variants = ['baseline', 'compiled-in', 'metadata']
    for fork in range(3):
        for variant in variants[fork:] + variants[:fork]:
            ...
            entry = 'ProvenanceAllocationProbe'
            if variant == 'metadata':
                classpath = os.pathsep.join([str(metadata_classes), classpath])
                entry = 'SharedMetadataAllocationProbe'
