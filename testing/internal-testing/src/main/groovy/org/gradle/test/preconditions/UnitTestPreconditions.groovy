...
class UnitTestPreconditions {
    ...
    static final class UnsupportedDaemonJdkVersion implements TestPrecondition {
        @Override
        boolean isSatisfied() {
            return JavaVersion.current() < SupportedJavaVersions.MINIMUM_JAVA_VERSION
        }
    }
    ...
    static final class DeprecatedDaemonJdkVersion implements TestPrecondition {
        @Override
        boolean isSatisfied() {
            return (JavaVersion.current() < SupportedJavaVersions.FUTURE_MINIMUM_JAVA_VERSION) &&
                (JavaVersion.current() >= SupportedJavaVersions.MINIMUM_JAVA_VERSION)
        }
    }
