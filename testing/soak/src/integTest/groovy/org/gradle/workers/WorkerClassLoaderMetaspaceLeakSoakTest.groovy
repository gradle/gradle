...
class WorkerClassLoaderMetaspaceLeakSoakTest extends AbstractIntegrationSpec {
    ...
    def "worker classLoaderIsolation does not leak class metadata across items within a single build session"() {
        ...
        // loader mid-build and its steady state stays a small fraction of it. Calibrated + verified in this repo:
        // FIXED passes in ~18s; the pre-fix baseline exhausts Metaspace (OOM, or a GC-thrash timeout under the cap).
        ...
        // Append to the fixture's standard TestWorkAction (WorkerTask defaults workActionClass to it), and write it
        // + its parameter types to buildSrc via the fixture helper.
        ...
        // metadata copies accumulate under the cap and the daemon exhausts Metaspace (a strong ref, so NOT freed by
        // the JVM's pre-OOM last-ditch soft-ref GC -- the trip-wire genuinely discriminates fixed from unfixed).
        succeeds("loadAll")
