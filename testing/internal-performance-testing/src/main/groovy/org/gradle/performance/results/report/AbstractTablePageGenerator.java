...
public abstract class AbstractTablePageGenerator extends HtmlPageGenerator<ResultsStore> {
    private static final Set<String> DEPENDENCY_PERFORMANCE_TEST_TEAM_CITY_BUILD_IDS =
            getDependencyPerformanceTestTeamCityBuildIds();
    ...
    public AbstractTablePageGenerator(
    ...
    protected abstract class TableHtml extends MetricsHtml {
        ...
        // @formatter:off
        {
            ...
            renderTable(
                    ...
                    "Compare the performance of the same build on different code versions.",
                    determineBaseline(),
                    getCrossVersionScenarios());
            renderTable(
                    "Cross build scenarios",
                    "Compare the performance of different builds",
                    Optional.empty(),
                    getCrossBuildScenarios());
            renderPopoverDiv();
            end();
            footer(this);
            endAll();
        ...
        private void renderTableHeader() {
            ...
            a().classAttr("btn btn-sm btn-outline-primary")
                    .attr("data-toggle", "tooltip")
                    .title("Go back to Performance Coordinator Build")
                    .href(getTeamCityWebUrlFromBuildId(System.getenv("BUILD_ID")))
                    .target("_blank")
                    .text("<-")
                    .end();
            end();
            ...
            text(getTableTitle());
            a().target("_blank")
                    .href("https://github.com/gradle/gradle/commits/" + executionDataProvider.getCommitId())
                    .small()
                    .classAttr("text-muted")
                    .text(executionDataProvider.getCommitId())
                    .end()
                    .end();
            end();
            div().classAttr("col-3 p-0");
            if (renderFailureSelectButton()) {
                button().id("failed-scenarios")
                        .classAttr("btn-sm btn-danger")
                        .text("Failed scenarios")
                        .end();
                button().id("all-scenarios")
                        .classAttr("btn-sm btn-primary")
                        .text("All scenarios")
                        .end();
            }
            end();
            div().classAttr("col text-right mt-1");
            i().classAttr("fa fa-filter")
                    .attr("data-toggle", "popover", "data-placement", "bottom")
                    .title("Filter by tag")
            ...
            div().classAttr("col p-0")
                    ...
                    .title(
                            "The difference between two series of execution data (usually baseline vs current Gradle), positive numbers indicate current Gradle is slower, and vice versa.")
                    .text("Difference");
            i().classAttr("fa fa-info-circle").text(" ").end().end();
            div().classAttr("col p-0")
                    .attr("data-toggle", "tooltip")
                    .style("font-size: smaller")
                    .title(
                            "The confidence with which these two data series are different. E.g. 90% means they're different with 90% confidence. Currently we fail the test if the confidence > 99.9%.")
                    .text("Confidence");
            i().classAttr("fa fa-info-circle").text(" ").end().end();
            end();
        }
        ...
        private void renderPopoverDiv() {
            ...
            Stream.of(FixedTag.values()).forEach(tag -> {
                ...
                input().classAttr("form-check-input")
                        .type("checkbox")
                        .checked("true")
                        .value(tag.getName())
                        .end();
        ...
        private void renderTable(
            ...
            i().classAttr("fa fa-info-circle")
                    .attr("data-toggle", "tooltip")
                    .title(description)
                    .text(" ")
                    .end();
            baselineVersion.ifPresent(
                    version -> b().text("(vs " + version + ")").end());
            end();
            end();
            scenarios.forEach(scenario -> renderScenario(counter.incrementAndGet(), scenario));
        }

        private String getTextColorCss(
                PerformanceReportScenario scenario, PerformanceReportScenarioHistoryExecution executionData) {
        ...
        private void renderScenario(int index, PerformanceReportScenario scenario) {
            ...
            div().classAttr("card m-0 p-0 alert " + determineScenarioBackgroundColorCss(scenario))
                    .attr("tag", tags.stream().map(Tag::getName).collect(joining(",")))
                    .id("scenario" + index);
            ...
            div().classAttr("col")
                    ...
                    .a()
                    .attr("data-toggle", "tooltip")
                    .classAttr("section-sign")
                    .title("Click to copy url of this scenario to clipboard")
                    .href("#scenario" + index)
                    .style("opacity:0")
                    .id("section-sign-" + index)
                    .text("§");
            end();
            end();
            div().classAttr("col-6");
            ...
            a().target("_blank")
                    ...
                    .href("tests/" + urlEncode(PerformanceTestHistory.convertToId(scenario.getName()) + ".html"))
                    .text("Graph")
                    .end();
            a().classAttr("btn btn-primary btn-sm collapsed")
                    .href("#")
                    .attr("data-toggle", "collapse", "data-target", "#collapse" + index)
                    .text("Detail")
                    .end();
            end();
            ...
            if (scenario.isBuildFailed()) {
                ...
                scenario.getCurrentExecutions().forEach(execution -> {
                    ...
                    div().classAttr("p-0 col " + getTextColorCss(scenario, execution))
                            .text(execution.getDifferenceDisplay())
                            .end();
                    div().classAttr("p-0 col " + getTextColorCss(scenario, execution))
                            .text(execution.getFormattedConfidence())
                            .end();
                    end();
                });
            }
            ...
            if (scenario.isBuildFailed()) {
                pre().text(scenario.getTeamCityExecutions().stream()
                                .map(PerformanceTestExecutionResult::getTestFailure)
                                .collect(joining("\n")))
                        .end();
            } else {
        ...
        private void renderTag(Tag tag) {
            if (tag.getUrl() == null) {
                span().classAttr(tag.getClassAttr())
                        .attr("data-toggle", "tooltip")
                        .title(tag.getTitle())
                        .text(tag.getName())
                        .end();
        ...
        private void renderDetailsTable(PerformanceReportScenario scenario) {
            ...
            th().text("Commit").end();
            renderVersionHeader(
                    scenario.getHistoryExecutions().isEmpty()
                            ? ""
                            : scenario.getHistoryExecutions()
                                    .get(0)
                                    .getBaseVersion()
                                    .getName());
            renderVersionHeader(
            ...
            scenario.getHistoryExecutions().forEach(execution -> {
                ...
                DataSeries<Duration> currentVersion =
                        execution.getCurrentVersion().getTotalTime();
                td().text(FormatSupport.timestamp(execution.getTime())).end();
                td().a()
                        .target("_blank")
                        .href("https://github.com/gradle/gradle/commits/" + execution.getShortCommitId())
                        .text(execution.getShortCommitId())
                        .end()
                        .end();
                td().classAttr(
                                baseVersion.getMedian().compareTo(currentVersion.getMedian()) < 0
                                        ? "text-success"
                                        : "text-danger")
                        .text(baseVersion.getMedian().format())
                        .end();
                td().classAttr("text-muted")
                        .text("se: " + baseVersion.getStandardError().format())
                        .end();
                td().classAttr(
                                baseVersion.getMedian().compareTo(currentVersion.getMedian()) >= 0
                                        ? "text-success"
                                        : "text-danger")
                        .text(currentVersion.getMedian().format())
                        .end();
                td().classAttr("text-muted")
                        .text("se: " + currentVersion.getStandardError().format())
                        .end();
                td().classAttr(getTextColorCss(scenario, execution))
                        .text(execution.getFormattedDifferencePercentage())
                        .end();
                td().classAttr(getTextColorCss(scenario, execution))
                        .text(execution.getFormattedConfidence())
                        .end();
    ...
    private Optional<String> determineBaseline() {
        return executionDataProvider.getReportScenarios().stream()
                ...
                .filter(execution ->
                        DEPENDENCY_PERFORMANCE_TEST_TEAM_CITY_BUILD_IDS.contains(execution.getTeamCityBuildId()))
                .findFirst()
