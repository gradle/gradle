...
class FlakinessDetectionPerformanceExecutionDataProvider extends PerformanceExecutionDataProvider {
    ...
    private static final Comparator<PerformanceReportScenario> SCENARIO_COMPARATOR = comparing(
                    PerformanceReportScenario::isBuildFailed)
            .reversed()
            ...
            .thenComparing(comparing(
                            org.gradle.performance.results.report.FlakinessDetectionPerformanceExecutionDataProvider
                                    ::isFlaky)
                    .reversed())
            .thenComparing(comparing(PerformanceReportScenario::getDifferencePercentage)
                    .reversed())
            .thenComparing(PerformanceReportScenario::getName);

    public FlakinessDetectionPerformanceExecutionDataProvider(
    ...
    protected TreeSet<PerformanceReportScenario> queryExecutionData(List<PerformanceTestExecutionResult> scenarioList) {
        Set<PerformanceTestExecutionResult> distinctScenarios = scenarioList.stream()
                .collect(treeSetCollector(comparing(PerformanceTestExecutionResult::getPerformanceExperiment)));

        return distinctScenarios.stream().map(this::queryExecutionData).collect(treeSetCollector(SCENARIO_COMPARATOR));
    }

    private PerformanceReportScenario queryExecutionData(PerformanceTestExecutionResult execution) {
        PerformanceTestHistory history = resultsStore.getTestResults(
                execution.getPerformanceExperiment(),
                MOST_RECENT_EXECUTIONS,
                PERFORMANCE_DATE_RETRIEVE_DAYS,
                ResultsStoreHelper.determineChannel(),
                ImmutableList.of());
        List<? extends PerformanceTestExecution> executionsOfSameCommit = history.getExecutions().stream()
                .filter(e -> e.getVcsCommits().contains(commitId))
                .collect(toList());
    ...
    public static boolean isFlaky(PerformanceReportScenario scenario) {
        return scenario.getCurrentExecutions().stream()
                .anyMatch(execution -> execution.getConfidencePercentage() > FLAKINESS_DETECTION_THRESHOLD);
    }
}
