...
public abstract class AbstractReportGenerator<R extends ResultsStore> {
    ...
    public static class FailureCollector {
        ...
        private static String renderScenarioSummary(String kind, PerformanceReportScenario scenario) {
            ...
            // The result JSONs no longer carry build IDs; report the builds of this pipeline that actually measured
            // this scenario, i.e. its current executions from the performance database.
            List<String> buildIds = scenario.getCurrentExecutions().stream()
