...
public class BaseCrossBuildResultsStore<R extends CrossBuildPerformanceResults>
        extends AbstractWritableResultsStore<R> {
    ...
    private void batchInsertOperation(Connection connection, R results, long executionId) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement(
                "insert into testOperation(testExecution, displayName, tasks, args, gradleOpts, daemon, totalTime, cleanTasks) values (?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    private long insertExecution(Connection connection, R results) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement(
                "insert into testExecution(testClass, testId, testProject, startTime, endTime, versionUnderTest, operatingSystem, jvm, vcsBranch, vcsCommit, testGroup, resultType, channel, host, teamCityBuildId) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    private void insertExecutionExperiment(Connection connection, R results) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement(
                "insert into testExecutionExperiment(testId, testProject, testClass, resultType) values (?, ?, ?, ?)")) {
    ...
    private void addOperations(
            ...
            long executionId,
            BuildDisplayInfo displayInfo,
            MeasuredOperationList operations)
            throws SQLException {
        for (MeasuredOperation operation : operations) {
            ...
            statement.setBigDecimal(
                    7, operation.getTotalTime().toUnits(Duration.MILLI_SECONDS).getValue());
    ...
    public List<PerformanceExperiment> getPerformanceExperiments() {
        return withConnection("load test history", connection -> {
            ...
            try (PreparedStatement testIdsStatement = connection.prepareStatement(
                    "select testClass, testId, testProject" + "   from testExecutionExperiment"
                            + "  where resultType = ?"
                ...
                try (ResultSet testExecutions = testIdsStatement.executeQuery()) {
                    while (testExecutions.next()) {
                        ...
                        if (testProject != null && testClass != null) {
                            testNames.add(new PerformanceExperiment(
                                    testProject, new PerformanceScenario(testClass, testName)));
                        }
                    }
    ...
    @Override
    public CrossBuildPerformanceTestHistory getTestResults(
            final PerformanceExperiment experiment,
            final int mostRecentN,
        ...
        return withConnection("load results", connection -> {
            ...
            try (PreparedStatement executionsForName = connection.prepareStatement(
                            "select id, startTime, endTime, versionUnderTest, operatingSystem, jvm, vcsBranch, vcsCommit, testGroup, channel, host, teamCityBuildId from testExecution where testClass = ? and testId = ? and testProject = ? and startTime >= ? and ("
                    ...
                    PreparedStatement operationsForExecution = connection.prepareStatement(
                            "select displayName, tasks, args, gradleOpts, daemon, totalTime, cleanTasks from testOperation where testExecution = ?")) {
                int idx = 0;
                ...
                try (ResultSet testExecutions = executionsForName.executeQuery()) {
                    ...
                    Set<BuildDisplayInfo> builds =
                            Sets.newTreeSet(Comparator.comparing(BuildDisplayInfo::getDisplayName));
                    while (testExecutions.next()) {
                        ...
                        performanceResults.setVcsBranch(
                                testExecutions.getString(7).trim());
