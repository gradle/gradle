...
public interface Tag {
    ...
    enum FixedTag implements Tag {
        FROM_CACHE(
                "FROM-CACHE",
                "badge badge-info",
                "The test is not really executed - its results are fetched from build cache."),
        ...
        IMPROVED(
                "IMPROVED",
                "badge badge-success",
                "Improvement confidence > 90%, rebaseline it to keep this improvement! :-)"),
        UNKNOWN("UNKNOWN", "badge badge-dark", "The status is unknown, may be it's cancelled?"),
        FLAKY(
                "FLAKY",
                "badge badge-danger",
                "The scenario's difference confidence > 95% even when running identical code."),
    ...
    class FlakinessInfoTag implements Tag {
        private static final String FLAKINESS_RATE_TITLE =
        ...
        private static final String FAILURE_THRESHOLD_TITLE =
        ...
        static FlakinessInfoTag createFailureThresholdTag(BigDecimal threshold) {
            return new FlakinessInfoTag(
                    String.format("FAILURE-THRESHOLD(%.2f%%)", threshold.doubleValue() * 100), FAILURE_THRESHOLD_TITLE);
        }
