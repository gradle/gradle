...
enum AuthScheme {
    ...
    private static class BasicAuthHandler extends AuthSchemeHandler {
        ...
        boolean containsUnexpectedAuthentication(HttpRequest request) {
            return !hideUnauthorized && request.getHeader("Authorization") != null
        }
    }
    ...
    private static class DigestAuthenticator implements Authenticator {
        ...
        private final String nonceSecret = new BigInteger(128, random).toString(16)
        ...
        private void challenge(HttpResponse response) {
            String salt = String.format("%032x", new BigInteger(128, random))
            String nonce = salt + sign(salt)
            response.setHeader("WWW-Authenticate", "Digest realm=\"" + realm.name + "\", qop=\"auth\", nonce=\"" + nonce + "\", opaque=\"" + md5("opaque") + "\"")
            response.sendError(401)
        }

        private boolean isNonceIssuedByUs(String nonce) {
            if (nonce.length() <= SALT_LENGTH) {
                return false
            }
            String salt = nonce.substring(0, SALT_LENGTH)
            return sign(salt) == nonce.substring(SALT_LENGTH)
        }

        private String sign(String salt) {
            PrimitiveHasher hasher = Hashing.sha256().newPrimitiveHasher()
            hasher.putString(nonceSecret)
            hasher.putString(salt)
