...
class NtlmAuthenticator implements AuthScheme.Authenticator {
    ...
    boolean authenticate(HttpRequest request, HttpResponse response) {
        ...
        if (header == null || !header.startsWith("NTLM ")) {
            if (authenticatedConnections.contains(connection)) {
                return true
            }
        ...
        switch (messageType(token)) {
            case TYPE_1_NEGOTIATE:
                return respondToNegotiate(connection, token, response)
            case TYPE_3_AUTHENTICATE:
                return respondToAuthenticate(connection, token, response)
            default:
                return unauthorized(response)
        }
    }

    private boolean respondToNegotiate(String connection, byte[] type1Token, HttpResponse response) {
        ...
        return unauthorized(response, "NTLM " + type2Token(type1Token, serverChallenge))
    }

    private boolean respondToAuthenticate(String connection, byte[] type3Token, HttpResponse response) {
        byte[] serverChallenge = challenges.remove(connection)
        if (serverChallenge == null) {
            return unauthorized(response)
        }
        if (passwordMatches(type3Token, serverChallenge)) {
