...
/// When the distribution under test bundles Groovy 5, expect this test (or all tests in this spec)
/// to fail. The specific failure is not asserted; an unexpected success fails the test.
///
/// Set [#skipBecause()] to skip instead of expecting failure (e.g. flaky, hangs).
@Retention(RetentionPolicy.RUNTIME)
@Target({ElementType.TYPE, ElementType.METHOD})
@ExtensionAnnotation(GradleModeTestingExtension.ToBeFixedForG5.class)
public @interface ToBeFixedForGroovy5 {

    /// Why this test is expected to fail with Groovy 5.
