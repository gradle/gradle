...
class TestFilesCleanupServiceTest {
        ...
        projectDir.resolve("build.gradle.kts").writeText(
            ...
            fun Project.configureTestWithLeftover(ignoreFailures: Boolean, hasLeftover: Boolean) {
                ...
                tasks.named<Test>("test").configure {
                    ...
                    if (hasLeftover) {
                        doFirst {
                            project.layout.buildDirectory.file("tmp/teșt files/leftover/leftover").get().asFile.apply {
                                parentFile.mkdirs()
                                createNewFile()
                            }
                        }
            ...
            open class TestWithLeftover: AbstractTestTask() {
                ...
                override fun executeTests() {
                    ...
                    if (project.name == "failed-report-with-leftover") {
                        project.touchInBuildDir("tmp/teșt files/leftover/leftover")
    ...
    fun `fail build if leftover file found and test passes`() {
        ...
        result.output.assertContains("successful-test-with-leftover/build/tmp/teșt files/leftover")

        assertLeftoverFilesCleanedUpEventually("successful-test-with-leftover/build/tmp/teșt files")
    }
    ...
    fun `leftover files are archived when test fails`() {
        ...
        assertEquals(1, StringUtils.countMatches(result.output, "Found non-empty test files dir"))
        assertEquals(1, StringUtils.countMatches(result.output, "Failed to stop service 'testFilesCleanupBuildService'"))
        result.output.assertContains("failed-report-with-leftover/build/tmp/teșt files/leftover")
        result.output.assertContains("failed-test-with-leftover/build/tmp/teșt files/leftover")
        ...
        assertLeftoverFilesCleanedUpEventually(
            "failed-report-with-leftover/build/tmp/teșt files",
            "successful-report/build/tmp/teșt files",
            "failed-test-with-leftover/build/tmp/teșt files"
        )
    }
    ...
    fun `build does not fail if a flaky test has leftover files`() {
        ...
        result.output.assertContains("flaky-test-with-leftover/build/tmp/teșt files/leftover")
        ...
        assertLeftoverFilesCleanedUpEventually("flaky-test-with-leftover/build/tmp/teșt files")
    }
