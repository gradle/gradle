...
import gradlebuild.packaging.tasks.GenerateClasspathModuleProperties.ModuleAlias
 ...
 * The properties file describes the artifact's dependencies, so its runtime classpath
...
abstract class GenerateClasspathModuleProperties : DefaultTask() {
    ...
    fun generate() {
        ...
        val modulesByName: Map<String, Pair<ModuleNode, ModuleArtifact>> = artifacts.get().associate { artifact ->
            ...
            require(node != null) {
                "Could not determine find node for artifact ${artifact.id}"
            }
            node.name to Pair(node, artifact)
        }

        modulesByName.forEach { (name, pair) ->
            val (properties, artifact) = pair
            val moduleDependencyNames = properties.dependencyComponentIds.mapNotNull {
                val dependencyProperties = nodesByComponentId[it]
                require(dependencyProperties != null) {
                    "Could not determine module dependencies for ${artifact.id}"
                }
                val targetModuleName = dependencyProperties.name
            ...
            generateProperties(
                ...
                outputDirectory.file(properties.name + ".properties").asFile
            )
        }
    }
