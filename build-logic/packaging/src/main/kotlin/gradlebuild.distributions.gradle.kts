...
fun generateModulePropertiesFor(moduleJar: TaskProvider<Jar>): TaskProvider<GenerateEmptyModuleProperties> {
    return tasks.register<GenerateEmptyModuleProperties>(moduleJar.name + "ModuleProperties") {
        ...
        outputFile = moduleJar.flatMap { it.archiveBaseName }.flatMap { generatedPropertiesFileFor(it) }
    }
}
...
generateModulePropertiesFor(runtimeApiInfoJar)
generateModulePropertiesFor(gradleApiKotlinExtensionsJar)

val additionalDistributionJars = listOf(
    runtimeApiInfoJar,
    gradleApiKotlinExtensionsJar,
)

// A standard Java runtime variant for embedded integration testing
consumableVariant("runtime", listOf(coreRuntimeOnly, pluginsRuntimeOnly), additionalDistributionJars) {
    configureAsRuntimeElements(objects)
}

consumableVariant("api", listOf(coreRuntimeOnly, pluginsRuntimeOnly), additionalDistributionJars) {
