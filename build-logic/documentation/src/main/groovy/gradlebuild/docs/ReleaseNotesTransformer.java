...
public class ReleaseNotesTransformer extends FilterReader {
    ...
    private Reader transform(Reader in) throws IOException {
        ...
        document.head()
                ...
                .append(
                        "<link rel='stylesheet' type='text/css' href='https://assets.gradle.com/lato/css/lato-font.css'/>");
    ...
    private void cleanUpIssueLinks(Document document) {
        ...
        rewritten = rewritten.replaceAll(
                "(gradle/[a-zA-Z\\-_]+)#(\\d+)", "<a href=\"https://github.com/$1/issues/$2\">$0</a>");
    ...
    private void handleVideos(Document document) {
        ...
        // E.g. @youtube(Summary,UN0AFCLASZA?si=9aG5tDzj6nL1_IKT&start=371)@ =>
        // https://www.youtube.com/embed/UN0AFCLASZA?si=9aG5tDzj6nL1_IKT&amp;start=371"
    ...
    private void addTOC(Document document) {
        Element tocSection = document.body()
                .select("section.topic")
                .first()
                .before("<section class='table-of-contents'/>")
        ...
        for (Element h23element : h23elements) {
            ...
            if (!name.startsWith("Table") && tag.equals("h2")) {
                toc.append("<li class=\"mainTopic\"><a/></li>")
                        .children()
                        .last()
                        .select("a")
                        .first()
                ...
                toc.append("<li class=\"subTopic\"><a/></li>")
                        .children()
                        .last()
                        .select("a")
                        .first()
