...
class DependencyAnalyzer {
    ...
    private fun parseDependencies(className: String, location: File, knownClasses: Set<String>): ClassDependencies {
        ...
        val visitor = ClassDependenciesVisitor(knownClasses)
    ...
    private class ClassDependenciesVisitor(private val knownClasses: Set<String>) : ClassVisitor(Opcodes.ASM9) {
        ...
        private fun maybeAddConstantDynamic(types: MutableSet<String>, constantDynamic: ConstantDynamic) {
            maybeAddType(types, constantDynamic.descriptor)
            maybeAddType(types, Type.getObjectType(constantDynamic.bootstrapMethod.owner))
            maybeAddType(types, constantDynamic.bootstrapMethod.desc)
            for (i in 0 until constantDynamic.bootstrapMethodArgumentCount) {
                val arg = constantDynamic.getBootstrapMethodArgument(i)
                if (arg is Type) {
                    maybeAddType(types, arg)
                } else if (arg is Handle) {
                    maybeAddType(types, Type.getObjectType(arg.owner))
                    maybeAddType(types, arg.desc)
                } else if (arg is ConstantDynamic) {
                    maybeAddConstantDynamic(types, arg)
            ...
            return object : MethodVisitor(Opcodes.ASM9) {
                ...
                override fun visitLdcInsn(value: Any) {
                    if (value is Type) {
                        ...
                        maybeAddConstantDynamic(implTypes, value)
                ...
                override fun visitInvokeDynamicInsn(
                    ...
                    for (arg in bootstrapMethodArguments) {
                        if (arg is Type) {
                            ...
                            maybeAddConstantDynamic(implTypes, arg)
                        } else if (arg is String) {
                            val normalized = arg.replace('/', '.')
                            if (knownClasses.contains(normalized)) {
                                maybeAddType(implTypes, Type.getObjectType(normalized.replace('.', '/')))
