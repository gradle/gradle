...
File[] fileList(String property) {
    ...
    static void createOutput(String gradleHome, File tutorialDir, File outputDir) {
        ...
        scripts().each {entry ->
            ...
            String output = Executer.execute(gradleHome, tutorialDir.absolutePath, [taskName], "${entry.key}file")
            output = ">gradle -q -b${entry.key}file $taskName$NL" + output
            new File(outputDir, "${entry.key}file.out").write(output)
        }
    }
