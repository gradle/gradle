...
class UpdateReleasedVersionsTest extends Specification {
    ...
    def "final release is added to list"() {
        ...
        def version = new ReleasedVersion('4.5', '20170913122310+0000')

        expect:
        updateReleasedVersions(version, versions) == releasedVersions(snapshot, rc, [version])
    }
    ...
    def "final releases are sorted by version"() {
        ...
        updateReleasedVersions(version, versions) == releasedVersions(snapshot, rc, expectedVersions)
    }
    ...
    def "newer snapshots are stored"() {
        ...
        expect:
        updateReleasedVersions(newSnapshot, versions) == releasedVersions(newSnapshot, rc, [])
    ...
    def "older snapshots are not stored"() {
        ...
        def newSnapshot = snapshot(version, referenceBuildTime + buildTime)
        expect:
        updateReleasedVersions(newSnapshot, versions) == releasedVersions(oldSnapshot, rc, [])
    ...
    def "newer rcs are stored"() {
        ...
        expect:
        updateReleasedVersions(newRc, versions) == releasedVersions(snapshotVersion, newRc, [])
    ...
    def "older rcs are not stored"() {
        ...
        def newRc = releasedVersion(version, referenceBuildTime + buildTime)
        expect:
        updateReleasedVersions(newRc, versions) == releasedVersions(snapshotVersion, oldRc, [])
    ...
    Map releasedVersions(ReleasedVersion snapshot, ReleasedVersion rc, List<ReleasedVersion> versions) {
