...
open class GradleDistributionsPlugin : Plugin<Project> {
    ...
    private
    fun Project.startScriptResolver(defaultDependency: String) =
        configurations.creating {
            attributes {
            ...
            isVisible = false
            withDependencies {
                add(this@startScriptResolver.dependencies.create(project(defaultDependency)))
            }
        }
    ...
    private
    fun Project.docsResolver(defaultDependency: String) =
        configurations.creating {
            attributes {
                attribute(Usage.USAGE_ATTRIBUTE, objects.named(Usage.JAVA_RUNTIME))
                attribute(Category.CATEGORY_ATTRIBUTE, objects.named(Category.DOCUMENTATION))
            ...
            isVisible = false
            withDependencies {
                add(this@docsResolver.dependencies.create(project(defaultDependency)))
            }
        }
