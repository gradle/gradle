...
import org.gradle.script.lang.kotlin.provider.splitIncluding
    ...
    companion object {
        ...
        fun of(paths: Iterable<List<String>>): CompactTree =
            paths
                ...
                .map { (label, remaining) ->
                    val subTree = of(remaining)
                    when (subTree) {
                        is Empty  -> Label(label)
                        is Label  -> Label(label + subTree.label)
                        is Branch -> Edge(Label(label), subTree)
                        is Edge   -> Edge(Label(label + subTree.label), subTree.tree)
                    }
                }.let {
                    when (it.size) {
                        0    -> Empty
