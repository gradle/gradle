...
class DefaultModuleRegistryTest extends Specification {
    ...
    def "does not add wrong resources dirs to classpath of 'gradle-resources' module"() {
        ...
        if (cpFiles.size() == 1) {
            ...
            assert cpFiles[0].path.contains("/resources/build/libs/gradle-resources".replace('/', File.separator)) // jar
        } else {
            assert cpFiles.size() == 3
            // folders - IDE test execution
            assert cpFiles[0].path.endsWith("/resources/build/generated-resources/main".replace('/', File.separator))
