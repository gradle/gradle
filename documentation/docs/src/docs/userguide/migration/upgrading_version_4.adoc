...
Large projects may have to increase this setting using the <<build_environment.adoc#sec:configuring_jvm_memory,`org.gradle.jvmargs`>> property.
...
Large projects may have to increase this setting on the relevant tasks, e.g. <<build_environment.adoc#sec:configuring_jvm_memory,`JavaCompile`>> or link:{groovyDslPath}/org.gradle.api.tasks.testing.Test.html[`Test`].
